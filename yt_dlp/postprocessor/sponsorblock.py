...
class SponsorBlockPP(FFmpegPostProcessor):
    ...
    def __init__(
        self, downloader, categories=None, api="https://sponsor.ajay.app"
    ):
    ...
    def run(self, info):
        ...
        info["sponsorblock_chapters"] = self._get_sponsor_chapters(
            info, info.get("duration")
        )
        return [], info

    def _get_sponsor_chapters(self, info, duration):
        segments = self._get_sponsor_segments(
            info["id"], self.EXTRACTORS[info["extractor_key"]]
        )
        ...
        def duration_filter(s):
            ...
            diff = (
                abs(duration - s["videoDuration"]) if s["videoDuration"] else 0
            )
            return diff < 1 or (
                diff < 5 and diff / (start_end[1] - start_end[0]) < 0.05
            )
        ...
        def to_chapter(s):
            ...
            title = (
                s["description"] if cat == "chapter" else self.CATEGORIES[cat]
            )
            return {
                "start_time": start,
                "end_time": end,
    ...
    def _get_sponsor_segments(self, video_id, service):
        ...
        url = (
            f"{self._API_URL}/api/skipSegments/{video_hash[:4]}?"
