...
class PostProcessor(metaclass=PostProcessorMetaClass):
    ...
    def deprecation_warning(self, msg):
        warn = getattr(
            self._downloader, "deprecation_warning", deprecation_warning
        )
        ...
        def decorator(func):
            ...
            def wrapper(self, info):
                if not simulated and (
                    self.get_param("simulate")
                    or self.get_param("skip_download")
                ):
                    return [], info
                format_type = (
                    "video"
                    if info.get("vcodec") != "none"
                    else "audio" if info.get("acodec") != "none" else "images"
                )
    ...
    def report_progress(self, s):
        ...
        if tmpl:
            self._downloader.to_screen(
                self._downloader.evaluate_outtmpl(tmpl, progress_dict),
                quiet=False,
            )

        self._downloader.to_console_title(
            self._downloader.evaluate_outtmpl(
                progress_template.get("postprocess-title")
    ...
    def _download_json(self, url, *, expected_http_errors=(404,)):
        ...
        for retry in RetryManager(
            self.get_param("extractor_retries", 3), self._retry_download
        ):
            try:
                ...
                if (
                    ...
                    and e.status in expected_http_errors
                ):
                    return None
                ...
                continue
        return json.loads(
            rsp.read().decode(rsp.headers.get_param("charset") or "utf-8")
        )
