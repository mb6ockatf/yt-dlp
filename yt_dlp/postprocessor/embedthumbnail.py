...
class EmbedThumbnailPP(FFmpegPostProcessor):
    ...
    def _get_thumbnail_resolution(self, filename, thumbnail_dict):
        def guess():
            width, height = thumbnail_dict.get("width"), thumbnail_dict.get(
                "height"
            )
            if width and height:
                return width, height
        ...
        try:
            ...
            size_result = self.run_ffmpeg(
                filename, None, ["-hide_banner"], expected_retcodes=(1,)
            )
            ...
            self.report_warning(
                f"unable to find the thumbnail resolution; {err}"
            )
            return guess()
    ...
    def run(self, info):
        ...
        idx = next(
            (
                -i
                for i, t in enumerate(info["thumbnails"][::-1], 1)
                if t.get("filepath")
            ),
            None,
        )
        ...
        if info["ext"] not in ("mkv", "mka") and thumbnail_ext not in (
            "jpg",
            "jpeg",
            "png",
        ):
        ...
        if info["ext"] == "mp3":
            ...
            prefer_atomicparsley = (
                "embed-thumbnail-atomicparsley"
                in self.get_param("compat_opts", [])
            )
            ...
            if not mutagen or prefer_atomicparsley:
                ...
                try:
                    ...
                    meta.tags["covr"] = [
                        MP4Cover(data=thumb_data, imageformat=f[type_])
                    ]
                    meta.save()
                    temp_filename = filename
                except Exception as err:
                    self.report_warning(
                        f"unable to embed using mutagen; {err}"
                    )
                    success = False
            ...
            if not success:
                ...
                if atomicparsley is None:
                    ...
                    self.write_debug(
                        f"AtomicParsley command line: {shell_quote(cmd)}"
                    )
            ...
            if not success:
                ...
                try:
                    ...
                    if old_stream is not None:
                        ...
                        new_stream -= 1
                    options.extend(
                        [f"-disposition:{new_stream}", "attached_pic"]
                    )
            ...
            self._report_run("mutagen", filename)
            f = {"opus": OggOpus, "flac": FLAC, "ogg": OggVorbis}[info["ext"]](
                filename
            )
            ...
            res = self._get_thumbnail_resolution(
                thumbnail_filename, info["thumbnails"][idx]
            )
            if res is not None:
        ...
        self._delete_downloaded_files(
            (
                thumbnail_filename
                if converted or not self._already_have_thumbnail
                else None
            ),
            (
                original_thumbnail
                if converted and not self._already_have_thumbnail
                else None
            ),
            info=info,
        )
