...
class ModifyChaptersPP(FFmpegPostProcessor):
    def __init__(
        ...
        *,
        ...
        self._remove_sponsor_segments = set(
            remove_sponsor_segments or []
        ...
        sponsor_chapters.extend(
            {
                ...
                "_categories": [
                    ("manually_removed", start, end, "Manually removed")
                ],
                "remove": True,
            }
        ...
        while chapters:
            ...
            # (normal, sponsor) and (sponsor, sponsor)
                ...
                if cur_chapter["end_time"] > c["end_time"]:
                    ...
                    after_c = dict(
                        copy.deepcopy(cur_chapter), start_time=c["end_time"]
                    )
                    heapq.heappush(
                        chapters, (after_c["start_time"], cur_i, after_c)
                    )
                ...
                if "_categories" in cur_chapter:
                    c["_categories"] = (
                        cur_chapter["_categories"] + c["_categories"]
                    )
    ...
    def remove_chapters(
        ...
        if force_keyframes:
            in_file = self.force_keyframes(
                ...
                (
                    t
                    for c in ranges_to_cut
                    for t in (c["start_time"], c["end_time"])
                ),
            )
