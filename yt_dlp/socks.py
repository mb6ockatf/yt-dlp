...
class InvalidVersionError(ProxyError):
    def __init__(self, expected_version, got_version):
        msg = (
            f"Invalid response version from server. Expected {expected_version:02x} got "
            f"{got_version:02x}"
        )
...
class sockssocket(socket.socket):
    ...
    def setproxy(
        self, proxytype, addr, port, rdns=True, username=None, password=None
    ):
        assert proxytype in (
            ProxyType.SOCKS4,
            ProxyType.SOCKS4A,
    ...
    def _setup_socks4(self, address, is_4a=False):
        ...
        _, ipaddr = self._resolve_address(
            ...
            use_remote_dns=is_4a,
            family=socket.AF_INET,
        )

        packet = (
            struct.pack(
                "!BBH", SOCKS4_VERSION, Socks4Command.CMD_CONNECT, port
            )
            + ipaddr
        )
    ...
    def _socks5_auth(self):
        ...
        if method == Socks5Auth.AUTH_NO_ACCEPTABLE or (
            ...
            and (not self._proxy.username or not self._proxy.password)
        ):
            self.close()
        ...
        if method == Socks5Auth.AUTH_USER_PASS:
            ...
            packet += self._len_and_data(username) + self._len_and_data(
                password
            )
            self.sendall(packet)
    ...
    def _setup_socks5(self, address):
        ...
        reserved = 0
        packet = struct.pack(
            "!BBB", SOCKS5_VERSION, Socks5Command.CMD_CONNECT, reserved
        )
        if ipaddr is None:
