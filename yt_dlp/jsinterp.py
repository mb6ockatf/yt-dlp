...
class Debugger:
    ...
    def wrap_interpreter(cls, f):
        def interpret_statement(
            self, stmt, local_vars, allow_recursion, *args, **kwargs
        ):
            ...
            try:
                ...
                if cls.ENABLED:
                    if isinstance(e, ExtractorError):
                        e = e.orig_msg
                    cls.write(
                        "=> Raises:", e, "<-|", stmt, level=allow_recursion
                    )
                raise
            if cls.ENABLED and stmt.strip():
...
class JSInterpreter:
    ...
    def _separate(expr, delim=",", max_split=None):
        ...
        in_quote, escaping, after_op, in_regex_char_group = (
            None,
            False,
            True,
            False,
        ...
        for idx, char in enumerate(expr):
            if not in_quote and char in _MATCHING_PARENS:
                ...
                if char in _QUOTES and in_quote in (char, None):
                    if in_quote or after_op or char != "/":
                        in_quote = (
                            None
                            if in_quote and not in_regex_char_group
                            else char
                        )
            ...
            in_unary_op = (
                ...
                and after_op not in (True, False)
                and char in "-+"
            )
            after_op = (
                char
                if (not in_quote and char in OP_CHARS)
                else (char.isspace() and after_op)
            )
    ...
    def _operator(
        self, op, left_val, right_expr, expr, local_vars, allow_recursion
    ):
        if op in ("||", "&&"):
        ...
        try:
            ...
            raise self.Exception(
                f"Failed to evaluate {left_val!r} {op} {right_val!r}",
                expr,
                cause=e,
            )
    ...
    def interpret_statement(self, stmt, local_vars, allow_recursion=100):
        ...
        m = re.match(
            r'(?P<var>(?:var|const|let)\s)|return(?:\s+|(?=["\'])|$)|(?P<throw>throw\s+)',
            stmt,
        )
        if m:
        ...
        if expr[0] in _QUOTES:
            ...
            if expr[0] == "/":
                ...
                inner = json.loads(
                    js_to_json(f"{inner}{expr[0]}", strict=True)
                )
            if not outer:
        ...
        if expr.startswith("new "):
            ...
            if obj.startswith("Date("):
                ...
                date = unified_timestamp(
                    self.interpret_expression(
                        left, local_vars, allow_recursion
                    ),
                    False,
                )
                if date is None:
        ...
        if expr.startswith("{"):
            ...
            if all(len(sub_expr) == 2 for sub_expr in sub_expressions):
                ...
                def dict_item(key, val):
                    ...
                    return (
                        self.interpret_expression(
                            key, local_vars, allow_recursion
                        ),
                        val,
                    )

                return (
                    dict(dict_item(k, v) for k, v in sub_expressions),
                    should_return,
                )
        ...
        if expr.startswith("["):
            ...
            name = self._named_object(
                ...
                [
                    self.interpret_expression(
                        item, local_vars, allow_recursion
                    )
        ...
        if md.get("if"):
            ...
            cndn = _js_ternary(
                self.interpret_expression(cndn, local_vars, allow_recursion)
            )
        ...
        if md.get("try"):
            ...
            pending = (None, False)
            m = re.match(rf"catch\s*(?P<err>\(\s*{_NAME_RE}\s*\))?\{{", expr)
            if m:
                sub_expr, expr = self._separate_at_paren(expr[m.end() - 1 :])
                if err:
                    catch_vars = {}
                    if m.group("err"):
                        catch_vars[m.group("err")] = (
            ...
            body, expr = self._separate_at_paren(remaining, "}")
            items = body.replace("default:", "case default:").split("case ")[
                1:
            ]
            for default in (False, True):
                ...
                for item in items:
                    case, stmt = (
                        i.strip() for i in self._separate(item, ":", 1)
                    )
                    if default:
        ...
        if m and m.group("assign"):
            ...
            if not m.group("index"):
                ...
                raise self.Exception(
                    f'Cannot index undefined variable {m.group("out")}', expr
                )
        ...
        for op in _OPERATORS:
            ...
            while True:
                if (
                    op in "?<>*-"
                    and len(separated) > 1
                    and not separated[-1].strip()
                ):
                    separated.pop()
                elif not (
                    separated and op == "?" and right_expr.startswith(".")
                ):
                    break
            ...
            return (
                self._operator(
                    op, left_val, right_expr, expr, local_vars, allow_recursion
                ),
                should_return,
            )
        ...
        if m and m.group("attribute"):
            ...
            def eval_method():
                ...
                if (variable, member) == ("console", "debug"):
                    if Debugger.ENABLED:
                        Debugger.write(
                            self.interpret_expression(
                                f"[{arg_str}]", local_vars, allow_recursion
                            )
                        )
                    return
                ...
                if isinstance(obj, type) and member.startswith("prototype."):
                    ...
                    assertion(
                        isinstance(argvals[0], obj),
                        f"needs binding to type {obj}",
                    )
                    if func_prototype == "call":
                        ...
                        obj, argvals = argvals
                        assertion(
                            isinstance(argvals, list),
                            "second argument needs to be a list",
                        )
                    else:
                        raise self.Exception(
                            f"Unsupported Function method {func_prototype}",
                            expr,
                        )
                    member = new_member
                ...
                if member == "split":
                    ...
                    assertion(
                        isinstance(obj, list), "must be applied on a list"
                    )
                    ...
                    assertion(
                        len(argvals) <= 2, "takes between 0 and 2 arguments"
                    )
                    ...
                    f, this = ([*argvals, ""])[:2]
                    return [
                        f((item, idx, obj), {"this": this}, allow_recursion)
                    ...
                    assertion(
                        isinstance(obj, str), "must be applied on a string"
                    )
                    assertion(len(argvals) == 1, "takes exactly one argument")
            ...
            if fname in local_vars:
                return (
                    local_vars[fname](
                        argvals, allow_recursion=allow_recursion
                    ),
                    should_return,
                )
    ...
    def extract_object(self, objname):
        ...
        for f in fields_m:
            ...
            obj[name] = function_with_repr(
                self.build_function(argnames, f.group("code")), f"F<{name}>"
            )
    ...
    def build_function(self, argnames, code, *global_stack):
        ...
        def resf(args, kwargs={}, allow_recursion=100):
            global_stack[0].update(
                itertools.zip_longest(argnames, args, fillvalue=None)
            )
