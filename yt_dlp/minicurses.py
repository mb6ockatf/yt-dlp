...
def format_text(text, f):
    ...
    if "ON" in tokens:
        ...
        if tokens[-1] != "ON":
            raise SyntaxError(
                f'Invalid format {f.split(" ON ", 1)[1]!r} in {f!r}'
            )
    ...
    if not tokens:
        ...
        if tokens and tokens[-1] == "LIGHT":
            ...
            tokens.pop()
        fg_style = (
            tokens.pop() if tokens and tokens[-1] in _TEXT_STYLES else "NORMAL"
        )
    ...
    if fg_color or bg_color:
        text = text.replace(
            CONTROL_SEQUENCES["RESET"], f"{fg_color}{bg_color}"
        )
...
class MultilinePrinter(MultilinePrinterBase):
    ...
    def print_at_line(self, text, pos):
        if self._HAVE_FULLCAP:
            self.write(
                *self._move_cursor(pos), CONTROL_SEQUENCES["ERASE_LINE"], text
            )
            return
        ...
        if self._HAVE_FULLCAP:
            self.write(
                *text,
                ...
                f'{CONTROL_SEQUENCES["UP"]}{CONTROL_SEQUENCES["ERASE_LINE"]}'
                * self.maximum,
            )
        else:
