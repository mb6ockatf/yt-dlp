...
if sys.version_info < (3, 9):
    raise ImportError(
        f"You are using an unsupported version of Python. Only Python versions 3.9 and above are supported by yt-dlp"
    )  # noqa: F541
...
def print_extractor_information(opts, urls):
    ...
    if opts.list_extractors:
        ...
        for ie in list_extractor_classes(opts.age_limit):
            out += (
                ie.IE_NAME
                + (" (CURRENTLY BROKEN)" if not ie.working() else "")
                + "\n"
            )
            if ie == GenericIE:
                matched_urls = [
                    url for url, matched in urls.items() if not matched
                ]
            else:
...
def validate_options(opts):
    def validate(cndn, name, value=None, msg=None):
        if cndn:
            return True
        raise ValueError(
            (msg or 'invalid {name} "{value}" given').format(
    ...
    validate_positive(
        "concurrent fragments", opts.concurrent_fragment_downloads, True
    )
    ...
    # Time ranges
    validate_positive(
        "subtitles sleep interval", opts.sleep_interval_subtitles
    )
    ...
    if opts.sleep_interval is None:
        ...
        validate_minmax(
            opts.sleep_interval, opts.max_sleep_interval, "sleep interval"
        )
    ...
    validate_regex(
        "audio format", opts.audioformat, FFmpegExtractAudioPP.FORMAT_RE
    )
    validate_in(
        "subtitle format",
    ...
    validate_regex(
        "recode video format",
        opts.recodevideo,
        FFmpegVideoConvertorPP.FORMAT_RE,
    )
    validate_regex(
    ...
    if opts.audioquality:
        ...
        validate_positive(
            "audio quality",
            int_or_none(float_or_none(opts.audioquality), default=0),
        )
    ...
    opts.file_access_retries = parse_retries(
        "file access", opts.file_access_retries
    )
    ...
    def parse_sleep_func(expr):
        ...
        if op == "exp":
            ...
            return lambda n: min(
                float(start) + float(step or default_step) * n,
                float(limit or "inf"),
            )
    ...
    opts.http_chunk_size = validate_bytes(
        "http chunk size", opts.http_chunk_size
    )
    ...
    validate_outtmpl(
        opts.sponsorblock_chapter_title, "SponsorBlock chapter title"
    )
    ...
    def parse_chapters(name, value, advanced=False):
        parse_timestamp = lambda x: (
            float("inf") if x in ("inf", "infinite") else parse_duration(x)
        )
        TIMESTAMP_RE = r"""(?x)(?:
        ...
        for regex in value or []:
            ...
            for range_ in map(str.strip, regex[1:].split(",")):
                ...
                if err:
                    raise ValueError(
                        f'invalid {name} time range "{regex}". {err}'
                    )
                ranges.append(dur)
    ...
    def metadataparser_actions(f):
        if isinstance(f, str):
            ...
            actions = (
                (MetadataParserPP.Actions.REPLACE, x, *f[1:])
                for x in f[0].split(",")
            )
    ...
    if opts.playlist_items is not None:
        try:
            ...
            raise ValueError(
                f"Invalid playlist-items {opts.playlist_items!r}: {err}"
            )
    ...
    opts.match_filter = match_filter_func(
        opts.match_filter, opts.breaking_match_filter
    )
    ...
    report_conflict(
        "--datebefore", "datebefore", "--date", "date", default=None
    )
    report_conflict(
    ...
    report_conflict(
        ...
        "--output",
        "outtmpl",
        val2=opts.outtmpl.get("default"),
    )
    report_conflict(
        "--remux-video", "remuxvideo", "--recode-video", "recodevideo"
    )
    report_conflict(
        "--sponskrub", "sponskrub", "--remove-chapters", "remove_chapters"
    )
    report_conflict(
    ...
    report_conflict(
        "--sponsorblock-remove", "sponsorblock_remove", default=set()
    )
    ...
    report_deprecation(
        opts.sponskrub,
        "--sponskrub",
        "--sponsorblock-mark or --sponsorblock-remove",
    )
    ...
    if (
        opts.addmetadata or opts.sponsorblock_mark
    ) and opts.addchapters is None:
    ...
    if opts.allsubtitles and not (
        opts.embedsubtitles or opts.writeautomaticsub
    ):
...
def parse_options(argv=None):
    ...
    urls = get_urls(
        urls,
        opts.batchfile,
        -1 if opts.quiet and not opts.verbose else opts.verbose,
    )
    ...
    playlist_pps = [
        pp for pp in postprocessors if pp.get("when") == "playlist"
    ]
    ...
    return ParsedOptions(
        ...
        urls,
        {
            ...
            "simulate": (
                (print_only or any_getting or None)
            ...
            "noprogress": (
                opts.quiet if opts.noprogress is None else opts.noprogress
            ),
    ...
    with YoutubeDL(ydl_opts) as ydl:
        ...
        if opts.list_impersonate_targets:
            ...
            def make_row(target, handler):
                return [
                    join_nonempty(
                        target.client.title(), target.version, delim="-"
                    )
                    or "-",
                    join_nonempty(
                        (target.os or "").title(), target.os_version, delim="-"
                    )
                    or "-",
                    handler,
                ]
                ...
                if not any(
                    ...
                    rows.append(
                        [
                            ...
                            for text in make_row(
                                known_target,
                                f"{known_handler} (not available)",
                            )
                        ]
                    )
            ...
            ydl.to_stdout(
                render_table(
                    ["Client", "OS", "Source"], rows, extra_gap=2, delim="-"
                )
            )
            return
        ...
        if not actual_use:
            ...
            if not args and os.name == "nt" and getattr(sys, "frozen", False):
                ...
                is_onefile = hasattr(sys, "_MEIPASS") and os.path.basename(
                    sys._MEIPASS
                ).startswith("_MEI")
                if attached_processes == 1 or (
                    is_onefile and attached_processes == 2
                ):
                    print(
        ...
        try:
            if opts.load_info_filename is not None:
                if all_urls:
                    ydl.report_warning(
                        "URLs are ignored due to --load-info-json"
                    )
                return ydl.download_with_info_file(
