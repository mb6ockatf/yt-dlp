...
def _get_variant_and_executable_path():
    ...
    if getattr(sys, "frozen", False):
        ...
        if not hasattr(sys, "_MEIPASS"):
            ...
            machine = (
                "_legacy"
                if version_tuple(platform.mac_ver()[0]) < (10, 15)
                else ""
            )
        else:
...
class Updater:
    ...
    def __init__(self, ydl, target: str | None = None):
        ...
        if not sep and (
            "/" in self.requested_tag
            or self.requested_tag in self._update_sources
        ):
        ...
        if "/" in self.requested_channel:
            ...
            self.requested_repo = self._update_sources.get(
    ...
    def _download_asset(self, name, tag=None):
        ...
        url = (
            f"https://github.com/{self.requested_repo}/releases/{path}/{name}"
        )
    ...
    def _get_version_info(self, tag: str) -> tuple[str | None, str | None]:
        ...
        if tag == "latest":
            ...
            match = re.search(
                rf"\s+(?P<version>{_VERSION_RE.pattern})$",
                api_info.get("name", ""),
            )
    ...
    def query_update(self, *, _output=False) -> UpdateInfo | None:
        ...
        try:
            ...
            self._report_network_error(
                f"obtain version info ({e})",
                delim="; Please try again later or",
            )
            return None
        ...
        if _output:
            update_label = _make_label(
                self.requested_repo, result_tag, result_version
            )
            self.ydl.to_screen(
    ...
    def update(self, update_info=NO_DEFAULT):
        ...
        self.ydl.to_screen(
            f"Current Build Hash: {_sha256_file(self.filename)}"
        )
        ...
        try:
            newcontent = self._download_asset(
                update_info.binary_name, update_info.tag
            )
        except network_exceptions as e:
    ...
    def _block_restart(self, msg):
        def wrapper():
            self._report_error(
                f"{msg}. Restart yt-dlp to use the updated version",
                expected=True,
            )
