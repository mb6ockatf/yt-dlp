...
if curl_cffi_version != (0, 5, 10) and not (
    (0, 7, 0) <= curl_cffi_version < (0, 7, 2)
):
    curl_cffi._yt_dlp__version = f"{curl_cffi.__version__} (unsupported)"
    raise ImportError(
        "Only curl_cffi versions 0.5.10, 0.7.0 and 0.7.1 are supported"
    )
...
class CurlCFFIRH(ImpersonateRequestHandler, InstanceStoreMixin):
        ...
        **(
            {
                ImpersonateTarget(
                    "chrome", "124", "macos", "14"
                ): curl_cffi.requests.BrowserType.chrome124,
                ImpersonateTarget(
                    "chrome", "123", "macos", "14"
                ): curl_cffi.requests.BrowserType.chrome123,
                ImpersonateTarget(
                    "chrome", "120", "macos", "14"
                ): curl_cffi.requests.BrowserType.chrome120,
                ImpersonateTarget(
                    "chrome", "119", "macos", "14"
                ): curl_cffi.requests.BrowserType.chrome119,
                ImpersonateTarget(
                    "chrome", "116", "windows", "10"
                ): curl_cffi.requests.BrowserType.chrome116,
            }
            if curl_cffi_version >= (0, 7, 0)
            else {}
        ),
        ImpersonateTarget(
            "chrome", "110", "windows", "10"
        ): curl_cffi.requests.BrowserType.chrome110,
        ImpersonateTarget(
            "chrome", "107", "windows", "10"
        ): curl_cffi.requests.BrowserType.chrome107,
        ImpersonateTarget(
            "chrome", "104", "windows", "10"
        ): curl_cffi.requests.BrowserType.chrome104,
        ImpersonateTarget(
            "chrome", "101", "windows", "10"
        ): curl_cffi.requests.BrowserType.chrome101,
        ImpersonateTarget(
            "chrome", "100", "windows", "10"
        ): curl_cffi.requests.BrowserType.chrome100,
        ImpersonateTarget(
            "chrome", "99", "windows", "10"
        ): curl_cffi.requests.BrowserType.chrome99,
        ImpersonateTarget(
            "edge", "101", "windows", "10"
        ): curl_cffi.requests.BrowserType.edge101,
        ImpersonateTarget(
            "edge", "99", "windows", "10"
        ): curl_cffi.requests.BrowserType.edge99,
        **(
            {
                ImpersonateTarget(
                    "safari", "17.0", "macos", "14"
                ): curl_cffi.requests.BrowserType.safari17_0,
            }
            if curl_cffi_version >= (0, 7, 0)
            else {}
        ),
        ...
        ): curl_cffi.requests.BrowserType.safari15_5,
        ImpersonateTarget(
            "safari", "15.3", "macos", "11"
        ): curl_cffi.requests.BrowserType.safari15_3,
        ImpersonateTarget(
            "chrome", "99", "android", "12"
        ): curl_cffi.requests.BrowserType.chrome99_android,
        **(
            {
                ImpersonateTarget(
                    "safari", "17.2", "ios", "17.2"
                ): curl_cffi.requests.BrowserType.safari17_2_ios,
            }
            if curl_cffi_version >= (0, 7, 0)
            else {}
        ),
    ...
    def _send(self, request: Request):
        ...
        if self._client_cert:
            session.curl.setopt(
                CurlOpt.SSLCERT, self._client_cert["client_certificate"]
            )
            ...
            if client_certificate_password:
                session.curl.setopt(
                    CurlOpt.KEYPASSWD, client_certificate_password
                )
        ...
        try:
            ...
            elif e.code == CurlECode.PROXY or (
                e.code == CurlECode.RECV_ERROR and "CONNECT" in str(e)
            ):
