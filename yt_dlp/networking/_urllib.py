...
class HTTPHandler(urllib.request.AbstractHTTPHandler):
    ...
    def http_response(self, req, resp):
        ...
        for encoding in (
            e.strip()
            for e in reversed(
                resp.headers.get("Content-encoding", "").split(",")
            )
        ):
            if encoding == "gzip":
                decoded_response = self.gz(decoded_response or resp.read())
            elif encoding == "deflate":
                decoded_response = self.deflate(
                    decoded_response or resp.read()
                )
...
class ProxyHandler(urllib.request.BaseHandler):
    ...
    def __init__(self, proxies=None):
        ...
        for scheme in ("http", "https", "ftp"):
            setattr(
                self, f"{scheme}_open", lambda r, meth=self.proxy_open: meth(r)
            )
...
class UrllibResponseAdapter(Response):
    ...
    def __init__(
        self, res: http.client.HTTPResponse | urllib.response.addinfourl
    ):
...
def handle_response_read_exceptions(e):
    if isinstance(e, http.client.IncompleteRead):
        ...
        e,
        (OSError, EOFError, http.client.HTTPException, *CONTENT_DECODE_ERRORS),
    ):
...
class UrllibRH(RequestHandler, InstanceStoreMixin):
    ...
    def _send(self, request):
        ...
        try:
            res = opener.open(
                urllib_req, timeout=self._calculate_timeout(request)
            )
        except urllib.error.HTTPError as e:
            if isinstance(
                e.fp, (http.client.HTTPResponse, urllib.response.addinfourl)
            ):
