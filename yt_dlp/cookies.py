...
def _extract_firefox_cookies(profile, container, logger):
    ...
    if profile is None:
        ...
        search_roots = [
            os.path.join(path, profile) for path in _firefox_browser_dirs()
        ]
    ...
    for root in map(os.path.abspath, roots):
        for pattern in ("", "*/", "Profiles/*/"):
            yield from glob.iglob(
                os.path.join(root, pattern, "cookies.sqlite")
            )
...
def _get_chromium_based_browser_settings(browser_name):
    ...
    if sys.platform in ("cygwin", "win32"):
        ...
        browser_dir = {
            ...
            "opera": os.path.join(
                appdata_roaming, R"Opera Software\Opera Stable"
            ),
            "vivaldi": os.path.join(appdata_local, R"Vivaldi\User Data"),
            "whale": os.path.join(
                appdata_local, R"Naver\Naver Whale\User Data"
            ),
        }[browser_name]
...
def _extract_chrome_cookies(browser_name, profile, keyring, logger):
    ...
    with tempfile.TemporaryDirectory(prefix="yt_dlp") as tmpdir:
        ...
        try:
            ...
            secure_column = (
                "is_secure" if "is_secure" in column_names else "secure"
            )
            cursor.execute(
...
class LinuxChromeCookieDecryptor(ChromeCookieDecryptor):
    ...
    def derive_key(password):
        ...
        return pbkdf2_sha1(
            password, salt=b"saltysalt", iterations=1, key_length=16
        )
    ...
    def decrypt(self, encrypted_value):
        ...
        if version == b"v10":
            ...
            self._logger.warning(
                f'unknown cookie version: "{version}"', only_once=True
            )
...
class MacChromeCookieDecryptor(ChromeCookieDecryptor):
    ...
    def derive_key(password):
        ...
        return pbkdf2_sha1(
            password, salt=b"saltysalt", iterations=1003, key_length=16
        )
...
class WindowsChromeCookieDecryptor(ChromeCookieDecryptor):
    ...
    def decrypt(self, encrypted_value):
        ...
        if version == b"v10":
            ...
            # https://chromium.googlesource.com/chromium/src/+/refs/heads/main/components/os_crypt/sync/os_crypt_win.cc
            return _decrypt_windows_dpapi(
                encrypted_value, self._logger
            ).decode()
...
def _parse_safari_cookies_page(data, jar, logger):
    ...
    with _create_progress_bar(logger) as progress_bar:
        for i, record_offset in enumerate(record_offsets):
            progress_bar.print(
                f"Loading cookie {i: 6d}/{number_of_cookies: 6d}"
            )
...
def _get_gnome_keyring_password(browser_keyring_name, logger):
    if not secretstorage:
        logger.error(
            f"secretstorage not available {_SECRETSTORAGE_UNAVAILABLE_REASON}"
        )
        return b""
...
def _get_linux_keyring_password(browser_keyring_name, keyring, logger):
    ...
    keyring = (
        _LinuxKeyring[keyring] if keyring else _choose_linux_keyring(logger)
    )
...
def _get_mac_keyring_password(browser_keyring_name, logger):
    ...
    try:
        ...
        logger.warning(
            f"exception running find-generic-password: {error_to_str(e)}"
        )
        return None
        ...
        def prepare_line(line):
            ...
            if len(cookie_list) != self._ENTRY_LEN:
                raise http.cookiejar.LoadError(
                    f"invalid length {len(cookie_list)}"
                )
