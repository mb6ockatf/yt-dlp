...
def parseOpts(
    ...
    def add_config(label, path=None, func=None):
        ...
        if root.parse_known_args()[0].ignoreconfig:
            ...
            args, current_path = next(
                filter(None, _load_from_config_dirs(func(PACKAGE_NAME))),
                (None, None),
            )
        else:
    ...
    def load_configs():
        ...
        yield add_config(
            "Home",
            expand_path(
                root.parse_known_args()[0].paths.get("home", "")
            ).strip(),
        )
        ...
        if loaded_all_configs:
            ...
            if root.parse_known_args()[0].ignoreconfig:
                user_conf = next(
                    (
                        i
                        for i, conf in enumerate(root.configs)
                        if conf.label == "User"
                    ),
                    None,
                )
...
class _YoutubeDLHelpFormatter(optparse.IndentedHelpFormatter):
    def __init__(self):
        ...
        super().__init__(
            width=max_width, max_help_position=int(0.45 * max_width)
        )
...
def create_parser():
        ...
        value = list(
            filter(
                ...
                (
                    [process(value)]
                    if delim is None
                    else map(process, value.split(delim))
                ),
            )
        )
        setattr(
    ...
    def _dict_from_options_callback(
        ...
        mobj = re.match(
            rf"(?is)(?P<keys>{allowed_keys}){delimiter}(?P<val>.*)$",
        ...
        try:
            ...
            raise optparse.OptionValueError(
                f"wrong {opt_str} formatting; {err}"
            )
        for key in keys:
    ...
    downloader.add_option(
        ...
        callback=_dict_from_options_callback,
        callback_kwargs={
            "allowed_keys": r"ffmpeg_[io]\d*|{}".format(
    ...
    verbosity = optparse.OptionGroup(
        parser, "Verbosity and Simulation Options"
    )
    ...
    filesystem.add_option(
        ...
        default="NA",
        help=(
            'Placeholder for unavailable fields in --output (default: "%default")'
        ),
    )
    filesystem.add_option(
        "--autonumber-size",
    ...
    thumbnail.add_option(
        ...
        callback=lambda option, _, __, parser: setattr(
            parser.values,
            option.dest,
            getattr(parser.values, option.dest) or True,
        ),
    ...
    extractor.add_option(
        ...
        callback_kwargs={
            "multiple_keys": False,
            "process": lambda val: dict(
                _extractor_arg_parser(*arg.split("=", 1))
