...
class HlsFD(FragmentFD):
    ...
    def real_download(self, filename, info_dict):
        ...
        if s:
            self.to_screen(
                f"[{self.FD_NAME}] Using m3u8 manifest from extracted info"
            )
        else:
            ...
            if self.params.get("write_pages"):
                ...
                self.to_screen(
                    f"[{self.FD_NAME}] Saving request to {dump_filename}"
                )
        ...
        can_download, message = (
            self.can_download(
                s, info_dict, self.params.get("allow_unplayable_formats")
            ),
            None,
        )
        if can_download:
            ...
            if no_crypto and has_ffmpeg:
                can_download, message = (
                    False,
                    "The stream has AES-128 encryption and pycryptodomex is not available",
                )
            elif no_crypto:
                message = (
            ...
            elif info_dict.get("extractor_key") == "Generic" and re.search(
                r"(?m)#EXT-X-MEDIA-SEQUENCE:(?!0$)", s
            ):
        ...
        if not can_download:
            if self._has_drm(s) and not self.params.get(
                "allow_unplayable_formats"
            ):
                if info_dict.get("has_drm") and self.params.get("test"):
                    self.to_screen(
                        f"[{self.FD_NAME}] This format is DRM protected",
                        skip_eol=True,
                    )
                else:
                    self.report_error(
        ...
        def is_ad_fragment_start(s):
            return (
                s.startswith("#ANVATO-SEGMENT-INFO") and "type=ad" in s
            ) or (s.startswith("#UPLYNK-SEGMENT") and s.endswith(",ad"))

        def is_ad_fragment_end(s):
            return (
                s.startswith("#ANVATO-SEGMENT-INFO") and "type=master" in s
        ...
        if extra_param_to_segment_url := info_dict.get(
            ...
            extra_segment_query = urllib.parse.parse_qs(
                extra_param_to_segment_url
            )
        ...
        if external_aes_key:
            external_aes_key = binascii.unhexlify(
                remove_start(external_aes_key, "0x")
            )
        ...
        if external_aes_iv:
            external_aes_iv = binascii.unhexlify(
                remove_start(external_aes_iv, "0x").zfill(32)
            )
        byte_range = {}
        byte_range_offset = 0
            ...
            if line:
                if not line.startswith("#"):
                    ...
                    if extra_segment_query:
                        frag_url = update_url_query(
                            frag_url, extra_segment_query
                        )
            ...
            def pack_fragment(frag_content, frag_index):
                ...
                for block in webvtt.parse_fragment(frag_content):
                    if isinstance(block, webvtt.CueBlock):
                        ...
                        dedup_window = extra_state.setdefault(
                            "webvtt_dedup_window", []
                        )
                        ...
                        block.mpegts += (
                            extra_state["webvtt_mpegts_adjust"] << 33
                        )
                        if block.mpegts < extra_state.get(
                            "webvtt_mpegts_last", 0
                        ):
                            overflow = True
                            block.mpegts += 1 << 33
                        ...
                        if frag_index == 1:
                            ...
                            if (
                                block.mpegts is not None
                                and block.local is not None
                            ):
                                adjust = (
                                    block.mpegts
                                    - extra_state.get("webvtt_mpegts", 0)
                                ) - (
                                    block.local
                                    - extra_state.get("webvtt_local", 0)
                                )
                            continue
