...
def get_suitable_downloader(
    ...
    downloaders = [
        _get_suitable_downloader(info_copy, proto, params, default)
...
def _get_suitable_downloader(info_dict, protocol, params, default):
    ...
    external_downloader = (
        downloaders
        if isinstance(downloaders, str) or downloaders is None
        else downloaders.get(
            shorten_protocol_name(protocol, True), downloaders.get("default")
        )
    )
