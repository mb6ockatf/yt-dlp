...
class FragmentFD(FileDownloader):
    ...
    def _read_ytdl_file(self, ctx):
        ...
        stream, _ = self.sanitize_open(
            self.ytdl_filename(ctx["filename"]), "r"
        )
        try:
            ytdl_data = json.loads(stream.read())
            ctx["fragment_index"] = ytdl_data["downloader"][
                "current_fragment"
            ]["index"]
    ...
    def _write_ytdl_file(self, ctx):
        frag_index_stream, _ = self.sanitize_open(
    ...
    def _download_fragment(
        self, ctx, frag_url, info_dict, headers=None, request_data=None
    ):
        ...
        fragment_info_dict["frag_resume_len"] = ctx["frag_resume_len"] = (
            frag_resume_len
        )
    ...
    def _read_fragment(self, ctx):
        if not ctx.get("fragment_filename_sanitized"):
            return None
        try:
            down, frag_sanitized = self.sanitize_open(
                ctx["fragment_filename_sanitized"], "rb"
            )
    ...
    def _prepare_frag_download(self, ctx):
        ...
        if self.__do_ytdl_file(ctx):
            ytdl_file_exists = os.path.isfile(
                self.ytdl_filename(ctx["filename"])
            )
            ...
            if continuedl and ytdl_file_exists:
                ...
                if is_corrupt or is_inconsistent:
                    message = (
                        ".ytdl file is corrupt"
                        if is_corrupt
                        else "Inconsistent state of incomplete fragment download"
                    )
                    self.report_warning(
    ...
    def decrypter(self, info_dict):
        ...
        def _get_key(url):
            if url not in _key_cache:
                _key_cache[url] = self.ydl.urlopen(
                    self._prepare_url(info_dict, url)
                ).read()
        ...
        def decrypt_fragment(fragment, frag_content):
            ...
            if not decrypt_info or decrypt_info["METHOD"] != "AES-128":
                return frag_content
            iv = decrypt_info.get("IV") or struct.pack(
                ">8xq", fragment["media_sequence"]
            )
            decrypt_info["KEY"] = decrypt_info.get("KEY") or _get_key(
                traverse_obj(info_dict, ("hls_aes", "uri"))
                or decrypt_info["URI"]
            )
            ...
            if self.params.get("test", False):
                return frag_content
            return unpad_pkcs7(
                aes_cbc_decrypt_bytes(frag_content, decrypt_info["KEY"], iv)
            )
        ...
        *,
        ...
        def download_fragment(fragment, ctx):
            ...
            for retry in RetryManager(
                self.params.get("fragment_retries"), error_callback
            ):
                try:
        ...
        def append_fragment(frag_content, frag_index, ctx):
            if frag_content:
                ...
                ctx["dest_stream"].close()
                self.report_error(
                    f"fragment {frag_index} not found, unable to continue"
                )
                return False
            return True
        ...
        max_workers = math.ceil(
            self.params.get("concurrent_fragment_downloads", 1)
            / ctx.get("max_progress", 1)
        )
        if max_workers > 1:
            ...
            with (
                tpe
                or concurrent.futures.ThreadPoolExecutor(max_workers) as pool
            ):
                try:
                    for fragment, frag_index, frag_filename in pool.map(
                        _download_fragment, fragments
                    ):
                        ctx.update(
                            {
                        ...
                        if not append_fragment(
                            decrypt_fragment(
                                fragment, self._read_fragment(ctx)
                            ),
                            frag_index,
                            ctx,
                        ):
