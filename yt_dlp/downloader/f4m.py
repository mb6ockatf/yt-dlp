...
def remove_encrypted_media(media):
    return list(
        filter(
            lambda e: "drmAdditionalHeaderId" not in e.attrib
...
class F4mFD(FragmentFD):
    ...
    def _get_unencrypted_media(self, doc):
        ...
        if not self.params.get("allow_unplayable_formats"):
            for e in doc.findall(_add_ns("drmAdditionalHeader")) + doc.findall(
                _add_ns("drmAdditionalHeaderSet")
            ):
    ...
    def real_download(self, filename, info_dict):
        ...
        manifest = fix_xml_ampersands(
            urlh.read().decode("utf-8", "ignore")
        ).strip()

        doc = compat_etree_fromstring(manifest)
        formats = [
            (int(f.attrib.get("bitrate", -1)), f)
        ...
        if requested_bitrate is None or len(formats) == 1:
            ...
            rate, media = next(
                filter(lambda f: int(f[0]) == requested_bitrate, formats)
            )
