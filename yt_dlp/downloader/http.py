...
class HttpFD(FileDownloader):
    def real_download(self, filename, info_dict):
        ...
        headers = HTTPHeaderDict(
            {"Accept-Encoding": "identity"}, info_dict.get("http_headers")
        )
        ...
        chunk_size = (
            ...
            if is_test
                ...
                or info_dict.get("downloader_options", {}).get(
                    "http_chunk_size"
                )
                or 0
            )
        ...
        def establish_connection():
            ...
            if ctx.chunk_size:
                ...
                range_end = (
                    chunk_aware_end
                    if req_end is None
                    else min(chunk_aware_end, req_end)
                )
            ...
            try:
                ...
                if has_range:
                    ...
                    content_range_start, content_range_end, content_len = (
                    ...
                    if range_start == content_range_start and (
                        ...
                        if content_len or req_end:
                            ctx.data_len = min(
                                content_len or req_end, req_end or content_len
                            ) - (req_start or 0)
                        return
                ...
                ctx.data_len = ctx.content_len = int_or_none(
                    ctx.data.headers.get("Content-length", None)
                )
        ...
        def download():
            ...
            while True:
                ...
                if ctx.stream is None:
                    ...
                    if (
                        ...
                        try:
                            ...
                            self.report_error(
                                f"unable to set filesize xattr: {err}"
                            )
                ...
                if not self.params.get("noresizebuffer", False):
                    block_size = self.best_block_size(
                        after - before, len(data_block)
                    )
                ...
                speed = self.calc_speed(
                    start, now, byte_counter - ctx.resume_len
                )
                ...
                if speed and speed < (
                    self.params.get("throttledratelimit") or 0
                ):
            ...
            if (
                not is_test
                and ctx.chunk_size
                and ctx.content_len is not None
                and byte_counter < ctx.content_len
            ):
                ctx.resume_len = byte_counter
                raise NextFragment
            ...
            if self.params.get("updatetime", True):
                info_dict["filetime"] = self.try_utime(
                    ctx.filename, ctx.data.headers.get("last-modified", None)
                )
        ...
        for retry in RetryManager(
            self.params.get("retries"), self.report_retry
        ):
            try:
