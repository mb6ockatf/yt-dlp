...
class YoutubeLiveChatFD(FragmentFD):
    ...
    def real_download(self, filename, info_dict):
        ...
        if (
            not self.params.get("skip_download")
            and info_dict["protocol"] == "youtube_live_chat"
        ):
            self.report_warning(
                "Live chat download runs until the livestream ends. "
        ...
        def dl_fragment(url, data=None, headers=None):
            http_headers = HTTPHeaderDict(
                info_dict.get("http_headers"), headers
            )
            return self._download_fragment(
                ctx, url, info_dict, http_headers, data
            )
        ...
        def parse_actions_replay(live_chat_continuation):
            ...
            if offset is not None:
                ...
                if continuation:
                    ...
                    click_tracking_params = continuation.get(
                        "clickTrackingParams"
                    )
        ...
        def try_refresh_replay_beginning(live_chat_continuation):
            ...
            refresh_continuation = try_get(
                live_chat_continuation,
                lambda x: x["header"]["liveChatHeaderRenderer"][
                    "viewSelector"
            ...
            if refresh_continuation:
                ...
                refresh_continuation_id = refresh_continuation.get(
                    "continuation"
                )
                offset = 0
                click_tracking_params = refresh_continuation.get(
                    "trackingParams"
                )
        ...
        def parse_actions_live(live_chat_continuation):
            ...
            for action in live_chat_continuation.get("actions", []):
                ...
                processed_fragment.extend(
                    json.dumps(pseudo_action, ensure_ascii=False).encode()
                    + b"\n"
                )
        ...
        def download_and_parse_fragment(
            url, frag_index, request_data=None, headers=None
        ):
            for retry in RetryManager(
                ...
                try:
                    ...
                    try:
                        data = ie.extract_yt_initial_data(
                            video_id, raw_fragment.decode("utf-8", "replace")
                        )
                    ...
                    func = (
                        (
                            info_dict["protocol"] == "youtube_live_chat"
                            and parse_actions_live
                        )
        ...
        visitor_data = try_get(
            innertube_context, lambda x: x["client"]["visitorData"], str
        )
        ...
        while continuation_id is not None:
            ...
            if frag_index > 1:
                ...
                if click_tracking_params:
                    request_data["context"]["clickTracking"] = {
                        "clickTrackingParams": click_tracking_params
                    }
                headers = ie.generate_api_headers(
                    ytcfg=ytcfg, visitor_data=visitor_data
                )
                headers.update({"content-type": "application/json"})
                fragment_request_data = (
                    json.dumps(request_data, ensure_ascii=False).encode()
                    + b"\n"
                )
                success, continuation_id, offset, click_tracking_params = (
                ...
                success, continuation_id, offset, click_tracking_params = (
                    download_and_parse_fragment(chat_page_url, frag_index)
                )
            if not success:
                return False
            if test:
    ...
    def parse_live_timestamp(action):
        ...
        if not isinstance(renderer, dict):
            return None
        parent_item_getters = [
            lambda x: x["showItemEndpoint"]["showLiveChatItemEndpoint"][
                "renderer"
            ],
            lambda x: x["contents"],
        ]
