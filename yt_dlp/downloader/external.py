...
class ExternalFD(FragmentFD):
    ...
    def real_download(self, filename, info_dict):
        ...
        if retval == 0:
            ...
            self.to_stderr("\n")
            self.report_error(
                "%s exited with code %d" % (self.get_basename(), retval)
            )
            return False
    ...
    def supports(cls, info_dict):
        return all(
            (
                not info_dict.get("to_stdout")
                or Features.TO_STDOUT in cls.SUPPORTED_FEATURES,
                "+" not in info_dict["protocol"]
                or Features.MULTIPLE_FORMATS in cls.SUPPORTED_FEATURES,
                not traverse_obj(
                    info_dict,
                    ("hls_aes", ...),
                ...
                all(
                    proto in cls.SUPPORTED_PROTOCOLS
                    for proto in info_dict["protocol"].split("+")
                ),
            )
        )
    ...
    def _call_downloader(self, tmpfilename, info_dict):
        ...
        cmd = [
            encodeArgument(a) for a in self._make_cmd(tmpfilename, info_dict)
        ]
        ...
        for frag_index, fragment in enumerate(info_dict["fragments"]):
            ...
            try:
                ...
                if skip_unavailable_fragments and frag_index > 1:
                    ...
                    continue
                self.report_error(
                    f"Unable to open fragment {frag_index}; {err}"
                )
                return -1
...
class Aria2cFD(ExternalFD):
    ...
    def supports_manifest(manifest):
        ...
        check_results = (
            not re.search(feature, manifest)
    ...
    def _call_downloader(self, tmpfilename, info_dict):
        ...
        if False and "no-external-downloader-progress" not in self.params.get(
            "compat_opts", []
        ):
            info_dict["__rpc"] = {
    ...
    def _make_cmd(self, tmpfilename, info_dict):
        ...
        if "fragments" in info_dict:
            ...
            for frag_index, fragment in enumerate(info_dict["fragments"]):
                fragment_filename = (
                    f"{os.path.basename(tmpfilename)}-Frag{frag_index}"
                )
                url_list.append(
                    "{}\n\tout={}".format(
                        fragment["url"],
                        self._aria2c_filename(fragment_filename),
                    )
                )
    ...
    def _call_process(self, cmd, info_dict):
        ...
        send_rpc = functools.partial(
            self.aria2c_rpc,
            info_dict["__rpc"]["port"],
            info_dict["__rpc"]["secret"],
        )
        started = time.time()
        ...
        def get_stat(key, *obj, average=False):
            val = tuple(
                filter(None, map(float, traverse_obj(obj, (..., ..., key))))
            ) or [0]
            return sum(val) / (len(val) if average else 1)

        with Popen(
            cmd, text=True, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE
        ) as p:
            ...
            while retval is None:
                ...
                downloaded = get_stat("totalLength", completed) + get_stat(
                    "completedLength", active
                )
                speed = get_stat("downloadSpeed", active)
                total = frag_count * get_stat(
                    "totalLength", active, completed, average=True
                )
                if total < downloaded:
                    total = None
                ...
                status.update(
                    {
                        ...
                        "fragment_index": (
                            min(frag_count, len(completed) + 1)
                            if fragmented
                            else None
                        ),
                        "elapsed": time.time() - started,
...
class FFmpegFD(ExternalFD):
    ...
    def _call_downloader(self, tmpfilename, info_dict):
        ...
        if not ffpp.available:
            self.report_error(
                "m3u8 download detected but ffmpeg could not be found. Please install"
            )
            return False
        ffpp.check_version()
        ...
        args += traverse_obj(
            info_dict, ("downloader_options", "ffmpeg_args", ...)
        )
        ...
        start_time, end_time = info_dict.get(
            "section_start"
        ) or 0, info_dict.get("section_end")

        selected_formats = info_dict.get("requested_formats") or [info_dict]
        for i, fmt in enumerate(selected_formats):
            is_http = re.match(r"https?://", fmt["url"])
            cookies = (
                self.ydl.cookiejar.get_cookies_for_url(fmt["url"])
                if is_http
                else []
            )
            if cookies:
            ...
            args += [
                *self._configuration_args((f"_i{i + 1}", "_i")),
                "-i",
                fmt["url"],
            ]
        ...
        if (
            info_dict.get("requested_formats")
            or protocol == "http_dash_segments"
        ):
            for i, fmt in enumerate(selected_formats):
        ...
        if protocol in ("m3u8", "m3u8_native"):
            use_mpegts = (tmpfilename == "-") or self.params.get(
                "hls_use_mpegts"
            )
            ...
            if use_mpegts:
                ...
                args += ["-f", "mp4"]
                if (
                    ffpp.basename == "ffmpeg"
                    and ffpp._features.get("needs_adtstoasc")
                ) and (
                    not info_dict.get("acodec")
                    or info_dict["acodec"].split(".")[0] in ("aac", "mp4a")
                ):
        ...
        args += traverse_obj(
            info_dict, ("downloader_options", "ffmpeg_args_out", ...)
        )
...
def get_external_downloader(external_downloader):
    ...
    return _BY_NAME.get(bn) or next(
        (klass for klass in _BY_NAME.values() if klass.EXE_NAME in bn), None
    )
