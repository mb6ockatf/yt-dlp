...
class FileDownloader:
    ...
    def format_speed(speed):
        return (
            " Unknown B/s"
            if speed is None
            else f"{format_bytes(speed):>10s}/s"
        )
    ...
    def wrap_file_access(action, *, fatal=False):
        ...
        def wrapper(self, func, *args, **kwargs):
            for retry in RetryManager(
                self.params.get("file_access_retries", 3),
                error_callback,
                fd=self,
            ):
                try:
    ...
    def _prepare_multiline_status(self, lines=1):
        if self.params.get("noprogress"):
            ...
            self._multiline = BreaklineStatusPrinter(
                self.ydl._out_files.out, lines
            )
        else:
        ...
        self._multiline.allow_colors = (
            self.ydl._allow_colors.out
            and self.ydl._allow_colors.out != "no_color"
        )
        ...
        s.update(
            {
                ...
                "_total_bytes_str": _format_bytes("total_bytes"),
                "_total_bytes_estimate_str": _format_bytes(
                    "total_bytes_estimate"
                ),
    ...
    def report_retry(
        self, err, count, retries, frag_index=NO_DEFAULT, fatal=True
    ):
        """Report retry"""
        ...
        RetryManager.report_retry(
            ...
            error=(
                IDENTITY
                if not fatal
                else lambda e: self.report_error(
    ...
    def download(self, filename, info_dict, subtitle=False):
        ...
        if subtitle:
            ...
            sleep_interval = random.uniform(
                min_sleep_interval,
                self.params.get("max_sleep_interval") or min_sleep_interval,
            )
        if sleep_interval > 0:
            self.to_screen(
                f"[download] Sleeping {sleep_interval:.2f} seconds ..."
            )
