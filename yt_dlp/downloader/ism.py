...
def write_piff_header(stream, params):
    ...
    if stream_type == "audio":
        ...
        media_header_box = full_box(
            b"smhd", 0, 0, smhd_payload
        )  # Sound Media Header
    elif stream_type == "video":
        vmhd_payload = u16.pack(0)  # graphics mode
        vmhd_payload += u16.pack(0) * 3  # opcolor
        media_header_box = full_box(
            b"vmhd", 0, 1, vmhd_payload
        )  # Video Media Header
    elif stream_type == "text":
        media_header_box = full_box(
            b"sthd", 0, 0, b""
    ...
    dref_payload += full_box(
        b"url ", 0, SELF_CONTAINED, b""
    )  # Data Entry URL Box
    ...
    if stream_type == "audio":
        ...
        codec_private_data = binascii.unhexlify(
            params["codec_private_data"].encode()
        )
    ...
    stbl_payload = full_box(
        b"stsd", 0, 0, stsd_payload
    )  # Sample Description Box

    stts_payload = u32.pack(0)  # entry count
    stbl_payload += full_box(
        b"stts", 0, 0, stts_payload
    )  # Decoding Time to Sample Box

    stsc_payload = u32.pack(0)  # entry count
    stbl_payload += full_box(
        b"stsc", 0, 0, stsc_payload
    )  # Sample To Chunk Box
