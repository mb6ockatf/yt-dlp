...
if Cryptodome.AES:
    ...
    def aes_cbc_decrypt_bytes(data, key, iv):
        ...
        return Cryptodome.AES.new(key, Cryptodome.AES.MODE_CBC, iv).decrypt(
            data
        )
    ...
    def aes_gcm_decrypt_and_verify_bytes(data, key, tag, nonce):
        ...
        return Cryptodome.AES.new(
            key, Cryptodome.AES.MODE_GCM, nonce
        ).decrypt_and_verify(data, tag)
    ...
    def aes_gcm_decrypt_and_verify_bytes(data, key, tag, nonce):
        ...
        return bytes(
            aes_gcm_decrypt_and_verify(*map(list, (data, key, tag, nonce)))
        )
...
def aes_gcm_decrypt_and_verify(data, key, tag, nonce):
    ...
    if len(nonce) == 12:
        ...
        ghash_in = (
            nonce + [0] * fill + list((8 * len(nonce)).to_bytes(8, "big"))
        )
    ...
    pad_len = (
        BLOCK_SIZE_BYTES - (len(data) % BLOCK_SIZE_BYTES)
    ) % BLOCK_SIZE_BYTES
    s_tag = ghash(
        hash_subkey,
        data
...
def key_expansion(data):
    ...
    while len(data) < expanded_key_size_bytes:
        ...
        for _ in range(
            3 if key_size_bytes == 32 else 2 if key_size_bytes == 24 else 0
        ):
            temp = data[-4:]
            data += xor(temp, data[-key_size_bytes : 4 - key_size_bytes])
