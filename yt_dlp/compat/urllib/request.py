...
if os.name == "nt":
    ...
    def getproxies_registry_patched():
        ...
        if sys.version_info >= (
            ...
            10,
            5,
        ) or (  # https://docs.python.org/3.10/whatsnew/changelog.html#python-3-10-5-final
            3,
            9,
            13,
        ) <= sys.version_info < (
            3,
            10,
        ):  # https://docs.python.org/3.9/whatsnew/changelog.html#python-3-9-13-final
            return proxies

        for scheme in ("https", "ftp"):
            if scheme in proxies and proxies[scheme].startswith(
                f"{scheme}://"
            ):
