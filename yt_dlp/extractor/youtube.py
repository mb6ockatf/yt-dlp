...
class YoutubeBaseInfoExtractor(InfoExtractor):
    ...
    def ucid_or_none(self, ucid):
        return self._search_regex(
            rf"^({self._YT_CHANNEL_UCID_RE})$", ucid, "UC-id", default=None
        )
    ...
    def _preferred_lang(self):
        ...
        preferred_lang = self._configuration_arg(
            "lang", ie_key="Youtube", casesense=True, default=[""]
        )[0]
    ...
    def _initialize_pref(self):
        ...
        if pref_cookie:
            try:
                ...
                self.report_warning(
                    "Failed to parse user PREF cookie" + bug_reports_message()
                )
        pref.update({"hl": self._preferred_lang or "en", "tz": "UTC"})
        self._set_cookie(
            ".youtube.com", name="PREF", value=urllib.parse.urlencode(pref)
        )
    ...
    def _youtube_login_hint(self):
        return (
            f'{self._login_hint(method="cookies")}. Also see  '
    ...
    _YT_INITIAL_DATA_RE = (
        r'(?:window\s*\[\s*["\']ytInitialData["\']\s*\]|ytInitialData)\s*='
    )
    ...
    def _ytcfg_get_safe(
        self, ytcfg, getter, expected_type=None, default_client="web"
    ):
    ...
    def _select_api_hostname(self, req_api_hostname, default_client=None):
        return (
            self._configuration_arg(
                "innertube_host", [""], ie_key=YoutubeIE.ie_key()
            )[0]
            or req_api_hostname
    ...
    def _extract_context(self, ytcfg=None, default_client="web"):
        context = get_first(
            (ytcfg, self._get_default_ytcfg(default_client)),
            "INNERTUBE_CONTEXT",
            expected_type=dict,
        )
    ...
    def _get_sid_authorization_header(
        self, origin="https://www.youtube.com", user_session_id=None
    ):
        """
    ...
    def _call_api(
        ...
        data = (
            ...
            if context
                ...
                "context": self._extract_context(default_client=default_client)
            }
        )
        data.update(query)
    ...
    def _extract_data_sync_id(self, *args):
        ...
        if data_sync_id := self._configuration_arg(
            "data_sync_id", [None], ie_key=YoutubeIE, casesense=True
        )[0]:
            return data_sync_id
    ...
    def _extract_visitor_data(self, *args):
        ...
        if visitor_data := self._configuration_arg(
            "visitor_data", [None], ie_key=YoutubeIE, casesense=True
        )[0]:
            return visitor_data
        return get_first(
            args,
    ...
    def _generate_cookie_auth_headers(
        ...
        **kwargs,
        ...
        delegated_session_id = (
            delegated_session_id or self._extract_delegated_session_id(ytcfg)
        )
        ...
        auth = self._get_sid_authorization_header(
            ...
            or self._extract_user_session_id(ytcfg),
        )
        if auth is not None:
        ...
        **kwargs,
        ...
        headers = {
            ...
            "X-YouTube-Client-Version": self._extract_client_version(
                ytcfg, default_client
            ),
            "Origin": origin,
            "X-Goog-Visitor-Id": visitor_data
            or self._extract_visitor_data(ytcfg),
    ...
    def _download_webpage_with_retries(
        self, *args, retry_fatal=False, retry_on_status=None, **kwargs
    ):
        for retry in self.RetryManager(fatal=retry_fatal):
            try:
                ...
                if isinstance(e.cause, network_exceptions):
                    if not isinstance(
                        e.cause, HTTPError
                    ) or e.cause.status not in (retry_on_status or (403, 429)):
                        retry.error = e
                        continue
    ...
    def _download_ytcfg(self, client, video_id):
        ...
        webpage = self._download_webpage_with_retries(
            ...
            headers=traverse_obj(
                self._get_default_ytcfg(client),
                {
                    "User-Agent": (
                        "INNERTUBE_CONTEXT",
                        "client",
    ...
    def _extract_continuation_ep_data(cls, continuation_ep: dict):
        if isinstance(continuation_ep, dict):
            continuation = try_get(
                continuation_ep,
                lambda x: x["continuationCommand"]["token"],
                str,
            )
            if not continuation:
                return
    ...
    def _report_alerts(
        self, alerts, expected=True, fatal=True, only_once=False
    ):
        errors, warnings = [], []
        ...
        if errors:
            raise ExtractorError(
                f"YouTube said: {errors[-1][1]}", expected=expected
            )
    ...
    def _extract_badges(self, badge_list: list):
        ...
        for badge in traverse_obj(
            badge_list,
            (..., lambda key, _: re.search(r"[bB]adgeRenderer$", key)),
        ):
    ...
    def _get_text(data, *path_list, max_runs=None):
        for path in path_list or [None]:
            if path is None:
                ...
                if not any(
                    key is ... or isinstance(key, (list, tuple))
            ...
            for item in obj:
                ...
                text = "".join(
                    traverse_obj(runs, (..., "text"), expected_type=str)
                )
                if text:
                    return text
    ...
    def _extract_response(
        ...
        raise_for_incomplete = bool(
            self._configuration_arg("raise_incomplete_data", ie_key=YoutubeIE)
        )
    ...
    def _extract_video(self, renderer):
        ...
        scheduled_timestamp = str_to_int(
            traverse_obj(
        ...
        live_status = (
            ...
            if scheduled_timestamp is not None
                ...
                "was_live"
                if "streamed" in time_text.lower()
                else (
                    "is_live"
                    if overlay_style == "LIVE"
                    or self._has_badge(badges, BadgeType.LIVE_NOW)
                    else None
                )
            )
        )
        ...
        view_count_text = (
            self._get_text(
                renderer, "viewCountText", "shortViewCountText", "videoInfo"
            )
            or ""
        )
        view_count = (
        ...
        channel = self._get_text(
            renderer, "ownerText", "shortBylineText"
        ...
        return {
            ...
            "timestamp": (
                ...
                if self._configuration_arg(
                    "approximate_date", ie_key=YoutubeTabIE
                )
                else None
            ),
            ...
            "availability": (
                ...
                if self._has_badge(badges, BadgeType.AVAILABILITY_PUBLIC)
                    ...
                    is_private=self._has_badge(
                        badges, BadgeType.AVAILABILITY_PRIVATE
                    )
                    or None,
                    needs_premium=self._has_badge(
                        badges, BadgeType.AVAILABILITY_PREMIUM
                    )
                    or None,
                    needs_subscription=self._has_badge(
                        badges, BadgeType.AVAILABILITY_SUBSCRIPTION
                    )
                    or None,
                    is_unlisted=self._has_badge(
                        badges, BadgeType.AVAILABILITY_UNLISTED
                    )
                    or None,
                )
            ),
...
class YoutubeIE(YoutubeBaseInfoExtractor):
    ...
    def _prepare_live_from_start_formats(
        ...
        def refetch_manifest(format_id, delay):
            ...
            _, _, prs, player_url = self._download_player_responses(
                url, smuggled_data, video_id, webpage_url
            )
            video_details = traverse_obj(
                prs, (..., "videoDetails"), expected_type=dict
            )
            ...
            _, live_status, _, formats, _ = self._list_formats(
    ...
    def _live_dash_fragments(
        self,
        video_id,
        format_id,
        live_start_time,
        mpd_feed,
        manifestless_orig_fmt,
        ctx,
    ):
        ...
        lack_early_segments = (
            download_start_time - (live_start_time or download_start_time)
            > MAX_DURATION
        )
        ...
        def _extract_sequence_from_mpd(refresh_sequence, immediate):
            ...
            expire_fast = immediate or (
                last_error
                and isinstance(last_error, HTTPError)
                and last_error.status == 403
            )
            mpd_url, stream_number, is_live = mpd_feed(
                format_id, 5 if expire_fast else 18000
            ...
            if manifestless_orig_fmt:
                ...
                fmt_info = next(
                    x
                    for x in fmts
                    if x["manifest_stream_number"] == stream_number
                )
            ...
            _last_seq = int(
                re.search(r"(?:/|^)sq/(\d+)", fragments[-1]["path"]).group(1)
            )
        ...
        while is_live:
            ...
            if last_segment_url:
                ...
                try:
                    ...
                    urlh = None
                last_seq = try_get(
                    urlh, lambda x: int_or_none(x.headers["X-Head-Seqnum"])
                )
                if last_seq is None:
                    no_fragment_score += 2
                ...
                should_continue, last_seq = _extract_sequence_from_mpd(
                    True, no_fragment_score > 15
                )
                no_fragment_score += 2
            ...
            if lack_early_segments:
                known_idx = max(
                    known_idx,
                    last_seq - int(MAX_DURATION // fragments[-1]["duration"]),
                )
            try:
                for idx in range(known_idx, last_seq):
                    # do not update sequence here or you'll get skipped some part of it
                    should_continue, _ = _extract_sequence_from_mpd(
                        False, False
                    )
                    if not should_continue:
                        known_idx = idx - 1
    ...
    def _extract_player_url(self, *ytcfgs, webpage=None):
        ...
        if "/player_ias_tce.vflset/" in player_url:
            ...
            player_url = player_url.replace(
                "/player_ias_tce.vflset/", "/player_ias.vflset/"
            )
    ...
    def _print_sig_code(self, func, example_sig):
        ...
        signature_id_tuple = "({})".format(
            ", ".join(str(len(p)) for p in example_sig.split("."))
        )
        code = (
            f"if tuple(len(p) for p in s.split('.')) == {signature_id_tuple}:\n"
            f"    return {expr_code}\n"
        )
        ...
        def inner(*args, **kwargs):
            if cache_id not in self._player_cache:
                try:
                    ...
                    self._player_cache[cache_id] = ExtractorError(
    ...
    def _decrypt_nsig(self, s, video_id, player_url):
        ...
        try:
            jsi, player_id, func_code = self._extract_n_function_code(
                video_id, player_url
            )
        except ExtractorError as e:
            raise ExtractorError(
                "Unable to extract nsig function code", cause=e
            )
        ...
        try:
            extract_nsig = self._cached(
                self._extract_n_function_from_code, "nsig func", player_url
            )
    ...
    def _extract_n_function_name(self, jscode, player_url=None):
        ...
        if not funcname:
            self.report_warning(
                join_nonempty(
                    ...
                    player_url and f"         player = {player_url}",
                    delim="\n",
                )
            )
            return self._search_regex(
        ...
        return json.loads(
            js_to_json(
                self._search_regex(
                    rf"var {re.escape(funcname)}\s*=\s*(\[.+?\])\s*[,;]",
                    jscode,
                    f"Initial JS player n function list ({funcname}.{idx})",
                )
            )
        )[int(idx)]
    ...
    def _extract_n_function_code(self, video_id, player_url):
        player_id = self._extract_player_info(player_url)
        func_code = self.cache.load(
            "youtube-nsig", player_id, min_ver="2025.02.19"
        )
        ...
        func_name = self._extract_n_function_name(
            jscode, player_url=player_url
        )

        # XXX: Workaround for the `typeof` gotcha
        func_code = self._fixup_n_function_code(
            *jsi.extract_function_code(func_name)
        )
    ...
    def _extract_signature_timestamp(
        self, video_id, player_url, ytcfg=None, fatal=False
    ):
        """
    ...
    def _mark_watched(self, video_id, player_responses):
        for is_full, key in enumerate(
            ("videostatsPlaybackUrl", "videostatsWatchtimeUrl")
        ):
            label = "fully " if is_full else ""
            url = get_first(
                player_responses,
                ("playbackTracking", key, "baseUrl"),
            ...
            cpn = "".join(
                CPN_ALPHABET[random.randint(0, 256) & 63] for _ in range(16)
            )
    ...
    def _extract_chapters_from_json(self, data, duration):
        ...
        return self._extract_chapters_helper(
            ...
            start_function=lambda chapter: float_or_none(
                traverse_obj(
                    chapter, ("chapterRenderer", "timeRangeStartMillis")
                ),
                scale=1000,
            ),
    ...
    def _extract_chapters_from_engagement_panel(self, data, duration):
        ...
        chapter_time = lambda chapter: parse_duration(
            self._get_text(chapter, "timeDescription")
        )
        ...
        return (
            traverse_obj(
                ...
                (
                    ...
                    "entityBatchUpdate",
                    "mutations",
                    lambda _, v: v["payload"]["macroMarkersListEntity"][
                        "markersList"
                    ]["markerType"]
                    == "MARKER_TYPE_HEATMAP",
                    "payload",
    ...
    def _extract_comment(self, entities, parent=None):
        comment_entity_payload = get_first(
            entities, ("payload", "commentEntityPayload", {dict})
        )
        if not (
            comment_id := traverse_obj(
                comment_entity_payload, ("properties", "commentId", {str})
            )
        ):
            return

        toolbar_entity_payload = get_first(
            entities,
            ("payload", "engagementToolbarStateEntityPayload", {dict}),
        )
        time_text = (
            traverse_obj(
                comment_entity_payload, ("properties", "publishedTime", {str})
            )
            or ""
        )
        ...
        return {
            ...
            **traverse_obj(
            ...
            "is_favorited": (
                ...
                else toolbar_entity_payload.get("heartState")
    ...
    def _extract_comment_old(self, comment_renderer, parent=None):
        ...
        author_is_uploader = traverse_obj(
            comment_renderer, "authorIsChannelOwner"
        )
        ...
        badges = self._extract_badges(
            [traverse_obj(comment_renderer, "authorCommentBadge")]
        )
    ...
    def _comment_entries(
        ...
        def extract_header(contents):
            ...
            for content in contents:
                comments_header_renderer = traverse_obj(
                    content, "commentsHeaderRenderer"
                )
                ...
                comment_sort_index = int(
                    get_single_config_arg("comment_sort") != "top"
                )  # 1 = new, 0 = top

                sort_menu_item = (
                    try_get(
                        comments_header_renderer,
                        lambda x: x["sortMenu"]["sortFilterSubMenuRenderer"][
                            "subMenuItems"
                        ][comment_sort_index],
                        dict,
                    )
                ...
                sort_continuation_ep = (
                    sort_menu_item.get("serviceEndpoint") or {}
                )

                _continuation = self._extract_continuation_ep_data(
                    sort_continuation_ep
                ) or self._extract_continuation(sort_menu_item)
        ...
        def extract_thread(contents, entity_payloads):
            ...
            for content in contents:
                if (
                    not parent
                    and tracker["total_parent_comments"] >= max_parents
                ):
                    yield
                comment_thread_renderer = try_get(
                    content, lambda x: x["commentThreadRenderer"]
                )
                ...
                if not entity_payloads:
                    ...
                    comment = self._extract_comment_old(
                        comment_renderer, parent
                    )
                ...
                # new comment format
                    ...
                    comment_keys = traverse_obj(
                        view_model, (("commentKey", "toolbarStateKey"), {str})
                    )
                    if not comment_keys:
                        continue
                    entities = traverse_obj(
                        entity_payloads,
                        lambda _, v: v["entityKey"] in comment_keys,
                    )
                    comment = self._extract_comment(entities, parent)
                    if comment:
                        comment["is_pinned"] = (
                            traverse_obj(view_model, ("pinnedText", {str}))
                            is not None
                        )
                ...
                if comment_replies_renderer:
                    ...
                    yield from itertools.islice(
                        ...
                        min(
                            ...
                            max(
                                0,
                                max_replies - tracker["total_reply_comments"],
                            ),
                        ),
                    )
        ...
        max_comments, max_parents, max_replies, max_replies_per_thread, *_ = (
            int_or_none(p, default=sys.maxsize)
            for p in self._configuration_arg("max_comments") + [""] * 4
        )
        ...
        if is_first_continuation and not continuation:
            ...
            continuation = self._build_api_continuation_query(
                self._generate_comment_continuation(video_id)
            )
        ...
        for page_num in itertools.count(0):
            ...
            headers = self.generate_api_headers(
                ytcfg=ytcfg, visitor_data=self._extract_visitor_data(response)
            )
            comment_prog_str = (
            ...
            if page_num == 0:
                ...
                note_prefix = (
                    "{}Downloading comment{} API JSON page {} {}".format(
                        "       " if parent else "",
            ...
            check_get_keys = None
            if not is_forced_continuation and not (
                tracker["est_total"] == 0 and tracker["running_total"] == 0
            ):
                check_get_keys = [
                    [
                        *continuation_items_path,
            ...
            try:
                ...
                if "incomplete data" in str(e).lower() and parent:
                    if self.get_param("ignoreerrors") in (
                        True,
                        "only_download",
                    ):
                        self.report_warning(
            ...
            for continuation_items in traverse_obj(
                response,
                continuation_items_path,
                expected_type=list,
                default=[],
        ...
        message = self._get_text(
            root_continuation_data,
            ("contents", ..., "messageRenderer", "text"),
            max_runs=1,
        )
        if message and not parent and tracker["running_total"] == 0:
            self.report_warning(
                f"Youtube said: {message}", video_id=video_id, only_once=True
            )
    ...
    def _get_comments(self, ytcfg, video_id, contents, webpage):
            ...
            renderer = next(
                (
                    ...
                    for item in traverse_obj(
                        contents, (..., "itemSectionRenderer"), default={}
                    )
        ...
        max_comments = int_or_none(
            self._configuration_arg("max_comments", [""])[0]
        )
        return itertools.islice(
            _real_comment_extract(contents), 0, max_comments
        )
    ...
    def _get_config_po_token(self, client: str, context: _PoTokenContext):
        po_token_strs = self._configuration_arg(
            "po_token", [], ie_key=YoutubeIE, casesense=True
        )
        for token_str in po_token_strs:
            ...
            if not sep:
                self.report_warning(
                    ...
                    f'Expected "CLIENT.CONTEXT+PO_TOKEN", got "{token_str}"',
                    only_once=True,
                )
                continue
            ...
            try:
                return base64.urlsafe_b64encode(
                    base64.urlsafe_b64decode(urllib.parse.unquote(po_token))
                ).decode()
    ...
    def _is_agegated(player_response):
        ...
        reasons = traverse_obj(
            player_response, ("playabilityStatus", ("status", "reason"))
        )
        AGE_GATE_REASONS = (
            "confirm your age",
            "age-restricted",
    ...
    def _is_unplayable(player_response):
        return (
            traverse_obj(player_response, ("playabilityStatus", "status"))
            == "UNPLAYABLE"
        )
    ...
    def _extract_player_response(
        ...
        headers = self.generate_api_headers(
            ...
            visitor_data=visitor_data,
            session_index=self._extract_session_index(
                master_ytcfg, player_ytcfg
            ),
            delegated_session_id=(
                self._parse_data_sync_id(data_sync_id)[0]
                or self._extract_delegated_session_id(
                    master_ytcfg, initial_pr, player_ytcfg
                )
            ),
            user_session_id=(
                self._parse_data_sync_id(data_sync_id)[1]
                or self._extract_user_session_id(
                    master_ytcfg, initial_pr, player_ytcfg
                )
            ),
        )
        ...
        if player_params := self._configuration_arg(
            "player_params", [default_pp], casesense=True
        )[0]:
        ...
        sts = (
            self._extract_signature_timestamp(
                video_id, player_url, master_ytcfg, fatal=False
            )
            if player_url
            else None
        )
        ...
        return (
            self._extract_response(
                ...
                fatal=True,
                default_client=client,
                note="Downloading {} player API JSON".format(
                    client.replace("_", " ").strip()
                ),
            )
            or None
        )
    ...
    def _get_requested_clients(self, url, smuggled_data):
        ...
        if not requested_clients:
            raise ExtractorError(
                "No player clients have been requested", expected=True
            )

        if self.is_authenticated:
            if (
                smuggled_data.get("is_music_url") or self.is_music_url(url)
            ...
            for client in unsupported_clients:
                self.report_warning(
                    f'Skipping client "{client}" since it does not support cookies',
                    only_once=True,
                )
    ...
    def _extract_player_responses(
        self, clients, video_id, webpage, master_ytcfg, smuggled_data
    ):
        initial_pr = None
        if webpage:
        ...
        if initial_pr and not self._invalid_player_response(
            initial_pr, video_id
        ):
        ...
        while clients:
            ...
            client, base_client, variant = _split_innertube_client(
                clients.pop()
            )
            player_ytcfg = master_ytcfg if client == "web" else {}
            if (
                "configs" not in self._configuration_arg("player_skip")
                and client != "web"
            ):
                player_ytcfg = (
                    self._download_ytcfg(client, video_id) or player_ytcfg
                )

            player_url = player_url or self._extract_player_url(
                master_ytcfg, player_ytcfg, webpage=webpage
            )
            ...
            visitor_data = visitor_data or self._extract_visitor_data(
                master_ytcfg, initial_pr, player_ytcfg
            )
            data_sync_id = data_sync_id or self._extract_data_sync_id(
                master_ytcfg, initial_pr, player_ytcfg
            )
            ...
            fetch_po_token_args = {
                ...
                "video_id": video_id,
                "data_sync_id": (
                    data_sync_id if self.is_authenticated else None
                ),
            ...
            required_pot_contexts = self._get_default_ytcfg(client)[
            ...
            if pr_id := self._invalid_player_response(pr, video_id):
                ...
                for f in traverse_obj(
                    sd, (("formats", "adaptiveFormats"), ..., {dict})
                ):
    ...
    def _extract_formats_and_subtitles(
        self, streaming_data, video_id, player_url, live_status, duration
    ):
        CHUNK_SIZE = 10 << 20
        ...
        streaming_formats = traverse_obj(
            streaming_data, (..., ("formats", "adaptiveFormats"), ...)
        )
        ...
        for fmt in streaming_formats:
            ...
            if language_code and (
                is_original or (is_default and not original_language)
            ):
            ...
            if query.get("n"):
                try:
                    decrypt_nsig = self._cached(
                        self._decrypt_nsig, "nsig", query["n"][0]
                    )
            ...
            format_duration = traverse_obj(
                fmt, ("approxDurationMs", {float_or_none(scale=1000)})
            )
            ...
            require_po_token = (
                not po_token
                and _PoTokenContext.GVS
                in self._get_default_ytcfg(client_name)[
                    "PO_TOKEN_REQUIRED_CONTEXTS"
                ]
                and itag not in ["18"]
            )  # these formats do not require PO Token

            if (
                require_po_token
                and "missing_pot" not in self._configuration_arg("formats")
            ):
            ...
            dct = {
                ...
                "format_note": join_nonempty(
                    join_nonempty(
                        display_name, is_default and " (default)", delim=""
                    ),
                    name,
                    fmt.get("isDrc") and "DRC",
                    try_get(
                        fmt,
                        lambda x: x["projectionType"]
                        .replace("RECTANGULAR", "")
                        .lower(),
                    ),
                    try_get(
                        fmt,
                        lambda x: x["spatialAudioType"]
                        .replace("SPATIAL_AUDIO_TYPE_", "")
                        .lower(),
                    ),
                    is_damaged and "DAMAGED",
                    require_po_token and "MISSING POT",
                    (self.get_param("verbose") or all_formats)
                ...
                "language": join_nonempty(
                    language_code, "desc" if is_descriptive else ""
                )
                or None,
                "language_preference": (
                    PREFERRED_LANG_VALUE
                    if is_original
                    else 5 if is_default else -10 if is_descriptive else -1
                ),
                # Strictly de-prioritize broken, damaged and 3gp formats
                "preference": (
                    -20
                    if require_po_token
                    else -10 if is_damaged else -2 if itag == "17" else None
                ),
            }
            mime_mobj = re.match(
                r'((?:[^/]+)/(?:[^;]+))(?:;\s*codecs="([^"]+)")?',
        ...
        needs_live_processing = self._needs_live_processing(
        ...
        if self._configuration_arg("include_incomplete_formats"):
            ...
            self._downloader.deprecated_feature(
                "[youtube] include_incomplete_formats extractor argument is deprecated. "
                "Use formats=incomplete extractor argument instead"
            )

        skip_manifests = set(self._configuration_arg("skip"))
        if (
            not self.get_param("youtube_include_hls_manifest", True)
            or needs_live_processing
        ...
        if self._configuration_arg("include_live_dash"):
            self._downloader.deprecated_feature(
                "[youtube] include_live_dash extractor argument is deprecated. "
                "Use formats=incomplete extractor argument instead"
            )
        elif (
            skip_bad_formats
        ...
        def process_manifest_format(f, proto, client_name, itag, po_token):
            ...
            if (
                ...
                if "missing_pot" not in self._configuration_arg("formats"):
                    self._report_pot_format_skipped(
                        video_id, client_name, proto
                    )
                    return False
                f["format_note"] = join_nonempty(
                    f.get("format_note"), "MISSING POT", delim=" "
                )
            ...
            if original_language and f.get("language") == original_language:
                f["format_note"] = join_nonempty(
                    f.get("format_note"), "(default)", delim=" "
                )
                f["language_preference"] = PREFERRED_LANG_VALUE

            if itag in ("616", "235"):
                f["format_note"] = join_nonempty(
                    f.get("format_note"), "Premium", delim=" "
                )
                f["source_preference"] += 100

            f["quality"] = q(
                itag_qualities.get(
                    try_get(f, lambda f: f["format_id"].split("-")[0]), -1
                )
            )
            if f["quality"] == -1 and f.get("height"):
                f["quality"] = q(
                    res_qualities[
                        min(res_qualities, key=lambda x: abs(x - f["height"]))
                    ]
                )
            if self.get_param("verbose") or all_formats:
                f["format_note"] = join_nonempty(
                    f.get("format_note"),
                    short_client_name(client_name),
                    delim=", ",
                )
        ...
        for sd in streaming_data:
            ...
            if hls_manifest_url:
                if po_token:
                    hls_manifest_url = (
                        hls_manifest_url.rstrip("/") + f"/pot/{po_token}"
                    )
            ...
            if dash_manifest_url:
                if po_token:
                    dash_manifest_url = (
                        dash_manifest_url.rstrip("/") + f"/pot/{po_token}"
                    )
                ...
                for f in formats:
                    if process_manifest_format(
                        f, "dash", client_name, f["format_id"], po_token
                    ):
    ...
    def _extract_storyboard(self, player_responses, duration):
        ...
        base_url = url_or_none(
            urljoin("https://i.ytimg.com/", spec.pop() or None)
        )
        if not base_url:
            return
        L = len(spec) - 1
        for i, args in enumerate(spec):
            ...
            url = (
                base_url.replace("$L", str(L - i)).replace("$N", N)
                + f"&sigh={sigh}"
            )
    ...
    def _download_player_responses(
        self, url, smuggled_data, video_id, webpage_url
    ):
        webpage = None
        if "webpage" not in self._configuration_arg("player_skip"):
            query = {"bpctr": "9999999999", "has_verified": "1"}
            pp = self._configuration_arg(
                "player_params", [None], casesense=True
            )[0]
            if pp:
                query["pp"] = pp
            webpage = self._download_webpage_with_retries(
    ...
    def _list_formats(
        ...
        live_broadcast_details = traverse_obj(
            microformats, (..., "liveBroadcastDetails")
        )
    ...
    def _real_extract(self, url):
        ...
        webpage, master_ytcfg, player_responses, player_url = (
            self._download_player_responses(
                url, smuggled_data, video_id, webpage_url
            )
        )
        ...
        video_details = traverse_obj(
            player_responses, (..., "videoDetails"), expected_type=dict
        )
        microformats = traverse_obj(
            player_responses,
            (..., "microformat", "playerMicroformatRenderer"),
        ...
        translated_description = self._get_text(
            microformats, (..., "description")
        )
        ...
        if multifeed_metadata_list and not smuggled_data.get(
            "force_singlefeed"
        ):
            ...
            if subreason:
                if (
                    ...
                    self.raise_geo_restricted(
                        subreason, countries, metadata_available=True
                    )
                reason += f". {subreason}"
            if reason:
        ...
        keywords = (
            get_first(video_details, "keywords", expected_type=list) or []
        )
        ...
        thumbnails = self._extract_thumbnails(
            (video_details, microformats), (..., ..., "thumbnail")
        )
        ...
        for thumb in thumbnails:
            i = next(
                (
                    i
                    for i, t in enumerate(thumbnail_names)
                    if f"/{video_id}/{t}" in thumb["url"]
                ),
                n_thumbnail_names,
            )
        ...
        live_start_time = parse_iso8601(
            get_first(live_broadcast_details, "startTimestamp")
        )
        live_end_time = parse_iso8601(
            get_first(live_broadcast_details, "endTimestamp")
        )
        ...
        for fmt in filter(is_bad_format, formats):
            ...
            fmt["format_note"] = join_nonempty(
                fmt.get("format_note"), "(Last 2 hours)", delim=" "
            )
        ...
        info = {
            ...
            "average_rating": float_or_none(
                get_first(video_details, "averageRating")
            ),
            "age_limit": (
                18
                if (
            ...
            "tags": keywords,
            "playable_in_embed": get_first(
                playability_statuses, "playableInEmbed"
            ),
            "live_status": live_status,
            "media_type": (
                "livestream"
        ...
        if pctr:
            ...
            translation_languages = {
                lang.get("languageCode"): self._get_text(
                    lang.get("languageName"), max_runs=1
                )
                for lang in traverse_obj(
                    pctr, (..., "translationLanguages", ...)
                )
            }

            def process_language(
                container, base_url, lang_code, sub_name, query
            ):
                    ...
                    lang_subs.append(
                        {
                            ...
                            "url": urljoin(
                                "https://www.youtube.com",
                                update_url_query(base_url, query),
                            ),
                            "name": sub_name,
                        }
                    )
            ...
            get_translated_subs = (
                "translated_subs" not in self._configuration_arg("skip")
                and (
                    self.get_param("writeautomaticsub", False)
                ...
                info.update(
                    {
                        ...
                        "artists": (
                            [a]
                            if (a := mobj.group("clean_artist"))
                            else [
                                a.strip()
                                for a in mobj.group("artist").split("·")
                            ]
                        ),
        ...
        initial_data = None
        if webpage:
            initial_data = self.extract_yt_initial_data(
                video_id, webpage, fatal=False
            )
        ...
        if initial_data:
            info["chapters"] = (
                ...
                or self._extract_chapters_from_engagement_panel(
                    initial_data, duration
                )
                or self._extract_chapters_from_description(
        ...
        if vpir:
            ...
            if stl:
                ...
                if (
                    try_get(vpir, lambda x: x["superTitleIcon"]["iconType"])
                    == "LOCATION_PIN"
                ):
                    info["location"] = stl
                else:
            ...
            for tlb in (
                try_get(
                    vpir,
                    lambda x: x["videoActions"]["menuRenderer"][
                        "topLevelButtons"
                    ],
                    list,
                )
                ...
                for tbr in tbrs:
                    for getter, regex in [
                        (
                            lambda x: x["defaultText"]["accessibility"][
                                "accessibilityData"
                            ],
        ...
        if vsir:
            ...
            info.update(
                {
                    ...
                    "channel_follower_count": self._get_count(
                        vor, "subscriberCountText"
                    ),
                }
            )
            ...
            rows = (
                try_get(
                    vsir,
                    lambda x: x["metadataRowContainer"][
            ...
            for row in rows:
                if (
                    try_get(
                        row,
                        lambda x: x["metadataRowRenderer"]["hasDividerLine"],
                    )
                    is True
                ):
                    multiple_songs = True
            ...
            for row in rows:
                ...
                if mrr_title == "License":
                    ...
                    if mrr_title == "Album":
                        ...
                        info["artists"] = (
                            [mrr_contents_text] if mrr_contents_text else None
                        )
                    elif mrr_title == "Song":
                        info["track"] = mrr_contents_text
            owner_badges = self._extract_badges(
                traverse_obj(vsir, ("owner", "videoOwnerRenderer", "badges"))
            )
            if self._has_badge(owner_badges, BadgeType.VERIFIED):
        ...
        upload_date = (
            dt.datetime.fromtimestamp(timestamp, dt.timezone.utc).strftime(
                "%Y%m%d"
            )
            if timestamp
            else (
        ...
        if not upload_date or (
            not timestamp and live_status in ("not_live", None)
        ):
            # this should be in UTC, as configured in the cookie/client context
            upload_date = (
                strftime_or_none(
                    self._parse_time_text(self._get_text(vpir, "dateText"))
                )
                or upload_date
            )
        ...
        if upload_date and live_status not in (
            "is_live",
            "post_live",
            "is_upcoming",
        ):
            # Newly uploaded videos' HLS formats are potentially problematic and need to be checked
            upload_datetime = datetime_from_str(upload_date).replace(
            ...
            if self._has_badge(badges, BadgeType.AVAILABILITY_PUBLIC)
                ...
                is_unlisted=(
                    ...
                    if is_private is None
                        ...
                        or get_first(
                            microformats, "isUnlisted", expected_type=bool
                        )
                    )
                ),
            )
        ...
        info["__post_extractor"] = self.extract_comments(
            master_ytcfg, video_id, contents, webpage
        )
...
class YoutubeTabBaseInfoExtractor(YoutubeBaseInfoExtractor):
    ...
    def _extract_channel_renderer(self, renderer):
        ...
        return {
            ...
            "playlist_count": (
                ...
                if self._get_count(renderer, "subscriberCountText") is not None
                else None
            ),
            "description": self._get_text(renderer, "descriptionSnippet"),
            "channel_is_verified": (
    ...
    def _shelf_entries(self, shelf_renderer, skip_channels=False):
        ep = try_get(
            shelf_renderer,
            lambda x: x["endpoint"]["commandMetadata"]["webCommandMetadata"][
                "url"
            ],
            str,
        )
        ...
        if entity_id:
            ...
            yield self.url_result(
                ...
                **traverse_obj(
                                ...
                                (
                                    "accessibilityText",
                                    {
                                        lambda x: re.fullmatch(
                                            r"(.+), (?:[\d,.]+(?:[KM]| million)?|No) views? - play Short",
                                            x,
                                        )
                                    },
                                    1,
                ...
                thumbnails=self._extract_thumbnails(
                    renderer, "thumbnail", final_key="sources"
                ),
            )
            return
    ...
    def _hashtag_tile_entry(self, hashtag_tile_renderer):
        ...
        if url:
            return self.url_result(
                url,
                ie=YoutubeTabIE.ie_key(),
                title=self._get_text(hashtag_tile_renderer, "hashtag"),
            )
    ...
    def _post_thread_entries(self, post_thread_renderer):
        ...
        playlist_id = try_get(
            post_renderer,
            lambda x: x["backstageAttachment"]["playlistRenderer"][
                "playlistId"
            ],
            str,
        )
        ...
        # inline video links
        runs = (
            try_get(post_renderer, lambda x: x["contentText"]["runs"], list)
            or []
        )
        for run in runs:
            ...
            ep_url = try_get(
                run,
                lambda x: x["navigationEndpoint"]["urlEndpoint"]["url"],
                str,
            )
            if not ep_url:
                continue
            ...
            if video_id == ep_video_id:
                continue
            yield self.url_result(
                ep_url, ie=YoutubeIE.ie_key(), video_id=ep_video_id
            )
    ...
    def _report_history_entries(self, renderer):
        for url in traverse_obj(
            ...
            (
                "rows",
                ...,
                "reportHistoryTableRowRenderer",
                "cells",
                ...,
                "reportHistoryTableCellRenderer",
                "cell",
                "reportHistoryTableTextCellRenderer",
                "text",
                "runs",
                ...,
                "navigationEndpoint",
            ...
            yield self.url_result(
                urljoin("https://www.youtube.com", url), YoutubeIE
            )
    ...
    def _extract_entries(self, parent_renderer, continuation_list):
        ...
        contents = (
            try_get(parent_renderer, lambda x: x["contents"], list) or []
        )
        for content in contents:
            if not isinstance(content, dict):
                continue
            ...
            isr_contents = (
                try_get(is_renderer, lambda x: x["contents"], list) or []
            )
            for isr_content in isr_contents:
                ...
                known_renderers = {
                    ...
                    "musicResponsiveListItemRenderer": lambda x: [
                        self._music_reponsive_list_entry(x)
                    ],
                    "backstagePostThreadRenderer": self._post_thread_entries,
                    "videoRenderer": lambda x: [self._video_entry(x)],
                    "playlistRenderer": lambda x: self._grid_entries(
                        {"items": [{"playlistRenderer": x}]}
                    ),
                    "channelRenderer": lambda x: self._grid_entries(
                        {"items": [{"channelRenderer": x}]}
                    ),
                    "hashtagTileRenderer": lambda x: [
                        self._hashtag_tile_entry(x)
                    ],
                    "richGridRenderer": lambda x: self._extract_entries(
                        x, continuation_list
                    ),
                    "lockupViewModel": lambda x: [
                        self._extract_lockup_view_model(x)
                    ],
                }
    ...
    def _entries(
        self, tab, item_id, ytcfg, delegated_session_id, visitor_data
    ):
        continuation_list = [None]
        ...
        parent_renderer = (
            ...
            or try_get(tab_content, lambda x: x["richGridRenderer"], dict)
            or {}
        )
        ...
        for page_num in itertools.count(1):
            ...
            if (
                ...
                and continuation_token in seen_continuations
            ):
                self.write_debug(
            ...
            known_renderers = {
                ...
                "backstagePostThreadRenderer": (
                    self._post_thread_continuation_entries,
                    "contents",
                ),
                "reportHistoryTableRowRenderer": (
                    self._report_history_entries,
                    "rows",
                ),
    ...
    def _extract_from_tabs(self, item_id, ytcfg, data, tabs):
        ...
        metadata["title"] += format_field(
            selected_tab, "expandedText", " - %s"
        )
    ...
    def _extract_metadata_from_tabs(self, item_id, data):
        ...
        info.update(
            {
                "title": (
                    traverse_obj(metadata_renderer, "title")
                    or self._get_text(
                        data, ("header", "hashtagHeaderRenderer", "hashtag")
                    )
                    or info["id"]
                ),
                ...
                "channel_follower_count": (
                    self._get_count(
                        data, ("header", ..., "subscriberCountText")
                    )
                    or traverse_obj(
                        page_header_view_model,
                        (
                            ...
                            ...,
                            "metadataParts",
                            lambda _, v: "subscribers" in v["text"]["content"],
                            "text",
                            "content",
                            {parse_count},
                            any,
                ...
                "description": try_get(
                    metadata_renderer, lambda x: x.get("description", "")
                ),
                "tags": (
                    traverse_obj(
                        data,
                ...
                "thumbnails": (primary_thumbnails or playlist_thumbnails)
                + avatar_thumbnails
                + channel_banners,
            }
        )
        ...
        channel_badges = self._extract_badges(
            traverse_obj(data, ("header", ..., "badges"), get_all=False)
        )
        ...
        playlist_stats = get_first(
            (primary_sidebar_renderer, playlist_header_renderer),
        ...
        if not info.get("channel_id"):
            ...
            browse_ep = (
                traverse_obj(
                    owner, ("runs", 0, "navigationEndpoint", "browseEndpoint")
                )
                or {}
            )
            info.update(
    ...
    def _extract_inline_playlist(self, playlist, playlist_id, data, ytcfg):
        ...
        for page_num in itertools.count(1):
            ...
            start = (
                next(
                    (i for i, v in enumerate(videos) if v["id"] == last_id), -1
                )
                + 1
            )
            if start >= len(videos):
            ...
            watch_endpoint = try_get(
                playlist,
                lambda x: x["contents"][-1]["playlistPanelVideoRenderer"][
                    "navigationEndpoint"
                ]["watchEndpoint"],
            )
            headers = self.generate_api_headers(
                ytcfg=ytcfg,
                delegated_session_id=self._extract_delegated_session_id(
                    ytcfg, data
                ),
    ...
    def _extract_from_playlist(self, item_id, url, data, playlist, ytcfg):
        ...
        playlist_url = urljoin(
            ...
            try_get(
                playlist,
                lambda x: x["endpoint"]["commandMetadata"][
                    "webCommandMetadata"
                ]["url"],
                str,
            ),
    ...
    def _extract_availability(self, data):
        ...
        playlist_header_renderer = (
            traverse_obj(data, ("header", "playlistHeaderRenderer")) or {}
        )
        ...
        privacy_setting_icon = get_first(
            ...
            (
                ...
                "dropdownRenderer",
                "entries",
                lambda _, v: v["privacyDropdownItemRenderer"]["isSelected"],
        ...
        return (
            ...
            if (
                ...
                is_private=(
                    ...
                    or player_header_privacy == "PRIVATE"
                    if player_header_privacy is not None
                    else (
                        privacy_setting_icon == "PRIVACY_PRIVATE"
                        if privacy_setting_icon is not None
                        else None
                    )
                ),
                is_unlisted=(
                    self._has_badge(badges, BadgeType.AVAILABILITY_UNLISTED)
                    or player_header_privacy == "UNLISTED"
                    if player_header_privacy is not None
                    else (
                        privacy_setting_icon == "PRIVACY_UNLISTED"
                        if privacy_setting_icon is not None
                        else (
                            microformats_is_unlisted
                            if microformats_is_unlisted is not None
                            else None
                        )
                    )
                ),
    ...
    def _extract_webpage(self, url, item_id, fatal=True):
        ...
        for retry in self.RetryManager(fatal=fatal):
            try:
                ...
                data = (
                    self.extract_yt_initial_data(
                        item_id, webpage or "", fatal=fatal
                    )
                    or {}
                )
            except ExtractorError as e:
    ...
    def _report_playlist_authcheck(self, ytcfg, fatal=True):
        ...
        if not ytcfg and self.is_authenticated:
            ...
            if (
                "authcheck"
                not in self._configuration_arg(
                    "skip", ie_key=YoutubeTabIE.ie_key()
                )
                and fatal
            ):
    ...
    def _extract_data(
        ...
        if not self.skip_webpage:
            ...
            selected_tab = (
                self._extract_selected_tab(
                    self._extract_tab_renderers(data), fatal=False
                )
                or {}
            )
            if (
                url != "https://www.youtube.com/feed/recommended"
                and selected_tab.get("tabIdentifier")
                == "FEwhat_to_watch"  # Home page
                and "no-youtube-channel-redirect"
                not in self.get_param("compat_opts", [])
            ):
    ...
    def _extract_tab_endpoint(
        self, url, item_id, ytcfg=None, fatal=True, default_client="web"
    ):
        headers = self.generate_api_headers(
        ...
        for ep_key, ep in endpoints.items():
            params = try_get(
                resolve_response, lambda x: x["endpoint"][ep_key], dict
            )
            if params:
                return self._extract_response(
                    item_id=item_id,
    ...
    def _search_results(self, query, params=NO_DEFAULT, default_client="web"):
        ...
        ytcfg = (
            self._download_ytcfg(default_client, display_id)
            if not self.skip_webpage
            else {}
        )
        ...
        for page_num in itertools.count(1):
            ...
            yield from self._extract_entries(
                {"contents": list(variadic(slr_contents))}, continuation_list
            )
            if not continuation_list[0]:
                break
...
class YoutubeTabIE(YoutubeTabBaseInfoExtractor):
    ...
    _TESTS = [
        ...
        },
        {
            ...
            "info_dict": {
                ...
                "categories": ["News & Politics"],
                "tags": [
                    "Cenk Uygur (TV Program Creator)",
                    "The Young Turks (Award-Winning Work)",
                    "Talk Show (TV Genre)",
                ],
                "like_count": int,
            },
    ...
    def _extract_tab_id_and_name(
        ...
        tab_id = (
            tab_url and self._get_url_mobj(tab_url)["tab"][1:]
    ...
    def _has_tab(self, tabs, tab_id):
        return any(
            self._extract_tab_id_and_name(tab)[0] == tab_id for tab in tabs
        )
    ...
    def _real_extract(self, url, smuggled_data):
        ...
        qs = parse_qs(url)
        video_id, playlist_id = (
            traverse_obj(qs, (key, 0)) for key in ("v", "list")
        )
        if not video_id and mobj["not_channel"].startswith("watch"):
            if not playlist_id:
                # If there is neither video or playlist ids, youtube redirects to home page, which is undesirable
                raise ExtractorError(
                    "A video URL was given without video ID", expected=True
                )
        ...
        if "no-youtube-unavailable-videos" not in compat_opts:
            data = (
                self._reload_with_unavailable_videos(display_id, data, ytcfg)
                or data
            )
        ...
        if self.get_param("playlist_items") == "0":
            entries.extend(
                self.url_result(u, YoutubeTabIE) for u in extra_tabs
            )
...
class YoutubePlaylistIE(YoutubeBaseInfoExtractor):
    ...
    _TESTS = [
        ...
        },
        {
            ...
            "expected_warnings": [
                r"[Uu]navailable videos? (is|are|will be) hidden",
                "Retrying",
                "Giving up",
            ],
        },
    ...
    def _real_extract(self, url):
        ...
        return self.url_result(
            url, ie=YoutubeTabIE.ie_key(), video_id=playlist_id
        )
...
class YoutubeYtUserIE(YoutubeBaseInfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        return self.url_result(
            f"https://www.youtube.com/user/{user_id}", YoutubeTabIE, user_id
        )
...
class YoutubeNotificationsIE(YoutubeTabBaseInfoExtractor):
    ...
    def _extract_notification_menu(self, response, continuation_list):
        ...
        for item in notification_list:
            entry = self._extract_notification_renderer(
                item.get("notificationRenderer")
            )
            if entry:
                yield entry
    ...
    def _extract_notification_renderer(self, notification):
        ...
        if not video_id:
            ...
            channel_id = self.ucid_or_none(
                traverse_obj(browse_ep, "browseId", expected_type=str)
            )
            post_id = self._search_regex(
                r"/post/(.+)",
                traverse_obj(browse_ep, "canonicalBaseUrl", expected_type=str),
                "post id",
                default=None,
            )
        ...
        title = self._search_regex(
            rf'{re.escape(channel or "")}[^:]+: (.+)',
            notification_title,
            "video title",
            default=None,
        )
        timestamp = (
            self._parse_time_text(self._get_text(notification, "sentTimeText"))
    ...
    def _notification_menu_entries(self, ytcfg):
        ...
        for page in itertools.count(1):
            ...
            response = self._extract_response(
                item_id=f"page {page}",
                query={"ctoken": ctoken} if ctoken else {},
                ytcfg=ytcfg,
                ep="notification/get_notification_menu",
                check_get_keys="actions",
                headers=self.generate_api_headers(
                    ytcfg=ytcfg,
                    visitor_data=self._extract_visitor_data(response),
                ),
            )
            yield from self._extract_notification_menu(
                response, continuation_list
            )
            if not continuation_list[0]:
                break
    ...
    def _real_extract(self, url):
        ...
        ytcfg = (
            self._download_ytcfg("web", display_id)
            if not self.skip_webpage
            else {}
        )
        self._report_playlist_authcheck(ytcfg)
        return self.playlist_result(
            self._notification_menu_entries(ytcfg), display_id, display_id
        )
...
class YoutubeSearchURLIE(YoutubeTabBaseInfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._search_results(query, qs.get("sp", (None,))[0]), query, query
        )
...
class YoutubeRecommendedIE(YoutubeFeedsInfoExtractor):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?youtube\.com/?(?:[?#]|$)|:ytrec(?:ommended)?"
    )
...
class YoutubeShortsAudioPivotIE(YoutubeBaseInfoExtractor):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?youtube\.com/source/(?P<id>[\w-]{11})/shorts"
    )
    _TESTS = [
        {
...
class YoutubeConsentRedirectIE(YoutubeBaseInfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if not redirect_url:
            raise ExtractorError(
                "Invalid cookie consent redirect URL", expected=True
            )
