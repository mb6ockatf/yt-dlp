...
class VoxMediaVolumeIE(OnceIE):
    ...
    def _real_extract(self, url):
        ...
        setup = self._parse_json(
            self._search_regex(r"setup\s*=\s*({.+});", webpage, "setup"),
            video_id,
        )
        ...
        info = {
            "id": video_id,
            "title": player_setup.get("title")
            or video_data.get("title_short"),
            "description": video_data.get("description_long")
            or video_data.get("description_short"),
            "thumbnail": formatted_metadata.get("thumbnail")
            or video_data.get("brightcove_thumbnail"),
            "timestamp": unified_timestamp(
                formatted_metadata.get("video_publish_date")
            ),
        }
        asset = (
            try_get(setup, lambda x: x["embed_assets"]["chorus"], dict) or {}
        )
                ...
                info.update(
                    {
                        ...
                        "url": (
                            provider_video_id
                            if provider_video_type == "youtube"
                            else f"{provider_video_type}:{provider_video_id}"
                        ),
