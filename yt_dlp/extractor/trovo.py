    ...
    def _extract_streamer_info(self, data):
        ...
        return {
            ...
            "uploader_url": format_field(
                username, None, "https://trovo.live/%s"
            ),
        }
...
class TrovoIE(TrovoBaseIE):
    _VALID_URL = (
        TrovoBaseIE._VALID_URL_BASE
        + r"(?:s/)?(?!(?:clip|video)/)(?P<id>(?!s/)[^/?&#]+(?![^#]+[?&]vid=))"
    )
    _TESTS = [
        {
            ...
            formats.append(
                {
                    "format_id": format_id,
                    "height": (
                        int_or_none(format_id[:-1]) if format_id else None
                    ),
                    "url": play_url,
...
class TrovoVodIE(TrovoBaseIE):
    ...
    def _real_extract(self, url):
        ...
        vod_detail_info = traverse_obj(
            resp, ("VodDetailInfos", vid), expected_type=dict
        )
        if not vod_detail_info:
            raise ExtractorError(
                "This video not found or not available anymore", expected=True
            )
        ...
        if try_get(
            vod_info,
            lambda x: x["playbackRights"]["playbackRights"] != "Normal",
        ):
            playback_rights_setting = vod_info["playbackRights"][
                "playbackRightsSetting"
            ]
...
class TrovoChannelBaseIE(TrovoBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._entries(spacename), playlist_id=spacename
        )
