...
class JioSaavnBaseIE(InfoExtractor):
    ...
    def _extract_song(self, song_data, url=None):
        info = traverse_obj(
            song_data,
            {
                ...
                "thumbnail": (
                    "image",
                    {url_or_none},
                    {lambda x: re.sub(r"-\d+x\d+\.", "-500x500.", x)},
                ),
        ...
        if webpage_url := info.get("webpage_url") or url:
            ...
            info["_old_archive_ids"] = [
                make_archive_id(JioSaavnSongIE, info["display_id"])
            ]
    ...
    def _yield_songs(self, playlist_data):
        for song_data in traverse_obj(
            playlist_data, ("songs", lambda _, v: v["id"] and v["perma_url"])
        ):
            ...
            yield self.url_result(
                url, JioSaavnSongIE, url_transparent=True, **song_info
            )
...
class JioSaavnAlbumIE(JioSaavnBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._yield_songs(album_data),
            display_id,
            traverse_obj(album_data, ("title", {str})),
        )
...
class JioSaavnPlaylistIE(JioSaavnBaseIE):
    ...
    def _fetch_page(self, token, page):
        return self._call_api(
            "playlist",
            token,
            f"playlist page {page}",
            {"p": page, "n": self._PAGE_SIZE},
        )

    def _entries(self, token, first_page_data, page):
        page_data = (
            first_page_data if not page else self._fetch_page(token, page + 1)
        )
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            ...
            display_id,
            traverse_obj(playlist_data, ("listname", {str})),
        )
