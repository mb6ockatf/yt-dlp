...
class XimalayaIE(XimalayaBaseIE):
    ...
    def _decrypt_filename(file_id, seed):
        ...
        parts = file_id.split("*")
        filename = "".join(
            cgstr[int(part)] for part in parts if part.isdecimal()
        )
    ...
    def _decrypt_url_params(encrypted_params):
        params = VideaIE.rc4(
            base64.b64decode(encrypted_params), "xkt3a41psizxrh9l"
        ).split("-")
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "uploader_url": (
                f"{scheme}://www.ximalaya.com/zhubo/{audio_uploader_id}/"
                if audio_uploader_id
                else None
            ),
            ...
            "categories": list(
                filter(None, [audio_info.get("category_name")])
            ),
...
class XimalayaAlbumIE(XimalayaBaseIE):
    ...
    IE_DESC = "喜马拉雅FM 专辑"
    _VALID_URL = (
        r"https?://(?:www\.|m\.)?ximalaya\.com/(?:\d+/)?album/(?P<id>[0-9]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        page_count = math.ceil(
            first_page["trackTotalCount"] / first_page["pageSize"]
        )

        entries = InAdvancePagedList(
            lambda idx: self._get_entries(
                self._fetch_page(playlist_id, idx + 1) if idx else first_page
            ),
            page_count,
            first_page["pageSize"],
        )
