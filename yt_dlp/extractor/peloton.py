...
class PelotonIE(InfoExtractor):
    ...
    _VALID_URL = (
        r"https?://members\.onepeloton\.com/classes/player/(?P<id>[a-f0-9]+)"
    )
    _TESTS = [
        {
        ...
        try:
            ...
            if isinstance(e.cause, HTTPError) and e.cause.status == 401:
                ...
                raise ExtractorError(
                    res["message"], expected=res["message"] == "Login failed"
                )
            else:
                raise
    ...
    def _real_extract(self, url):
        ...
        if ride_data.get("content_format") == "audio":
            url = self._MANIFEST_URL_TEMPLATE % (
                ride_data.get("vod_stream_url"),
            ...
            if ride_data.get("vod_stream_url"):
                url = "https://members.onepeloton.com/.netlify/functions/m3u8-proxy?displayLanguage=en&acceptedSubtitles={}&url={}?hdnea={}".format(
                    ",".join(
                        [
                            re.sub("^([a-z]+)-([A-Z]+)$", r"\1", caption)
                            for caption in ride_data["captions"]
                        ]
                    ),
...
class PelotonLiveIE(InfoExtractor):
    ...
    _VALID_URL = (
        r"https?://members\.onepeloton\.com/player/live/(?P<id>[a-f0-9]+)"
    )
    _TEST = {
    ...
    def _real_extract(self, url):
        ...
        if peloton.get("ride_id"):
            if (
                not peloton.get("is_live")
                or peloton.get("is_encore")
                or peloton.get("status") != "PRE_START"
            ):
                return self.url_result(
                    "https://members.onepeloton.com/classes/player/{}".format(
                        peloton["ride_id"]
                    )
                )
            else:
