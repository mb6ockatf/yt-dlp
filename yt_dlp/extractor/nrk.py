...
class NRKBaseIE(InfoExtractor):
    ...
    def _extract_nrk_formats(self, asset_url, video_id):
        ...
        asset_url = re.sub(
            r"(?:bw_(?:low|high)=\d+|no_audio_only)&?", "", asset_url
        )
    ...
    def _raise_error(self, data):
        ...
        if (
            ...
            or try_get(data, lambda x: x["usageRights"]["isGeoBlocked"])
            is True
        ):
    ...
    def _call_api(
        self, path, video_id, item=None, note=None, fatal=True, query=None
    ):
        return self._download_json(
            urljoin("https://psapi.nrk.no/", path),
            video_id,
            note or f"Downloading {item} JSON",
            fatal=fatal,
            query=query,
            headers={
...
class NRKIE(NRKBaseIE):
    ...
    def _real_extract(self, url):
        ...
        description = try_get(
            preplay, lambda x: x["description"].replace("\r", "\n")
        )
        ...
        thumbnails = []
        for image in (
            try_get(preplay, lambda x: x["poster"]["images"], list) or []
        ):
        ...
        is_series = (
            try_get(data, lambda x: x["_links"]["series"]["name"]) == "series"
        )
        ...
        if is_series:
            series = season_id = season_number = episode = episode_number = (
                None
            )
...
class NRKTVIE(InfoExtractor):
    ...
    _VALID_URL = (
        rf"https?://(?:tv|radio)\.nrk(?:super)?\.no/(?:[^/]+/)*{_EPISODE_RE}"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        nrk_id = (
            ...
            or self._search_regex(
                rf'data-program-id=["\']({NRKTVIE._EPISODE_RE})',
                webpage,
                "nrk id",
            )
        )
        ...
        for page_num in itertools.count(1):
            ...
            next_url_path = try_get(
                ...
                (
                    ...
                    lambda x: x["_embedded"][assets_key]["_links"]["next"][
                        "href"
                    ],
                ),
                str,
...
class NRKTVSeasonIE(NRKTVSerieBaseIE):
    ...
    def _real_extract(self, url):
        ...
        title = (
            try_get(data, lambda x: x["titles"]["title"], str) or display_id
        )
        return self.playlist_result(
            self._entries(data, display_id), display_id, title
        )
...
class NRKTVSeriesIE(NRKTVSerieBaseIE):
    ...
    def _real_extract(self, url):
        ...
        linked_seasons = (
            try_get(series, lambda x: x["_links"]["seasons"]) or []
        )
        ...
        entries.extend(
            self._entries(embedded.get("extraMaterial") or {}, series_id)
        )
...
class NRKPlaylistIE(NRKPlaylistBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?nrk\.no/(?!video|skole)(?:[^/]+/)+(?P<id>[^/]+)"
    )
