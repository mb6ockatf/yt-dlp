...
class ImgGamingBaseIE(InfoExtractor):
        ...
        try:
            ...
            if isinstance(e.cause, HTTPError) and e.cause.status == 403:
                raise ExtractorError(
                    self._parse_json(
                        e.cause.response.read().decode(), media_id
                    )["messages"][0],
                    expected=True,
                )
            raise
    ...
    def _real_extract(self, url):
        ...
        if media_type == "playlist":
            ...
            entries = []
            for video in (
                try_get(playlist, lambda x: x["videos"]["vods"]) or []
            ):
        ...
        for proto in ("hls", "dash"):
            media_url = video_data.get(proto + "Url") or try_get(
                video_data, lambda x: x[proto]["url"]
            )
            if not media_url:
                continue
            if proto == "hls":
