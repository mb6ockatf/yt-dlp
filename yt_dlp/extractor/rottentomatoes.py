...
class RottenTomatoesIE(InfoExtractor):
    ...
    def _extract_videos(self, data, display_id):
        for video in traverse_obj(
            data,
            (lambda _, v: v["publicId"] and v["file"] and v["type"] == "hls"),
        ):
            yield {
    ...
    def _real_extract(self, url):
        playlist_id, trailers, video_id = self._match_valid_url(url).group(
            "playlist", "tr", "id"
        )
        playlist_id = join_nonempty(playlist_id, trailers)
        webpage = self._download_webpage(url, playlist_id)
        data = self._search_json(
            r'<script[^>]+\bid=["\'](?:heroV|v)ideos["\'][^>]*>',
            webpage,
            "data",
            playlist_id,
        ...
        if video_id:
            video_data = traverse_obj(
                data, lambda _, v: v["publicId"] == video_id
            )
            if not video_data:
        ...
        return self.playlist_result(
            self._extract_videos(data, playlist_id),
            playlist_id,
