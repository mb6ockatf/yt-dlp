    ...
    def _real_extract(self, url):
        ...
        for playback in feed.get("playbacks") or []:
            ...
            if ext == "m3u8":
                ...
                mobj = re.search(
                    r"_(\d+)x(\d+)_(\d+)_(\d+)K\.mp4", playback_url
                )
                if mobj:
                    f.update(
                        {
...
class MLBVideoIE(MLBBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?mlb\.com/(?:[^/]+/)*video/(?P<id>[^/?&#]+)"
    )
    _TEST = {
...
mutation initPlaybackSession(
    ...
    def _set_device_id(self, username):
        ...
        self.cache.store(
            self._NETRC_MACHINE, "device_ids", {username: self._device_id}
        )

    def _perform_login(self, username, password):
        try:
            self._access_token = self._download_json(
                "https://ids.mlb.com/oauth2/aus1m088yK07noBfh356/v1/token",
                None,
                "Logging in",
                "Unable to log in",
                headers={
        ...
        self._token_expiry = (
            traverse_obj(
                self._access_token, ({jwt_decode_hs256}, "exp", {int})
            )
            or 0
        )
        ...
        self._session_id = self._call_api(
            ...
            None,
            "session ID",
        )["data"]["initSession"]["sessionId"]

    def _call_api(
        self, data, video_id, description="GraphQL JSON", fatal=True
    ):
        return self._download_json(
    ...
    def _extract_formats_and_subtitles(self, broadcast, video_id):
        ...
        if not (m3u8_url and token):
            errors = "; ".join(
                traverse_obj(response, ("errors", ..., "message", {str}))
            )
            if "not entitled" in errors:
                raise ExtractorError(errors, expected=True)
            elif (
                errors
            ):  # Only warn when 'blacked out' since radio formats are available
                self.report_warning(
        ...
        for fmt in fmts:
            ...
            fmt.setdefault(
                "format_note", join_nonempty(feed, medium, delim=" ")
            )
    ...
    def _real_extract(self, url):
        ...
        metadata = traverse_obj(
            ...
            (
                ...
                ...,
                "games",
                lambda _, v: str(v["gamePk"]) == video_id
                and not v.get("rescheduleDate"),
                any,
            ),
        )
        ...
        broadcasts = traverse_obj(
            ...
            (
                "broadcasts",
                lambda _, v: v["mediaId"]
                and v["mediaState"]["mediaStateCode"] != "MEDIA_OFF",
            ),
        )
        ...
        return {
            ...
            "is_live": traverse_obj(
                broadcasts, (..., "mediaState", "mediaStateCode", {str}, any)
            )
            == "MEDIA_ON",
            "release_timestamp": traverse_obj(
                metadata, ("gameDate", {parse_iso8601})
            ),
            "formats": formats,
            "subtitles": subtitles,
        }
...
class MLBArticleIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        apollo_cache_json = self._search_json(
            r"window\.initState\s*=", webpage, "window.initState", display_id
        )["apolloCache"]

        content_real_info = traverse_obj(
            apollo_cache_json,
            ("ROOT_QUERY", lambda k, _: k.startswith("getArticle")),
            get_all=False,
        )
