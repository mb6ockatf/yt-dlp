...
class MxplayerIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        video_type, display_id, video_id = self._match_valid_url(url).group(
            "type", "display_id", "id"
        )
        ...
        m3u8_url = urljoin(
            ...
            traverse_obj(
                data_json,
                ("stream", (("thirdParty", "hlsUrl"), ("hls", "high"))),
                get_all=False,
            ),
        )
        if m3u8_url:
        ...
        mpd_url = urljoin(
            ...
            traverse_obj(
                data_json,
                ("stream", (("thirdParty", "dashUrl"), ("dash", "high"))),
                get_all=False,
            ),
        )
        if mpd_url:
        ...
        return {
            ...
            "series": traverse_obj(
                data_json, ("container", "container", "title")
            ),
            "description": data_json.get("description"),
            "season": season,
            "season_number": int_or_none(
                self._search_regex(
                    r"Season (\d+)", season, "Season Number", default=None
                )
            ),
