...
class TeamTreeHouseIE(InfoExtractor):
    ...
    def _perform_login(self, username, password):
        ...
        signin_page = self._download_webpage(
            "https://teamtreehouse.com/signin", None, "Downloading signin page"
        )
            ...
            if is_preview:
                ...
                if not duration:
                    duration = parse_duration(
                        get_element_by_id("video-duration", webpage)
                    )
            ...
            if workshop_videos:
                ...
                if stages_path:
                    stages_page = self._download_webpage(
                        urljoin(url, stages_path),
                        display_id,
                        "Downloading stages page",
                    )
                    for chapter_number, (chapter, steps_list) in enumerate(
                        re.findall(
                            r"(?s)<h2[^>]*>\s*(.+?)\s*</h2>.+?<ul[^>]*>(.+?)</ul>",
                            stages_page,
                        ),
