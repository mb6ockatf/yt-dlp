...
class StagePlusVODConcertIE(InfoExtractor):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?stage-plus\.com/video/(?P<id>vod_concert_\w+)"
    )
    _TESTS = [
        {
  ...
  title
    ...
    def _real_initialize(self):
        ...
        self._TOKEN = try_call(
            lambda: self._get_cookies("https://www.stage-plus.com/")[
                "dgplus_access_token"
            ].value
        )
        if not self._TOKEN:
    ...
    def _real_extract(self, url):
        ...
        for idx, video in enumerate(
            traverse_obj(
                ...
                (
                    "performanceWorks",
                    lambda _, v: v["id"] and url_or_none(v["stream"]["url"]),
                ),
            ),
            1,
        ):
            ...
            entries.append(
                {
                    ...
                    **traverse_obj(
                        video,
                        {
                            ...
                            "chapters": (
                                "cuePoints",
                                lambda _, v: float_or_none(v["mark"])
                                is not None,
                                {
                                    "title": "title",
        ...
        return self.playlist_result(
            entries, concert_id, thumbnails=thumbnails, **metadata
        )
