...
class BoxIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        shared_name, file_id, service = self._match_valid_url(url).group(
            "shared_name", "id", "service"
        )
        ...
        request_token = self._search_json(
            r"Box\.config\s*=", webpage, "Box config", file_id
        )["requestToken"]
        access_token = self._download_json(
            f"https://{service}.box.com/app-api/enduserapp/elements/tokens",
            file_id,
        ...
        for url_tmpl in traverse_obj(
            ...
            (
                "representations",
                "entries",
                lambda _, v: v["representation"] == "dash",
                "content",
                "url_template",
                {url_or_none},
            ),
            ...
            manifest_url = update_url_query(
                url_tmpl.replace("{+asset_path}", "manifest.mpd"), query
            )
