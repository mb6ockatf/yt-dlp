...
class CallinIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        host = traverse_obj(
            show_json, ("pageProps", "show", "hosts", 0)
        ) or traverse_obj(episode, ("speakers", 0))

        host_nick = traverse_obj(host, ("linkObj", "resourceUrl"))
        host_nick = (
            host_nick.rsplit("/", 1)[1]
            if (host_nick and "/" in host_nick)
            else None
        )
        ...
        cast = list(
            filter(
                None,
                [
                    self.try_get_user_name(u)
                    for u in traverse_obj(
                        episode, (("speakers", "callerTags"), ...)
                    )
                    or []
                ],
            )
        ...
        episode_list = (
            traverse_obj(show_json, ("pageProps", "show", "episodes")) or []
        )
        episode_number = next(
            (
                len(episode_list) - i
        ...
        return {
            "id": video_id,
            "_old_archive_ids": [
                make_archive_id(self, display_id.rsplit("-", 1)[-1])
            ],
