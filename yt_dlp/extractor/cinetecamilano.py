    ...
    def _real_extract(self, url):
        ...
        try:
            film_json = self._download_json(
                ...
                headers={
                    "Referer": url,
                    "Authorization": try_get(
                        self._get_cookies("https://www.cinetecamilano.it"),
                        lambda x: f'Bearer {x["cnt-token"].value}',
                    )
                    or "",
                },
            )
            ...
            if (
                isinstance(e.cause, HTTPError) and e.cause.status == 500
        ...
        return {
            ...
            "release_timestamp": parse_iso8601(
                archive.get("updated_at"), delimiter=" "
            ),
            "modified_timestamp": parse_iso8601(
                archive.get("created_at"), delimiter=" "
            ),
            "thumbnail": urljoin(
                url,
                try_get(
                    archive,
                    lambda x: x["thumb"]["src"].replace(
                        "/public/", "/storage/"
                    ),
                ),
            ),
            "formats": self._extract_m3u8_formats(
                urljoin(url, traverse_obj(archive, ("drm", "hls"))),
                video_id,
                "mp4",
            ),
        }
