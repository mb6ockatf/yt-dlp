...
class FancodeVodIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        media = (
            try_get(metadata_json, lambda x: x["data"]["media"], dict) or {}
        )
        ...
        return {
            ...
            "url": BRIGHTCOVE_URL_TEMPLATE
            % (brightcove_user_id, brightcove_video_id),
...
class FancodeLiveIE(FancodeVodIE):  # XXX: Do not subclass from concrete IE
    ...
    def _real_extract(self, url):
        ...
        self._check_login_required(
            match_info.get("isUserEntitled"), True
        ...
        return {
            ...
            "formats": self._extract_akamai_formats(
                try_get(match_info, lambda x: x["videoStreamUrl"]["url"]),
                video_id,
            ),
            "ext": mimetype2ext(
                try_get(
                    match_info, lambda x: x["videoStreamUrl"]["deliveryType"]
                )
            ),
            "is_live": True,
