...
class RtlNlIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        m3u8_url = (
            meta.get("videohost", "http://manifest.us.rtl.nl") + videopath
        )
...
class RTLLuBaseIE(InfoExtractor):
    ...
    def get_formats_and_subtitles(self, webpage, video_id):
        video_url, audio_url = self.get_media_url(
            webpage, video_id, "video"
        ), self.get_media_url(webpage, video_id, "audio")
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "formats": formats,
            "subtitles": subtitles,
            "thumbnail": self.get_media_url(webpage, video_id, "thumbnail")
...
class RTLLuRadioIE(RTLLuBaseIE):
    _VALID_URL = (
        r"https?://www\.rtl\.lu/radio/(?:[\w-]+)/s/(?P<id>\d+)(\.html)?"
    )
    _TESTS = [
        {
