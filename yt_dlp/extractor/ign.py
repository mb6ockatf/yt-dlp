...
class IGNBaseIE(InfoExtractor):
    def _call_api(self, slug):
        return self._download_json(
            f"http://apis.ign.com/{self._PAGE_TYPE}/v3/{self._PAGE_TYPE}s/slug/{slug}",
            slug,
        )
        ...
        try:
            ...
            if isinstance(e.cause, HTTPError) and e.cause.status == 404:
                ...
                raise ExtractorError(
                    "Content not found: expired?", cause=e.cause, expected=True
                )
            raise
...
class IGNVideoIE(IGNBaseIE):
    ...
    def _real_extract(self, url):
        ...
        embed_url = urllib.parse.urlunparse(
            parsed_url._replace(
                path=parsed_url.path.rsplit("/", 1)[0] + "/embed"
            )
        )
        ...
        ign_url = urllib.parse.parse_qs(
            urllib.parse.urlparse(new_url).query
        ).get("url", [None])[-1]
        if ign_url:
            ...
            def entries():
                for player in traverse_obj(
                    ...
                    (
                        ...
                        "apolloState",
                        "ROOT_QUERY",
                        lambda k, _: k.startswith("videoPlayerProps("),
                        "__ref",
                    ),
                ):
                    ...
                    if traverse_obj(
                        ...
                        (
                            "props",
                            "apolloState",
                            player.replace("PlayerProps", "ModernContent"),
                        ),
                        expected_type=dict,
                    ):
                        continue
