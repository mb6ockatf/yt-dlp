...
class SportDeutschlandIE(InfoExtractor):
    ...
    def _process_video(self, asset_id, video):
        ...
        token = self._download_json(
            ...
            video["id"],
            query={"type": video["type"], "playback_id": video["src"]},
        )["token"]
        formats, subtitles = self._extract_m3u8_formats_and_subtitles(
            f'https://stream.mux.com/{video["src"]}.m3u8?token={token}',
            video["id"],
            live=is_live,
        )
            ...
            **traverse_obj(
                meta,
                {
                    ...
                    "channel_url": (
                        "profile",
                        "slug",
                        {lambda x: f"https://sportdeutschland.tv/{x}"},
                    ),
                },
                get_all=False,
            ),
