...
class IxiguaIE(InfoExtractor):
    ...
    def _get_json_data(self, webpage, video_id):
        ...
        if not js_data:
            ...
            raise ExtractorError(
                "Cookies (not necessarily logged in) are needed", expected=True
            )

        return self._parse_json(
            js_data.replace("window._SSR_HYDRATED_DATA=", ""),
            video_id,
    ...
    def _media_selector(self, json_data):
        for path, override in (
            ...
            (
                ("dynamic_video", "dynamic_audio_list"),
                {"vcodec": "none", "ext": "m4a"},
            ),
        ):
    ...
    def _real_extract(self, url):
        ...
        json_data = self._get_json_data(webpage, video_id)["anyVideo"][
            "gidInformation"
        ]["packerData"]["video"]
