    ...
    def _extract_video(self, video_id, url=None):
        ...
        for res, fmt_url in (info.get("files") or {}).items():
            formats.extend(
                ...
                else self._extract_mpd_formats(
                    fmt_url, video_id, mpd_id=res, fatal=False
                )
            )

        webpage = (
            url and self._download_webpage(url, video_id, fatal=False)
        ) or ""
        thumbnail = url_or_none(
            dict_get(info, ("image", "preview"))
            or self._og_search_thumbnail(webpage)
        )
        upload_date = self._search_regex(
            r"/(\d{6})_", thumbnail, "upload_date", default=None
        )
        upload_date = (
            unified_strdate(
        ...
        return {
            ...
            "description": clean_html(
                get_element_by_class("wysiwyg", webpage)
            ),
...
class Alsace20TVIE(Alsace20TVBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?alsace20\.tv/(?:[\w-]+/)+[\w-]+-(?P<id>[\w]+)"
    )
    _TESTS = [
        {
