...
class InstagramBaseIE(InfoExtractor):
    ...
    def _api_headers(self):
        return {
            "X-IG-App-ID": self._configuration_arg(
                "app_id", ["936619743392459"], ie_key=InstagramIE
            )[0],
            "X-ASBD-ID": "198387",
            "X-IG-WWW-Claim": "0",
    ...
    def _get_dimension(self, name, media, webpage=None):
        return traverse_obj(
            media, ("dimensions", name), expected_type=int_or_none
        ) or int_or_none(
    ...
    def _extract_product_media(self, product_media):
        media_id = product_media.get("code") or _pk_to_id(
        ...
        if dash_manifest_raw:
            formats.extend(
                self._parse_mpd_formats(
                    self._parse_xml(dash_manifest_raw, media_id), mpd_id="dash"
                )
            )
        ...
        thumbnails = [
            ...
            for thumbnail in traverse_obj(
                product_media, ("image_versions2", "candidates")
            )
            or []
        ]
        return {
    ...
    def _extract_product(self, product_info):
        ...
        info_dict = {
            "id": _pk_to_id(
                traverse_obj(
                    product_info, "pk", "id", expected_type=str_or_none
                )[:19]
            ),
            "title": product_info.get("title")
            or f'Video by {user_info.get("username")}',
            "description": traverse_obj(
                product_info, ("caption", "text"), expected_type=str_or_none
            ),
        ...
        for comment_dict in comment_data or []:
            yield {
                ...
                "author_id": traverse_obj(
                    comment_dict, ("node", "owner", "id"), ("user", "pk")
                ),
...
class InstagramIOSIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        return self.url_result(
            f"http://instagram.com/tv/{video_id}", InstagramIE, video_id
        )
...
class InstagramIE(InstagramBaseIE):
    ...
    def _extract_embed_urls(cls, url, webpage):
        ...
        mobj = re.search(
            r'<a[^>]+href=([\'"])(?P<link>[^\'"]+)\1',
            get_element_by_attribute("class", "instagram-media", webpage)
            or "",
        )
        if mobj:
    ...
    def _real_extract(self, url):
        ...
        csrf_token = self._get_cookies("https://www.instagram.com").get(
            "csrftoken"
        )
        ...
        if not csrf_token:
            ...
            csrf_token = (
                csrf_token.value if api_check.get("status") == "ok" else None
            )
            if not csrf_token:
                self.report_warning(
                    "Instagram API is not granting access", video_id
                )
        ...
        if not general_info:
            ...
            if shared_data and self._LOGIN_URL not in urlh.url:
                ...
                product_item = traverse_obj(
                    additional_data, ("items", 0), expected_type=dict
                )
                if product_item:
            ...
            xdt_shortcode_media = (
                traverse_obj(
                    general_info, ("data", "xdt_shortcode_media", {dict})
                )
                or {}
            )
            if not xdt_shortcode_media:
                error = join_nonempty(
                    "title", "description", delim=": ", from_dict=api_check
                )
        ...
        if not video_url:
            ...
            raise ExtractorError(
                "There is no video in this post", expected=True
            )
        ...
        if dash:
            formats.extend(
                self._parse_mpd_formats(
                    self._parse_xml(dash, video_id), mpd_id="dash"
                )
            )
        ...
        comments = (
            [
                {
                    "author": traverse_obj(
                        comment_dict, ("node", "owner", "username")
                    ),
                    "author_id": traverse_obj(
                        comment_dict, ("node", "owner", "id")
                    ),
        ...
        return {
            ...
            "timestamp": traverse_obj(
                media, "taken_at_timestamp", "date", expected_type=int_or_none
            ),
...
class InstagramPlaylistBaseIE(InstagramBaseIE):
    ...
    def _extract_graphql(self, data, url):
        ...
        for page_num in itertools.count(1):
            ...
            nodes = (
                traverse_obj(media, ("edges", ..., "node"), expected_type=dict)
                or []
            )
            if not nodes:
                break
...
class InstagramUserIE(InstagramPlaylistBaseIE):
    ...
    def _query_vars_for(data):
        ...
        return {
            "id": data["entry_data"]["ProfilePage"][0]["graphql"]["user"][
                "id"
            ],
        }
...
class InstagramTagIE(InstagramPlaylistBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?instagram\.com/explore/tags/(?P<id>[^/]+)"
    )
    ...
    def _query_vars_for(data):
        ...
        # on the GraphQL of the original page
        return {
            "tag_name": data["entry_data"]["TagPage"][0]["graphql"]["hashtag"][
                "name"
            ],
        }
...
class InstagramStoryIE(InstagramBaseIE):
    ...
    def _real_extract(self, url):
        ...
        if (
            username == "highlights" and not story_id
        ):  # story id is only mandatory for highlights
            raise ExtractorError(
        ...
        user_info = self._search_json(
            r'"user":', story_info, "user info", display_id, fatal=False
        )
        if not user_info:
        ...
        full_name = traverse_obj(
            videos,
            (f"highlight:{story_id}", "user", "full_name"),
        ...
        highlights = traverse_obj(
            videos, (f"highlight:{story_id}", "items"), (user_id, "items")
        )
        info_data = []
        ...
        if (
            ...
            return traverse_obj(
                info_data, (lambda _, v: v["id"] == _pk_to_id(story_id), any)
            )

        return self.playlist_result(
            info_data, playlist_id=story_id, playlist_title=story_title
        )
