...
class EurosportIE(InfoExtractor):
    ...
    def _real_initialize(self):
        if EurosportIE._TOKEN is None:
            EurosportIE._TOKEN = self._download_json(
                "https://eu3-prod-direct.eurosport.com/token?realm=eurosport",
                None,
                "Trying to get token",
    ...
    def _real_extract(self, url):
        ...
        json_data = self._download_json(
            ...
            display_id,
            query={"usePreAuth": True},
            headers={"Authorization": f"Bearer {EurosportIE._TOKEN}"},
        )["data"]
        ...
        for stream_type in json_data["attributes"]["streaming"]:
            if stream_type == "hls":
                fmts, subs = self._extract_m3u8_formats_and_subtitles(
                    traverse_obj(
                        json_data,
                        ("attributes", "streaming", stream_type, "url"),
                    ),
                    display_id,
                    ext="mp4",
                    fatal=False,
