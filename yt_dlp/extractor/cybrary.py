...
class CybraryBaseIE(InfoExtractor):
    ...
    def _call_api(self, endpoint, item_id):
        return self._download_json(
            self._ENDPOINTS[endpoint].format(item_id),
            item_id,
...
class CybraryIE(CybraryBaseIE):
    ...
    def _real_extract(self, url):
        activity_id, enrollment_id = self._match_valid_url(url).group(
            "id", "enrollment"
        )
        course = self._call_api("enrollment", enrollment_id)["content"]
        activity = traverse_obj(
            course,
            (
                "learning_modules",
                ...,
                "activities",
                lambda _, v: int(activity_id) == v["id"],
            ),
            get_all=False,
        )
        ...
        module = next(
            (
                m
                for m in course.get("learning_modules") or []
                if int(activity_id)
                in traverse_obj(m, ("activities", ..., "id"))
            ),
            None,
        )
        ...
        return {
            ...
            "series_id": str_or_none(
                traverse_obj(course, ("content_description", "id"))
            ),
            "id": vimeo_id,
...
class CybraryCourseIE(CybraryBaseIE):
    _VALID_URL = (
        r"https?://app\.cybrary\.it/browse/course/(?P<id>[\w-]+)/?(?:$|[#?])"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            entries,
            traverse_obj(
                course, ("content_item", "id"), expected_type=str_or_none
            ),
            course.get("title"),
