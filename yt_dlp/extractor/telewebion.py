...
class TelewebionIE(InfoExtractor):
    ...
    def _call_graphql_api(
        ...
        result = self._download_json(
            "https://graph.telewebion.com/graphql",
            video_id,
            note,
            data=json.dumps(
                {
                    ...
                    "variables": {
                        name: value
                        for name, (_, value) in (variables or {}).items()
                    },
                },
                separators=(",", ":"),
            ).encode(),
        ...
        if not result or traverse_obj(result, "errors"):
            message = ", ".join(
                traverse_obj(result, ("errors", ..., "message", {str}))
            )
            ...
            (
                ...
                0,
                {
                    ...
                    "channel_url": (
                        "channel",
                        "descriptor",
                        {_fmt_url("https://telewebion.com/live/%s")},
                    ),
                    "thumbnail": (
                        "image",
                        {
                            _fmt_url(
                                "https://static.telewebion.com/episodeImages/%s/default"
                            )
                        },
                    ),
                    "formats": (
                        ...
                        {
                            _fmt_url(
                                f"https://cdna.telewebion.com/%s/episode/{video_id}/playlist.m3u8"
                            )
                        },
                        {
                            functools.partial(
