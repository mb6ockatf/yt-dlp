...
class DailymotionBaseInfoExtractor(InfoExtractor):
    ...
    def _real_initialize(self):
        ...
        self._FAMILY_FILTER = (
            ff == "on"
            if ff
            else age_restricted(18, self.get_param("age_limit"))
        )
    ...
    def _get_token(self, xid):
        ...
        ) or self._get_cookie_value(cookies, "client_token")
        if token:
            return token
        ...
        try:
            ...
            if isinstance(e.cause, HTTPError) and e.cause.status == 400:
                raise ExtractorError(
                    self._parse_json(e.cause.response.read().decode(), xid)[
                        "error_description"
                    ],
                    expected=True,
                )
    ...
    def _call_api(
        self, object_type, xid, object_fields, note, filter_extra=None
    ):
...
class DailymotionIE(DailymotionBaseInfoExtractor):
    ...
    IE_NAME = "dailymotion"
    _EMBED_REGEX = [
        rf'(?ix)<(?:(?:embed|iframe)[^>]+?src=|input[^>]+id=[\'"]dmcloudUrlEmissionSelect[\'"][^>]+value=)["\'](?P<url>{_VALID_URL[5:]})'
    ]
    _TESTS = [
        {
    ...
    def _extract_embed_urls(cls, url, webpage):
        ...
        for mobj in re.finditer(
            r'(?s)<script [^>]*\bsrc=(["\'])(?:https?:)?//[\w-]+\.dailymotion\.com/player/(?:(?!\1).)+\1[^>]*>',
            webpage,
        ):
    ...
    def _real_extract(self, url):
        ...
        video_id, is_playlist, playlist_id = self._match_valid_url(url).group(
            "id", "is_playlist", "playlist_id"
        )
    ...
    ... on Live {
      ...
      isOnAir
            ...
            % (self._COMMON_MEDIA_FIELDS, self._COMMON_MEDIA_FIELDS),
            "Downloading media JSON metadata",  # noqa: UP031
            (
                'password: "{}"'.format(self.get_param("videopassword"))
                if password
                else None
            ),
        )
        ...
        metadata = self._download_json(
            ...
            query=traverse_obj(smuggled_data, "query")
        ...
        for quality, media_list in metadata["qualities"].items():
            for m in media_list:
                ...
                if (
                    not media_url
                    or media_type == "application/vnd.lumberjack.manifest"
                ):
                    continue
        ...
        subtitles_data = (
            try_get(metadata, lambda x: x["subtitles"]["data"], dict) or {}
        )
        ...
        return {
            ...
            or int_or_none(media.get("audienceCount")),
...
class DailymotionPlaylistBaseIE(DailymotionBaseInfoExtractor):
      ...
      edges {
        node {
          ...
          url
            ...
            % (
                "false" if self._FAMILY_FILTER else "true",
                self._PAGE_SIZE,
                page,
            ),
    ...
    def _real_extract(self, url):
        ...
        entries = OnDemandPagedList(
            functools.partial(self._fetch_page, playlist_id), self._PAGE_SIZE
        )
...
class DailymotionPlaylistIE(DailymotionPlaylistBaseIE):
    ...
    def _extract_embed_urls(cls, url, webpage):
        ...
        for mobj in re.finditer(
            ...
            for p in re.findall(
                r"list\[\]=/playlist/([^/]+)/", unescapeHTML(mobj.group("url"))
            ):
...
class DailymotionSearchIE(DailymotionPlaylistBaseIE):
    ...
    def _fetch_page(self, term, page):
        ...
        response = self._call_search_api(
            term, page, f'Searching "{term}" page {page}'
        )
        for xid in traverse_obj(
            response, ("videos", "edges", ..., "node", "xid")
        ):
            yield self.url_result(
                f"https://www.dailymotion.com/video/{xid}", DailymotionIE, xid
            )
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            OnDemandPagedList(
                functools.partial(self._fetch_page, term), self._PAGE_SIZE
            ),
            term,
            term,
        )
