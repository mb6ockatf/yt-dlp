...
class CHZZKLiveIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        thumbnail_template = traverse_obj(
            live_playback,
            ("thumbnail", "snapshotThumbnailTemplate", {url_or_none}),
        )
        ...
        for media in traverse_obj(
            live_playback, ("media", lambda _, v: url_or_none(v["path"]))
        ):
...
class CHZZKVideoIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        live_status = (
            "was_live" if video_meta.get("liveOpenDate") else "not_live"
        )
        video_status = video_meta.get("vodStatus")
        if video_status == "UPLOAD":
            playback = self._parse_json(
                video_meta["liveRewindPlaybackJson"], video_id
            )
