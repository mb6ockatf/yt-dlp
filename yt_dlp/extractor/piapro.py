    ...
    def _real_extract(self, url):
        ...
        if category_id not in ("1", "2", "21", "22", "23", "24", "25"):
            raise ExtractorError(
                "The URL does not contain audio.", expected=True
            )
        ...
        return {
            "id": video_id,
            "title": clean_html(
                get_element_by_class("contents_title", webpage)
            ),
            "description": clean_html(
                get_element_by_class("contents_description", webpage)
            ),
            "uploader": clean_html(
                get_element_by_class("contents_creator_txt", webpage)
            ),
            "uploader_id": self._search_regex(
                r'<a\s+href="/([^"]+)"',
                get_element_by_class("contents_creator", webpage),
                "uploader id",
                default=None,
            ),
            "timestamp": unified_timestamp(
                extract_info("投稿日", "timestamp"), False
            ),
            ...
            "filesize_approx": parse_filesize(
                (extract_info("サイズ", "size") or "").replace(",", "")
            ),
