...
class MegaTVComIE(MegaTVComBaseIE):
    ...
    def _real_extract(self, url):
        ...
        description = get_element_by_class(
            "article-wrapper" if _is_article else "story_content", webpage
        )
        description = clean_html(
            re.sub(r"<script[^>]*>[^<]+</script>", "", description)
        )
        if not description:
            description = self._og_search_description(webpage)
        thumbnail = player_attrs.get("image") or self._og_search_thumbnail(
            webpage
        )
...
class MegaTVComEmbedIE(MegaTVComBaseIE):
    ...
    def _real_extract(self, url):
        ...
        canonical_url = player_attrs.get(
            "share_url"
        ) or self._match_canonical_url(webpage)
        ...
        canonical_url = self._request_webpage(
            HEADRequest(canonical_url),
            video_id,
            note="Resolve canonical URL",
            errnote="Could not resolve canonical URL",
        ).url
