    ...
    def _real_extract(self, url):
        ...
        headers = {
            "Token": self._search_regex(
                r'data-token="([^"]+)"', webpage, "data token"
            ),
            "Token-Expiry": self._search_regex(
                r'data-expiry="([^"]+)"', webpage, "data expiry"
            ),
            "Uvid": video_id,
        }
        ...
        streams = self._download_json(
            traverse_obj(show_stream, ("response", "tokenization", "url")),
            video_id,
            headers=headers,
        )
...
class IslamChannelSeriesIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_from_matches(
            ...
            pl_id,
            getter=lambda x: urljoin(url, x.group(1)),
            ie=IslamChannelIE,
        )
