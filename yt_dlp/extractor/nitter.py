...
class NitterIE(InfoExtractor):
    ...
    _VALID_URL = rf"https?://{_INSTANCES_RE}/(?P<uploader_id>.+)/status/(?P<id>[0-9]+)(#.)?"
    ...
    def _real_extract(self, url):
        ...
        counts = {
            f"{x[0]}_count": self._html_search_regex(
                rf'<span[^>]+class="icon-{x[1]}[^>]*></span>([^<]*)</div>',
                webpage,
                f"{x[0]} count",
                fatal=False,
            )
        ...
        counts = {
            field: 0 if count == "" else parse_count(count)
            for field, count in counts.items()
        }

        thumbnail = self._html_search_meta(
            "og:image", full_webpage, "thumbnail url"
        ) or remove_end(
            "{}{}".format(
                base_url,
                self._html_search_regex(
