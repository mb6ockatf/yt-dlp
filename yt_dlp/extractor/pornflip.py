...
class PornFlipIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?pornflip\.com/(?:(embed|sv|v)/)?(?P<id>[^/]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        webpage = self._download_webpage(
            f"https://{self._HOST}/sv/{video_id}",
            video_id,
        ...
        duration = self._search_regex(
            r'"duration":\s+"([^"]+)",', webpage, "duration", fatal=False
        )
        ...
        upload_date = self._search_regex(
            r'"uploadDate":\s+"([^"]+)",', webpage, "upload_date", fatal=False
        )
