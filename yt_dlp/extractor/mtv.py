...
class MTVServicesInfoExtractor(InfoExtractor):
    ...
    def _get_thumbnail_url(self, uri, itemdoc):
        search_path = "{}/{}".format(
            _media_xml_tag("group"), _media_xml_tag("thumbnail")
        )
    ...
    def _extract_mobile_video_formats(self, mtvn_id):
        ...
        webpage = self._download_webpage(
            req, mtvn_id, "Downloading mobile page"
        )
        metrics_url = unescapeHTML(
            self._search_regex(
                r'<a href="(http://metrics.+?)"', webpage, "url"
            )
        )
    ...
    def _extract_video_formats(self, mdoc, mtvn_id, video_id):
        if (
            ...
            if mtvn_id is not None and self._MOBILE_TEMPLATE is not None:
                self.to_screen(
                    "The normal version is not available from your "
                        ...
                        [
                            {
                                "ext": (
                                    "flv"
                                    if rtmp_video_url.startswith("rtmp")
                                    else ext
                                ),
                                "url": rtmp_video_url,
    ...
    def _extract_triforce_mgid(self, webpage, data_zone=None, video_id=None):
        triforce_feed = self._parse_json(
            self._search_regex(
        ...
        data_zone = self._search_regex(
            r'data-zone=(["\'])(?P<zone>.+?_lc_promo.*?)\1',
            webpage,
            "data zone",
            default=data_zone,
            group="zone",
        ...
        feed_url = try_get(
            triforce_feed,
            lambda x: x["manifest"]["zones"][data_zone]["feed"],
            str,
        )
        if not feed_url:
            return
...
class MTVServicesEmbeddedIE(MTVServicesInfoExtractor):
    ...
    _VALID_URL = (
        r"https?://media\.mtvnservices\.com/embed/(?P<mgid>.+?)(\?|/|$)"
    )
    _EMBED_REGEX = [
...
class MTVItaliaProgrammaIE(
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?mtv\.it/(?:programmi|playlist)/(?P<id>[0-9a-z]+)"
    )
    _TESTS = [
        {
    ...
    def _get_entries(self, title, url):
        while True:
            ...
            url = try_get(entries, lambda x: x["result"]["nextPageURL"], str)
            entries = try_get(
                entries,
                (
    ...
    def _real_extract(self, url):
        ...
        redirect = try_get(info, lambda x: x["newLocation"]["url"], str)
        if redirect:
            return self.url_result(redirect)

        title = info.get("title")
        video_id = try_get(info, lambda x: x["reporting"]["itemId"], str)
        parent_id = try_get(info, lambda x: x["reporting"]["parentId"], str)
        ...
        title = title or try_get(
            info, (lambda x: x["title"], lambda x: x["headline"]), str
        )
        ...
        if current_url:
            season = try_get(
                ...
                lambda x: x["result"]["data"],
                dict,
            )
            current = try_get(season, lambda x: x["currentSeason"], str)
