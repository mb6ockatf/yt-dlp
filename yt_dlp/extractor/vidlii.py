...
class VidLiiIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        for source in sources:
            ...
            height = int(
                self._search_regex(r"(\d+).mp4", source, "height", default=360)
            )
        ...
        description = self._html_search_meta(
            ("description", "twitter:description"), webpage, default=None
        ) or strip_or_none(get_element_by_id("des_text", webpage))
        ...
        upload_date = unified_strdate(
            self._html_search_meta("datePublished", webpage, default=None)
        ...
        category = self._html_search_regex(
            r"<div>Category\s*:\s*</div>\s*<div>\s*<a[^>]+>([^<]+)",
            webpage,
            "category",
            fatal=False,
        )
        ...
        tags = [
            ...
            for tag in re.findall(
                r'<a[^>]+\bhref=["\']/results\?.*?q=[^>]*>([^<]+)', webpage
            )
            if strip_or_none(tag)
        ] or None
