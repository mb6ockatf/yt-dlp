...
class MediasiteIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        service_path = urllib.parse.urljoin(
            redirect_url,
            self._html_search_regex(
                r'<div[^>]+\bid=["\']ServicePath[^>]+>(.+?)</div>',
                webpage,
                resource_id,
        ...
        player_options = self._download_json(
            f"{service_path}/GetPlayerOptions",
            resource_id,
            headers={
    ...
    def _real_extract(self, url):
        ...
        if anti_forgery_token:
            anti_forgery_header = self._search_regex(
                ...
                default="X-SOFO-AntiForgeryHeader",
                group="value",
            )
        ...
        for video in catalog["PresentationDetailsList"]:
            ...
            entries.append(
                self.url_result(
                    ...
                    ie=MediasiteIE.ie_key(),
                    video_id=video_id,
                )
            )

        title = try_get(catalog, lambda x: x["CurrentFolder"]["Name"], str)
