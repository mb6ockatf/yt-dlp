...
class OdnoklassnikiIE(InfoExtractor):
    ...
    def _extract_desktop(self, url):
        ...
        if (
            ...
            and not smuggled.get("referrer")
            and mode == "videoembed"
        ):
            return self._extract_desktop(
                smuggle_url(url, {"referrer": "https://boosty.to"})
            )
        elif error:
        ...
        formats = [
            ...
            for f in traverse_obj(
                metadata, ("videos", lambda _, v: url_or_none(v["url"]))
            )
        ]
    ...
    def _extract_mobile(self, url):
        ...
        redirect_url = self._request_webpage(
            HEADRequest(json_data["videoSrc"]),
            video_id,
        ...
        return {
            ...
            "duration": float_or_none(
                json_data.get("videoDuration"), scale=1000
            ),
