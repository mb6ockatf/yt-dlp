...
class MicrosoftEmbedIE(InfoExtractor):
    ...
    _TESTS = [
        {
            ...
            "expected_warnings": [
                "Failed to parse XML: syntax error: line 1, column 0"
            ],
        }
    ]
    _API_URL = (
    ...
    def _real_extract(self, url):
            ...
            if source_type == "smooth_Streaming":
                formats.extend(
                    self._extract_ism_formats(
                        source["url"], video_id, "mss", fatal=False
                    )
                )
                ...
                formats.extend(
                    self._extract_mpd_formats(
                        source["url"], video_id, fatal=False
                    )
                )
            else:
                formats.append(
        ...
        subtitles = {
            ...
            for lang, data in traverse_obj(
                metadata, "captions", default={}
            ).items()
        }
        ...
        return {
            ...
            "timestamp": unified_timestamp(
                traverse_obj(metadata, ("snippet", "activeStartDate"))
            ),
            "age_limit": int_or_none(
                traverse_obj(metadata, ("snippet", "minimumAge"))
            )
            or 0,
            "formats": formats,
            "subtitles": subtitles,
...
class MicrosoftMediusIE(MicrosoftMediusBaseIE):
    ...
    def _extract_subtitle(self, webpage, video_id):
        captions = traverse_obj(
            self._search_json(
                r"const\s+captionsConfiguration\s*=",
                webpage,
                "captions",
                video_id,
                default=None,
            ...
            {
                "url": url,
                "tag": url_basename(url).split(".vtt")[0].split("_")[-1],
            }
            for url in re.findall(
                r"var\s+file\s+=\s+\{[^}]+\'(https://[^\']+\.vtt\?[^\']+)",
                webpage,
            )
        ]
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "formats": self._extract_ism(
                self._search_regex(
                    r'StreamUrl\s*=\s*"([^"]+manifest)"', webpage, "ism url"
                ),
                video_id,
            ),
...
class MicrosoftLearnPlaylistIE(InfoExtractor):
        ...
        while True:
            ...
            for url_path in url_paths:
                yield self.url_result(
                    f"https://learn.microsoft.com/en-us{url_path}"
                )
            skip += len(url_paths)
    ...
    def _real_extract(self, url):
        playlist_id, playlist_type = self._match_valid_url(url).group(
            "id", "type"
        )
        ...
        return self.playlist_result(
            self._entries(url_base, playlist_id), playlist_id, **metainfo
        )
...
class MicrosoftLearnEpisodeIE(MicrosoftMediusBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return {
            "id": entry_id,
            "formats": self._extract_ism(
                video_info["publicVideo"]["adaptiveVideoUrl"], video_id
            ),
    ...
    def _real_extract(self, url):
        ...
        metainfo = {
            ...
            "timestamp": parse_iso8601(
                self._html_search_meta("startDate", webpage, "startDate")
            ),
        }

        return self.url_result(
            self._html_search_meta(
                "externalVideoUrl", webpage, "videoUrl", fatal=True
            ),
