...
class GoProIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        media_data = self._download_json(
            "https://api.gopro.com/media/{}/download".format(video_info["id"]),
            video_id,
        )

        formats = []
        for fmt in (
            try_get(media_data, lambda x: x["_embedded"]["variations"]) or []
        ):
        ...
        return {
            ...
            "duration": int_or_none(video_info.get("source_duration")),
            "artist": str_or_none(video_info.get("music_track_artist"))
            or None,
            "track": str_or_none(video_info.get("music_track_name")) or None,
        }
