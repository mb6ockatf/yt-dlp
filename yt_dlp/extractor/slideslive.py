...
class SlidesLiveIE(InfoExtractor):
    _VALID_URL = (
        r"https?://slideslive\.com/(?:embed/(?:presentation/)?)?(?P<id>[0-9]+)"
    )
    _TESTS = [
        {
    ...
    def _extract_embed_urls(cls, url, webpage):
        ...
        for embed_id in re.findall(
            r'(?s)new\s+SlidesLiveEmbed\s*\([^)]+\bpresentationId:\s*["\'](\d+)["\']',
            webpage,
        ):
    ...
    def _download_embed_webpage_handle(self, video_id, headers):
        return self._download_webpage_handle(
            f"https://slideslive.com/embed/presentation/{video_id}",
            video_id,
            headers=headers,
            query=traverse_obj(
                headers,
    ...
    def _extract_custom_m3u8_info(self, m3u8_data):
        ...
        for key in ("video_servers", "subtitles"):
            if key in m3u8_dict:
                m3u8_dict[key] = (
                    self._parse_json(m3u8_dict[key], None, fatal=False) or []
                )

        return m3u8_dict

    def _extract_formats_and_duration(
        self, cdn_hostname, path, video_id, skip_duration=False
    ):
        ...
        if hls_formats:
            if not skip_duration:
                duration = self._extract_m3u8_vod_duration(
                    hls_formats[0]["url"],
                    video_id,
                    note="Extracting duration from HLS manifest",
                )
    ...
    def _real_extract(self, url):
        ...
        if "domain_not_allowed" in redirect_url:
            ...
            webpage, _ = self._download_embed_webpage_handle(
                video_id,
                headers={
        ...
        player_token = self._search_regex(
            r'data-player-token="([^"]+)"', webpage, "player token"
        )
        player_data = self._download_webpage(
            f"https://ben.slideslive.com/player/{video_id}",
            video_id,
            note="Downloading player info",
        ...
        slide_url_template = (
            "https://slides.slideslive.com/%s/slides/original/%s%s"
        )
        slides, slides_info = {}, []
        ...
        if player_info.get("slides_json_url"):
            slides = (
                self._download_json(
                    player_info["slides_json_url"],
                    video_id,
                    fatal=False,
                    note="Downloading slides JSON",
                    errnote=False,
                )
                or {}
            )
            ...
            for slide_id, slide in enumerate(
                traverse_obj(slides, ("slides", ...), expected_type=dict), 1
            ):
                slides_info.append(
                    (
                        slide_id,
                        traverse_obj(slide, ("image", "name")),
                        traverse_obj(
                            slide,
                            ("image", "extname"),
        ...
        if not slides and player_info.get("slides_xml_url"):
            slides = self._download_xml(
                player_info["slides_xml_url"],
                video_id,
                fatal=False,
                note="Downloading slides XML",
            ...
            if isinstance(slides, xml.etree.ElementTree.Element):
                ...
                for slide_id, slide in enumerate(slides.findall("./slide")):
                    slides_info.append(
                        (
                            slide_id,
                            xpath_text(slide, "./slideName", "name"),
                            ".jpg",
                            int_or_none(
                                xpath_text(slide, "./timeSec", "time")
                            ),
                        )
                    )
        ...
        for slide_id, slide_path, slide_ext, start_time in slides_info:
            if slide_path:
                thumbnails.append(
                    {
                        "id": f"{slide_id:03d}",
                        "url": slide_url_template
                        % (video_id, slide_path, slide_ext),
                    }
                )
            chapters.append(
                {
        ...
        for sub in traverse_obj(
            player_info, ("subtitles", ...), expected_type=dict
        ):
        ...
        info = {
            ...
            or self._html_search_meta("title", webpage, default=""),
        ...
        def entries():
            ...
            service_data = (
                self._download_json(
                    ...
                    note="Downloading video slides info",
            ...
            for slide_id, slide in enumerate(
                ...
                yield {
                    ...
                    "duration": int_or_none(
                        traverse_obj(slide, ("video", "duration_ms")),
                        scale=1000,
                    ),
                    "formats": formats,
                }
        ...
        return self.playlist_result(
            entries(), f"{video_id}-playlist", info["title"]
        )
