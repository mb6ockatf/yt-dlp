...
class GettrIE(GettrBaseIE):
    ...
    def _real_extract(self, url):
        ...
        user_data = (
            try_get(
                api_data, lambda x: x["aux"]["uinf"][post_data["uid"]], dict
            )
            or {}
        )
        ...
        uploader = str_or_none(
            ...
            or self._search_regex(
                r"^(.+?) on GETTR",
                self._og_search_title(webpage, default=""),
                "uploader",
                fatal=False,
            )
        )
        ...
        formats, subtitles = (
            self._extract_m3u8_formats_and_subtitles(
                urljoin(self._MEDIA_BASE_URL, vid),
                post_id,
                "mp4",
        ...
        return {
            ...
            "thumbnail": url_or_none(
                ...
                or self._html_search_meta(
                    ["og:image", "image"], webpage, "thumbnail", fatal=False
                )
            ),
            "timestamp": float_or_none(
                dict_get(post_data, ["cdate", "udate"]), scale=1000
            ),
    ...
    def _real_extract(self, url):
        ...
        thumbnails = [
            ...
            for thumbnail in try_get(
                video_info, lambda x: x["postData"]["imgs"], list
            )
            or []
        ]
        ...
        return {
            ...
            "description": try_get(
                video_info, lambda x: x["postData"]["dsc"], str
            ),
            "formats": formats,
            "subtitles": subtitles,
            "thumbnails": thumbnails,
            "uploader": try_get(
                video_info, lambda x: x["liveHostInfo"]["nickname"], str
            ),
            "uploader_id": try_get(
                video_info, lambda x: x["liveHostInfo"]["_id"], str
            ),
