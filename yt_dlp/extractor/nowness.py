...
class NownessBaseIE(InfoExtractor):
                ...
                if media["type"] == "video":
                    ...
                    if source == "brightcove":
                        player_code = self._download_webpage(
                            f"http://www.nowness.com/iframe?id={video_id}",
                            video_id,
    ...
    def _api_request(self, url, request_path):
        ...
        request = Request(
            ...
            headers={
                "X-Nowness-Language": (
                    "zh-cn" if "cn.nowness.com" in url else "en-us"
                ),
            },
        )
...
class NownessPlaylistIE(NownessBaseIE):
    ...
    def _real_extract(self, url):
        ...
        entries = [
            self._extract_url_result(item) for item in playlist["items"]
        ]
...
class NownessSeriesIE(NownessBaseIE):
    ...
    def _real_extract(self, url):
        ...
        if translations:
            series_title = (
                translations[0].get("title") or translations[0]["seoTitle"]
            )
