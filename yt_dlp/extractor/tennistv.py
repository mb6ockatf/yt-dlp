...
class TennisTVIE(InfoExtractor):
    ...
    _AUTH_BASE_URL = (
        "https://sso.tennistv.com/auth/realms/TennisTV/protocol/openid-connect"
    )
    _HEADERS = {
    ...
    def _perform_login(self, username, password):
        ...
        if "Your username or password was incorrect" in temp_page:
            raise ExtractorError(
                "Your username or password was incorrect", expected=True
            )
    ...
    def _real_extract(self, url):
        ...
        if k_session is None:
            ...
            k_session = self._download_session_json(video_id, entryid).get(
                "KSession"
            )
            if k_session is None:
                raise ExtractorError(
        ...
        formats, subtitles = self._extract_m3u8_formats_and_subtitles(
            self._FORMAT_URL.format(
                partner=self._PARTNER_ID, entry=entryid, session=k_session
            ),
            video_id,
        )
        ...
        return {
            ...
            "series": self._html_search_regex(
                r'data-series\s*?=\s*?"(.*?)"', webpage, "series", fatal=False
            )
            or None,
            ...
            "episode": self._html_search_regex(
                r'data-round\s*?=\s*?"(.*?)"', webpage, "round", fatal=False
            )
            or None,
            "formats": formats,
            "subtitles": subtitles,
