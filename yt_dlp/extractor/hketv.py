...
class HKETVIE(InfoExtractor):
    ...
    _GEO_COUNTRIES = ["HK"]
    _VALID_URL = (
        r"https?://(?:www\.)?hkedcity\.net/etv/resource/(?P<id>[0-9]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        response = self._download_json(
            self._APPS_BASE_URL + "/media/play/handler.php",
            video_id,
        ...
        for track in tracks:
            ...
            subtitles.setdefault(
                self._CC_LANGS.get(track_label, track_label), []
            ).append(
                {
        ...
        like_count = int_or_none(
            try_get(emotion, lambda x: x["data"]["emotion_data"][0]["count"])
        )
        ...
        return {
            ...
            "upload_date": unified_strdate(
                self._html_search_meta("ed_date", webpage, fatal=False),
                day_first=False,
            ),
