...
class RadioKapitalBaseIE(InfoExtractor):
    def _call_api(
        self, resource, video_id, note="Downloading JSON metadata", qs={}
    ):
...
class RadioKapitalShowIE(RadioKapitalBaseIE):
    ...
    def _entries(self, series_id):
        for page_no in itertools.count(1):
            ...
            yield from (
                self._parse_episode(ep) for ep in episode_list["items"]
            )
    ...
    def _real_extract(self, url):
        ...
        show = self._call_api(
            f"shows/{series_id}", series_id, "Downloading show metadata"
        )
