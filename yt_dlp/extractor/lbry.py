...
class LBRYBaseIE(InfoExtractor):
    ...
    def _resolve_url(self, url, display_id, resource):
        return self._call_api_proxy(
            "resolve", display_id, {"urls": url}, resource
        )[url]
        ...
        for item in traverse_obj(
            ...
            yield {
                **self._parse_stream(item, url),
                "_type": "url",
                "id": item["claim_id"],
                "url": self._permanent_url(
                    url, item["name"], item["claim_id"]
                ),
            }
    ...
    def _playlist_entries(self, url, display_id, claim_param, metadata):
        ...
        params = {
            ...
            "stream_types": (
                ...
                else self._SUPPORTED_STREAM_TYPES
            ),
            **claim_param,
        }
...
class LBRYIE(LBRYBaseIE):
    ...
    _TESTS = [
        ...
        },
        {
            ...
            "info_dict": {
                ...
                "tags": [
                    "smart skin surveillance",
                    "biotechnology invasion of skin",
                    "morgellons",
                ],
                "license": "None",
    ...
    def _real_extract(self, url):
        ...
        if stream_type in self._SUPPORTED_STREAM_TYPES:
            ...
            result.get("value_type") == "stream"
            and stream_type not in self._UNSUPPORTED_STREAM_TYPES
        ):
            claim_id, is_live = result["signing_channel"]["claim_id"], True
            live_data = self._download_json(
                "https://api.odysee.live/livestream/is_live",
                claim_id,
                query={"channel_claim_id": claim_id},
                note="Downloading livestream JSON metadata",
            )["data"]
...
class LBRYChannelIE(LBRYBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self._playlist_entries(
            url, claim_id, {"channel_ids": [claim_id]}, result
        )
...
class LBRYPlaylistIE(LBRYBaseIE):
    ...
    _VALID_URL = (
        LBRYBaseIE._BASE_URL_REGEX + r"\$/(?:play)?list/(?P<id>[0-9a-f-]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        claim_param = {
            "claim_ids": traverse_obj(result, ("value", "claims", ..., {str}))
        }
