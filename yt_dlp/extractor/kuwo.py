    ...
    def _get_formats(self, song_id, tolerate_ip_deny=False):
        ...
        for file_format in self._FORMATS:
            ...
            song_url = self._download_webpage(
                ...
                song_id,
                note="Download {} url info".format(file_format["format"]),
                query=query,
    ...
    def _real_extract(self, url):
        ...
        if album_id is not None:
            album_info_page = self._download_webpage(
                f"http://www.kuwo.cn/album/{album_id}/",
                song_id,
    ...
    def _real_extract(self, url):
        ...
        def page_func(page_num):
            webpage = self._download_webpage(
                ...
                singer_id,
                note=f"Download song list page #{page_num + 1}",
...
class KuwoCategoryIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        jsonm = self._parse_json(
            self._html_search_regex(
                r"var\s+jsonm\s*=\s*([^;]+);", webpage, "category songs"
            ),
            category_id,
        )
        ...
        entries = [
            self.url_result(
                "http://www.kuwo.cn/yinyue/{}/".format(song["musicrid"]),
                "Kuwo",
            )
...
class KuwoMvIE(KuwoBaseIE):
    ...
    def _real_extract(self, url):
        ...
        webpage = self._download_webpage(
            url,
            song_id,
            note=f"Download mv detail info: {song_id}",
        ...
        mv_url = self._download_webpage(
            ...
            song_id,
            note=f"Download {song_id} MV URL",
        )
        formats.append(
            {
                "url": mv_url,
