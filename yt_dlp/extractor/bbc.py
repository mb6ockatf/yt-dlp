...
class BBCCoUkIE(InfoExtractor):
    ...
    def _extract_asx_playlist(self, connection, programme_id):
        asx = self._download_xml(
            connection.get("href"), programme_id, "Downloading ASX playlist"
        )
        ...
        for media_set in self._MEDIA_SETS:
            try:
                fmts, subs = self._download_media_selector_url(
                    self._MEDIA_SELECTOR_URL_TEMPL % (media_set, programme_id),
                    programme_id,
                )
                formats.extend(fmts)
                if subs:
                ...
                for connection in self._extract_connections(media):
                    ...
                    if supplier == "asx":
                        ...
                        try:
                            ...
                            if not (
                                isinstance(e.exc_info[1], HTTPError)
        ...
        try:
            ...
            for version in playlist.get("allAvailableVersions", []):
                ...
                for item in smp_config["items"]:
                    ...
                    version_formats, version_subtitles = (
                        self._download_media_selector(programme_id)
                    )
                    types = version["types"]
            ...
            if not (
                isinstance(ee.cause, HTTPError) and ee.cause.status == 404
            ):
                raise
...
class BBCIE(BBCCoUkIE):  # XXX: Do not subclass from concrete IE
    ...
    def _extract_from_media_meta(self, media_meta, video_id):
        ...
        if source_files:
            return [
                ...
                for format_id, f in source_files.items()
                if f.get("url")
            ], []
        ...
        if href:
            ...
            _, _, _, _, formats, subtitles = (
                self._extract_from_legacy_playlist(playlist, video_id)
            )
            return formats, subtitles
    ...
    def _real_extract(self, url):
        ...
        playlist_title = (
            json_ld_info.get("title")
            or re.sub(
                r"(.+)\s*-\s*BBC.*?$",
                r"\1",
                self._generic_title("", webpage, default=""),
            ).strip()
            or None
        )
        ...
        playlists.extend(
            re.findall(r'data-media-id="([^"]+/playlist\.sxml)"', webpage)
        )
        if playlists:
            entries = [
                self._extract_from_playlist_sxml(
            ...
            for _, data_playable_json in data_playables:
                ...
                if settings:
                    ...
                    if playlist_object:
                        ...
                        playlist = data_playable.get("otherSettings", {}).get(
                            "playlist", {}
                        )
                        if playlist:
                            entry = None
                            for key in ("streaming", "progressiveDownload"):
                                ...
                                try:
                                    ...
                                    # http://www.bbc.com/turkce/haberler/2015/06/150615_telabyad_kentin_cogu)
                                    if (
                                        isinstance(e.cause, HTTPError)
                                        and e.cause.status == 500
                                    ):
                                        continue
                                    raise
                            if entry:
        ...
        if programme_id:
            ...
            timestamp = (
                parse_iso8601(page_info.get("publicationDate")) or timestamp
            )
            return {
                "id": programme_id,
                "title": title,
        ...
        if initial_data:
            init_data = (
                try_get(
                    initial_data, lambda x: x["initData"]["items"][0], dict
                )
                or {}
            )
            ...
            if version_id:
                ...
                return {
                    ...
                    "thumbnail": (
                        image_url.replace("$recipe", "raw")
                        if image_url
                        else None
                    ),
                    "description": smp_data.get("summary")
                    or init_data.get("shortSummary"),
                    "upload_date": (
                        display_date.replace("-", "") if display_date else None
                    ),
                    "subtitles": subtitles,
        ...
        if morph_payload:
            for lead_media in traverse_obj(
                morph_payload,
                ("body", "components", ..., "props", "leadMedia", {dict}),
            ):
                programme_id = traverse_obj(
                    lead_media,
                    ("identifiers", ("vpid", "playablePid"), {str}, any),
                )
                if not programme_id:
                    continue
                formats, subtitles = self._download_media_selector(
            ...
            body = self._parse_json(
                traverse_obj(
                    morph_payload, ("body", "content", "article", "body")
                ),
                playlist_id,
                fatal=False,
            )
            for video_data in traverse_obj(
                ...
                if video_data.get("vpid"):
                    ...
                    entry = self.url_result(
                        f"https://www.bbc.co.uk/programmes/{video_id}",
                        BBCCoUkIE,
                        video_id,
            ...
            if entries:
                playlist_title = (
                    traverse_obj(
                        morph_payload,
                        ("body", "content", "article", "headline", {str}),
                    )
                    or playlist_title
                )
                return self.playlist_result(
        ...
        if programme_id and current_programme.get("type") == "playable_item":
            title = (
                traverse_obj(
                    current_programme,
                    ("titles", ("tertiary", "secondary"), {str}, any),
                )
                or playlist_title
            )
            formats, subtitles = self._download_media_selector(programme_id)
                ...
                **traverse_obj(
                    current_programme,
                    {
                        ...
                        "thumbnail": (
                            "image_url",
                            {
                                lambda u: url_or_none(
                                    u.replace("{recipe}", "raw")
                                )
                            },
                        ),
                        ...
                        "timestamp": (
                            (("availability", "from"), ("release", "date")),
                            {parse_iso8601},
                            any,
                        ),
                ...
                "chapters": traverse_obj(
                    ...
                    (
                        "tracklist",
                        "tracks",
                        lambda _, v: float(v["offset"]["start"]),
                        {
                            "title": (
                                "titles",
                                {
                                    lambda x: join_nonempty(
                                        "primary",
                                        "secondary",
                                        "tertiary",
                                        delim=" - ",
        ...
        asset_id = traverse_obj(
            preload_state,
            (
                "entities",
                "articles",
                lambda k, _: k.rsplit("/", 1)[-1] == playlist_id,
                "assetVideo",
                0,
                {str},
                any,
            ...
            if video_id:
                article = traverse_obj(
                    ...
                    (
                        "entities",
                        "articles",
                        lambda _, v: v["assetVideo"][0] == asset_id,
                        any,
                    ),
                )
                ...
                return {
                    ...
                    **traverse_obj(
                    ...
                    "subtitles": subtitles,
                    "timestamp": traverse_obj(
                        article, ("displayDate", {parse_iso8601})
                    ),
                }
            else:
                return self.url_result(
                    f"https://www.bbc.co.uk/programmes/{asset_id}",
                    BBCCoUkIE,
                    asset_id,
                    playlist_title,
        ...
        if payload:
            ...
            bbc3_playlist = try_get(
                payload, lambda x: x["content"]["bbcMedia"]["playlist"], dict
            )
            if bbc3_playlist:
        ...
        if initial_data:
            ...
            def parse_media(media):
                if not media:
                    return
                for item in (
                    try_get(media, lambda x: x["media"]["items"], list) or []
                ):
                    item_id = item.get("id")
                    ...
                    item_time = None
                    for meta in (
                        try_get(media, lambda x: x["metadata"]["items"], list)
                        or []
                    ):
        ...
        if simorgh_data:
            ...
            for video_data in traverse_obj(
                ...
                if video_data["type"] == "video":
                    ...
                    block_id, entry = (
                        traverse_obj(model, ("blockId", {str})),
                        None,
                    )
                    media_data = traverse_obj(
                        simorgh_data,
                        (
                            "pageData",
                            "promo",
        ...
        entries.extend(re.findall(rf'setPlaylist\("({EMBED_URL})"\)', webpage))
        if entries:
            return self.playlist_result(
        ...
        for num, media_meta in enumerate(medias, start=1):
            ...
            if not video_id:
                video_id = (
                    playlist_id if len(medias) == 1 else f"{playlist_id}-{num}"
                )
            ...
            if not title:
                title = (
                    playlist_title
                    if len(medias) == 1
                    else f"{playlist_title} - Video {num}"
                )

            duration = int_or_none(
                media_meta.get("durationInSeconds")
...
class BBCCoUkArticleIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        entries = [
            self.url_result(programme_url)
            for programme_url in re.findall(
    ...
    def _entries(self, webpage, url, playlist_id):
        ...
        for page_num in itertools.count(2):
            ...
            if not next_page:
                break
            webpage = self._download_webpage(
                urllib.parse.urljoin(url, next_page),
                playlist_id,
...
class BBCCoUkIPlayerPlaylistBaseIE(InfoExtractor):
    ...
    def _fetch_page(self, programme_id, per_page, series_id, page):
        elements = self._get_elements(
            self._call_api(programme_id, per_page, page + 1, series_id)
        )
    ...
    def _real_extract(self, url):
        ...
        entries = (
            fetch_page(int(page) - 1)
            if page
            else OnDemandPagedList(fetch_page, self._PAGE_SIZE)
        )
        playlist_data = self._get_playlist_data(self._call_api(pid, 1))
        return self.playlist_result(
            entries,
            pid,
            self._get_playlist_title(playlist_data),
