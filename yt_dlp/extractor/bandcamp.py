...
class BandcampIE(InfoExtractor):
    ...
    def _extract_data_attr(
        self, webpage, video_id, attr="tralbum", fatal=True
    ):
...
class BandcampAlbumIE(BandcampIE):  # XXX: Do not subclass from concrete IE
    ...
    def _real_extract(self, url):
        ...
        entries = [
            self.url_result(
                urljoin(url, t["title_link"]),
                BandcampIE.ie_key(),
                str_or_none(t.get("track_id") or t.get("id")),
                t.get("title"),
            )
            for t in track_info
            if t.get("duration")
...
class BandcampWeeklyIE(BandcampIE):  # XXX: Do not subclass from concrete IE
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?bandcamp\.com/?\?(?:.*?&)?show=(?P<id>\d+)"
    )
    _TESTS = [
        {
...
class BandcampUserIE(InfoExtractor):
    ...
    def _yield_items(self, webpage):
        yield from (
            re.findall(
                r'<li data-item-id=["\'][^>]+>\s*<a href=["\'](?![^"\'/]*?/merch)([^"\']+)',
                webpage,
            )
            or re.findall(
        ...
        yield from traverse_obj(
            webpage,
            (
                {find_element(id="music-grid", html=True)},
                {extract_attributes},
                "data-client-items",
                {json.loads},
                ...,
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_from_matches(
            self._yield_items(webpage),
            uploader,
            f"Discography of {uploader}",
            getter=urljoin(url),
        )
