...
class TikTokBaseIE(InfoExtractor):
    ...
    @functools.cached_property
    def _DEVICE_ID(self):
        return self._KNOWN_DEVICE_ID or str(
            random.randint(7250000000000000000, 7351147085025500000)
        )
    ...
    def _get_next_app_info(self):
        if self._APP_INFO_POOL is None:
            defaults = {
                key: self._configuration_arg(key, [default], ie_key=TikTokIE)[
                    0
                ]
            ...
            self._APP_INFO_POOL = [
                {
                    ...
                    **dict(
                        (k, v)
                        for k, v in zip(
                            self._APP_INFO_DEFAULTS, app_info.split("/")
                        )
                        if v
                    ),
                }
    ...
    def _get_sigi_state(self, webpage, display_id):
        return self._search_json(
            r'<script[^>]+\bid="(?:SIGI_STATE|sigi-persisted-data)"[^>]*>',
            webpage,
            "sigi state",
            display_id,
    ...
    def _call_api_impl(
        ...
        self._set_cookie(
            self._API_HOSTNAME,
            "odin_tt",
            "".join(random.choices("0123456789abcdef", k=160)),
        )
        webpage_cookies = self._get_cookies(self._WEBPAGE_HOST)
        if webpage_cookies.get("sid_tt"):
            self._set_cookie(
                self._API_HOSTNAME, "sid_tt", webpage_cookies["sid_tt"].value
            )
        return self._download_json(
            f"https://{self._API_HOSTNAME}/aweme/v1/{ep}/",
            video_id=video_id,
            fatal=fatal,
            note=note,
            errnote=errnote,
        ...
        return filter_dict(
            {
                **query,
                ...
                "version_code": "".join(
                    f"{int(v):02d}"
                    for v in self._APP_INFO["app_version"].split(".")
                ),
                "version_name": self._APP_INFO["app_version"],
                "manifest_version_code": self._APP_INFO[
                ...
                if (
                    ...
                    if not self._get_next_app_info():
                        if fatal:
                            ...
                            return
                    self.report_warning(
                        f"{message}. Retrying... (attempt {count} of {max_tries})"
                    )
                    continue
                raise
    ...
    def _extract_aweme_app(self, aweme_id):
        ...
        if not aweme_detail:
            raise ExtractorError(
                "Unable to extract aweme detail info", video_id=aweme_id
            )
    ...
    def _extract_web_data_and_status(self, url, video_id, fatal=True):
        ...
        res = self._download_webpage_handle(
            url, video_id, fatal=fatal, headers={"User-Agent": "Mozilla/5.0"}
        )
        if res is False:
        ...
        elif next_data := self._search_nextjs_data(
        ...
        if not traverse_obj(video_data, ("video", {dict})) and traverse_obj(
            video_data, ("isContentClassified", {bool})
        ):
            message = "This post may not be comfortable for some audiences. Log in for access"
            if fatal:
                self.raise_login_required(message)
            self.report_warning(
                f"{message}. {self._login_hint()}", video_id=video_id
            )
    ...
    def _get_subtitles(self, aweme_detail, aweme_id, user_name):
        ...
        for caption in captions_info:
            ...
            caption_json = self._download_json(
                caption_url,
                aweme_id,
                note="Downloading captions",
                errnote="Unable to download captions",
                fatal=False,
            )
            if not caption_json:
                continue
            subtitles.setdefault(caption.get("language", "en"), []).append(
                {
                    ...
                    "data": "\n\n".join(
                        ...
                        for i, line in enumerate(caption_json["utterances"])
                        if line.get("text")
                    ),
                }
            )
        ...
        if not subtitles:
            if (
                ...
                aweme_detail, _ = self._extract_web_data_and_status(
                    self._create_url(user_name, aweme_id),
                    aweme_id,
                    fatal=False,
                )
            for caption in traverse_obj(
                aweme_detail, ("video", "subtitleInfos", lambda _, v: v["Url"])
            ):
                subtitles.setdefault(
                    caption.get("LanguageCodeName") or "en", []
                ).append(
                    {
                        "url": caption["Url"],
                        "ext": EXT_MAP.get(caption.get("Format")),
        ...
        for lang, subs_list in subtitles.items():
            subtitles[lang] = sorted(
                subs_list, key=lambda x: preference(x["ext"])
            )
    ...
    def _parse_aweme_video_app(self, aweme_detail):
        ...
        def extract_addr(addr, add_meta={}):
            ...
            if res:
                known_resolutions.setdefault(res, {}).setdefault(
                    "height", int_or_none(addr.get("height"))
                )
                known_resolutions[res].setdefault(
                    "width", int_or_none(addr.get("width"))
                )
            ...
            return [
                {
                    ...
                    "acodec": "aac",
                    "source_preference": (
                        -2 if "aweme/v1" in url else -1
                    ),  # Downloads from API might get blocked
                    **add_meta,
                    **parsed_meta,
                    # bytevc2 is bytedance's own custom h266/vvc codec, as-of-yet unplayable
                    "preference": -100 if is_bytevc2 else -1,
                    "format_note": join_nonempty(
                        add_meta.get("format_note"),
                        "(API)" if "aweme/v1" in url else None,
                        "(UNPLAYABLE)" if is_bytevc2 else None,
                        delim=" ",
                    ),
                    **audio_meta(url),
                }
            ...
            formats.extend(
                extract_addr(
                    download_addr,
                    {
                        ...
                        "format_note": "Download video%s"
                        % (
                            ", watermarked"
                            if video_info.get("has_watermark")
                            else ""
                        ),
                        "vcodec": "h264",
                        "width": dl_width,
                        "height": try_call(
                            lambda: int(dl_width / ratio)
                        ),  # download_addr['height'] is wrong
                        "preference": (
                            -2 if video_info.get("has_watermark") else -1
                        ),
                    },
                )
            )
                ...
                formats.extend(
                    extract_addr(
                        bitrate["play_addr"],
                        {
                            ...
                            "tbr": try_get(
                                bitrate, lambda x: x["bit_rate"] / 1000
                            ),
                            "vcodec": (
                                "h265"
                                if traverse_obj(
                                    bitrate, "is_bytevc1", "is_h265"
                                )
                                else "h264"
                            ),
                            "fps": bitrate.get("FPS"),
        ...
        if auth_cookie:
            for f in formats:
                self._set_cookie(
                    urllib.parse.urlparse(f["url"]).hostname,
                    "sid_tt",
                    auth_cookie.value,
                )
        ...
        labels = traverse_obj(
            aweme_detail, ("hybrid_label", ..., "text"), expected_type=str
        )

        contained_music_track = traverse_obj(
            music_info,
            ("matched_song", "title"),
            ("matched_pgc_sound", "title"),
            expected_type=str,
        )
        contained_music_author = traverse_obj(
            music_info,
            ("matched_song", "author"),
            ("matched_pgc_sound", "author"),
            "author",
            expected_type=str,
        )

        is_generic_og_trackname = music_info.get(
            "is_original_sound"
        ) and music_info.get("title") == "original sound - {}".format(
            music_info.get("owner_handle")
        )
        if is_generic_og_trackname:
            music_track, music_author = (
                contained_music_track or "original sound",
                contained_music_author,
            )
        else:
            music_track, music_author = music_info.get("title"), traverse_obj(
                music_info, ("author", {str})
            )
        ...
        return {
            ...
            **author_info,
            ...
            "artists": (
                re.split(r"(?:, | & )", music_author) if music_author else None
            ),
            "formats": formats,
            "subtitles": self.extract_subtitles(
                ...
                aweme_id,
                traverse_obj(
                    author_info, "uploader", "uploader_id", "channel_id"
                ),
            ),
            "thumbnails": [
                {
                    ...
                    "url": cover_url,
                    "preference": (
                        -1 if cover_id in ("cover", "origin_cover") else -2
                    ),
                }
                for cover_id in (
                    "cover",
                ...
                for cover_url in traverse_obj(
                    video_info, (cover_id, "url_list", ...)
                )
            ],
            "duration": (
                traverse_obj(
    ...
    def _extract_web_formats(self, aweme_detail):
        ...
        for bitrate_info in traverse_obj(
            video_info, ("bitrateInfo", lambda _, v: v["PlayAddr"]["UrlList"])
        ):
            ...
            format_info.update(
                {
                    ...
                    "filesize": traverse_obj(
                        bitrate_info, ("PlayAddr", "DataSize", {int_or_none})
                    ),
                }
            )
            ...
            for video_url in traverse_obj(
                bitrate_info, ("PlayAddr", "UrlList", ..., {url_or_none})
            ):
                formats.append(
                    {
                        **COMMON_FORMAT_INFO,
        ...
        play_quality = traverse_obj(
            formats, (lambda _, v: v["width"] == play_width, "quality", any)
        )

        for play_url in traverse_obj(
            video_info, ("playAddr", ((..., "src"), None), {url_or_none})
        ):
            formats.append(
                {
                    **COMMON_FORMAT_INFO,
        ...
        for download_url in traverse_obj(
            video_info, (("downloadAddr", ("download", "url")), {url_or_none})
        ):
            formats.append(
                {
                    **COMMON_FORMAT_INFO,
        ...
        if not formats and traverse_obj(
            aweme_detail, ("music", "playUrl", {url_or_none})
        ):
    ...
    def _parse_aweme_video_web(
        self, aweme_detail, webpage_url, video_id, extract_flat=False
    ):
        author_info = traverse_obj(
            aweme_detail,
            (
        ...
        return {
            ...
            "formats": (
                None
                if extract_flat
                else self._extract_web_formats(aweme_detail)
            ),
            "subtitles": (
                None
                if extract_flat
                else self.extract_subtitles(aweme_detail, video_id, None)
            ),
            ...
            **traverse_obj(
            ...
            "thumbnails": [
                ...
                for cover_url in traverse_obj(
                    aweme_detail, ((None, "video"), cover_id, {url_or_none})
                )
            ],
        }
...
class TikTokIE(TikTokBaseIE):
    ...
    def _real_extract(self, url):
        ...
        raise ExtractorError(
            f"Video not available, status code {status}", video_id=video_id
        )
...
class TikTokUserIE(TikTokBaseIE):
    ...
    def _entries(self, sec_uid, user_name):
        ...
        for page in itertools.count(1):
            ...
            for video in traverse_obj(
                response, ("itemList", lambda _, v: v["id"])
            ):
                video_id = video["id"]
                ...
                yield self.url_result(
                    webpage_url,
                    TikTokIE,
                    **self._parse_aweme_video_web(
                        video, webpage_url, video_id, extract_flat=True
                    ),
                )
            ...
            cursor = traverse_obj(
                response,
                ("itemList", -1, "createTime", {lambda x: int(x * 1e3)}),
            )
            ...
            if cursor < 1472706000000 or not traverse_obj(
                response, "hasMorePrevious"
            ):
                break
    ...
    def _get_sec_uid(self, user_url, user_name, msg):
        webpage = (
            self._download_webpage(
                ...
                note=f"Downloading {msg} webpage",
                errnote=f"Unable to download {msg} webpage",
            )
            or ""
        )
        return traverse_obj(
    ...
    def _real_extract(self, url):
        ...
        if mobj := re.fullmatch(r"MS4wLjABAAAA[\w-]{64}", user_name):
            ...
            sec_uid = self._get_sec_uid(
                self._UPLOADER_URL_FORMAT % user_name, user_name, "user"
            ) or self._get_sec_uid(
                self._UPLOADER_URL_FORMAT % f"{user_name}/live",
                user_name,
                "live",
            )
        ...
        if not sec_uid:
            webpage = (
                self._download_webpage(
                    f"https://www.tiktok.com/embed/@{user_name}",
                    user_name,
                    note="Downloading user embed page",
                    fatal=False,
                )
                or ""
            )
            ...
            for aweme_id in traverse_obj(
                data, ("videoList", ..., "id", {str})
            ):
                webpage_url = self._create_url(user_name, aweme_id)
                video_data, _ = self._extract_web_data_and_status(
                    webpage_url, aweme_id, fatal=False
                )
                sec_uid = self._parse_aweme_video_web(
                    video_data, webpage_url, aweme_id, extract_flat=True
                ).get("channel_id")
                if sec_uid:
                    break
        ...
        return self.playlist_result(
            self._entries(sec_uid, user_name), sec_uid, user_name
        )


class TikTokBaseListIE(
    TikTokBaseIE
...
class TikTokSoundIE(TikTokBaseListIE):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?tiktok\.com/music/[\w\.-]+-(?P<id>[\d]+)[/?#&]?"
    )
    _WORKING = False
...
class TikTokEffectIE(TikTokBaseListIE):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?tiktok\.com/sticker/[\w\.-]+-(?P<id>[\d]+)[/?#&]?"
    )
    _WORKING = False
...
class TikTokTagIE(TikTokBaseListIE):
    ...
    def _real_extract(self, url):
        ...
        tag_id = self._html_search_regex(
            r"snssdk\d*://challenge/detail/(\d+)", webpage, "tag ID"
        )
        return self.playlist_result(
            self._entries(tag_id, display_id), tag_id, display_id
        )
...
class TikTokCollectionIE(TikTokBaseIE):
        ...
        for page in itertools.count(1):
            ...
            for video in traverse_obj(
                ...
                author = (
                    traverse_obj(
                        video,
                        ("author", ("uniqueId", "secUid", "id"), {str}, any),
                    )
                    or "_"
                )
    ...
    def _real_extract(self, url):
        collection_id, title, user_name = self._match_valid_url(url).group(
            "id", "title", "user_id"
        )

        return self.playlist_result(
            self._entries(collection_id),
            collection_id,
            "-".join((user_name, title)),
        )
    ...
    def _real_extract(self, url):
        ...
        if not detail:
            ...
            raise ExtractorError(
                ...
                expected=not self._get_cookies(self._WEBPAGE_HOST).get(
                    "s_v_web_id"
                ),
            )
...
class TikTokLiveIE(TikTokBaseIE):
    ...
    def _real_extract(self, url):
        ...
        for quality, stream in traverse_obj(
            live_info,
            (
                "stream_url",
                "live_core_sdk_data",
                ...
                (
                    ...
                    {parse_inner},
                    {
                        ...
                        "resolution": (
                            "resolution",
                            {
                                lambda x: re.match(r"(?i)\d+x\d+|\d+p", x)
                                .group()
                                .lower()
                            },
                        ),
        ...
        for stream in ("hls", "rtmp"):
            stream_url = traverse_obj(
                live_info, ("stream_url", f"{stream}_pull_url", {url_or_none})
            )
            if stream_url:
                formats.append(
                    {
                        ...
                        "protocol": (
                            "m3u8_native" if stream == "hls" else "https"
                        ),
        ...
        for f_id, f_url in traverse_obj(
            live_info, ("stream_url", "flv_pull_url", {dict}), default={}
        ).items():
        ...
        uploader = uploader or traverse_obj(
            live_info, ("ownerInfo", "uniqueId"), ("owner", "display_id")
        )
        ...
        return {
            ...
            "uploader": uploader,
            "uploader_url": format_field(
                uploader, None, self._UPLOADER_URL_FORMAT
            )
            or None,
            "is_live": True,
            "formats": formats,
            ...
            **traverse_obj(
                live_info,
                {
                    ...
                    "concurrent_view_count": (
                        ("user_count", ("liveRoomStats", "userCount")),
                        {int_or_none},
                    ),
                },
                get_all=False,
