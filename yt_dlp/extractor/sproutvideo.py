...
class SproutVideoIE(InfoExtractor):
    _NO_SCHEME_RE = (
        r"//videos\.sproutvideo\.com/embed/(?P<id>[\da-f]+)/[\da-f]+"
    )
    ...
    def _policy_to_qs(policy, signature_key, as_string=False):
        ...
        return (
            urllib.parse.urlencode(query, doseq=True) if as_string else query
        )
    ...
    def _real_extract(self, url):
        ...
        if traverse_obj(data, "hls"):
            ...
            formats.extend(
                self._extract_m3u8_formats(
                    self._M3U8_URL_TMPL.format(**data),
                    video_id,
                    "mp4",
                    m3u8_id="hls",
                    headers=headers,
        ...
        if downloads := traverse_obj(
            data, ("downloads", {dict.items}, lambda _, v: url_or_none(v[1]))
        ):
        ...
        for sub_data in traverse_obj(
            data, ("subtitleData", lambda _, v: url_or_none(v["src"]))
        ):
...
class VidsIoIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if embed_url := next(
            SproutVideoIE._extract_embed_urls(url, webpage), None
        ):
