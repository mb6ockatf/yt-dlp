...
class FloatplaneIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:(?:www|beta)\.)?floatplane\.com/post/(?P<id>\w+)"
    )
    _TESTS = [
        {
    ...
    def _real_initialize(self):
        if not self._get_cookies("https://www.floatplane.com").get(
            "sails.sid"
        ):
    ...
    def _real_extract(self, url):
        ...
        post_data = self._download_json(
            "https://www.floatplane.com/api/v3/content/post",
            post_id,
            query={"id": post_id},
        ...
        if not any(
            traverse_obj(post_data, ("metadata", ("hasVideo", "hasAudio")))
        ):
            raise ExtractorError(
                "Post does not contain a video or audio track", expected=True
            )
        ...
        uploader_url = (
            format_field(
                post_data,
                [("creator", "urlname")],
                "https://www.floatplane.com/channel/%s/home",
            )
            or None
        )
        ...
        common_info = {
            ...
            "channel_url": urljoin(
                f"{uploader_url}/",
                traverse_obj(post_data, ("channel", "urlname")),
            ),
        ...
        for media in traverse_obj(
            post_data, (("videoAttachments", "audioAttachments"), ...)
        ):
            media_id = media["id"]
            media_typ = media.get("type") or "video"

            metadata = self._download_json(
                f"https://www.floatplane.com/api/v3/content/{media_typ}",
                media_id,
                query={"id": media_id},
            ...
            stream = self._download_json(
                "https://www.floatplane.com/api/v2/cdn/delivery",
                media_id,
                query={
            ...
            for quality in traverse_obj(
                stream, ("resource", "data", "qualityLevels", ...)
            ):
                url = urljoin(
                    stream["cdn"],
                    format_path(
                ...
                formats.append(
                    {
                        ...
                        **parse_codecs(quality.get("codecs")),
                        "url": url,
                        "ext": determine_ext(
                            url.partition("/chunk.m3u8")[0], "mp4"
                        ),
                    }
                )
...
class FloatplaneChannelIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        channel_data = (
            traverse_obj(
                creator_data,
                ("channels", lambda _, v: v["urlname"] == channel),
                get_all=False,
            )
            or {}
        )
