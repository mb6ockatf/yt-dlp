...
class ViuBaseIE(InfoExtractor):
    def _call_api(self, path, *args, headers={}, **kwargs):
        response = self._download_json(
            f"https://www.viu.com/api/{path}",
            *args,
            **kwargs,
            headers={**self.geo_verification_headers(), **headers},
        )["response"]
        if response.get("status") != "success":
            raise ExtractorError(
                f'{self.IE_NAME} said: {response["message"]}', expected=True
            )
        return response
...
class ViuOTTIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        product_data = self._download_json(
            f"http://www.viu.com/ott/{country_code}/index.php",
            video_id,
        ...
        if self._yes_playlist(series_id, video_id, idata):
            ...
            if product:
                entries = []
                for entry in sorted(
                    product, key=lambda x: int_or_none(x.get("number", 0))
                ):
        ...
        query = {
            ...
            "language_flag_id": self._LANGUAGE_FLAG.get(lang_code.lower())
            or "3",
        }
            ...
            if duration_limit:
                ...
                query = dict(
                    urllib.parse.parse_qsl(
                        old_stream_url.query, keep_blank_values=True
                    )
                )
                query.update(
                    {
                        "duration": video_data.get("time_duration")
                        or "9999999",
                        "duration_start": "0",
                    }
                )
            ...
            formats.append(
                {
                    ...
                    "ext": "mp4",
                    "filesize": try_get(
                        stream_data, lambda x: x["size"][vid_format], int
                    ),
                }
            )
...
class ViuOTTIndonesiaBaseIE(InfoExtractor):
    ...
    def _real_initialize(self):
        ViuOTTIndonesiaBaseIE._TOKEN = self._download_json(
            ...
            note="Downloading token information",
        )["token"]


class ViuOTTIndonesiaIE(ViuOTTIndonesiaBaseIE):
    _VALID_URL = (
        r"https?://www\.viu\.com/ott/\w+/\w+/all/video-[\w-]+-(?P<id>\d+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        for key, url in initial_state.items():
            lang, ext = self._search_regex(
                r"^subtitle_(?P<lang>[\w-]+)_(?P<ext>\w+)$",
                key,
                "subtitle metadata",
                default=(None, None),
                group=("lang", "ext"),
        ...
        episode = (
            traverse_obj(
                list(
                    filter(
                        lambda x: x.get("@type") in ("TVEpisode", "Movie"),
        ...
        return {
            "id": display_id,
            "title": (
                traverse_obj(initial_state, "title", "display_title")
                or episode.get("name")
            ),
            ...
            "cast": traverse_obj(
                episode, ("actor", ..., "name"), default=None
            ),
