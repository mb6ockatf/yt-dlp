...
class GronkhIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        data_json = self._download_json(
            f"https://api.gronkh.tv/v1/video/info?episode={video_id}", video_id
        )
        m3u8_url = self._download_json(
            f"https://api.gronkh.tv/v1/video/playlist?episode={video_id}",
            video_id,
        )["playlist_url"]
        ...
        return {
            ...
            "chapters": traverse_obj(
                ...
                (
                    "chapters",
                    lambda _, v: float_or_none(v["offset"]) is not None,
                    {
                        "title": "title",
    ...
    def _entries(self):
        for type_ in ("recent", "views"):
            info = self._download_json(
                f"https://api.gronkh.tv/v1/video/discovery/{type_}",
                "feed",
                note=f"Downloading {type_} API JSON",
            )
            for item in traverse_obj(info, ("discovery", ...)) or []:
                yield self.url_result(
                    f'https://gronkh.tv/watch/stream/{item["episode"]}',
                    GronkhIE,
                    item.get("title"),
                )
...
class GronkhVodsIE(InfoExtractor):
    ...
    def _fetch_page(self, page):
        items = traverse_obj(
            self._download_json(
                "https://api.gronkh.tv/v1/search",
                "vods",
                query={
                    "offset": self._PER_PAGE * page,
    ...
    def _real_extract(self, url):
        entries = OnDemandPagedList(
            functools.partial(self._fetch_page), self._PER_PAGE
        )
