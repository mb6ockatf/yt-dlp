...
class Art19IE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        player_metadata = self._download_json(
            f"https://art19.com/episodes/{episode_id}",
            episode_id,
            note="Downloading player metadata",
            fatal=False,
...
class Art19ShowIE(InfoExtractor):
    _VALID_URL_BASE = (
        r"https?://(?:www\.)?art19\.com/shows/(?P<id>[\w-]+)(?:/embed)?/?"
    )
    _VALID_URL = [
        rf"{_VALID_URL_BASE}(?:$|[#?])",
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "entries": [
                self.url_result(
                    f"https://rss.art19.com/episodes/{episode_id}.mp3", Art19IE
                )
                for episode_id in traverse_obj(
                    series_metadata, ("series", "episode_ids", ..., {str})
                )
            ],
            **traverse_obj(
                series_metadata,
