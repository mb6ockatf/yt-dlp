    ...
    def _real_extract(self, url):
        ...
        def livx_mode(mode):
            ...
            return update_url_query(
                f"https://r.dcs.redcdn.pl/{mode}/o2/{tenant}/{path}.livx{suffix}",
                file_qs,
            )
        ...
        video_id = join_nonempty(
            title.replace("/", "-"), start_time, stop_time
        )
        ...
        if ism_res is not False:
            ...
            formats, _ = self._parse_ism_formats_and_subtitles(
                ism_doc, ism_urlh.url, "ss"
            )

        nvr_urlh = self._request_webpage(
            HEADRequest(livx_mode("nvr")),
            video_id,
            "Follow flv file redirect",
            fatal=False,
        ...
        formats.extend(
            self._extract_mpd_formats(
                livx_mode("livedash"), video_id, mpd_id="dash", fatal=False
            )
        )
        formats.extend(
        ...
        time_scale = (
            traverse_obj(ism_doc, ("@TimeScale", {int_or_none})) or 10000000
        )
        duration = (
            traverse_obj(
                ism_doc, ("@Duration", {float_or_none(scale=time_scale)})
            )
            or None
        )
