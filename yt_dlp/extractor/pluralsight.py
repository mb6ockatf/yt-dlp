...
query BootstrapPlayer {
  rpc {
    ...
    def _download_course(self, course_id, url, display_id):
        try:
            ...
            return self._download_json(
                ...
                display_id,
                data=urlencode_postdata({"courseId": course_id}),
                headers={"Referer": url},
            )
    ...
    def _download_course_rpc(self, course_id, url, display_id):
        ...
        course = try_get(
            response,
            lambda x: x["data"]["rpc"]["bootstrapPlayer"]["course"],
            dict,
        )
        if course:
            return course
...
class PluralsightIE(PluralsightBaseIE):
    IE_NAME = "pluralsight"
    _VALID_URL = (
        r"https?://(?:(?:www|app)\.)?pluralsight\.com/(?:training/)?player\?"
    )
  ...
  viewClip(input: {
    ...
    def _perform_login(self, username, password):
        ...
        if all(
            ...
            if any(
                p in response for p in (MUST_AGREE, ">Disagree<", ">Agree<")
            ):
                raise ExtractorError(
    ...
    def _get_subtitles(
        self, author, clip_idx, clip_id, lang, name, duration, video_id
    ):
        captions = None
        if clip_id:
        ...
        if not captions:
            ...
            captions = self._download_json(
                f"{self._API_BASE}/player/retrieve-captions",
                video_id,
                "Downloading captions JSON",
                "Unable to download captions JSON",
                fatal=False,
                data=json.dumps(captions_post).encode(),
    ...
    def _convert_subtitles(duration, subs):
        ...
        for num, current in enumerate(subs):
            ...
            start, text = (
                float_or_none(
                    dict_get(
                        current, TIME_OFFSET_KEYS, skip_false_values=False
                    )
                ),
        ...
        if self.get_param("listformats", False):
            ...
            def guess_allowed_qualities():
                ...
                if len(req_format_split) > 1:
                    ...
                    for allowed_quality in ALLOWED_QUALITIES:
                        if (
                            req_ext == allowed_quality.ext
                            and req_quality in allowed_quality.qualities
                        ):
                            return (AllowedQuality(req_ext, (req_quality,)),)
                req_ext = (
                    "webm" if self.get_param("prefer_free_formats") else "mp4"
                )
            ...
            for quality in qualities_:
                ...
                try:
                    viewclip = self._download_json(
                        ...
                        data=json.dumps(
                            {
                                "query": self.GRAPHQL_VIEWCLIP_TMPL
                                % clip_post,
                                "variables": {},
                            }
                        ).encode(),
                    ...
                    viewclip = self._download_json(
                        f"{self._API_BASE}/video/clips/viewclip",
                        display_id,
                        f"Downloading {format_id} viewclip JSON",
                        fatal=False,
                    ...
                    clip_f.update(
                        {
                            ...
                            "ext": ext,
                            "format_id": (
                                f"{format_id}-{cdn}" if cdn else format_id
                            ),
                            "quality": quality_key(quality),
                            "source_preference": int_or_none(
                                clip_url_data.get("rank")
                            ),
                        }
                    )
                    formats.append(clip_f)

        duration = int_or_none(clip.get("duration")) or parse_duration(
...
class PluralsightCourseIE(PluralsightBaseIE):
    ...
    def _real_extract(self, url):
        ...
        description = course.get("description") or course.get(
            "shortDescription"
        )
