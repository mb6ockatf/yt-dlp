...
class DPlayBaseIE(InfoExtractor):
    ...
    def _update_disco_api_headers(
        self, headers, disco_base, display_id, realm
    ):
        headers["Authorization"] = self._get_auth(
            disco_base, display_id, realm, False
        )
    ...
    def _get_disco_api_info(
        self, url, display_id, disco_host, realm, country, domain=""
    ):
...
class DPlayIE(DPlayBaseIE):
    ...
    def _real_extract(self, url):
        ...
        country = (
            mobj.group("country")
            or mobj.group("subdomain_country")
            or mobj.group("plus_country")
        )
        host = (
            "disco-api." + domain
            if domain[0] == "d"
            else "eu2-prod.disco-api.com"
        )
...
class DiscoveryPlusBaseIE(DPlayBaseIE):
    ...
    def _real_extract(self, url):
        return self._get_disco_api_info(
            url, self._match_id(url), **self._DISCO_API_PARAMS
        )
...
class GoDiscoveryIE(DiscoveryPlusBaseIE):
    _VALID_URL = (
        r"https?://(?:go\.)?discovery\.com/video" + DPlayBaseIE._PATH_REGEX
    )
    _TESTS = [
        {
...
class TravelChannelIE(DiscoveryPlusBaseIE):
    _VALID_URL = (
        r"https?://(?:watch\.)?travelchannel\.com/video"
        + DPlayBaseIE._PATH_REGEX
    )
    _TESTS = [
        {
...
class CookingChannelIE(DiscoveryPlusBaseIE):
    _VALID_URL = (
        r"https?://(?:watch\.)?cookingchanneltv\.com/video"
        + DPlayBaseIE._PATH_REGEX
    )
    _TESTS = [
        {
...
class HGTVUsaIE(DiscoveryPlusBaseIE):
    _VALID_URL = (
        r"https?://(?:watch\.)?hgtv\.com/video" + DPlayBaseIE._PATH_REGEX
    )
    _TESTS = [
        {
...
class FoodNetworkIE(DiscoveryPlusBaseIE):
    _VALID_URL = (
        r"https?://(?:watch\.)?foodnetwork\.com/video"
        + DPlayBaseIE._PATH_REGEX
    )
    _TESTS = [
        {
...
class DestinationAmericaIE(DiscoveryPlusBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?destinationamerica\.com/video"
        + DPlayBaseIE._PATH_REGEX
    )
    _TESTS = [
        {
...
class InvestigationDiscoveryIE(DiscoveryPlusBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?investigationdiscovery\.com/video"
        + DPlayBaseIE._PATH_REGEX
    )
    _TESTS = [
        {
...
class AmHistoryChannelIE(DiscoveryPlusBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?ahctv\.com/video" + DPlayBaseIE._PATH_REGEX
    )
    _TESTS = [
        {
...
class ScienceChannelIE(DiscoveryPlusBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?sciencechannel\.com/video"
        + DPlayBaseIE._PATH_REGEX
    )
    _TESTS = [
        {
...
class DiscoveryLifeIE(DiscoveryPlusBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?discoverylife\.com/video"
        + DPlayBaseIE._PATH_REGEX
    )
    _TESTS = [
        {
...
class AnimalPlanetIE(DiscoveryPlusBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?animalplanet\.com/video" + DPlayBaseIE._PATH_REGEX
    )
    _TESTS = [
        {
...
class DiscoveryPlusIE(DiscoveryPlusBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?discoveryplus\.com/(?!it/)(?:(?P<country>[a-z]{2})/)?video(?:/sport|/olympics)?"
        + DPlayBaseIE._PATH_REGEX
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        return self._get_disco_api_info(
            url, video_id, **self._DISCO_API_PARAMS
        )


class DiscoveryPlusIndiaIE(DiscoveryPlusBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?discoveryplus\.in/videos?"
        + DPlayBaseIE._PATH_REGEX
    )
    _TESTS = [
        {
...
class DiscoveryPlusShowBaseIE(DPlayBaseIE):
    ...
    def _entries(self, show_name):
        headers = {
            ...
            "Authentication": self._get_auth(
                self._BASE_API, None, self._REALM
            ),
        }
        show_json = self._download_json(
            ...
            video_id=show_name,
            headers=headers,
        )["included"][self._INDEX]["attributes"]["component"]
        ...
        for season in show_json["filters"][0]["options"]:
            ...
            while page_num < total_pages:
                season_json = self._download_json(
                    season_url.format(season_id, show_id, str(page_num + 1)),
                    show_name,
                    headers=headers,
                    note="Downloading season {} JSON metadata{}".format(
                        season_id, f" page {page_num}" if page_num else ""
                    ),
                )
                if page_num == 0:
                    total_pages = (
                        try_get(
                            season_json, lambda x: x["meta"]["totalPages"], int
                        )
                        or 1
                    )
                ...
                for episode in episodes_json:
                    ...
                    yield self.url_result(
                        ...
                        ie=self._VIDEO_IE.ie_key(),
                        video_id=episode.get("id") or video_path,
                    )
                page_num += 1
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._entries(show_name), playlist_id=show_name
        )


class DiscoveryPlusItalyIE(DiscoveryPlusBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?discoveryplus\.com/it/video(?:/sport|/olympics)?"
        + DPlayBaseIE._PATH_REGEX
    )
    _TESTS = [
        {
