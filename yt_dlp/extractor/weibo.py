...
class WeiboBaseIE(InfoExtractor):
    ...
    def _extract_formats(self, video_info):
        ...
        formats = traverse_obj(
            ...
            (
                "playback_list",
                lambda _, v: url_or_none(v["play_info"]["url"]),
                "play_info",
                {
                    "url": "url",
    ...
    def _parse_video_info(self, video_info):
        ...
        return {
            ...
            **traverse_obj(
            ...
            "tags": traverse_obj(
                video_info, ("topic_struct", ..., "topic_title", {str})
            )
            or None,
        }
...
class WeiboIE(WeiboBaseIE):
    ...
    def _entries(self, mix_media_info):
        for media_info in traverse_obj(
            mix_media_info, lambda _, v: v["type"] != "pic"
        ):
            yield self._parse_video_info(
                traverse_obj(
                    media_info,
                    {
                        "id": ("data", "object_id"),
                        "page_info": {
                            "media_info": ("data", "media_info", {dict})
                        },
                    },
                )
            )
...
class WeiboUserIE(WeiboBaseIE):
    ...
    def _fetch_page(self, uid, cursor=0, page=1):
        return self._weibo_download_json(
            ...
            uid,
            note=f"Downloading videos page {page}",
            query={"uid": uid, "cursor": cursor},
        )["data"]
    ...
    def _entries(self, uid, first_page):
        ...
        for page in itertools.count(1):
            response = (
                first_page
                if page == 1
                else self._fetch_page(uid, cursor, page)
            )
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._entries(uid, first_page), uid, **metainfo
        )
