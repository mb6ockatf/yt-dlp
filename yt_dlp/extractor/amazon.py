    ...
    def _real_extract(self, url):
        ...
        if not formats:
            self.raise_no_formats(
                "No video found for this customer review", expected=True
            )
        ...
        return {
            ...
            "title": (
                clean_html(
                    get_element_by_attribute(
                        "data-hook", "review-title", webpage
                    )
                )
            ...
            "uploader": clean_html(
                get_element_by_class("a-profile-name", webpage)
            ),
            "average_rating": float_or_none(
                clean_html(
                    get_element_by_attribute(
                        "data-hook", "review-star-rating", webpage
                    )
                    or ""
                ).partition(" ")[0]
            ),
