...
class MedalTVIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        webpage = self._download_webpage(
            url, video_id, query={"mobilebypass": "true"}
        )

        hydration_data = self._search_json(
            r"<script[^>]*>[^<]*\bhydrationData\s*=",
            webpage,
            "next data",
            video_id,
            end_pattern="</script>",
        ...
        def add_item(
            container, item_url, height, id_key="format_id", item_id=None
        ):
        ...
        if not formats and error:
            if error == 404:
                ...
                self.raise_no_formats(
                    f"An unknown error occurred ({error}).", video_id=video_id
                )

        # Necessary because the id of the author is not known in advance.
        # Won't raise an issue if no profile can be found as this is optional.
        author = (
            traverse_obj(hydration_data, ("profiles", ...), get_all=False)
            or {}
        )
