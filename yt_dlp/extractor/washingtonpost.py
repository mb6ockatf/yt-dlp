...
class WashingtonPostArticleIE(InfoExtractor):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?washingtonpost\.com/(?:[^/]+/)*(?P<id>[^/?#]+)"
    )
    _TESTS = [
        {
    ...
    def suitable(cls, url):
        return (
            False if WashingtonPostIE.suitable(url) else super().suitable(url)
        )
    ...
    def _real_extract(self, url):
        ...
        entries = [
            self.url_result(f"washingtonpost:{uuid}", "WashingtonPost", uuid)
            for uuid in uuids
        ]
