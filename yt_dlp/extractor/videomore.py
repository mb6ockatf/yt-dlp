...
class VideomoreIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        partner_id = (
            mobj.group("partner_id")
            or parse_qs(url).get("partner_id", [None])[0]
            or "97"
        )

        item = self._download_json(
            "https://siren.more.tv/player/config",
            video_id,
            query={
...
class VideomoreVideoIE(VideomoreBaseIE):
    ...
    _VALID_URL = (
        VideomoreBaseIE._VALID_URL_BASE
        + r"(?P<id>(?:(?:[^/]+/){2})?[^/?#&]+)(?:/*|[?#&].*?)$"
    )
    _TESTS = [
        {
...
class VideomoreSeasonIE(VideomoreBaseIE):
    ...
    def suitable(cls, url):
        return (
            False
            if (VideomoreIE.suitable(url) or VideomoreVideoIE.suitable(url))
    ...
    def _real_extract(self, url):
        ...
        tracks = self._download_json(
            self._API_BASE_URL + f"seasons/{season_id}/tracks", season_id
        )["data"]
        entries = []
        for track in tracks:
