...
class SimplecastBaseIE(InfoExtractor):
    ...
    def _call_search_api(self, resource, resource_id, resource_url):
        return self._download_json(
            f"https://api.simplecast.com/{resource}s/search",
            resource_id,
    ...
    def _parse_episode(self, episode):
        ...
        audio_file_url = (
            audio_file.get("url")
            or episode.get("audio_file_url")
            or episode["enclosure_url"]
        )
        ...
        return {
            ...
            "filesize": int_or_none(
                audio_file.get("size") or episode.get("audio_file_size")
            ),
        }
...
class SimplecastEpisodeIE(SimplecastBaseIE):
    ...
    _VALID_URL = (
        r"https?://(?!api\.)[^/]+\.simplecast\.com/episodes/(?P<id>[^/?&#]+)"
    )
    _TEST = {
