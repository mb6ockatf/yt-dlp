...
class BravoTVIE(AdobePassIE):
    ...
    def _real_extract(self, url):
        ...
        tve = extract_attributes(
            get_element_html_by_class("tve-video-deck-app", webpage) or ""
        )
        query = {
            "manifest": "m3u",
            "formats": "m3u,mpeg4",
        ...
        if tve:
            ...
            if tve.get("data-entitlement") == "auth":
                ...
                resource = self._get_mvpd_resource(
                    tve.get("data-adobe-pass-resource-id")
                    or auth.get("adobePassResourceId")
                    or site,
                    tve["data-title"],
                    release_pid,
                    tve.get("data-rating"),
            ...
            ls_playlist = (
                traverse_obj(
                    settings, ("ls_playlist", ..., {dict}), get_all=False
                )
                or {}
            )
            ...
            metadata = traverse_obj(
                ls_playlist,
                ("videos", lambda _, v: v["guid"] == video_id, {dict}),
                get_all=False,
            )
        ...
        m3u8_url = self._request_webpage(
            HEADRequest(update_url_query(f"{tp_url}/stream.m3u8", query)),
            video_id,
            "Checking m3u8 URL",
        ).url
            ...
            **merge_dicts(
                traverse_obj(
                    tp_metadata,
                    {
                        ...
                        "season_number": (
                            ("pl1$seasonNumber", "nbcu$seasonNumber"),
                            {int_or_none},
                        ),
                        "episode_number": (
                            ("pl1$episodeNumber", "nbcu$episodeNumber"),
                            {int_or_none},
                        ),
                        "series": (
                            ("pl1$show", "nbcu$show"),
