...
class SubsplashBaseIE(InfoExtractor):
    def _get_headers(self, url, display_id):
        token = try_call(
            lambda: self._get_cookies(url)["ss-token-guest"].value
        )
        if not token:
    ...
    def _extract_video(self, data, video_id):
        ...
        m3u8_url = traverse_obj(
            video_data,
            ("playlists", 0, "_links", "related", "href", {url_or_none}),
        )
        if m3u8_url:
            formats.extend(
        ...
        mp4_entry = traverse_obj(
            ...
            (
                "video-outputs",
                lambda _, v: url_or_none(v["_links"]["related"]["href"]),
                any,
            ),
        )
        if mp4_entry:
...
class SubsplashIE(SubsplashBaseIE):
    ...
    def _real_extract(self, url):
        ...
        data = self._download_json(
            ...
            video_id,
            headers=self._get_headers(url, video_id),
            query={
        ...
        return self._extract_video(
            traverse_obj(data, ("_embedded", "media-items", 0)), video_id
        )
...
class SubsplashPlaylistIE(SubsplashBaseIE):
    ...
    def _entries(self, series_id, headers, page):
        ...
        for entry in traverse_obj(
            data, ("_embedded", "media-items", lambda _, v: v["short_code"])
        ):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            InAdvancePagedList(
                functools.partial(self._entries, series_data["id"], headers),
                total_pages,
                self._PAGE_SIZE,
            ),
            display_id,
