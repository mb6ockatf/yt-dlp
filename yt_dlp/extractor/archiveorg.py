    ...
    def _real_extract(self, url):
        ...
        info = {
            ...
            "creators": traverse_obj(
                m, ("creator", {variadic}, {lambda x: x[0] and list(x)})
            ),
            "license": m.get("licenseurl"),
            "release_date": unified_strdate(m.get("date")),
            "timestamp": unified_timestamp(
                dict_get(m, ["publicdate", "addeddate"])
            ),
        ...
        for f in metadata["files"]:
            ...
            is_logged_in = bool(
                self._get_cookies("https://archive.org").get("logged-in-sig")
            )
                ...
                entry["formats"].append(
                    {
                        ...
                        "protocol": "https",
                        "source_preference": (
                            0 if f.get("source") == "original" else -1
                        ),
...
class YoutubeWebArchiveIE(InfoExtractor):
    ...
    _YT_INITIAL_PLAYER_RESPONSE_RE = rf"""(?x:
    ...
    def _call_cdx_api(
        ...
        res = self._download_json(
            ...
            item_id,
            note or "Downloading CDX API JSON",
            query=query,
            fatal=fatal,
        )
        if isinstance(res, list) and len(res) >= 2:
            ...
            self.report_warning(
                "Error while parsing CDX API response" + bug_reports_message()
            )
    ...
    def _extract_metadata(self, video_id, webpage):
        ...
        ytcfg = {}
        for j in re.findall(
            r"yt\.setConfig\(\s*(?P<json>{\s*(?s:.+?)\s*})\s*\);", webpage
        ):  # ~June 2010
            ytcfg.update(
                self._parse_json(
                    j,
        ...
        player_config = (
            ...
            or ytcfg.get("PLAYER_CONFIG")
            or {}
        )

        # XXX: this may also contain a 'creator' key.
        swf_args = self._search_json(
            r"swfArgs\s*=", webpage, "swf config", video_id, default={}
        )
        ...
        if not player_response:
            ...
            player_response = self._parse_json(
                traverse_obj(player_config, ("args", "player_response"))
                or "{}",
                video_id,
                fatal=False,
            )
        ...
        uploader_or_channel_url = self._search_regex(
            [
                rf"<(?:link\s*itemprop=\"url\"|a\s*id=\"watch-username\").*?\b{_CHANNEL_URL_HREF_RE}>",  # @fd05024
                rf"<div\s*id=\"(?:watch-channel-stats|watch-headline-user-info)\"[^>]*>\s*<a[^>]*\b{_CHANNEL_URL_HREF_RE}",
            ],  # ~ May 2009, ~June 2012
            webpage,
            "uploader or channel url",
            default=None,
        )

        owner_profile_url = url_or_none(
            microformats.get("ownerProfileUrl")
        )  # @a6211d2
        ...
        uploader_url = (
            f"https://www.youtube.com/user/{uploader_id}"
            if uploader_id
            else None
        )
        ...
        uploader = (
            ...
            or self._html_search_regex(
                [
                    ...
                    r"(?s)<a[^>]*yt-user-name[^>]*>\s*(.*?)\s*</a",
                ],  # july 2013
                get_element_by_id("watch7-user-header", webpage),
                "uploader",
                default=None,
            )
            or self._html_search_regex(
                r'<button\s*href="/user/[^>]*>\s*<span[^>]*>\s*(.+?)\s*<',  # April 2012
                get_element_by_id("watch-headline-user-info", webpage),
                "uploader",
                default=None,
            )
        ...
        duration = int_or_none(
            ...
            or traverse_obj(
                player_config, ("args", ("length_seconds", "l")), get_all=False
            )
        ...
        description = (
            ...
            or clean_html(
                get_element_by_id("eow-description", webpage)
            )  # @9e6dd23
            or search_meta(
    ...
    def _extract_thumbnails(self, video_id):
        ...
        thumbnail_base_urls = [
            ...
            for server in (
                self._YT_ALL_THUMB_SERVERS
                if try_all
                else self._YT_DEFAULT_THUMB_SERVERS
            )
            for ext in (("jpg", "webp") if try_all else ("jpg",))
        ]
        ...
        for url in thumbnail_base_urls:
            response = self._call_cdx_api(
                video_id,
                url,
                filters=["mimetype:image/(?:webp|jpeg)"],
                collapse=["urlkey"],
                query={"matchType": "prefix"},
            )
            if not response:
                continue
            thumbnails.extend(
                {
                    "url": (
                        ...
                        % (
                            int_or_none(thumbnail_dict.get("timestamp"))
                            or self._OLDEST_CAPTURE_DATE
                        )
                    )
    ...
    def _get_capture_dates(self, video_id, url_date):
        ...
        all_captures = sorted(
            int_or_none(r["timestamp"])
            for r in response
            if int_or_none(r["timestamp"]) is not None
        )
    ...
    def _real_extract(self, url):
        video_id, url_date, url_date_2 = self._match_valid_url(url).group(
            "id", "date", "date2"
        )
        ...
        if retry_manager.error:
            self.raise_no_formats(
                retry_manager.error, expected=True, video_id=video_id
            )
        ...
        self.write_debug(
            "Captures to try: " + join_nonempty(*capture_dates, delim=", ")
        )
        info = {"id": video_id}
        for capture in capture_dates:
            webpage = self._download_webpage(
                (self._WAYBACK_BASE_URL + "http://www.youtube.com/watch?v=%s")
                % (capture, video_id),
                video_id=video_id,
                fatal=False,
