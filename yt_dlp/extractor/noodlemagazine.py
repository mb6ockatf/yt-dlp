...
class NoodleMagazineIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www|adult\.)?noodlemagazine\.com/watch/(?P<id>[0-9-_]+)"
    )
    _TEST = {
    ...
    def _real_extract(self, url):
        ...
        description = self._og_search_property(
            "description", webpage, default=""
        ...
        view_count = parse_count(
            self._html_search_meta("ya:ovs:views_total", webpage, default=None)
        )
        like_count = parse_count(
            self._html_search_meta("ya:ovs:likes", webpage, default=None)
        )
        upload_date = unified_strdate(
            self._html_search_meta("ya:ovs:upload_date", webpage, default="")
        )
        ...
        for source in traverse_obj(
            playlist_info, ("sources", lambda _, v: v["file"])
        ):
            if source.get("type") == "hls":
                formats.extend(
                    self._extract_m3u8_formats(
                        build_url(source["file"]),
                        video_id,
                        "mp4",
                        fatal=False,
                        m3u8_id="hls",
        ...
        return {
            ...
            "thumbnail": self._og_search_property(
                "image", webpage, default=None
            )
            or playlist_info.get("image"),
            "duration": duration,
