        ...
        return json.loads(
            self._download_json(
                ...
                data=urlencode_postdata(
                    {
                        "f.req": json.dumps(
                            [[[func_id, json.dumps(params), None, "1"]]]
                        ),
                    }
                ),
                transform_source=lambda x: self._search_regex(
                    r"(?s)(\[.+\])", x, "data"
                ),
            )[0][2]
        )
...
class GooglePodcastsIE(GooglePodcastsBaseIE):
    ...
    _VALID_URL = (
        GooglePodcastsBaseIE._VALID_URL_BASE
    ...
    def _real_extract(self, url):
        ...
        episode = self._batch_execute(
            "oNjqVe", b64_guid, [b64_feed_url, b64_guid]
        )[1]
...
class GooglePodcastsFeedIE(GooglePodcastsBaseIE):
    ...
    _VALID_URL = (
        GooglePodcastsBaseIE._VALID_URL_BASE + r"(?P<id>[^/?&#]+)/?(?:[?#&]|$)"
    )
    _TEST = {
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            entries,
            playlist_title=try_get(feed, lambda x: x[0]),
