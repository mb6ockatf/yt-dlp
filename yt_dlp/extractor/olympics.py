...
class OlympicsReplayIE(InfoExtractor):
    ...
    def _extract_from_nextjs_data(self, webpage, video_id):
        data = traverse_obj(
            ...
            (
                ...
                "page",
                "items",
                lambda _, v: v["name"] == "videoPlaylist",
                "data",
                "currentVideo",
                {dict},
                any,
        ...
        m3u8_url = (
            traverse_obj(data, ("videoUrl", {url_or_none}))
            or data["streamUrl"]
        )
        tokenized_url = self._tokenize_url(
            m3u8_url, data["jwtToken"], is_live, video_id
        )
        ...
        try:
            ...
            if (
                ...
                and "georestricted" in e.cause.msg
            ):
                self.raise_geo_restricted(countries=geo_countries)
            raise
    ...
    def _tokenize_url(self, url, token, is_live, video_id):
        return self._download_json(
            "https://metering.olympics.com/tokengenerator",
            video_id,
            "Downloading tokenized m3u8 url",
            query={
                **parse_qs(url),
    ...
    def _legacy_tokenize_url(self, url, video_id):
        return self._download_json(
            "https://olympics.com/tokenGenerator",
            video_id,
            "Downloading legacy tokenized m3u8 url",
            query={"url": url},
        )
    ...
    def _real_extract(self, url):
        ...
        if not thumbnails_list:
            ...
            thumbnails_list = (
                thumbnails_list.replace("[", "").replace("]", "").split(",")
            )
            thumbnails_list = [
                thumbnail.strip() for thumbnail in thumbnails_list
            ]
        thumbnails = []
        for thumbnail in thumbnails_list:
            ...
            width_a, height_a, width = (
                int_or_none(width_a),
                int_or_none(height_a),
                int_or_none(width),
            )
            thumbnails.append(
                {
                    ...
                    "width": width,
                    "height": int_or_none(
                        try_get(width, lambda x: x * height_a / width_a)
                    ),
                }
            )

        formats, subtitles = self._extract_m3u8_formats_and_subtitles(
            self._legacy_tokenize_url(m3u8_url, video_uuid),
            video_uuid,
            "mp4",
            m3u8_id="hls",
        )
