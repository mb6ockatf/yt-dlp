...
class TencentBaseIE(InfoExtractor):
    ...
    def _get_ckey(self, video_id, url, guid):
        ...
        payload = (
            f"{video_id}|{int(time.time())}|mg3c3b04ba|{self._APP_VERSION}|{guid}|"
    ...
    def _get_video_api_response(
        ...
        return self._search_json(
            r"QZOutputJson=",
            self._download_webpage(self._API_URL, video_id, query=query),
            "api_response",
            video_id,
        )
    ...
    def _extract_video_formats_and_subtitles(self, api_response, video_id):
        ...
        for video_format in video_response["ul"]["ui"]:
            if (
                video_format.get("hls")
                or determine_ext(video_format["url"]) == "m3u8"
            ):
                fmts, subs = self._extract_m3u8_formats_and_subtitles(
                    video_format["url"]
                    + traverse_obj(video_format, ("hls", "pt"), default=""),
                    video_id,
                    "mp4",
                    fatal=False,
                )
        ...
        format_response = (
            traverse_obj(
                api_response,
                ("fl", "fi", lambda _, v: v["br"] == identifier),
                expected_type=dict,
                get_all=False,
            )
            or {}
        ...
        common_info = {
            ...
            "abr": float_or_none(
                format_response.get("audiobandwidth"), scale=1000
            ),
            ...
            "dynamic_range": {"hdr10": "hdr10"}.get(
                format_response.get("name"), "sdr"
            ),
            ...
            subtitles.setdefault(subtitle["lang"].lower(), []).append(
                {
                    "url": subtitle["url"],
                    "ext": (
                        "srt" if subtitle.get("captionType") == 1 else "vtt"
                    ),
                    "protocol": (
                        "m3u8_native"
                        if determine_ext(subtitle["url"]) == "m3u8"
                        else "http"
                    ),
                }
            )
...
class VQQVideoIE(VQQBaseIE):
    ...
    def _real_extract(self, url):
        video_id, series_id = self._match_valid_url(url).group(
            "id", "series_id"
        )
        ...
        return {
            ...
            "series": traverse_obj(
                webpage_metadata, ("global", "coverInfo", "title")
            ),
        }
...
class VQQSeriesIE(VQQBaseIE):
    IE_NAME = "vqq:series"
    _VALID_URL = (
        VQQBaseIE._VALID_URL_BASE + r"/x/cover/(?P<id>\w+)\.html/?(?:[?#]|$)"
    )
    ...
    def _real_extract(self, url):
        ...
        episode_paths = [
            f"/x/cover/{series_id}/{video_id}.html"
        ...
        return self.playlist_from_matches(
            ...
            getter=urljoin(url),
            title=self._get_clean_title(
                traverse_obj(webpage_metadata, ("coverInfo", "title"))
                or self._og_search_title(webpage)
            ),
            description=(
                traverse_obj(webpage_metadata, ("coverInfo", "description"))
...
class WeTvBaseIE(TencentBaseIE):
    ...
    def _extract_episode(self, url):
        ...
        return {
            ...
            "duration": int_or_none(
                traverse_obj(webpage_metadata, ("videoInfo", "duration"))
            ),
            "series": traverse_obj(webpage_metadata, ("coverInfo", "title")),
            "episode_number": int_or_none(
                traverse_obj(webpage_metadata, ("videoInfo", "episode"))
            ),
        }
    ...
    def _extract_series(self, url, ie):
        ...
        episode_paths = [
            f'/play/{series_id}/{episode["vid"]}'
            for episode in webpage_metadata.get("videoList")
        ] or re.findall(
            r'<a[^>]+class="play-video__link"[^>]+href="(?P<path>[^"]+)',
            webpage,
        )
...
class WeTvSeriesIE(WeTvBaseIE):
    _VALID_URL = (
        WeTvBaseIE._VALID_URL_BASE + r"/(?P<id>\w+)(?:-[^/?#]+)?/?(?:[?#]|$)"
    )
...
class IflixSeriesIE(IflixBaseIE):
    _VALID_URL = (
        IflixBaseIE._VALID_URL_BASE + r"/(?P<id>\w+)(?:-[^/?#]+)?/?(?:[?#]|$)"
    )
