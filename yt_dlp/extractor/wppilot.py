    ...
    def _get_channel_list(self, cache=True):
        ...
        for qhash in page_data["staticQueryHashes"]:
            qhash_content = self._download_json(
                f"{page_data_base_url}/sq/d/{qhash}.json",
                None,
                "Searching for channel list",
            )
            channel_list = try_get(
                qhash_content, lambda x: x["data"]["allChannels"]["nodes"]
            )
...
class WPPilotIE(WPPilotBaseIE):
    ...
    def _real_extract(self, url):
        ...
        is_authorized = (
            is_authorized is not None
            and not is_authorized.value.startswith("g:")
        )
        ...
        if stream_token:
            close = self._download_json(
                "https://pilot.wp.pl/api/v1/channels/close",
                video_id,
                "Invalidating previous stream session",
                headers=self._HEADERS_WEB,
                data=json.dumps(
                    {
                        "channelId": video_id,
            ...
            if fmt["type"] == "hls@live:abr":
                formats.extend(
                    self._extract_m3u8_formats(
                        random.choice(fmt["url"]), video_id, live=True
                    )
                )
...
class WPPilotChannelsIE(WPPilotBaseIE):
    _VALID_URL = (
        r"(?:https?://pilot\.wp\.pl/(?:tv/?)?(?:\?[^#]*)?#?|wppilot:)$"
    )
