  ...
  artist(slug: "%s") {
    ...
    def _real_initialize(self):
        self._ARTIST_FIELDS_TMPL = (
            self._ARTIST_FIELDS_TMPL % self._MUSIC_FIELDS
        )
...
class PalcoMP3IE(PalcoMP3BaseIE):
    ...
    def suitable(cls, url):
        return (
            False if PalcoMP3VideoIE.suitable(url) else super().suitable(url)
        )
...
class PalcoMP3ArtistIE(PalcoMP3BaseIE):
    ...
    def suitable(cls, url):
        return (
            False
            if PalcoMP3IE._match_valid_url(url)
            else super().suitable(url)
        )

    def _real_extract(self, url):
        artist_slug = self._match_id(url)
        artist = self._call_api(artist_slug, self._ARTIST_FIELDS_TMPL)[
            "artist"
        ]

        def entries():
            for music in (
                try_get(artist, lambda x: x["musics"]["nodes"], list) or []
            ):
