    ...
    def _real_extract(self, url):
        ...
        result = {
            ...
            "language": traverse_obj(
                data, ("potentialAction", "target", "inLanguage")
            ),
            ...
            "channel_url": format_field(
                channel_id, None, "https://rad.live/content/channel/%s"
            ),
        }
...
class RadLiveSeasonIE(RadLiveIE):  # XXX: Do not subclass from concrete IE
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?rad\.live/content/season/(?P<id>[a-f0-9-]+)"
    )
    _TESTS = [
        {
        ...
        entries = [
            {
                ...
                "series": try_get(
                    content_info, lambda x: x["series"]["title"]
                ),
...
class RadLiveChannelIE(RadLiveIE):  # XXX: Do not subclass from concrete IE
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?rad\.live/content/channel/(?P<id>[a-f0-9-]+)"
    )
    _TESTS = [
        {
  ...
  channel (id:$lrn) {
    ...
    def _real_extract(self, url):
        ...
        graphql = self._download_json(
            "https://content.mhq.12core.net/graphql",
            channel_id,
