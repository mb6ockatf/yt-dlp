...
class NetverseBaseIE(InfoExtractor):
    ...
    def _call_api(
        self, slug, endpoint, query={}, season_id="", display_id=None
    ):
        ...
        for i in itertools.count(1):
            comment_data = (
                self._download_json(
                    ...
                    fatal=False,
                    query={"page": i},
                    note=f"Downloading JSON comment metadata page {i}",
                )
                or {}
            )
...
class NetverseIE(NetverseBaseIE):
    ...
    def _real_extract(self, url):
        display_id, sites_type = self._match_valid_url(url).group(
            "display_id", "type"
        )
        ...
        return {
            ...
            "url": smuggle_url(
                videos["dailymotion_url"],
                {"query": {"embedder": "https://www.netverse.id"}},
            ),
            "display_id": display_id,
            "title": videos.get("title"),
            "season": videos.get("season_name"),
            "thumbnail": traverse_obj(
                videos, ("program_detail", "thumbnail_image")
            ),
            "description": traverse_obj(
                videos, ("program_detail", "description")
            ),
            ...
            for current_page in range(
                ...
                for slug in traverse_obj(
                    ...
                    yield self.url_result(
                        f"https://www.netverse.id/video/{slug}", NetverseIE
                    )

    def _real_extract(self, url):
        playlist_id, sites_type = self._match_valid_url(url).group(
            "display_id", "type"
        )
        ...
        return self.playlist_result(
            ...
            traverse_obj(
                playlist_data, ("response", "webseries_info", "slug")
            ),
            traverse_obj(
    ...
    def _search_results(self, query):
        ...
        for i in itertools.count(1):
            search_data = self._download_json(
                "https://api.netverse.id/search/elastic/search",
                query,
                query={"q": query, "page": i},
            ...
            for video in videos:
                yield self.url_result(
                    f'https://netverse.id/video/{video["slug"]}', NetverseIE
                )
