...
class ProSiebenSat1BaseIE(InfoExtractor):
    ...
    def _extract_video_info(self, url, clip_id):
        ...
        if (
            ...
            and video.get("is_protected") is True
        ):
        ...
        if self._ACCESS_ID:
            raw_ct = (
                self._ENCRYPTION_KEY + clip_id + self._IV + self._ACCESS_ID
            )
            protocols = (
                self._download_json(
                    self._V4_BASE_URL + "protocols",
                    clip_id,
            ...
            for source_id in source_ids:
                ...
                for source in urls_sources:
                    ...
                    if (
                        ...
                        or "f4mgenerator" in source_url
                        or determine_ext(source_url) == "f4m"
                    ):
                        formats.extend(
...
class ProSiebenSat1IE(ProSiebenSat1BaseIE):
    ...
    def _extract_clip(self, url, webpage):
        ...
        title = self._html_search_regex(
            self._TITLE_REGEXES, webpage, "title", default=None
        ...
        upload_date = unified_strdate(
            self._html_search_meta(
                "og:published_time", webpage, "upload date", default=None
            )
            ...
            info.update(
                {
                    ...
                    or item.get("teaser", {}).get("headline"),
    ...
    def _real_extract(self, url):
        ...
        page_type = self._search_regex(
            self._PAGE_TYPE_REGEXES, webpage, "page type", default="clip"
        ).lower()
        if page_type == "clip":
