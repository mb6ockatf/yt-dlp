    ...
    def _real_extract(self, url):
        ...
        if rcid:
            ...
            url = self._search_regex(
                self._VALID_URL, webpage, "redirection url", group="url"
            )
        ...
        if stream.get("preventDownload") is False:
            ...
            audiourl = try_get(
                stream,
                lambda x: x["downloadRecordingInfo"]["downloadInfo"][
                    "audioURL"
                ],
            )
            if audiourl:
        ...
        return {
            ...
            "uploader_id": stream.get("ownerUserName")
            or stream.get("ownerId"),
