...
class AudioBoomIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        webpage = self._download_webpage(
            f"https://audioboom.com/posts/{video_id}", video_id
        )
        ...
        return {
            "id": video_id,
            "url": clip.get("clipURLPriorToLoading")
            or self._og_search_property("audio", webpage, "audio url"),
            ...
            "description": (
                ...
                or clean_html(clip.get("formattedDescription"))
                or self._og_search_description(webpage)
            ),
            "duration": float_or_none(
                clip.get("duration")
                or self._html_search_meta("weibo:audio:duration", webpage)
            ),
            "uploader": clip.get("author")
            or self._html_search_meta(
                [
                    "og:audio:artist",
            ...
            "uploader_url": clip.get("author_url")
