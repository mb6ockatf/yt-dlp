...
class CeskaTelevizeIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        site_name = self._og_search_property(
            "site_name", webpage, fatal=False, default="Česká televize"
        )
        playlist_title = self._og_search_title(webpage, default=None)
        if site_name and playlist_title:
            playlist_title = re.split(
                rf"\s*[—|]\s*{site_name}", playlist_title, maxsplit=1
            )[0]
        ...
        if any(
            not_found in webpage
            for not_found in (
                "Neplatný parametr pro videopřehrávač",
                "IDEC nebyl nalezen",
            )
        ):
            raise ExtractorError(
                "no video with IDEC available", video_id=idec, expected=True
            )
                ...
                entries.append(
                    {
                        ...
                        "title": final_title,
                        "description": (
                            playlist_description if playlist_len == 1 else None
                        ),
                        "thumbnail": thumbnail,
                        "duration": duration,
                        "formats": formats,
        ...
        def _fix_subtitle(subtitle):
            for line in subtitle.splitlines():
                ...
                if m:
                    yield m.group(1)
                    start, stop = (
                        _msectotimecode(int(t)) for t in m.groups()[1:]
                    )
                    yield f"{start} --> {stop}"
                else:
                    yield line
