...
class AdobePassIE(
    ...
    def _extract_mvpd_auth(self, url, video_id, requestor_id, resource):
        ...
        while count < 2:
            requestor_info = (
                self.cache.load(self._MVPD_CACHE, requestor_id) or {}
            )
            ...
            if not authn_token:
                ...
                if not mso_id:
                    ...
                    if (
                        ...
                        self._download_webpage(
                            oauth_redirect_url,
                            video_id,
                            "Confirming auto login",
                        )
                    elif (
                        "automatically signed in with"
                        in provider_redirect_page
                    ):
                        ...
                        if (
                            ...
                            post_form(
                                mvpd_confirm_page_res, "Confirming Login"
                            )
                elif mso_id == "Philo":
                    ...
                    self._download_webpage(
                        ...
                        data=urlencode_postdata(
                            {
                                "SAMLResponse": saml_response_json[
                                    "SAMLResponse"
                                ],
                    ...
                    mvpd_confirm_page_res = post_form(
                        provider_login_page_res, "Logging in", form_data
                    )
                    if mso_id != "Rogers":
                        post_form(mvpd_confirm_page_res, "Confirming Login")
                ...
                self.cache.store(
                    self._MVPD_CACHE, requestor_id, requestor_info
                )
            ...
            if not authz_token:
                ...
                if "<error" in authorize:
                    raise ExtractorError(
                        xml_text(authorize, "details"), expected=True
                    )
