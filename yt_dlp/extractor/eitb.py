...
class EitbIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if hds_url:
            formats.extend(
                self._extract_f4m_formats(
                    "{}?hdcore=3.7.0".format(
                        hds_url.replace("euskalsvod", "euskalvod")
                    ),
                    video_id,
                    f4m_id="hds",
                    fatal=False,
        ...
        return {
            "id": video_id,
            "title": media.get("NAME_ES")
            or media.get("name")
            or media["NAME_EU"],
            "description": media.get("SHORT_DESC_ES")
            or video.get("desc_group")
            or media.get("SHORT_DESC_EU"),
