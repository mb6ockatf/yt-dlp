  ...
  video(id: $id) {
        ...
        return (
            traverse_obj(
                self._download_json(
                    self._GRAPHQL_API,
                    media_id,
                    "Downloading JSON from GraphQL API",
                    data=json.dumps(
                        {
    ...
    def _extract_formats_and_subtitles(self, video_id, content_media_json):
        ...
        for video in traverse_obj(
            ...
            ext = mimetype2ext(video.get("mimetype")) or determine_ext(
                video_url
            )
            if ext == "m3u8":
                m3u8_fmts, m3u8_subs = (
                ...
                formats.append(
                    {
                        "url": video_url,
                        "format_id": format_id,
                        "vcodec": video.get("videoencoding")
                        or video.get("video_codec"),
    ...
    def _extract_video(self, media_id):
        ...
        return {
            ...
            "creator": ", ".join(
                traverse_obj(
                    data,
                    (  # TODO: change to 'creators'
                        "bylines",
                        ...,
                        "renderedRepresentation",
                        {lambda x: remove_start(x, "By ")},
                    ),
                )
            ),
            "formats": formats,
            ...
            "thumbnails": self._extract_thumbnails(
                traverse_obj(
                    data, ("promotionalMedia", "crops", ..., "renditions", ...)
                )
            ),
        }
...
class NYTimesArticleIE(NYTimesBaseIE):
        ...
        return {
            **details,
            "thumbnails": self._extract_thumbnails(
                traverse_obj(
                    block,
                    ("promotionalMedia", "crops", ..., "renditions", ...),
                )
            ),
            "formats": formats,
            "subtitles": subtitles,
    ...
    def _real_extract(self, url):
        ...
        art_json = self._search_json(
            ...
            "media details",
            page_id,
            transform_source=lambda x: x.replace("undefined", "null"),
        ...
        common_info = {
            ...
            "timestamp": traverse_obj(
                art_json, ("firstPublished", {parse_iso8601})
            ),
            "creator": ", ".join(
                traverse_obj(
                    art_json, ("bylines", ..., "creators", ..., "displayName")
                )
            ),  # TODO: change to 'creators' (list)
            "thumbnails": self._extract_thumbnails(
                traverse_obj(
                    art_json,
                    ("promotionalMedia", "assetCrops", ..., "renditions", ...),
                )
            ),
        }
...
class NYTimesCookingIE(NYTimesBaseIE):
    ...
    def _real_extract(self, url):
        ...
        media_ids = traverse_obj(
            get_elements_html_by_class("video-item", webpage),
            (..., {extract_attributes}, "data-video-id"),
        )

        if media_ids:
            media_ids.append(lead_video_id)
            return self.playlist_result(
                map(self._extract_video, media_ids),
                page_id,
                title,
                description,
            )
...
class NYTimesCookingRecipeIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        page_id = self._match_id(url)
        webpage = self._download_webpage(url, page_id)
        recipe_data = self._search_nextjs_data(webpage, page_id)["props"][
            "pageProps"
        ]["recipe"]
            ...
            **traverse_obj(
            ...
            "thumbnails": [
                ...
                for thumb_url in traverse_obj(
                    recipe_data,
                    ("image", "crops", "recipe", ..., {url_or_none}),
                )
            ],
        }
