...
class SapoIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        item = self._download_xml(
            f"http://rd3.videos.sapo.pt/{video_id}/rss2", video_id
        ).find("./channel/item")

        title = item.find("./title").text
        description = item.find("./{http://videos.sapo.pt/mrss/}synopse").text
        thumbnail = item.find("./{http://search.yahoo.com/mrss/}content").get(
            "url"
        )
        duration = parse_duration(
            item.find("./{http://videos.sapo.pt/mrss/}time").text
        )
        uploader = item.find("./{http://videos.sapo.pt/mrss/}author").text
        upload_date = unified_strdate(item.find("./pubDate").text)
        view_count = int(
            item.find("./{http://videos.sapo.pt/mrss/}views").text
        )
        comment_count = int(
            item.find("./{http://videos.sapo.pt/mrss/}comment_count").text
        )
        tags = item.find("./{http://videos.sapo.pt/mrss/}tags").text
        categories = tags.split() if tags else []
        age_limit = (
            18
            if item.find("./{http://videos.sapo.pt/mrss/}m18").text == "true"
            else 0
        )

        video_url = item.find("./{http://videos.sapo.pt/mrss/}videoFile").text
        video_size = item.find(
            "./{http://videos.sapo.pt/mrss/}videoSize"
        ).text.split("x")
