...
class IwaraBaseIE(InfoExtractor):
    ...
    def _is_token_expired(self, token, token_type):
        ...
        if (try_call(lambda: jwt_decode_hs256(token)["exp"]) or 0) <= int(
            time.time() - 120
        ):
...
class IwaraIE(IwaraBaseIE):
    IE_NAME = "iwara"
    _VALID_URL = (
        r"https?://(?:www\.|ecchi\.)?iwara\.tv/videos?/(?P<id>[a-zA-Z0-9]+)"
    )
    _TESTS = [
        {
    ...
    def _extract_formats(self, video_id, fileurl):
        ...
        x_version = hashlib.sha1(
            "_".join(
                (paths[-1], q["expires"][0], "5nFp9kmbNnHdAFhaqMvt")
            ).encode()
        ).hexdigest()
        ...
        files = self._download_json(
            fileurl, video_id, headers={"X-Version": x_version}
        )
        for fmt in files:
            yield traverse_obj(
                fmt,
    ...
    def _real_extract(self, url):
        ...
        if errmsg == "errors.privateVideo":
            ...
            self.raise_login_required(
                "Video may need login to view", method="password"
            )
        ...
        return {
            "id": video_id,
            "age_limit": (
                18 if video_data.get("rating") == "ecchi" else 0
            ...
            **traverse_obj(
            ...
            "formats": list(
                self._extract_formats(video_id, video_data.get("fileUrl"))
            ),
        }
...
class IwaraUserIE(IwaraBaseIE):
    ...
    def _real_extract(self, url):
        ...
        user_info = self._download_json(
            f"https://api.iwara.tv/profile/{playlist_id}",
            playlist_id,
        ...
        return self.playlist_result(
            ...
            playlist_id,
            traverse_obj(user_info, ("user", "name")),
        )
...
class IwaraPlaylistIE(IwaraBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            ...
            playlist_id,
            traverse_obj(page_0, ("title", "name")),
        )
