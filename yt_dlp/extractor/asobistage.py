...
class AsobiStageIE(InfoExtractor):
    ...
    def _owned_tickets(self):
        ...
        for path, name in [
            ...
            response = self._download_json(
                ...
                None,
                f"Downloading {name}",
                f"Unable to download {name}",
                expected_status=400,
            )
            if (
                traverse_obj(response, ("payload", "error_message"), "error")
                == "notlogin"
            ):
    ...
    def _get_available_channel_id(self, channel):
        ...
        if traverse_obj(
            channel, ("viewrights", lambda _, v: v["rights_type_id"] == 6)
        ):
            return channel_id
    ...
    def _real_extract(self, url):
        ...
        video_id, event, type_, slug = self._match_valid_url(urlh.url).group(
            "id", "event", "type", "slug"
        )
        ...
        available_channels = traverse_obj(
            ...
            (
                video_type,
                lambda _, v: v["broadcast_slug"] == slug,
                "channels",
                lambda _, v: v["chennel_vspf_id"] != "00000",
            ),
        )

        entries = []
        for channel_id in traverse_obj(
            available_channels, (..., {self._get_available_channel_id})
        ):
            if video_type == "archives":
                ...
                channel_json = self._download_json(
                    f"https://survapi.channel.or.jp/ex/events/{channel_id}",
                    channel_id,
                    "Getting live channel info",
                    "Unable to get live channel info",
                    fatal=False,
                    headers=self._HEADERS,
                    query={"embed": "channel"},
                )
            ...
            entries.append(
                {
                    ...
                    "formats": self._extract_m3u8_formats(
                        channel_data.get("m3u8_url"), channel_id, fatal=False
                    ),
