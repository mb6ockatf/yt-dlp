...
class BpbIE(InfoExtractor):
    ...
    _TESTS = [
        ...
        },
        {
            ...
            "info_dict": {
                ...
                "ext": "mp4",
                "creators": [
                    "Institute for Strategic Dialogue Germany gGmbH (ISD)"
                ],
    ...
    def _parse_vue_attributes(self, name, string, video_id):
        attributes = extract_attributes(
            self._search_regex(rf'(<{name}(?:"[^"]*?"|[^>])*>)', string, name)
        )

        for key, value in attributes.items():
            if key.startswith(":"):
                attributes[key] = self._parse_json(
                    value, video_id, transform_source=js_to_json, fatal=False
                )
    ...
    def _real_extract(self, url):
        ...
        title_result = traverse_obj(
            webpage,
            (
                {find_element(cls="opening-header__title")},
                {self._TITLE_RE.match},
            ),
        )
        ...
        return {
            "id": video_id,
            "title": traverse_obj(title_result, ("title", {str.strip}))
            or None,
            # This metadata could be interpreted otherwise, but it fits "series" the most
            "series": traverse_obj(title_result, ("series", {str.strip}))
            or None,
                ...
                *traverse_obj(
                    ...
                    [
                        (
                            ...
                            [
                                {find_element(tag="bpb-accordion-item")},
                                {find_element(cls="text-content")},
                            ],
                        ),
                        {clean_html},
                    ],
            ...
            "creators": traverse_obj(
                self._html_search_meta("author", webpage), all
            ),
            ...
            "tags": traverse_obj(
                json_lds, (..., "keywords", {lambda x: x.split(",")}, ...)
            ),
            **traverse_obj(
                self._parse_vue_attributes("bpb-player", webpage, video_id),
                {
