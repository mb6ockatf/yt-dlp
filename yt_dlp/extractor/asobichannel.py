...
class AsobiChannelIE(AsobiChannelBaseIE):
    ...
    def _real_initialize(self):
        token = self._download_json(
            "https://asobichannel-api.asobistore.jp/api/v1/vspf/token",
            None,
    ...
    def _process_live(self, video_id, metadata):
        ...
        if player_type == "poster":
            self.raise_no_formats(
                "Live event has not yet started", expected=True
            )
            ...
            live_status = "is_live"
            formats = self._extract_m3u8_formats(
                event_data["data"]["Channel"]["Custom_live_url"],
                video_id,
                live=True,
            )
        else:
        ...
        return {
            "release_timestamp": traverse_obj(
                metadata, ("period", "start", {parse_iso8601})
            ),
    ...
    def _real_extract(self, url):
        ...
        metadata = self._download_json(
            f"https://channel.microcms.io/api/v1/media/{video_id}",
            video_id,
...
class AsobiChannelTagURLIE(AsobiChannelBaseIE):
    ...
    _VALID_URL = (
        r"https?://asobichannel\.asobistore\.jp/tag/(?P<id>[a-z0-9-_]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        title = traverse_obj(
            self._search_nextjs_data(webpage, tag_id, fatal=False),
            ("props", "pageProps", "data", "name", {str}),
        )
