...
class PornboxIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        subtitles = {
            ...
            for country_code in traverse_obj(
                public_data, ("subtitles", ..., {str})
            )
        }

        is_free_scene = traverse_obj(
            public_data,
            ("price", "is_available_for_free", {bool}),
            default=False,
        )
        ...
        metadata = {
            ...
            **traverse_obj(
            ...
            "availability": self._availability(
                needs_auth=True, needs_premium=not is_free_scene
            ),
            "subtitles": subtitles,
        }
        ...
        if not media_id:
            self.raise_no_formats(
                "Could not find stream id", video_id=video_id
            )
