...
class TV2IE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?tv2\.no/v(?:ideo)?\d*/(?:[^?#]+/)*(?P<id>\d+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        for protocol in self._PROTOCOLS:
            try:
                data = self._download_json(
                    ...
                    data=b'{"device":{"id":"1-1-1","name":"Nettleser (HTML)"}}',
                )["playback"]
        ...
        return {
            ...
            "timestamp": parse_iso8601(
                asset.get("live_broadcast_time") or asset.get("update_time")
            ),
...
class TV2ArticleIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if not assets:
            # New embed pattern
            for v in re.findall(
                r"(?s)(?:TV2ContentboxVideo|TV2\.TV2Video)\(({.+?})\)", webpage
            ):
        ...
        description = remove_end(
            self._og_search_description(webpage), " - TV2.no"
        )
...
class KatsomoIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        asset = self._download_json(
            api_base + ".json", video_id, "Downloading metadata JSON"
        )["asset"]
        ...
        for protocol in self._PROTOCOLS:
            try:
                data = self._download_json(
                    api_base
                    + f"/play.json?protocol={protocol}&videoFormat=SMIL+ISMUSP",
                    video_id,
            ...
            for item in items:
                ...
                if not video_url or video_url in format_urls:
                    continue
                format_id = "{}-{}".format(
                    protocol.lower(), item.get("mediaFormat")
                )
                if not self._is_valid_url(video_url, video_id, format_id):
                    continue
                format_urls.append(video_url)
...
class MTVUutisetArticleIE(InfoExtractor):
    _WORKING = False
    _VALID_URL = (
        r"https?://(?:www\.)mtvuutiset\.fi/artikkeli/[^/]+/(?P<id>\d+)"
    )
    _TESTS = [
        {
