    ...
    def _call_api(self, path, video_id=None, app_code=None, query=None):
        ...
        try:
            return self._download_json(
                "https://services.radio-canada.ca/media/" + path,
                video_id,
                query=query,
            )
            ...
            if isinstance(e.cause, HTTPError) and e.cause.status in (401, 422):
                ...
                error = (
                    ...
                    or data["errorMessage"]["text"]
                )
                raise ExtractorError(error, expected=True)
            raise

    def _extract_info(self, app_code, video_id):
        metas = self._call_api("meta/v1/index.ashx", video_id, app_code)[
            "Metas"
        ]
        ...
        subtitles = {}
        closed_caption_url = get_meta("closedCaption") or get_meta(
            "closedCaptionHTML5"
        )
        ...
        return {
            ...
            or get_meta("ShortDescription"),
            "thumbnail": get_meta("imageHR")
            or get_meta("imageMR")
            or get_meta("imageBR"),
...
class RadioCanadaAudioVideoIE(InfoExtractor):
    ...
    _VALID_URL = (
        r"https?://ici\.radio-canada\.ca/([^/]+/)*media-(?P<id>[0-9]+)"
    )
    _TESTS = [
        {
