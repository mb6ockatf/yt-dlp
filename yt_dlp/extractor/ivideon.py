...
class IvideonIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if webpage:
            ...
            if config_string:
                config = self._parse_json(
                    config_string, server_id, fatal=False
                )
                camera_info = config.get("ivTvAppOptions", {}).get(
                    "currentCameraInfo"
                )
                if camera_info:
                    camera_name = camera_info.get("camera_name")
                    description = camera_info.get("misc", {}).get(
                        "description"
                    )
            if not camera_name:
                camera_name = self._html_search_meta(
                    "name", webpage, "camera name", default=None
                ) or self._search_regex(
                    r'<h1[^>]+class="b-video-title"[^>]*>([^<]+)',
                    webpage,
                    "camera name",
                    default=None,
                )
        ...
        formats = [
            {
                "url": "https://streaming.ivideon.com/flv/live?{}".format(
