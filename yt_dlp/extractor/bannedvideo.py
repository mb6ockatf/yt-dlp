...
query GetCommentReplies($id: String!) {
    ...
    def _parse_comment(comment_data, parent):
        return {
            ...
            "parent": parent,
            "like_count": try_get(
                comment_data, lambda x: x["voteCount"]["positive"]
            ),
        }
    ...
    def _real_extract(self, url):
        ...
        comments = [
            self._parse_comment(comment, "root")
            for comment in video_json.get("getVideoComments")
        ]
        ...
        formats = (
            ...
            if url_or_none(video_info.get("directUrl"))
            else []
        )
        ...
        return {
            ...
            "__post_extractor": self.extract_comments(
                video_id, comments, video_json.get("getVideoComments")
            ),
        }
