...
class UtreonIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        json_data = self._download_json(
            "https://api.playeur.com/v1/videos/" + video_id, video_id
        )
        ...
        formats = [
            ...
            for format_key, format_url in videos_json.items()
            if url_or_none(format_url)
        ]
        thumbnail = url_or_none(
            dict_get(json_data, ("cover_image_url", "preview_image_url"))
        )
        return {
            "id": video_id,
            ...
            "uploader": str_or_none(
                try_get(json_data, lambda x: x["channel"]["title"])
            ),
            "thumbnail": thumbnail,
            "release_date": unified_strdate(
                json_data.get("published_datetime")
            ),
        }
