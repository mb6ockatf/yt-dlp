...
class YoukuIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if error:
            ...
            if (
                error_note is not None
                and "因版权原因无法观看此视频" in error_note
            ):
...
class YoukuShowIE(InfoExtractor):
    ...
    def _extract_entries(self, playlist_data_url, show_id, note, query):
        ...
        playlist_data = self._download_json(
            ...
            query=query,
            note=note,
            transform_source=lambda s: js_to_json(strip_jsonp(s)),
        ).get("html")
        if playlist_data is None:
            return [None, None]
        drama_list = get_element_by_class(
            "p-drama-grid", playlist_data
        ...
        return playlist_data, [
            self.url_result(
                self._proto_relative_url(video_url, "http:"), YoukuIE.ie_key()
            )
    ...
    def _real_extract(self, url):
        ...
        playlist_description = (
            get_element_by_class("intro-more", detail_li)
            if detail_li
            else None
        )
