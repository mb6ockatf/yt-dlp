  ...
  bonanzaPage(
    ...
    metadata {
      ... on VideoPageData {
        ...
        tp_path = "NnzsPC/media/guid/{}/{}".format(
            video_data.get("mpxAccountId") or "2410887629", video_id
        )
        ...
        theplatform_url = smuggle_url(
            update_url_query(
                "http://link.theplatform.com/s/NnzsPC/media/guid/{}/{}".format(
                    video_data.get("mpxAccountId") or "2410887629", video_id
                ),
                query,
            ),
...
class NBCSportsVPlayerIE(InfoExtractor):
    ...
    _EMBED_REGEX = [
        rf'(?:iframe[^>]+|var video|div[^>]+data-(?:mpx-)?)[sS]rc\s?=\s?"(?P<url>{_VALID_URL_BASE}[^\"]+)'
    ]
    ...
    def _real_extract(self, url):
        ...
        theplatform_url = self._html_search_regex(
            r'tp:releaseUrl="(.+?)"', webpage, "url"
        )
...
class NBCSportsStreamIE(AdobePassIE):
    ...
    def _real_extract(self, url):
        ...
        is_live = (
            video_source.get("type") == "live"
            or video_source.get("status") == "Live"
        )
...
class NBCNewsIE(ThePlatformIE):  # XXX: Do not subclass from concrete IE
    ...
    def _real_extract(self, url):
        ...
        data = self._search_nextjs_data(webpage, video_id)["props"][
            "initialState"
        ]
        ...
        return {
            ...
            "title": title,
            "description": try_get(
                video_data, lambda x: x["description"]["primary"]
            ),
            "thumbnail": try_get(
                video_data, lambda x: x["primaryImage"]["url"]["primary"]
            ),
...
class NBCOlympicsStreamIE(AdobePassIE):
    ...
    def _real_extract(self, url):
        ...
        is_live = {"live": True, "replay": False}.get(
            event_config.get("eventStatus")
        )
        ...
        if event_config.get("cdnToken"):
            ...
            media_token = self._extract_mvpd_auth(
                url,
                pid,
                event_config.get("requestorId", "NBCOlympics"),
                ap_resource,
            )
        ...
        formats = self._extract_m3u8_formats(
            source_url, pid, "mp4", live=is_live
        )
        for f in formats:
    ...
    def _real_extract(self, url):
        ...
        if not video_data:
            raise ExtractorError(
                "No video metadata found in webpage", expected=True
            )
        ...
        query = {
            ...
            "fwNetworkID": traverse_obj(
                nbc_data, ("video", "fwNetworkID"), default="382114"
            ),
        ...
        if is_live:
            player_id = traverse_obj(
                ...
                (
                    (None, ("video", "meta")),
                    ("mpx_m3upid", "mpx_pid", "pid_streaming_web_medium"),
                ),
                get_all=False,
            )
            ...
            player_id = traverse_obj(
                ...
                (
                    (None, ("video", "meta")),
                    ("pid_streaming_web_high", "mpx_pid"),
                ),
                get_all=False,
            )
            ...
            if date_string:
                ...
                date_string = traverse_obj(
                    nbc_data,
                    ("dataLayer", "adobe", ("prop70", "eVar70", "eVar59")),
                    get_all=False,
                )

            video_url = traverse_obj(
                video_data,
                ((None, ("video", "meta")), "mp4_url"),
                get_all=False,
            )
            if video_url:
            ...
            info.update(
                {
                    ...
                    or traverse_obj(
                        ...
                        (
                            "dataLayer",
                            (None, "adobe"),
                            ("contenttitle", "title", "prop22"),
                        ),
                        get_all=False,
                    ),
                    "description": traverse_obj(
                        video_data, "summary", "excerpt", "video_hero_text"
                    )
                    or clean_html(
                        traverse_obj(nbc_data, ("dataLayer", "summary"))
                    ),
        ...
        if player_id and fw_ssid:
            smil = self._download_xml(
                f"https://link.theplatform.com/s/{pdk_acct}/{player_id}",
                video_id,
            ...
            if not isinstance(smil, xml.etree.ElementTree.Element):
                smil = None
        subtitles = (
            self._parse_smil_subtitles(smil, default_ns)
            if smil is not None
            else {}
        )
        for video in (
            smil.findall(self._xpath_ns(".//video", default_ns))
            if smil is not None
            else []
        ):
            info["duration"] = float_or_none(
                remove_end(video.get("dur"), "ms"), 1000
            )
            video_src_url = video.get("src")
            ext = mimetype2ext(
                video.get("type"), default=determine_ext(video_src_url)
            )
            if ext == "m3u8":
