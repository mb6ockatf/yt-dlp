...
def _parse_japanese_date(text):
    ...
    mobj = re.search(
        rf"({ERA_RE})?(\d+)年(\d+)月(\d+)日", re.sub(r"[\s\u3000]+", "", text)
    )
    if not mobj:
        return None
...
def _parse_japanese_duration(text):
    ...
    days, hours, mins, secs = (
        int_or_none(x, default=0) for x in mobj.groups()
    )
        ...
        return [
            {
                ...
                "url": smuggle_url(
                    f"https://www.shugiintv.go.jp/jp/index.php?room_id={x.group(1)}",
                    {"g": x.groups()},
                ),
                "ie_key": ShugiinItvLiveIE.ie_key(),
            }
            for x in re.finditer(
                r'(?s)<a\s+href="[^"]+\?room_id=(room\d+)"\s*class="play_live".+?class="s12_14">(.+?)</td>',
                webpage,
            )
        ]
    ...
    def _fetch_rooms(self):
        if not self._INDEX_ROOMS:
            webpage = self._download_webpage(
                ...
                None,
                encoding="euc-jp",
                note="Downloading proceedings info",
            )
...
class ShugiinItvLiveIE(ShugiinItvBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?shugiintv\.go\.jp/(?:jp|en)(?:/index\.php)?$"
    )
    ...
    def suitable(cls, url):
        return super().suitable(url) and not any(
            x.suitable(url) for x in (ShugiinItvLiveRoomIE, ShugiinItvVodIE)
        )
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._fetch_rooms(), playlist_title="All proceedings for today"
        )
...
class ShugiinItvVodIE(ShugiinItvBaseIE):
    ...
    def _real_extract(self, url):
        ...
        webpage = self._download_webpage(
            f"https://www.shugiintv.go.jp/jp/index.php?ex=VL&media_type=&deli_id={video_id}",
            video_id,
            encoding="euc-jp",
        )

        m3u8_url = self._search_regex(
            r'id="vtag_src_base_vod"\s*value="(http.+?\.m3u8)"',
            webpage,
            "m3u8 url",
        )
        ...
        title = self._html_search_regex(
            (
                ...
                r'<TD.+?<IMG\s*src=".+?/spacer\.gif".+?height="15">(.+?)<IMG',
            ),
            webpage,
            "title",
            fatal=False,
        ...
        release_date = _parse_japanese_date(
        ...
        chapters = []
        for chp in re.finditer(
            r'(?i)<A\s+HREF="([^"]+?)"\s*class="play_vod">(?!<img)(.+)</[Aa]>',
            webpage,
        ):
            chapters.append(
                {
                    ...
                    "start_time": try_call(
                        lambda: float(
                            parse_qs(chp.group(1))["time"][0].strip()
                        )
                    ),
                }
            )
        ...
        last_tr = re.findall(r'(?s)<TR\s*class="s14_24">(.+?)</TR>', webpage)[
            -1
        ]
        if last_tr and chapters:
            last_td = re.findall(r"<TD.+?</TD>", last_tr)[-1]
            if last_td:
                chapters[-1]["end_time"] = chapters[-1][
                    "start_time"
                ] + _parse_japanese_duration(clean_html(last_td))
...
class SangiinInstructionIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        raise ExtractorError(
            ...
            "get the URL of the frame showing the video.",
            expected=True,
        )
...
class SangiinIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        is_live = bool(
            self._html_search_regex(
                r"<dt[^>]*>\s*公報掲載時刻\s*</dt>\s*<dd[^>]*>\s*(.+?)\s*</dd>",
                webpage,
                "is_live",
                default=None,
            )
