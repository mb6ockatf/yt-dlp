...
class StarTrekIE(InfoExtractor):
    _WORKING = False
    _VALID_URL = (
        r"(?P<base>https?://(?:intl|www)\.startrek\.com)/videos/(?P<id>[^/]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        player = self._search_regex(
            r'(<\s*div\s+id\s*=\s*"cvp-player-[^<]+<\s*/div\s*>)',
            webpage,
            "player",
        )

        hls = self._html_search_regex(
            r'\bdata-hls\s*=\s*"([^"]+)"', player, "HLS URL"
        )
        ...
        captions = self._html_search_regex(
            r'\bdata-captions-url\s*=\s*"([^"]+)"',
            player,
            "captions URL",
            fatal=False,
        )
        if captions:
            subtitles.setdefault("en-US", [])[:0] = [
                {"url": urljoin(urlbase, captions)}
            ]
