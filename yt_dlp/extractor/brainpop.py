...
class BrainPOPBaseIE(InfoExtractor):
    ...
    def _VALID_URL(cls):
        root = (
            re.escape(cls._ORIGIN)
            .replace(r"https:", r"https?:")
            .replace(r"www\.", r"(?:www\.)?")
        )
        return rf"{root}/(?P<slug>[^/]+/[^/]+/(?P<id>[^/?#&]+))"

    def _assemble_formats(
        self, slug, format_id, display_id, token="", extra_fields={}
    ):
        formats = []
    ...
    def _extract_adaptive_formats(
        self, data, token, display_id, key_format="%s", extra_fields={}
    ):
        formats = []
        ...
        for (
            additional_key_format,
            additional_key_fields,
        ) in additional_key_formats.items():
...
class BrainPOPIE(BrainPOPBaseIE):
    ...
    def _real_extract(self, url):
        ...
        for lang, localized_feature in traverse_obj(
            movie_feature, "localization", default={}, expected_type=dict
        ).items():
            formats.extend(
                self._extract_adaptive_formats(
                    localized_feature,
                    localized_feature.get("token", ""),
                    display_id,
                    "%s_v2",
                    {
                        "language": lang,
...
class BrainPOPLegacyBaseIE(BrainPOPBaseIE):
    ...
    def _real_extract(self, url):
        ...
        token = self._search_regex(
            r'ec_token\s*:\s*[\'"]([^\'"]+)', webpage, "video token"
        )
