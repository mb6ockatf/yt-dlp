            ...
            self._download_json(
                ...
                headers=(
                    ...
                    if data
                        ...
                        self._stream_headers(timestamp, sig)
                        if query is None
                        else None
                    )
                ),
...
class VikiIE(VikiBaseIE):
    IE_NAME = "viki"
    _VALID_URL = (
        rf"{VikiBaseIE._VALID_URL_BASE}(?:videos|player)/(?P<id>[0-9]+v)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        if not title:
            title = (
                ...
                else video.get("id") or video_id
            )
            container_titles = (
                try_get(video, lambda x: x["container"]["titles"], dict) or {}
            )
        ...
        subtitles = dict(
            (
                ...
                [
                    {
                        "ext": ext,
                        "url": self._API_URL_TEMPLATE
                        % self._api_query(
                            f"videos/{video_id}/auth_subtitles/{lang}.{ext}",
                            stream_id=stream_id,
                        ),
                    }
                    for ext in ("srt", "vtt")
            ...
            for lang in (video.get("subtitle_completions") or {})
        )
        ...
        return {
            ...
            "formats": formats,
            "title": title,
            "description": self.dict_selection(
                video.get("descriptions", {}), "en"
            ),
            ...
            "like_count": int_or_none(
                try_get(video, lambda x: x["likes"]["count"])
            ),
...
class VikiChannelIE(VikiBaseIE):
    ...
    def _entries(self, channel_id):
        ...
        video_types = (
            self._configuration_arg("video_types") or self._video_types
        )
        for video_type in video_types:
            ...
            while True:
                ...
                res = self._call_api(
                    f"containers/{channel_id}/{video_type}.json",
                    channel_id,
                    query=params,
                    fatal=False,
                ...
                for video_id in res.get("response") or []:
                    yield self.url_result(
                        f"https://www.viki.com/videos/{video_id}",
                        VikiIE.ie_key(),
                        video_id,
                    )
                if not res.get("more"):
