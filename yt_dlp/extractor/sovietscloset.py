...
class SovietsClosetBaseIE(InfoExtractor):
    ...
    def video_meta(
        self, video_id, game_name, category_name, episode_number, stream_date
    ):
        title = game_name
...
class SovietsClosetIE(SovietsClosetBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?sovietscloset\.com/video/(?P<id>[0-9]+)/?"
    )
    _TESTS = [
        {
        ...
        return self.url_result(
            ...
            **self.video_meta(
                ...
                category_name=try_get(
                    stream, lambda x: x["subcategory"]["name"], str
                ),
...
class SovietsClosetPlaylistIE(SovietsClosetBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?sovietscloset\.com/(?!video)(?P<id>[^#?]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        sovietscloset = self.parse_nuxt_jsonp(
            f"{static_assets_base}/payload.js", playlist_id, "global"
        )["games"]
        ...
        category = next(
            (
                ...
                if cat.get("slug", "").lower() == category_slug
            ),
        ...
        entries = [
            {
                **self.url_result(
                    f'https://sovietscloset.com/video/{stream["id"]}',
                    ie=SovietsClosetIE.ie_key(),
                ),
                **self.video_meta(
                    video_id=stream["id"],
                    game_name=game["name"],
