...
class GenericIE(InfoExtractor):
    ...
    def _extra_manifest_info(self, info, manifest_url):
        ...
        key_query = self._configuration_arg(
            "key_query", [None], casesense=True
        )[0]
        if key_query is not None:
            info["extra_param_to_key_url"] = (
                urllib.parse.urlparse(key_query).query
                or key_query
                or urllib.parse.urlparse(manifest_url).query
                or None
            )
        ...
        def hex_or_none(value):
            return (
                value
                if re.fullmatch(r"(0x)?[\da-f]+", value, re.IGNORECASE)
                else None
            )

        info["hls_aes"] = (
            traverse_obj(
                self._configuration_arg("hls_key", casesense=True),
                {
                    "uri": (0, {url_or_none}),
                    "key": (0, {hex_or_none}),
                    "iv": (1, {hex_or_none}),
        ...
        variant_query = self._configuration_arg(
            "variant_query", [None], casesense=True
        )[0]
        if variant_query is not None:
            query = urllib.parse.parse_qs(
                urllib.parse.urlparse(variant_query).query
                or variant_query
                or urllib.parse.urlparse(manifest_url).query
            )
        ...
        m3u8_format = next(
            (
                f
                for f in self._downloader._get_formats(info)
                if determine_protocol(f) == "m3u8_native"
            ),
            None,
        )
        if m3u8_format:
            ...
            if is_live is not None:
                info["live_status"] = (
                    "not_live" if is_live == "false" else "is_live"
                )
                return
            headers = m3u8_format.get("http_headers") or info.get(
                "http_headers"
            )
            duration = self._extract_m3u8_vod_duration(
                m3u8_format["url"],
                info.get("id"),
                note="Checking m3u8 live status",
    ...
    def _extract_rss(self, url, video_id, doc):
        ...
        for it in doc.findall("./channel/item"):
            ...
            def itunes(key):
                return xpath_text(
                    it, xpath_with_ns(f"./itunes:{key}", NS_MAP), default=None
                )
            ...
            entries.append(
                {
                    ...
                    "thumbnail": url_or_none(
                        xpath_attr(
                            it, xpath_with_ns("./itunes:image", NS_MAP), "href"
                        )
                    ),
            ...
            formats.append(
                {
                    "url": urljoin(
                        url,
                        self._kvs_get_real_url(
                            flashvars[key], flashvars["license_code"]
                        ),
                    ),
                    "format_id": format_id,
                    "ext": "mp4",
                    **(
                        ...
                        or parse_resolution(flashvars[key])
                    ),
    ...
    def _real_extract(self, url):
        ...
        if not parsed_url.scheme:
            ...
            if default_search in ("error", "fixup_error"):
                raise ExtractorError(
                    ...
                    f'Set --default-search "ytsearch" (or run  yt-dlp "ytsearch:{url}" ) to search YouTube',
                    expected=True,
                )
            else:
        ...
        try:
            ...
            if not self._downloader._impersonate_target_available(
                ...
                msg += (
                    "see  https://github.com/yt-dlp/yt-dlp#impersonation  for "
        ...
        if m:
            ...
            ext = determine_ext(url, default_ext=None) or urlhandle_detect_ext(
                full_response
            )
            subtitles = {}
                ...
                formats = [
                    {
                        ...
                        "ext": ext,
                        "vcodec": (
                            "none" if m.group("type") == "audio" else None
                        ),
                    }
                ]
        ...
        if first_bytes.startswith(b"#EXTM3U"):
            ...
            info_dict["formats"], info_dict["subtitles"] = (
                self._extract_m3u8_formats_and_subtitles(url, video_id, "mp4")
            )
            self._extra_manifest_info(info_dict, url)
            return info_dict
        ...
        try:
            ...
            if doc.tag == "rss":
                ...
                info_dict["formats"], info_dict["subtitles"] = (
                    self._parse_ism_formats_and_subtitles(doc, url)
                )
                ...
                info_dict["formats"], info_dict["subtitles"] = (
                    self._parse_mpd_formats_and_subtitles(
                        doc,
                        mpd_base_url=full_response.url.rpartition("/")[0],
                        mpd_url=url,
                    )
                )
                info_dict["live_status"] = (
                    "is_live" if doc.get("type") == "dynamic" else None
                )
                self._extra_manifest_info(info_dict, url)
                self.report_detected("DASH manifest")
                return info_dict
            elif re.match(
                r"^{http://ns\.adobe\.com/f4m/[12]\.0}manifest$", doc.tag
            ):
        ...
        embeds = list(
            self._extract_embeds(
                original_url, webpage, urlh=full_response, info_dict=info_dict
            )
        )
        if len(embeds) == 1:
    ...
    def _extract_embeds(self, url, webpage, *, urlh=None, info_dict={}):
        ...
        if jwplayer_data:
            if isinstance(jwplayer_data.get("playlist"), str):
                ...
                return [
                    self.url_result(jwplayer_data["playlist"], "JWPlatform")
                ]
            try:
        ...
        if mobj is not None:
            ...
            for sub_match in re.finditer(
                ...
                subtitles.setdefault(
                    dict_get(sub, ("language", "srclang")) or "und", []
                ).append(
                    {
        ...
        if not found:
            ...
            found = filter_video(
                re.findall(
                    r'[^A-Za-z0-9]?(?:file|source)=(http[^\'"&]*)', webpage
                )
            )
            if found:
        ...
        if not found:
            ...
            m_video_type = re.findall(
                r'<meta.*?property="og:video:type".*?content="video/(.*?)"',
                webpage,
            )
        ...
        domain_name = self._search_regex(
            r"^(?:https?://)?([^/]*)/.*", url, "video uploader", default=None
        )
        ...
        for video_url in orderedSet(found):
            ...
            if ext == "smil":
                entry_info_dict = {
                    **self._extract_smil_info(video_url, video_id),
                    **entry_info_dict,
                }
            elif ext == "xspf":
                return [self._extract_xspf_playlist(video_url, video_id)]
            elif ext == "m3u8":
                entry_info_dict["formats"], entry_info_dict["subtitles"] = (
                ...
                re.search(r"(?i)\.(?:ism|smil)/manifest", video_url)
                and video_url != url
            ):
