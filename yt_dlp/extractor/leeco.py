...
class LeIE(InfoExtractor):
        ...
        if playstatus["status"] == 0:
            ...
            if flag == 1:
                ...
                raise ExtractorError(
                    "Generic error. flag = %d" % flag, expected=True
                )
    ...
    def _real_extract(self, url):
        ...
        play_json_flash = self._download_json(
            ...
            media_id,
            "Downloading flash playJson data",
            query={
                "id": media_id,
                "platid": 1,
                "splatid": 105,
        ...
        def get_flash_urls(media_url, format_id):
            ...
            req = self._request_webpage(
                nodes_data["nodelist"][0]["location"],
                media_id,
            ...
            return {
                "hls": encode_data_uri(
                    m3u8_data, "application/vnd.apple.mpegurl"
                ),
            }
...
class LePlaylistIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        media_ids = orderedSet(
            re.findall(
                r'<a[^>]+href="http://www\.letv\.com/ptv/vplay/(\d+)\.html',
                page,
            )
        )
        entries = [
            self.url_result(LeIE._URL_TEMPLATE % media_id, ie="Le")
            for media_id in media_ids
        ]

        title = self._html_search_meta("keywords", page, fatal=False).split(
            "，"
        )[0]
        description = self._html_search_meta("description", page, fatal=False)
...
class LetvCloudIE(InfoExtractor):
    ...
    def _get_formats(self, cf, uu, vu, media_id):
        ...
        if not play_json.get("data"):
            if play_json.get("message"):
                ...
                raise ExtractorError(
                    "Letv cloud returned error %d" % play_json["code"],
                    expected=True,
                )
            else:
    ...
    def _real_extract(self, url):
        ...
        formats = self._get_formats(
            "flash", uu, vu, media_id
        ) + self._get_formats("html5", uu, vu, media_id)
