...
class MedialaanIE(InfoExtractor):
    ...
    def _extract_embed_urls(cls, url, webpage):
        ...
        for element in re.findall(
            r'(<div[^>]+data-mychannels-type="video"[^>]*>)', webpage
        ):
            mychannels_id = extract_attributes(element).get(
                "data-mychannels-id"
            )
            if mychannels_id:
                entries.append(
                    "https://mychannels.video/embed/" + mychannels_id
                )
        return entries
