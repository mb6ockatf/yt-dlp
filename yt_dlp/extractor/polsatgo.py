...
class PolsatGoIE(InfoExtractor):
    ...
    def _extract_formats(self, sources, video_id):
        for source in sources or []:
            ...
            yield {
                "url": url,
                "height": int_or_none(
                    try_get(source, lambda x: x["quality"][:-1])
                ),
            }
    ...
    def _real_extract(self, url):
        ...
        media = self._call_api(
            "navigation", video_id, "prePlayData", {"mediaId": video_id}
        )["mediaItem"]

        formats = list(
            self._extract_formats(
                try_get(media, lambda x: x["playback"]["mediaSources"]),
                video_id,
            )
        )
    ...
    def _call_api(self, endpoint, media_id, method, params):
        ...
        res = self._download_json(
            f"https://b2c-mobile.redefine.pl/rpc/{endpoint}/",
            media_id,
