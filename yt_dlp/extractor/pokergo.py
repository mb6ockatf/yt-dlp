...
class PokerGoIE(PokerGoBaseIE):
    ...
    def _real_extract(self, url):
        ...
        thumbnails = [
            ...
            for image in data_json.get("images") or []
            if image.get("url")
        ]
        series_json = (
            traverse_obj(
                data_json,
                ("show_tags", lambda _, v: v["video_id"] == video_id, any),
            )
            or {}
        )
...
class PokerGoCollectionIE(PokerGoBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?pokergo\.com/collections/(?P<id>[^&$#/?]+)"
    )
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._entries(playlist_id), playlist_id=playlist_id
        )
