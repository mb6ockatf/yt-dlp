...
class ABCIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if mobj is None:
            ...
            video = (
                mobj.group("type") == "Video"
                or traverse_obj(
                    urls_info, (0, ("contentType", "MIMEType")), get_all=False
                )
                == "video/mp4"
            )
...
class ABCIViewIE(InfoExtractor):
    ...
    _VALID_URL = (
        r"https?://iview\.abc\.net\.au/(?:[^/]+/)*video/(?P<id>[^/?#]+)"
    )
    _GEO_COUNTRIES = ["AU"]
    ...
    def _real_extract(self, url):
        ...
        title = unescapeHTML(
            video_params.get("title") or video_params["seriesTitle"]
        )
        stream = next(
            s
            for s in video_params["playlist"]
            if s.get("type") in ("program", "livestream")
        )
        ...
        sig = hmac.new(
            b"android.content.res.Resources", path.encode(), hashlib.sha256
        ).hexdigest()
        ...
        for sd in ("1080", "720", "sd", "sd-low"):
            sd_url = try_get(stream, lambda x: x["streams"]["hls"][sd], str)
            if not sd_url:
                continue
            formats = self._extract_m3u8_formats(
                tokenize_url(sd_url, token),
                video_id,
                "mp4",
...
class ABCIViewShowSeriesIE(InfoExtractor):
    ...
    _VALID_URL = (
        r"https?://iview\.abc\.net\.au/show/(?P<id>[^/]+)(?:/series/\d+)?$"
    )
    _GEO_COUNTRIES = ["AU"]
    ...
    def _real_extract(self, url):
        ...
        highlight = try_get(
            video_data, lambda x: x["highlightVideo"]["shareUrl"]
        )
        if not self._yes_playlist(
            show_id, bool(highlight), video_label="highlight video"
        ):
        ...
        return {
            ...
            "entries": [
                ...
                for episode_url in traverse_obj(
                    series,
                    (
                        "_embedded",
                        "videoEpisodes",
            ...
            "thumbnail": traverse_obj(
                series,
                "thumbnail",
                ("images", lambda _, v: v["name"] == "seriesThumbnail", "url"),
                get_all=False,
            ),
        }
