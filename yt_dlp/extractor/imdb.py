...
class ImdbIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        data = video_info.get("playbackURLs") or try_get(
            self._download_json(
                "https://www.imdb.com/ve/data/VIDEO_PLAYBACK_DATA",
                video_id,
                query={
        ...
        for encoding in data:
            ...
            if not video_url:
                continue
            ext = mimetype2ext(encoding.get("mimeType")) or determine_ext(
                video_url
            )
            if ext == "m3u8":
        ...
        return {
            ...
            "formats": formats,
            "description": try_get(
                video_info, lambda x: x["description"]["value"]
            ),
            "thumbnail": url_or_none(
                try_get(video_info, lambda x: x["thumbnail"]["url"])
            ),
            "duration": int_or_none(
                try_get(video_info, lambda x: x["runtime"]["value"])
            ),
            "subtitles": subtitles,
        }
...
class ImdbListIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        entries = [
            ...
            for m in re.findall(
                rf'href="(/list/ls{list_id}/videoplayer/vi[^"]+)"', webpage
            )
        ]
