...
class TedBaseIE(InfoExtractor):
    _VALID_URL_BASE = (
        r"https?://www\.ted\.com/(?:{type})(?:/lang/[^/#?]+)?/(?P<id>[\w-]+)"
    )
        ...
        for entry in try_get(playlist, lambda x: x["videos"]["nodes"], list):
            if entry.get("__typename") == "Video" and entry.get(
                ...
                yield self.url_result(
                    entry["canonicalUrl"], TedTalkIE.ie_key()
                )
...
class TedTalkIE(TedBaseIE):
    ...
    def _real_extract(self, url):
        ...
        talk_info = self._search_nextjs_data(webpage, display_id)["props"][
            "pageProps"
        ]["videoData"]
        ...
        if http_url:
            m3u8_formats = [
                f
                for f in formats
                if f.get("protocol") == "m3u8" and f.get("vcodec") != "none"
            ]
        ...
        if not formats:
            ...
            ext_url = (
                external.get("code") if service.lower() == "youtube" else None
            )
            return self.url_result(ext_url or external["uri"])

        thumbnail = player_data.get("thumb") or self._og_search_property(
            "image", webpage
        )
        if thumbnail:
        ...
        return {
            ...
            or parse_duration(
                self._og_search_property("video:duration", webpage)
            ),
            ...
            "tags": try_get(
                player_data, lambda x: x["targeting"]["tag"].split(",")
            ),
        }
...
class TedSeriesIE(TedBaseIE):
    ...
    def _real_extract(self, url):
        ...
        info = self._search_nextjs_data(webpage, display_id)["props"][
            "pageProps"
        ]

        entries = itertools.chain.from_iterable(
            self._parse_playlist(s)
...
class TedPlaylistIE(TedBaseIE):
    ...
    def _real_extract(self, url):
        display_id = self._match_id(url)
        webpage = self._download_webpage(url, display_id)
        playlist = self._search_nextjs_data(webpage, display_id)["props"][
            "pageProps"
        ]["playlist"]
        ...
        return self.playlist_result(
            ...
            playlist.get("id"),
            playlist.get("title")
            or self._og_search_title(webpage, default="").replace(
                " | TED Talks", ""
            )
            or None,
            self._og_search_description(webpage),
...
class TedEmbedIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        return self.url_result(
            re.sub(r"://embed(-ssl)?", "://www", url), TedTalkIE.ie_key()
        )
