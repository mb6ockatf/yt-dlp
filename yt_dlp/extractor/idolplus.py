...
class IdolPlusIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        subtitles = {}
        for caption in (
            traverse_obj(player_data, ("vodPlayerList", "caption")) or []
        ):
        ...
        if (
            traverse_obj(player_data, ("detail", "has_cuesheet")) == "Y"
            ...
            for member in traverse_obj(cuesheet, ("members", ...)):
                ...
                member_video_url = traverse_obj(
                    cuesheet,
                    ("omni_view", index, "cdn_url", 0, "url", {url_or_none}),
                )
                ...
                member_formats = self._extract_m3u8_formats(
                    member_video_url,
                    video_id,
                    note=f'Downloading m3u8 for multicam {member["name"]}',
                )
                for mf in member_formats:
                    mf["format_id"] = (
                        f'{mf["format_id"]}-{member["name"].replace(" ", "_")}'
                    )
        ...
        return {
            ...
            "release_date": traverse_obj(
                player_data, ("detail", "broadcastDate")
            ),
        }
