...
class CTVNewsIE(InfoExtractor):
    ...
    def _ninecninemedia_url_result(self, clip_id):
        return self.url_result(
            f"9c9media:ctvnews_web:{clip_id}", NineCNineMediaIE, clip_id
        )
    ...
    def _real_extract(self, url):
        ...
        if mobj := re.fullmatch(
            self._VIDEO_ID_RE, urllib.parse.urlparse(url).fragment
        ):
        ...
        webpage = self._download_webpage(
            f"https://www.ctvnews.ca/{page_id}",
            page_id,
            query={
        ...
        entries = [
            self._ninecninemedia_url_result(clip_id)
            for clip_id in orderedSet(
        ...
        if not entries:
            ...
            if 'getAuthStates("' in webpage:
                entries = [
                    self._ninecninemedia_url_result(clip_id)
                    for clip_id in self._search_regex(
                        r'getAuthStates\("([\d+,]+)"', webpage, "clip ids"
                    ).split(",")
                ]
            else:
                entries = [
                    self._ninecninemedia_url_result(clip_id)
                    for clip_id in traverse_obj(
                        webpage,
                        (
                            {
                                find_element(
                                    tag="jasper-player-container", html=True
                                )
                            },
                            {extract_attributes},
                            "axis-ids",
