...
class VVVVIDIE(InfoExtractor):
    _VALID_URL_BASE = (
        r"https?://(?:www\.)?vvvvid\.it/(?:#!)?(?:show|anime|film|series)/"
    )
    ...
    def _real_initialize(self):
        self._conn_id = self._download_json(
            "https://www.vvvvid.it/user/login", None, headers=self._headers
        )["data"]["conn_id"]
    ...
    def _download_info(self, show_id, path, video_id, fatal=True, query=None):
        ...
        if response.get("result") == "error":
            raise ExtractorError(
                "{} said: {}".format(self.IE_NAME, response["message"]),
                expected=True,
            )
    ...
    def _real_extract(self, url):
        ...
        vid = int(video_id)
        video_data = next(
            filter(lambda episode: episode.get("video_id") == vid, response)
        )
        ...
        for quality in ("", "_sd"):
            ...
            if video_type == "video/kenc":
                embed_code = re.sub(
                    r"https?(://[^/]+)/z/", r"https\1/i/", embed_code
                ).replace("/manifest.f4m", "/master.m3u8")
                kenc = (
                    self._download_json(
                        "https://www.vvvvid.it/kenc",
                        video_id,
                        query={
                            "action": "kt",
                            "conn_id": self._conn_id,
                ...
                formats.extend(
                    self._extract_akamai_formats(embed_code, video_id)
                )
