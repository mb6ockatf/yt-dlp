...
class AnchorFMEpisodeIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        channel_name, episode_id = self._match_valid_url(url).group(
            "channel_name", "episode_id"
        )
        ...
        return {
            ...
            "duration": float_or_none(
                traverse_obj(api_data, ("episode", "duration")), 1000
            ),
            "modified_timestamp": unified_timestamp(
                traverse_obj(api_data, ("episode", "modified"))
            ),
            "release_timestamp": int_or_none(
                traverse_obj(api_data, ("episode", "publishOnUnixTimestamp"))
            ),
            "episode_id": episode_id,
            "uploader": traverse_obj(api_data, ("creator", "name")),
            "uploader_id": str_or_none(
                traverse_obj(api_data, ("creator", "userId"))
            ),
            "season_number": int_or_none(
                traverse_obj(api_data, ("episode", "podcastSeasonNumber"))
            ),
            "channel": channel_name
            or traverse_obj(api_data, ("creator", "vanitySlug")),
        }
