...
class WhoWatchIE(InfoExtractor):
    ...
    def _real_extract(self, url):
                ...
                formats.append(
                    {
                        ...
                        "width": try_get(
                            hls_fmts, lambda x: x[0]["width"], int
                        ),
                        "height": try_get(
                            hls_fmts, lambda x: x[0]["height"], int
                        ),
                    }
                )
        ...
        uploader_id = str(
            try_get(metadata, lambda x: x["live"]["user"]["id"], int)
        )
        uploader = try_get(metadata, lambda x: x["live"]["user"]["name"], str)
        thumbnail = try_get(
            metadata, lambda x: x["live"]["latest_thumbnail_url"], str
        )
        timestamp = int_or_none(
            try_get(metadata, lambda x: x["live"]["started_at"], int),
            scale=1000,
        )
        view_count = try_get(
            metadata, lambda x: x["live"]["total_view_count"], int
        )
