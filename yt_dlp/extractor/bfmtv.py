...
class BFMTVBaseIE(InfoExtractor):
    ...
    _VALID_URL_TMPL = (
        _VALID_URL_BASE + r"(?:[^/]+/)*[^/?&#]+_%s[A-Z]-(?P<id>\d{12})\.html"
    )
    ...
    def _extract_video(self, video_block):
        ...
        if video_element:
            ...
            if not video_id:
                return
            account_id = (
                video_element_attrs.get("data-account") or "876450610001"
            )
...
class BFMTVIE(BFMTVBaseIE):
    ...
    def _real_extract(self, url):
        ...
        video = self._extract_video(
            self._search_regex(self._VIDEO_BLOCK_REGEX, webpage, "video block")
        )
        if not video:
