...
class StitcherBaseIE(InfoExtractor):
    ...
    def _call_api(self, path, video_id, query):
        resp = self._download_json(
            "https://api.prod.stitcher.com/" + path, video_id, query=query
        )
    ...
    def _extract_description(self, data):
        return clean_html(
            data.get("html_description") or data.get("description")
        )
...
class StitcherIE(StitcherBaseIE):
    _VALID_URL = (
        StitcherBaseIE._VALID_URL_BASE
        + r"(?:[^/]+/)+e(?:pisode)?/(?:[^/#?&]+-)?(?P<id>\d+)"
    )
    _TESTS = [
        {
...
class StitcherShowIE(StitcherBaseIE):
    _VALID_URL = (
        StitcherBaseIE._VALID_URL_BASE + r"(?P<id>[^/#?&]+)/?(?:[?#&]|$)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        for episode in data.get("episodes") or []:
            ...
            if not audio_url:
                continue
            entries.append(
                self._extract_episode(episode, audio_url, show_info)
            )
