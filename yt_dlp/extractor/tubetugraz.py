...
class TubeTuGrazBaseIE(InfoExtractor):
    ...
    def _perform_login(self, username, password):
        ...
        urlh = self._request_webpage(
            ...
            fatal=False,
            headers={"referer": urlh.url},
            note="logging in with TFA",
            errnote="unable to log in with TFA",
            data=urlencode_postdata(
                {
                    "lang": "de",
                    "_eventId_proceed": "",
        ...
        if (
            not urlh
            or urlh.url == "https://tube.tugraz.at/paella/ui/index.html"
        ):
            return
    ...
    def _extract_episode(self, episode_info):
        ...
        formats = list(
            self._extract_formats(
                traverse_obj(episode_info, ("mediapackage", "media", "track")),
                video_id,
            )
        )
        ...
        return {
            ...
            "duration": traverse_obj(
                episode_info, ("mediapackage", "duration"), "dcExtent"
            ),
            "series": series_title,
            "series_id": traverse_obj(
                episode_info, ("mediapackage", "series"), "dcIsPartOf"
            ),
        ...
        for format_info in format_list or []:
            ...
            if transport == "https":
                formats = [
                    {
                        "url": url,
                        "abr": float_or_none(
                            traverse_obj(format_info, ("audio", "bitrate")),
                            1000,
                        ),
                        "vbr": float_or_none(
                            traverse_obj(format_info, ("video", "bitrate")),
                            1000,
                        ),
                        "fps": traverse_obj(
                            format_info, ("video", "framerate")
                        ),
                        **parse_resolution(
                            traverse_obj(format_info, ("video", "resolution"))
                        ),
                    }
                ]
            elif transport == "hls":
                has_hls, formats = True, self._extract_m3u8_formats(
                    ...
                    "mp4",
                    fatal=False,
                    note=f"downloading {fmt_type} HLS manifest",
                )
            elif transport == "dash":
                has_dash, formats = True, self._extract_mpd_formats(
                    ...
                    video_id,
                    fatal=False,
                    note=f"downloading {fmt_type} DASH manifest",
                )
            else:
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            map(
                ...
                episodes_data["search-results"]["result"],
            ),
            playlist_id,
            traverse_obj(
                series_data,
