...
class TelecincoBaseIE(InfoExtractor):
    def _parse_content(self, content, url):
        ...
        try:
            cdn = self._download_json(
                ...
                data=json.dumps(
                    {
                        "bbx": caronte["bbx"],
                        "gbx": self._download_json(services["gbx"], video_id)[
                            "gbx"
                        ],
                    }
                ).encode(),
        ...
        return {
            ...
            "formats": formats,
            "thumbnail": (
                traverse_obj(content, ("dataPoster", {url_or_none}))
                or traverse_obj(
                    config, "poster", "imageUrl", expected_type=url_or_none
                )
            ),
    ...
    def _real_extract(self, url):
        ...
        if article.get("editorialType") != "VID":
            ...
            for p in traverse_obj(
                article,
                ((("opening", all), "body"), lambda _, v: v["content"]),
            ):
                content = p["content"]
                type_ = p.get("type")
            ...
            return self.playlist_result(
                entries,
                str_or_none(article.get("id")),
                traverse_obj(article, ("title", {str})),
