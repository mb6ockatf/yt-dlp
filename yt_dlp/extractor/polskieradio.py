            ...
            entry.update(
                {
                    ...
                    "vcodec": (
                        "none" if media.get("provider") == "audio" else None
                    ),
                }
            )
...
class PolskieRadioIE(PolskieRadioBaseExtractor):
    ...
    def _real_extract(self, url):
        ...
        article_data = traverse_obj(
            ...
            ("props", "pageProps", (("data", "articleData"), "post", "data")),
            get_all=False,
        )
        ...
        entries = [
            ...
            for entry in article_data.get("attachments") or ()
            if entry.get("fileType") in ("Audio",)
        ]
...
class PolskieRadioAuditionIE(InfoExtractor):
    ...
    _VALID_URL = (
        r"https?://(?:[^/]+\.)?polskieradio\.pl/audycj[ae]/(?P<id>\d+)"
    )
    _TESTS = [
        {
            # articles, PR1
    ...
    def _entries(self, playlist_id, has_episodes, has_articles):
        for i in itertools.count(0) if has_episodes else []:
            page = self._call_lp3(
                ...
                playlist_id,
                f"Downloading episode list page {i + 1}",
            )
        ...
        for i in itertools.count(0) if has_articles else []:
            page = self._call_lp3(
                ...
                playlist_id,
                f"Downloading article list page {i + 1}",
            )
            if not traverse_obj(page, "data"):
                break
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._entries(playlist_id, has_episodes, has_articles),
            playlist_id,
            title=traverse_obj(page_props, ("details", "name")),
            description=traverse_obj(
                page_props, ("details", "description", "lead")
            ),
...
class PolskieRadioCategoryIE(InfoExtractor):
    ...
    def suitable(cls, url):
        return (
            False
            if PolskieRadioLegacyIE.suitable(url)
    ...
    def _entries(self, url, page, category_id):
        ...
        for page_num in itertools.count(2):
            ...
            for a_entry in re.findall(r"<span data-media=({[^ ]+})", content):
                yield traverse_obj(
                    self._parse_json(a_entry, category_id),
                    {
                        "url": "file",
                        "id": "uid",
                        "duration": "length",
                        ...
                        dict(
                            zip(
                                (
                                    ...
                                    "queryString",
                                    "name",
                                    "openArticlesInParentTemplate",
                                    "idSectionFromUrl",
                                    "maxDocumentAge",
                                    "showCategoryForArticle",
                                    "pageNumber",
                                ),
                                params,
                            )
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._entries(url, webpage, category_id), category_id, title
        )
...
class PolskieRadioPlayerIE(InfoExtractor):
    ...
    def _get_channel_list(self, channel_url="no_channel"):
        player_code = self._download_webpage(
            self._PLAYER_URL, channel_url, note="Downloading js player"
        )
        channel_list = js_to_json(
            self._search_regex(
    ...
    def _real_extract(self, url):
        ...
        station_list = self._download_json(
        ...
        station = next(
            (
                ...
                if s.get("Name")
                == (channel.get("streamName") or channel.get("name"))
            ),
            None,
        )
        if not station:
        ...
        for stream_url in station["Streams"]:
            ...
            if stream_url.endswith("/playlist.m3u8"):
                ...
                formats.extend(
                    self._extract_mpd_formats(stream_url, channel_url)
                )
            elif stream_url.endswith("/Manifest"):
                formats.extend(
                    self._extract_ism_formats(stream_url, channel_url)
                )
            else:
                formats.append(
                    {
...
class PolskieRadioPodcastListIE(PolskieRadioPodcastBaseExtractor):
    ...
    def _real_extract(self, url):
        ...
        def get_page(page_num):
            page_data = (
                self._call_api(podcast_id, page_num + 1) if page_num else data
            )
        ...
        return {
            ...
            "entries": InAdvancePagedList(
                get_page,
                math.ceil(data["itemCount"] / self._PAGE_SIZE),
                self._PAGE_SIZE,
            ),
            "id": str(data["id"]),
