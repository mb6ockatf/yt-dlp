    ...
    def _real_extract(self, url):
        ...
        formats = [
            {
                ...
                "ext": "mp4",
                "width": try_get(
                    video, lambda x: x["res"]["width"], expected_type=int
                ),
                "height": try_get(
                    video, lambda x: x["res"]["height"], expected_type=int
                ),
            }
            for video in try_get(
                data_json, lambda x: x["video"]["mp4"], expected_type=list
            )
            or []
            if video.get("$url")
        ]
        ...
        return {
            ...
            "description": clean_html(
                dict_get(data_json, ("longDesc", "shortDesc"))
            ),
            "thumbnail": data_json.get("poster"),
            "upload_date": unified_strdate(
                dict_get(
                    data_json, ("webPublishAt", "featuredAt", "diffusionDate")
                )
            ),
            "duration": try_get(
                data_json, lambda x: x["video"]["duration"], expected_type=int
            ),
            "formats": formats,
            "subtitles": subtitles,
        }
