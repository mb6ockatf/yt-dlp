...
class ReutersIE(InfoExtractor):
    _WORKING = False
    _VALID_URL = (
        r"https?://(?:www\.)?reuters\.com/.*?\?.*?videoId=(?P<id>[0-9]+)"
    )
    _TEST = {
    ...
    def _real_extract(self, url):
        ...
        webpage = self._download_webpage(
            f"http://www.reuters.com/assets/iframe/yovideo?videoId={video_id}",
            video_id,
        )
        video_data = js_to_json(
            self._search_regex(
        ...
        mmid, fid = re.search(
            r",/(\d+)\?f=(\d+)", get_json_value("flv", fatal=True)
        ).groups()
