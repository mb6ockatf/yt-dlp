...
class AcFunVideoIE(AcFunVideoBaseIE):
    ...
    def _real_extract(self, url):
        ...
        json_all = self._search_json(
            r"window.videoInfo\s*=", webpage, "videoInfo", video_id
        )
...
class AcFunBangumiIE(AcFunVideoBaseIE):
    ...
    def _real_extract(self, url):
        ...
        season_number = season_id and next(
            (
                ...
                for idx, v in enumerate(
                    json_bangumi_data.get("relatedBangumis") or [], 1
                )
                if v.get("id") == season_id
            ),
            1,
        )
        ...
        video_internal_id = int_or_none(
            traverse_obj(json_bangumi_data, ("currentVideoInfo", "id"))
        )
        episode_number = video_internal_id and next(
            (
                idx
                for idx, v in enumerate(
                    json_bangumi_list.get("items") or [], 1
                )
        ...
        return {
            **self._extract_metadata(video_id, video_info),
            ...
            "comment_count": int_or_none(
                json_bangumi_data.get("commentCount")
            ),
        }
