...
class PBSIE(InfoExtractor):
    _STATIONS = (
        ...
        (
            r"video\.wyomingpbs\.org",
            "Wyoming PBS (KCWC)",
        ),  # http://www.wyomingpbs.org
        (
            r"video\.cpt12\.org",
            "Colorado Public Television / KBDI 12 (KBDI)",
        ),  # http://www.cpt12.org/
        (
            r"video\.kbyueleven\.org",
            "KBYU-TV (KBYU)",
        ),  # http://www.kbyutv.org/
        (
            r"(?:video\.|www\.)thirteen\.org",
            "Thirteen/WNET New York (WNET)",
        ),  # http://www.thirteen.org
        (r"video\.wgbh\.org", "WGBH/Channel 2 (WGBH)"),  # http://wgbh.org
        (r"video\.wgby\.org", "WGBY (WGBY)"),  # http://www.wgby.org
        (
            r"watch\.njtvonline\.org",
            "NJTV Public Media NJ (WNJT)",
        ...
        (
            r"video\.mpt\.tv",
            "mpt/Maryland Public Television (WMPB)",
        ),  # http://www.mpt.org
        (
            r"watch\.weta\.org",
            "WETA Television and Radio (WETA)",
        ),  # http://www.weta.org
        (r"video\.whyy\.org", "WHYY (WHYY)"),  # http://www.whyy.org
        (r"video\.wlvt\.org", "PBS 39 (WLVT)"),  # http://www.wlvt.org/
        (
            r"video\.wvpt\.net",
            "WVPT - Your Source for PBS and More! (WVPT)",
        ),  # http://www.wvpt.net
        (
            r"video\.whut\.org",
        ...
        (
            r"watch\.wxel\.org",
            "WXEL/Channel 42 (WXEL)",
        ),  # http://www.wxel.org/home/
        (
            r"video\.wlrn\.org",
            "WLRN/Channel 17 (WLRN)",
        ),  # http://www.wlrn.org/
        (
            r"video\.wusf\.usf\.edu",
            "WUSF Public Broadcasting (WUSF)",
        ),  # http://wusf.org/
        ...
        (
            r"video\.pbshawaii\.org",
            "PBS Hawaii - Oceanic Cable Channel 10 (KHET)",
        ),  # http://www.pbshawaii.org/
        (
            r"video\.idahoptv\.org",
            "Idaho Public Television (KAID)",
        ),  # http://idahoptv.org
        (r"video\.ksps\.org", "KSPS (KSPS)"),  # http://www.ksps.org/home/
        (r"watch\.opb\.org", "OPB (KOPB)"),  # http://www.opb.org
        (
            r"watch\.nwptv\.org",
            "KWSU/Channel 10 & KTNW/Channel 31 (KWSU)",
        ),  # http://www.kwsu.org
        (
            r"video\.will\.illinois\.edu",
            "WILL-TV (WILL)",
        ),  # http://will.illinois.edu/
        (
            r"video\.networkknowledge\.tv",
        ...
        (
            r"video\.iptv\.org",
            "Iowa Public Television/IPTV (KDIN)",
        ),  # http://www.iptv.org/
        (
            r"video\.ninenet\.org",
            "Nine Network (KETC)",
        ...
        (
            r"video\.wfyi\.org",
            "WFYI Indianapolis (WFYI)",
        ),  # http://www.wfyi.org
        (
            r"video\.mptv\.org",
            "Milwaukee Public Television (WMVS)",
        ),  # http://www.mptv.org
        (r"video\.wnin\.org", "WNIN (WNIN)"),  # http://www.wnin.org/
        (
            r"video\.wnit\.org",
            "WNIT Public Television (WNIT)",
        ),  # http://www.wnit.org/
        ...
        (
            r"video\.indianapublicmedia\.org",
            "WTIU (WTIU)",
        ),  # http://indianapublicmedia.org/tv/
        (
            r"watch\.cetconnect\.org",
            "CET  (WCET)",
        ...
        (
            r"watch\.easttennesseepbs\.org",
            "East Tennessee PBS (WSJK)",
        ...
        (
            r"watch\.wqed\.org",
            "WQED Multimedia (WQED)",
        ),  # http://www.wqed.org/
        (
            r"video\.wmht\.org",
            "WMHT Educational Telecommunications (WMHT)",
        ),  # http://www.wmht.org/home/
        (
            r"video\.deltabroadcasting\.org",
            "Q-TV (WDCQ)",
        ),  # http://www.deltabroadcasting.org
        (
            r"video\.dptv\.org",
            "WTVS Detroit Public TV (WTVS)",
        ),  # http://www.dptv.org/
        (
            r"video\.wcmu\.org",
            "CMU Public Television (WCMU)",
        ),  # http://www.wcmu.org
        (r"video\.wkar\.org", "WKAR-TV (WKAR)"),  # http://wkar.org/
        (
            r"wnmuvideo\.nmu\.edu",
            "WNMU-TV Public TV 13 (WNMU)",
        ),  # http://wnmutv.nmu.edu
        ...
        (
            r"video\.kmos\.org",
            "KMOS-TV - Channels 6.1, 6.2 and 6.3 (KMOS)",
        ),  # http://www.kmos.org/
        (
            r"watch\.montanapbs\.org",
            "MontanaPBS (KUSM)",
        ),  # http://montanapbs.org
        ...
        (
            r"on-demand\.wvia\.org",
            "WVIA Public Media Studios (WVIA)",
        ),  # http://www.wvia.org/
        ...
        (
            r"video\.ideastream\.org",
            "WVIZ/PBS ideastream (WVIZ)",
        ),  # http://www.wviz.org/
        (r"video\.kcts9\.org", "KCTS 9 (KCTS)"),  # http://kcts9.org/
        (
            r"video\.basinpbs\.org",
            "Basin PBS (KPBT)",
        ...
        (
            r"video\.wtjx\.org",
            "WTJX Channel 12 (WTJX)",
        ),  # http://www.wtjx.org/
        (
            r"video\.ideastations\.org",
            "WCVE PBS (WCVE)",
        ),  # http://ideastations.org/
        (
            r"video\.kbtc\.org",
            "KBTC Public Television (KBTC)",
        ),  # http://kbtc.org
    )

    IE_NAME = "pbs"
    IE_DESC = (
        "Public Broadcasting Service (PBS) and member stations: {}".format(
            ", ".join(list(zip(*_STATIONS))[1])
        )
    )
    ...
    def _real_extract(self, url):
        ...
        if http_url:
            for m3u8_format in m3u8_formats:
                ...
                f.update(
                    {
                        "url": f_url,
                        "format_id": m3u8_format["format_id"].replace(
                            "hls", "http"
                        ),
                        "protocol": "http",
                    }
        ...
        description = (
            ...
            or info.get("program", {}).get("description")
            or description
        )
...
class PBSKidsIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        meta = self._search_json(
            r"window\._PBS_KIDS_DEEPLINK\s*=", webpage, "video info", video_id
        )
