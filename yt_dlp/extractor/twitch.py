...
class TwitchBaseIE(InfoExtractor):
    ...
    def _perform_login(self, username, password):
        ...
        def login_step(page, urlh, note, data):
            ...
            redirect_url = urljoin(
                post_url, response.get("redirect") or response["redirect_path"]
            )
            return self._download_webpage_handle(
                redirect_url,
                None,
                "Downloading login redirect page",
                headers=headers,
            )
        ...
        if (
            re.search(r'(?i)<form[^>]+id="two-factor-submit"', redirect_page)
            is not None
        ):
    ...
    def _download_access_token(self, video_id, token_kind, param_name):
        ...
        return self._download_base_gql(
            video_id, ops, f"Downloading {token_kind} access token GraphQL"
        )["data"][method]
...
class TwitchVodIE(TwitchBaseIE):
    ...
    def _download_info(self, item_id):
        ...
        if video is None:
            raise ExtractorError(
                f"Video {item_id} does not exist", expected=True
            )

        video["moments"] = traverse_obj(
            data, (..., "data", "video", "moments", "edges", ..., "node")
        )
    ...
    def _extract_chapters(self, info, item_id):
        ...
        for moment in info["moments"]:
            ...
            if start_time is None or duration is None:
                self.report_warning(
                    f"Important chapter information missing for chapter {name}",
                    item_id,
                )
                continue
            yield {
    ...
    def _extract_info_gql(self, info, item_id):
        ...
        return {
            ...
            "uploader": try_get(
                info, lambda x: x["owner"]["displayName"], str
            ),
    ...
    def _extract_storyboard(self, item_id, storyboard_json_url, duration):
        ...
        spec = (
            self._download_json(
                storyboard_json_url,
                item_id,
                "Downloading storyboard metadata JSON",
                fatal=False,
            )
            or []
        )
    ...
    def _real_extract(self, url):
        ...
        formats.extend(
            self._extract_storyboard(
                vod_id, video.get("storyboard"), info.get("duration")
            )
        )
...
class TwitchCollectionIE(TwitchBaseIE):
    _VALID_URL = (
        r"https?://(?:(?:www|go|m)\.)?twitch\.tv/collections/(?P<id>[^/]+)"
    )
...
class TwitchVideosIE(TwitchPlaylistBaseIE):
    ...
    def suitable(cls, url):
        return (
            ...
            if any(
                ie.suitable(url)
                for ie in (TwitchVideosClipsIE, TwitchVideosCollectionsIE)
            )
...
class TwitchStreamIE(TwitchBaseIE):
    ...
    def _real_extract(self, url):
        ...
        thumbnail = url_or_none(
            try_get(
                gql,
                lambda x: x[2]["data"]["user"]["stream"]["previewImageURL"],
                str,
            )
        )
...
class TwitchClipsIE(TwitchBaseIE):
    ...
    def _real_extract(self, url):
        ...
        clip = self._download_gql(
            ...
            "Downloading clip access token GraphQL",
        )[0]["data"]["clip"]
  ...
  clip(slug: "%s") {
    ...
    viewCount
        ...
        return {
            ...
            "_old_archive_ids": (
                [make_archive_id(self, old_id)] if old_id else None
            ),
            "display_id": video_id,
            ...
            "thumbnails": thumbnails,
            "creator": try_get(
                clip, lambda x: x["broadcaster"]["displayName"], str
            ),
            "uploader": try_get(
                clip, lambda x: x["curator"]["displayName"], str
            ),
