...
class JStreamIE(InfoExtractor):
    ...
    def _parse_jsonp(self, callback, string, video_id):
        return self._search_json(
            rf"\s*{re.escape(callback)}\s*\(", string, callback, video_id
        )

    def _find_formats(
        self, video_id, movie_list_hls, host, publisher, subtitles
    ):
        for value in movie_list_hls:
            ...
            fmts, subs = self._extract_m3u8_formats_and_subtitles(
                f'https://{publisher}.eq.webcdn.stream.ne.jp/{host}/{publisher}/jmc_pub/{value.get("url")}',
                video_id,
                "mp4",
                m3u8_id=m3u8_id,
            )
    ...
    def _real_extract(self, url):
        host, publisher, mid, video_id = self._match_valid_url(url).group(
            "host", "publisher", "mid", "id"
        )
        ...
        video_info = self._parse_jsonp(
            "metaDataResult", video_info_jsonp, video_id
        )["movie"]
        subtitles = {}
        formats = list(
            self._find_formats(
                video_id,
                video_info.get("movie_list_hls"),
                host,
                publisher,
                subtitles,
            )
    ...
    def _extract_embed_urls(cls, url, webpage):
        ...
        script_tag = re.search(
            r'<script\s*[^>]+?src="https://ssl-cache\.stream\.ne\.jp/(?P<host>www\d+)/(?P<publisher>[a-z0-9]+)/[^"]+?/if\.js"',
            webpage,
        )
        if not script_tag:
            return
        ...
        for m in re.finditer(
            r"(?s)PlayerFactoryIF\.create\(\s*({[^\}]+?})\s*\)\s*;", webpage
        ):
