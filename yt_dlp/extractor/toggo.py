...
class ToggoIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        data = self._download_json(
            f"https://production-n.toggo.de/api/assetstore/vod/asset/{display_id}",
            display_id,
        )["data"]

        brightcove_id = next(
            x["value"]
            for x in data["custom_fields"]
            if x.get("key") == "video-cloud-id"
        )
        ...
        for f in info["formats"]:
            if "/dash/live/cenc/" in f.get("fragment_base_url", ""):
                ...
                f["fragment_base_url"] = f["fragment_base_url"].replace(
                    "/cenc/", "/clear/"
                )
                f["has_drm"] = False
        ...
        thumbnails = [
            {
                ...
                "url": url,
                "width": int_or_none(
                    next(iter(parse_qs(url).get("width", [])), None)
                ),
            }
