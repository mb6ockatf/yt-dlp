...
class QingTingIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        info = self._search_json(
            r"window\.__initStores\s*=", webpage, "program info", pid
        )
        return {
            "id": pid,
            "title": traverse_obj(
                info, ("ProgramStore", "programInfo", "title")
            ),
            "channel_id": channel_id,
            "channel": traverse_obj(
                info, ("ProgramStore", "channelInfo", "title")
            ),
            ...
            "url": traverse_obj(
                info, ("ProgramStore", "programInfo", "audioUrl")
            ),
            "vcodec": "none",
            "acodec": "m4a",
            "ext": "m4a",
