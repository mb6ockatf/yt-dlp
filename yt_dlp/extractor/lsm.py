...
class LSMLREmbedIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        video_id = traverse_obj(
            query,
            (("show", "id"), 0, {int_or_none}, filter, {str_or_none}),
            get_all=False,
        )
        ...
        player_json = (
            self._parse_json(
                player_data, video_id, transform_source=js_to_json, fatal=False
            )
            or {}
        )
        ...
        for item in traverse_obj(
            ...
            if id_.startswith("v") and not title:
                title = traverse_obj(
                    ...
                    (
                        "audio",
                        lambda _, v: v["id"][1:] == id_[1:],
                        "title",
                        {lambda x: x and f"{x} - Video Version"},
                    ),
                    get_all=False,
                )
