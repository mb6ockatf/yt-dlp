...
class DamtomoBaseIE(InfoExtractor):
    def _real_extract(self, url):
        ...
        webpage, handle = self._download_webpage_handle(
            self._WEBPAGE_URL_TMPL % video_id, video_id, encoding="sjis"
        )

        if handle.url == "https://www.clubdam.com/sorry/":
            raise ExtractorError(
                "You are rate-limited. Try again later.", expected=True
            )
        ...
        data_dict = {
            mobj.group("class"): re.sub(
                r"\s+", " ", clean_html(mobj.group("value"))
            )
        ...
        stream_tree = self._download_xml(
            self._DKML_XML_URL % video_id,
            video_id,
            note="Requesting stream information",
            encoding="sjis",
        ...
        m3u8_url = try_get(
            stream_tree,
            lambda x: x.find(
                ".//d:streamingUrl", {"d": self._DKML_XML_NS}
            ).text.strip(),
            str,
        )
        if not m3u8_url:
        ...
        return {
            ...
            "view_count": int_or_none(
                self._search_regex(
                    r"(\d+)", data_dict["audience"], "view_count", default=None
                )
            ),
            "like_count": int_or_none(
                self._search_regex(
                    r"(\d+)", data_dict["nice"], "like_count", default=None
                )
            ),
            "track": title,
