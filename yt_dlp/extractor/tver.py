...
class TVerIE(InfoExtractor):
    ...
    def _call_platform_api(
        self, path, video_id, note=None, fatal=True, query=None
    ):
    ...
    def _real_extract(self, url):
        ...
        if video_type == "series":
            series_info = self._call_platform_api(
                f"v2/callSeries/{video_id}",
                video_id,
                "Downloading series info",
            )
            return self.playlist_from_matches(
                ...
                traverse_obj(
                    series_info,
                    ("result", "content", "content", "title", {str}),
                ),
                ie=TVerIE,
        ...
        episode_content = (
            traverse_obj(episode_info, ("result", "episode", "content")) or {}
        )

        version = traverse_obj(
            episode_content, ("version", {str_or_none}), default="5"
        )
        ...
        r_id = traverse_obj(
            video_info, ("video", ("videoRefID", "videoID")), get_all=False
        )
        if not r_id:
        ...
        title = join_nonempty(series, episode, delim=" ") or str_or_none(
        ...
        return {
            ...
            "alt_title": join_nonempty(
                title, provider, onair_label, delim=" "
            ),
            "channel": provider,
