    ...
    def assert_error(self, response):
        ...
        if error_message:
            raise ExtractorError(
                f"Mirrativ says: {error_message}", expected=True
            )
...
class MirrativIE(MirrativBaseIE):
    ...
    def _real_extract(self, url):
        ...
        live_response = self._download_json(
            f"https://www.mirrativ.com/api/live/live?live_id={video_id}",
            video_id,
        )
        self.assert_error(live_response)

        hls_url = dict_get(
            live_response, ("archive_url_hls", "streaming_url_hls")
        )
        is_live = bool(live_response.get("is_live"))
        if not hls_url:
            raise ExtractorError(
                "Neither archive nor live is available.", expected=True
            )
        ...
        return {
            ...
            or self._search_regex(
                r"<title>\s*(.+?) - Mirrativ\s*</title>", webpage
            )
            or live_response.get("title"),
            "is_live": is_live,
            ...
            "duration": (
                try_get(
                    live_response, lambda x: x["ended_at"] - x["started_at"]
                )
                if not is_live
                else None
            ),
...
class MirrativUserIE(MirrativBaseIE):
        ...
        while page is not None:
            ...
            for live in lives:
                ...
                yield self.url_result(
                    url, video_id=live_id, video_title=live.get("title")
                )
    ...
    def _real_extract(self, url):
        ...
        user_info = self._download_json(
            f"https://www.mirrativ.com/api/user/profile?user_id={user_id}",
            user_id,
