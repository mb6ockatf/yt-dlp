...
class AlphaPornoIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        ext = self._html_search_meta(
            "encodingFormat", webpage, "ext", default=".mp4"
        )[1:]
        ...
        timestamp = parse_iso8601(
            self._html_search_meta("uploadDate", webpage, "upload date")
        )
        duration = parse_duration(
            self._html_search_meta("duration", webpage, "duration")
        )
        filesize_approx = parse_filesize(
            self._html_search_meta("contentSize", webpage, "file size")
        )
        bitrate = int_or_none(
            self._html_search_meta("bitrate", webpage, "bitrate")
        )
