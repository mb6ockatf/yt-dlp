...
class NexxIE(InfoExtractor):
    ...
    def _handle_error(self, response):
        if traverse_obj(response, ("metadata", "notice"), expected_type=str):
            self.report_warning(
                "{} said: {}".format(
                    self.IE_NAME, response["metadata"]["notice"]
                )
            )
        status = int_or_none(
            try_get(response, lambda x: x["metadata"]["status"]) or 200
        )
        if 200 <= status < 300:
            return
        raise ExtractorError(
            "{} said: {}".format(
                self.IE_NAME, response["metadata"]["errorhint"]
            ),
            expected=True,
        )
    ...
    def _call_api(self, domain_id, path, video_id, data=None, headers={}):
        ...
        result = self._download_json(
            f"https://api.nexx.cloud/v3/{domain_id}/{path}",
            video_id,
            f"Downloading {path} JSON",
    ...
    def _extract_3q_formats(self, video, video_id):
        ...
        q_acc, q_prefix, q_locator, q_hash = (
            stream_data["qAccount"],
            stream_data["qPrefix"],
            stream_data["qLocator"],
            stream_data["qHash"],
        )
        ...
        def get_cdn_shield_base(shield_type=""):
            ...
            return (
                f'http://sdn-global-{"prog" if shield_type.lower() == "prog" else "streaming"}-cache.3qsdn.com/'
        ...
        azure_file_distribution = (
            stream_data.get("azureFileDistribution") or ""
        )
    ...
    def _extract_azure_formats(self, video, video_id):
        ...
        is_ml = "," in language
        azure_manifest_url = (
            "{}{}/{}_src{}.ism/Manifest".format(
                azure_stream_base,
                azure_locator,
                video_id,
                ("_manifest" if is_ml else ""),
            )
            + "%s"
        )
    ...
    def _real_extract(self, url):
        ...
        if not video:
            ...
            request_token = hashlib.md5(
                "".join((op, domain_id, secret)).encode()
            ).hexdigest()

            result = self._call_api(
                domain_id,
                f"videos/{op}/{video_id}",
                video_id,
                data={
        ...
        if cdn == "azure":
            ...
            self.raise_no_formats(
                f"{cdn} formats are currently not supported", video_id
            )
        ...
        return {
            ...
            "creator": general.get("studio")
            or general.get("studio_adref")
            or None,
            "thumbnail": try_get(
            ...
            "episode_number": traverse_obj(
                video,
                (("episodedata", "general"), "episode"),
                expected_type=int,
                get_all=False,
            ),
            "season_number": traverse_obj(
                video,
                (("episodedata", "general"), "season"),
                expected_type=int,
                get_all=False,
            ),
            "cast": traverse_obj(
                video, ("connectedmedia", ..., "title"), expected_type=str
            ),
            "formats": formats,
            "subtitles": subtitles,
        }
...
class NexxEmbedIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        return self.url_result(
            NexxIE._extract_url(webpage), ie=NexxIE.ie_key()
        )
