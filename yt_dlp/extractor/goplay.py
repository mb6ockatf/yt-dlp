...
class GoPlayIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(www\.)?goplay\.be/video/([^/?#]+/[^/?#]+/|)(?P<id>[^/#]+)"
    )
        ...
        if traverse_obj(meta, ("program", "subtype")) != "movie":
            for season_data in traverse_obj(
                ...
                episode_data = traverse_obj(
                    season_data,
                    ("videos", lambda _, v: v["videoId"] == video_id, any),
                )
                if not episode_data:
                    continue
...
class AwsIdp:
    ...
    def __init__(self, ie, pool_id, client_id):
        ...
        self.k = self.__hex_to_long(
            self.__hex_hash("00" + self.n_hex + "0" + self.g_hex)
        )
    ...
    def authenticate(self, username, password):
        ...
        auth_response_json = self.ie._download_json(
            ...
            note="Authenticating username",
            errnote="Invalid username",
        )
        ...
        auth_response_json = self.ie._download_json(
            ...
            note="Authenticating password",
            errnote="Invalid password",
        )
    ...
    def __get_hkdf_key_for_password(
        ...
        x_value = self.__hex_to_long(
            self.__hex_hash(self.__pad_hex(salt) + username_password_hash)
        )
    ...
    def __calculate_u(self, big_a, big_b):
        ...
        u_hex_hash = self.__hex_hash(
            self.__pad_hex(big_a) + self.__pad_hex(big_b)
        )
