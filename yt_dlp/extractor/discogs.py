...
class DiscogsReleasePlaylistIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?discogs\.com/(?P<type>release|master)/(?P<id>\d+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        response = self._download_json(
            f"https://api.discogs.com/{playlist_type}s/{playlist_id}",
            display_id,
        )

        entries = [
            self.url_result(
                video["uri"], YoutubeIE, video_title=video.get("title")
            )
            for video in traverse_obj(
                response, ("videos", lambda _, v: YoutubeIE.suitable(v["uri"]))
            )
        ]
