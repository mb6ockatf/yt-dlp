...
class YahooIE(InfoExtractor):
    ...
    def _extract_yahoo_video(self, video_id, country):
        ...
        for fmt in fmts:
            media_obj = self._download_json(
                ...
                + video_id,
                video_id,
                f"Downloading {fmt} JSON metadata",
    ...
    def _real_extract(self, url):
        ...
        items = self._download_json(
            f"https://{country}.yahoo.com/caas/content/article",
            display_id,
        ...
        if item.get("type") != "video":
            ...
            if cover.get("type") == "yvideo":
                ...
                if cover_url:
                    entries.append(
                        self.url_result(cover_url, "Yahoo", cover.get("uuid"))
                    )
            ...
            if items.get("markup"):
                entries.extend(
                    self.url_result(yt_url)
                    for yt_url in YoutubeIE._extract_embed_urls(
                        url, items["markup"]
                    )
                )
...
class YahooSearchIE(SearchInfoExtractor):
    ...
    def _search_results(self, query):
        for pagenum in itertools.count(0):
            ...
            yield from (
                self.url_result(result["rurl"]) for result in info["results"]
            )
    ...
    def _real_extract(self, url):
        ...
        if content_id is None:
            raise ExtractorError(
                "This article does not contain a video", expected=True
            )
        ...
        description = traverse_obj(
            preloaded_state, ("pageData", "description"), expected_type=str
        ) or self._html_search_meta(
        ...
        thumbnail = (
            traverse_obj(
                preloaded_state, ("pageData", "ogpImage"), expected_type=str
            )
