...
class Mx3BaseIE(InfoExtractor):
    ...
    def _extract_formats(self, track_id):
        ...
        for fmt in self._FORMATS:
            format_url = (
                f'https://{self._DOMAIN}/tracks/{track_id}/{fmt["url"]}'
            )
    ...
    def _real_extract(self, url):
        ...
        data = self._download_json(
            f"https://{self._DOMAIN}/t/{track_id}.json", track_id, fatal=False
        )
        ...
        return {
            ...
            "genre": self._html_search_regex(
                r'<div\b[^>]+class="single-band-genre"[^>]*>([^<]+)</div>',
                webpage,
                "genre",
                default=None,
            ),
            ...
            **traverse_obj(
                data,
                {
                    ...
                    "thumbnail": (
                        ("picture_url_xlarge", "picture_url"),
                        {url_or_none},
                    ),
                },
                get_all=False,
