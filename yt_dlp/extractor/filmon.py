    ...
    def _real_extract(self, url):
        ...
        try:
            response = self._download_json(
                f"https://www.filmon.com/api/vod/movie?id={video_id}", video_id
            )["response"]
            ...
            if isinstance(e.cause, HTTPError):
                ...
                raise ExtractorError(
                    f"{self.IE_NAME} said: {errmsg}", expected=True
                )
            raise
        ...
        if response.get("type_id") == 1:
            entries = [
                self.url_result("filmon:" + episode_id)
                for episode_id in response.get("episodes", [])
            ]
...
class FilmOnChannelIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        try:
            ...
            if isinstance(e.cause, HTTPError):
                errmsg = self._parse_json(
                    e.cause.response.read().decode(), channel_id
                )["message"]
                raise ExtractorError(
                    f"{self.IE_NAME} said: {errmsg}", expected=True
                )
        ...
        is_live = not channel_data.get("is_vod") and not channel_data.get(
            "is_vox"
        )
