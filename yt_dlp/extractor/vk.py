...
class VKBaseIE(InfoExtractor):
    ...
    def _download_webpage_handle(
        self, url_or_request, video_id, *args, fatal=True, **kwargs
    ):
    ...
    def _real_extract(self, url):
        ...
        if video_id:
            ...
            info_page = self._download_webpage(
                "https://vk.com/video_ext.php?" + mobj.group("embed_query"),
                video_id,
            )
            ...
            ERRORS = {
                r">Видеозапись .*? была изъята из публичного доступа в связи с обращением правообладателя.<": ERROR_COPYRIGHT,
                r">The video .*? was removed from public access by request of the copyright holder.<": ERROR_COPYRIGHT,
                r"<!>Please log in or <": "Video %s is only available for registered users, "
                "use --username and --password options to provide account credentials.",
                r"<!>Unknown error": "Video %s does not exist.",
                r"<!>Видео временно недоступно": "Video %s is temporarily unavailable.",
                r"<!>Access denied": "Access denied to video %s.",
                r"<!>Видеозапись недоступна, так как её автор был заблокирован.": "Video %s is no longer available, because its author has been blocked.",
                r"<!>This video is no longer available, because its author has been blocked.": "Video %s is no longer available, because its author has been blocked.",
                r"<!>This video is no longer available, because it has been deleted.": "Video %s is no longer available, because it has been deleted.",
                r"<!>The video .+? is not available in your region.": "Video %s is not available in your region.",
            }
        ...
        dailymotion_url = next(
            DailymotionIE._extract_embed_urls(url, info_page), None
        )
        if dailymotion_url:
        ...
        sibnet_url = next(
            SibnetEmbedIE._extract_embed_urls(url, info_page), None
        )
        if sibnet_url:
        ...
        for format_id, format_url in data.items():
            ...
            if format_id.startswith(("url", "cache")) or format_id in (
                ...
                and format_id != "hls_live_playback"
            ):
            ...
            elif format_id.startswith("dash") and format_id not in (
                "dash_live_playback",
                "dash_uni",
            ):
        ...
        return {
            ...
            **traverse_obj(
                data,
                {
                    ...
                    "chapters": (
                        "time_codes",
                        lambda _, v: isinstance(v["time"], int),
                        {
                            "title": ("text", {str}),
                            "start_time": "time",
                        },
...
class VKUserVideosIE(VKBaseIE):
    ...
    def _entries(self, page_id, section):
        video_list_json = self._download_payload(
            "al_video",
            page_id,
            {
        ...
        while True:
            ...
            if not new_count:
                self.to_screen(
                    f"{page_id}: Skipping {total - count} unavailable videos"
                )
                break
            count += new_count
            video_list = video_list_json["list"]
    ...
    def _real_extract(self, url):
        ...
        if u_id.startswith("@"):
            page_id = traverse_obj(
                self._search_json(
                    r"\bvar newCur\s*=", webpage, "cursor data", u_id
                ),
        ...
        playlist_title = clean_html(
            get_element_by_class("VideoInfoPanel__title", webpage)
        )
        return self.playlist_result(
            self._entries(page_id, section),
            f"{page_id}_{section}",
            playlist_title,
        )
...
class VKWallPostIE(VKBaseIE):
    ...
    def _real_extract(self, url):
        ...
        webpage = self._download_payload(
            "wkview",
            post_id,
            {
                "act": "show",
        ...
        uploader = clean_html(
            get_element_by_class("PostHeaderTitle__authorName", webpage)
        )
            ...
            entries.append(
                {
                    ...
                    "thumbnails": try_call(
                        lambda: [
                            {"url": u} for u in audio["coverUrl"].split(",")
                        ]
                    ),
        ...
        entries.extend(
            self.url_result(urljoin(url, entry), VKIE)
            for entry in set(
                re.findall(
                    r'<a[^>]+href=(?:["\'])(/video(?:-?[\d_]+)[^"\']*)',
        ...
        return self.playlist_result(
            entries,
            post_id,
            join_nonempty(uploader, f"Wall post {post_id}", delim=" - "),
    ...
    def _extract_formats(self, stream_info, video_id):
        ...
        for stream in traverse_obj(
            ...
            (
                ...
                0,
                "playerUrls",
                lambda _, v: url_or_none(v["url"]) and v["type"],
            ),
        ):
            url = stream["url"]
            format_id = str_or_none(stream["type"])
            if (
                format_id in ("hls", "live_hls", "live_playback_hls")
                or ".m3u8" in url
            ):
                formats.extend(
