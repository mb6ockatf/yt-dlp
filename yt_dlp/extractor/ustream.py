...
class UstreamIE(InfoExtractor):
    ...
    def _get_stream_info(self, url, video_id, app_id_ver, extra_note=None):
        ...
        conn_info = self._download_json(
            ...
            video_id,
            note="Downloading connection info" + extra_note,
            query={
                "type": "viewer",
                "appId": app_id_ver[0],
        ...
        return self._download_json(
            ...
            video_id,
            note="Downloading stream info" + extra_note,
        )
    ...
    def _get_streams(self, url, video_id, app_id_ver):
        ...
        for trial_count in range(3):
            stream_info = self._get_stream_info(
                ...
                app_id_ver,
                extra_note=(
                    f" (try {trial_count + 1})" if trial_count > 0 else ""
                ),
            )
            ...
            fragments = [
                {
                    "url": resolve_dash_template(
                        provider["url"] + stream["initUrl"],
                        0,
                        dash_stream_info["hashes"]["0"],
                    ),
                }
            ]
            for idx in range(
                ...
                fragments.append(
                    {
                        "url": resolve_dash_template(
                            provider["url"] + stream["segmentUrl"],
                            idx,
    ...
    def _real_extract(self, url):
        ...
        if m.group("type") == "embed":
            ...
            content_video_ids = self._parse_json(
                self._search_regex(
                    r"ustream\.vars\.offAirContentVideoIds=([^;]+);",
                    webpage,
                    "content video IDs",
                ),
                video_id,
            ...
            return self.playlist_result(
                (
                    self.url_result(
                        "http://www.ustream.tv/recorded/" + u, "Ustream"
                    )
                    for u in content_video_ids
                ),
                video_id,
            )
        ...
        thumbnails = [
            ...
            for thumbnail_id, thumbnail_url in video.get(
                "thumbnail", {}
            ).items()
        ]
    ...
    def _real_extract(self, url):
        ...
        while next_url:
            reply = self._download_json(
                urllib.parse.urljoin(BASE, next_url),
                display_id,
                note=f"Downloading video information (next: {len(video_ids) + 1})",
            )
            video_ids.extend(
                re.findall(r'data-content-id="(\d.*)"', reply["data"])
            )
