...
class LivestreamIE(InfoExtractor):
    ...
    def _parse_smil_formats_and_subtitles(
        ...
        base = (
            base_ele.get("content")
            if base_ele is not None
            else "http://livestreamvod-f.akamaihd.net/"
        )
        ...
        for vn in video_nodes:
            ...
            furl = update_url_query(
                urllib.parse.urljoin(base, vn.attrib["src"]),
                {
                    "v": "3.0.3",
                    "fp": "WIN% 14,0,0,145",
                },
    ...
    def _extract_video_info(self, video_data):
        ...
        if smil_url:
            formats.extend(
                self._extract_smil_formats(smil_url, video_id, fatal=False)
            )
    ...
    def _generate_event_playlist(self, event_data):
        ...
        feed_root_url = (
            self._API_URL_TEMPLATE % (account_id, event_id) + "/feed.json"
        )
        ...
        for i in itertools.count(1):
            if last_video is None:
                ...
                info_url = (
                    f"{feed_root_url}?&id={last_video}&newer=-1&type=video"
                )
            videos_info = self._download_json(
                info_url, event_id, f"Downloading page {i}"
            )["data"]
            videos_info = [
                v["data"] for v in videos_info if v["type"] == "video"
            ]
            if not videos_info:
                break
    ...
    def _real_extract(self, url):
        ...
        if video_id:
            ...
            return self.playlist_result(
                self._generate_event_playlist(event_data),
                str(event_data["id"]),
        ...
        return self.playlist_result(
            itertools.chain.from_iterable(
                map(self._generate_event_playlist, items)
            ),
            account_data.get("id"),
...
class LivestreamOriginalIE(InfoExtractor):
    ...
    def _extract_video_info(self, user, video_id):
        ...
        view_count = int_or_none(
            xpath_text(item, xpath_with_ns("ls:viewsCount", ls_ns))
        )
