...
class RoosterTeethBaseIE(InfoExtractor):
    ...
    def _extract_video_info(self, data):
        ...
        for image in traverse_obj(data, ("included", "images")):
            ...
            thumbnails.extend(
                [
                    ...
                    for name, url in (image.get("attributes") or {}).items()
                    if url_or_none(url)
                ]
            )
        ...
        if (
            video_id and "parent_content_id" in attributes
        ...
        return {
            ...
            "series": traverse_obj(
                attributes, "show_title", "parent_content_title"
            ),
            ...
            "release_timestamp": parse_iso8601(
                attributes.get("original_air_date")
            ),
            "thumbnails": thumbnails,
            "availability": self._availability(
                needs_premium=sub_only,
                needs_subscription=sub_only,
                needs_auth=sub_only,
                is_private=False,
                is_unlisted=False,
            ),
    ...
    def _real_extract(self, url):
        ...
        try:
            video_data = self._download_json(
                ...
                0
            ]  # web client-type yields ad-free streams
        except ExtractorError as e:
            if isinstance(e.cause, HTTPError) and e.cause.status == 403:
                if (
                    self._parse_json(
                        e.cause.response.read().decode(), display_id
                    ).get("access")
                    is False
                ):
        ...
        is_brightcove = (
            traverse_obj(video_data, ("attributes", "encoding_pipeline"))
            == "brightcove"
        )
        ...
        episode = self._download_json(
            api_episode_url, display_id, "Downloading episode JSON metadata"
        )["data"][0]
...
class RoosterTeethSeriesIE(RoosterTeethBaseIE):
    _VALID_URL = (
        r"https?://(?:.+?\.)?roosterteeth\.com/series/(?P<id>[^/?#&]+)"
    )
    _TESTS = [
        {
    ...
    def _entries(self, series_id, season_number):
        ...
        series_data = self._download_json(
            f"{self._API_BASE_URL}/shows/{series_id}/seasons?order=asc&order_by",
            display_id,
        )
        for season_data in traverse_obj(
            ...
            if season_number is not None and idx != season_number:
                continue
            yield from yield_episodes(
                self._download_json(
                    urljoin(self._API_BASE, season_data["links"]["episodes"]),
                    display_id,
                    f"Downloading season {idx} JSON metadata",
                    query={"per_page": 1000},
                )
            )
        ...
        if season_number is None:  # extract series-level bonus features
            yield from yield_episodes(
                self._download_json(
                    ...
                    display_id,
                    "Downloading bonus features JSON metadata",
                    fatal=False,
                )
            )
