...
class FourTubeBaseIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        media_id = self._search_regex(
            r'<button[^>]+data-id=(["\'])(?P<id>\d+)\1[^>]+data-quality=',
            webpage,
            "media id",
            default=None,
            group="id",
...
class PornTubeIE(FourTubeBaseIE):
    ...
    def _real_extract(self, url):
        ...
        sources = [
            str(e["height"]) for e in video["encodings"] if e.get("height")
        ]
        formats = self._extract_formats(url, video_id, media_id, sources)
        ...
        uploader_id = str_or_none(
            try_get(video, lambda x: x["user"]["id"], int)
        )
        channel = try_get(video, lambda x: x["channel"]["name"], str)
        channel_id = str_or_none(
            try_get(video, lambda x: x["channel"]["id"], int)
        )
