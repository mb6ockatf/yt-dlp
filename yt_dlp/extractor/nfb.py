...
class NFBBaseIE(InfoExtractor):
    ...
    def _extract_ep_info(self, data, video_id, slug=None):
        info = traverse_obj(
            data,
            (
                lambda _, v: video_id in v["embed_url"],
                {
        ...
        return {
            **info,
            "id": video_id,
            "title": join_nonempty(
                "series", "episode", from_dict=info, delim=" - "
            ),
...
class NFBIE(NFBBaseIE):
    ...
    def _real_extract(self, url):
        site, type_, slug = self._match_valid_url(url).group(
            "site", "type", "id"
        )
        # Need to construct the URL since we match /embed/player/ URLs as well
        webpage, urlh = self._download_webpage_handle(
            f"https://www.{site}.ca/{type_}/{slug}/", slug
        )
        ...
        player_data = self._search_json(
            r"window\.PLAYER_OPTIONS\[[^\]]+\]\s*=",
            webpage,
            "player data",
            slug,
        )
        video_id = self._match_id(
            player_data["overlay"]["url"]
        )  # overlay url always has unique slug
        ...
        info = (
            ...
            if type_ == "film"
            else self._extract_ep_info(
                self._extract_ep_data(webpage, video_id, slug), video_id
            )
        )
...
class NFBSeriesIE(NFBBaseIE):
    ...
    def _entries(self, episodes):
        for episode in traverse_obj(
            episodes, lambda _, v: NFBIE.suitable(v["embed_url"])
        ):
            mobj = NFBIE._match_valid_url(episode["embed_url"])
            yield self.url_result(
                mobj[0],
                NFBIE,
                **self._extract_ep_info([episode], mobj.group("id")),
            )

    def _real_extract(self, url):
        site, type_, series_id = self._match_valid_url(url).group(
            "site", "type", "id"
        )
        season_path = "saison" if type_ == "serie" else "season"
        webpage = self._download_webpage(
            f"https://www.{site}.ca/{type_}/{series_id}/{season_path}1/episode1",
            series_id,
        )
