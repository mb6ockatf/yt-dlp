...
class PRXBaseIE(InfoExtractor):
    ...
    def _extract_base_info(cls, response):
        ...
        description = clean_html(response.get("description")) or response.get(
            "shortDescription"
        )
        return {
            "id": item_id,
    ...
    def _extract_series_info(cls, series_response):
        ...
        if not base_info:
            return
        account_info = (
            cls._extract_account_info(
                cls._get_prx_embed_response(series_response, "account")
            )
            or {}
        )
        return {
    ...
    def _extract_account_info(cls, account_response):
        ...
        return {
            **base_info,
            ...
            "channel_url": "https://beta.prx.org/accounts/{}".format(
                base_info.get("id")
            ),
            "channel": name,
        }
    ...
    def _extract_story_info(cls, story_response):
        ...
        if not base_info:
            return
        series = (
            cls._extract_series_info(
                cls._get_prx_embed_response(story_response, "series")
            )
            or {}
        )
        account = (
            cls._extract_account_info(
                cls._get_prx_embed_response(story_response, "account")
            )
            or {}
        )
        return {
        ...
        return [
            {
                ...
                "asr": int_or_none(
                    piece_response.get("frequency"), scale=1000
                ),
