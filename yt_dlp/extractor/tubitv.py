...
class TubiTvIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        for resource in traverse_obj(
            video_data,
            (
                "video_resources",
                lambda _, v: url_or_none(v["manifest"]["url"]),
            ),
        ):
        ...
        elif not formats and not video_data.get(
            "policy_match"
        ):  # policy_match is False if content was removed
            raise ExtractorError(
        ...
        for sub in traverse_obj(
            video_data, ("subtitles", lambda _, v: url_or_none(v["url"]))
        ):
...
class TubiTvShowIE(InfoExtractor):
    ...
    def _entries(self, show_url, playlist_id, selected_season):
        ...
        path = (
            [lambda _, v: str(v["number"]) == selected_season]
            if selected_season
            else [..., {dict}]
        )

        for season in traverse_obj(
            data, ("byId", lambda _, v: v["type"] == "s", "seasons", *path)
        ):
            season_number = int_or_none(season.get("number"))
            for episode in traverse_obj(
                season, ("episodes", lambda _, v: v["id"])
            ):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._entries(url, playlist_id, selected_season), playlist_id
        )
