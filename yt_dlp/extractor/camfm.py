...
class CamFMShowIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "entries": [
                self.url_result(
                    urljoin("https://camfm.co.uk", i), CamFMEpisodeIE
                )
                for i in re.findall(
                    r"javascript:popup\('(/player/[^']+)', 'listen'", page
                )
            ],
            "thumbnail": urljoin(
                "https://camfm.co.uk",
            ...
            "title": self._html_search_regex(
                "<h1>([^<]+)</h1>", page, "title", fatal=False
            ),
            "description": clean_html(
                get_element_by_class("small-12 medium-8 cell", page)
            ),
        }
...
class CamFMEpisodeIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        date = self._html_search_regex(
            ">Aired at ([^<]+)<", card_section, "air date", fatal=False
        )
