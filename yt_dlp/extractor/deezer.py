...
class DeezerBaseInfoExtractor(InfoExtractor):
    def get_data(self, url):
        if not self.get_param("test"):
            self.report_warning(
                "For now, this extractor only supports the 30 second previews. Patches welcome!"
            )
...
class DeezerPlaylistIE(DeezerBaseInfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?deezer\.com/(../)?playlist/(?P<id>[0-9]+)"
    )
    _TEST = {
    ...
    def _real_extract(self, url):
        ...
        playlist_thumbnail = self._search_regex(
            r'<img id="naboo_playlist_image".*?src="([^"]+)"',
            webpage,
            "playlist thumbnail",
        )
...
class DeezerAlbumIE(DeezerBaseInfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        album_thumbnail = self._search_regex(
            r'<img id="naboo_album_image".*?src="([^"]+)"',
            webpage,
            "album thumbnail",
        )
