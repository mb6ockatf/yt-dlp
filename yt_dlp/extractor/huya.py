...
class HuyaLiveIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        stream_data = self._search_json(
            r"stream:\s", webpage, "stream", video_id=video_id, default=None
        )
        room_info = try_get(
            stream_data, lambda x: x["data"][0]["gameLiveInfo"]
        )
        if not room_info:
            raise ExtractorError(
                "Can not extract the room info", expected=True
            )
        title = (
            room_info.get("roomName")
            or room_info.get("introduction")
        ...
        for stream_info in stream_info_list:
            ...
            params = dict(
                urllib.parse.parse_qsl(
                    unescapeHTML(stream_info["sFlvAntiCode"])
                )
            )
            fm, ss = "", ""
            if re_secret:
                ...
                formats.append(
                    {
                        ...
                        "format_id": str_or_none(
                            stream_info.get("iLineIndex")
                        ),
                        "tbr": rate,
                        "url": update_url_query(
                            f'{stream_url}/{stream_name}.{stream_info.get("sFlvUrlSuffix")}',
                            query=params,
                        ),
...
class HuyaVideoIE(InfoExtractor):
    ...
    def _real_extract(self, url: str):
        ...
        for definition in traverse_obj(
            video_data, ("definitions", lambda _, v: url_or_none(v["url"]))
        ):
            formats.append(
                {
                    "url": definition["url"],
