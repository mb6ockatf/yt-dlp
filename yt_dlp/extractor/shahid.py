...
class ShahidIE(ShahidBaseIE):
    ...
    def _real_extract(self, url):
        ...
        playout = self._call_api("playout/new/url/" + video_id, video_id)[
            "playout"
        ]

        if not self.get_param("allow_unplayable_formats") and playout.get(
            "drm"
        ):
        ...
        categories = [
            category["name"]
            for category in video.get("genres", [])
            if "name" in category
        ]
...
class ShahidShowIE(ShahidBaseIE):
    ...
    def _real_extract(self, url):
        ...
        product = self._call_api(
            "playableAsset", show_id, {"showId": show_id}
        )["productModel"]
