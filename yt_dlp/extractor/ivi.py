...
class IviIE(InfoExtractor):
    ...
    _GEO_COUNTRIES = ["RU"]
    _LIGHT_KEY = (
        b"\xf1\x02\x32\xb7\xbc\x5c\x7a\xe8\xf7\x96\xc1\x33\x2b\x27\xa1\x8c"
    )
    ...
    def _real_extract(self, url):
        ...
        title = (
            f"{compilation} - {title}" if compilation is not None else title
        )
...
class IviCompilationIE(InfoExtractor):
    ...
    def _extract_entries(self, html, compilation_id):
        return [
            self.url_result(
                f"http://www.ivi.ru/watch/{compilation_id}/{serie}",
                IviIE.ie_key(),
            )
            for serie in re.findall(
                rf'<a\b[^>]+\bhref=["\']/watch/{compilation_id}/(\d+)["\']',
                html,
            )
        ]
            ...
            if not seasons:  # No seasons in this compilation
                ...
                for season_id in seasons:
                    season_page = self._download_webpage(
                        ...
                        compilation_id,
                        f"Downloading season {season_id} web page",
                    )
                    entries.extend(
                        self._extract_entries(season_page, compilation_id)
                    )
