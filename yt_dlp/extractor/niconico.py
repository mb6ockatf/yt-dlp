...
class NiconicoIE(InfoExtractor):
    ...
    def _perform_login(self, username, password):
        ...
        page = self._download_webpage(
            "https://account.nicovideo.jp/login/redirector?show_button_twitter=1&site=niconico&show_button_facebook=1",
            None,
            note="Logging in",
        ...
        if "oneTimePw" in page:
            ...
            page = self._download_webpage(
                urljoin("https://account.nicovideo.jp", post_url),
                None,
                note="Performing MFA",
            ...
            if "oneTimePw" in page or "formError" in page:
                ...
                self.report_warning(
                    f'Unable to log in: MFA challenge failed, "{err_msg}"'
                )
                return False
    ...
    def _get_heartbeat_info(self, info_dict):
        video_id, video_src_id, audio_src_id = (
            info_dict["url"].split(":")[1].split("/")
        )
        dmc_protocol = info_dict["expected_protocol"]

        api_data = info_dict.get("_api_data") or self._parse_json(
        ...
        def ping():
            tracking_id = traverse_obj(
                api_data, ("media", "delivery", "trackingId")
            )
            if tracking_id:
                tracking_url = update_url_query(
                ...
                if (
                    traverse_obj(watch_request_response, ("meta", "status"))
                    != 200
                ):
                    self.report_warning(
                        "Failed to acquire permission for playing video. Video download may fail."
                    )
        ...
        if dmc_protocol == "http":
            ...
            protocol_parameters = {
                "http_output_download_parameters": {
                    ...
                    "use_well_known_port": yesno(
                        session_api_data["urls"][0]["isWellKnownPort"]
                    ),
                },
            }
            ...
            segment_duration = (
                try_get(
                    self._configuration_arg("segment_duration"),
                    lambda x: int(x[0]),
                )
                or 6000
            )
            parsed_token = self._parse_json(
                session_api_data["token"], video_id
            )
            encryption = traverse_obj(
                api_data, ("media", "delivery", "encryption")
            )
            protocol_parameters = {
                "hls_parameters": {
        ...
        session_response = self._download_json(
            ...
            note="Downloading JSON metadata for {}".format(
                info_dict["format_id"]
            ),
            data=json.dumps(
                {
                    "session": {
                        ...
                        "content_auth": {
                            "auth_type": try_get(
                                session_api_data,
                                lambda x: x["authTypes"][
                                    session_api_data["protocols"][0]
                                ],
                            ),
                            "content_key_timeout": session_api_data.get(
                                "contentKeyTimeout"
                            ),
                            "service_id": "nicovideo",
                            "service_user_id": session_api_data.get(
                                "serviceUserId"
                            ),
                        },
        ...
        heartbeat_info_dict = {
            ...
            + "/"
            + session_response["data"]["session"]["id"]
            + "?_format=json&_method=PUT",
            "data": json.dumps(session_response["data"]),
            # interval, convert milliseconds to seconds, then halve to make a buffer.
            "interval": float_or_none(
                session_api_data.get("heartbeatLifetime"), scale=3000
            ),
            "ping": ping,
        }
    ...
    def _extract_format_for_quality(
        ...
        if not audio_quality.get("isAvailable") or not video_quality.get(
            "isAvailable"
        ):
            return None
        ...
        format_id = "-".join(
            [
                remove_start(s["id"], "archive_")
                for s in (video_quality, audio_quality)
            ]
            + [dmc_protocol]
        )
        ...
        return {
            "url": "niconico_dmc:{}/{}/{}".format(
                video_id, video_quality["id"], audio_quality["id"]
            ),
            "format_id": format_id,
            "format_note": join_nonempty(
                "DMC", vid_qual_label, dmc_protocol.upper(), delim=" "
            ),
    ...
    def _yield_dmc_formats(self, api_data, video_id):
        ...
        protocols = traverse_obj(
            dmc_data, ("session", "protocols", ..., {str})
        )
        if not all((audios, videos, protocols)):
            return

        for audio_quality, video_quality, protocol in itertools.product(
            audios, videos, protocols
        ):
            if fmt := self._extract_format_for_quality(
                video_id, audio_quality, video_quality, protocol
            ):
                yield fmt
    ...
    def _yield_dms_formats(self, api_data, video_id):
        ...
        videos = traverse_obj(
            api_data, ("media", "domand", "videos", fmt_filter)
        )
        audios = traverse_obj(
            api_data, ("media", "domand", "audios", fmt_filter)
        )
        access_key = traverse_obj(
            api_data, ("media", "domand", "accessRightKey", {str})
        )
        ...
        for audio_fmt in traverse_obj(
            dms_fmts, lambda _, v: v["vcodec"] == "none"
        ):
            yield {
                **audio_fmt,
                **traverse_obj(
                    audios,
                    (
                        lambda _, v: audio_fmt["format_id"].startswith(
                            v["id"]
                        ),
                        {
                            "format_id": ("id", {str}),
        ...
        min_abr = (
            min(
                traverse_obj(audios, (..., "bitRate", {float_or_none})),
                default=0,
            )
            / 1000
        )
        for video_fmt in video_fmts:
            ...
            video_fmt["format_id"] = url_basename(video_fmt["url"]).rpartition(
                "."
            )[0]
            video_fmt["quality"] = (
                traverse_obj(
                    videos,
                    (
                        lambda _, v: v["id"] == video_fmt["format_id"],
                        "qualityLevel",
                        {int_or_none},
                        any,
                    ),
    ...
    def _real_extract(self, url):
        ...
        try:
            ...
            api_data = traverse_obj(
                self._parse_json(
                    self._html_search_meta("server-response", webpage) or "",
                    video_id,
                ),
                ("data", "response", {dict}),
            )
            ...
            try:
                api_data = self._download_json(
                    f"https://www.nicovideo.jp/api/watch/v3/{video_id}?_frontendId=6&_frontendVersion=0&actionTrackId=AAAAAAAAAA_{round(time.time() * 1000)}",
                    video_id,
        ...
        if not formats:
            ...
            if (
                fail_msg
                and "された地域と同じ地域からのみ視聴できます。" in fail_msg
            ):
                availability = None
                ...
                self.raise_login_required(
                    "This video requires premium", metadata_available=True
                )
            elif availability == "subscriber_only":
                self.raise_login_required(
                    "This video is for members only", metadata_available=True
                )
        ...
        if webpage:
            ...
            og_video_tags = re.finditer(
                r'<meta\s+property="og:video:tag"\s*content="(.*?)">', webpage
            )
            tags = list(
                filter(None, (clean_html(x.group(1)) for x in og_video_tags))
            )
            if not tags:
                # use keywords and split with comma (not logged in)
                kwds = self._html_search_meta(
                    "keywords", webpage, default=None
                )
                if kwds:
        ...
        thumb_prefs = qualities(
            ["url", "middleUrl", "largeUrl", "player", "ogp"]
        )

        def get_video_info(*items, get_first=True, **kwargs):
            return traverse_obj(
                api_data, ("video", *items), get_all=not get_first, **kwargs
            )

        return {
            "id": video_id,
            "_api_data": api_data,
            "title": get_video_info(("originalTitle", "title"))
            ...
            "thumbnails": [
                ...
                for key, url in (get_video_info("thumbnail") or {}).items()
                if url
            ],
            ...
            "channel": traverse_obj(
                api_data, ("channel", "name"), ("community", "name")
            ),
            "channel_id": traverse_obj(
                api_data, ("channel", "id"), ("community", "id")
            ),
            "view_count": int_or_none(get_video_info("count", "view")),
            "tags": tags,
            "genre": traverse_obj(
                api_data, ("genre", "label"), ("genre", "key")
            ),
            "comment_count": get_video_info(
                "count", "comment", expected_type=int
            ),
            "duration": (
                parse_duration(
                    self._html_search_meta(
            ...
            "webpage_url": url_or_none(url)
            or f"https://www.nicovideo.jp/watch/{video_id}",
            "subtitles": self.extract_subtitles(video_id, api_data),
        }

    def _get_subtitles(self, video_id, api_data):
        comments_info = (
            traverse_obj(api_data, ("comment", "nvComment", {dict})) or {}
        )
        ...
        danmaku = traverse_obj(
            self._download_json(
                f'{comments_info["server"]}/v1/threads',
                video_id,
                data=json.dumps(
                    {
                        "additionals": {},
...
class NiconicoPlaylistBaseIE(InfoExtractor):
    ...
    def _parse_owner(item):
        return {
            "uploader": traverse_obj(
                item, ("owner", ("name", ("user", "nickname")), {str}, any)
            ),
    ...
    def _entries(self, list_id):
        return OnDemandPagedList(
            functools.partial(self._fetch_page, list_id), self._PAGE_SIZE
        )
...
class NiconicoPlaylistIE(NiconicoPlaylistBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._entries(list_id),
            list_id,
            mylist.get("name"),
            mylist.get("description"),
...
class NiconicoSeriesIE(NiconicoPlaylistBaseIE):
    ...
    def _call_api(self, list_id, resource, query):
        return self._download_json(
            f"https://nvapi.nicovideo.jp/v2/series/{list_id}",
            list_id,
            f"Downloading {resource}",
            query=query,
            headers=self._API_HEADERS,
...
class NiconicoHistoryIE(NiconicoPlaylistBaseIE):
    ...
    _VALID_URL = (
        r"https?://(?:www\.|sp\.)?nicovideo\.jp/my/(?P<id>history(?:/like)?)"
    )
    ...
    def _call_api(self, list_id, resource, query):
        ...
        return self._download_json(
            f"https://nvapi.nicovideo.jp/v1/users/me/{path}",
            list_id,
            f"Downloading {resource}",
            query=query,
            headers=self._API_HEADERS,
    ...
    def _real_extract(self, url):
        ...
        try:
            ...
            if isinstance(e.cause, HTTPError) and e.cause.status == 401:
                self.raise_login_required(
                    "You have to be logged in to get your history"
                )
            raise
        return self.playlist_result(
            self._entries(list_id), list_id, **self._parse_owner(mylist)
        )
...
class NicovideoSearchBaseIE(InfoExtractor):
    ...
    def _entries(
        self, url, item_id, query=None, note="Downloading page %(page)s"
    ):
        query = query or {}
        ...
        for page_num in pages:
            ...
            for item in results:
                yield self.url_result(
                    f"https://www.nicovideo.jp/watch/{item}", "Niconico", item
                )
            if not results:
                break
    ...
    def _search_results(self, query):
        return self._entries(
            self._proto_relative_url(
                f"//www.nicovideo.jp/{self._SEARCH_TYPE}/{query}"
            ),
            query,
        )
...
class NicovideoSearchDateIE(NicovideoSearchBaseIE, SearchInfoExtractor):
    ...
    def _entries(self, url, item_id, start_date=None, end_date=None):
        start_date, end_date = (
            ...
            end_date or dt.datetime.now().date(),
        )
        ...
        if last_page_len == self._RESULTS_PER_PAGE and start_date != end_date:
            ...
            self.to_screen(
                f"{item_id}: Downloading results from {start_date} to {end_date}"
            )
    ...
    def _get_entries_for_date(
        self, url, item_id, start_date, end_date=None, page_num=None, note=None
    ):
        query = {
    ...
    def _entries(self, list_id):
        ...
        while count < total_count:
            json_parsed = self._download_json(
                self._API_URL % (list_id, self._PAGE_SIZE, page_num + 1),
                list_id,
                headers=self._API_HEADERS,
                note="Downloading JSON metadata%s"
                % (f" page {page_num}" if page_num else ""),
            )
            if not page_num:
            ...
            for entry in json_parsed["data"]["items"]:
                count += 1
                yield self.url_result(
                    f'https://www.nicovideo.jp/watch/{entry["essential"]["id"]}',
                    ie=NiconicoIE,
                )
            page_num += 1
...
class NiconicoLiveIE(InfoExtractor):
    ...
    IE_DESC = "ニコニコ生放送"
    _VALID_URL = (
        r"https?://(?:sp\.)?live2?\.nicovideo\.jp/(?:watch|gate)/(?P<id>lv\d+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        webpage, urlh = self._download_webpage_handle(
            f"https://live.nicovideo.jp/watch/{video_id}", video_id
        )

        embedded_data = self._parse_json(
            unescapeHTML(
                self._search_regex(
                    r'<script\s+id="embedded-data"\s*data-props="(.+?)"',
                    webpage,
                    "embedded data",
                )
            ),
        ...
        ws_url = traverse_obj(
            embedded_data, ("site", "relive", "webSocketUrl")
        )
        if not ws_url:
            raise ExtractorError(
                "The live hasn't started yet or already ended.", expected=True
            )
        ws_url = update_url_query(
            ws_url,
            {
                "frontend_id": traverse_obj(
                    embedded_data, ("site", "frontendId")
                )
                or "9",
            },
        )

        hostname = remove_start(
            urllib.parse.urlparse(urlh.url).hostname, "sp."
        )
        ...
        ws = self._request_webpage(
            ...
            video_id=video_id,
            note="Connecting to WebSocket server",
        )
        ...
        title = traverse_obj(
            embedded_data, ("program", "title")
        ) or self._html_search_meta(
            ("og:title", "twitter:title"), webpage, "live title", fatal=False
        )

        raw_thumbs = (
            traverse_obj(embedded_data, ("program", "thumbnail")) or {}
        )
        thumbnails = []
        for name, value in raw_thumbs.items():
            ...
            for k, img_url in value.items():
                res = parse_resolution(k, lenient=True) or parse_resolution(
                    img_url, lenient=True
                )
        ...
        return {
            ...
            **traverse_obj(
            ...
            "description": clean_html(
                traverse_obj(embedded_data, ("program", "description"))
            ),
            "timestamp": int_or_none(
                traverse_obj(embedded_data, ("program", "openTime"))
            ),
            "is_live": True,
