...
class QDanceIE(InfoExtractor):
    ...
    def _call_login_api(self, data, note="Logging in"):
        login = self._download_json(
            "https://members.id-t.com/api/auth/login",
            None,
            note,
            headers={
        ...
        if not tokens.get("_id-t-accounts-token"):
            error = ": ".join(
                traverse_obj(login, ("error", ("code", "message"), {str}))
            )
    ...
    def _real_initialize(self):
        ...
        self._refresh_token = try_call(
            lambda: cookies["_id-t-accounts-refresh"].value
        )
        self._access_token = try_call(
            lambda: cookies["_id-t-accounts-token"].value
        )
    ...
    def _get_auth(self):
        if (
            try_call(lambda: jwt_decode_hs256(self._access_token)["exp"]) or 0
        ) <= int(time.time() - 120):
    ...
    def _real_extract(self, url):
        ...
        data = self._search_nuxt_data(
            webpage, video_id, traverse=("data", 0, "data")
        )

        def extract_availability(level):
            level = int_or_none(level) or 0
            return self._availability(
                needs_premium=(level >= 20),
                needs_subscription=(level >= 15),
                needs_auth=True,
            )
        ...
        if (
            not m3u8_url
            and traverse_obj(stream, ("error", "code")) == "unauthorized"
        ):
            raise ExtractorError(
        ...
        formats = (
            self._extract_m3u8_formats(
                m3u8_url, video_id, fatal=False, live=True
            )
            if m3u8_url
            else []
        )
        if not formats:
            self.raise_no_formats(
                "No active streams found", expected=bool(info.get("is_live"))
            )
