    ...
    def _extract_from_id(self, video_id, webpage=None):
        ...
        if error:
            raise ExtractorError(
                "{} said: {}".format(self.IE_NAME, error["message"]),
                expected=True,
            )
                    ...
                    if format_id.startswith("ism"):
                        ...
                        if format_type == "audio":
                            ...
                            http_f.update(
                                {
                                    "height": int_or_none(
                                        f.get("vertical_resolution")
                                    ),
                                    "width": int_or_none(
                                        f.get("horizontal_resolution")
                                    ),
                                    "vbr": float_or_none(
                                        f.get("video_bitrate")
                                    ),
        ...
        title = (
            self._og_search_title(webpage, default=None) if webpage else None
        ) or meta["title"]
        description = (
...
class OnetChannelIE(OnetBaseIE):
    ...
    def _real_extract(self, url):
        ...
        current_clip_info = self._parse_json(
            self._search_regex(
                r"var\s+currentClip\s*=\s*({[^}]+})", webpage, "video info"
            ),
            channel_id,
        ...
        if not self._yes_playlist(
            channel_id, video_name, playlist_label="channel"
        ):
        ...
        channel_title = strip_or_none(
            get_element_by_class("o_channelName", webpage)
        )
