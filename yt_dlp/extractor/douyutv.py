...
class DouyuBaseIE(InfoExtractor):
    ...
    def _get_cryptojs_md5(self, video_id):
        return self.cache.load(
            "douyu", "crypto-js-md5", min_ver="2024.07.04"
        ) or self._download_cryptojs_md5(video_id)
    ...
    def _calc_sign(self, sign_func, video_id, a):
        ...
        result = phantom.execute(
            js_script, video_id, note="Executing JS signing script"
        ).strip()
...
class DouyuTVIE(DouyuBaseIE):
    ...
    def _extract_stream_formats(self, stream_formats):
        ...
        for stream_info in traverse_obj(stream_formats, (..., "data")):
            stream_url = urljoin(
                traverse_obj(stream_info, "rtmp_url"),
                traverse_obj(stream_info, "rtmp_live"),
            )
            if stream_url:
                rate_id = traverse_obj(stream_info, ("rate", {int_or_none}))
                rate_info = traverse_obj(
                    stream_info,
                    ("multirates", lambda _, v: v["rate"] == rate_id),
                    get_all=False,
                )
                ...
                formats.append(
                    {
                        ...
                        "protocol": (
                            "m3u8_native" if ext == "m3u8" else "https"
                        ),
                        "quality": (
                            rate_id % -10000 if rate_id is not None else None
                        ),
                        **traverse_obj(
                            rate_info,
                            {
    ...
    def _real_extract(self, url):
        ...
        if (
            self._search_regex(
                r'"videoLoop"\s*:\s*(\d+)', webpage, "loop", default=""
            )
            == "1"
        ):
            raise UserNotLive(
                "The channel is auto-playing VODs", video_id=video_id
            )
        if (
            self._search_regex(
        ...
        params["auth"] = hashlib.md5(
            f"room/{room_id}?{urllib.parse.urlencode(params)}zNzMV1y4EMxOHS6I5WKm".encode()
        ).hexdigest()
        room = traverse_obj(
            self._download_json(
                f"http://www.douyutv.com/api/v1/room/{room_id}",
                video_id,
                note="Downloading room info",
                query=params,
                fatal=False,
            ),
            "data",
        ...
        stream_formats = [
            self._download_json(
                ...
                video_id,
                note="Downloading livestream format",
            ...
            if rate_id != traverse_obj(stream_formats[0], ("data", "rate")):
                ...
                stream_formats.append(
                    self._download_json(
                        f"https://www.douyu.com/lapi/live/getH5Play/{room_id}",
                        video_id,
                        note=f"Downloading livestream format {rate_id}",
                        data=urlencode_postdata(form_data),
                    )
                )
...
class DouyuShowIE(DouyuBaseIE):
    ...
    def _real_extract(self, url):
        ...
        form_data = {
            "vid": video_id,
            **self._calc_sign(
                js_sign_func, video_id, video_info["ROOM"]["point_id"]
            ),
        }
