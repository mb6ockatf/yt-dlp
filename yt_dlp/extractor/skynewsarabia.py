...
class SkyNewsArabiaBaseIE(InfoExtractor):
    ...
    def _call_api(self, path, value):
        return self._download_json(
            f"http://api.skynewsarabia.com/web/rest/v2/{path}/{value}.json",
            value,
        )

    def _get_limelight_media_id(self, url):
        return self._search_regex(
            r"/media/[^/]+/([a-z0-9]{32})", url, "limelight media id"
        )

    def _get_image_url(self, image_path_template, width="1600", height="1200"):
        return self._IMAGE_BASE_URL + image_path_template.format(
    ...
    def _extract_video_info(self, video_data):
        ...
        return {
            ...
            "url": "limelight:media:{}".format(
                self._get_limelight_media_id(video_data["videoUrl"][0]["url"])
            ),
            "id": video_id,
...
class SkyNewsArabiaIE(SkyNewsArabiaBaseIE):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?skynewsarabia\.com/web/video/(?P<id>[0-9]+)"
    )
    _TEST = {
...
class SkyNewsArabiaArticleIE(SkyNewsArabiaBaseIE):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?skynewsarabia\.com/web/article/(?P<id>[0-9]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        entries = [
            self._extract_video_info(item)
            for item in article_data.get("inlineItems", [])
            if item["type"] == "VIDEO"
        ]
