...
class RayWenderlichIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        ) or self._html_search_meta("twitter:image", webpage, "thumbnail")

        if ">Subscribe to unlock" in webpage:
            raise ExtractorError(
                "This content is only available for subscribers", expected=True
            )
        ...
        if not vimeo_id:
            ...
            video_id = self._extract_video_id(
                data, lesson_id
            ) or self._search_regex(r"/videos/(\d+)/", thumbnail, "video id")
            headers = {
                "Referer": url,
...
class RayWenderlichCourseIE(InfoExtractor):
    ...
    def suitable(cls, url):
        return (
            False if RayWenderlichIE.suitable(url) else super().suitable(url)
        )
