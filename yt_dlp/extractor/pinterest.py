...
class PinterestBaseIE(InfoExtractor):
    ...
    def _extract_video(self, data, extract_formats=True):
        ...
        info = {
            "title": strip_or_none(
                traverse_obj(data, "title", "grid_title", default="")
            ),
            ...
            "uploader_id": str_or_none(
                traverse_obj(data, ("closeup_attribution", "id"))
            ),
            "repost_count": int_or_none(data.get("repin_count")),
            "comment_count": int_or_none(data.get("comment_count")),
            "categories": traverse_obj(
                data, ("pin_join", "visual_annotation"), expected_type=list
            ),
        ...
        if domain.lower() != "uploaded by user" and traverse_obj(
            data, ("embed", "src")
        ):
            if not info["title"]:
                info["title"] = None
...
class PinterestIE(PinterestBaseIE):
    _VALID_URL = (
        rf"{PinterestBaseIE._VALID_URL_BASE}/pin/(?:[\w-]+--)?(?P<id>\d+)"
    )
    _TESTS = [
        {
