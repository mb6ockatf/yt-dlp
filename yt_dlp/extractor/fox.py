...
class FOXIE(InfoExtractor):
                ...
                for e in entitlement_issues:
                    if e.get("errorCode") == 1005:
                        raise ExtractorError(
                            ...
                            "subscription. You may want to use --cookies.",
                            expected=True,
                        )
                messages = ", ".join(
                    [e["message"] for e in entitlement_issues]
                )
    ...
    def _real_initialize(self):
        if not self._access_token:
            ...
            if mvpd_auth:
                self._access_token = (
                    self._parse_json(
                        urllib.parse.unquote(mvpd_auth.value),
                        None,
                        fatal=False,
                    )
                    or {}
    ...
    def _real_extract(self, url):
        ...
        data = (
            try_get(video, lambda x: x["trackingData"]["properties"], dict)
            or {}
        )

        duration = (
            int_or_none(video.get("durationInSeconds"))
            or int_or_none(video.get("duration"))
            or parse_duration(video.get("duration"))
        )
        timestamp = unified_timestamp(video.get("datePublished"))
        creator = (
            data.get("brand") or data.get("network") or video.get("network")
        )
        series = (
            video.get("seriesName")
        ...
        return {
            ...
            "thumbnail": traverse_obj(
                video, ("images", "still", "raw"), expected_type=url_or_none
            ),
