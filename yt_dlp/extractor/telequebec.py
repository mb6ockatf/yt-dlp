...
class TeleQuebecBaseIE(InfoExtractor):
    ...
    def _brightcove_result(
        brightcove_id, player_id, account_id="6150020952001"
    ):
        return {
...
class TeleQuebecIE(TeleQuebecBaseIE):
    ...
    def _real_extract(self, url):
        ...
        info.update(
            {
                "description": try_get(
                    media, lambda x: x["descriptions"][-1]["text"], str
                ),
...
class TeleQuebecSquatIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        video = self._download_json(
            f"https://squat.api.telequebec.tv/v1/videos/{video_id}", video_id
        )
...
class TeleQuebecVideoIE(TeleQuebecBaseIE):
    ...
    def _real_extract(self, url):
        ...
        account_id = (
            try_get(
                stream, lambda x: x["video_provider_details"]["account_id"]
            )
            or "6101674910001"
        )
