...
class LoomIE(InfoExtractor):
    ...
    def _call_graphql_api(self, operations, video_id, note=None, errnote=None):
        ...
        return self._download_json(
            ...
            video_id,
            note or "Downloading GraphQL JSON",
            errnote or "Failed to download GraphQL JSON",
            headers={
            ...
            data=json.dumps(
                [
                    ...
                    for operation_name in variadic(operations)
                ],
                separators=(",", ":"),
            ).encode(),
        )
    ...
    def _call_url_api(self, endpoint, video_id):
        response = self._download_json(
            f"https://www.loom.com/api/campaigns/sessions/{video_id}/{endpoint}",
            video_id,
            f"Downloading {endpoint} JSON",
            f"Failed to download {endpoint} JSON",
            fatal=False,
            headers={
                "Accept": "application/json",
                "Content-Type": "application/json",
    ...
    def _extract_formats(self, video_id, metadata, gql_data):
        ...
        formats.extend(
            get_formats(raw_url, "raw", quality=1)
        )  # original quality

        transcoded_url = self._call_url_api("transcoded-url", video_id)
        formats.extend(
            get_formats(transcoded_url, "transcoded", quality=-1)
        )  # transcoded quality
        ...
        valid_urls = [
            update_url(url, query=None)
            for url in (raw_url, transcoded_url)
            if url
        ]
        if cdn_url and update_url(cdn_url, query=None) not in valid_urls:
            formats.extend(
                get_formats(cdn_url, "cdn", quality=0)
    ...
    def _real_extract(self, url):
        ...
        gql_data = self._call_graphql_api(
            ["FetchChapters", "FetchVideoTranscript", "GetVideoSource"],
            video_id,
        )
        ...
        return {
            ...
            "chapters": self._extract_chapters_from_description(
                get_first(
                    gql_data, ("data", "fetchVideoChapters", "content", {str})
                ),
                duration,
            )
            or None,
...
class LoomFolderIE(InfoExtractor):
    IE_NAME = "loom:folder"
    _VALID_URL = (
        r"https?://(?:www\.)?loom\.com/share/folder/(?P<id>[\da-f]{32})"
    )
    _TESTS = [
        {
    ...
    def _extract_folder_data(self, folder_id):
        return self._download_json(
            f"https://www.loom.com/v1/folders/{folder_id}",
            folder_id,
            "Downloading folder info JSON",
            query={"limit": "10000"},
        )
    ...
    def _extract_folder_entries(self, folder_id, initial_folder_data=None):
        ...
        for video in traverse_obj(
            ...
            yield self.url_result(
                f"https://www.loom.com/share/{video_id}",
                LoomIE,
                video_id,
                video.get("name"),
            )
        ...
        for subfolder_id in traverse_obj(
            folder_data,
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._extract_folder_entries(playlist_id, playlist_data),
            playlist_id,
