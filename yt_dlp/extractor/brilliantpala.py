...
class BrilliantpalaBaseIE(InfoExtractor):
    ...
    def _initialize_pre_login(self):
        ...
        self._CONTENT_API = (
            f"{self._HOMEPAGE}/api/v2.4/contents/{{content_id}}/"
        )
        self._HLS_AES_URI = (
            f"{self._HOMEPAGE}/api/v2.5/video_contents/{{content_id}}/key/"
        )
    ...
    def _perform_login(self, username, password):
        ...
        self._set_cookie(
            self._DOMAIN, "csrftoken", login_form["csrfmiddlewaretoken"]
        )
    ...
    def _real_extract(self, url):
        course_id, content_id = self._match_valid_url(url).group(
        ...
        content_json = self._download_json(
            self._CONTENT_API.format(content_id=content_id),
            video_id,
        ...
        for stream in traverse_obj(
            content_json,
            ("video", "streams", lambda _, v: v["id"] and v["url"]),
        ):
