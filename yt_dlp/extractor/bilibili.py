...
class BilibiliBaseIE(InfoExtractor):
    ...
    _WBI_KEY_CACHE_TIMEOUT = (
        30  # exact expire timeout is unclear, use 30s for one session
    )
    _wbi_key_cache = {}
    ...
    def is_logged_in(self):
        return bool(
            self._get_cookies("https://api.bilibili.com").get("SESSDATA")
        )
        ...
        formats = [
            {
                ...
                "ext": mimetype2ext(
                    traverse_obj(audio, "mimeType", "mime_type")
                ),
        ...
        formats.extend(
            {
                ...
                "ext": mimetype2ext(
                    traverse_obj(video, "mimeType", "mime_type")
                ),
                "fps": float_or_none(
                    traverse_obj(video, "frameRate", "frame_rate")
                ),
                "width": int_or_none(video.get("width")),
                "height": int_or_none(video.get("height")),
                "vcodec": video.get("codecs"),
                "acodec": "none" if audios else None,
                "dynamic_range": {126: "DV", 125: "HDR10"}.get(
                    int_or_none(video.get("id"))
                ),
                ...
                "format_id": traverse_obj(
                    video,
                    (("baseUrl", "base_url"), {self._FORMAT_ID_RE.search}, 1),
                    ("id", {str_or_none}),
                    get_all=False,
                ),
            ...
            formats.append(
                {
                    "url": fragments[0]["url"],
                    "filesize": sum(
                        traverse_obj(fragments, (..., "filesize"))
                    ),
                    **(
                        {
                            "fragments": fragments,
                    ...
                    **parse_resolution(
                        format_names.get(play_info.get("quality"))
                    ),
                }
            )
        return formats
    ...
    def _get_wbi_key(self, video_id):
        if (
            time.time()
            < self._wbi_key_cache.get("ts", 0) + self._WBI_KEY_CACHE_TIMEOUT
        ):
            ...
            subtitles.setdefault(s["lan"], []).append(
                {
                    "ext": "srt",
                    "data": self.json2srt(
                        self._download_json(s["subtitle_url"], video_id)
                    ),
                }
            )
        return subtitles
    ...
    def _get_chapters(self, aid, cid):
        chapters = (
            aid
            and cid
            and self._download_json(
                "https://api.bilibili.com/x/player/wbi/v2",
                aid,
                query={"aid": aid, "cid": cid},
                note="Extracting chapters",
                fatal=False,
                headers=self._HEADERS,
            )
        )
        ...
        for idx in itertools.count(1):
            replies = traverse_obj(
                self._download_json(
                    ...
                    aid,
                    note=f"Extracting comments from page {idx}",
                    fatal=False,
                ),
                ("data", "replies"),
            )
    ...
    def _get_all_children(self, reply):
        ...
        for children in map(
            self._get_all_children, traverse_obj(reply, ("replies", ...))
        ):
            yield from children
    ...
    def _get_episodes_from_season(self, ss_id, url):
        ...
        for entry in traverse_obj(
            ...
            yield self.url_result(
                entry["share_url"],
                BiliBiliBangumiIE,
                str_or_none(entry.get("id")),
            )

    def _get_divisions(
        self, video_id, graph_version, edges, edge_id, cid_edges=None
    ):
        ...
        cid_edges.setdefault(edges[edge_id]["cid"], {})[edge_id] = edges[
            edge_id
        ]
            ...
            yield {
                **metainfo,
                ...
                "duration": float_or_none(
                    play_info.get("timelength"), scale=1000
                ),
...
class BiliBiliIE(BilibiliBaseIE):
    ...
    _TESTS = [
        ...
        },
        {
            ...
            "params": {
                "extractor_args": {"bilibili": {"prefer_multi_flv": ["32"]}}
            },
            "playlist_count": 19,
            "playlist": [
                {
    ...
    def _real_extract(self, url):
        ...
        page_list_json = (
            ...
            and traverse_obj(
                self._download_json(
                    ...
                    video_id,
                    fatal=False,
                    query={"bvid": video_id, "jsonp": "jsonp"},
        ...
        if (
            is_anthology
            and not part_id
            and self._yes_playlist(video_id, video_id)
        ):
        ...
        cid = (
            ...
            if part_id
            else video_data.get("cid")
        )

        festival_info = {}
        if is_festival:
            festival_info = traverse_obj(
                initial_state,
                {
        ...
        metainfo = {
            ...
            **traverse_obj(
            ...
            "_old_archive_ids": (
                [make_archive_id(self, old_video_id)] if old_video_id else None
            ),
            "title": title,
        ...
        if is_interactive:
            return self.playlist_result(
                self._get_interactive_entries(
                    video_id, cid, metainfo, headers=headers
                ),
                **metainfo,
        ...
        if not traverse_obj(play_info, "dash"):
            ...
            for qn in traverse_obj(
                play_info,
                ("accept_quality", lambda _, v: not has_qn(v), {int}),
            ):
                formats.extend(
                    traverse_obj(
            ...
            if flv_formats and len(flv_formats) < len(formats):
                ...
                if not self._configuration_arg("prefer_multi_flv"):
                    dropped_fmts = ", ".join(
                        f'{f.get("format_note")} ({f.get("format_id")})'
                        for f in flv_formats
                    )
                    ...
                    formats = traverse_obj(
                        ...
                        formats,
                        lambda _, v: v["quality"]
                        == int(self._configuration_arg("prefer_multi_flv")[0]),
                ...
                "entries": [
                    {
                        ...
                        "subtitles": (
                            self.extract_subtitles(video_id, cid)
                            if idx == 0
                            else None
                        ),
                        "__post_extractor": (
                            self.extract_comments(aid) if idx == 0 else None
                        ),
                    }
...
class BiliBiliBangumiIE(BilibiliBaseIE):
    ...
    def _real_extract(self, url):
        ...
        play_info = self._search_json(
            ...
            episode_id,
            "Extracting episode",
            query={"fnval": 12240, "ep_id": episode_id},
            headers=headers,
        )

        premium_only = play_info.get("code") == -10403
        play_info = (
            traverse_obj(play_info, ("result", "video_info", {dict})) or {}
        )
            ...
            **traverse_obj(
                episode_info,
                {
                    ...
                    "title": {
                        lambda v: v
                        and join_nonempty(
                            "title", "long_title", delim=" ", from_dict=v
                        )
                    },
                },
            ),
            ...
            "subtitles": self.extract_subtitles(
                episode_id, episode_info.get("cid"), aid=aid
            ),
            "__post_extractor": self.extract_comments(aid),
            "http_headers": {"Referer": url},
        }
...
class BiliBiliBangumiMediaIE(BilibiliBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?bilibili\.com/bangumi/media/md(?P<id>\d+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._get_episodes_from_season(ss_id, url),
            media_id,
            **traverse_obj(
                initial_state,
                (
...
class BiliBiliBangumiSeasonIE(BilibiliBaseIE):
    _VALID_URL = (
        r"(?x)https?://(?:www\.)?bilibili\.com/bangumi/play/ss(?P<id>\d+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._get_episodes_from_season(ss_id, url), ss_id, **metainfo
        )
...
class BilibiliCheeseBaseIE(BilibiliBaseIE):
    def _extract_episode(self, season_info, ep_id):
        episode_info = traverse_obj(
            season_info,
            ("episodes", lambda _, v: v["id"] == int(ep_id)),
            get_all=False,
        )
...
class BilibiliCheeseIE(BilibiliCheeseBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self._extract_episode(
            self._download_season_info("ep_id", ep_id), ep_id
        )
...
class BilibiliCheeseSeasonIE(BilibiliCheeseBaseIE):
    ...
    def _get_cheese_entries(self, season_info):
        for ep_id in traverse_obj(
            season_info, ("episodes", lambda _, v: v["episode_can_view"], "id")
        ):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._get_cheese_entries(season_info),
            season_id,
            **traverse_obj(
                season_info,
                {
...
class BilibiliSpaceVideoIE(BilibiliSpaceBaseIE):
    ...
    def _real_extract(self, url):
        playlist_id, is_video_url = self._match_valid_url(url).group(
            "id", "video"
        )
        if not is_video_url:
            self.to_screen(
        ...
        def fetch_page(page_idx):
            query = {
                "keyword": "",
                "mid": playlist_id,
                "order": traverse_obj(parse_qs(url), ("order", 0))
                or "pubdate",
                "order_avoided": "true",
                "platform": "web",
                "pn": page_idx + 1,
                ...
                "dm_img_str": base64.b64encode(
                    "".join(
                        random.choices(
                            string.printable, k=random.randint(16, 64)
                        )
                    ).encode()
                )[:-2].decode(),
                "dm_cover_img_str": base64.b64encode(
                    "".join(
                        random.choices(
                            string.printable, k=random.randint(32, 128)
                        )
                    ).encode()
                )[:-2].decode(),
        ...
        def get_entries(page_data):
            for entry in traverse_obj(page_data, ("list", "vlist")) or []:
                yield self.url_result(
                    f'https://www.bilibili.com/video/{entry["bvid"]}',
                    BiliBiliIE,
                    entry["bvid"],
                )
...
class BilibiliSpaceListBaseIE(BilibiliSpaceBaseIE):
    def _get_entries(self, page_data, bvid_keys, ending_key="bvid"):
        for bvid in traverse_obj(
            page_data,
            (
                *variadic(bvid_keys, (str, bytes, dict, set)),
                ...,
                ending_key,
                {str},
            ),
...
class BilibiliCollectionListIE(BilibiliSpaceListBaseIE):
    ...
    def suitable(cls, url):
        return (
            False
            if BilibiliSeriesListIE.suitable(url)
...
class BilibiliFavoritesListIE(BilibiliSpaceListBaseIE):
    ...
    def _real_extract(self, url):
        ...
        list_info = self._download_json(
            ...
            fid,
            note="Downloading favlist metadata",
        )
...
class BilibiliWatchlaterIE(BilibiliSpaceListBaseIE):
    ...
    def _real_extract(self, url):
        list_id = getattr(
            self._get_cookies(url).get("DedeUserID"), "value", "watchlater"
        )
        watchlater_info = self._download_json(
            "https://api.bilibili.com/x/v2/history/toview/web?jsonp=jsonp",
            list_id,
        )
...
class BilibiliPlaylistIE(BilibiliSpaceListBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?bilibili\.com/(?:medialist/play|list)/(?P<id>\w+)"
    )
    _TESTS = [
        {
    ...
    def _extract_medialist(self, query, list_id):
        for page_num in itertools.count(1):
            page_data = self._download_json(
                ...
                list_id,
                query=query,
                note=f'getting playlist {query["biz_id"]} page {page_num}',
            )["data"]
            yield from self._get_entries(
                page_data, "media_list", ending_key="bv_id"
            )
    ...
    def _real_extract(self, url):
        ...
        if (
            traverse_obj(initial_state, ("error", "code", {int_or_none}))
            != 200
        ):
            error_code = traverse_obj(
                initial_state, ("error", "trueCode", {int_or_none})
            )
            error_message = traverse_obj(
                initial_state, ("error", "message", {str_or_none})
            )
            ...
            raise ExtractorError(
                f"Could not access playlist: {error_code} {error_message}"
            )
        ...
        return self.playlist_result(
            self._extract_medialist(query, list_id), **metadata
        )
...
class BilibiliCategoryIE(InfoExtractor):
    ...
    def _fetch_page(self, api_url, num_pages, query, page_num):
        ...
        for video in video_list:
            yield self.url_result(
                "https://www.bilibili.com/video/{}".format(video["bvid"]),
                "BiliBili",
                video["bvid"],
            )
    ...
    def _entries(self, category, subcategory, query):
        ...
        api_url = (
            "https://api.bilibili.com/x/web-interface/newlist?rid=%d&type=1&ps=20&jsonp=jsonp"
            % rid_value
        )
        ...
        count, size = int_or_none(page_data.get("count")), int_or_none(
            page_data.get("size")
        )
        ...
        return OnDemandPagedList(
            functools.partial(self._fetch_page, api_url, num_pages, query),
            size,
        )
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._entries(category, subcategory, query), query, query
        )
    ...
    def _search_results(self, query):
        ...
        for page_num in itertools.count(1):
            videos = self._download_json(
                ...
                query,
                note=f"Extracting results from page {page_num}",
                query={
                    "Search_key": query,
                    "keyword": query,
                    "page": page_num,
            ...
            for video in videos:
                yield self.url_result(
                    video["arcurl"], "BiliBili", str(video["aid"])
                )
...
class BilibiliAudioAlbumIE(BilibiliAudioBaseIE):
    ...
    def _real_extract(self, url):
        ...
        songs = self._call_api(
            "song/of-menu", am_id, {"sid": am_id, "pn": 1, "ps": 100}
        )["data"]
...
class BiliBiliPlayerIE(InfoExtractor):
    _VALID_URL = (
        r"https?://player\.bilibili\.com/player\.html\?.*?\baid=(?P<id>\d+)"
    )
    _TEST = {
...
class BiliBiliDynamicIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if not video_url or (
            self.suitable(video_url) and post_id == self._match_id(video_url)
        ):
...
class BiliIntlBaseIE(InfoExtractor):
    ...
    def _call_api(self, endpoint, *args, **kwargs):
        ...
        if json.get("code"):
            if json["code"] in (10004004, 10004005, 10023006):
                ...
                if (
                    json.get("message")
                    and str(json["code"]) != json["message"]
                ):
        ...
        return "\n\n".join(
            ...
            for i, line in enumerate(
                traverse_obj(
                    ...
                    (
                        "body",
                        lambda _, l: l["content"] and l["from"] and l["to"],
                    ),
                )
            )
        )
    ...
    def _get_subtitles(self, *, ep_id=None, aid=None):
        sub_json = (
            self._call_api(
                ...
                ep_id or aid,
                fatal=False,
                note="Downloading subtitles list",
                errnote="Unable to download subtitles list",
                query=filter_dict(
                    {
                        "platform": "web",
                        "s_locale": "en_US",
    ...
    def _parse_video_metadata(self, video_data):
        return {
            "title": video_data.get("title_display")
    ...
    def _perform_login(self, username, password):
        ...
        login_post = self._download_json(
            "https://passport.bilibili.tv/x/intl/passport-login/web/login/password?lang=en-US",
            None,
...
class BiliIntlIE(BiliIntlBaseIE):
    ...
    def _extract_video_metadata(self, url, video_id, season_id):
        ...
        initial_data = self._search_json(
            ...
            video_id,
            default={},
        ) or self._search_nuxt_data(
            webpage, video_id, "__initialState", fatal=False, traverse=None
        )
        video_data = (
            traverse_obj(
                initial_data,
        ...
        if season_id and not video_data:
            ...
            season_json = self._call_api(
                f"/web/v2/ogv/play/episodes?season_id={season_id}&platform=web",
                video_id,
            )
            video_data = traverse_obj(
                ...
                (
                    ...
                    ...,
                    "episodes",
                    lambda _, v: str(v["episode_id"]) == video_id,
                ),
                expected_type=dict,
                get_all=False,
            )
        ...
        return merge_dicts(
            self._parse_video_metadata(video_data),
            {
                ...
                "description": get_element_by_class(
                    "bstar-meta__desc", webpage
                )
                or self._html_search_meta("og:description", webpage),
            },
    ...
    def _get_comments_reply(self, root_id, next_id=0, display_id=None):
        comment_api_raw_data = self._download_json(
            "https://api.bilibili.tv/reply/web/detail",
            display_id,
        ...
        if not traverse_obj(
            comment_api_raw_data, ("data", "cursor", "is_end")
        ):
            yield from self._get_comments_reply(
                root_id,
                comment_api_raw_data["data"]["cursor"]["next"],
                display_id,
            )
    ...
    def _get_comments(self, video_id, ep_id):
        for i in itertools.count(0):
            comment_api_raw_data = self._download_json(
                "https://api.bilibili.tv/reply/web/root",
                video_id,
                ...
                query={
                    ...
                    "type": (
                        3 if ep_id else 1
                    ),  # 1: user generated content, 3: series content
            ...
            for replies in traverse_obj(
                ...
                yield {
                    ...
                    "author_is_uploader": bool(
                        traverse_obj(replies, ("member", "type"))
                    ),
                }
                if replies.get("count"):
                    yield from self._get_comments_reply(
                        replies.get("rpid"), display_id=video_id
                    )
    ...
    def _real_extract(self, url):
        season_id, ep_id, aid = self._match_valid_url(url).group(
            "season_id", "ep_id", "aid"
        )
                ...
                chapters = [
                    {
                        ...
                        "end_time": float_or_none(
                            traverse_obj(
                                intro_ending_json, ("skip", "opening_end_time")
                            ),
                            1000,
                        ),
                        "title": "Intro",
                    },
                    {
                        ...
                        "end_time": float_or_none(
                            traverse_obj(
                                intro_ending_json, ("skip", "ending_end_time")
                            ),
                            1000,
                        ),
                        "title": "Outro",
...
class BiliIntlSeriesIE(BiliIntlBaseIE):
    ...
    def _entries(self, series_id):
        series_json = self._call_api(
            f"/web/v2/ogv/play/episodes?season_id={series_id}&platform=web",
            series_id,
        )
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            ...
            thumbnail=url_or_none(series_info.get("horizontal_cover")),
...
class BiliLiveIE(InfoExtractor):
    ...
    def _call_api(self, path, room_id, query):
        ...
        if api_result.get("code") != 0:
            raise ExtractorError(
                api_result.get("message") or "Unable to download JSON metadata"
            )
    ...
    def _real_extract(self, url):
        ...
        for qn in self._FORMATS:
            ...
            for fmt in (
                traverse_obj(
                    stream_data,
                    ("playurl_info", "playurl", "stream", ..., "format", ...),
                )
                or []
            ):
