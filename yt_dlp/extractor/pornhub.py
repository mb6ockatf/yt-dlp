...
class PornHubBaseIE(InfoExtractor):
    ...
    def _download_webpage_handle(self, *args, **kwargs):
        def dl(*args, **kwargs):
            return super(PornHubBaseIE, self)._download_webpage_handle(
                *args, **kwargs
            )
...
class PornHubIE(PornHubBaseIE):
    ...
    def _extract_count(self, pattern, webpage, name):
        return str_to_int(
            self._search_regex(pattern, webpage, f"{name} count", default=None)
        )
    ...
    def _real_extract(self, url):
        ...
        if error_msg:
            ...
            raise ExtractorError(
                f"PornHub said: {error_msg}", expected=True, video_id=video_id
            )
        ...
        if not video_urls:
            ...
            js_vars = extract_js_vars(
                webpage,
                r"(var\s+(?:{})_.+)".format("|".join(FORMAT_PREFIXES)),
                default=None,
            )
            if js_vars:
        ...
        return merge_dicts(
            {
                ...
                "uploader_id": remove_start(
                    model_profile.get("modelProfileLink"), "/model/"
                ),
...
class PornHubPlaylistBaseIE(PornHubBaseIE):
    def _extract_page(self, url):
        return int_or_none(
            self._search_regex(r"\bpage=(\d+)", url, "page", default=None)
        )
...
class PornHubPagedPlaylistBaseIE(PornHubPlaylistBaseIE):
    ...
    def _entries(self, url, host, item_id):
        ...
        for page_num in (
            (first_page,) if has_page else itertools.count(first_page)
        ):
            try:
                try:
                    ...
                    if (
                        is_404(e)
                        and page_num == first_page
                        and VIDEOS in base_url
                    ):
...
class PornHubPagedVideoListIE(PornHubPagedPlaylistBaseIE):
    ...
    def suitable(cls, url):
        return (
            False
            if PornHubIE.suitable(url)
            or PornHubUserIE.suitable(url)
            or PornHubUserVideosUploadIE.suitable(url)
...
class PornHubPlaylistIE(PornHubPlaylistBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._entries(mobj.group("url"), host, item_id), item_id
        )
