...
class FptplayIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        video_id, slug_episode = self._match_valid_url(url).group(
            "id", "episode"
        )
        webpage = (
            self._download_webpage(url, video_id=video_id, fatal=False) or ""
        )
        title = self._search_regex(
            r'(?s)<h4\s+class="mb-1 text-2xl text-white"[^>]*>(.+)</h4>',
            webpage,
            "title",
            fatal=False,
        )
        real_episode = (
            ...
            if not title
                ...
                r'<p.+title="(?P<episode>[^">]+)"\s+class="epi-title active"',
                webpage,
                "episode",
                fatal=False,
            )
        )
        title = strip_or_none(title) or self._html_search_meta(
            ("og:title", "twitter:title"), webpage
        )

        info = self._download_json(
            self.get_api_with_st_token(
                video_id, int(slug_episode) - 1 if slug_episode else 0
            ),
            video_id,
        )
        formats, subtitles = self._extract_m3u8_formats_and_subtitles(
            info["data"]["url"], video_id, "mp4"
        )
        return {
            "id": video_id,
    ...
    def get_api_with_st_token(self, video_id, episode):
        ...
        t = (
            hashlib.md5(f"WEBv6Dkdsad90dasdjlALDDDS{timestamp}{path}".encode())
            .hexdigest()
            .upper()
        )
        ...
        st_token = (
            convert(n).replace("+", "-").replace("/", "_").replace("=", "")
        )
