...
class TNAFlixNetworkBaseIE(InfoExtractor):
    ...
    def _extract_thumbnails(self, flix_xml):
        ...
        width = int_or_none(
            xpath_text(timeline, "./imageWidth", "thumbnail width")
        )
        height = int_or_none(
            xpath_text(timeline, "./imageHeight", "thumbnail height")
        )

        return [
            {
                "url": self._proto_relative_url(
                    pattern_el.text.replace("#", str(i)), "http:"
                ),
                "width": width,
                "height": height,
            }
    ...
    def _real_extract(self, url):
        ...
        if cfg_url:
            ...
            for item in cfg_xml.findall("./quality/item"):
                ...
                formats.append(
                    {
                        "url": self._proto_relative_url(
                            extract_video_url(video_link), "http:"
                        ),
                        "format_id": format_id,
                        "height": height,
                    }
            ...
            thumbnails.append(
                {
                    "url": self._proto_relative_url(
                        xpath_text(cfg_xml, "./startThumb", "thumbnail"),
                        "http:",
                    ),
                }
            )
                ...
                formats.append(
                    {
                        ...
                        "format_id": (
                            f"{height}p"
                            if height
                            else url_basename(video_url).split(".")[0]
                        ),
                    }
                )
            thumbnail = self._proto_relative_url(
                self._search_regex(
                    r'data-poster="([^"]+)"', player, "thumbnail", default=None
                ),
                "http:",
            )
        ...
        def extract_field(pattern, name):
            return (
                self._html_search_regex(pattern, webpage, name, default=None)
                if pattern
                else None
            )
        ...
        return {
            ...
            "title": (
                extract_field(self._TITLE_REGEX, "title")
                or self._og_search_title(webpage, default=None)
                or json_ld.get("title")
            ),
            "description": extract_field(
                self._DESCRIPTION_REGEX, "description"
            )
            ...
            "view_count": str_to_int(
                extract_field(self._VIEW_COUNT_REGEX, "view count")
            ),
            "comment_count": str_to_int(
                extract_field(self._COMMENT_COUNT_REGEX, "comment count")
            ),
            "average_rating": float_or_none(
                extract_field(self._AVERAGE_RATING_REGEX, "average rating")
            ),
            "categories": list(
                map(
                    str.strip,
                    (
                        extract_field(self._CATEGORIES_REGEX, "categories")
                        or ""
                    ).split(","),
                )
            ),
...
class TNAFlixNetworkEmbedIE(TNAFlixNetworkBaseIE):
    _VALID_URL = (
        r"https?://player\.(?P<host>tnaflix|empflix)\.com/video/(?P<id>\d+)"
    )
    _EMBED_REGEX = [
...
class TNAFlixIE(TNAEMPFlixBaseIE):
    ...
    _TITLE_REGEX = (
        r"<title>(.+?) - (?:TNAFlix Porn Videos|TNAFlix\.com)</title>"
    )
...
class MovieFapIE(TNAFlixNetworkBaseIE):
    ...
    _AVERAGE_RATING_REGEX = (
        r"Current Rating\s*<br>\s*<strong>([\d.]+)</strong>"
    )
    _CATEGORIES_REGEX = (
        r'(?s)<div[^>]+id="vid_info"[^>]*>\s*<div[^>]*>.+?</div>(.*?)<br>'
    )
