...
class VidyardBaseIE(InfoExtractor):
    ...
    def _get_formats_and_subtitles(self, sources, video_id):
        ...
        if master_m3u8_url := traverse_obj(
            hls_list,
            (lambda _, v: v["profile"] == "auto", "url", {url_or_none}, any),
        ):
            add_hls_fmts_and_subs(master_m3u8_url)
        if (
            not formats
        ):  # These are duplicate and unnecesary requests if we got 'auto' hls fmts
        ...
        for source_type, source_list in traverse_obj(
            ...
            for source in traverse_obj(
                source_list, lambda _, v: url_or_none(v["url"])
            ):
                ...
                formats.append(
                    {
                        "url": source["url"],
                        "ext": mimetype2ext(
                            source.get("mimeType"), default=None
                        ),
                        "format_id": join_nonempty(
                            "http", source_type, profile
                        ),
    ...
    def _get_direct_subtitles(self, caption_json):
        ...
        for caption in traverse_obj(
            caption_json, lambda _, v: url_or_none(v["vttUrl"])
        ):
    ...
    def _fetch_video_json(self, video_id):
        return self._download_json(
            f"https://play.vidyard.com/player/{video_id}.json", video_id
        )["payload"]
    ...
    def _process_video_json(self, json_data, video_id):
        ...
        self._merge_subtitles(
            self._get_direct_subtitles(json_data.get("captions")),
            target=subtitles,
        )
...
class VidyardIE(VidyardBaseIE):
    ...
    def _extract_embed_urls(cls, url, webpage):
        ...
        for embed_element in re.findall(
            ...
            if video_id := extract_attributes(embed_element[0]).get(
                "data-uuid"
            ):
                yield f"https://play.vidyard.com/{video_id}"

        for embed_id in re.findall(
            r'<script[^>]* id=["\']vidyard_embed_code_([\w-]+)["\']', webpage
        ):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            (
                self._process_video_json(chapter, video_id)
                for chapter in video_json["chapters"]
            ),
            str(video_json["playerUuid"]),
            video_json.get("name"),
        )
