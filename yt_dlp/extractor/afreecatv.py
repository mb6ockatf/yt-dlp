...
class AfreecaTVBaseIE(InfoExtractor):
    ...
    def _call_api(
        self, endpoint, display_id, data=None, headers=None, query=None
    ):
...
class AfreecaTVIE(AfreecaTVBaseIE):
    ...
    def _real_extract(self, url):
        ...
        for file_num, file_element in enumerate(
            traverse_obj(data, ("files", lambda _, v: url_or_none(v["file"]))),
            start=1,
        ):
            ...
            entries.append(
                {
                    **common_info,
                    "id": file_element.get("file_info_key")
                    or f"{video_id}_{file_num}",
        ...
        if not entries and traverse_obj(data, ("sub_upload_type", {str})):
            self.raise_login_required(
                "This VOD is for subscribers only", method="password"
            )
...
class AfreecaTVCatchStoryIE(AfreecaTVBaseIE):
    ...
    def _entries(self, data):
        ...
        yield from traverse_obj(
            ...
            (
                "data",
                lambda _, v: v["story_type"] == "catch",
                "catch_list",
                lambda _, v: v["files"][0]["file"],
                {
...
class AfreecaTVLiveIE(AfreecaTVBaseIE):
    ...
    def _extract_formats(self, channel_info, broadcast_no, aid):
        stream_base_url = (
            channel_info.get("RMD")
            or "https://livestream-manager.sooplive.co.kr"
        )
        ...
        default_cdn_ids = orderedSet(
            [
                *traverse_obj(
                    channel_info,
                    ("CDN", {str}, all, lambda _, v: v not in self._BAD_CDNS),
                ),
                *self._WORKING_CDNS,
            ]
        )
        ...
        for attempt, cdn_id in enumerate(cdn_ids, start=1):
            m3u8_url = traverse_obj(
                self._download_json(
                    urljoin(stream_base_url, "broad_stream_assign.html"),
                    broadcast_no,
    ...
    def _real_extract(self, url):
        broadcaster_id, broadcast_no = self._match_valid_url(url).group(
            "id", "bno"
        )
        channel_info = (
            traverse_obj(
                self._download_json(
                    self._LIVE_API_URL,
                    broadcaster_id,
                    data=urlencode_postdata({"bid": broadcaster_id}),
                ),
                ("CHANNEL", {dict}),
            )
            or {}
        ...
        token_info = (
            traverse_obj(
                self._download_json(
                    ...
                    "Unable to download access token for stream",
                    data=urlencode_postdata(
                        filter_dict(
                            {
                                "bno": broadcast_no,
        ...
        return {
            ...
            or station_info.get("station_title"),
            "uploader": channel_info.get("BJNICK")
            or station_info.get("station_name"),
    ...
    def _fetch_page(self, user_id, user_type, page):
        ...
        for item in info["data"]:
            yield self.url_result(
                f'https://vod.sooplive.co.kr/player/{item["title_no"]}/',
                AfreecaTVIE,
                item["title_no"],
            )

    def _real_extract(self, url):
        user_id, user_type = self._match_valid_url(url).group(
            "id", "slug_type"
        )
