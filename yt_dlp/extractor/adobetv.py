...
class AdobeTVBaseIE(InfoExtractor):
    def _call_api(self, path, video_id, query, note=None):
        return self._download_json(
            "http://tv.adobe.com/api/v4/" + path, video_id, note, query=query
        )["data"]
...
class AdobeTVEmbedIE(AdobeTVBaseIE):
    ...
    def _real_extract(self, url):
        ...
        video_data = self._call_api(
            "episode/" + video_id, video_id, {"disclosure": "standard"}
        )[0]
...
class AdobeTVPlaylistBaseIE(AdobeTVBaseIE):
    ...
    def _extract_playlist_entries(self, display_id, query):
        return OnDemandPagedList(
            functools.partial(self._fetch_page, display_id, query),
            self._PAGE_SIZE,
        )
...
class AdobeTVChannelIE(AdobeTVPlaylistBaseIE):
    ...
    def _real_extract(self, url):
        language, channel_urlname, category_urlname = self._match_valid_url(
            url
        ).groups()
        if not language:
            language = "en"
...
class AdobeTVVideoIE(AdobeTVBaseIE):
    ...
    def _real_extract(self, url):
        ...
        video_data = self._parse_json(
            self._search_regex(
                r"var\s+bridge\s*=\s*([^;]+);", webpage, "bridged data"
            ),
            video_id,
        )
        title = video_data["title"]
            ...
            formats.append(
                {
                    "filesize": int_or_none(
                        source.get("kilobytes") or None, invscale=1000
                    ),
                    "format_id": join_nonempty(
                        source.get("format"), source.get("label")
                    ),
