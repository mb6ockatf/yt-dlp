...
class OpencastBaseIE(InfoExtractor):
    ...
    def _call_api(self, host, video_id, **kwargs):
        return self._download_json(
            self._API_BASE % (host, video_id), video_id, **kwargs
        )
    ...
    def _parse_mediapackage(self, video):
            ...
            if transport == "DASH" or ext == "mpd":
                ...
                formats.extend(
                    self._extract_ism_formats(
                        href, video_id, ism_id="smooth", fatal=False
                    )
                )
            elif ext == "smil":
                formats.extend(
                ...
                track_obj = {
                    ...
                    "fps": int_or_none(
                        traverse_obj(track, ("video", "framerate"))
                    ),
                    "vbr": int_or_none(
                        traverse_obj(track, ("video", "bitrate")), scale=1000
                    ),
                    "vcodec": (
                        traverse_obj(track, ("video", "encoder", "type"))
                        if track.get("video")
                        else "none"
                    ),
                    "abr": int_or_none(
                        traverse_obj(track, ("audio", "bitrate")), scale=1000
                    ),
                    "asr": int_or_none(
                        traverse_obj(track, ("audio", "samplingrate"))
                    ),
                    "acodec": (
                        traverse_obj(track, ("audio", "encoder", "type"))
                        if track.get("audio")
                        else "none"
                    ),
                }
                    ...
                    track_obj.update(
                        {
                            "app": m_obj.group("app"),
                            "ext": m_obj.group("ext"),
                            "play_path": m_obj.group("ext")
                            + ":"
                            + m_obj.group("playpath"),
                            "rtmp_live": True,
                            "preference": -2,
                        }
                    )
        ...
        return {
            ...
            "thumbnail": traverse_obj(
                video, ("attachments", "attachment", ..., "url"), get_all=False
            ),
        }
...
class OpencastIE(OpencastBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self._parse_mediapackage(
            self._call_api(host, video_id)["search-results"]["result"][
                "mediapackage"
            ]
        )
...
class OpencastPlaylistIE(OpencastBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            entries, video_id, traverse_obj(entries, (0, "series"))
        )
