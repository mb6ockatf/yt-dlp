...
class RedBullTVIE(InfoExtractor):
    ...
    def extract_info(self, video_id):
        session = self._download_json(
            "https://api.redbull.tv/v3/session",
            video_id,
        ...
        if session.get("code") == "error":
            raise ExtractorError(
                "{} said: {}".format(self.IE_NAME, session["message"])
            )
        token = session["token"]
        ...
        return {
            "id": video_id,
            "title": title,
            "description": video.get("long_description")
            or video.get("short_description"),
...
class RedBullEmbedIE(RedBullTVIE):  # XXX: Do not subclass from concrete IE
  ...
  resource(id: "%s", enforceGeoBlocking: false) {
    ...
    %s
                ...
                % (
                    rrn_id,
                    self._VIDEO_ESSENSE_TMPL % "LiveVideo",
