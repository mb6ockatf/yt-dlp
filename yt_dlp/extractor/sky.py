...
class SkyBaseIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        info = self._process_video_element(
            webpage,
            self._search_regex(self._SDC_EL_REGEX, webpage, "sdc element"),
            url,
        )
        info.update(
            {
...
class SkyNewsStoryIE(SkyBaseIE):
    ...
    def _real_extract(self, url):
        ...
        entries = [
            self._process_video_element(webpage, sdc_el, url)
...
class SkySportsNewsIE(SkyBaseIE):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?skysports\.com/([^/]+/)*news/\d+/(?P<id>\d+)"
    )
    _TEST = {
