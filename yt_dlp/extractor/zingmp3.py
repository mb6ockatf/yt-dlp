    ...
    def _real_initialize(self):
        if not self._cookies_passed:
            self._request_webpage(
                self._api_url("bai-hat", {"id": ""}),
                None,
                note="Updating cookies",
            )
...
class ZingMp3IE(ZingMp3BaseIE):
    ...
    def _real_extract(self, url):
                ...
                if k == "hls":
                    formats.extend(
                        self._extract_m3u8_formats(
                            video_url, item_id, "mp4", m3u8_id=k, fatal=False
                        )
                    )
                    continue
                formats.append(
        ...
        return {
            ...
            "artist": traverse_obj(
                item, "artistsNames", "artists_names", ("artists", 0, "name")
            ),
            "album": traverse_obj(
                item,
                ("album", ("name", "title")),
...
class ZingMp3ChartMusicVideoIE(ZingMp3BaseIE):
    ...
    def _real_extract(self, url):
        song_id, regions, url_type = self._match_valid_url(url).group(
            "id", "regions", "type"
        )
        return self.playlist_result(
            self._paged_list(song_id, url_type),
            song_id,
...
class ZingMp3UserIE(ZingMp3BaseIE):
    ...
    def _fetch_page(self, user_id, url_type, page):
        url_type = (
            "user-list-song" if url_type == "bai-hat" else "user-list-video"
        )
        return self._call_api(
            url_type,
            {
    ...
    def _real_extract(self, url):
        ...
        user_info = self._call_api(
            "info-artist", {}, alias, query={"alias": alias}
        )
            ...
            if url_type in ("bai-hat", "video"):
                ...
                entries = self._parse_items(
                    traverse_obj(
                        ...
                        (
                            "sections",
                            lambda _, v: v["sectionId"] == section_id,
                            "items",
                            ...,
                        ),
                    )
...
class ZingMp3LiveRadioIE(ZingMp3BaseIE):
    ...
    def _real_extract(self, url):
        url_type, live_radio_id = self._match_valid_url(url).group(
            "type", "id"
        )
...
class ZingMp3PodcastIE(ZingMp3BaseIE):
    ...
    def _real_extract(self, url):
        ...
        items = self._call_api(
            "cgrs" if url_type == "cgr" else url_type, params
        )["items"]
        return self.playlist_result(self._parse_items(items), url_type)
