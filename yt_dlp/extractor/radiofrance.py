...
class FranceCultureIE(RadioFranceBaseIE):
    ...
    def _real_extract(self, url):
        video_id, display_id = self._match_valid_url(url).group(
            "id", "display_id"
        )
        ...
        video_data = self._search_json(
            ...
            "audio data",
            display_id,
            contains_pattern=r'{\s*"@type"\s*:\s*"AudioObject".+}',
        )
        ...
        return {
            ...
            "vcodec": (
                "none" if video_data.get("encodingFormat") == "mp3" else None
            ),
            ...
            "description": self._html_search_regex(
                r'(?s)<meta name="description"\s*content="([^"]+)',
                webpage,
                "description",
                default=None,
            ),
...
class RadioFranceLiveIE(RadioFranceBaseIE):
    ...
    def _real_extract(self, url):
        station_id, substation_id = self._match_valid_url(url).group(
            "id", "substation_id"
        )

        if substation_id:
            webpage = self._download_webpage(url, station_id)
            api_response = self._extract_data_from_webpage(
                webpage, station_id, "webRadioData"
            )
        else:
        ...
        for media_source in traverse_obj(
            api_response,
            (("now", None), "media", "sources", lambda _, v: v["url"]),
        ):
            if media_source.get("format") == "hls":
                fmts, subs = self._extract_m3u8_formats_and_subtitles(
                    media_source["url"], station_id, fatal=False
                )
                formats.extend(fmts)
        ...
        return {
            ...
            "title": traverse_obj(api_response, ("visual", "legend"))
            or join_nonempty(
                ("now", "firstLine", "title"),
                ("now", "secondLine", "title"),
                from_dict=api_response,
                delim=" - ",
            ),
            "formats": formats,
            "subtitles": subtitles,
...
class RadioFrancePlaylistBaseIE(RadioFranceBaseIE):
        ...
        for page_num in itertools.count(2):
            for entry in content_response["items"]:
                yield self.url_result(
                    f'https://www.radiofrance.fr/{entry["path"]}',
            ...
            next_cursor = traverse_obj(
                content_response, (("pagination", None), "next"), get_all=False
            )
            if not next_cursor:
                break

            content_response = self._call_api(
                content_id, next_cursor, page_num
            )
    ...
    def _real_extract(self, url):
        ...
        metadata = self._download_json(
            "https://www.radiofrance.fr/api/v2.1/path",
            display_id,
            query={"value": urllib.parse.urlparse(url).path},
        )["content"]

        content_id = metadata["id"]

        return self.playlist_result(
            self._generate_playlist_entries(
                content_id, metadata[self._METADATA_KEY]
            ),
            content_id,
            display_id=display_id,
            **{
...
class RadioFrancePodcastIE(RadioFrancePlaylistBaseIE):
    ...
    def _call_api(self, podcast_id, cursor, page_num):
        return self._download_json(
            f"https://www.radiofrance.fr/api/v2.1/concepts/{podcast_id}/expressions",
            podcast_id,
...
class RadioFranceProfileIE(RadioFrancePlaylistBaseIE):
    _VALID_URL = (
        rf"{RadioFranceBaseIE._VALID_URL_BASE}/personnes/(?P<id>[\w-]+)"
    )
    ...
    def _call_api(self, profile_id, cursor, page_num):
        resp = self._download_json(
            f"https://www.radiofrance.fr/api/v2.1/taxonomy/{profile_id}/documents",
            profile_id,
            note=f"Downloading page {page_num}",
            query={
...
class RadioFranceProgramScheduleIE(RadioFranceBaseIE):
    ...
    def _generate_playlist_entries(self, webpage_url, api_response):
        for entry in traverse_obj(
            api_response, ("steps", lambda _, v: v["expression"]["path"])
        ):
            yield self.url_result(
                urljoin(webpage_url, f'/{entry["expression"]["path"]}'),
                ie=FranceCultureIE,
                url_transparent=True,
                **traverse_obj(
                    entry,
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            ...
            join_nonempty(station, "program", upload_date),
