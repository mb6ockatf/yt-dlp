...
class HRTiBaseIE(InfoExtractor):
    ...
    def _initialize_pre_login(self):
        ...
        uuid = self._download_json(
            self._API_URL,
            None,
            note="Downloading uuid",
            errnote="Unable to download uuid",
            data=json.dumps(init_data).encode(),
        )["uuid"]
        ...
        resources = self._download_json(
            req,
            None,
            note="Downloading session information",
        ...
        self._search_url = modules["vod_catalog"]["resources"]["search"][
            "uri"
        ].format(
        ...
        self._login_url = (
            modules["user"]["resources"]["login"]["uri"] + "/format/json"
        ).format(session_id=self._session_id)
...
class HRTiIE(HRTiBaseIE):
    ...
    def _real_extract(self, url):
        ...
        age_limit = parse_age_limit(
            video.get("parental_control", {}).get("rating")
        )
...
class HRTiPlaylistIE(HRTiBaseIE):
    ...
    def _real_extract(self, url):
        ...
        entries = [
            self.url_result(f"hrti:{video_id}") for video_id in video_ids
        ]
