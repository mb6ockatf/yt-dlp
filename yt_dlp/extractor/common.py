...
class InfoExtractor:
    ...
    def _login_hint(self, method=NO_DEFAULT, netrc=None):
        ...
        return {
            ...
            (
                ...
                else "any" if self.supports_login() else "cookies"
            )
        ]
    ...
    def _match_valid_url(cls, url):
        ...
        if "_VALID_URL_RE" not in cls.__dict__:
            cls._VALID_URL_RE = tuple(
                map(re.compile, variadic(cls._VALID_URL))
            )
        return next(
            filter(None, (regex.match(url) for regex in cls._VALID_URL_RE)),
            None,
        )
    ...
    def initialize(self):
        ...
        if not self._ready:
            ...
            elif self.get_param("username") and False not in (
                self.IE_DESC,
                self._NETRC_MACHINE,
            ):
                self.report_warning(
                ...
                try:
                    ...
                    self.to_screen(
                        ...
                        % (
                            ...
                            else truncate_string(url, 100, 20)
                        )
                    )
                    ...
                    if self._x_forwarded_for_ip:
                        ie_result["__x_forwarded_for_ip"] = (
    ...
    def _create_request(
        ...
        url_or_request.update(
            data=data, headers=headers, query=query, extensions=extensions
        )
    ...
    def _request_webpage(
        ...
        requested_targets = (
            [
                (
                    t
                    if isinstance(t, ImpersonateTarget)
                    else ImpersonateTarget.from_str(t)
                )
        ...
        available_target = next(
            filter(
                self._downloader._impersonate_target_available,
                requested_targets,
            ),
            None,
        )
        if available_target:
            ...
            self.report_warning(
                f"{message}; if you encounter errors, then {info_msg}",
                only_once=True,
            )

        try:
            return self._downloader.urlopen(
                self._create_request(
                    url_or_request, data, headers, query, extensions
                )
            )
    ...
    def __check_blocked(self, content):
        ...
        if (
            "<title>Access to this site is blocked</title>" in content
        ...
        if (
            ...
            raise ExtractorError(msg, expected=True)
        if (
            "<title>TTK :: Доступ к ресурсу ограничен</title>" in content
    ...
    def __decode_webpage(self, webpage_bytes, encoding, headers):
        if not encoding:
            encoding = self._guess_encoding_from_content(
                headers.get("Content-Type", ""), webpage_bytes
            )
        try:
    ...
    def _parse_xml(
        ...
        try:
            ...
            self.__print_error(
                "Failed to parse XML" if errnote is None else errnote,
                fatal,
                video_id,
                ve,
            )
    ...
    def _parse_json(
        ...
        **parser_kwargs,
        ...
        try:
            ...
            self.__print_error(
                "Failed to parse JSON" if errnote is None else errnote,
                fatal,
                video_id,
                ve,
            )
    ...
    def _parse_socket_response_as_json(self, data, *args, **kwargs):
        return self._parse_json(
            data[data.find("{") : data.rfind("}") + 1], *args, **kwargs
        )
    ...
    def __create_download_methods(name, parser, note, errnote, return_value):
        ...
        def download_content(
            ...
            if self.get_param("load_pages"):
                url_or_request = self._create_request(
                    url_or_request, data, headers, query
                )
                filename = _request_dump_filename(
                    url_or_request.url,
                    video_id,
                ...
                try:
                    ...
                    self.report_warning(
                        f"Unable to load request from disk: {e}"
                    )
                else:
                    content = self.__decode_webpage(
                        webpage_bytes, encoding, url_or_request.headers
                    )
                    return parse(
                        self,
                        content,
            ...
            res = getattr(self, download_handle.__name__)(
                url_or_request, video_id, **kwargs
            )
        ...
        impersonate(
            download_handle,
            f"_download_{name}_handle",
            f"({return_value}, URL handle)",
        )
    ...
    __download_webpage = __create_download_methods(
        "webpage", None, None, None, "data of the page as a string"
    )[1]
    ...
    def _download_webpage(
        ...
        **kwargs,
        ...
        while True:
            try:
                return self.__download_webpage(
                    url_or_request,
                    video_id,
                    note,
                    errnote,
    ...
    def report_warning(
        self, msg, video_id=None, *args, only_once=False, **kwargs
    ):
    ...
    def write_debug(self, msg, *args, **kwargs):
        self._downloader.write_debug(
            f"[{self.IE_NAME}] {msg}", *args, **kwargs
        )
    ...
    def report_drm(self, video_id, partial=NO_DEFAULT):
        if partial is not NO_DEFAULT:
            self._downloader.deprecation_warning(
                "InfoExtractor.report_drm no longer accepts the argument partial"
            )
        self.raise_no_formats(
            "This video is DRM protected", expected=True, video_id=video_id
        )
    ...
    def raise_login_required(
        ...
        if metadata_available and (
            self.get_param("ignore_no_formats_error")
    ...
    def playlist_from_matches(
        ...
        **kwargs,
        ...
        return cls.playlist_result(
            (
                cls.url_result(m, ie, **(video_kwargs or {}))
    ...
    def _search_regex(
        ...
        _name = self._downloader._format_err(
            name, self._downloader.Styles.EMPHASIS
        )
        ...
        if mobj:
            ...
            self.report_warning(
                f"unable to extract {_name}" + bug_reports_message()
            )
            return None
    ...
    def _search_json(
        ...
        **kwargs,
        ...
        try:
            return self._parse_json(
                json_string, video_id, ignore_extra=True, **kwargs
            )
        except ExtractorError as e:
            if fatal:
                raise ExtractorError(
                    f"Unable to extract {_name} - Failed to parse JSON",
                    cause=e.cause,
                    video_id=video_id,
                )
            elif not has_default:
                self.report_warning(
                    f"Unable to extract {_name} - Failed to parse JSON: {e}",
                    video_id=video_id,
                )
        return default
    ...
    def _get_netrc_login_info(self, netrc_machine=None):
        ...
        if cmd:
            ...
            netrc_file = compat_expanduser(
                self.get_param("netrc_location") or "~"
            )
    ...
    def _og_search_property(self, prop, html, name=None, **kargs):
        ...
        escaped = self._search_regex(
            og_regexes, html, name, flags=re.DOTALL, **kargs
        )
        if escaped is None:
            return None
    ...
    def _og_search_thumbnail(self, html, **kargs):
        return self._og_search_property(
            "image", html, "thumbnail URL", fatal=False, **kargs
        )

    def _og_search_description(self, html, **kargs):
        return self._og_search_property(
            "description", html, fatal=False, **kargs
        )
    ...
    def _og_search_video_url(
        self, html, name="video url", secure=True, **kargs
    ):
    ...
    def _html_extract_title(
        self, html, name="title", *, fatal=False, **kwargs
    ):
    ...
    def _html_search_meta(
        self, name, html, display_name=None, fatal=False, **kwargs
    ):
        name = variadic(name)
    ...
    def _rta_search(html):
        ...
        for marker in AGE_LIMIT_MARKERS:
            ...
            if mobj:
                age_limit = max(
                    age_limit, int(traverse_obj(mobj, 1, default=18))
                )
        return age_limit
    ...
    def _twitter_search_player(self, html):
        return self._html_search_meta(
            "twitter:player", html, "twitter card player"
        )

    def _yield_json_ld(
        self, html, video_id, *, fatal=True, default=NO_DEFAULT
    ):
    ...
    def _search_json_ld(
        ...
        *,
        ...
        info = self._json_ld(
            list(
                self._yield_json_ld(
                    html, video_id, fatal=fatal, default=default
                )
            ),
            video_id,
            fatal=fatal,
        ...
        if default is not NO_DEFAULT:
            ...
            self.report_warning(
                f"unable to extract JSON-LD {bug_reports_message()}"
            )
            return {}
    ...
    def _json_ld(self, json_ld, video_id, fatal=True, expected_type=None):
        ...
        def extract_interaction_statistic(e):
            ...
            for is_e in interaction_statistic:
                ...
                interaction_count = str_to_int(
                    is_e.get("userInteractionCount")
                )
                if interaction_count is None:
                    continue
                count_kind = INTERACTION_TYPE_MAP.get(
                    interaction_type.split("/")[-1]
                )
                if not count_kind:
                    continue
        ...
        def extract_chapter_information(e):
            ...
            for idx, (last_c, current_c, next_c) in enumerate(
                ...
                current_c["end_time"] = (
                    current_c["end_time"] or next_c["start_time"]
                )
                current_c["start_time"] = (
                    current_c["start_time"] or last_c["end_time"]
                )
                if None in current_c.values():
                    self.report_warning(
                        f"Chapter {idx} contains broken data. Not extracting chapters"
                    )
                    return
            if chapters:
                chapters[-1]["end_time"] = (
                    chapters[-1]["end_time"] or info["duration"]
                )
            ...
            info.update(
                {
                    ...
                    "thumbnails": [
                        ...
                        for url in variadic(
                            traverse_obj(e, "thumbnailUrl", "thumbnailURL")
                        )
                        if url_or_none(url)
                    ],
                    ...
                    "filesize": int_or_none(
                        float_or_none(e.get("contentSize"))
                    ),
                ...
                if is_type(e, "TVEpisode", "Episode", "PodcastEpisode"):
                    ...
                    if is_type(
                        ...
                        info.update(
                            {
                                "season": unescapeHTML(
                                    part_of_season.get("name")
                                ),
                                "season_number": int_or_none(
                                    part_of_season.get("seasonNumber")
                                ),
                            }
                        )
                    part_of_series = e.get("partOfSeries") or e.get(
                        "partOfTVSeries"
                    )
                    if is_type(
                        part_of_series,
                        ...
                        info["series"] = unescapeHTML(
                            part_of_series.get("name")
                        )
                    ...
                    info.update(
                        {
                            ...
                            "description": unescapeHTML(
                                e.get("articleBody") or e.get("description")
                            ),
                        }
                    )
    ...
    def _search_nextjs_data(
        self, webpage, video_id, *, fatal=True, default=NO_DEFAULT, **kw
    ):
        if default == "{}":
            self._downloader.deprecation_warning(
                "using `default='{}'` is deprecated, use `default={}` instead"
            )
            default = {}
        if default is not NO_DEFAULT:
            fatal = False
    ...
    def _search_nuxt_data(
        ...
        *,
        ...
        js, arg_keys, arg_vals = self._search_regex(
            (
                rf"<script>\s*window\.{rectx}={FUNCTION_RE}\s*\)\s*;?\s*</script>",
                rf"{rectx}\(.*?{FUNCTION_RE}",
            ),
            webpage,
            context_name,
        ...
        ret = self._parse_json(
            js,
            video_id,
            transform_source=functools.partial(js_to_json, vars=args),
            fatal=fatal,
        )
        ...
        if formats:
            formats[:] = filter(
                lambda f: self._is_valid_url(
                    ...
                    video_id,
                    item=(
                        "{} video format".format(f.get("format_id"))
                        if f.get("format_id")
                        else "video"
                    ),
                ),
        ...
        for f in formats:
            ...
            if callable(fragments):
                ...
                fragment_urls = frozenset(
                    fragment.get("url")
                    or urljoin(f["fragment_base_url"], fragment["path"])
    ...
    def _is_valid_url(self, url, video_id, item="video", headers={}):
        ...
        try:
            self._request_webpage(
                url, video_id, f"Checking {item} URL", headers=headers
            )
            return True
        except ExtractorError as e:
            self.to_screen(
    ...
    def http_scheme(self):
        ...
        return (
            "http:" if self.get_param("prefer_insecure", False) else "https:"
        )
    ...
    def _parse_f4m_formats(
        ...
        if (
            not isinstance(manifest, xml.etree.ElementTree.Element)
            and not fatal
        ):
            return []
        ...
        if not media_nodes:
            ...
            media_nodes = manifest.findall(
                "{http://ns.adobe.com/f4m/2.0}media"
            )
        ...
        bootstrap_info = xpath_element(
            manifest,
            [
                "{http://ns.adobe.com/f4m/1.0}bootstrapInfo",
                "{http://ns.adobe.com/f4m/2.0}bootstrapInfo",
            ],
            "bootstrap info",
            default=None,
        )
    ...
    def _m3u8_meta_format(
        self, m3u8_url, ext=None, preference=None, quality=None, m3u8_id=None
    ):
        return {
    ...
    def _parse_m3u8_formats_and_subtitles(
        ...
        def format_url(url):
            return (
                ...
                else urllib.parse.urljoin(m3u8_url, url)
            )

        if self.get_param("hls_split_discontinuity", False):

            def _extract_m3u8_playlist_indices(
                manifest_url=None, m3u8_doc=None
            ):
                if not m3u8_doc:
                    if not manifest_url:
                        return []
            ...
            formats = [
                {
                    ...
                    "url": m3u8_url
                    or encode_data_uri(
                        m3u8_doc.encode(), "application/x-mpegurl"
                    ),
                    "ext": ext,
        ...
        for line in m3u8_doc.splitlines():
            if line.startswith("#EXT-X-STREAM-INF:"):
                ...
                tbr = float_or_none(
                    ...
                    or last_stream_inf.get("BANDWIDTH"),
                    scale=1000,
                )
                ...
                for idx in _extract_m3u8_playlist_indices(manifest_url):
                    ...
                    f = {
                        ...
                        "ext": ext,
                        "fps": float_or_none(
                            last_stream_inf.get("FRAME-RATE")
                        ),
                        "protocol": entry_protocol,
                        "preference": preference,
                        "quality": quality,
                    ...
                    # YouTube-specific
                    if yt_audio_content_id := last_stream_inf.get(
                    ...
                    if resolution:
                        mobj = re.search(
                            r"(?P<width>\d+)[xX](?P<height>\d+)", resolution
                        )
                        if mobj:
                    ...
                    mobj = re.search(
                        r"audio.*?(?:%3D|=)(\d+)(?:-video.*?(?:%3D|=)(\d+))?",
                        f["url"],
                    )
                    if mobj:
                        abr, vbr = mobj.groups()
                        abr, vbr = float_or_none(abr, 1000), float_or_none(
                            vbr, 1000
                        )
                        f.update(
                            {
                    ...
                    if not f.get("ext"):
                        f["ext"] = (
                            "m4a" if f.get("vcodec") == "none" else "mp4"
                        )
                    formats.append(f)
    ...
    def _extract_m3u8_vod_duration(
        ...
        m3u8_vod = self._download_webpage(
            ...
            errnote=(
                "Failed to download VOD manifest"
                if errnote is None
                else errnote
            ),
            fatal=False,
    ...
    def _extract_mpd_vod_duration(
        ...
        if not isinstance(mpd_doc, xml.etree.ElementTree.Element):
            return None
        return int_or_none(
            parse_duration(mpd_doc.get("mediaPresentationDuration"))
        )
    ...
    def _extract_smil_formats_and_subtitles(
        ...
        res = self._download_smil(
            smil_url, video_id, fatal=fatal, transform_source=transform_source
        )
        if res is False:
            assert not fatal
            return [], {}
    ...
    def _extract_smil_info(
        self, smil_url, video_id, fatal=True, f4m_params=None
    ):
        ...
        return self._parse_smil(
            smil, smil_url, video_id, f4m_params=f4m_params
        )

    def _download_smil(
        self, smil_url, video_id, fatal=True, transform_source=None
    ):
    ...
    def _parse_smil_formats_and_subtitles(
        ...
        for medium in media:
            ...
            bitrate = float_or_none(
                medium.get("system-bitrate") or medium.get("systemBitrate"),
                1000,
            )
            filesize = int_or_none(
                medium.get("size") or medium.get("fileSize")
            )
            ...
            src_ext = (
                ...
                or urlhandle_detect_ext(
                    self._request_webpage(
                        HEADRequest(src),
                        video_id,
                        note="Requesting extension info",
                        fatal=False,
                    )
                )
            )
                ...
                formats.append(
                    {
                        ...
                        "ext": "flv",
                        "format_id": "rtmp-%d"
                        % (rtmp_count if bitrate is None else bitrate),
                        "tbr": bitrate,
                        "filesize": filesize,
                        "width": width,
                        "height": height,
            ...
            src_url = (
                ...
                else urllib.parse.urljoin(f"{base}/", src)
            )
            src_url = src_url.strip()

            if proto == "m3u8" or src_ext == "m3u8":
                m3u8_formats, m3u8_subs = (
                ...
                if len(m3u8_formats) == 1:
                    ...
                    m3u8_formats[0].update(
                        {
                            "format_id": "hls-%d"
                            % (m3u8_count if bitrate is None else bitrate),
                            "tbr": bitrate,
                            "width": width,
                            "height": height,
                        }
                ...
                ism_formats, ism_subs = (
                    self._extract_ism_formats_and_subtitles(
                        src_url, video_id, ism_id="mss", fatal=False
                    )
                )
                formats.extend(ism_formats)
                self._merge_subtitles(ism_subs, target=subtitles)
            elif src_url.startswith("http") and self._is_valid_url(
                src, video_id
            ):
                http_count += 1
                formats.append(
        ...
        for medium in smil.findall(
            self._xpath_ns(".//imagestream", namespace)
        ):
            src = medium.get("src")
            if not src or src in srcs:
                continue
    ...
    def _parse_smil_subtitles(self, smil, namespace=None, subtitles_lang="en"):
        urls = []
        subtitles = {}
        for textstream in smil.findall(
            self._xpath_ns(".//textstream", namespace)
        ):
            ...
            ext = (
                textstream.get("ext")
                or mimetype2ext(textstream.get("type"))
                or determine_ext(src)
            )
            lang = (
    ...
    def _parse_xspf(
        self, xspf_doc, playlist_id, xspf_url=None, xspf_base_url=None
    ):
        NS_MAP = {
        ...
        entries = []
        for track in xspf_doc.findall(
            xpath_with_ns("./xspf:trackList/xspf:track", NS_MAP)
        ):
            title = xpath_text(
                track,
            ...
            duration = float_or_none(
                xpath_text(
                    track, xpath_with_ns("./xspf:duration", NS_MAP), "duration"
                ),
                1000,
            )

            formats = []
            for location in track.findall(
                xpath_with_ns("./xspf:location", NS_MAP)
            ):
                ...
                formats.append(
                    {
                        ...
                        "format_id": location.get(
                            xpath_with_ns("s1:label", NS_MAP)
                        ),
                        "width": int_or_none(
                            location.get(xpath_with_ns("s1:width", NS_MAP))
                        ),
                        "height": int_or_none(
                            location.get(xpath_with_ns("s1:height", NS_MAP))
                        ),
                    }
                )
        ...
        for period in periods:
            for f in period["formats"]:
                ...
                if format_key not in formats:
                    ...
                    formats[format_key].setdefault("fragments", []).extend(
                        f["fragments"]
                    )
    ...
    def _parse_mpd_periods(
        ...
        def extract_multisegment_info(element, ms_parent_info):
            ...
            def extract_Initialization(source):
                ...
                if initialization is not None:
                    ms_info["initialization_url"] = initialization.attrib[
                        "sourceURL"
                    ]
            ...
            if segment_list is not None:
                ...
                if segment_urls_e:
                    ms_info["segment_urls"] = [
                        segment.attrib["media"] for segment in segment_urls_e
                    ]
            else:
        ...
        for period_idx, period in enumerate(
            mpd_doc.findall(_add_ns("Period"))
        ):
            period_entry = {
            ...
            period_duration = (
                parse_duration(period.get("duration")) or mpd_duration
            )
            ...
            for adaptation_set in period.findall(_add_ns("AdaptationSet")):
                ...
                for representation in adaptation_set.findall(
                    _add_ns("Representation")
                ):
                    representation_attrib = adaptation_set.attrib.copy()
                    representation_attrib.update(representation.attrib)
                    # According to [1, 5.3.7.2, Table 9, page 41], @mimeType is mandatory
                    mime_type = representation_attrib["mimeType"]
                    content_type = representation_attrib.get(
                    ...
                    if content_type not in ("video", "audio", "text"):
                        if mime_type == "image/jpeg":
                            ...
                            content_type = "text"
                        elif mimetype2ext(mime_type) in (
                            "tt",
                            "dfxp",
                            "ttml",
                            "xml",
                            ...
                            self.report_warning(
                                f"Unknown MIME type {mime_type} in DASH manifest"
                            )
                            continue
                    ...
                    if content_type in ("video", "audio"):
                        f = {
                            ...
                            "width": int_or_none(
                                representation_attrib.get("width")
                            ),
                            "height": int_or_none(
                                representation_attrib.get("height")
                            ),
                            ...
                            "fps": int_or_none(
                                representation_attrib.get("frameRate")
                            ),
                            "language": (
                                lang
                    ...
                    if is_drm_protected(adaptation_set) or is_drm_protected(
                        representation
                    ):
                        f["has_drm"] = True
                    ...
                    if "initialization" in representation_ms_info:
                        ...
                        representation_ms_info["initialization_url"] = (
                    ...
                    if (
                        ...
                        if (
                            ...
                            segment_time = 0
                            segment_d = None
                            segment_number = representation_ms_info[
                                "start_number"
                            ]
                        ...
                        for segment_url in representation_ms_info[
                            "segment_urls"
                        ]:
                            fragment = {
                    ...
                    if "fragments" in representation_ms_info:
                        ...
                        if not period_duration:
                            period_duration = try_get(
                                representation_ms_info,
                                lambda r: sum(
                                    frag["duration"] for frag in r["fragments"]
                                ),
                                float,
                            )
                    else:
    ...
    def _parse_ism_formats_and_subtitles(self, ism_doc, ism_url, ism_id=None):
        ...
        for stream in ism_doc.findall("StreamIndex"):
            ...
            stream_timescale = (
                int_or_none(stream.get("TimeScale")) or timescale
            )
            ...
            if len(stream_language) != 3:
                stream_language = (
                    ISO639Utils.short2long(stream_language) or "und"
                )
            for track in stream.findall("QualityLevel"):
                KNOWN_TAGS = {"255": "AACL", "65534": "EC-3"}
                fourcc = track.get("FourCC") or KNOWN_TAGS.get(
                    track.get("AudioTag")
                )
                ...
                width = int_or_none(
                    track.get("MaxWidth") or track.get("Width")
                )
                height = int_or_none(
                    track.get("MaxHeight") or track.get("Height")
                )
                sampling_rate = int_or_none(track.get("SamplingRate"))

                track_url_pattern = re.sub(
                    r"{[Bb]itrate}", track.attrib["Bitrate"], url_pattern
                )
                track_url_pattern = urllib.parse.urljoin(
                    ism_url, track_url_pattern
                )
                ...
                for stream_fragment_index, stream_fragment in enumerate(
                    stream_fragments
                ):
                    fragment_ctx["time"] = (
                        int_or_none(stream_fragment.get("t"))
                        or fragment_ctx["time"]
                    )
                    fragment_repeat = (
                        int_or_none(stream_fragment.get("r")) or 1
                    )
                    fragment_ctx["duration"] = int_or_none(
                        stream_fragment.get("d")
                    )
                    if not fragment_ctx["duration"]:
                        try:
                            next_fragment_time = int(
                                stream_fragment[
                                    stream_fragment_index + 1
                                ].attrib["t"]
                            )
                        except IndexError:
                            next_fragment_time = duration
                        fragment_ctx["duration"] = (
                            next_fragment_time - fragment_ctx["time"]
                        ) / fragment_repeat
                        ...
                        fragments.append(
                            {
                                "url": re.sub(
                                    r"{start[ _]time}",
                                    str(fragment_ctx["time"]),
                                    track_url_pattern,
                                ),
                                "duration": fragment_ctx["duration"]
                                / stream_timescale,
                            }
                        )
                ...
                if stream_type == "text":
                    ...
                    formats.append(
                        {
                            "format_id": join_nonempty(
                                ism_id, stream_name, tbr
                            ),
                            "url": ism_url,
                            "manifest_url": ism_url,
                            "ext": (
                                "ismv" if stream_type == "video" else "isma"
                            ),
                            "width": width,
                            "height": height,
                            "tbr": tbr,
                            "asr": sampling_rate,
                            "vcodec": (
                                "none" if stream_type == "audio" else fourcc
                            ),
                            "acodec": (
                                "none" if stream_type == "video" else fourcc
                            ),
                            "protocol": "ism",
                            "fragments": fragments,
                            "has_drm": ism_doc.find("Protection") is not None,
                            "language": stream_language,
                            "audio_channels": int_or_none(
                                track.get("Channels")
                            ),
                            "_download_params": {
                                "stream_type": stream_type,
                                ...
                                "bits_per_sample": int_or_none(
                                    track.get("BitsPerSample", 16)
                                ),
                                "nal_unit_length_field": int_or_none(
                                    track.get("NALUnitLengthField", 4)
                                ),
                            },
                        }
                    )
    ...
    def _parse_html5_media_entries(
                ...
                formats = [
                    {
                        "url": full_url,
                        "vcodec": (
                            "none" if cur_media_type == "audio" else None
                        ),
                        "ext": ext,
                    }
                ]
        ...
        media_tags = [
            ...
            for media_tag, media_tag_name, media_type in re.findall(
                rf"(?s)(<({_MEDIA_TAG_NAME_RE})[^>]*/>)", webpage
            )
        ]
        media_tags.extend(
            re.findall(
        ...
        for media_tag, _, media_type, media_content in media_tags:
            ...
            media_info["thumbnail"] = absolute_url(
    ...
    def _extract_akamai_formats_and_subtitles(
        self, manifest_url, video_id, hosts={}
    ):
        ...
        m3u8_url = re.sub(
            r"(https?://[^/]+)/z/", r"\1/i/", manifest_url
        ).replace("/manifest.f4m", "/master.m3u8")
        hls_host = hosts.get("hls")
        if hls_host:
            m3u8_url = re.sub(r"(https?://)[^/]+", r"\1" + hls_host, m3u8_url)
        m3u8_formats, m3u8_subtitles = (
            self._extract_m3u8_formats_and_subtitles(
                m3u8_url,
                video_id,
                "mp4",
                    ...
                    if f["vcodec"] != "none":
                        for protocol in ("http", "https"):
                            ...
                            http_url = re.sub(
                                REPL_REGEX,
                                protocol
                                + rf"://{http_host}/\g<1>{qualities[i]}\3",
                                f["url"],
                            )
                            http_f.update(
                                {
                                    "format_id": http_f["format_id"].replace(
                                        "hls-", protocol + "-"
                                    ),
                                    "url": http_url,
                                    "protocol": protocol,
    ...
    def _extract_wowza_formats(
        ...
        url = re.sub(
            r"/(?:manifest|playlist|jwplayer)\.(?:m3u8|f4m|mpd|smil)", "", url
        )
        mobj = re.search(
            r"(?:(?:http|rtmp|rtsp)(?P<s>s)?:)?(?P<url>//[^?]+)", url
        )
        url_base = mobj.group("url")
        http_base_url = "{}{}:{}".format(
            "http", mobj.group("s") or "", url_base
        )
        formats = []
        ...
        if re.search(r"(?:/smil:|\.smil)", url_base):
            if "smil" not in skip_protocols:
                rtmp_formats = self._extract_smil_formats(
                    manifest_url("jwplayer.smil"), video_id, fatal=False
                )
                for rtmp_format in rtmp_formats:
                    rtsp_format = rtmp_format.copy()
                    rtsp_format["url"] = "{}/{}".format(
                        rtmp_format["url"], rtmp_format["play_path"]
                    )
                    del rtsp_format["play_path"]
                    del rtsp_format["ext"]
                    rtsp_format.update(
                        {
                            "url": rtsp_format["url"].replace(
                                "rtmp://", "rtsp://"
                            ),
                            "format_id": rtmp_format["format_id"].replace(
                                "rtmp", "rtsp"
                            ),
                            "protocol": "rtsp",
                        }
    ...
    def _find_jwplayer_data(
        self, webpage, video_id=None, transform_source=js_to_json
    ):
        return self._search_json(
    ...
    def _extract_jwplayer_data(
        self, webpage, video_id, *args, transform_source=js_to_json, **kwargs
    ):
    ...
    def _parse_jwplayer_data(
        ...
        for video_data in playlist_items:
            ...
            entry = {
                ...
                "thumbnail": urljoin(
                    base_url, self._proto_relative_url(video_data.get("image"))
                ),
                "timestamp": int_or_none(video_data.get("pubdate")),
                "duration": float_or_none(
                    jwplayer_data.get("duration") or video_data.get("duration")
                ),
                "subtitles": subtitles,
                "alt_title": clean_html(
                    video_data.get("subtitle")
                ),  # attributes used e.g. by Tele5 ...
                "genre": clean_html(video_data.get("genre")),
                "channel": clean_html(
                    dict_get(video_data, ("category", "channel"))
                ),
            ...
            if len(formats) == 1 and re.search(
                r"^(?:http|//).*(?:youtube\.com|youtu\.be)/.+",
                formats[0]["url"],
            ):
                entry.update(
                    {
    ...
    def _parse_jwplayer_formats(
        ...
        for source in jwplayer_sources_data:
            ...
            ext = determine_ext(
                source_url, default_ext=mimetype2ext(source_type)
            )
            if (
                source_type == "hls"
                or ext == "m3u8"
                or "format=m3u8-aapl" in source_url
            ):
                formats.extend(
                    self._extract_m3u8_formats(
                        source_url,
                ...
                source_type == "dash"
                or ext == "mpd"
                or "format=mpd-time-csf" in source_url
            ):
                formats.extend(
                    self._extract_mpd_formats(
    ...
    def _get_cookies(self, url):
        ...
        return LenientSimpleCookie(
            self._downloader.cookiejar.get_cookie_header(url)
        )
    ...
    def get_testcases(cls, include_onlymatching=False):
        ...
        if t:
            assert not hasattr(
                cls, "_TESTS"
            ), f"{cls.ie_key()}IE has _TEST and _TESTS"
            tests = [t]
        else:
    ...
    def age_limit(cls):
        ...
        return max(
            traverse_obj(
                (
                    *cls.get_testcases(include_onlymatching=False),
                    *cls.get_webpage_testcases(),
                ),
    ...
    def _RETURN_TYPE(cls):
        ...
        if not tests:
            ...
            any(k.startswith("playlist") for k in test) for test in tests
        ):
            return "playlist"
        return "any"
    ...
    def extract_subtitles(self, *args, **kwargs):
        if self.get_param("writesubtitles", False) or self.get_param(
            "listsubtitles"
        ):
    ...
    def extract_automatic_captions(self, *args, **kwargs):
        if self.get_param("writeautomaticsub", False) or self.get_param(
            "listsubtitles"
        ):
    ...
    def _cookies_passed(self):
        ...
        return (
            self.get_param("cookiefile") is not None
            or self.get_param("cookiesfrombrowser") is not None
        )
    ...
    def mark_watched(self, *args, **kwargs):
        if not self.get_param("mark_watched", False):
            return
        if (
            self.supports_login() and self._get_login_info()[0] is not None
        ) or self._cookies_passed:
    ...
    def _generic_id(url):
        return urllib.parse.unquote(
            os.path.splitext(url.rstrip("/").split("/")[-1])[0]
        )
    ...
    def _extract_chapters_helper(
        ...
        for idx, chapter in enumerate(chapter_list):
            if chapter["start_time"] is None:
                ...
                chapters[-1]["start_time"] <= chapter["start_time"] <= duration
            ):
                ...
                if needs_premium
                    ...
                    if needs_subscription
                        ...
                        if needs_auth
                            ...
                            "unlisted"
                            if is_unlisted
                            else "public" if all_known else None
                        )
                    )
                )
            )
    ...
    def _configuration_arg(
        self, key, default=NO_DEFAULT, *, ie_key=None, casesense=False
    ):
        """
        ...
        """
        ie_key = (
            ie_key if isinstance(ie_key, str) else (ie_key or self).ie_key()
        )
        val = traverse_obj(
            self._downloader.params, ("extractor_args", ie_key.lower(), key)
        )
        if val is None:
    ...
    def _error_or_warning(self, err, _count=None, _retries=0, *, fatal=True):
        RetryManager.report_retry(
            ...
            sleep_func=self.get_param("retry_sleep_functions", {}).get(
                "extractor"
            ),
        )
    ...
    def RetryManager(self, **kwargs):
        return RetryManager(
            self.get_param("extractor_retries", 3),
    ...
    def _extract_generic_embeds(
        ...
        *args,
        info_dict={},
        note="Extracting generic embeds",
        **kwargs,
    ):
    ...
    def extract_from_webpage(cls, ydl, url, webpage):
        ie = (
            cls
            if isinstance(cls._extract_from_webpage, types.MethodType)
    ...
    def _extract_from_webpage(cls, url, webpage):
        for embed_url in orderedSet(
            ...
            yield cls.url_result(
                embed_url, None if cls._VALID_URL is False else cls
            )
    ...
    def __init_subclass__(cls, *, plugin_name=None, **kwargs):
        if plugin_name:
            ...
            if not any(
                override.PLUGIN_NAME == plugin_name
                for override in plugin_ies_overrides.value[super_class]
            ):
                cls.__wrapped__ = next_mro_class
                cls.PLUGIN_NAME, cls.ie_key = (
                    plugin_name,
                    next_mro_class.ie_key,
                )
                cls.IE_NAME = f"{next_mro_class.IE_NAME}+{plugin_name}"

                setattr(
                    sys.modules[super_class.__module__],
...
class SearchInfoExtractor(InfoExtractor):
    ...
    def _real_extract(self, query):
        ...
        if prefix == "":
            ...
            if n <= 0:
                raise ExtractorError(
                    f'invalid download number {n} for query "{query}"'
                )
            elif n > self._MAX_RESULTS:
                self.report_warning(
                    "%s returns max %i results (you requested %i)"
                    % (self._SEARCH_KEY, self._MAX_RESULTS, n)
                )
                n = self._MAX_RESULTS
