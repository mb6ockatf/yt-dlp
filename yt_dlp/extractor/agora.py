...
class WyborczaVideoIE(InfoExtractor):
    ...
    _VALID_URL = (
        r"(?:wyborcza:video:|https?://wyborcza\.pl/(?:api-)?video/)(?P<id>\d+)"
    )
    ...
    def _real_extract(self, url):
        ...
        formats = []
        base_url = (
            meta["redirector"].replace("http://", "https://")
            + meta["basePath"]
        )
...
class TokFMPodcastIE(InfoExtractor):
    _VALID_URL = (
        r"(?:https?://audycje\.tokfm\.pl/podcast/|tokfm:podcast:)(?P<id>\d+),?"
    )
...
class TokFMAuditionIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        entries = OnDemandPagedList(
            functools.partial(self._fetch_page, audition_id, data),
            self._PAGE_SIZE,
        )
    ...
    def _fetch_page(self, audition_id, data, page):
        for retry in self.RetryManager():
            podcast_page = self._download_json(
                ...
                audition_id,
                f"Downloading podcast list page {page + 1}",
                headers=self._HEADERS,
            )
            if not podcast_page:
