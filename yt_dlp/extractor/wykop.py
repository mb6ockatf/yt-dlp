    ...
    def _get_token(self, force_refresh=False):
        ...
        new_token = traverse_obj(
            self._do_call_api(
                "auth",
                None,
                "Downloading anonymous auth token",
                data={
    ...
    def _common_data_extract(self, data):
        ...
        return {
            ...
            "uploader_url": format_field(
                author, None, "https://wykop.pl/ludzie/%s"
            ),
            "timestamp": parse_iso8601(
                data.get("created_at"), delimiter=" "
            ),  # time it got submitted
            "like_count": traverse_obj(
                data, ("votes", "up"), expected_type=int
            ),
            "dislike_count": traverse_obj(
                data, ("votes", "down"), expected_type=int
            ),
            "comment_count": traverse_obj(
                data, ("comments", "count"), expected_type=int
            ),
...
class WykopDigIE(WykopBaseExtractor):
    ...
    def suitable(cls, url):
        return cls._match_valid_url(url) and not WykopDigCommentIE.suitable(
            url
        )
        ...
        return {
            **self._common_data_extract(data),
            ...
            "release_timestamp": parse_iso8601(
                data.get("published_at"), delimiter=" "
            ),
        }
...
class WykopDigCommentIE(WykopBaseExtractor):
    ...
    def _real_extract(self, url):
        dig_id, comment_id = self._search_regex(
            self._VALID_URL, url, "dig and comment ids", group=("dig_id", "id")
        )
        data = self._call_api(
            f"links/{dig_id}/comments/{comment_id}", comment_id
        )["data"]
...
class WykopPostIE(WykopBaseExtractor):
    ...
    @classmethod
    def suitable(cls, url):
        return cls._match_valid_url(url) and not WykopPostCommentIE.suitable(
            url
        )
...
class WykopPostCommentIE(WykopBaseExtractor):
    ...
    def _real_extract(self, url):
        ...
        data = self._call_api(
            f"entries/{post_id}/comments/{comment_id}", comment_id
        )["data"]
