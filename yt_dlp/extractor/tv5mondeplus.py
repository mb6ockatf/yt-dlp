...
class TV5MondePlusIE(InfoExtractor):
    ...
    def _extract_subtitles(data_captions):
        ...
        for f in traverse_obj(
            data_captions, ("files", lambda _, v: url_or_none(v["file"]))
        ):
            subtitles.setdefault(f.get("label") or "fra", []).append(
                {"url": f["file"]}
            )
        return subtitles
        ...
        return {
            **traverse_obj(
            ...
            "description": clean_html(
                get_element_by_class(
                    "text",
                    get_element_html_by_class("ep-summary", webpage) or "",
                )
            ),
            ...
            "subtitles": self._extract_subtitles(
                self._parse_json(
                    traverse_obj(
                        vpl_data, ("data-captions", {str}), default="{}"
                    ),
                    display_id,
                    fatal=False,
                )
