...
class WDRIE(InfoExtractor):
    ...
    def _asset_url(self, wdr_id):
        ...
        return "".join(
            (
                "https:",
                self.__API_URL_TPL % (wdr_id[: id_len - 4], wdr_id),
                ".js",
            )
        )
...
class WDRPageIE(WDRIE):  # XXX: Do not subclass from concrete IE
    ...
    _VALID_URL = (
        r"https?://(?:www\d?\.)?(?:(?:kinder\.)?wdr\d?|sportschau)\.de"
        + _PAGE_REGEX
        + "|"
        + _MAUS_REGEX
    )
    ...
    def _real_extract(self, url):
        ...
        for mobj in re.finditer(
            ...
            if jsonp_url:
                ...
                if jsonp_url.endswith(".assetjsonp"):
                    ...
                    clip_id = try_get(
                        asset, lambda x: x["trackerData"]["trackerClipId"], str
                    )
                if clip_id:
        ...
        if not entries:
            entries = [
                ...
                if re.match(self._PAGE_REGEX, mobj.group("href"))
            ]
...
class WDRElefantIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if zmdb_url_element is None:
            raise ExtractorError(f"{display_id} is not a video", expected=True)
