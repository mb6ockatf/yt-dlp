...
class URPlayIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        urplayer_data = (
            self._search_nextjs_data(webpage, video_id, fatal=False) or {}
        )
        if urplayer_data:
            ...
            accessible_episodes = self._parse_json(
            ...
            urplayer_data = next(
                e
                for e in accessible_episodes
                if e.get("id") == int_or_none(video_id)
            )
        episode = urplayer_data["title"]

        host = self._download_json(
            "http://streaming-loadbalancer.ur.se/loadbalancer.json", video_id
        )["redirect"]
        formats = []
        ...
        series_title = dict_get(series, ("seriesTitle", "title")) or dict_get(
        ...
        return {
            "id": video_id,
            "title": (
                f"{series_title} : {episode}" if series_title else episode
            ),
            ...
            "age_limit": parse_age_limit(
                min(
                    try_get(a, lambda x: x["from"], int) or 0
