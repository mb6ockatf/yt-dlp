...
class RTVCPlayBaseIE(InfoExtractor):
    ...
    def _extract_player_config(self, webpage, video_id):
        return self._search_json(
            r"<script\b[^>]*>[^<]*(?:var|let|const)\s+config\s*=",
            re.sub(r'"\s*\+\s*"', "", webpage),
            "player_config",
            video_id,
            transform_source=js_to_json,
        )

    def _extract_formats_and_subtitles_player_config(
        self, player_config, video_id
    ):
        formats, subtitles = [], {}
        for source in traverse_obj(
            player_config, ("sources", ..., lambda _, v: url_or_none(v["url"]))
        ):
            ext = mimetype2ext(
                source.get("mimetype"), default=determine_ext(source["url"])
            )
            if ext == "m3u8":
...
class RTVCPlayIE(RTVCPlayBaseIE):
    _VALID_URL = (
        RTVCPlayBaseIE._BASE_VALID_URL
        + r"/(?P<category>(?!embed)[^/]+)/(?:[^?#]+/)?(?P<id>[\w-]+)"
    )
    ...
    def _real_extract(self, url):
        ...
        if asset_id:
            hls_url = hydration["base_url_hls"].replace(
                "[node:field_asset_id]", asset_id
            )
        else:
            ...
            if not seasons:
                ...
                if not podcast_episodes:
                    raise ExtractorError(
                        "Could not find asset_id nor program playlist nor podcast episodes"
                    )
...
class RTVCPlayEmbedIE(RTVCPlayBaseIE):
    ...
    def _real_extract(self, url):
        ...
        formats, subtitles = self._extract_formats_and_subtitles_player_config(
        ...
        metadata = (
            {}
            if not asset_id
            else self._download_json(
                f"https://cms.rtvcplay.co/api/v1/video/asset-id/{asset_id}",
                video_id,
                fatal=False,
            )
        )
    ...
    def _real_extract(self, url):
        ...
        metadata = (
            ...
            if not channel_id
                ...
                f"https://cms.rtvcplay.co/api/v1/taxonomy_term/streaming/{channel_id}",
                video_id,
                fatal=False,
            )
        )
        ...
        fmts, subs = self._extract_m3u8_formats_and_subtitles(
            traverse_obj(metadata, ("channel", "hls")),
            video_id,
            "mp4",
            fatal=False,
        )
