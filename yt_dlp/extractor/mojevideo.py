...
class MojevideoIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        video_id_dec = self._search_regex(
            r"\bvId\s*=\s*(\d+)", webpage, "video id", fatal=False
        ) or str(int(video_id, 16))
        ...
        formats = []
        for video_hash, (suffix, quality, format_note) in zip(
            video_hashes,
            [
                ("", 1, "normálna kvalita"),
                ("_lq", 0, "nízka kvalita"),
        ...
        return {
            ...
            "title": (
                ...
                or remove_end(
                    self._html_extract_title(webpage, "title"), " - Mojevideo"
                )
            ),
