...
class OnDemandKoreaIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        data = self._download_json(
            f"https://odkmedia.io/odx/api/v3/playback/{video_id}/",
            video_id,
            fatal=False,
            headers={"service-name": "odk"},
            query={"did": str(uuid.uuid4())},
            expected_status=(403, 404),
        )
        if not traverse_obj(data, ("result", {dict})):
            msg = traverse_obj(
                data, ("messages", "__default"), "title", expected_type=str
            )
            raise ExtractorError(
                msg or "Got empty response from playback API", expected=True
            )
        ...
        def try_geo_bypass(url):
            return (
                traverse_obj(url, ({parse_qs}, "stream_url", 0, {url_or_none}))
                or url
            )
        ...
        for m3u8_url in traverse_obj(
            ...
            if mod_url != m3u8_url:
                mod_format = self._extract_m3u8_formats(
                    mod_url,
                    video_id,
                    note="Checking for higher quality format",
                    errnote="No higher quality format found",
                    fatal=False,
                )
                if mod_format:
                    ...
                    continue
            formats.extend(
                self._extract_m3u8_formats(m3u8_url, video_id, fatal=False)
            )

        subtitles = {}
        for track in traverse_obj(
            data, ("text_tracks", lambda _, v: url_or_none(v["url"]))
        ):
        ...
        def if_series(key=None):
            return lambda obj: (
                obj[key] if key and obj["kind"] == "series" else None
            )
...
class OnDemandKoreaProgramIE(InfoExtractor):
    ...
    def _fetch_page(self, display_id, page):
        ...
        page_data = self._download_json(
            f"https://odkmedia.io/odx/api/v3/program/{display_id}/episodes/",
            display_id,
            headers={"service-name": "odk"},
            query={
                "page": page,
    ...
    def _real_extract(self, url):
        ...
        entries = OnDemandPagedList(
            functools.partial(self._fetch_page, display_id), self._PAGE_SIZE
        )
