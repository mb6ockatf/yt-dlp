    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "release_date": unified_strdate(
                get_element_by_class(
                    "date",
                    get_element_html_by_class("descriptions", webpage) or "",
                )
            ),
            "thumbnail": self._og_search_thumbnail(webpage),
            "season_number": int_or_none(season),
            "url": extract_attributes(
                get_element_html_by_class("audiofile", webpage) or ""
            ).get("href"),
        }
...
class RadioComercialPlaylistIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        name = try_call(
            lambda: get_element_text_and_html_by_tag("h1", webpage)[0]
        )
        title = (
            name
            if name == season
            else join_nonempty(name, season, delim=" - Temporada ")
        )

        return self.playlist_from_matches(
            self._entries(url, playlist_id),
            playlist_id,
            title,
            ie=RadioComercialIE,
        )
