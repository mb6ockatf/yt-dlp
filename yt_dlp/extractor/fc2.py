...
class FC2IE(InfoExtractor):
    _VALID_URL = (
        r"(?:https?://video\.fc2\.com/(?:[^/]+/)*content/|fc2:)(?P<id>[^/]+)"
    )
    IE_NAME = "fc2"
    _NETRC_MACHINE = "fc2"
    _TESTS = [
    ...
    def _real_extract(self, url):
        ...
        vidplaylist = self._download_json(
            f"https://video.fc2.com/api/v3/videoplaylist/{video_id}?sh=1&fs=0",
            video_id,
...
class FC2LiveIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        webpage = self._download_webpage(
            f"https://live.fc2.com/{video_id}/", video_id
        )
        ...
        control_server = self._download_json(
            ...
            video_id,
            note="Downloading ControlServer data",
            data=urlencode_postdata(
                {
                    "channel_id": video_id,
                    "mode": "play",
                    "orz": "",
                    "channel_version": member_api["data"]["channel_data"][
                        "version"
                    ],
        ...
        ws_url = update_url_query(
            control_server["url"],
            {"control_token": control_server["control_token"]},
        )
        playlist_data = None
        ...
        ws = self._request_webpage(
            ...
            video_id,
            note="Fetching HLS playlist info via WebSocket",
        )
        ...
        while True:
            ...
            self.write_debug(
                "Server said: {}{}".format(
                    recv[:100], "..." if len(recv) > 100 else ""
                )
            )

        if not playlist_data:
            raise ExtractorError(
                "Unable to fetch HLS playlist info via WebSocket"
            )
        ...
        for name, playlists in playlist_data["arguments"].items():
            if not isinstance(playlists, list):
                continue
            for pl in playlists:
                if pl.get("status") == 0 and "master_playlist" in pl.get(
                    "url"
                ):
                    formats.extend(
        ...
        if not title:
            title = self._html_extract_title(
                webpage, "html title", fatal=False
            )
            if title:
        ...
        live_info_view = (
            self._search_regex(
                r"(?s)liveInfoView\s*:\s*({.+?}),\s*premiumStateView",
                webpage,
                "user info",
                fatal=False,
            )
        ...
        if live_info_view:
            ...
            live_info_view = self._parse_json(
                js_to_json(live_info_view), video_id
            )
        ...
        return {
            ...
            or traverse_obj(live_info_view, "info"),
            "formats": formats,
