                ...
                formats.append(
                    {
                        "url": self._search_regex(
                            r"File1=(.*)", playlist_content, "url", fatal=False
                        ),
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "description": traverse_obj(
                metadata, ("profiles", station_id, "description")
            ),
            "thumbnail": traverse_obj(
                metadata, ("profiles", station_id, "image")
            ),
            "timestamp": parse_iso8601(
                traverse_obj(
                    metadata,
                    ("profiles", station_id, "actions", "play", "publishTime"),
                )
            ),
            ...
            "is_live": traverse_obj(
                metadata, ("profiles", station_id, "actions", "play", "isLive")
            ),
        }
...
class TuneInPodcastIE(TuneInBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            ...
            title=traverse_obj(metadata, ("profiles", podcast_id, "title")),
            description=traverse_obj(
...
class TuneInPodcastEpisodeIE(TuneInBaseIE):
    ...
    def _real_extract(self, url):
        podcast_id, episode_id = self._match_valid_url(url).group(
            "podcast_id", "id"
        )
        ...
        return {
            ...
            "description": traverse_obj(
                metadata, ("profiles", episode_id, "description")
            ),
            "thumbnail": traverse_obj(
                metadata, ("profiles", episode_id, "image")
            ),
            "timestamp": parse_iso8601(
                traverse_obj(
                    metadata,
                    ("profiles", episode_id, "actions", "play", "publishTime"),
                )
            ),
            "series_id": podcast_id,
            "series": traverse_obj(
                metadata, ("profiles", podcast_id, "title")
            ),
            "formats": formats,
            "subtitles": subtitles,
        }
