...
class YandexMusicTrackIE(YandexMusicBaseIE):
    ...
    def _real_extract(self, url):
        ...
        key = hashlib.md5(
            (
                "XGRlBW9FXlekgbPrRHuSiA" + fd_data["path"][1:] + fd_data["s"]
            ).encode()
        ).hexdigest()
        f_url = "http://{}/get-mp3/{}/{}?track-id={} ".format(
            fd_data["host"], key, fd_data["ts"] + fd_data["path"], track["id"]
        )
        ...
        if albums and isinstance(albums, list):
            ...
            if isinstance(album, dict):
                year = album.get("year")
                disc_number = int_or_none(
                    try_get(album, lambda x: x["trackPosition"]["volume"])
                )
                track_number = int_or_none(
                    try_get(album, lambda x: x["trackPosition"]["index"])
                )
                track_info.update(
                    {
...
class YandexMusicPlaylistBaseIE(YandexMusicBaseIE):
    def _extract_tracks(self, source, item_id, url, tld):
        ...
        if len(tracks) < len(track_ids):
            present_track_ids = {
                str(track["id"]) for track in tracks if track.get("id")
            }
            missing_track_ids = [
                track_id
...
class YandexMusicAlbumIE(YandexMusicPlaylistBaseIE):
    ...
    def suitable(cls, url):
        return (
            False
            if YandexMusicTrackIE.suitable(url)
    ...
    def _real_extract(self, url):
        ...
        entries = self._build_playlist(
            [track for volume in album["volumes"] for track in volume]
        )
...
class YandexMusicArtistTracksIE(YandexMusicArtistBaseIE):
    ...
    _VALID_URL = (
        rf"{YandexMusicBaseIE._VALID_URL_BASE}/artist/(?P<id>\d+)/tracks"
    )
...
class YandexMusicArtistAlbumsIE(YandexMusicArtistBaseIE):
    ...
    _VALID_URL = (
        rf"{YandexMusicBaseIE._VALID_URL_BASE}/artist/(?P<id>\d+)/albums"
    )
