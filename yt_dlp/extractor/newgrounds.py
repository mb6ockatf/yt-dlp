...
class NewgroundsIE(InfoExtractor):
    ...
    def _perform_login(self, username, password):
        ...
        if errors := traverse_obj(result, ("errors", ..., {str})):
            raise ExtractorError(
                ", ".join(errors) or "Unknown Error", expected=True
            )
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "age_limit": self._AGE_LIMIT.get(
                self._html_search_regex(
                    r'<h2\s+class=["\']rated-([etma])["\']',
                    webpage,
                    "age_limit",
                    default="e",
                )
...
class NewgroundsUserIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        entries = OnDemandPagedList(
            functools.partial(self._fetch_page, channel_id, url),
            self._PAGE_SIZE,
        )
