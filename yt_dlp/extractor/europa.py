...
class EuropaIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        playlist = self._download_xml(
            f"http://ec.europa.eu/avservices/video/player/playlist.cfm?ID={video_id}",
            video_id,
        )
        ...
        upload_date = unified_strdate(
            xpath_text(playlist, "./info/date", "upload date")
        )
        duration = parse_duration(
            xpath_text(playlist, "./info/duration", "duration")
        )
...
class EuroParlWebstreamIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        webpage_nextjs = self._search_nextjs_data(webpage, display_id)[
            "props"
        ]["pageProps"]

        json_info = self._download_json(
            "https://acs-api.europarl.connectedviews.eu/api/FullMeeting",
            display_id,
            query={
                "api-version": 1.0,
        ...
        for hls_url in traverse_obj(
            json_info, ((("meetingVideo"), ("meetingVideos", ...)), "hlsUrl")
        ):
        ...
        return {
            ...
            "is_live": traverse_obj(
                webpage_nextjs, ("mediaItem", "mediaSubType")
            )
            == "Live",
        }
