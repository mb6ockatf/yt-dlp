...
class VideoKenBaseIE(InfoExtractor):
    ...
    _BASE_URL_RE = (
        rf'https?://(?P<host>{"|".join(map(re.escape, _ORGANIZATIONS))})/'
    )
    ...
    def _real_extract(self, url):
        ...
        # fallback for API error 400 response
            ...
            return self.url_result(
                self._create_slideslive_url(None, video_id, url),
                SlidesLiveIE,
                video_id,
            )
...
class VideoKenPlaylistIE(VideoKenBaseIE):
    _VALID_URL = (
        VideoKenBaseIE._BASE_URL_RE + r"(?:category/\d+/)?playlist/(?P<id>\d+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._extract_videos(videos, url), playlist_id, videos.get("title")
        )


class VideoKenCategoryIE(VideoKenBaseIE):
    _VALID_URL = (
        VideoKenBaseIE._BASE_URL_RE + r"category/(?P<id>\d+)/?(?:$|[?#])"
    )
    _TESTS = [
        {
...
class VideoKenTopicIE(VideoKenBaseIE):
    _VALID_URL = (
        VideoKenBaseIE._BASE_URL_RE + r"topic/(?P<id>[^/#?]+)/?(?:$|[?#])"
    )
    _TESTS = [
        {
    ...
    def _get_topic_page(
        self, topic, org_id, search_id, api_key, page=1, note=None
    ):
        return (
            self._download_json(
                ...
                headers={"Accept": "application/json"},
                note=note if note else f"Downloading topic page {page}",
            )
            or {}
        )
    ...
    def _real_extract(self, url):
        ...
        search_id = base64.b64encode(
            f":{topic}:{int(time.time())}:transient".encode()
        ).decode()
        total_pages = int_or_none(
            self._get_topic_page(
                ...
                search_id,
                api_key,
                note="Downloading topic info",
            )["total_no_of_pages"]
        )
        return self.playlist_result(
            InAdvancePagedList(
                functools.partial(
                    self._entries, topic, org_id, search_id, api_key, url
                ),
                total_pages,
                self._PAGE_SIZE,
            ),
