...
class DropoutIE(InfoExtractor):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?dropout\.tv/(?:[^/]+/)*videos/(?P<id>[^/]+)/?$"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "url": VHXEmbedIE._smuggle_referrer(
                embed_url, "https://www.dropout.tv"
            ),
            "id": self._search_regex(
                r"embed\.vhx\.tv/videos/(.+?)\?", embed_url, "id"
            ),
            ...
            "thumbnail": (
                thumbnail.split("?")[0] if thumbnail else None
            ),  # Ignore crop/downscale
            "series": clean_html(
                get_element_by_class("series-title", watch_info)
            ),
...
class DropoutSeasonIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            OnDemandPagedList(
                functools.partial(self._fetch_page, url, season_id),
                self._PAGE_SIZE,
            ),
