...
class GlomexBaseIE(InfoExtractor):
    ...
    def _download_api_data(self, video_id, integration, current_url=None):
        ...
        return self._download_json(
            self._API_URL,
            video_id,
            f"Downloading {video_id_type} JSON",
    ...
    def _download_and_extract_api_data(
        ...
        return (
            videos[0]
            if len(videos) == 1
            else self.playlist_result(videos, video_id)
        )
    ...
    def _real_extract(self, url):
        ...
        return self.url_result(
            GlomexEmbedIE.build_player_url(
                video_id, self._INTEGRATION_ID, url
            ),
...
class GlomexEmbedIE(GlomexBaseIE):
    ...
    _VALID_URL = (
        rf"https?:{_BASE_PLAYER_URL_RE}\?([^#]+&)?playlistId=(?P<id>[^#&]+)"
    )
    ...
    def build_player_url(cls, video_id, integration, origin_url=None):
        ...
        return cls._smuggle_origin_url(
            f"https:{cls._BASE_PLAYER_URL}?{query_string}", origin_url
        )
    ...
    def _extract_embed_urls(cls, url, webpage):
        ...
        for mobj in re.finditer(regex, webpage):
            ...
            if attrs.get("data-integration-id") and attrs.get(
                "data-playlist-id"
            ):
                yield cls.build_player_url(
                    attrs["data-playlist-id"],
                    attrs["data-integration-id"],
                    url,
                )
