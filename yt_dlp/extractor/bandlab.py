                ...
                revision_data,
                {
                    "webpage_url": (
                        ...
                        (
                            ...
                            {
                                format_field(
                                    template="https://www.bandlab.com/revision/%s"
                                )
                            },
                        ),
                        filter,
                ...
                track_data,
                {
                    "webpage_url": (
                        ...
                        (
                            ...
                            {
                                format_field(
                                    template="https://www.bandlab.com/post/%s"
                                )
                            },
                        ),
                        filter,
...
class BandlabIE(BandlabBaseIE):
    ...
    def _real_extract(self, url):
        display_id, url_type = self._match_valid_url(url).group(
            "id", "url_type"
        )
        ...
        if not revision_id:
            ...
            if not revision_data and not revision_id:
                ...
                raise ExtractorError(
                    f"Could not extract data for post type {post_type!r}"
                )
    ...
    def _real_extract(self, url):
        ...
        for endpoint in endpoints:
            playlist_data = self._call_api(
                endpoint,
                playlist_id,
                note=f"Downloading {endpoint[:-1]} data",
                fatal=False,
                expected_status=404,
            )
