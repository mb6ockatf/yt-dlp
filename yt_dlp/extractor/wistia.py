...
class WistiaBaseIE(InfoExtractor):
    ...
    def _download_embed_config(self, config_type, config_id, referer):
        ...
        embed_config = self._download_json(
            ...
            headers={
                "Referer": (
                    referer if referer.startswith("http") else base_url
    ...
    def _get_real_ext(self, url):
        ...
        if ext == "bin":
            urlh = self._request_webpage(
                HEADRequest(url),
                None,
                note="Checking media extension",
                errnote="HEAD request returned error",
                fatal=False,
            )
            if urlh:
...
class WistiaChannelIE(WistiaBaseIE):
    ...
    def _real_extract(self, url):
        ...
        if not self._yes_playlist(
            channel_id, media_id, playlist_label="channel"
        ):
        ...
        try:
            ...
            webpage = self._download_webpage(
                f"https://fast.wistia.net/embed/channel/{channel_id}",
                channel_id,
            )
        ...
        entries = [
            self.url_result(
                f'wistia:{video["hashedId"]}',
                WistiaIE,
                title=video.get("name"),
            )
            for video in traverse_obj(series, ("sections", ..., "videos", ...))
            or []
        ...
        for match in cls._extract_wistia_async_embed(webpage):
            if match.group("type") == "wistia_channel":
                ...
                yield update_url_query(
                    f'wistiachannel:{match.group("id")}', parse_qs(url)
                )
