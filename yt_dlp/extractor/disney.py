    ...
    def _real_extract(self, url):
        ...
        if not video_id:
            ...
            grill = re.sub(
                ...
                "",
                self._search_regex(
                    r"Grill\.burger\s*=\s*({.+})\s*:", webpage, "grill data"
                ),
            )
            page_data = next(
                s
                for s in self._parse_json(grill, display_id)["stack"]
                if s.get("type") == "video"
            )
            ...
            page_data = self._parse_json(
                self._search_regex(
                    r"Disney\.EmbedVideo\s*=\s*({.+});", webpage, "embed data"
                ),
                video_id,
            )
        ...
        for flavor in video_data.get("flavors", []):
            ...
            if (
                not flavor_url
                or not re.match(r"https?://", flavor_url)
                or flavor_format == "mp4_access"
            ):
                continue
        ...
        if not formats and video_data.get("expired"):
            self.raise_no_formats(
                "{} said: {}".format(
                    self.IE_NAME, page_data["translations"]["video_expired"]
                ),
                expected=True,
            )
        ...
        return {
            ...
            or video_data.get("short_desc"),
            "thumbnail": video_data.get("thumb")
            or video_data.get("thumb_secure"),
