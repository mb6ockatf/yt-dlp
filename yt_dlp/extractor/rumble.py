...
class RumbleEmbedIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if video.get("live") == 0:
            live_status = (
                "not_live"
                if video.get("livestream_has_dvr") is None
                else "was_live"
            )
        ...
        thumbnails = traverse_obj(
            video, ("t", ..., {"url": "i", "width": "w", "height": "h"})
        )
...
class RumbleIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?rumble\.com/(?P<id>v(?!ideos)[\w.-]+)[^/]*$"
    )
    _EMBED_REGEX = [
    ...
    def _real_extract(self, url):
        ...
        url_info = next(
            RumbleEmbedIE.extract_from_webpage(self._downloader, url, webpage),
            None,
        )
        if not url_info:
        ...
        return {
            ...
            "description": clean_html(
                get_element_by_class("media-description", webpage)
            ),
        }
...
class RumbleChannelIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self.entries(url, playlist_id), playlist_id=playlist_id
        )
