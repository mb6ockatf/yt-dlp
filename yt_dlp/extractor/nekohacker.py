...
class NekoHackerIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?nekohacker\.com/(?P<id>(?!free-dl)[\w-]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        if not playlist:
            iframe_src = traverse_obj(
                webpage,
                (
                    {find_element(tag="iframe", html=True)},
                    {extract_attributes},
                    "src",
                    {url_or_none},
                ),
        ...
        for track_number, track in enumerate(
            re.findall(r"(<li[^>]+data-audiopath[^>]+>)", playlist), 1
        ):
            entry = traverse_obj(
                extract_attributes(track),
                {
        ...
        return self.playlist_result(
            entries, playlist_id, traverse_obj(entries, (0, "album"))
        )
