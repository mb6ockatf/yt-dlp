...
class TVPIE(InfoExtractor):
    ...
    def _parse_vue_website_data(self, webpage, page_id):
        ...
        return self._parse_json(
            website_data, page_id, transform_source=js_to_json
        )

    def _extract_vue_video(self, video_data, page_id=None):
        if isinstance(video_data, str):
            video_data = self._parse_json(
                video_data, page_id, transform_source=js_to_json
            )
        thumbnails = []
    ...
    def _vuejs_entries(self, url, website_data, page_id):
        ...
        if website_data.get("items_total_count") > website_data.get(
            "items_per_page"
        ):
            for page in itertools.count(2):
                ...
                if not page_website_data.get(
                    "videos"
                ) and not page_website_data.get("items"):
                    break
        ...
        for ie_cls in (TVPVODSeriesIE, TVPVODVideoIE):
            if ie_cls.suitable(urlh.url):
                return self.url_result(
                    urlh.url, ie=ie_cls.ie_key(), video_id=page_id
                )

        if re.search(
            r"window\.__(?:video|news|website|directory)Data\s*=", webpage
        ):
...
class TVPStreamIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        channel_url = self._proto_relative_url(
            f"//stream.tvp.pl/?channel_id={channel_id}" or "default"
        )
        webpage = self._download_webpage(
            channel_url, channel_id or "default", "Downloading channel webpage"
        )
        ...
        channel = (
            traverse_obj(
                channels,
                (lambda _, v: channel_id == str(v["id"])),
                get_all=False,
            )
            if channel_id
            else channels[0]
        )
        audition = traverse_obj(
            channel,
            ("items", lambda _, v: v["is_live"] is True),
            get_all=False,
        )
        return {
            "_type": "url_transparent",
...
class TVPEmbedIE(InfoExtractor):
    ...
    def _real_extract(self, url):
            ...
            if ext == "m3u8":
                ...
                formats.extend(
                    self._extract_ism_formats(
                        video_url, video_id, ism_id="mss", fatal=False
                    )
                )
            else:
                formats.append(
                    {
                        ...
                        "fps": int_or_none(
                            traverse_obj(file, ("quality", "fps"))
                        ),
                        "tbr": int_or_none(
                            traverse_obj(file, ("quality", "bitrate")),
                            scale=1000,
                        ),
                        "width": int_or_none(
                            traverse_obj(file, ("quality", "width"))
                        ),
                        "height": int_or_none(
                            traverse_obj(file, ("quality", "height"))
                        ),
                    }
                )
        ...
        if age_limit == 1:
            age_limit = 0
        duration = (
            try_get(info, lambda x: x["duration"], int)
            if not is_live
            else None
        )
...
class TVPVODBaseIE(InfoExtractor):
    ...
    def _call_api(self, resource, video_id, query={}, **kwargs):
        ...
        if is_valid(urlh.status):
            return document
        raise ExtractorError(
            f'Woronicza said: {document.get("code")} (HTTP {urlh.status})'
        )
    ...
    def _parse_video(self, video, with_url=True):
        ...
        info_dict["description"] = clean_html(
            dict_get(video, ("lead", "description"))
        )
        if with_url:
            info_dict.update(
                {
...
class TVPVODVideoIE(TVPVODBaseIE):
    ...
    def _real_extract(self, url):
        ...
        info_dict = self._parse_video(
            self._call_api(f"{entity}/{video_id}", video_id), with_url=False
        )

        playlist = self._call_api(
            f"{video_id}/videos/playlist",
            video_id,
            query={"videoType": "MOVIE"},
        )

        info_dict["formats"] = []
        for manifest_url in traverse_obj(
            playlist, ("sources", "HLS", ..., "src")
        ):
            info_dict["formats"].extend(
                self._extract_m3u8_formats(manifest_url, video_id, fatal=False)
            )
        for manifest_url in traverse_obj(
            playlist, ("sources", "DASH", ..., "src")
        ):
            info_dict["formats"].extend(
                self._extract_mpd_formats(manifest_url, video_id, fatal=False)
            )

        info_dict["subtitles"] = {}
        for sub in playlist.get("subtitles") or []:
            info_dict["subtitles"].setdefault(
                sub.get("language") or "und", []
            ).append(
                {
                    "url": sub["url"],
                    "ext": "ttml",
...
class TVPVODSeriesIE(TVPVODBaseIE):
    ...
    def _real_extract(self, url):
        ...
        metadata = self._call_api(
            f"vods/serials/{playlist_id}",
            playlist_id,
            note="Downloading serial metadata",
        )
        seasons = self._call_api(
            f"vods/serials/{playlist_id}/seasons",
            playlist_id,
