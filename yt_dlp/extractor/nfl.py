...
class NFLBaseIE(InfoExtractor):
    ...
    def _get_account_info(self):
        ...
        if "ucid" not in cookies:
            raise ExtractorError(
                ...
                "or else try using --cookies-from-browser instead",
                expected=True,
            )
    ...
    def _get_auth_token(self):
        ...
        token = self._download_json(
            ...
            data=json.dumps(
                {**self._CLIENT_DATA, **self._ACCOUNT_INFO},
                separators=(",", ":"),
            ).encode(),
        )
    ...
    def _extract_video(self, mcp_id, is_live=False):
        ...
        data = self._download_json(
            ...
            data=json.dumps(
                {"init": True, "live": is_live}, separators=(",", ":")
            ).encode(),
        )
    ...
    def _parse_video_config(self, video_config, display_id):
        ...
        if image_url := traverse_obj(
            item, "imageSrc", "posterImage", expected_type=url_or_none
        ):
            thumbnails = [
                {
                    "url": image_url,
...
class NFLIE(NFLBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self._parse_video_config(
            self._search_regex(
                self._VIDEO_CONFIG_REGEX, webpage, "video config"
            ),
            display_id,
        )
...
class NFLArticleIE(NFLBaseIE):
    ...
    def _real_extract(self, url):
        ...
        title = clean_html(
            get_element_by_class("nfl-c-article__title", webpage)
    ...
    _TESTS = [
        ...
        },
        {
            ...
            "params": {
                "skip_download": "m3u8",
                "extractor_args": {
                    "nflplusreplay": {"type": ["condensed_game"]}
                },
            },
        },
    ]
    ...
    def _real_extract(self, url):
        ...
        if not video_id:
            ...
            if len(requested_types) == 1:
                video_id = traverse_obj(
                    ...
                    (
                        "items",
                        lambda _, v: v["subType"] == requested_types[0],
                        "mcpPlaybackId",
                    ),
                    get_all=False,
                )
        ...
        def entries():
            for replay in traverse_obj(
                ...
                (
                    "items",
                    lambda _, v: v["mcpPlaybackId"]
                    and v["subType"] in requested_types,
                ),
            ):
    ...
    _TESTS = [
        {
            ...
            "info_dict": {
                ...
                "categories": ["Analysis"],
                "tags": [
                    "Cincinnati Bengals at Kansas City Chiefs (2022-POST-3)"
                ],
                "thumbnail": r"re:^https?://.*\.jpg",
            },
            "params": {"skip_download": "m3u8"},
