...
class EplusIbIE(InfoExtractor):
    ...
    def _login(self, username, password, urlh):
        ...
        if not login_json.get("isSuccess"):
            raise ExtractorError(
                "Login failed: Invalid id or password", expected=True
            )
    ...
    def _real_extract(self, url):
        ...
        data_json = self._search_json(
            r"<script>\s*var app\s*=", webpage, "data json", video_id
        )
        ...
        release_timestamp = try_call(
            lambda: unified_timestamp(data_json["event_datetime"]) - 32400
        )
        ...
        if not m3u8_playlist_urls:
            ...
            for m3u8_playlist_url in m3u8_playlist_urls:
                formats.extend(
                    self._extract_m3u8_formats(m3u8_playlist_url, video_id)
                )
