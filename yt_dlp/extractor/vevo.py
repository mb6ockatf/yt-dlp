...
class VevoIE(VevoBaseIE):
    ...
    def _initialize_api(self, video_id):
        ...
        if re.search(
            r"(?i)THIS PAGE IS CURRENTLY UNAVAILABLE IN YOUR REGION", webpage
        ):
    ...
    def _call_api(self, path, *args, **kwargs):
        try:
            data = self._download_json(
                self._api_url_template % path, *args, **kwargs
            )
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "view_count": int_or_none(
                video_info.get("views", {}).get("total")
            ),
            "age_limit": age_limit,
            "track": track,
            "artist": uploader,
...
class VevoPlaylistIE(VevoBaseIE):
    ...
    def _real_extract(self, url):
        ...
        playlists = self._extract_json(webpage, playlist_id)["default"][
            f"{playlist_kind}s"
        ]

        playlist = (
            next(iter(playlists.values()))
        ...
        return self.playlist_result(
            entries,
            playlist.get("playlistId") or playlist_id,
            playlist.get("name"),
