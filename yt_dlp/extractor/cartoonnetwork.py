...
class CartoonNetworkIE(TurnerBaseIE):
    ...
    def _real_extract(self, url):
        ...
        def find_field(
            global_re, name, content_re=None, value_re='[^"]+', fatal=False
        ):
            metadata_re = ""
            if content_re:
        ...
        media_id = find_field(
            "mediaId", "media id", "id", "[0-9a-f]{40}", True
        )
        title = find_field(
            "episodeTitle", "title", "(?:episodeName|name)", fatal=True
        )
        ...
        info = self._extract_ngtv_info(
            ...
            {"networkId": "cartoonnetwork"},
            {
                ...
                "auth_required": find_field("authType", "auth type")
                != "unauth",
            },
        )

        series = find_field(
            "propertyName", "series", "showName"
        ) or self._html_search_meta("partOfSeries", webpage)
        info.update(
            {
                "id": media_id,
        ...
        for field in ("season", "episode"):
            ...
            info[field + "_number"] = int_or_none(
                find_field(field_name, field + " number", value_re=r"\d+")
