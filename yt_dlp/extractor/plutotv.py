...
class PlutoTVIE(InfoExtractor):
    ...
    def _to_ad_free_formats(self, video_id, formats, subtitles):
        ...
        for fmt in formats:
            res = self._download_webpage(
                fmt.get("url"),
                video_id,
                note="Downloading m3u8 playlist",
                fatal=False,
            )
            if not res:
                continue
            first_segment_url = re.search(
                r"^(https?://.*/)0\-(end|[0-9]+)/[^/]+\.ts$", res, re.MULTILINE
            )
            if first_segment_url:
                ...
                continue
            first_segment_url = re.search(
                r"^(https?://.*/).+\-0+[0-1]0\.ts$", res, re.MULTILINE
            )
            if first_segment_url:
                m3u8_urls.add(
                    urllib.parse.urljoin(
                        first_segment_url.group(1), "master.m3u8"
                    )
                )
                continue
    ...
    def _get_video_info(self, video_json, slug, series_name=None):
        ...
        formats, subtitles = [], {}
        for video_url in (
            try_get(video_json, lambda x: x["stitched"]["urls"], list) or []
        ):
        ...
        formats, subtitles = self._to_ad_free_formats(
    ...
    def _real_extract(self, url):
        ...
        if mobj["video_type"] == "series":
            ...
            season_number, episode_slug = mobj.get("season_number"), mobj.get(
                "episode_slug"
            )

            videos = []
            for season in video_json["seasons"]:
                if season_number is not None and season_number != int_or_none(
                    season.get("number")
                ):
                    continue
                for episode in season["episodes"]:
                    if (
                        episode_slug is not None
                        and episode_slug != episode.get("slug")
                    ):
                        continue
                    videos.append(
                        self._get_video_info(
                            episode, episode_slug, series_name
                        )
                    )
            if not videos:
