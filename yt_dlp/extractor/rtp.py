...
class RTPIE(InfoExtractor):
    ...
    def _cleanup_media_url(url):
        if urllib.parse.urlparse(url).netloc == "streaming-ondemand.rtp.pt":
            return None
        return url.replace("/drm-fps/", "/hls/").replace(
            "/drm-dash/", "/dash/"
        )
    ...
    def _extract_formats(self, media_urls, episode_id):
        ...
        for media_url in set(
            traverse_obj(
                media_urls, (..., {url_or_none}, {self._cleanup_media_url})
            )
        ):
    ...
    def _extract_from_api(self, program_id, episode_id):
        ...
        for sub_data in traverse_obj(
            asset_urls,
            ("subtitles", "vtt_list", lambda _, v: url_or_none(v["file"])),
        ):
        ...
        return {
            ...
            "thumbnail": traverse_obj(
                episode_data, ("assets", 0, "asset_thumbnail", {url_or_none})
            ),
            **traverse_obj(
                episode_data,
                (
    ...
    def _real_extract(self, url):
        program_id, episode_id = self._match_valid_url(url).group(
            "program_id", "id"
        )
        return self._extract_from_api(
            program_id, episode_id
        ) or self._extract_from_html(url, episode_id)
