...
class RTVEALaCartaIE(InfoExtractor):
    ...
    def _real_initialize(self):
        user_agent_b64 = base64.b64encode(
            self.get_param("http_headers")["User-Agent"].encode()
        ).decode("utf-8")
    ...
    def _real_extract(self, url):
        ...
        if info["state"] == "DESPU":
            raise ExtractorError(
                "The video is no longer available", expected=True
            )
    ...
    def _get_subtitles(self, video_id, sub_file):
        subs = self._download_json(
            sub_file + ".json", video_id, "Downloading subtitles info"
        )["page"]["items"]
        return dict(
            (s["lang"], [{"ext": "vtt", "url": s["src"]}]) for s in subs
        )
...
class RTVEAudioIE(RTVEALaCartaIE):  # XXX: Do not subclass from concrete IE
    ...
    def _real_extract(self, url):
        ...
        info = self._download_json(
            f"https://www.rtve.es/api/audios/{audio_id}.json", audio_id
        )["page"]["items"][0]
...
class RTVELiveIE(RTVEALaCartaIE):  # XXX: Do not subclass from concrete IE
    ...
    def _real_extract(self, url):
        ...
        title = remove_end(
            self._og_search_title(webpage), " en directo en RTVE.es"
        )
...
class RTVETelevisionIE(InfoExtractor):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?rtve\.es/television/[^/]+/[^/]+/(?P<id>\d+).shtml"
    )
