...
class FlickrIE(InfoExtractor):
    ...
    def _call_api(self, method, video_id, api_key, note, secret=None):
        ...
        data = self._download_json(
            self._API_BASE_URL + urllib.parse.urlencode(query), video_id, note
        )
        if data["stat"] != "ok":
    ...
    def _real_extract(self, url):
        ...
        api_key = self._download_json(
            "https://www.flickr.com/hermes_error_beacon.gne",
            video_id,
            "Downloading api key",
        )["site_key"]

        video_info = self._call_api(
            "photos.getInfo", video_id, api_key, "Downloading video info"
        )["photo"]
        if video_info["media"] == "video":
            ...
            uploader_url = format_field(
                uploader_path, None, "https://www.flickr.com/photos/%s/"
            )

            return {
                "id": video_id,
                "title": video_info["title"]["_content"],
                "description": video_info.get("description", {}).get(
                    "_content"
                ),
                "formats": formats,
                "timestamp": int_or_none(video_info.get("dateuploaded")),
                "duration": int_or_none(
                    video_info.get("video", {}).get("duration")
                ),
                "uploader_id": uploader_id,
                "uploader": owner.get("realname"),
                "uploader_url": uploader_url,
                "comment_count": int_or_none(
                    video_info.get("comments", {}).get("_content")
                ),
                "view_count": int_or_none(video_info.get("views")),
                "tags": [
                    tag.get("_content")
                    for tag in video_info.get("tags", {}).get("tag", [])
                ],
