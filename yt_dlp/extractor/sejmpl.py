...
def is_dst(date):
    ...
    last_sunday_march = last_march - dt.timedelta(
        days=last_march.isoweekday() % 7
    )
    last_sunday_october = last_october - dt.timedelta(
        days=last_october.isoweekday() % 7
    )
    return (
        last_sunday_march.replace(hour=2)
        <= date
        <= last_sunday_october.replace(hour=3)
    )
