...
class MicrosoftStreamIE(InfoExtractor):
    ...
    def _get_all_subtitles(self, api_url, video_id, headers):
        ...
        text_tracks = (
            self._download_json(
                f"{api_url}/videos/{video_id}/texttracks",
                video_id,
                note="Downloading subtitles JSON",
                fatal=False,
                headers=headers,
                query={"api-version": "1.4-private"},
            ).get("value")
            or []
        )
        for track in text_tracks:
            if not track.get("language") or not track.get("url"):
                continue
            sub_dict = (
                automatic_captions if track.get("autoGenerated") else subtitles
            )
    ...
    def extract_all_subtitles(self, *args, **kwargs):
        if (
            self.get_param("writesubtitles", False)
    ...
    def _real_extract(self, url):
        ...
        access_token = self._html_search_regex(
            r'"AccessToken":"(.+?)"', webpage, "access token"
        )
        api_url = self._html_search_regex(
            r'"ApiGatewayUri":"(.+?)"', webpage, "api url"
        )
        ...
        for thumbnail_id in ("extraSmall", "small", "medium", "large"):
            thumbnail_url = try_get(
                video_data,
                lambda x: x["posterImage"][thumbnail_id]["url"],
                str,
            )
            ...
            thumb_name = str(
                base64.b64decode(thumb_name + "=" * (-len(thumb_name) % 4))
            )
        ...
        return {
            ...
            "uploader": try_get(
                video_data, lambda x: x["creator"]["name"], str
            ),
            "uploader_id": try_get(
                video_data,
                (lambda x: x["creator"]["mail"], lambda x: x["creator"]["id"]),
                str,
            ),
            ...
            **self.extract_all_subtitles(api_url, video_id, headers),
            ...
            "duration": parse_duration(
                try_get(video_data, lambda x: x["media"]["duration"])
            ),
            "webpage_url": f"https://web.microsoftstream.com/video/{video_id}",
            "view_count": try_get(
                video_data, lambda x: x["metrics"]["views"], int
            ),
            "like_count": try_get(
                video_data, lambda x: x["metrics"]["likes"], int
            ),
            "comment_count": try_get(
                video_data, lambda x: x["metrics"]["comments"], int
            ),
            "formats": formats,
        }
