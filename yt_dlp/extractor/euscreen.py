...
class EUScreenIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        info_js = self._download_webpage(
            "https://euscreen.eu/lou/LouServlet/domain/euscreenxl/html5application/euscreenxlitem",
            video_id,
            data=args_for_js_request.replace("screenid", "screenId").encode(),
        )
        video_json = self._parse_json(
            self._search_regex(
                r"setVideo\(({.+})\)\(\$end\$\)put", info_js, "Video JSON"
            ),
            video_id,
        ...
        meta_json = self._parse_json(
            self._search_regex(
                r"setData\(({.+})\)\(\$end\$\)", info_js, "Metadata JSON"
            ),
            video_id,
            transform_source=js_to_json,
        )
        ...
        return {
            ...
            "description": "{}\n{}".format(
                meta_json.get("summaryOriginal", ""),
                meta_json.get("summaryEnglish", ""),
            ),
            "series": meta_json.get("series")
            or meta_json.get("seriesEnglish"),
            "episode": meta_json.get("episodeNumber"),
            "uploader": meta_json.get("provider"),
            "thumbnail": meta_json.get("screenshot")
            or video_json.get("screenshot"),
            "formats": formats,
        }
