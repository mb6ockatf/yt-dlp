...
class GoIE(AdobePassIE):
    ...
    def _real_extract(self, url):
        ...
        sub_domain = remove_start(
            remove_end(mobj.group("sub_domain") or "", ".go"), "www."
        )
        ...
        if not video_id or not site_info:
            ...
            if not site_info:
                ...
                site_info = next(
                    si
                    for _, si in self._SITE_INFO.items()
        ...
        for asset in video_data.get("assets", {}).get("asset", []):
            ...
            if ext == "m3u8":
                ...
                if re.search(r"(?:/mp4/source/|_source\.mp4)", asset_url):
                    f.update(
                        {
                            "format_id": (f"{format_id}-" if format_id else "")
                            + "SOURCE",
                            "quality": 1,
                        }
                    )
        ...
        return {
            "id": video_id,
            "title": title,
            "description": video_data.get("longdescription")
            or video_data.get("description"),
            "duration": int_or_none(
                video_data.get("duration", {}).get("value"), 1000
            ),
            ...
            "season_number": int_or_none(
                video_data.get("season", {}).get("num")
            ),
            "thumbnails": thumbnails,
            "formats": formats,
            "subtitles": subtitles,
            "timestamp": unified_timestamp(
                traverse_obj(video_data, ("airdates", "airdate", 0))
            ),
        }
