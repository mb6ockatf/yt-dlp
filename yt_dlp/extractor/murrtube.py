...
class MurrtubeIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        video_attrs = extract_attributes(
            get_element_html_by_id("video", video_page)
        )
        ...
        return {
            "id": video_id,
            "title": remove_end(
                self._og_search_title(video_page), " - Murrtube"
            ),
            "age_limit": 18,
            ...
            "thumbnail": update_url(
                self._og_search_thumbnail(video_page, default=""), query=None
            )
            or None,
            "uploader": clean_html(
                get_element_by_class(
                    "pl-1 is-size-6 has-text-lighter", video_page
                )
            ),
    ...
    __typename
        ...
        if data is None:
            raise ExtractorError(
                f"Failed to retrieve video list for page {page + 1}"
            )
        ...
        for entry in media:
            yield self.url_result(
                "murrtube:{}".format(entry["id"]), MurrtubeIE.ie_key()
            )
  ...
  user(id: $id) {
    ...
    __typename
        ...
        entries = OnDemandPagedList(
            functools.partial(self._fetch_page, username, user.get("id")),
            self._PAGE_SIZE,
        )
