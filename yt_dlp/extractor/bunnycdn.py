...
class BunnyCdnIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        video_id, library_id = self._match_valid_url(url).group(
            "id", "library_id"
        )
        webpage = self._download_webpage(
            ...
            query=traverse_obj(
                parse_qs(url), {"token": "token", "expires": "expires"}
            ),
        )

        if (
            html_title := self._html_extract_title(webpage, default=None)
            == "403"
        ):
            raise ExtractorError(
                "This video is inaccessible. Setting a Referer header "
                "might be required to access the video",
                expected=True,
            )
        elif html_title == "404":
        ...
        info = (
            traverse_obj(
                self._parse_html5_media_entries(
                    url, webpage, video_id, _headers=headers
                ),
                0,
            )
            or {}
                ...
                formats.append(
                    {
                        ...
                        "filesize": int_or_none(
                            urlh.get_header("Content-Length")
                        ),
                    }
                )
        ...
        return {
            ...
            **traverse_obj(
                webpage,
                (
                    {find_element(id="main-video", html=True)},
                    {extract_attributes},
                    {
                        "title": (
                            "data-plyr-config",
