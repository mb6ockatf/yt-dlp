...
class OpenRecBaseIE(InfoExtractor):
    ...
    def _extract_movie(self, webpage, video_id, name, is_live):
        ...
        movie_stores = [
            ...
            traverse_obj(
                window_stores, ("v8", "state", "movie"), expected_type=dict
            ),
        ...
        formats = list(
            self._expand_media(video_id, get_first(movie_stores, "media"))
        )
        if not formats:
            # archived livestreams or subscriber-only videos
            cookies = self._get_cookies("https://www.openrec.tv/")
            detail = self._download_json(
                f"https://apiv5.openrec.tv/api/v5/movies/{video_id}/detail",
                video_id,
                headers={
                    "Origin": "https://www.openrec.tv",
                    "Referer": "https://www.openrec.tv/",
                    "access-token": try_get(
                        cookies, lambda x: x.get("access_token").value
                    ),
                    "uuid": try_get(cookies, lambda x: x.get("uuid").value),
                },
            )
            new_media = traverse_obj(
                detail, ("data", "items", ..., "media"), get_all=False
            )
        ...
        return {
            ...
            "timestamp": int_or_none(
                get_first(movie_stores, ["publishedAt", "time"]), scale=1000
            )
            or unified_timestamp(get_first(movie_stores, "publishedAt")),
            "is_live": is_live,
...
class OpenRecIE(OpenRecBaseIE):
    ...
    def _real_extract(self, url):
        ...
        webpage = self._download_webpage(
            f"https://www.openrec.tv/live/{video_id}", video_id
        )
...
class OpenRecCaptureIE(OpenRecBaseIE):
    ...
    def _real_extract(self, url):
        video_id = self._match_id(url)
        webpage = self._download_webpage(
            f"https://www.openrec.tv/capture/{video_id}", video_id
        )
        ...
        return {
            ...
            "timestamp": unified_timestamp(
                traverse_obj(movie_store, "createdAt", expected_type=str)
            ),
            "uploader": traverse_obj(
                movie_store, ("channel", "name"), expected_type=str
            ),
            "uploader_id": traverse_obj(
                movie_store, ("channel", "id"), expected_type=str
            ),
...
class OpenRecMovieIE(OpenRecBaseIE):
    ...
    def _real_extract(self, url):
        video_id = self._match_id(url)
        webpage = self._download_webpage(
            f"https://www.openrec.tv/movie/{video_id}", video_id
        )
