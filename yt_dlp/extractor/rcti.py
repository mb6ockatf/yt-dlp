    ...
    def _call_api(self, url, video_id, note=None):
        ...
        if json.get("status", {}).get("code", 0) != 0:
            raise ExtractorError(
                f'{self.IE_NAME} said: {json["status"]["message_client"]}',
                cause=json,
            )
...
class RCTIPlusIE(RCTIPlusBaseIE):
    ...
    def _real_extract(self, url):
        ...
        video_type, video_id, display_id = (
            match["type"],
            match["id"],
            match["display_id"],
        )
        ...
        appier_id = "23984824_" + str(
            random.randint(0, 10000000000)
        )  # Based on the webpage's uuidRandom generator
        video_json = self._call_api(
            f"https://api.rctiplus.com/api/{url_api_version}/{video_type}/{video_id}/url?appierid={appier_id}",
            display_id,
            "Downloading video URL JSON",
        )[0]
        video_url = video_json["url"]

        is_upcoming = try_get(
            video_json, lambda x: x["current_date"] < x["live_at"]
        )
        if is_upcoming is None:
            is_upcoming = try_get(
                video_json, lambda x: x["current_date"] < x["start_date"]
            )
        if is_upcoming:
            self.raise_no_formats(
                (
                    "This event will start at {}.".format(
                        video_json["live_label"]
                    )
                    if video_json.get("live_label")
                    else "This event has not started yet."
                ),
                expected=True,
            )
        if "akamaized" in video_url:
            ...
            conviva_json_res = self._download_json(
                "https://ff84ae928c3b33064b76dec08f12500465e59a6f.cws.conviva.com/0/wsg",
                display_id,
                "Creating Conviva session",
                "Failed to create Conviva session",
                fatal=False,
                data=json.dumps(conviva_json_data).encode(),
            )
            if conviva_json_res and conviva_json_res.get("err") != "ok":
                self.report_warning(
                    "Conviva said: {}".format(str(conviva_json_res.get("err")))
                )

        video_meta, meta_paths = self._call_api(
            f"https://api.rctiplus.com/api/v1/{video_type}/{video_id}",
            display_id,
            "Downloading video metadata",
        )

        thumbnails, image_path = [], meta_paths.get(
            "image_path", "https://rstatic.akamaized.net/media/"
        )
            ...
            thumbnails.append(
                {
                    "id": "portrait_image",
                    "url": "{}{}{}".format(
                        image_path, 2000, video_meta["portrait_image"]
                    ),  # 2000px seems to be the highest resolution that can be given
                }
            )
            ...
            thumbnails.append(
                {
                    "id": "landscape_image",
                    "url": "{}{}{}".format(
                        image_path, 2000, video_meta["landscape_image"]
                    ),
                }
            )
        try:
            ...
            if isinstance(e.cause, HTTPError) and e.cause.status == 403:
                self.raise_geo_restricted(
                    countries=["ID"], metadata_available=True
                )
            else:
                raise e
        for f in formats:
            if "akamaized" in f["url"] or "cloudfront" in f["url"]:
                f.setdefault("http_headers", {})[
                    "Referer"
                ] = "https://www.rctiplus.com/"  # Referer header is required for akamai/cloudfront CDNs

        return {
            "id": video_meta.get("product_id") or video_json.get("product_id"),
            "title": dict_get(video_meta, ("title", "name"))
            or dict_get(video_json, ("content_name", "assets_name")),
            "display_id": display_id,
            "description": video_meta.get("summary"),
            "timestamp": video_meta.get("release_date")
            or video_json.get("start_date"),
            "duration": video_meta.get("duration"),
            "categories": (
                [video_meta["genre"]] if video_meta.get("genre") else None
            ),
            "average_rating": video_meta.get("star_rating"),
            "series": video_meta.get("program_title")
            or video_json.get("program_title"),
...
class RCTIPlusSeriesIE(RCTIPlusBaseIE):
    ...
    def _entries(
        ...
        url,
        display_id=None,
        note="Downloading entries JSON",
        metadata={},
    ):
        total_pages = 0
        try:
            total_pages = self._call_api(
                f"{url}&length=20&page=0", display_id, note
        ...
        for page_num in range(1, total_pages + 1):
            ...
            for video_json in episode_list:
                yield {
                    ...
                    "display_id": video_json.get("title_code").replace(
                        "_", "-"
                    ),
    ...
    def _series_entries(
        self, series_id, display_id=None, video_type=None, metadata={}
    ):
        if not video_type or video_type in "episodes":
            ...
            for season in seasons_list:
                yield from self._entries(
                    ...
                    display_id,
                    f'Downloading season {season["season"]} episode entries',
                    metadata,
                )
        ...
        if not video_type or video_type in "clips":
            yield from self._entries(
                ...
                display_id,
                "Downloading clip entries",
                metadata,
            )

    def _real_extract(self, url):
        series_id, display_id, video_type = self._match_valid_url(url).group(
            "id", "display_id", "type"
        )
        if video_type:
            self.report_warning(
        ...
        series_meta, meta_paths = self._call_api(
            f"https://api.rctiplus.com/api/v1/program/{series_id}/detail",
            display_id,
        ...
        metadata = {
            "age_limit": try_get(
                series_meta,
                lambda x: self._AGE_RATINGS[x["age_restriction"][0]["code"]],
            ),
            "cast": traverse_obj(
                series_meta,
        ...
        return self.playlist_result(
            self._series_entries(series_id, display_id, video_type, metadata),
            series_id,
            series_meta.get("title"),
            series_meta.get("summary"),
            display_id=display_id,
            **metadata,
        )
...
class RCTIPlusTVIE(RCTIPlusBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self.url_result(
            f"https://www.rctiplus.com/{video_type}/{video_id}/{tv_id}",
            "RCTIPlus",
        )
