...
class CBCIE(InfoExtractor):
    IE_NAME = "cbc.ca"
    _VALID_URL = (
        r"https?://(?:www\.)?cbc\.ca/(?!player/)(?:[^/]+/)+(?P<id>[^/?#]+)"
    )
    _TESTS = [
        {
            # with mediaId
        ...
        if not media_id:
            ...
            if not media_id:
                media_id = self._download_json(
                    "http://feed.theplatform.com/f/h9dtGB/punlNGjMlc1F?fields=id&byContent=byReleases%3DbyId%253D"
                    + clip_id,
                    clip_id,
    ...
    def _real_extract(self, url):
        ...
        entries = [
            ...
            for player_init in re.findall(
                r"CBC\.APP\.Caffeine\.initInstance\(({.+?})\);", webpage
            )
        ]
        media_ids = []
        for media_id_re in (
...
class CBCPlayerIE(InfoExtractor):
    ...
    _TESTS = [
        ...
        },
        {
            ...
            "info_dict": {
                ...
                "duration": 77.678,
                "subtitles": {
                    "eng": [{"ext": "vtt", "protocol": "m3u8_native"}]
                },
        ...
        },
        {
            ...
            "info_dict": {
                ...
                "categories": [
                    "Olympics Summer Soccer",
                    "Summer Olympics Replays",
                    "Summer Olympics Soccer Replays",
                ],
                "location": "Canada",
            },
            "params": {"skip_download": "m3u8"},
    ...
    def _parse_param(self, asset_data, name):
        return traverse_obj(
            asset_data,
            ("params", lambda _, v: v["name"] == name, "value", {str}, any),
        )
    ...
    def _real_extract(self, url):
        ...
        assets = traverse_obj(
            ...
            (
                "media",
                "assets",
                lambda _, v: url_or_none(v["key"]) and v["type"],
            ),
        )
        ...
        for sub in traverse_obj(
            data, ("media", "textTracks", lambda _, v: url_or_none(v["src"]))
        ):
            ...
            if ext == "m3u8":
                ...
                best_video_fmt = (
                    traverse_obj(
                        fmts,
                        (
                            lambda _, v: v.get("vcodec") != "none"
                            and v["tbr"],
                            all,
                            {
                                functools.partial(
                                    sorted, key=lambda x: x["tbr"]
                                )
                            },
                            -1,
                            {dict},
                ...
                if not base_url or "/live/" in base_url:
                    continue
                mp4_url = base_url + replace_extension(
                    url_basename(best_video_fmt["url"]), "mp4"
                )
                ...
                formats.append(
                    {
                        ...
                        "vcodec": (
                            "none"
                            if self._parse_param(asset_data, "mediaType")
                            == "audio"
                            else None
                        ),
                    }
        ...
        chapters = traverse_obj(
            ...
            (
                "media",
                "chapters",
                lambda _, v: float(v["startTime"]) is not None,
                {
        ...
        if len(chapters) == 1 and not (
            chapters[0].get("start_time") or chapters[0].get("end_time")
        ):
            chapters = []
            ...
            **traverse_obj(
                data,
                {
                    ...
                    "duration": (
                        ...
                        "duration",
                        {float_or_none},
                        {lambda x: None if is_live else x},
                    ),
...
class CBCPlayerPlaylistIE(InfoExtractor):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?cbc\.ca/(?:player/)(?!play/)(?P<id>[^?#]+)"
    )
    _TESTS = [
        {
        ...
        def entries():
            for video_id in traverse_obj(
                ...
                (
                    "video",
                    "clipsByCategory",
                    lambda k, _: k.lower() == playlist_id,
                    "items",
                    ...,
                    "id",
                ),
                ...
                yield self.url_result(
                    f"https://www.cbc.ca/player/play/{video_id}", CBCPlayerIE
                )
...
class CBCGemIE(CBCGemBaseIE):
    ...
    def _fetch_access_token(self):
        if self._is_jwt_expired(self._access_token):
            try:
                ...
                self.cache.store(
                    self._NETRC_MACHINE, "token_data", [None, None]
                )
                self.report_warning(
    ...
    def _fetch_claims_token(self):
        ...
        if not self._claims_token or self._is_jwt_expired(self._claims_token):
            ...
            self.cache.store(
                self._NETRC_MACHINE, "claims_token", self._claims_token
            )
        else:
    ...
    def _real_extract(self, url):
        video_id, season_number = self._match_valid_url(url).group(
            "id", "season"
        )
        video_info = self._call_show_api(video_id)
        item_info = traverse_obj(
            video_info,
            (
                "content",
                ...,
...
class CBCGemPlaylistIE(CBCGemBaseIE):
    ...
    def _entries(self, season_info):
        for episode in traverse_obj(
            ...
            yield self.url_result(
                f'https://gem.cbc.ca/media/{episode["url"]}',
                CBCGemIE,
                **self._extract_item_info(episode),
            )

    def _real_extract(self, url):
        season_id, show, season = self._match_valid_url(url).group(
            "id", "show", "season"
        )
        show_info = self._call_show_api(show, display_id=season_id)
        season_info = traverse_obj(
            show_info,
            (
                "content",
                ...,
                "lineups",
                lambda _, v: v["seasonNumber"] == int(season),
                any,
                {require("season info")},
            ),
        )
...
class CBCGemLiveIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        video_info = self._search_nextjs_data(webpage, video_id)["props"][
            "pageProps"
        ]["data"]
        ...
        live_status = (
            "was_live" if video_info.get("isVodEnabled") else "is_live"
        )
        ...
        if (
            ...
            and release_timestamp > time.time()
        ):
            formats = []
            live_status = "is_upcoming"
            self.raise_no_formats(
                "This livestream has not yet started", expected=True
            )
        else:
