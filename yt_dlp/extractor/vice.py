...
class ViceBaseIE(InfoExtractor):
    def _call_api(
        self, resource, resource_key, resource_id, locale, fields, args=""
    ):
        return self._download_json(
            "https://video.vice.com/api/v1/graphql",
            resource_id,
            query={
                "query": """{
  %s(locale: "%s", %s: "%s"%s) {
    ...
    title""",
        ...
        try:
            ...
            if isinstance(e.cause, HTTPError) and e.cause.status in (400, 401):
                ...
                error_message = (
                    error.get("error_description") or error["details"]
                )
        ...
        for subtitle in preplay.get("subtitleURLs", []):
            ...
            language_code = (
                try_get(
                    subtitle, lambda x: x["languages"][0]["language_code"], str
                )
                or "en"
            )
        ...
        return {
            ...
            "age_limit": parse_age_limit(
                video_data.get("video_rating") or rating
            ),
            "series": try_get(
                video_data, lambda x: x["show"]["base"]["display_title"], str
            ),
            "episode_number": int_or_none(episode.get("episode_number")),
            "episode_id": str_or_none(
                episode.get("id") or video_data.get("episode_id")
            ),
            "season_number": int_or_none(season.get("season_number")),
            "season_id": str_or_none(
                season.get("id") or video_data.get("season_id")
            ),
...
class ViceShowIE(ViceBaseIE):
    ...
    id
    url""",
            f", page: {page + 1}, per_page: {self._PAGE_SIZE}",
        )
        for video in videos:
...
class ViceArticleIE(ViceBaseIE):
    ...
    embed_code""",
        ...
        video_url = self._html_search_regex(
            r'data-video-url="([^"]+)"', article["embed_code"], "video URL"
        )
