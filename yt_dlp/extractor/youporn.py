...
class YouPornIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if not watchable:
            msg = re.split(
                r"\s{2}",
                clean_html(get_element_by_id("mainContent", webpage)) or "",
            )[0]
            raise ExtractorError(
                f"{self.IE_NAME} says: {msg}" if msg else "Video unavailable",
                expected=True,
            )
        ...
        def get_format_data(data, stream_type):
            info_url = traverse_obj(
                data,
                (
                    lambda _, v: v["format"] == stream_type,
                    "videoUrl",
                    {url_or_none},
                    any,
                ),
            ...
            return traverse_obj(
                ...
                lambda _, v: v["format"] == stream_type
                and url_or_none(v["videoUrl"]),
            )

        formats = []
        # Try to extract only the actual master m3u8 first, avoiding the duplicate single resolution "master" m3u8s
        for hls_url in traverse_obj(
            get_format_data(definitions, "hls"),
            (
                lambda _, v: not isinstance(v["defaultQuality"], bool),
                "videoUrl",
            ),
            (..., "videoUrl"),
        ):
        ...
        data = self._search_json_ld(
            webpage, video_id, expected_type="VideoObject", fatal=False
        )
        data.pop("url", None)
...
class YouPornListBase(InfoExtractor):
    def _get_next_url(self, url, pl_id, html):
        return urljoin(
            ...
            self._search_regex(
                ...
                get_element_by_id("next", html) or "",
                "next page",
                group="url",
                default=None,
            ),
        ...
        for page in itertools.count(start):
            ...
            for element in get_elements_html_by_class("video-title", html):
                if video_url := traverse_obj(
                    element, ({extract_attributes}, "href", {urljoin(url)})
                ):
    ...
    def _real_extract(self, url, html=None):
        ...
        pl_id, page_type, sort = (
            m_dict.get(k) for k in ("id", "type", "sort")
        )
        ...
        return self.playlist_result(
            self._entries(
                url, pl_id, html=html, page_num=int_or_none(qs.get("page"))
            ),
            playlist_id=pl_id,
...
class YouPornTagIE(YouPornListBase):
    ...
    _TESTS = [
        {
            ...
            "expected_warnings": [
                "YouPorn tag pages are not correctly cached"
            ],
        },
        {
            "note": "Filtered paginated list with single page result",
    ...
    def _real_extract(self, url):
        self.report_warning(
            ...
            "often return incorrect results",
            only_once=True,
        )
...
class YouPornStarIE(YouPornListBase):
    ...
    def _real_extract(self, url):
        ...
        if infos := self._search_regex(
            INFO_ELEMENT_RE, html, "infos", group="info", default=""
        ):
            infos = re.sub(
                r"(?:\s*nl=nl)+\s*",
                " ",
                re.sub(
                    r"(?u)\s+", " ", clean_html(re.sub("\n", "nl=nl", infos))
                ),
            ).replace("ribe Subsc", "")
