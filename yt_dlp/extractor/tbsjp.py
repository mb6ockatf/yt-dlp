...
class TBSJPEpisodeIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        meta = self._search_json(
            r"window\.app\s*=", webpage, "episode info", video_id, fatal=False
        )
        ...
        tf_path = self._search_regex(
            r'<script[^>]+src=["\'](/assets/tf\.[^"\']+\.js)["\']',
            webpage,
            "stream API config",
        )
        tf_js = self._download_webpage(
            urljoin(url, tf_path),
            video_id,
            note="Downloading stream API config",
        )
        video_url = self._search_regex(
            r'videoPlaybackUrl:\s*[\'"]([^\'"]+)[\'"]', tf_js, "stream API url"
        )
        api_key = self._search_regex(
            r'api_key:\s*[\'"]([^\'"]+)[\'"]', tf_js, "stream API key"
        )
        ...
        return {
            "title": traverse_obj(
                webpage, ({find_element(tag="h3")}, {clean_html})
            ),
            "id": video_id,
            **traverse_obj(
                episode,
...
class TBSJPProgramIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        meta = self._search_json(
            r"window\.app\s*=", webpage, "programme info", programme_id
        )
        ...
        return {
            ...
            "entries": [
                self.url_result(
                    f"https://cu.tbs.co.jp/episode/{video_id}",
                    TBSJPEpisodeIE,
                    video_id,
                )
                for video_id in traverse_obj(
                    programme,
                    ("custom_data", "seriesList", "episodeCode", ...),
                )
            ],
            "id": programme_id,
            **traverse_obj(
...
class TBSJPPlaylistIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        meta = self._search_json(
            r"window\.app\s*=", page, "playlist info", playlist_id
        )
            ...
            for entry in traverse_obj(
                ...
                if content_type == "tv_show":
                    yield self.url_result(
                        f"https://cu.tbs.co.jp/program/{content_id}",
                        TBSJPProgramIE,
                        content_id,
                    )
                elif content_type == "tv_episode":
                    yield self.url_result(
                        f"https://cu.tbs.co.jp/episode/{content_id}",
                        TBSJPEpisodeIE,
                        content_id,
                    )
                else:
        ...
        return self.playlist_result(
            entries(),
            playlist_id,
            traverse_obj(playlist, ("display_name", "value")),
        )
