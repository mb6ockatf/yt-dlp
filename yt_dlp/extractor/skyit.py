...
class SkyItPlayerIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        domain = urllib.parse.parse_qs(urllib.parse.urlparse(url).query).get(
            "domain", [None]
        )[0]
...
class SkyItVideoLiveIE(SkyItPlayerIE):  # XXX: Do not subclass from concrete IE
    ...
    def _real_extract(self, url):
        ...
        asset_id = str(
            self._search_nextjs_data(webpage, display_id)["props"][
                "initialState"
            ]["livePage"]["content"]["asset_id"]
        )
...
class TV8ItIE(SkyItVideoIE):  # XXX: Do not subclass from concrete IE
    IE_NAME = "tv8.it"
    _VALID_URL = (
        r"https?://(?:www\.)?tv8\.it/(?:show)?video/[0-9a-z-]+-(?P<id>\d+)"
    )
    _TESTS = [
        {
