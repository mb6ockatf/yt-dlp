...
class AitubeKZVideoIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        nextjs_data = self._search_nextjs_data(webpage, video_id)["props"][
            "pageProps"
        ]["videoInfo"]
        json_ld_data = self._search_json_ld(webpage, video_id)

        formats, subtitles = self._extract_m3u8_formats_and_subtitles(
            f"https://api-http.aitube.kz/kz.aitudala.aitube.staticaccess/video/{video_id}/video",
            video_id,
        )
        ...
        return merge_dicts(
            {
                "id": video_id,
                "title": nextjs_data.get("title")
                or self._html_search_meta(["name", "og:title"], webpage),
                "description": nextjs_data.get("description"),
                "formats": formats,
                "subtitles": subtitles,
                "view_count": (
                    nextjs_data.get("viewCount")
                    or int_or_none(
                        self._html_search_meta("ya:ovs:views_total", webpage)
                    )
                ),
                ...
                "channel_follower_count": int_or_none(
                    nextjs_data.get("channelSubscriberCount")
                ),
            },
            json_ld_data,
        )
