...
class ManotoTVIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        episode_json = self._download_json(
            _API_URL.format("showmodule", "episodedetails", video_id), video_id
        )
        ...
        return {
            ...
            "duration": int_or_none(
                details.get("durationInMinutes"), invscale=60
            ),
...
class ManotoTVShowIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        show_json = self._download_json(
            _API_URL.format("showmodule", "details", show_id), show_id
        )
        ...
        series_json = self._download_json(
            _API_URL.format("showmodule", "serieslist", show_id), show_id
        )
        playlist_id = str(
            traverse_obj(series_json, ("details", "list", 0, "id"))
        )

        playlist_json = self._download_json(
            _API_URL.format("showmodule", "episodelist", playlist_id),
            playlist_id,
        )
        ...
        entries = [
            self.url_result(
                "https://www.manototv.com/episode/{}".format(item["slideID"]),
                ie=ManotoTVIE.ie_key(),
                video_id=item["slideID"],
            )
...
class ManotoTVLiveIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        json = self._download_json(
            _API_URL.format("livemodule", "details", ""), video_id
        )
