        ...
        if cookies and cookies.get("userUP"):
            ...
            token = self._download_json(
                ...
                data=json.dumps(
                    {
                        "device_ids": [
                            {"id": str(uuid.uuid4()), "type": "device_id"}
                        ]
                    }
                ).encode(),
                headers={
        ...
        for video in traverse_obj(results, (("assets", None), "items", ...)):
            if video.get("contentId"):
                yield self.url_result(
                    HotStarIE._video_url(video["contentId"], root=root),
                    HotStarIE,
                    video["contentId"],
                )
...
class HotStarIE(HotStarBaseIE):
    ...
    @classmethod
    def _video_url(
        cls, video_id, video_type=None, *, slug="ignore_me", root=None
    ):
    ...
    def _real_extract(self, url):
        ...
        video_data = (
            traverse_obj(
                self._call_api_v1(
                    ...
                    video_id,
                    fatal=False,
                    query={"tas": 10000, "contentId": video_id},
                ),
                ("body", "results", "item", {dict}),
            )
            or {}
        )
        if not self.get_param("allow_unplayable_formats") and video_data.get(
            "drmProtected"
        ):
            self.report_drm(video_id)

        # See https://github.com/yt-dlp/yt-dlp/issues/396
        st = self._download_webpage_handle(f"{self._BASE_URL}/in", video_id)[
            1
        ].headers.get("x-origin-date")
        ...
        playback_sets = self._call_api_v2(
            "play/v1/playback", video_id, st=st, cookies=cookies
        )["playBackSets"]
        for playback_set in playback_sets:
            ...
            tag_dict = dict(
                (*t.split(":", 1), None)[:2] for t in tags.split(";")
            )
        ...
        return {
            ...
            "timestamp": int_or_none(
                traverse_obj(video_data, "broadcastDate", "startDate")
            ),
...
class HotStarPrefixIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        return self.url_result(
            HotStarIE._video_url(video_id, video_type), HotStarIE, video_id
        )
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._playlist_entries(
                "tray/find", id_, query={"tas": 10000, "uqId": id_}
            ),
            id_,
        )
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._playlist_entries(
                ...
                season_id,
                url,
                query={"tao": 0, "tas": 0, "size": 10000, "id": season_id},
            ),
            season_id,
        )
...
class HotStarSeriesIE(HotStarBaseIE):
    ...
    def _real_extract(self, url):
        ...
        id_ = self._call_api_v1(
            "show/detail", series_id, query={"contentId": series_id}
        ...
        return self.playlist_result(
            self._playlist_entries(
                ...
                series_id,
                url,
                query={"tao": 0, "tas": 10000, "etid": 0, "eid": id_},
            ),
            series_id,
        )
