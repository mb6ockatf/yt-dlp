...
class GlobalPlayerBaseIE(InfoExtractor):
    def _get_page_props(self, url, video_id):
        ...
        return self._search_nextjs_data(webpage, video_id)["props"][
            "pageProps"
        ]

    def _request_ext(self, url, video_id):
        return urlhandle_detect_ext(
            self._request_webpage(  # Server rejects HEAD requests
...
class GlobalPlayerLiveIE(GlobalPlayerBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return {
            "id": station["id"],
            "display_id": join_nonempty("brandSlug", "slug", from_dict=station)
...
class GlobalPlayerAudioIE(GlobalPlayerBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "id": video_id,
            "entries": [
                self._extract_audio(ep, series)
                for ep in traverse_obj(
                    series,
                    ("episodes", lambda _, v: v["id"] and v["streamUrl"]),
                )
            ],
            "categories": traverse_obj(series, ("categories", ..., "name"))
            or None,
            **traverse_obj(
                series,
                {
...
class GlobalPlayerAudioEpisodeIE(GlobalPlayerBaseIE):
    ...
    def _real_extract(self, url):
        ...
        props = self._get_page_props(url, video_id)
        episode = (
            props["podcastEpisode"] if podcast else props["catchupEpisode"]
        )

        return self._extract_audio(
            episode,
            traverse_obj(episode, "podcast", "show", expected_type=dict) or {},
        )
