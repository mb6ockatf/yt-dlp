...
class RaiBaseIE(InfoExtractor):
    ...
    def _fix_m3u8_formats(self, media_url, video_id):
        ...
        for f in fmts:
            ...
            if re.search(r"chunklist(?:_b\d+)*_ao[_.]", man_url):  # audio only
                ...
                r"chunklist(?:_b\d+)*_vo[_.]", man_url
            ):  # video only
                f["acodec"] = "none"
            else:  # video+audio
                if f["acodec"] == "none":
    ...
    def _extract_relinker_info(self, relinker_url, video_id, audio_only=False):
        ...
        relinker = self._download_xml(
            relinker_url,
            video_id,
            note="Downloading XML metadata",
            transform_source=fix_cdata,
            query={"output": 64},
        ...
        media_url = xpath_text(
            relinker, './url[@type="content"]', default=None
        )
        ...
        geoprotection = (
            xpath_text(relinker, "./geoprotection", default="N") == "Y"
        )
        ...
        if (
            not formats and geoprotection is True
        ) or "/video_no_available.mp4" in media_url:
        ...
        if not audio_only and not is_live:
            formats.extend(
                self._create_http_urls(
                    media_url, relinker_url, formats, video_id
                )
            )
    ...
    def _create_http_urls(self, manifest_url, relinker_url, fmts, video_id):
        ...
        def percentage(number, target, pc=20, roof=125):
            ...
            if not number or number < 0:
                return False
            return abs(target - number) < min(
                float(number) * float(pc) / 100.0, roof
            )
        ...
        try:
            self._request_webpage(
                HEADRequest(_MP4_TMPL % (relinker_url, "*")),
                video_id,
                "Checking MP4 availability",
            )
        except ExtractorError as e:
            self.to_screen(
                f"{video_id}: MP4 direct download is not available: {e.cause}"
            )
            return []
        ...
        if not mobj:
            return []
        available_qualities = (
            mobj.group("quality").split(",")
            if mobj.group("quality")
            else ["*"]
        )
    ...
    def _get_thumbnails_list(thumbs, url):
        return [
            ...
            for thumb_url in (thumbs or {}).values()
            if thumb_url
        ]
    ...
    def _extract_subtitles(url, video_data):
        ...
        subtitles = {}
        subtitles_array = (
            video_data.get("subtitlesArray")
            or video_data.get("subtitleList")
            or []
        )
...
class RaiPlayIE(RaiBaseIE):
    ...
    _TESTS = [
        ...
        },
        {
            ...
            "params": {"skip_download": True},
            "expected_warnings": [
                "Video not available. Likely due to geo-restriction."
            ],
        },
        {
            # 1500 quality
    ...
    def _real_extract(self, url):
        ...
        alt_title = join_nonempty(
            media.get("subtitle"), media.get("toptitle"), delim=" - "
        )
        ...
        return {
            ...
            "release_year": int_or_none(
                traverse_obj(media, ("track_info", "edit_year"))
            ),
            **relinker_info,
        }
    ...
    def _real_extract(self, url):
        ...
        uid = try_get(
            ...
            lambda x: remove_start(
                remove_start(x["uniquename"], "ContentItem-"), "Page-"
            ),
        )
        ...
        date_published = try_get(
            media,
            (
                lambda x: f'{x["create_date"]} {x.get("create_time") or ""}',
                lambda x: x["live"]["create_date"],
            ),
        )

        podcast_info = (
            traverse_obj(media, "podcast_info", ("live", "cards", 0)) or {}
        )
        ...
        return {
            **info,
            ...
            "alt_title": traverse_obj(
                media,
                ("track_info", "media_name"),
                expected_type=strip_or_none,
            ),
            "description": media.get("description"),
            "uploader": traverse_obj(
                media, ("track_info", "channel"), expected_type=strip_or_none
            ),
            "creator": traverse_obj(
                media, ("track_info", "editor"), expected_type=strip_or_none
            ),
            "timestamp": unified_timestamp(date_published),
            "thumbnails": self._get_thumbnails_list(
                podcast_info.get("images"), url
            ),
...
class RaiPlaySoundPlaylistIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        base, playlist_id, extra_id = self._match_valid_url(url).group(
            "base", "id", "extra_id"
        )
        url = f"{base}.json"
        program = self._download_json(
        ...
        if extra_id:
            ...
            program = self._download_json(
                urljoin("https://www.raiplaysound.it", path),
                playlist_id,
        ...
        entries = [
            self.url_result(
                urljoin(base, c["path_id"]), ie=RaiPlaySoundIE.ie_key()
            )
                ...
                "formats": [
                    {
                        "format_id": join_nonempty(
                            "https", media.get("formatoAudio"), delim="-"
                        ),
                        "url": media["audioUrl"],
...
class RaiNewsIE(RaiBaseIE):
    ...
    def _real_extract(self, url):
        ...
        player_data = self._search_json(
            rf"<rai{self._PLAYER_TAG}-player\s*data=\'",
            webpage,
            "player_data",
            video_id,
