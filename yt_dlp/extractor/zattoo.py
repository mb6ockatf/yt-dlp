...
class ZattooPlatformBaseIE(InfoExtractor):
    ...
    def _host_url(self):
        return "https://%s" % (
            self._API_HOST if hasattr(self, "_API_HOST") else self._HOST
        )

    def _real_initialize(self):
        if not self._power_guide_hash:
            self.raise_login_required(
                "An account is needed to access this media", method="password"
            )

    def _perform_login(self, username, password):
        try:
            data = self._download_json(
                f"{self._host_url()}/zapi/v2/account/login",
                None,
                "Logging in",
    ...
    def _initialize_pre_login(self):
        session_token = self._download_json(
            f"{self._host_url()}/token.json", None, "Downloading session token"
        )["session_token"]

        # Will setup appropriate cookies
        self._request_webpage(
            f"{self._host_url()}/zapi/v3/session/hello",
            None,
            "Opening session",
            data=urlencode_postdata(
                {
    ...
    def _extract_ondemand_info(self, ondemand_id):
        ...
        return (
            data["terms_catalog"][0]["terms"][0]["token"],
            data["type"],
            info_dict,
        )
    ...
    def _extract_formats(
        ...
        for stream_type in ("dash", "hls7"):
            ...
            data = self._download_json(
                url,
                video_id,
                f"Downloading {stream_type.upper()} formats",
                data=urlencode_postdata(postdata),
                fatal=False,
            )
            if not data:
                continue
    ...
    def _extract_ondemand(self, ondemand_id):
        ondemand_termtoken, ondemand_type, info_dict = (
            self._extract_ondemand_info(ondemand_id)
        )
        info_dict["formats"], info_dict["subtitles"] = self._extract_formats(
            None,
            ondemand_id,
...
def _create_valid_url(host, match, qs, base_re=None):
    match_base = rf"|{base_re}/(?P<vid1>{match})" if base_re else "(?P<vid1>)"
...
class ZattooIE(ZattooBaseIE):
    _VALID_URL = _create_valid_url(
        ZattooBaseIE._HOST, r"\d+", "program", "(?:program|watch)/[^/]+"
    )
    _TYPE = "video"
    _TESTS = [
        {
...
class ZattooLiveIE(ZattooBaseIE):
    _VALID_URL = _create_valid_url(
        ZattooBaseIE._HOST, r"[^/?&#]+", "channel", "live"
    )
    _TYPE = "live"
    _TESTS = [
        {
...
class ZattooMoviesIE(ZattooBaseIE):
    _VALID_URL = _create_valid_url(
        ZattooBaseIE._HOST, r"\w+", "movie_id", "vod/movies"
    )
    _TYPE = "ondemand"
    _TESTS = [
        {
...
class NetPlusTVIE(NetPlusTVBaseIE):
    _VALID_URL = _create_valid_url(
        NetPlusTVBaseIE._HOST, r"\d+", "program", "(?:program|watch)/[^/]+"
    )
    _TYPE = "video"
    _TESTS = [
        {
...
class NetPlusTVLiveIE(NetPlusTVBaseIE):
    _VALID_URL = _create_valid_url(
        NetPlusTVBaseIE._HOST, r"[^/?&#]+", "channel", "live"
    )
    _TYPE = "live"
    _TESTS = [
        {
...
class MNetTVIE(MNetTVBaseIE):
    _VALID_URL = _create_valid_url(
        MNetTVBaseIE._HOST, r"\d+", "program", "(?:program|watch)/[^/]+"
    )
    _TYPE = "video"
    _TESTS = [
        {
...
class MNetTVLiveIE(MNetTVBaseIE):
    _VALID_URL = _create_valid_url(
        MNetTVBaseIE._HOST, r"[^/?&#]+", "channel", "live"
    )
    _TYPE = "live"
    _TESTS = [
        {
...
class WalyTVIE(WalyTVBaseIE):
    _VALID_URL = _create_valid_url(
        WalyTVBaseIE._HOST, r"\d+", "program", "(?:program|watch)/[^/]+"
    )
    _TYPE = "video"
    _TESTS = [
        {
...
class WalyTVLiveIE(WalyTVBaseIE):
    _VALID_URL = _create_valid_url(
        WalyTVBaseIE._HOST, r"[^/?&#]+", "channel", "live"
    )
    _TYPE = "live"
    _TESTS = [
        {
...
class BBVTVIE(BBVTVBaseIE):
    _VALID_URL = _create_valid_url(
        BBVTVBaseIE._HOST, r"\d+", "program", "(?:program|watch)/[^/]+"
    )
    _TYPE = "video"
    _TESTS = [
        {
...
class VTXTVIE(VTXTVBaseIE):
    _VALID_URL = _create_valid_url(
        VTXTVBaseIE._HOST, r"\d+", "program", "(?:program|watch)/[^/]+"
    )
    _TYPE = "video"
    _TESTS = [
        {
...
class GlattvisionTVIE(GlattvisionTVBaseIE):
    _VALID_URL = _create_valid_url(
        GlattvisionTVBaseIE._HOST, r"\d+", "program", "(?:program|watch)/[^/]+"
    )
    _TYPE = "video"
    _TESTS = [
        {
...
class GlattvisionTVLiveIE(GlattvisionTVBaseIE):
    _VALID_URL = _create_valid_url(
        GlattvisionTVBaseIE._HOST, r"[^/?&#]+", "channel", "live"
    )
    _TYPE = "live"
    _TESTS = [
        {
    ...
    def suitable(cls, url):
        return (
            False if GlattvisionTVIE.suitable(url) else super().suitable(url)
        )
...
class SAKTVIE(SAKTVBaseIE):
    _VALID_URL = _create_valid_url(
        SAKTVBaseIE._HOST, r"\d+", "program", "(?:program|watch)/[^/]+"
    )
    _TYPE = "video"
    _TESTS = [
        {
...
class EWETVIE(EWETVBaseIE):
    _VALID_URL = _create_valid_url(
        EWETVBaseIE._HOST, r"\d+", "program", "(?:program|watch)/[^/]+"
    )
    _TYPE = "video"
    _TESTS = [
        {
...
class QuantumTVIE(QuantumTVBaseIE):
    _VALID_URL = _create_valid_url(
        QuantumTVBaseIE._HOST, r"\d+", "program", "(?:program|watch)/[^/]+"
    )
    _TYPE = "video"
    _TESTS = [
        {
...
class QuantumTVLiveIE(QuantumTVBaseIE):
    _VALID_URL = _create_valid_url(
        QuantumTVBaseIE._HOST, r"[^/?&#]+", "channel", "live"
    )
    _TYPE = "live"
    _TESTS = [
        {
...
class OsnatelTVIE(OsnatelTVBaseIE):
    _VALID_URL = _create_valid_url(
        OsnatelTVBaseIE._HOST, r"\d+", "program", "(?:program|watch)/[^/]+"
    )
    _TYPE = "video"
    _TESTS = [
        {
...
class OsnatelTVLiveIE(OsnatelTVBaseIE):
    _VALID_URL = _create_valid_url(
        OsnatelTVBaseIE._HOST, r"[^/?&#]+", "channel", "live"
    )
    _TYPE = "live"
    _TESTS = [
        {
...
class EinsUndEinsTVIE(EinsUndEinsTVBaseIE):
    _VALID_URL = _create_valid_url(
        EinsUndEinsTVBaseIE._HOST, r"\d+", "program", "(?:program|watch)/[^/]+"
    )
    _TYPE = "video"
    _TESTS = [
        {
...
class EinsUndEinsTVLiveIE(EinsUndEinsTVBaseIE):
    _VALID_URL = _create_valid_url(
        EinsUndEinsTVBaseIE._HOST, r"[^/?&#]+", "channel", "live"
    )
    _TYPE = "live"
    _TESTS = [
        {
    ...
    def suitable(cls, url):
        return (
            False if EinsUndEinsTVIE.suitable(url) else super().suitable(url)
        )
...
class SaltTVIE(SaltTVBaseIE):
    _VALID_URL = _create_valid_url(
        SaltTVBaseIE._HOST, r"\d+", "program", "(?:program|watch)/[^/]+"
    )
    _TYPE = "video"
    _TESTS = [
        {
...
class SaltTVLiveIE(SaltTVBaseIE):
    _VALID_URL = _create_valid_url(
        SaltTVBaseIE._HOST, r"[^/?&#]+", "channel", "live"
    )
    _TYPE = "live"
    _TESTS = [
        {
