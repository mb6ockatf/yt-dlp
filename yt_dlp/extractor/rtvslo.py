...
class RTVSLOIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        meta = self._download_json(
            self._API_BASE.format("getRecordingDrm", v_id), v_id
        )["response"]
        ...
        for s in traverse_obj(meta, "subs", "subtitles", default=[]):
            lang = self.SUB_LANGS_MAP.get(
                s.get("language"), s.get("language") or "und"
            )
        ...
        if not jwt:
            raise ExtractorError(
                "Site did not provide an authentication token, cannot proceed."
            )

        media = self._download_json(
            self._API_BASE.format("getMedia", v_id), v_id, query={"jwt": jwt}
        )["response"]

        formats = []
        skip_protocols = ["smil", "f4m", "dash"]
        adaptive_url = traverse_obj(
            media, ("addaptiveMedia", "hls_sec"), expected_type=url_or_none
        )
        if adaptive_url:
            formats = self._extract_wowza_formats(
                adaptive_url, v_id, skip_protocols=skip_protocols
            )
        ...
        for mediafile in traverse_obj(
            media,
            ("mediaFiles", lambda _, v: url_or_none(v["streams"]["https"])),
        ):
            formats.append(
                traverse_obj(
                    mediafile,
        ...
        for mediafile in traverse_obj(
            media,
            ("mediaFiles", lambda _, v: url_or_none(v["streams"]["hls_sec"])),
        ):
            formats.extend(
                self._extract_wowza_formats(
                    mediafile["streams"]["hls_sec"],
                    v_id,
        ...
        if (
            any("dummy_720p.mp4" in x.get("manifest_url", "") for x in formats)
            and meta.get("stub") == "error"
        ):
            raise ExtractorError(
                f"{self.IE_NAME} said: Clip not available", expected=True
            )

        return {
            "id": v_id,
            "webpage_url": "".join(
                traverse_obj(meta, ("canonical", ("domain", "path")))
            ),
            ...
            "timestamp": unified_timestamp(
                traverse_obj(meta, "broadcastDate", ("broadcastDates", 0))
            ),
            "release_timestamp": unified_timestamp(meta.get("recordingDate")),
            "duration": meta.get("duration")
            or parse_duration(meta.get("length")),
            "tags": meta.get("genre"),
...
class RTVSLOShowIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_from_matches(
            ...
            getter=urljoin("https://365.rtvslo.si"),
            ie=RTVSLOIE,
