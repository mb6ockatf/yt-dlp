...
class DacastBaseIE(InfoExtractor):
    ...
    @classproperty
    def _VALID_URL(cls):
        return rf"https?://iframe\.dacast\.com/{cls._URL_TYPE}/(?P<user_id>[\w-]+)/(?P<id>[\w-]+)"
    ...
    def _get_url_from_id(cls, content_id):
        ...
        return (
            f"https://iframe.dacast.com/{cls._URL_TYPE}/{user_id}/{media_id}"
        )
...
class DacastVODIE(DacastBaseIE):
    ...
    def _usp_signing_secret(self):
        ...
        return (
            ...
            or "odnInCGqhvtyRTtIiddxtuRtawYYICZP"
        )
    ...
    def _real_extract(self, url):
        ...
        info = self._download_json(
            self._API_INFO_URL, video_id, query=query, fatal=False
        )
        access = self._download_json(
            "https://playback.dacast.com/content/access",
            video_id,
            note="Downloading access JSON",
            query=query,
            expected_status=403,
        )
        ...
        if "DRM_EXT" in hls_url:
            ...
            signature = (
                hashlib.sha1(
                    f"{10413792000 - ts}{ts}{self._usp_signing_secret}".encode()
                )
                .digest()
                .hex()
            )
            hls_aes["uri"] = (
                f"https://keys.dacast.com/uspaes/{video_id}.key?s={signature}&ts={ts}"
            )
...
class DacastPlaylistIE(DacastBaseIE):
    ...
    def _real_extract(self, url):
        user_id, playlist_id = self._match_valid_url(url).group(
            "user_id", "id"
        )
        info = self._download_json(
            self._API_INFO_URL,
        ...
        def entries(info):
            for video in traverse_obj(
                info,
                ("features", "playlist", "contents", lambda _, v: v["id"]),
            ):
                yield self.url_result(
                    DacastVODIE._get_url_from_id(video["id"]),
                    DacastVODIE,
                    video["id"],
                    video.get("title"),
                )
