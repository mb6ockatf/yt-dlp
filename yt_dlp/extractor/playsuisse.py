...
class PlaySuisseIE(InfoExtractor):
    ...
    def _perform_login(self, username, password):
        ...
        request_id = parse_qs(
            self._request_webpage(
                f"{self._LOGIN_BASE}/authz-srv/authz",
                None,
        ...
        try:
            exchange_id = self._download_json(
                f"{self._LOGIN_BASE}/verification-srv/v2/authenticate/initiate/password",
                None,
                "Submitting username",
        ...
        try:
            login_data = self._download_json(
                f"{self._LOGIN_BASE}/verification-srv/v2/authenticate/authenticate/password",
                None,
                "Submitting password",
                headers={"content-type": "application/json"},
                data=json.dumps(
        ...
        authorization_code = parse_qs(
            self._request_webpage(
                f"{self._LOGIN_BASE}/login-srv/verification/login",
                None,
                "Logging in",
                data=urlencode_postdata(
                    {
                        "requestId": request_id,
                        "exchange_id": login_data["exchange_id"][
                            "exchange_id"
                        ],
        ...
        self._ID_TOKEN = self._download_json(
            f"{self._LOGIN_BASE}/proxy/token",
            None,
            "Downloading token",
            data=b"",
            query={
            ...
            info.update(
                {
                    "_type": "playlist",
                    "entries": map(
                        self._extract_single, media_data["episodes"]
                    ),
                }
            )
        return info
    ...
    def _extract_single(self, media_data):
        ...
        return {
            ...
            "episode": (
                media_data.get("name")
                if media_data.get("episodeNumber")
                else None
            ),
