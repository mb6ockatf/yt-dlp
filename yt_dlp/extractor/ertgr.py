...
class ERTFlixBaseIE(InfoExtractor):
    ...
    def _call_api_get_tiles(self, video_id, *tile_ids):
        ...
        tiles_response = self._call_api(
            video_id,
            method="Tile/GetTiles",
            api_version=2,
            data={"RequestedTiles": requested_tiles},
        )
        tiles = try_get(tiles_response, lambda x: x["Tiles"], list) or []
        if tile_ids:
            if sorted([tile["Id"] for tile in tiles]) != sorted(
                requested_tile_ids
            ):
                raise ExtractorError(
                    "Requested tiles not found", video_id=video_id
                )
            return tiles
        try:
...
class ERTFlixCodenameIE(ERTFlixBaseIE):
    ...
    def _extract_formats_and_subs(self, video_id):
        ...
        for media in traverse_obj(
            ...
            (
                "MediaFiles",
                lambda _, v: v["RoleCodename"] == "main",
                "Formats",
...
class ERTFlixIE(ERTFlixBaseIE):
    ...
    def _extract_episode(self, episode):
        ...
        description = clean_html(
            dict_get(episode, ("ShortDescription", "TinyDescription"))
        )
        if (
            not codename
            or not title
            or not episode.get("HasPlayableStream", True)
        ):
            return
        thumbnail = next(
            (
                url_or_none(thumb.get("Url"))
                for thumb in variadic(
                    dict_get(episode, ("Images", "Image")) or {}
                )
                if thumb.get("IsMain")
            ),
            None,
    ...
    def _extract_series(
        self, video_id, season_titles=None, season_numbers=None
    ):
        ...
        series_info = {
            ...
            "description": dict_get(
                series, ("ShortDescription", "TinyDescription")
            ),
        }
        if season_numbers:
            season_titles = season_titles or []
            for season in try_get(series, lambda x: x["Seasons"], list) or []:
                if season.get("SeasonNumber") in season_numbers and season.get(
                    "Title"
                ):
                    season_titles.append(season["Title"])

        def gen_episode(m_info, season_titles):
            for episode_group in (
                try_get(m_info, lambda x: x["EpisodeGroups"], list) or []
            ):
                if (
                    season_titles
                    and episode_group.get("Title") not in season_titles
                ):
                    continue
                episodes = try_get(
                ...
                try:
                    episodes = [
                        (int(ep["EpisodeNumber"]), ep) for ep in episodes
                    ]
                    episodes.sort()
        ...
        return self.playlist_result(
            gen_episode(media_info, season_titles),
    ...
    def _real_extract(self, url):
        ...
        if video_id.startswith("ser."):
            ...
            param_season = [
                (
                    have_number,
                    int_or_none(v) if have_number else str_or_none(v),
                )
