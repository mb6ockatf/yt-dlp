...
class ThreeSpeakIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?3speak\.tv/watch\?v\=[^/]+/(?P<id>[^/$&#?]+)"
    )
    ...
    def _real_extract(self, url):
        ...
        data_json = self._parse_json(
            self._parse_json(f'"{json_str}"', video_id), video_id
        )
        ...
        if ipfs_m3u8:
            ipfs_frmts, ipfs_subs = self._extract_m3u8_formats_and_subtitles(
                f"https://ipfs.3speak.tv/ipfs/{ipfs_m3u8}",
                video_id,
                fatal=False,
                m3u8_id="ipfs",
            )
            ...
            formats.append(
                {
                    ...
                    "format_id": "https-mp4",
                    "duration": try_get(
                        video_json, lambda x: x["video"]["info"]["duration"]
                    ),
                    "filesize": try_get(
                        video_json, lambda x: x["video"]["info"]["filesize"]
                    ),
                    "quality": 11,
        ...
        return {
            ...
            "description": try_get(
                video_json, lambda x: x["video"]["content"]["description"]
            ),
            "tags": try_get(
                video_json, lambda x: x["video"]["content"]["tags"]
            ),
...
class ThreeSpeakUserIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        entries = [
            self.url_result(
                f"https://3speak.tv/watch?v={video}", ie=ThreeSpeakIE.ie_key()
            )
            for video in re.findall(
                r"data-payout\s?\=\s?\"([^\"]+)\"", webpage
            )
            if video
        ]
