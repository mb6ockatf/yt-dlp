...
class GoogleDriveIE(InfoExtractor):
    ...
    def _get_captions_by_type(
        self, video_id, subtitles_id, caption_type, origin_lang_code=None
    ):
    ...
    def _real_extract(self, url):
        ...
        video_info = urllib.parse.parse_qs(
        ...
        if fmt_stream_map and fmt_list:
            ...
            for fmt in fmt_list:
                mobj = re.search(
                    r"^(?P<format_id>\d+)/(?P<width>\d+)[xX](?P<height>\d+)",
                    fmt,
                )
                if mobj:
            ...
            if urlh.headers.get("Content-Disposition"):
                ...
                if confirmation_webpage:
                    confirmed_source_url = extract_attributes(
                        get_element_html_by_id(
                            "download-form", confirmation_webpage
                        )
                        or ""
                    ).get("action")
                    if confirmed_source_url:
                        ...
                        self.report_warning(
                            get_element_by_class(
                                "uc-error-subcaption", confirmation_webpage
                            )
                            or get_element_by_class(
                                "uc-error-caption", confirmation_webpage
                            )
        ...
        if ttsurl:
            ...
            # query string
            subtitles_id = (
                ttsurl.encode().decode("unicode_escape").split("=")[-1]
            )
...
"""
    ...
    def _call_api(self, folder_id, key, data, **kwargs):
        ...
        return (
            self._search_json(
                "", response, "api response", folder_id, **kwargs
            )
            or {}
        )
    ...
    def _real_extract(self, url):
        ...
        folder_info = self._call_api(
            folder_id,
            key,
            self._DATA % f"/drive/v2beta/files/{folder_id} HTTP/1.1",
            fatal=False,
        )

        return self.playlist_from_matches(
            self._get_folder_items(folder_id, key),
            folder_id,
