...
class ZoomIE(InfoExtractor):
    ...
    def _get_real_webpage(self, url, base_url, video_id, url_type):
        ...
        validation = self._download_json(
            base_url
            + "rec/validate%s_passwd" % ("_meet" if is_meeting else ""),
            video_id,
            "Validating passcode",
            "Wrong passcode",
    ...
    def _real_extract(self, url):
        base_url, url_type, video_id = self._match_valid_url(url).group(
            "base_url", "type", "id"
        )
        query = {}
            ...
            formats.append(
                {
                    ...
                    "url": data["viewMp4Url"],
                    "width": int_or_none(
                        traverse_obj(data, ("viewResolvtions", 0))
                    ),
                    "height": int_or_none(
                        traverse_obj(data, ("viewResolvtions", 1))
                    ),
                    "format_id": "view",
                    "ext": "mp4",
                    "filesize_approx": parse_filesize(
                        str_or_none(
                            traverse_obj(data, ("recording", "fileSizeInMB"))
                        )
                    ),
                    "preference": 0,
                }
            ...
            formats.append(
                {
                    ...
                    "width": int_or_none(
                        traverse_obj(data, ("shareResolvtions", 0))
                    ),
                    "height": int_or_none(
                        traverse_obj(data, ("shareResolvtions", 1))
                    ),
                    "format_id": "share",
                    "ext": "mp4",
                    "preference": -1,
            ...
            formats.append(
                {
                    **parse_resolution(
                        self._search_regex(
                            r"_(\d+x\d+)\.mp4",
                            url_basename(view_with_share_url),
                            "resolution",
                            default=None,
                        )
                    ),
