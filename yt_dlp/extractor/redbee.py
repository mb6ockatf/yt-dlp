    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "timestamp": traverse_obj(
                video_info,
                ("event", "publishedStartTime"),
                expected_type=unified_timestamp,
            ),
...
class RTBFIE(RedBeeBaseIE):
    ...
    def _get_formats_and_subtitles(self, url, media_id):
        ...
        session_jwt = (
            try_call(lambda: self._get_cookies(url)["rtbf_jwt"].value)
    ...
    def _real_extract(self, url):
        ...
        embed_page = self._download_webpage(
            "https://www.rtbf.be/auvio/embed/"
            + ("direct" if live else "media"),
            media_id,
            query={"id": media_id},
        )
        ...
        m3u8_url = (
            None
            if data.get("isLive")
            else traverse_obj(data, "urlHlsAes128", "urlHls")
        )
        if m3u8_url:
            fmts, subs = self._extract_m3u8_formats_and_subtitles(
        ...
        fix_url = lambda x: (
            x.replace("//rtbf-vod.", "//rtbf.") if "/geo/drm/" in x else x
        )
        ...
        if formats and http_url and re.search(height_re, http_url):
            ...
            for m3u8_f in formats[:]:
                ...
                f.update(
                    {
                        "format_id": m3u8_f["format_id"].replace(
                            "hls-", "http-"
                        ),
        ...
        if mpd_url and (
            self.get_param("allow_unplayable_formats") or not data.get("drm")
        ):
            fmts, subs = self._extract_mpd_formats_and_subtitles(
