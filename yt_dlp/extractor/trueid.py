...
class TrueIDIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        try:
            ...
            if "country" in errmsg:
                self.raise_geo_restricted(
                    errmsg,
                    (
                        [initial_data["display_country"]]
                        if initial_data.get("display_country")
                        else None
                    ),
                    True,
                )
        ...
        thumbnails = [
            ...
            for thumb_key, thumb_url in (
                initial_data.get("thumb_list") or {}
            ).items()
        ...
        return {
            ...
            "categories": traverse_obj(
                initial_data, ("article_category_details", ..., "name")
            ),
            "release_timestamp": unified_timestamp(
                initial_data.get("publish_date")
            ),
            "release_year": int_or_none(initial_data.get("release_year")),
            "formats": formats,
            "subtitles": subs,
            "thumbnails": thumbnails,
            "age_limit": self._CUSTOM_RATINGS.get(initial_data.get("rate"))
            or parse_age_limit(initial_data.get("rate")),
