...
class JixieBaseIE(InfoExtractor):
    ...
    def _extract_data_from_jixie_id(self, display_id, video_id, webpage):
        json_data = self._download_json(
            "https://apidam.jixie.io/api/public/stream",
            display_id,
            query={"metadata": "full", "video_id": video_id},
        )["data"]
        ...
        for stream in json_data["streams"]:
            if stream.get("type") == "HLS":
                fmt, sub = self._extract_m3u8_formats_and_subtitles(
                    stream.get("url"), display_id, ext="mp4"
                )
        ...
        return {
            ...
            "description": (
                clean_html(
                    traverse_obj(json_data, ("metadata", "description"))
                )
            ...
            "duration": float_or_none(
                traverse_obj(json_data, ("metadata", "duration"))
            ),
            "tags": try_call(
                lambda: (json_data["metadata"]["keywords"] or None).split(",")
            ),
            "categories": try_call(
                lambda: (json_data["metadata"]["categories"] or None).split(
                    ","
                )
            ),
