...
class LiTVIE(InfoExtractor):
    ...
    def _extract_playlist(self, playlist_data, content_type):
        all_episodes = [
            ...
            for episode in traverse_obj(
                playlist_data,
                ("seasons", ..., "episodes", lambda _, v: v["content_id"]),
            )
        ]
    ...
    def _real_extract(self, url):
        ...
        vod_data = self._search_nextjs_data(webpage, video_id)["props"][
            "pageProps"
        ]

        program_info = (
            traverse_obj(vod_data, ("programInformation", {dict})) or {}
        )
        ...
        puid = try_call(
            lambda: self._get_cookies("https://www.litv.tv/")["PUID"].value
        )
        if puid:
            endpoint = "get-urls"
        else:
        ...
        video_data = self._download_json(
            f"https://www.litv.tv/api/{endpoint}",
            video_id,
            data=json.dumps(
                {"AssetId": asset_id, "MediaType": media_type, "puid": puid}
            ).encode(),
        ...
        return {
            ...
            "formats": formats,
            "title": join_nonempty(
                "title", "secondary_mark", delim="", from_dict=program_info
            ),
            **traverse_obj(
                program_info,
                {
                    ...
                    "thumbnail": (
                        "picture",
                        {urljoin("https://p-cdnstatic.svc.litv.tv/")},
                    ),
