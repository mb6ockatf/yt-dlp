...
class TwitCastingIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if re.search(
            r'<form\s+method="POST">\s*<input\s+[^>]+?name="password"', webpage
        ):
        ...
        title = clean_html(
            get_element_by_id("movietitle", webpage)
        ...
        video_js_data = try_get(
            webpage,
            lambda x: self._parse_data_movie_playlist(
                self._search_regex(
        ...
        thumbnail = traverse_obj(
            video_js_data, (0, "thumbnailUrl")
        ) or self._og_search_thumbnail(webpage)
        description = clean_html(
            get_element_by_id("authorcomment", webpage)
        ) or self._html_search_meta(
        ...
        stream_server_data = self._download_json(
            f"https://twitcasting.tv/streamserver.php?target={uploader_id}&mode=client",
            video_id,
            "Downloading live info",
            fatal=False,
        )
        ...
        m3u8_urls = (
            try_get(webpage, find_dmu, list)
            or traverse_obj(video_js_data, (..., "source", "url"))
            or (
                [f"https://twitcasting.tv/{uploader_id}/metastream.m3u8"]
                if is_live
                else None
            )
        )
        ...
        if is_live:
            ...
            if websockets:
                ...
                streams = (
                    traverse_obj(stream_server_data, ("llfmp4", "streams"))
                    or {}
                )
    ...
    def _entries(self, uploader_id):
        ...
        for page_num in itertools.count(1):
            ...
            next_url = self._search_regex(
                r'<a href="(/%s/show/%d-\d+)[?"]'
                % (re.escape(uploader_id), page_num),
                webpage,
                "next url",
                default=None,
            )
