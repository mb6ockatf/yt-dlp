...
class PatreonBaseIE(InfoExtractor):
    ...
    def _call_api(
        self, ep, item_id, query=None, headers=None, fatal=True, note=None
    ):
        if headers is None:
            headers = {}
        ...
        try:
            ...
            if (
                ...
                or mimetype2ext(e.cause.response.headers.get("Content-Type"))
                != "json"
            ):
                raise
            ...
            err_message = traverse_obj(
                err_json, ("errors", ..., "detail"), get_all=False
            )
            if err_message:
                raise ExtractorError(
                    f"Patreon said: {err_message}", expected=True
                )
            raise
...
class PatreonIE(PatreonBaseIE):
    ...
    def _real_extract(self, url):
        ...
        entries = []
        idx = 0
        for include in traverse_obj(
            post, ("included", lambda _, v: v["type"])
        ):
            include_type = include["type"]
            if include_type == "media":
                media_attributes = (
                    traverse_obj(include, ("attributes", {dict})) or {}
                )
                download_url = url_or_none(
                    media_attributes.get("download_url")
                )
                    ...
                    entries.append(
                        {
                            ...
                            "filesize": size_bytes,
                            "url": download_url,
                            "alt_title": traverse_obj(
                                media_attributes, ("file_name", {str})
                            ),
                        }
                    )
        ...
        if traverse_obj(attributes, ("embed", "provider")) == "Vimeo":
            v_url = urllib.parse.unquote(
                self._html_search_regex(
                    r"(https(?:%3A%2F%2F|://)player\.vimeo\.com.+app_id(?:=|%3D)+\d+)",
                    traverse_obj(attributes, ("embed", "html", {str})),
                    "vimeo url",
                    fatal=False,
                )
                or ""
            ...
            if url_or_none(v_url) and self._request_webpage(
                v_url,
                video_id,
                "Checking Vimeo embed URL",
                headers=headers,
                fatal=False,
                errnote=False,
                expected_status=429,
            ):  # 429 is TLS fingerprint rejection
                entries.append(
                    self.url_result(
                        VimeoIE._smuggle_referrer(
                            v_url, "https://patreon.com/"
                        ),
                        VimeoIE,
        ...
        if embed_url and (
            ...
            meta_description = clean_html(
                self._html_search_meta(
                    "description",
                    self._webpage_read_content(
                        urlh, embed_url, video_id, fatal=False
                    ),
                    default=None,
                )
            )
            ...
            if (
                urlh.status != 403
                and meta_description != "Sorry, we couldn’t find that page"
            ) or VidsIoIE.suitable(embed_url):
                entries.append(
                    self.url_result(smuggle_url(embed_url, headers))
                )
        ...
        if post_file:
            ...
            if ext in KNOWN_EXTENSIONS:
                ...
                name == "video"
                or determine_ext(post_file.get("url")) == "m3u8"
            ):
                formats, subtitles = self._extract_m3u8_formats_and_subtitles(
                    post_file["url"], video_id
                )
                entries.append(
                    {
                        "id": video_id,
        ...
        if not entries and can_view_post is False:
            ...
            self.raise_no_formats(
                "No supported media found in this post",
                video_id=video_id,
                expected=True,
            )
        elif len(entries) == 1:
            ...
            return self.playlist_result(
                entries, video_id, **info, __post_extractor=comments
            )
    ...
    def _get_comments(self, post_id):
        ...
        for page in itertools.count(1):
            ...
            for comment in traverse_obj(
                ...
                (
                    ("data", "included"),
                    lambda _, v: v["type"] == "comment" and v["id"],
                ),
            ):
                count += 1
                author_id = traverse_obj(
                    comment, ("relationships", "commenter", "data", "id")
                )
...
class PatreonCampaignIE(PatreonBaseIE):
    ...
    def _entries(self, campaign_id):
        ...
        for page in itertools.count(1):
            ...
            params.update({"page[cursor]": cursor} if cursor else {})
            posts_json = self._call_api(
                "posts",
                campaign_id,
                query=params,
                note=f"Downloading posts page {page}",
            )

            cursor = traverse_obj(
                posts_json, ("meta", "pagination", "cursors", "next")
            )
            for post_url in traverse_obj(
                posts_json, ("data", ..., "attributes", "patreon_url")
            ):
                yield self.url_result(
                    urljoin("https://www.patreon.com/", post_url), PatreonIE
                )
    ...
    def _real_extract(self, url):
        ...
        campaign_id, vanity = self._match_valid_url(url).group(
            "campaign_id", "vanity"
        )
        if campaign_id is None:
            webpage = self._download_webpage(
                url, vanity, headers={"User-Agent": self.patreon_user_agent}
            )
            campaign_id = self._search_nextjs_data(webpage, vanity)["props"][
                "pageProps"
            ]["bootstrapEnvelope"]["pageBootstrap"]["campaign"]["data"]["id"]
        ...
        return {
            ...
            "description": clean_html(
                traverse_obj(campaign_info, ("attributes", "summary"))
            ),
            "channel_url": traverse_obj(campaign_info, ("attributes", "url")),
            "channel_follower_count": int_or_none(
                traverse_obj(campaign_info, ("attributes", "patron_count"))
            ),
            ...
            "playlist_count": traverse_obj(
                campaign_info, ("attributes", "creation_count")
            ),
            "age_limit": (
                18
                if traverse_obj(campaign_info, ("attributes", "is_nsfw"))
                else 0
            ),
