...
class RbgTumCourseIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        course_id, hostname, year, term, slug = self._match_valid_url(
            url
        ).group("id", "hostname", "year", "term", "slug")
        meta = (
            self._download_json(
                f"https://{hostname}/api/courses/{slug}/",
                course_id,
                fatal=False,
        ...
        lectures = [
            self.url_result(
                f"https://{hostname}/w/{slug}/{stream_id}", RbgTumIE
            )
        ...
        if not lectures:
            ...
            lectures = [
                self.url_result(f"https://{hostname}{lecture_path}", RbgTumIE)
                for lecture_path in re.findall(
                    r'href="(/w/[^/"]+/[^/"]+)"', webpage
                )
            ]
...
class RbgTumNewCourseIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        query = parse_qs(url)
        errors = [
            key for key in ("year", "term", "slug") if not query.get(key)
        ]
        if errors:
            raise ExtractorError(
                f'Input URL is missing query parameters: {", ".join(errors)}'
            )
