...
class ClipchampIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        subdomain = (
            self._search_regex(
                r'\bcustomer-domain-prefix=["\']([\w-]+)["\']',
                iframe,
                "subdomain",
                fatal=False,
            )
            or "customer-2ut9yn3y6fta1yxe"
        )

        formats = self._extract_mpd_formats(
            self._STREAM_URL_TMPL % (subdomain, path, "mpd"),
            video_id,
            query=self._STREAM_URL_QUERY,
            fatal=False,
            mpd_id="dash",
        )
        formats.extend(
            self._extract_m3u8_formats(
                self._STREAM_URL_TMPL % (subdomain, path, "m3u8"),
                video_id,
                "mp4",
                query=self._STREAM_URL_QUERY,
                fatal=False,
        ...
        return {
            ...
            "uploader": " ".join(
                traverse_obj(
                    data, ("creator", ("first_name", "last_name"), {str})
                )
            )
            or None,
            **traverse_obj(
