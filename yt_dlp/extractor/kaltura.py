...
class KalturaIE(InfoExtractor):
    ...
    def _extract_embed_urls(cls, url, webpage):
        ...
        for mobj in finditer:
            ...
            if service_mobj:
                embed_url = smuggle_url(
                    embed_url, {"service_url": service_mobj.group("id")}
                )
        ...
        for idx, status in enumerate(data):
            ...
            if status.get("objectType") == "KalturaAPIException":
                raise ExtractorError(
                    "{} said: {} ({})".format(
                        self.IE_NAME, status["message"], idx
                    )
                )
    ...
    def _get_video_info(
        self, video_id, partner_id, service_url=None, player_type="html5"
    ):
        assert player_type in ("html5", "kwidget")
        if player_type == "kwidget":
            return self._get_video_info_kwidget(
                video_id, partner_id, service_url
            )
    ...
    def _get_video_info_html5(self, video_id, partner_id, service_url=None):
        ...
        return self._kaltura_api_call(
            ...
            actions,
            service_url,
            note="Downloading video info JSON (Kaltura html5 player)",
        )
    ...
    def _get_video_info_kwidget(self, video_id, partner_id, service_url=None):
        ...
        header, _, _info, flavor_assets, captions = self._kaltura_api_call(
            ...
            actions,
            service_url,
            note="Downloading video info JSON (Kaltura kwidget player)",
        )
    ...
    def _real_extract(self, url):
        ...
        partner_id, entry_id, player_type = mobj.group(
            "partner_id", "id", "player_type"
        )
        ...
        if partner_id and entry_id:
            ...
            if "entry_id" in params:
                ...
                info, flavor_assets = (
                    entry_data["meta"],
                    entry_data["contextData"]["flavorAssets"],
                )
                entry_id = info["id"]
                ...
                "uiconf_id" in params
                and "flashvars[playlistAPI.kpl0Id]" in params
            ):
                ...
                return self.playlist_from_matches(
                    traverse_obj(
                        playlist_data, (playlist_id, "items", ..., "id")
                    ),
                    playlist_id,
                    traverse_obj(playlist_data, (playlist_id, "name")),
                    ie=KalturaIE,
                    getter=lambda x: f"kaltura:{partner_id}:{x}:{player_type}",
                )
            else:
                raise ExtractorError("Invalid URL", expected=True)
            ks = params.get("flashvars[ks]", [None])[0]

        return self._per_video_extract(
            smuggled_data, entry_id, info, ks, flavor_assets, captions
        )

    def _per_video_extract(
        self, smuggled_data, entry_id, info, ks, flavor_assets, captions
    ):
        source_url = smuggled_data.get("source_url")
        if source_url:
            referrer = base64.b64encode(
                "://".join(urllib.parse.urlparse(source_url)[:2]).encode()
            ).decode("utf-8")
        else:
            referrer = None
        ...
        for f in flavor_assets:
            ...
            if not f.get("fileExt"):
                ...
                if f.get("containerFormat") == "qt":
                    ...
                    f["fileExt"] = "mp4"
            video_url = sign_url("{}/flavorId/{}".format(data_url, f["id"]))
            ...
            vcodec = (
                "none"
                if "videoCodecId" not in f and f.get("frameRate") == 0
                else f.get("videoCodecId")
            )
            formats.append(
                {
                    "format_id": format_id,
                    ...
                    "filesize_approx": int_or_none(
                        f.get("size"), invscale=1024
                    ),
        ...
        if captions:
            for caption in captions.get("objects", []):
                ...
                subtitles.setdefault(
                    ...
                    {
                        "url": "{}/api_v3/service/caption_captionasset/action/serve/captionAssetId/{}".format(
                            self._SERVICE_URL, caption["id"]
                        ),
                        "ext": caption.get("fileExt")
                        or self._CAPTION_TYPES.get(caption_format)
                        or "ttml",
                    }
                )
