...
class MediasetIE(ThePlatformBaseIE):
    ...
    _TESTS = [
        {
            ...
            "info_dict": {
                ...
                "episode_number": 1,
                "chapters": [
                    {"start_time": 0.0, "end_time": 439.88},
                    {"start_time": 439.88, "end_time": 1685.84},
                    {"start_time": 1685.84, "end_time": 2682.0},
                ],
            },
        },
        {
            ...
            "info_dict": {
                ...
                "episode_number": 5,
                "chapters": [
                    {"start_time": 0.0, "end_time": 3409.08},
                    {"start_time": 3409.08, "end_time": 6565.008},
                ],
        ...
        },
        {
            ...
            "info_dict": {
                ...
                "uploader_id": "B6",
                "chapters": [
                    {"start_time": 0.0, "end_time": 1938.56},
                    {"start_time": 1938.56, "end_time": 5233.01},
                ],
            },
            "params": {
    ...
    def _check_drm_formats(self, tp_formats, video_id):
        ...
        nodrm_manifest = re.sub(
            r"_sampleaes/(\w+)_fp_", r"/\1_no_", drm_manifest
        )
        ...
        tp_formats.extend(
            self._extract_m3u8_formats(
                nodrm_manifest, video_id, m3u8_id="hls", fatal=False
            )
            or []
        )
        ...
        for f in ("MPEG4", "MPEG-DASH", "M3U"):
            try:
                tp_formats, tp_subtitles = self._extract_theplatform_smil(
                    update_url_query(
                        f"http://link.theplatform.{self._TP_TLD}/s/{tp_path}",
                        {
                            "mbr": "true",
                            "formats": f,
                    ...
                    guid,
                    f'Downloading {f.split("+")[0]} SMIL data',
                )
            ...
            info.update(
                {
                    ...
                    or feed_data.get("description")
                    or feed_data.get("longDescription"),
...
class MediasetShowIE(MediasetIE):  # XXX: Do not subclass from concrete IE
    ...
    def _real_extract(self, url):
        playlist_id, st, sb = self._match_valid_url(url).group(
            "id", "st", "sb"
        )
        if not sb:
            ...
            entries = [
                self.url_result(
                    urljoin("https://mediasetinfinity.mediaset.it", url)
                )
                for url in re.findall(
                    r'href="([^<>=]+SE\d{12},ST\d{12},sb\d{9})">[^<]+<', page
                )
            ]
        ...
        entries = OnDemandPagedList(
            functools.partial(self._fetch_page, sb), self._PAGE_SIZE
        )
