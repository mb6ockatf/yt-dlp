...
class LivestreamfailsIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?livestreamfails\.com/(?:clip|post)/(?P<id>[0-9]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        api_response = self._download_json(
            f"https://api.livestreamfails.com/clip/{video_id}", video_id
        )
        ...
        return {
            ...
            "thumbnail": format_field(
                api_response,
                "imageId",
                "https://livestreamfails-image-prod.b-cdn.net/image/%s",
            ),
        }
