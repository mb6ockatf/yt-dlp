    ...
    def _real_extract(self, url):
        ...
        if is_song:
            ...
            video = self._parse_json(
                self._search_regex(r"context = ({.*?});", webpage, "context"),
                video_id,
            )["video"]
            formats = formats_from_stream_urls(
                video.get("streamUrl"),
                video.get("hlsStreamUrl"),
                video.get("mp4StreamUrl"),
...
class MySpaceAlbumIE(InfoExtractor):
    ...
    _VALID_URL = (
        r"https?://myspace\.com/([^/]+)/music/album/(?P<title>.*-)(?P<id>\d+)"
    )
    ...
    def _real_extract(self, url):
        ...
        if not tracks_paths:
            raise ExtractorError(
                f"{display_id}: No songs found, try using proxy", expected=True
            )
        entries = [
