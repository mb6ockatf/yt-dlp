...
class OneNewsNZIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        for item in traverse_obj(fusion_metadata, "content_elements") or []:
            ...
            if item_type == "video":
                ...
                brightcove_url = self.BRIGHTCOVE_URL_TEMPLATE % (
                    traverse_obj(brightcove_config, "brightcoveAccount")
                    or "963482464001",
                    traverse_obj(brightcove_config, "brightcoveVideoId"),
                )
                entries.append(
                    self.url_result(brightcove_url, BrightcoveNewIE)
                )
                ...
                if video_id_or_url:
                    entries.append(
                        self.url_result(video_id_or_url, ie="Youtube")
                    )
        ...
        playlist_title = traverse_obj(
            fusion_metadata, ("headlines", "basic")
