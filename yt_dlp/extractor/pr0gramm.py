...
class Pr0grammIE(InfoExtractor):
    ...
    def _call_api(
        self, endpoint, video_id, query={}, note="Downloading API json"
    ):
        ...
        if error in (
            ...
            raise ExtractorError(
                f"API returned error: {message}", expected=True
            )
    ...
    def _real_extract(self, url):
        ...
        video_info = traverse_obj(
            self._call_api(
                "get", video_id, {"id": video_id, "flags": self._maximum_flags}
            ),
            ("items", 0, {dict}),
        )
        ...
        metadata = self._call_api(
            "info", video_id, {"itemId": video_id}, note="Downloading tags"
        )
        ...
        if confidences:
            tags = [
                tag for _, tag in sorted(zip(confidences, tags), reverse=True)
            ]
        ...
        return {
            ...
            "age_limit": (
                18
                if traverse_obj(video_info, ("flags", {0b110.__and__}))
                else 0
            ),
            ...
            **traverse_obj(
                video_info,
                {
                    ...
                    "thumbnail": (
                        "thumb",
                        {urljoin("https://thumb.pr0gramm.com")},
                    ),
                },
            ),
        }
