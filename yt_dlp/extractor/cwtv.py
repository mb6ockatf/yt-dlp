    ...
    def _real_extract(self, url):
        ...
        if traverse_obj(data, "result") != "ok":
            raise ExtractorError(
                traverse_obj(data, (("error_msg", "msg"), {str}, any)),
                expected=True,
            )
        ...
        mpx_url = update_url_query(
            ...
            or f"https://link.theplatform.com/s/cwtv/media/guid/2703454149/{video_id}",
            {"formats": "M3U+none"},
        )
...
class CWTVMovieIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        app_url = self._html_search_meta(
            "al:ios:url", webpage, default=None
        ) or self._html_search_meta("al:android:url", webpage, default=None)
        video_id = traverse_obj(
            parse_qs(app_url),
            ("video_id", 0, {lambda x: re.fullmatch(self._UUID_RE, x)}, 0),
