...
class VoicyBaseIE(InfoExtractor):
    def _extract_from_playlist_data(self, value):
        ...
        items = [
            self._extract_single_article(voice_data)
            for voice_data in value["VoiceData"]
        ]
        return {
            "_type": "multi_video",
            "entries": items,
    ...
    def _call_api(self, url, video_id, **kwargs):
        ...
        if response.get("Status") != 0:
            ...
            if not message:
                message = (
                    "There was a error in the response: %d"
                    % response.get("Status")
                )
    ...
    def _real_extract(self, url):
        ...
        if not article_list:
            article_list = self._call_api(
                self.ARTICLE_LIST_API_URL % (channel_id, voice_id), voice_id
            )
...
class VoicyChannelIE(VoicyBaseIE):
    ...
    def _real_extract(self, url):
        ...
        articles = (
            itertools.chain([first_article], articles)
            if first_article
            else articles
        )
