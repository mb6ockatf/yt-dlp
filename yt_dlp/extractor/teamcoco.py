...
class TeamcocoIE(TeamcocoBaseIE):
    ...
    def _real_extract(self, url):
        ...
        data = self._search_nextjs_data(webpage, display_id)["props"][
            "pageProps"
        ]["pageData"]
        info = merge_dicts(
            *traverse_obj(
                ...
                (
                    "blocks",
                    lambda _, v: v["name"]
                    in ("meta-tags", "video-player", "video-info"),
                    "props",
                    {dict},
                ),
            )
        ...
        thumbnail = traverse_obj(
            info,
            (("image", "poster"), {urljoin("https://teamcoco.com/")}),
            get_all=False,
        )
    ...
    def _real_extract(self, url):
        ...
        video_id = traverse_obj(
            ...
            (
                ...
                "props",
                "fieldDefs",
                lambda _, v: v["name"] == "incomingVideoId",
                "value",
            ),
            ("blocks", ..., "props", "fields", "incomingVideoRecord", "id"),
            get_all=False,
        )
        if not video_id:
            self.raise_no_formats(
                "Unable to extract video ID from webpage", expected=True
            )
        ...
        media_id = traverse_obj(
            ...
            (
                "data",
                ("findRecord", "findRecordVideoMetadata"),
                "turnerMediaId",
            ),
            get_all=False,
        )
        if media_id:
            ...
            if not token:
                raise ExtractorError(
                    "No Turner Media auth token found in API response"
                )
            ...
            formats, subtitles = self._get_formats_and_subtitles(
                traverse_obj(response, ("data", "findRecordVideoMetadata")),
                video_id,
            )
            info.update(
                {
