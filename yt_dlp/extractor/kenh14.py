...
class Kenh14VideoIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        formats = [
            {"url": f"https://{direct_url}", "format_id": "http", "quality": 1}
        ]
        subtitles = {}
        ...
        return {
            **traverse_obj(
            ...
            "thumbnail": (
                self._og_search_thumbnail(webpage) or attrs.get("data-thumb")
            ),
            "tags": traverse_obj(
                self._html_search_meta("keywords", webpage),
...
class Kenh14PlaylistIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        category_detail = (
            get_element_by_class("category-detail", webpage) or ""
        )
        embed_info = (
            traverse_obj(
                self._yield_json_ld(webpage, playlist_id),
                (lambda _, v: v["name"] and v["alternateName"], any),
            )
            or {}
        )
        ...
        return self.playlist_from_matches(
            ...
            playlist_id,
            (
                clean_html(get_element_by_class("name", category_detail))
                or unescapeHTML(embed_info.get("name"))
            ),
            getter=lambda x: "https://video.kenh14.vn/video/video-{}.chn".format(
                extract_attributes(x)["data-id"]
            ),
            ie=Kenh14VideoIE,
            playlist_description=(
                clean_html(
                    get_element_by_class("description", category_detail)
                )
