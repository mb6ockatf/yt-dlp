    ...
    def _parse_video_data(video_data):
        ...
        info = traverse_obj(
            video_data,
            {
                ...
                "thumbnails": (
                    ("clipImageThumb", "clipImageSource"),
                    {"url": {media_url_or_none}},
                ),
        ...
        if info.get("id") and info.get("url"):
            ...
            info["webpage_url"] = (
                f'https://allstar.gg/{basename}?{basename}={info["id"]}'
            )
        ...
        info.update(
            {
                ...
                "uploader_url": urljoin(
                    "https://allstar.gg/u/", info.get("uploader_id")
                ),
            }
        )
...
class AllstarProfileIE(AllstarBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?allstar\.gg/(?:profile\?user=|u/)(?P<id>[^/?#&]+)"
    )
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            ...
            playlist_id=join_nonempty(
                user_id, query_id.lower().split()[0], game
            ),
            playlist_title=join_nonempty(
                (username or display_id), query_id, game, delim=" - "
            ),
        )
