...
class TrillerBaseIE(InfoExtractor):
    ...
    def _perform_login(self, username, password):
        ...
        user_check = traverse_obj(
            self._download_json(
                f"{self._API_BASE_URL}/api/user/is-valid-username",
                None,
        ...
        if user_check:  # endpoint returns `"status":false` if username exists
            raise ExtractorError(
                "Unable to login: Invalid username", expected=True
            )

        login = (
            self._download_json(
                f"{self._API_BASE_URL}/user/auth",
                None,
                note="Logging in",
                fatal=False,
                expected_status=400,
    ...
    def _get_comments(self, video_id, limit=15):
        comment_info = (
            self._download_json(
                ...
                video_id,
                fatal=False,
                note="Downloading comments API JSON",
    ...
    def _parse_video_info(
        self, video_info, username, user_id, display_id=None
    ):
        ...
        if traverse_obj(
            ...
            (
                None,
                ("transcoded_url", "video_url", "stream_url", "audio_url"),
                {lambda x: re.search(r"/copyright/", x)},
            ),
            get_all=False,
        ):
        ...
        for video in traverse_obj(
            video_info, ("video_set", lambda _, v: url_or_none(v["url"]))
        ):
            formats.append(
                {
        ...
        video_url = traverse_obj(
            video_info, "video_url", "stream_url", expected_type=url_or_none
        )
        if video_url:
            formats.append(
                {
        ...
        return {
            ...
            "uploader": username,
            "uploader_id": user_id
            or traverse_obj(video_info, ("user", "user_id", {str_or_none})),
            "webpage_url": urljoin(
                f"https://triller.co/@{username}/video/", display_id
            ),
                ...
                video_info,
                {
                    ...
                    "creator": (
                        (
                            ...
                            (
                                "users",
                                lambda _, v: str(v["user_id"]) == user_id,
                            ),
                        ),
                        "name",
                    ),
...
class TrillerIE(TrillerBaseIE):
    ...
    def _real_extract(self, url):
        username, display_id = self._match_valid_url(url).group(
            "username", "id"
        )

        video_info = self._download_json(
            f"{self._API_BASE_URL}/api/videos/{display_id}",
            display_id,
            headers=self._API_HEADERS,
        )["videos"][0]
...
class TrillerUserIE(TrillerBaseIE):
    ...
    def _real_extract(self, url):
        ...
        if user_info.get("private") and user_info.get(
            ...
            raise ExtractorError(
                "The author of the video is blocked", expected=True
            )
        ...
        return self.playlist_result(
            self._entries(username, user_id),
            user_id,
            username,
...
class TrillerShortIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        real_url = self._request_webpage(
            HEADRequest(url), self._match_id(url)
        ).url
        if self.suitable(
            real_url
