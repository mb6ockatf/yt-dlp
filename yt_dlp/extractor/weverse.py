...
class WeverseBaseIE(InfoExtractor):
    ...
    def _perform_login(self, username, password):
        ...
        valid_username = traverse_obj(
            self._download_json(
                f"{self._ACCOUNT_API_BASE}/v2/signup/email/status",
                None,
                note="Checking username",
        ...
        try:
            auth = self._download_json(
                f"{self._ACCOUNT_API_BASE}/v3/auth/token/by-credentials",
                None,
                data=json.dumps(
                    {
                        "email": username,
            ...
            if isinstance(e.cause, HTTPError) and e.cause.status == 401:
                raise ExtractorError(
                    "Invalid password provided", expected=True
                )
            raise

        WeverseBaseIE._API_HEADERS["Authorization"] = (
            f'Bearer {auth["accessToken"]}'
        )
    ...
    def _real_initialize(self):
        ...
        token = try_call(
            lambda: self._get_cookies("https://weverse.io/")[
                "we2_access_token"
            ].value
        )
        if token:
    ...
    def _call_api(self, ep, video_id, data=None, note="Downloading API JSON"):
        ...
        try:
            return self._download_json(
                f"https://global.apis.naver.com/weverse/wevweb{api_path}",
                video_id,
                note=note,
                data=data,
    ...
    def _call_post_api(self, video_id):
        ...
        return self._call_api(
            f"/post/v1.0/post-{video_id}{path}?fieldSet=postV1", video_id
        )
    ...
    def _get_formats(self, data, video_id):
        formats = traverse_obj(
            ...
            (
                "videos",
                "list",
                lambda _, v: url_or_none(v["source"]),
                {
                    "url": "source",
        ...
        for stream in traverse_obj(
            ...
            (
                "streams",
                lambda _, v: v["type"] == "HLS" and url_or_none(v["source"]),
            ),
        ):
            query = {}
            for param in traverse_obj(
                stream,
                ("keys", lambda _, v: v["type"] == "param" and v["name"]),
            ):
    ...
    def _parse_post_meta(self, metadata):
        return traverse_obj(
            ...
            {
                "title": (
                    (("extension", "mediaInfo", "title"), "title"),
                    {str},
                ),
                "description": (
                    (("extension", "mediaInfo", "body"), "body"),
                    {str},
                ),
                ...
                "thumbnail": (
                    "extension",
                    (("mediaInfo", "thumbnail", "url"), ("video", "thumb")),
                    {url_or_none},
                ),
                "view_count": (
                    "extension",
    ...
    def _real_extract(self, url):
        ...
        if live_status == "is_upcoming":
            self.raise_no_formats(
                "Livestream has not yet started", expected=True
            )
            ...
            in_key = self._call_api(
                ...
                video_id,
                data=b"{}",
                note="Downloading VOD API key",
            )["inKey"]

            video_info = self._download_json(
                f"https://global.apis.naver.com/rmcnmv/rmcnmv/vod/play/v2.0/{infra_video_id}",
                video_id,
                note="Downloading VOD JSON",
                query={
                    "key": in_key,
                    "sid": traverse_obj(
                        post, ("extension", "video", "serviceId")
                    )
                    or "2070",
                    "pid": str(uuid.uuid4()),
            ...
            has_drm = (
                traverse_obj(
                    video_info, ("meta", "provider", "name", {str.lower})
                )
                == "drm"
            )
...
class WeverseMediaIE(WeverseBaseIE):
    ...
    def _real_extract(self, url):
        ...
        if media_type == "vod":
            ...
            self.raise_no_formats(
                "No video content found in webpage", expected=True
            )
        elif media_type:
...
class WeverseMomentIE(WeverseBaseIE):
    ...
    def _real_extract(self, url):
        channel, uploader_id, video_id = self._match_valid_url(url).group(
            "artist", "uid", "id"
        )
        post = self._call_post_api(video_id)
...
class WeverseTabBaseIE(WeverseBaseIE):
        ...
        for page in itertools.count(1):
            posts = (
                ...
                if page == 1
                    ...
                    update_url_query(self._ENDPOINT % channel_id, query),
                    channel,
    ...
    def _real_extract(self, url):
        ...
        first_page = self._call_api(
            update_url_query(self._ENDPOINT % channel_id, self._QUERY),
            channel,
            note=f"Downloading {self._PATH} tab page 1",
        )

        return self.playlist_result(
            self._entries(channel_id, channel, first_page),
            f"{channel}-{self._PATH}",
            **traverse_obj(
                first_page,
                (
                    "data",
...
class WeverseLiveTabIE(WeverseTabBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.|m\.)?weverse\.io/(?P<id>[^/?#]+)/live/?(?:[?#]|$)"
    )
    _TESTS = [
        {
...
class WeverseLiveIE(WeverseBaseIE):
    ...
    def _real_extract(self, url):
        ...
        video_id = traverse_obj(
            ...
            (
                ("onAirLivePosts", "reservedLivePosts"),
                "data",
                lambda _, v: self._extract_live_status(v)
