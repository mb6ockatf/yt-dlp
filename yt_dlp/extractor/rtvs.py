...
class RTVSIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        iframe_id = self._search_regex(
            r'<iframe[^>]+id\s*=\s*"player_[^_]+_([0-9]+)"',
            webpage,
            "Iframe ID",
        )
        iframe_url = self._search_regex(
            rf'<iframe[^>]+id\s*=\s*"player_[^_]+_{re.escape(iframe_id)}"[^>]+src\s*=\s*"([^"]+)"',
            webpage,
            "Iframe URL",
        )

        webpage = self._download_webpage(
            iframe_url, video_id, "Downloading iframe"
        )
        json_url = self._search_regex(
            r'var\s+url\s*=\s*"([^"]+)"\s*\+\s*ruurl', webpage, "json URL"
        )
        data = self._download_json(
            f"https:{json_url}b=mozilla&p=win&v=97&f=0&d=1", video_id
        )
        ...
        if (
            traverse_obj(data, ("playlist", 0, "sources", 0, "type"))
            == "audio/mp3"
        ):
            formats = [
                {
                    "url": traverse_obj(
                        data, ("playlist", 0, "sources", 0, "src")
                    )
                }
            ]
        else:
            formats = self._extract_m3u8_formats(
                traverse_obj(data, ("playlist", 0, "sources", 0, "src")),
                video_id,
            )
        ...
        return {
            ...
            "duration": parse_duration(
                traverse_obj(data, ("playlist", 0, "length"))
            ),
            "thumbnail": traverse_obj(data, ("playlist", 0, "image")),
            "timestamp": unified_timestamp(
                traverse_obj(data, ("playlist", 0, "datetime_create"))
            ),
            "formats": formats,
        }
