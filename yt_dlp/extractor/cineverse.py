...
class CineverseIE(CineverseBaseIE):
    ...
    def _real_extract(self, url):
        ...
        idetails = self._search_nextjs_data(html, video_id)["props"][
            "pageProps"
        ]["idetails"]
...
class CineverseDetailsIE(CineverseBaseIE):
    _VALID_URL = (
        rf"{CineverseBaseIE._VALID_URL_BASE}/details/(?P<id>[A-Z0-9]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        pageprops = self._search_nextjs_data(html, series_id)["props"][
            "pageProps"
        ]
        ...
        geoblocked = (
            traverse_obj(pageprops, ("itemDetailsData", "playback_err_msg"))
            == "This title is not available in your location."
        )

        def item_result(item):
            item_url = (
                f'https://www.{host}/watch/{item["item_id"]}/{item["title"]}'
            )
            if geoblocked:
        ...
        season = traverse_obj(
            ...
            (
                ...
                ...,
                "episodes",
                lambda _, v: v["item_id"] and v["title"],
            ),
        )
        if season:
            return self.playlist_result(
                ...
                playlist_title=traverse_obj(
                    pageprops, ("itemDetailsData", "title")
                ),
            )
