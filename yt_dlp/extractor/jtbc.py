    ...
    def _real_extract(self, url):
        ...
        if display_id.startswith("vo"):
            ...
            video_id = self._search_regex(
                r'data-vod="(VO\d+)"', webpage, "vod id"
            )

        playback_data = self._download_json(
            f"https://api.jtbc.co.kr/vod/{video_id}",
            video_id,
            note="Downloading VOD playback data",
        )

        subtitles = {}
        for sub in traverse_obj(
            playback_data, ("tracks", lambda _, v: v["file"])
        ):
            subtitles.setdefault(sub.get("label", "und"), []).append(
                {"url": sub["file"]}
            )

        formats = []
        for stream_url in traverse_obj(
            playback_data, ("sources", "HLS", ..., "file", {url_or_none})
        ):
            stream_url = re.sub(
                r"/playlist(?:_pd\d+)?\.m3u8", "/index.m3u8", stream_url
            )
            formats.extend(
                self._extract_m3u8_formats(stream_url, video_id, fatal=False)
            )

        metadata = self._download_json(
            "https://now-api.jtbc.co.kr/v1/vod/detail",
            video_id,
            note="Downloading mobile details",
            fatal=False,
            query={"vodFileId": video_id},
        )
        return {
            "id": video_id,
            ...
            **traverse_obj(
                ...
                (
                    "vodDetail",
                    {
                        ...
                        "release_date": (
                            "broadcastDate",
                            {lambda x: re.match(r"\d{8}", x.replace(".", ""))},
                            0,
                        ),
...
class JTBCProgramIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        vod_list = self._download_json(
            "https://now-api.jtbc.co.kr/v1/vodClip/programHome/programReplayVodList",
            program_id,
            note="Downloading program replay list",
            query={
        ...
        entries = [
            self.url_result(
                f"https://vod.jtbc.co.kr/player/program/{video_id}",
                JTBCIE,
                video_id,
            )
            for video_id in traverse_obj(
                vod_list, ("programReplayVodList", ..., "episodeId")
            )
        ]
