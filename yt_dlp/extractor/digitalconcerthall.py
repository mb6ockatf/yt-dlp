...
class DigitalConcertHallIE(InfoExtractor):
    ...
    def _set_access_token(self, value):
        ...
        self._access_token_expiry = (
            traverse_obj(value, ({jwt_decode_hs256}, "exp", {int})) or 0
        )
    ...
    def _fetch_new_tokens(self, invalidate=False):
        ...
        if not self._refresh_token:
            ...
            raise ExtractorError(
                ...
                f"and try again, {self._REFRESH_HINT}",
                expected=True,
            )
        ...
        bearer_token = (
            ...
            or self._download_json(
                ...
                "Unable to obtain initial token",
    ...
    def _perform_login(self, username, password):
        ...
        if username != "cache":
            raise ExtractorError(
                ...
                f"for this site. {self._LOGIN_HINT}, {self._REFRESH_HINT}",
                expected=True,
            )
    ...
    def _real_initialize(self):
        if not self._access_token:
            self.raise_login_required(
                ...
                f"{self._LOGIN_HINT}, {self._REFRESH_HINT}",
                method=None,
            )
    ...
    def _entries(self, items, language, type_, **kwargs):
        for item in items:
            ...
            yield {
                ...
                or stream_info.get("short_description"),
                **kwargs,
                "chapters": (
                    [
                        {
                            ...
                            "end_time": try_get(
                                chapter, lambda x: x["time"] + x["duration"]
                            ),
                            "title": chapter.get("text"),
                        }
                        for chapter in item["cuepoints"]
                    ]
                    if item.get("cuepoints") and type_ == "concert"
                    else None
                ),
            }

    def _real_extract(self, url):
        language, type_, video_id, part = self._match_valid_url(url).group(
            "language", "type", "id", "part"
        )
        if not language:
            language = "en"
        ...
        videos = (
            [vid_info]
            if type_ == "film"
            else traverse_obj(vid_info, ("_embedded", ..., ...))
        )
        ...
        thumbnail = traverse_obj(
            ...
            (
                ...
                {self._proto_relative_url},
                {url_or_none},
                {lambda x: x.format(width=0, height=0)},
                any,
            ),
