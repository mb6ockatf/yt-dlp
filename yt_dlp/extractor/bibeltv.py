...
class BibelTVBaseIE(InfoExtractor):
        ...
        for media_url in traverse_obj(data, (..., "src", {url_or_none})):
            ...
            if media_ext == "m3u8":
                ...
                mpd_formats, mpd_subs = (
                    self._extract_mpd_formats_and_subtitles(media_url, crn_id)
                )
    ...
    def _extract_url_info(self, data):
        return {
            "_type": "url",
            "url": format_field(
                data, "slug", "https://www.bibeltv.de/mediathek/videos/%s"
            ),
    ...
    def _extract_video_info(self, data):
        ...
        json_data = (
            self._download_json(
                format_field(data, "id", f"{self.API_URL}/video/%s"),
                crn_id,
                headers={"Authorization": self.AUTH_TOKEN},
                fatal=False,
                errnote="No formats available",
            )
            or {}
        )
...
class BibelTVVideoIE(BibelTVBaseIE):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?bibeltv\.de/mediathek/videos/(?P<id>\d+)[\w-]+"
    )
    ...
    def _real_extract(self, url):
        ...
        video_data = traverse_obj(
            self._search_nextjs_data(
                self._download_webpage(url, crn_id), crn_id
            ),
...
class BibelTVSeriesIE(BibelTVBaseIE):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?bibeltv\.de/mediathek/serien/(?P<id>\d+)[\w-]+"
    )
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            ...
            clean_html(series_data.get("description")),
        )
