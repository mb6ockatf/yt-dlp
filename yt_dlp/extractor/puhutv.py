...
class PuhuTVIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        info = self._download_json(
            urljoin(url, f"/api/slug/{display_id}-izle"), display_id
        )["data"]
        ...
        for video in videos["data"]["videos"]:
            ...
            if (
                video.get("stream_type") == "hls" and playlist is True
            ) or "playlist.m3u8" in media_url:
                formats.extend(
        ...
        creator = try_get(show, lambda x: x["producer"]["name"], str)
...
class PuhuTVSerieIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        info = self._download_json(
            urljoin(url, f"/api/slug/{playlist_id}-detay"), playlist_id
        )["data"]
        ...
        return self.url_result(
            f"https://puhutv.com/{video_id}-izle", PuhuTVIE.ie_key(), video_id
        )
