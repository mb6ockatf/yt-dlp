...
class ZDFBaseIE(InfoExtractor):
            ...
            f.update(
                {
                    ...
                    "format_id": join_nonempty(
                        "http", meta.get("type"), meta.get("quality")
                    ),
                    "tbr": int_or_none(
                        self._search_regex(
                            r"_(\d+)k_", format_url, "tbr", default=None
                        )
                    ),
                }
            )
            ...
            merge_dicts(
                f,
                {
                    "format_note": join_nonempty(
                        "quality", "class", from_dict=meta, delim=", "
                    ),
                    "language": meta.get("language"),
                    "language_preference": (
                        10
                        if meta.get("class") == "main"
                        else -10 if meta.get("class") == "ad" else -1
                    ),
    ...
    def _extract_ptmd(self, ptmd_url, video_id, api_token, referrer):
        ...
        duration = float_or_none(
            try_get(ptmd, lambda x: x["attributes"]["duration"]["value"]),
            scale=1000,
        )
    ...
    def _extract_entry(self, url, player, content, video_id):
        ...
        info = self._extract_ptmd(
            urljoin(url, ptmd_path.replace("{playerId}", "android_native_5")),
            video_id,
            player["apiToken"],
            url,
        )
        ...
        return merge_dicts(
            ...
            {
                "title": title,
                "description": content.get("leadParagraph")
    ...
    def _extract_mobile(self, video_id):
        ...
        if isinstance(teaser_bild, dict):
            for thumbnail_key, thumbnail in teaser_bild.items():
                thumbnail_url = try_get(thumbnail, lambda x: x["url"], str)
                if thumbnail_url:
                    thumbnails.append(
                        {
                            "url": thumbnail_url,
        ...
        return {
            ...
            "timestamp": unified_timestamp(document.get("date"))
        ...
        if webpage:
            ...
            if player:
                return self._extract_regular(
                    url, player, video_id, query={"profile": "player-3"}
                )
...
class ZDFChannelIE(ZDFBaseIE):
    ...
    def _entries(self, data, document_id):
        for entry in traverse_obj(
            ...
            (
                "cluster",
                lambda _, v: v["type"] == "teaser",
                # If 'brandId' differs, it is a 'You might also like' video. Filter these out
                "teaser",
                lambda _, v: v["type"] == "video"
                and v["brandId"] == document_id
                and v["sharingUrl"],
            ),
        ):
    ...
    def _real_extract(self, url):
        ...
        main_video = (
            traverse_obj(
                ...
                (
                    "cluster",
                    lambda _, v: v["type"] == "teaserContent",
                    "teaser",
                    lambda _, v: v["type"] == "video"
                    and v["basename"]
                    and v["sharingUrl"],
        ...
        return self.playlist_result(
            self._entries(data, document_id),
            channel_id,
            re.split(
                r"\s+[-|]\s+ZDF(?:mediathek)?$",
            ...
            join_nonempty(
                ...
                "text",
                delim="\n\n",
                from_dict=traverse_obj(
                    data, ("shortText", {dict}), default={}
                ),
            )
            or None,
        )
