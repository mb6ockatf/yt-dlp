...
class RCSBaseIE(InfoExtractor):
    ...
    def _create_http_formats(self, m3u8_formats, video_id):
        for f in m3u8_formats:
            ...
            format_id = try_call(
                lambda: http_f["format_id"].replace("hls-", "https-")
            )
            ...
            http_f.update(
                {
                    ...
                    "filesize_approx": int_or_none(
                        urlh.headers.get("Content-Length", None)
                    ),
                }
            )
            yield http_f
    ...
    def _real_extract(self, url):
        ...
        if re.match(self._UUID_RE, video_id) or re.match(
            self._RCS_ID_RE, video_id
        ):
            ...
            ) or get_element_html_by_class("divVideoPlayer", webpage)
            ...
            if data_config:
                ...
                display_id, video_id = (
                    video_id,
                    data_config.get("uuid") or video_id,
                )
                url = f"https://video.{cdn}/video-json/{video_id}"
            else:
        ...
        return {
            ...
            "formats": list(
                self._create_formats(self._get_video_src(video_data), video_id)
            ),
        }
...
class RCSEmbedsIE(RCSBaseIE):
    ...
    def _extract_embed_urls(cls, url, webpage):
        return map(
            cls._sanitize_url, super()._extract_embed_urls(url, webpage)
        )
