...
class SoundcloudEmbedIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:w|player|p)\.soundcloud\.com/player/?.*?\burl=(?P<id>.+)"
    )
    _EMBED_REGEX = [
    ...
    def _real_extract(self, url):
        ...
        if secret_token:
            api_url = update_url_query(
                api_url, {"secret_token": secret_token[0]}
            )
...
class SoundcloudBaseIE(InfoExtractor):
    ...
    _API_AUTH_URL_PW = (
        "https://api-auth.soundcloud.com/web-auth/sign-in/password%s"
    )
    _API_VERIFY_AUTH_TOKEN = (
        "https://api-auth.soundcloud.com/connect/session%s"
    )
    _HEADERS = {}
        ...
        return re.compile(
            r"|".join(
                set(
                    ...
                    for pattern in self._configuration_arg(
                        "formats", ["default"], ie_key=SoundcloudIE
                    )
                )
            )
        ).fullmatch
    ...
    def _call_api(self, *args, **kwargs):
        ...
        for _ in range(2):
            ...
            try:
                ...
                if isinstance(e.cause, HTTPError) and e.cause.status in (
                    401,
                    403,
                ):
    ...
    def _initialize_pre_login(self):
        self._CLIENT_ID = (
            self.cache.load("soundcloud", "client_id")
            or "a3e059563d7fd3372b49b37f00a00bcf"
        )

    def _verify_oauth_token(self, token):
        if self._request_webpage(
            self._API_VERIFY_AUTH_TOKEN
            % (self._API_AUTH_QUERY_TEMPLATE % self._CLIENT_ID),
            None,
            ...
            self.report_warning(
                "Provided authorization token is invalid. Continuing as guest"
            )
    ...
    def _real_initialize(self):
        if self._HEADERS:
            return
        if token := try_call(
            lambda: self._get_cookies(self._BASE_URL)["oauth_token"].value
        ):
    ...
    def _extract_info_dict(
        self, info, full_title=None, secret_token=None, extract_flat=False
    ):
        ...
        if (
            ...
            try:
                ...
                if isinstance(e.cause, HTTPError) and e.cause.status == 401:
                    ...
                    self.write_debug(
                        "Original download format is not available for this client"
                    )
                else:
                    self.report_warning(e.msg)
                download_data = None

            if redirect_url := traverse_obj(
                download_data, ("redirectUri", {url_or_none})
            ):
                    ...
                    formats.append(
                        {
                            ...
                            "filesize": int_or_none(
                                urlh.headers.get("Content-Length")
                            ),
                            "url": format_url,
                            "quality": 10,
        ...
        # New API
        for t in traverse_obj(
            info,
            (
                "media",
                "transcodings",
                lambda _, v: url_or_none(v["url"]) and v["preset"],
            ),
        ):
            if extract_flat:
                break
            ...
            if preset_base == "abr":
                self.write_debug(
                    f'Skipping broken "{short_identifier}" format'
                )
                continue
            if not self._is_requested(short_identifier):
                self.write_debug(
                    f'"{short_identifier}" is not a requested format, skipping'
                )
                continue
            ...
            codec = self._search_regex(
                r'codecs="([^"]+)"', mime_type, "codec", default=None
            )
            ext = {
                "mp4a": "m4a",
                "opus": "opus",
            }.get(
                codec[:4] if codec else None
            ) or mimetype2ext(mime_type, default=None)
            ...
            formats.append(
                {
                    "format_id": join_nonempty(
                        protocol, preset, is_preview and "preview", delim="_"
                    ),
                    "url": stream_url,
                    "ext": ext,
                    "acodec": codec,
                    ...
                    "quality": (
                        5 if is_premium else 0 if (abr and abr >= 160) else -1
                    ),
        ...
        if url_or_none(thumbnail):
            if mobj := re.search(self._IMAGE_REPL_RE, thumbnail):
                for image_id, size in self._ARTWORK_MAP.items():
                    ...
                    ext = (
                        mobj.group("ext") if image_id == "original" else "jpg"
                    )
                    i = {
                        "id": image_id,
                        "url": re.sub(
        ...
        return {
            ...
            or extract_count("likes"),
            "comment_count": extract_count("comment"),
            "repost_count": extract_count("reposts"),
            "genres": traverse_obj(
                info, ("genre", {str}, filter, all, filter)
            ),
            "tags": traverse_obj(
                info, ("tag_list", {self._TAGS_RE.findall}, ..., ..., filter)
            ),
            "artists": traverse_obj(
                info,
                ("publisher_metadata", "artist", {str}, filter, all, filter),
            ),
...
class SoundcloudIE(SoundcloudBaseIE):
    ...
    _TESTS = [
        ...
        # downloadable song
        {
            ...
            "expected_warnings": [
                "Original download format is only available for registered users"
            ],
        },
        ...
        # .png "original" artwork, 160kbps m4a HLS format
        {
            ...
            "info_dict": {
                ...
                "thumbnails": [
                    {
                        "id": "mini",
                        "url": "https://i1.sndcdn.com/artworks-a1wKGMYNreDLTMrT-fGjRiw-mini.jpg",
                    },
                    {
                        "id": "tiny",
                        "url": "https://i1.sndcdn.com/artworks-a1wKGMYNreDLTMrT-fGjRiw-tiny.jpg",
                    },
                    {
                        "id": "small",
                        "url": "https://i1.sndcdn.com/artworks-a1wKGMYNreDLTMrT-fGjRiw-small.jpg",
                    },
                    {
                        "id": "badge",
                        "url": "https://i1.sndcdn.com/artworks-a1wKGMYNreDLTMrT-fGjRiw-badge.jpg",
                    },
                    {
                        "id": "t67x67",
                        "url": "https://i1.sndcdn.com/artworks-a1wKGMYNreDLTMrT-fGjRiw-t67x67.jpg",
                    },
                    {
                        "id": "large",
                        "url": "https://i1.sndcdn.com/artworks-a1wKGMYNreDLTMrT-fGjRiw-large.jpg",
                    },
                    {
                        "id": "t300x300",
                        "url": "https://i1.sndcdn.com/artworks-a1wKGMYNreDLTMrT-fGjRiw-t300x300.jpg",
                    },
                    {
                        "id": "crop",
                        "url": "https://i1.sndcdn.com/artworks-a1wKGMYNreDLTMrT-fGjRiw-crop.jpg",
                    },
                    {
                        "id": "t500x500",
                        "url": "https://i1.sndcdn.com/artworks-a1wKGMYNreDLTMrT-fGjRiw-t500x500.jpg",
                    },
                    {
                        "id": "original",
                        "url": "https://i1.sndcdn.com/artworks-a1wKGMYNreDLTMrT-fGjRiw-original.png",
                    },
                ],
            },
    ...
    def _real_extract(self, url):
        ...
        if track_id:
            ...
            full_title = resolve_title = "{}/{}".format(
                *mobj.group("uploader", "title")
            )
...
class SoundcloudSetIE(SoundcloudPlaylistBaseIE):
    ...
    def _real_extract(self, url):
        ...
        if "errors" in info:
            ...
            raise ExtractorError(
                "unable to download video webpage: {}".format(",".join(msgs))
            )
...
class SoundcloudTrackStationIE(SoundcloudPagedPlaylistBaseIE):
    ...
    def _real_extract(self, url):
        ...
        track = self._call_api(
            self._resolv_url(url), track_name, headers=self._HEADERS
        )
...
class SoundcloudRelatedIE(SoundcloudPagedPlaylistBaseIE):
    ...
    def _real_extract(self, url):
        ...
        if track.get("errors"):
            raise ExtractorError(
                f"{self.IE_NAME} said: %s"
                % ",".join(
                    str(err["error_message"]) for err in track["errors"]
                ),
                expected=True,
            )
        ...
        return self._extract_playlist(
            ...
            str(track["id"]),
            "{} ({})".format(
                track.get("title") or slug, relation.capitalize()
            ),
        )
