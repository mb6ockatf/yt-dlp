    ...
    def _real_extract(self, url):
        ...
        if "&vod" not in url:
            ...
            if bc:
                ...
                return self.url_result(
                    smuggle_url(bc_url, {"source_url": url})
                )
        ...
        if jwsetup:
            ...
            return merge_dicts(
                ...
                {
                    "title": title,
                    "thumbnail": get_element_by_attribute(
                        "itemprop", "thumbnailUrl", webpage
                    ),
                    "description": description,
                    "timestamp": parse_iso8601(
                        get_element_by_attribute(
                            "itemprop", "uploadDate", webpage
                        )
                    ),
                    "location": get_element_by_attribute(
                        "itemprop", "contentLocation", webpage
                    ),
        ...
        if results:
            ...
            m = re.search(
                r'data-(?P<type>clip|prog)id=["\'](?P<id>\d+)', webpage
            )
            if m:
                video_id = m.group("id")
        ...
        if data["@status"] != "Success":
            raise ExtractorError(
                "{} said: {}".format(
                    self.IE_NAME, get_text_attr(data, "error")
                ),
                expected=True,
            )
...
class CSpanCongressIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        video_id = join_nonempty(
            query.get("chamber", ["senate"])[0], video_date, delim="_"
        )
        webpage = self._download_webpage(url, video_id)
        if not video_date:
            jwp_date = re.search(
                r"jwsetup.clipprogdate = \'(?P<date>\d{4}-\d{2}-\d{2})\';",
                webpage,
            )
            if jwp_date:
