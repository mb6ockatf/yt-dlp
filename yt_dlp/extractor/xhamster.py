...
class XHamsterIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        def get_height(s):
            return int_or_none(
                self._search_regex(r"^(\d+)[pP]", s, "height", default=None)
            )
        ...
        if initials:
            ...
            for format_id, formats_dict in sources.items():
                if not isinstance(formats_dict, dict):
                    continue
                download_sources = (
                    try_get(sources, lambda x: x["download"], dict) or {}
                )
                    ...
                    for format_id, formats_list in standard_sources.items():
                        ...
                        for standard_format in formats_list:
                            ...
                            for standard_format_key in ("url", "fallback"):
                                ...
                                if (
                                    not standard_url
                                    or standard_url in format_urls
                                ):
                                    continue
                                ...
                                if ext == "m3u8":
                                    ...
                                    continue
                                quality = (
                                    str_or_none(standard_format.get("quality"))
                                    or str_or_none(
            ...
            return {
                ...
                "uploader_id": (
                    uploader_url.split("/")[-1] if uploader_url else None
                ),
                "thumbnail": video.get("thumbURL"),
                "duration": int_or_none(video.get("duration")),
                "view_count": int_or_none(video.get("views")),
                "like_count": int_or_none(
                    try_get(video, lambda x: x["rating"]["likes"], int)
                ),
                "dislike_count": int_or_none(
                    try_get(video, lambda x: x["rating"]["dislikes"], int)
                ),
        ...
        (like_count, dislike_count) = (
            (mobj.group("likecount"), mobj.group("dislikecount"))
            if mobj
            else (None, None)
        )

        mobj = re.search(
            r"</label>Comments \((?P<commentcount>\d+)\)</div>", webpage
        )
        comment_count = mobj.group("commentcount") if mobj else 0

        categories_html = self._search_regex(
            r"(?s)<table.+?(<span>Categories:.+?)</table>",
            webpage,
            "categories",
            default=None,
        )
...
class XHamsterEmbedIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if not video_url:
            ...
            video_url = dict_get(
                player_vars,
                ("downloadLink", "homepageLink", "commentsLink", "shareUrl"),
            )
...
class XHamsterUserIE(InfoExtractor):
    ...
    def _entries(self, user_id, is_user):
        prefix, suffix = (
            ("users", "videos") if is_user else ("creators", "exclusive")
        )
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._entries(user_id, bool(user)), user_id
        )
