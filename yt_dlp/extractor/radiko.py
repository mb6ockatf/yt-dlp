...
class RadikoBaseIE(InfoExtractor):
    ...
    _HOSTS_FOR_LIVE = ("https://c-radiko.smartstream.ne.jp",)

    def _negotiate_token(self):
        _, auth1_handle = self._download_webpage_handle(
            "https://radiko.jp/v2/api/auth1",
            None,
            "Downloading authentication page",
            headers={
    ...
    def _find_program(self, video_id, station, cursor):
        station_program = self._download_xml(
            f"https://radiko.jp/v3/program/station/weekly/{station}.xml",
            video_id,
        ...
        for element in m3u8_playlist_data.findall(
            ...
            for sf in subformats:
                if (is_onair ^ pcu.startswith(self._HOSTS_FOR_LIVE)) or (
                    not is_onair
                    and pcu.startswith(
                        self._HOSTS_FOR_TIME_FREE_FFMPEG_UNSUPPORTED
                    )
                ):
                    sf["preference"] = -100
...
class RadikoIE(RadikoBaseIE):
    ...
    def _real_extract(self, url):
        station, timestring = self._match_valid_url(url).group(
            "station", "timestring"
        )
        ...
        return {
            ...
            "description": clean_html(
                try_call(lambda: prog.find("info").text)
            ),
            "uploader": try_call(lambda: station_program.find(".//name").text),
            "uploader_id": station,
            "timestamp": vid_int,
            "duration": try_call(
                lambda: unified_timestamp(radio_end, False)
                - unified_timestamp(radio_begin, False)
            ),
            "is_live": True,
