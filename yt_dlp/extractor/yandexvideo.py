...
class YandexVideoIE(InfoExtractor):
  ...
  player(content_id: "%s") {
    ...
    views_count
        ...
        timestamp = (
            int_or_none(content.get("release_date"))
...
class ZenYandexIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        redirect = self._search_json(
            r"var it\s*=", webpage, "redirect", id, default={}
        ).get("retpath")
        if redirect:
        ...
        data_json = self._search_json(
            ...
            "metadata",
            video_id,
            contains_pattern=r'{["\']_*serverState_*video.+}',
        )
        serverstate = self._search_regex(
            r"(_+serverState_+video-site_[^_]+_+)", webpage, "server state"
        )
        uploader = self._search_regex(
            r'(<a\s*class=["\']card-channel-link[^"\']+["\'][^>]+>)',
            webpage,
            "uploader",
            default="<a>",
        )
        ...
        item_id = traverse_obj(
            data_json, (serverstate, "videoViewer", "openedItemId", {str})
        )
        video_json = (
            traverse_obj(
                data_json,
                (serverstate, "videoViewer", "items", item_id, {dict}),
            )
            or {}
        )

        formats, subtitles = [], {}
        for s_url in traverse_obj(
            video_json, ("video", "streams", ..., {url_or_none})
        ):
            ext = determine_ext(s_url)
            if ext == "mpd":
                fmts, subs = self._extract_mpd_formats_and_subtitles(
                    s_url, video_id, mpd_id="dash"
                )
            elif ext == "m3u8":
        ...
        return {
            ...
            or data_json.get("authorName")
            or try_get(data_json, lambda x: x["publisher"]["name"]),
            "description": video_json.get("description")
            or self._og_search_description(webpage),
            "thumbnail": self._og_search_thumbnail(webpage)
            or try_get(data_json, lambda x: x["og"]["imageUrl"]),
        }
...
class ZenYandexChannelIE(InfoExtractor):
    ...
    def _entries(self, item_id, server_state_json, server_settings_json):
        items = traverse_obj(
            server_state_json, ("feed", "items", ...)
        ) or traverse_obj(server_settings_json, ("exportData", "items", ...))

        more = traverse_obj(
            server_state_json, ("links", "more")
        ...
        for page in itertools.count(1):
            for item in items or []:
                if item.get("type") != "gif":
                    continue
                video_id = (
                    traverse_obj(item, "publication_id", "publicationId") or ""
                )
                yield self.url_result(
                    item["link"], ZenYandexIE, video_id.split(":")[-1]
                )

            current_page_id = next_page_id
            next_page_id = traverse_obj(parse_qs(more), ("next_page_id", -1))
            if not all(
                (more, items, next_page_id, next_page_id != current_page_id)
            ):
                break

            data = self._download_json(
                more, item_id, note=f"Downloading Page {page}"
            )
            items, more = data.get("items"), traverse_obj(
                data, ("more", "link")
            )
    ...
    def _real_extract(self, url):
        ...
        redirect = self._search_json(
            r"var it\s*=", webpage, "redirect", item_id, default={}
        ).get("retpath")
        if redirect:
            item_id = self._match_id(redirect)
            webpage = self._download_webpage(
                redirect, item_id, note="Redirecting"
            )
        data = self._search_json(
            ...
            "channel data",
            item_id,
            contains_pattern=r"{\"__serverState__.+}",
        )
        server_state_json = traverse_obj(
            data, lambda k, _: k.startswith("__serverState__"), get_all=False
        )
        server_settings_json = traverse_obj(
            data,
            lambda k, _: k.startswith("__serverSettings__"),
            get_all=False,
        )
        ...
        return self.playlist_result(
            ...
            item_id,
            traverse_obj(server_state_json, ("channel", "source", "title")),
            traverse_obj(
                server_state_json, ("channel", "source", "description")
            ),
        )
