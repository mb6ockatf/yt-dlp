...
class SpotifyBaseIE(InfoExtractor):
    ...
    def _real_initialize(self):
        self._ACCESS_TOKEN = self._download_json(
            "https://open.spotify.com/get_access_token", None
        )["accessToken"]
    ...
    def _extract_episode(self, episode, series):
        ...
        if audio_preview_url:
            ...
            if audio_preview_format:
                ...
                mobj = re.match(
                    r"([0-9A-Z]{3})_(?:[A-Z]+_)?(\d+)", audio_preview_format
                )
                if mobj:
                    f.update(
                        {
        ...
        thumbnails = []
        for source in (
            try_get(episode, lambda x: x["coverArt"]["sources"]) or []
        ):
        ...
        return {
            ...
            "duration": float_or_none(
                try_get(episode, lambda x: x["duration"]["totalMilliseconds"]),
                1000,
            ),
            "release_date": unified_strdate(
                try_get(episode, lambda x: x["releaseDate"]["isoString"])
            ),
            "series": series,
        }
...
class SpotifyShowIE(SpotifyBaseIE):
    ...
    def _fetch_page(self, show_id, page=0):
        return self._call_api(
            ...
            note=f"Downloading page {page + 1} JSON metadata",
        )["podcast"]
    ...
    def _real_extract(self, url):
        ...
        def _entries(page):
            ...
            yield from map(
                functools.partial(
                    self._extract_episode, series=podcast.get("name")
                ),
