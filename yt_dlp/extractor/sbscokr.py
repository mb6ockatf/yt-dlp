...
class SBSCoKrIE(InfoExtractor):
    ...
    def _call_api(self, video_id, rscuse=""):
        return self._download_json(
            f"https://api.play.sbs.co.kr/1.0/sbs_vodall/{video_id}",
            video_id,
    ...
    def _real_extract(self, url):
        ...
        source = (
            traverse_obj(details, ("vod", "source", "mediasource", {dict}))
            or {}
        )
        ...
        for stream in traverse_obj(
            ...
            (
                ...
                "source",
                "mediasourcelist",
                lambda _, v: v["mediaurl"] or v["mediarscuse"],
            ),
            default=[source],
        ):
            if not stream.get("mediaurl"):
                ...
                if new_source.get("mediarscuse") == source.get(
                    "mediarscuse"
                ) or not new_source.get("mediaurl"):
                    continue
                stream = new_source
            formats.append(
                {
        ...
        return {
            ...
            **traverse_obj(
            ...
            "formats": formats,
            "subtitles": (
                {"ko": [{"url": caption_url}]} if caption_url else None
            ),
        }
...
class SBSCoKrAllvodProgramIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            [
                self.url_result(
                    f"https://allvod.sbs.co.kr/allvod/vodEndPage.do?mdaId={video_id}",
                    SBSCoKrIE,
                )
                for video_id in traverse_obj(details, ("list", ..., "mdaId"))
            ],
            program_id,
        )
