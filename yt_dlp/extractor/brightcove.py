...
class BrightcoveLegacyIE(InfoExtractor):
    ...
    def _build_brightcove_url(cls, object_str):
        ...
        video_player = (
            find_param("@videoPlayer")
            or find_param("videoId")
            or find_param("videoID")
            or find_param("@videoList")
        )
        if video_player is not None:
            ...
            if not (
                ...
                or video_player.startswith("ref:")
            ):
                return None
    ...
    def _extract_brightcove_urls(cls, webpage):
        ...
        if matches:
            return list(
                filter(None, [cls._build_brightcove_url(m) for m in matches])
            )
    ...
    def _extract_from_webpage(self, url, webpage):
        ...
        for bc_url in bc_urls:
            yield self.url_result(
                smuggle_url(bc_url, {"Referer": url}), BrightcoveLegacyIE
            )
    ...
    def _real_extract(self, url):
        ...
        url = re.sub(
            r"(?<=[?&])(videoI(d|D)|idVideo|bctid)", "%40videoPlayer", url
        )
        ...
        if video_player:
            ...
            referer = query.get("linkBaseURL", [None])[0] or smuggled_data.get(
                "Referer", url
            )
            ...
            if not publisher_id:
                ...
                if player_key and "," in player_key[0]:
                    ...
                    if player_id and player_id[0].isdigit():
                        ...
                        player_page = self._download_webpage(
                            "https://link.brightcove.com/services/player/bcpid"
                            + player_id[0],
                            video_id,
                            headers=headers,
                            fatal=False,
                ...
                if player_key:
                    ...
                    publisher_id = struct.unpack(
                        ">Q", base64.urlsafe_b64decode(enc_pub_id)
                    )[0]
            if publisher_id:
    ...
    def _parse_brightcove_metadata(self, json_data, video_id, headers={}):
        ...
        for source in sources:
            ...
            if ext == "m3u8" or container == "M2TS":
                ...
                stream_name, app_name = source.get("stream_name"), source.get(
                    "app_name"
                )
                if (
                    not src
                    and not streaming_src
                    and (not stream_name or not app_name)
                ):
                    continue
                ...
                def build_format_id(kind):
                    return join_nonempty(
                        kind, tbr and f"{int(tbr)}k", height and f"{height}p"
                    )
                    ...
                    f.update(
                        {
                            ...
                            "format_id": build_format_id(
                                "http" if src else "http-streaming"
                            ),
        ...
        headers.pop(
            "Authorization", None
        )  # or else http formats will give error 400
        for f in formats:
        ...
        for text_track in json_data.get("text_tracks", []):
            ...
            if not text_track_url:
                continue
            lang = (
                str_or_none(text_track.get("srclang"))
                or str_or_none(text_track.get("label"))
                or "en"
            ).lower()
...
class BrightcoveNewIE(BrightcoveNewBaseIE):
    ...
    def _extract_from_webpage(self, url, webpage):
        ...
        for bc_url in bc_urls:
            yield self.url_result(
                smuggle_url(bc_url, {"referrer": url}), BrightcoveNewIE
            )
    ...
    def _real_extract(self, url):
        ...
        account_id, player_id, embed, content_type, video_id = (
            self._match_valid_url(url).groups()
        )
        ...
        def extract_policy_key():
            ...
            config = (
                self._download_json(
                    base_url + "config.json", video_id, fatal=False
                )
                or {}
            )
            policy_key = try_get(
        ...
        for _ in range(2):
            ...
            try:
                ...
                if isinstance(e.cause, HTTPError) and e.cause.status in (
                    ...
                    message = (
                        json_data.get("message") or json_data["error_code"]
                    )
                    if json_data.get("error_subcode") == "CLIENT_GEO":
                        self.raise_geo_restricted(msg=message)
                    elif (
                        json_data.get("error_code") == "INVALID_POLICY_KEY"
                        and not policy_key_extracted
                    ):
                        policy_key = None
                        store_pk(None)
                        continue
            ...
            return self.playlist_result(
                (
                    ...
                    for vid in traverse_obj(
                        json_data, ("videos", lambda _, v: v["id"])
                    )
                ),
                json_data.get("id"),
                json_data.get("name"),
