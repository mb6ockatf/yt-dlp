...
class TumblrIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        webpage, urlh = self._download_webpage_handle(
            url, video_id, headers={"User-Agent": "WhatsApp/2.0"}
        )  # whatsapp ua bypasses problems
        ...
        if self._ACCESS_TOKEN:
            post_json = (
                traverse_obj(
                    ...
                    ("response", "timeline", "elements", 0, {dict}),
                )
                or {}
            )
        ...
        info_dict = {
            ...
            "uploader_url": (
                f"https://{uploader_id}.tumblr.com/" if uploader_id else None
            ),
            **traverse_obj(
                post_json,
                {
        ...
        for video_json in traverse_obj(
            content_json, lambda _, v: v["type"] in ("video", "audio")
        ):
            media_json = video_json.get("media") or {}
            if (
                api_only
                and not media_json.get("url")
                and not video_json.get("url")
            ):
                raise ExtractorError(
            ...
            if provider in ("tumblr", None):
                fallback_format = {
                    ...
                    "width": int_or_none(
                        media_json.get("width")
                        or self._og_search_property(
                            "video:width", webpage, default=None
                        )
                    ),
                    "height": int_or_none(
                        media_json.get("height")
                        or self._og_search_property(
        ...
        iframe_url = self._search_regex(
            rf"src=\'(https?://www\.tumblr\.com/video/{blog}/{video_id}/[^\']+)\'",
            webpage,
            "iframe url",
            default=None,
        )
        if iframe_url:
            ...
            options = self._parse_json(
                self._search_regex(
                    r'data-crt-options=(["\'])(?P<options>.+?)\1',
                    iframe,
                    "hd video url",
                    default="",
                    group="options",
            ...
            if options:
                ...
                if hd_url:
                    ...
                    formats = [
                        ...
                        for quality, (video_url, format_id) in enumerate(
                            sources
                        )
                    ]
            ...
            entries.insert(
                0,
                {
                    **info_dict,
                    ...
                    "thumbnail": (
                        traverse_obj(
                            video_json, ("poster", 0, "url", {url_or_none})
                        )
