...
class TestURLIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        extractor_id, num = self._match_valid_url(url).group(
            "extractor", "num"
        )
        if not extractor_id:
        ...
        matching_extractors = [
            e for e in gen_extractor_classes() if rex.search(e.IE_NAME)
        ]

        if len(matching_extractors) == 0:
            raise ExtractorError(
                f"No extractors matching {extractor_id!r} found", expected=True
            )
        elif len(matching_extractors) > 1:
            extractor = next(
                (  # Check for exact match
                    ie
                    for ie in matching_extractors
                    if ie.IE_NAME.lower() == extractor_id.lower()
                ),
                None,
            ) or next(
                (  # Check for exact match without plugin suffix
                    ie
                    for ie in matching_extractors
                    if ie.IE_NAME.split("+")[0].lower() == extractor_id.lower()
                ),
                None,
            )
            if not extractor:
                raise ExtractorError(
                    "Found multiple matching extractors: {}".format(
