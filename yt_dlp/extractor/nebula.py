...
_BASE_URL_RE = (
    r"https?://(?:www\.|beta\.)?(?:watchnebula\.com|nebula\.app|nebula\.tv)"
)
...
class NebulaBaseIE(InfoExtractor):
    ...
    def _perform_login(self, username, password):
        try:
            response = self._download_json(
                "https://nebula.tv/auth/login/",
                None,
                "Logging in to Nebula",
                "Login failed",
                data=json.dumps(
            ...
            if isinstance(e.cause, HTTPError) and e.cause.status == 400:
                raise ExtractorError(
                    "Login failed: Invalid username or password", expected=True
                )
            raise
    ...
    def _real_initialize(self):
        if not self._api_token:
            self._api_token = try_call(
                lambda: self._get_cookies("https://nebula.tv")[
    ...
    def _extract_video_metadata(self, episode):
        channel_url = traverse_obj(
            episode,
            (("channel_slug", "class_slug"), {urljoin("https://nebula.tv/")}),
            get_all=False,
        )
        return {
                ...
                episode,
                {
                    ...
                    "_old_archive_ids": (
                        "zype_id",
                        {
                            lambda x: (
                                [
                                    make_archive_id(NebulaIE, x),
                                    make_archive_id(NebulaSubscriptionsIE, x),
                                ]
                                if x
                                else None
                            )
    ...
    _TESTS = [
        {
            ...
            "info_dict": {
                ...
                "_old_archive_ids": [
                    "nebula 5c271b40b13fd613090034fd",
        ...
        },
        {
            ...
            "info_dict": {
                ...
                "thumbnail": r"re:https://\w+\.cloudfront\.net/[\w-]+",
                "_old_archive_ids": [
                    "nebula 5e7e78171aaf320001fbd6be",
        ...
        },
        {
            ...
            "info_dict": {
                ...
                "_old_archive_ids": [
                    "nebula 5e779ebdd157bc0001d1c75a",
        ...
        },
        {
            ...
            "info_dict": {
                ...
                "_old_archive_ids": [
                    "nebula 63f64c74366fcd00017c1513",
...
class NebulaClassIE(NebulaBaseIE):
    ...
    def _real_extract(self, url):
        ...
        metadata = self._call_api(
            ...
            slug,
            note="Fetching class/podcast metadata",
        )
...
class NebulaSubscriptionsIE(NebulaBaseIE):
    ...
    _VALID_URL = (
        rf"{_BASE_URL_RE}/(?P<id>myshows|library/latest-videos)/?(?:$|[?#])"
    )
    _TESTS = [
        {
...
class NebulaChannelIE(NebulaBaseIE):
    ...
    _VALID_URL = (
        rf"{_BASE_URL_RE}/(?!myshows|library|videos)(?P<id>[\w-]+)/?(?:$|[?#])"
    )
    _TESTS = [
        {
            ...
            for episode in episodes["results"]:
                ...
                yield self.url_result(
                    smuggle_url(
                        ...
                        or f'https://nebula.tv/videos/{metadata["display_id"]}',
                        {"id": episode["id"]},
                    ),
                    NebulaIE,
                    url_transparent=True,
    ...
    def _generate_class_entries(self, channel):
        for lesson in channel["lessons"]:
            ...
            yield self.url_result(
                smuggle_url(
                    lesson.get("share_url")
                    or f'https://nebula.tv/{metadata["class_slug"]}/{metadata["slug"]}',
                    {"id": lesson["id"]},
                ),
                NebulaClassIE,
                url_transparent=True,
    ...
    def _generate_podcast_entries(self, collection_id, collection_slug):
        ...
        for page_num in itertools.count(1):
            ...
            for episode in traverse_obj(
                episodes, ("results", lambda _, v: url_or_none(v["share_url"]))
            ):
