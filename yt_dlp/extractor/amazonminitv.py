    ...
    def _call_api(self, asin, data=None, note=None):
        ...
        if resp.get("errors"):
            raise ExtractorError(
                f'MiniTV said: {resp["errors"][0]["message"]}'
            )
        elif not data:
            return resp
  ...
  content(
    ...
    def _real_extract(self, url):
        ...
        credits_time = try_get(
            title_info, lambda x: x["timecode"]["endCreditsTime"] / 1000
        )
        ...
        return {
            ...
            "description": traverse_obj(
                title_info, ("description", "synopsis")
            ),
            "release_timestamp": int_or_none(
                try_get(title_info, lambda x: x["publicReleaseDateUTC"] / 1000)
            ),
            "duration": traverse_obj(
                title_info, ("description", "contentLengthInSeconds")
            ),
            "chapters": (
                [
                    {
    ...
    def _entries(self, asin):
        ...
        for episode in season_info["episodes"]:
            yield self.url_result(
                f'amazonminitv:{episode["contentId"]}',
                AmazonMiniTVIE,
                episode["contentId"],
            )
...
"""
    ...
    def _entries(self, asin):
        ...
        for season in season_info["seasons"]:
            yield self.url_result(
                f'amazonminitv:season:{season["seasonId"]}',
