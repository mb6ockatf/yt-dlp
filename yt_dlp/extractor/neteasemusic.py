...
class NetEaseMusicBaseIE(InfoExtractor):
    ...
    def _download_eapi_json(
        self, path, video_id, query_body, headers={}, **kwargs
    ):
        cookies = {
            "osver": "undefined",
            "deviceId": "undefined",
            ...
            **traverse_obj(
                self._get_cookies(self._API_BASE),
                {
        ...
        return self._download_json(
            urljoin("https://interface3.music.163.com/", f"/eapi{path}"),
            video_id,
    ...
    def _extract_formats(self, info):
        ...
        for level in self._LEVELS:
            song = traverse_obj(
                self._call_player_api(song_id, level),
                ("data", lambda _, v: url_or_none(v["url"]), any),
            )
            if not song:
    ...
    def _query_api(self, endpoint, video_id, note):
        result = self._download_json(
            ...
            video_id,
            note,
            headers={"Referer": self._API_BASE},
        )
    ...
    def _get_entries(
        self, songs_data, entry_keys=None, id_key="id", name_key="name"
    ):
        for song in traverse_obj(
            songs_data,
            (
            ...
            yield self.url_result(
                f"http://music.163.com/#/song?id={song_id}",
                NetEaseMusicIE,
                song_id,
                traverse_obj(song, (name_key, {str})),
            )
...
class NetEaseMusicIE(NetEaseMusicBaseIE):
    ...
    def _process_lyrics(self, lyrics_info):
        ...
        merged = "\n".join(
            join_nonempty(
                f"{timestamp}{text}",
                translation_ts_dict.get(timestamp, ""),
                delim=" / ",
            )
    ...
    def _real_extract(self, url):
        ...
        metainfo = {
            "title": self._og_search_property(
                "title", webpage, "title", fatal=False
            ),
            "description": self._html_search_regex(
                (
                    rf'<div[^>]+\bid="album-desc-{suffix}"[^>]*>(.*?)</div>'
                    for suffix in ("more", "dot")
                ),
                webpage,
                "description",
                flags=re.S,
            ...
            "upload_date": unified_strdate(
                self._html_search_meta(
                    "music:release_date", webpage, "date", fatal=False
                )
            ),
        }
...
class NetEaseMusicSingerIE(NetEaseMusicBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._get_entries(info, "hotSongs"), singer_id, name
        )
...
class NetEaseMusicListIE(NetEaseMusicBaseIE):
    ...
    def _real_extract(self, url):
        ...
        if traverse_obj(info, ("playlist", "specialType")) == 10:
            metainfo["title"] = (
                f'{metainfo.get("title")} {strftime_or_none(metainfo.get("timestamp"), "%Y-%m-%d")}'
            )

        return self.playlist_result(
            self._get_entries(info, ("playlist", "tracks")),
            list_id,
            **metainfo,
        )
...
class NetEaseMusicMvIE(NetEaseMusicBaseIE):
    ...
    def _real_extract(self, url):
        ...
        info = self._query_api(
            f"mv/detail?id={mv_id}&type=mp4", mv_id, "Downloading mv info"
        )["data"]
        ...
        return {
            "id": mv_id,
            "formats": formats,
            "creators": traverse_obj(info, ("artists", ..., "name"))
        ...
        if not self._yes_playlist(
            ...
            return {
                ...
                "duration": traverse_obj(
                    info, ("mainSong", "duration", {int_or_none(scale=1000)})
                ),
                **metainfo,
            }
    ...
    def _real_extract(self, url):
        ...
        for offset in itertools.count(start=0, step=self._PAGE_SIZE):
            info = self._query_api(
                ...
                dj_id,
                note=f"Downloading dj programs - {offset}",
            )
