...
class RokfinIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?rokfin\.com/(?P<id>(?P<type>post|stream)/\d+)"
    )
    ...
    def _real_extract(self, url):
        ...
        metadata = self._download_json_using_access_token(
            f"{_API_BASE_URL}{video_id}", video_id
        )
        ...
        live_status = (
            ...
            if metadata.get("stoppedAt")
                ...
                "is_upcoming"
                if scheduled
                else "is_live" if video_type == "stream" else "not_live"
            )
        )
        ...
        uploader = traverse_obj(
            metadata, ("createdBy", "username"), ("creator", "username")
        )
        timestamp = (
            scheduled
        ...
        return {
            ...
            "subtitles": subtitles,
            "title": str_or_none(
                traverse_obj(metadata, "title", ("content", "contentTitle"))
            ),
            "duration": float_or_none(
                traverse_obj(metadata, ("content", "duration"))
            ),
            "thumbnail": url_or_none(
                traverse_obj(
                    metadata, "thumbnail", ("content", "thumbnailUrl1")
                )
            ),
            "description": str_or_none(
                traverse_obj(
                    metadata, "description", ("content", "contentDescription")
                )
            ),
            ...
            "channel": str_or_none(
                traverse_obj(
                    metadata, ("createdBy", "name"), ("creator", "name")
                )
            ),
            "channel_id": str_or_none(
                traverse_obj(metadata, ("createdBy", "id"), ("creator", "id"))
            ),
            "channel_url": (
                url_or_none(f"https://rokfin.com/{uploader}")
                if uploader
                else None
            ),
            "timestamp": timestamp,
            "release_timestamp": (
                timestamp if live_status != "not_live" else None
            ),
            "tags": traverse_obj(
                metadata, ("tags", ..., "title"), expected_type=str_or_none
            ),
            "live_status": live_status,
            "availability": self._availability(
                needs_premium=bool(
                    traverse_obj(metadata, "premiumPlan", "premium")
                ),
                is_private=False,
            ...
            "__post_extractor": (
                self.extract_comments(video_id)
                if video_type == "post"
                else None
            ),
        }
    ...
    def _perform_login(self, username, password):
        ...
        if not self._authentication_active():
            if re.search(
                r"(?i)(invalid\s+username\s+or\s+password)", resp_body or ""
            ):
            ...
            data=urlencode_postdata(
                {
                    "code": urllib.parse.parse_qs(
                        urllib.parse.urldefrag(urlh.url).fragment
                    ).get("code")[0],
                    "client_id": "web",
    ...
    def _get_auth_token(self):
        return try_get(
            self._access_mgmt_tokens,
            lambda x: " ".join([x["token_type"], x["access_token"]]),
        )

    def _download_json_using_access_token(
        self, url_or_request, video_id, headers={}, query={}
    ):
        ...
        return self._download_json(
            url_or_request, video_id, headers=headers, query=query
        )
    ...
    def _get_video_data(self, metadata):
        for content in metadata.get("content") or []:
            ...
            yield self.url_result(
                ...
                video_title=str_or_none(
                    traverse_obj(content, ("content", "contentTitle"))
                ),
            )
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._get_video_data(
                self._download_json(f"{_API_BASE_URL}stack/{list_id}", list_id)
            ),
            list_id,
        )
...
class RokfinChannelIE(RokfinPlaylistBaseIE):
    ...
    def _validate_extractor_args(self):
        ...
        if requested_tabs is not None and (
            len(requested_tabs) > 1 or requested_tabs[0] not in self._TABS
        ):
            raise ExtractorError(
                f'Invalid extractor-arg "tab". Must be one of {", ".join(self._TABS)}',
                expected=True,
            )
    ...
    def _entries(self, channel_id, channel_name, tab):
        ...
        for page_n in itertools.count(0):
            ...
            if is_last or (
                page_n > pages_total if pages_total else is_last is not False
            ):
                return
    ...
    def _real_extract(self, url):
        ...
        channel_info = self._download_json(
            f"{_API_BASE_URL}user/{channel_name}", channel_name
        )
...
class RokfinSearchIE(SearchInfoExtractor):
    ...
    def _real_initialize(self):
        self._db_url, self._db_access_key = self.cache.load(
            self.ie_key(), "auth", default=(None, None)
        )
        if not self._db_url:
    ...
    def _search_results(self, query):
        ...
        for page_number in itertools.count(1):
            ...
            for result in search_results.get("results") or []:
                video_id_key, video_type = self._TYPES.get(
                    traverse_obj(result, ("content_type", "raw")), (None, None)
                )
                video_id = traverse_obj(
                    result, video_id_key, expected_type=int_or_none
                )
                if video_id and video_type:
                    yield self.url_result(
                        url=f"https://rokfin.com/{video_type}/{video_id}"
                    )
    ...
    def _get_db_access_credentials(self, video_id=None):
        ...
        for js_file_path in re.findall(
            r'<script\b[^>]*\ssrc\s*=\s*"(/static/js/[^">]+)"',
            notfound_err_page,
        ):
            ...
            auth_data.update(
                re.findall(
                    rf'REACT_APP_({"|".join(auth_data.keys())})\s*:\s*"([^"]+)"',
                    js_content or "",
                )
            )
            ...
            self._db_url = url_or_none(
                f'{auth_data["ENDPOINT_BASE"]}/api/as/v1/engines/rokfin-search/search.json'
            )
            self._db_access_key = f'Bearer {auth_data["SEARCH_KEY"]}'
            self.cache.store(
                self.ie_key(), "auth", (self._db_url, self._db_access_key)
            )
            return
