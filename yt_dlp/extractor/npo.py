    ...
    def _real_extract(self, url):
        ...
        if urllib.parse.urlparse(url).netloc in ["www.ntr.nl", "ntr.nl"]:
            player = self._download_json(
                f"https://www.ntr.nl/ajax/player/embed/{video_id}",
                video_id,
            ...
            self._request_webpage(
                "https://www.npostart.nl/api/token",
                video_id,
                "Downloading token",
                headers={
                    "Referer": url,
            ...
            player = self._download_json(
                f"https://www.npostart.nl/player/{video_id}",
                video_id,
                "Downloading player JSON",
                ...
                headers={
                    "x-xsrf-token": try_call(
                        lambda: urllib.parse.unquote(
                            self._get_cookies("https://www.npostart.nl")[
                                "XSRF-TOKEN"
                            ].value
                        )
                    ),
        ...
        for profile in ("hls", "dash-widevine", "dash-playready", "smooth"):
            streams = self._download_json(
                ...
                video_id,
                f"Downloading {profile} profile JSON",
                fatal=False,
                query={
                    "profile": profile,
                    "quality": "npoplus",
            ...
            if (
                stream.get("protection") is not None
                or stream.get("keySystemOptions") is not None
            ):
                drm = True
                continue
            ...
            if stream_type == "application/dash+xml" or stream_ext == "mpd":
                ...
                stream_type == "application/vnd.apple.mpegurl"
                or stream_ext == "m3u8"
            ):
                formats.extend(
                    ...
                    return merge_dicts(
                        {
                            ...
                            "thumbnail": url_or_none(
                                video.get("still_image_url")
                                or video.get("orig_image_url")
                            ),
                            "duration": int_or_none(video.get("duration")),
                            "timestamp": unified_timestamp(
                                video.get("broadcastDate")
                            ),
...
class NPOLiveIE(InfoExtractor):
    IE_NAME = "npo.nl:live"
    _VALID_URL = (
        r"https?://(?:www\.)?npo(?:start)?\.nl/live(?:/(?P<id>[^/?#&]+))?"
    )
...
class NPORadioIE(InfoExtractor):
    ...
    def suitable(cls, url):
        return (
            False
            if NPORadioFragmentIE.suitable(url)
            else super().suitable(url)
        )
...
class HetKlokhuisIE(NPODataMidEmbedIE):
    IE_NAME = "hetklokhuis"
    _VALID_URL = (
        r"https?://(?:www\.)?hetklokhuis\.nl/[^/]+/\d+/(?P<id>[^/?#&]+)"
    )
...
class NPOPlaylistBaseIE(NPOIE):  # XXX: Do not subclass from concrete IE
    def _real_extract(self, url):
        ...
        entries = [
            ...
            for video_id in orderedSet(
                re.findall(self._PLAYLIST_ENTRY_RE, webpage)
            )
        ]

        playlist_title = self._html_search_regex(
            self._PLAYLIST_TITLE_RE, webpage, "playlist title", default=None
...
class WNLIE(NPOPlaylistBaseIE):
    IE_NAME = "wnl"
    _VALID_URL = (
        r"https?://(?:www\.)?omroepwnl\.nl/video/detail/(?P<id>[^/]+)__\d+"
    )
    _PLAYLIST_TITLE_RE = r'(?s)<h1[^>]+class="subject"[^>]*>(.+?)</h1>'
    _PLAYLIST_ENTRY_RE = (
        r'<a[^>]+href="([^"]+)"[^>]+class="js-mid"[^>]*>Deel \d+'
    )
