...
class BitChuteIE(InfoExtractor):
    ...
    def _check_format(self, video_url, video_id):
        ...
        for url in urls:
            try:
                ...
                self.to_screen(
                    f"{video_id}: URL is invalid, skipping: {e.cause}"
                )
                continue
            return {
                "url": url,
    ...
    def _raise_if_restricted(self, webpage):
        page_title = (
            clean_html(get_element_by_class("page-title", webpage)) or ""
        )
    ...
    @staticmethod
    def _make_url(html):
        path = extract_attributes(
            get_element_html_by_class("spa", html) or ""
        ).get("href")
    ...
    def _real_extract(self, url):
        ...
        webpage = self._download_webpage(
            f"https://old.bitchute.com/video/{video_id}",
            video_id,
            headers=self._HEADERS,
        )

        self._raise_if_restricted(webpage)
        publish_date = clean_html(
        ...
        for format_ in traverse_obj(entries, (0, "formats", ...)):
            if self.get_param("check_formats") is not False:
                format_.update(
                    self._check_format(format_.pop("url"), video_id) or {}
                )
    ...
    def _fetch_page(self, playlist_id, playlist_type, page_num):
        ...
        for video_html in get_elements_html_by_class(
            ...
            yield self.url_result(
                ...
                title=clean_html(
                    get_element_by_class(classes["title"], video_html)
                ),
                description=clean_html(
                    get_element_by_class(classes["description"], video_html)
                ),
                duration=parse_duration(
                    get_element_by_class("video-duration", video_html)
                ),
                view_count=parse_count(
                    clean_html(get_element_by_class("video-views", video_html))
                ),
            )

    def _real_extract(self, url):
        playlist_type, playlist_id = self._match_valid_url(url).group(
            "type", "id"
        )
        webpage = self._download_webpage(
            self._make_url(playlist_id, playlist_type), playlist_id
        )
        ...
        return self.playlist_result(
            OnDemandPagedList(page_func, self.PAGE_SIZE),
            playlist_id,
