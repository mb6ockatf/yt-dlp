...
class MixcloudBaseIE(InfoExtractor):
    def _call_api(
        self, object_type, object_fields, display_id, username, slug=None
    ):
...
class MixcloudIE(MixcloudBaseIE):
    ...
    def _real_extract(self, url):
        ...
        username, slug = urllib.parse.unquote(username), urllib.parse.unquote(
            slug
        )
        track_id = f"{username}_{slug}"

        cloudcast = self._call_api(
            "cloudcast",
            """audioLength
    comments(first: 100) {
      edges {
    ...
    id""",
        ...
        return {
            ...
            "thumbnail": try_get(
                cloudcast, lambda x: x["picture"]["url"], str
            ),
            ...
            "comments": comments,
            "tags": tags,
            "artist": ", ".join(cloudcast.get("featuringArtistList") or [])
            or None,
        }
      ...
      pageInfo {
        ...
        hasNextPage
            ...
            for edge in items.get("edges", []):
                ...
                video_id = (
                    f"{owner_username}_{item_slug}"
                    if item_slug and owner_username
                    else None
                )
                entries.append(
                    self.url_result(
