...
class FrontendMastersBaseIE(InfoExtractor):
    ...
    def _perform_login(self, username, password):
        ...
        # Successful login
        if any(
            p in response for p in ("wp-login.php?action=logout", ">Logout")
        ):
            return
...
class FrontendMastersPageBaseIE(FrontendMastersBaseIE):
    def _download_course(self, course_name, url):
        return self._download_json(
            f"{self._API_BASE}/courses/{course_name}",
            course_name,
    ...
    def _extract_lesson(chapters, lesson_id, lesson):
        ...
        if (
            ...
            and isinstance(element_index, int)
        ...
        for ext in ("webm", "mp4"):
            for quality in ("low", "mid", "high"):
                ...
                format_url = self._download_json(
                    source_url,
                    lesson_id,
                    f"Downloading {format_id} source JSON",
                    query={
                        "f": ext,
                        "r": resolution["height"],
                    },
...
class FrontendMastersCourseIE(FrontendMastersPageBaseIE):
    ...
    def suitable(cls, url):
        return (
            False
            if FrontendMastersLessonIE.suitable(url)
            else super(FrontendMastersBaseIE, cls).suitable(url)
        )
