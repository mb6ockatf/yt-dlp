...
class DailyWireBaseIE(InfoExtractor):
    ...
    def _get_json(self, url):
        ...
        json_data = self._search_nextjs_data(
            self._download_webpage(url, slug), slug
        )
    ...
    def _real_extract(self, url):
        ...
        for url in urls:
            if determine_ext(url) != "m3u8":
                ...
                continue
            format_, subs_ = self._extract_m3u8_formats_and_subtitles(
                url, slug
            )
...
class DailyWirePodcastIE(DailyWireBaseIE):
    ...
    def _real_extract(self, url):
        slug, episode_info = self._get_json(url)
        audio_id = traverse_obj(
            episode_info,
            "audioMuxPlaybackId",
            "VUsAipTrBVSgzw73SpC2DAJD401TYYwEp",
        )
