...
class STVPlayerIE(InfoExtractor):
    IE_NAME = "stv:player"
    _VALID_URL = (
        r"https?://player\.stv\.tv/(?P<type>episode|video)/(?P<id>[a-z0-9]{4})"
    )
    _TESTS = [
        {
            # shortform
    ...
    def _real_extract(self, url):
        ...
        props = (
            self._search_nextjs_data(webpage, video_id, default={}).get(
                "props"
            )
            or {}
        )
        player_api_cache = (
            try_get(props, lambda x: x["initialReduxState"]["playerApiCache"])
            or {}
        )
        ...
        for k, v in player_api_cache.items():
            ...
            episode_id = str_or_none(
                try_get(props, lambda x: x["pageProps"]["episodeId"])
            )
