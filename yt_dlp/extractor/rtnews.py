...
class RTNewsIE(InfoExtractor):
    ...
    def _entries(self, webpage):
        video_urls = set(
            re.findall(r"https://cdnv\.rt\.com/.*[a-f0-9]+\.mp4", webpage)
        )
    ...
    def _real_extract(self, url):
        ...
        if not ld_json:
            self.raise_no_formats(
                "No video/audio found at the provided url.", expected=True
            )
        ...
        formats = [
            {"url": src["file"]}
            for src in media_json.get("sources") or []
            if src.get("file")
        ]
...
class RTDocumentryPlaylistIE(InfoExtractor):
    ...
    def _entries(self, webpage, playlist_id):
        video_urls = set(
            re.findall(r'list-2__link\s*"\s*href="([^"]+)"', webpage)
        )
        for v_url in video_urls:
            if playlist_id not in v_url:
                continue
            yield self.url_result(
                f"https://rtd.rt.com{v_url}", ie=RTDocumentryIE.ie_key()
            )
...
class RuptlyIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?ruptly\.tv/[a-z]{2}/videos/(?P<id>\d+-\d+)"
    )
    ...
    def _real_extract(self, url):
        ...
        m3u8_url = self._search_regex(
            r'preview_url"\s?:\s?"(https?://storage\.ruptly\.tv/video_projects/.+\.m3u8)"',
            webpage,
            "m3u8 url",
            fatal=False,
        )
