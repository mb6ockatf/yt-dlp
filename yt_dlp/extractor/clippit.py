    ...
    def _real_extract(self, url):
        ...
        for format_id in FORMATS:
            url = self._html_search_regex(
                rf'data-{format_id}-file="(.+?)"', webpage, "url", fatal=False
            )
            if not url:
                continue
        ...
        uploader_url = (
            "https://www.clippituser.tv/p/" + uploader if uploader else None
        )

        timestamp = self._html_search_regex(
            r'datetime="(.+?)"', webpage, "date", fatal=False
        )
        thumbnail = self._html_search_regex(
            r'data-image="(.+?)"', webpage, "thumbnail", fatal=False
        )
