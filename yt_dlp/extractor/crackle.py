    ...
    def _real_extract(self, url):
        ...
        for num, country in enumerate(countries):
            if num == 1:  # start hard-coded list
                self.report_warning(
                    "%s. Trying with a list of known countries"
                    % (
                        f"Unable to obtain video formats from {geo_bypass_country} API"
                ...
                geo_info = (
                    self._download_json(
                        ...
                        errnote="Unable to fetch geo-location information from crackle",
                    )
                    or {}
                )
            ...
            try:
                media = self._download_json(
                    ...
                    video_id,
                    note=f"Downloading media JSON from {country} API",
        ...
        duration = int_or_none(
            media.get("DurationInSeconds")
        ) or parse_duration(media.get("Duration"))
