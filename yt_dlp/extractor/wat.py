...
class WatIE(InfoExtractor):
    _VALID_URL = (
        r"(?:wat:|https?://(?:www\.)?wat\.tv/video/.*-)(?P<id>[0-9a-z]+)"
    )
    IE_NAME = "wat.tv"
    _TESTS = [
        {
        ...
        },
        {
            ...
            "expected_warnings": [
                "Ce contenu n'est pas disponible pour l'instant."
            ],
            "skip": "This content is no longer available",
        },
        {
    ...
    def _real_extract(self, url):
            ...
            for f, f_url in manifest_urls.items():
                ...
                if f in ("dash", "mpd"):
                    fmts, subs = self._extract_mpd_formats_and_subtitles(
                        f_url.replace(
                            "://das-q1.tf1.fr/", "://das-q1-ssl.tf1.fr/"
                        ),
                        video_id,
                        mpd_id="dash",
                        fatal=False,
        ...
        if not formats:
            ...
            manifest_urls = self._download_json(
                "http://www.wat.tv/get/webhtml/" + video_id,
                video_id,
                fatal=False,
            )
            if manifest_urls:
