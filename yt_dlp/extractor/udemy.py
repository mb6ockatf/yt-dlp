...
class UdemyIE(InfoExtractor):
    ...
    def _download_webpage_handle(self, *args, **kwargs):
        ...
        ret = super()._download_webpage_handle(*args, **kwargs)
        if not ret:
            return ret
        webpage, _ = ret
        if any(
    ...
    def _real_extract(self, url):
        ...
        def extract_formats(source_list):
            ...
            for source in source_list:
                ...
                if (
                    ...
                    or determine_ext(video_url) == "m3u8"
                ):
                    formats.extend(
        ...
        def extract_subtitles(track_list):
            ...
            for track in track_list:
                ...
                sub_dict = (
                    automatic_captions
                    if track.get("autogenerated") is True
                    else subtitles
                )
        ...
        if view_html:
            ...
            for source in re.findall(r"<source[^>]+>", view_html):
                ...
                if (
                    attributes.get("type") == "application/x-mpegURL"
                    or determine_ext(src) == "m3u8"
                ):
                    ...
                    for f in m3u8_formats:
                        m = re.search(
                            r"/hls_(?P<height>\d{3,4})_(?P<tbr>\d{2,})/",
                            f["url"],
                        )
                        if m:
                            if not f.get("height"):
        ...
        for entry in response["results"]:
            ...
            if clazz == "lecture":
                ...
                if isinstance(asset, dict):
                    asset_type = asset.get("asset_type") or asset.get(
                        "assetType"
                    )
                    if asset_type != "Video":
                        continue
