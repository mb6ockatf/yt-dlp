...
class SharePointIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if (
            urllib.parse.urlparse(urlh.url).hostname
            == "login.microsoftonline.com"
        ):
            self.raise_login_required(
                "Session cookies are required for this URL and can be passed "
                "with the --cookies option. The --cookies-from-browser option will not work",
                method=None,
            )

        video_data = self._search_json(
            r"g_fileInfo\s*=", webpage, "player config", display_id
        )
        video_id = video_data["VroomItemId"]

        parsed_url = urllib.parse.urlparse(video_data[".transformUrl"])
        base_media_url = urllib.parse.urlunparse(
            parsed_url._replace(
                path=urllib.parse.urljoin(
                    f"{parsed_url.path}/", "../videomanifest"
                ),
        ...
        if video_url := traverse_obj(
            video_data, ("downloadUrl", {url_or_none})
        ):
            formats.append(
                {
                    "url": video_url,
                    "ext": determine_ext(
                        video_data.get("extension") or video_data.get("name")
                    ),
                    "quality": 1,
                    "format_id": "source",
                    "filesize": int_or_none(video_data.get("size")),
                    "vcodec": (
                        "none" if video_data.get("isAudio") is True else None
                    ),
                }
            )
