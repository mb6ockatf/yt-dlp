...
class PrankCastIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        json_info = self._search_nextjs_data(webpage, video_id)["props"][
            "pageProps"
        ]["ssr_data_showreel"]
        ...
        return {
            "id": video_id,
            "title": json_info.get("broadcast_title")
            ...
            "duration": try_call(
                lambda: parse_iso8601(json_info["end_date"]) - start_date
            ),
            "cast": list(
                filter(
                    None, [uploader, *traverse_obj(guests_json, (..., "name"))]
                )
            ),
...
class PrankCastPostIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        webpage = self._download_webpage(url, video_id)
        post = self._search_nextjs_data(webpage, video_id)["props"][
            "pageProps"
        ]["ssr_data_posts"]
        ...
        guests_json = (
            traverse_obj(content, ("guests_json", {json.loads}, {dict})) or {}
        )
        ...
        return {
            ...
            "timestamp": parse_iso8601(
                content.get("start_date") or content.get("crdate"), " "
            ),
            "uploader": uploader,
            ...
            "tags": try_call(
                lambda: list(filter("", post["post_tags"].split(",")))
            ),
            "subtitles": (
                {
                    "live_chat": [
