...
class VidioBaseIE(InfoExtractor):
    ...
    def _perform_login(self, username, password):
        ...
        if login_post_urlh.status == 401:
            if get_element_by_class(
                "onboarding-content-register-popup__title", login_post
            ):
            ...
            reason = get_element_by_class(
                "onboarding-form__general-error", login_post
            ...
            if "Akun terhubung ke" in reason:
                raise ExtractorError(
                    ...
                    "Use --cookies to provide account credentials instead",
                    expected=True,
                )
            elif reason:
                subreason = (
                    get_element_by_class(
                        "onboarding-modal__description-text", login_post
                    )
                    or ""
                )
                raise ExtractorError(
    ...
    def _initialize_pre_login(self):
        self._api_key = self._download_json(
            "https://www.vidio.com/auth", None, data=b""
        )["api_key"]
...
class VidioIE(VidioBaseIE):
    ...
    _TESTS = [
        ...
        },
        {
            ...
            "info_dict": {
                ...
                "tags": [
                    "monkeypox indonesia",
                    "cacar monyet menyebar",
                    "suspek cacar monyet di indonesia",
                    "fakta",
                    "hoax atau bukan?",
                    "jawa tengah",
                ],
    ...
    def _real_extract(self, url):
        ...
        data = self._call_api(
            "https://api.vidio.com/videos/" + video_id, display_id
        )
        video = data["videos"][0]
        ...
        if is_premium:
            sources = self._download_json(
                ...
                display_id,
                note="Downloading premier API JSON",
            )
            ...
            if sources.get(
                "source_dash"
            ):  # TODO: Find video example with source_dash
        ...
        get_first = (
            lambda x: try_get(data, lambda y: y[x + "s"][0], dict) or {}
        )
        ...
        return {
            ...
            "uploader_url": format_field(
                username, None, "https://www.vidio.com/@%s"
            ),
    ...
    def _real_extract(self, url):
        ...
        if (
            ...
            return self.playlist_result(
                ...
                playlist_title=idata.get("title"),
            )

        playlist_data = self._call_api(
            f"https://api.vidio.com/content_profiles/{playlist_id}/playlists",
            display_id,
        )

        return self.playlist_from_matches(
            playlist_data.get("data", []),
            playlist_id=playlist_id,
            ie=self.ie_key(),
            getter=lambda data: smuggle_url(
                url,
                {
        ...
        if stream_meta.get("is_premium"):
            ...
            if str_or_none(sources.get("source")):
                token_json = self._download_json(
                    ...
                    display_id,
                    note="Downloading HLS token JSON",
                    data=b"",
                )
                formats.extend(
                    self._extract_m3u8_formats(
                        sources["source"] + "?" + token_json.get("token", ""),
                        display_id,
                        "mp4",
                        "m3u8_native",
                    )
