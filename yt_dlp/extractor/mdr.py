...
class MDRIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        doc = self._download_xml(urllib.parse.urljoin(url, data_url), video_id)

        title = xpath_text(
            doc, ["./title", "./broadcast/broadcastName"], "title", fatal=True
        )
        ...
        for asset in doc.findall("./assets/asset"):
            for source in (
                ...
                if ext == "m3u8":
                    ...
                    media_type = xpath_text(
                        asset, "./mediaType", "media type", default="MP4"
                    )
                    vbr = int_or_none(
                        xpath_text(asset, "./bitrateVideo", "vbr"), 1000
                    )
                    abr = int_or_none(
                        xpath_text(asset, "./bitrateAudio", "abr"), 1000
                    )
                    filesize = int_or_none(
                        xpath_text(asset, "./fileSize", "file size")
                    )
                    ...
                    if vbr:
                        f.update(
                            {
                                "width": int_or_none(
                                    xpath_text(asset, "./frameWidth", "width")
                                ),
                                "height": int_or_none(
                                    xpath_text(
                                        asset, "./frameHeight", "height"
                                    )
                                ),
                            }
                        )
        ...
        description = xpath_text(
            doc, "./broadcast/broadcastDescription", "description"
        )
