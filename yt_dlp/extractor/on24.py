...
class On24IE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        event_id = (
            str(
                try_get(
                    event_data, lambda x: x["presentationLogInfo"]["eventid"]
                )
            )
            or event_id
        )
        ...
        for media in event_data.get("mediaUrlInfo", []):
            media_url = urljoin(
                "https://event.on24.com/media/news/corporatevideo/events/",
                str(media.get("url")),
            )
            if not media_url:
                continue
        ...
        return {
            ...
            "timestamp": int_or_none(
                try_get(event_data, lambda x: x["session"]["startdate"]), 1000
            ),
