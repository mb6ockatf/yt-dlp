...
class EpiconIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        cid = self._search_regex(
            r"class=\"mylist-icon\ iconclick\"\ id=\"(\d+)", webpage, "cid"
        )
        headers = {
        ...
        data_json = self._parse_json(
            self._download_json(
                "https://www.epicon.in/ajaxplayer/",
                video_id,
                headers=headers,
                data=data,
            ),
...
class EpiconSeriesIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        episodes = re.findall(
            rf"ct-tray-url=\"(tv-shows/{playlist_id}/[^\"]+)", webpage
        )
        entries = [
            self.url_result(f"https://www.epicon.in/{episode}", EpiconIE)
