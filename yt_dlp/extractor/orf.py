...
class ORFRadioIE(InfoExtractor):
    ...
    def _entries(self, data, station):
        ...
        for info in data["streams"]:
            ...
            yield {
                ...
                "duration": try_call(
                    lambda: (info["end"] - info["start"]) / 1000
                ),
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._entries(data, station or station2),
            show_id,
            data.get("title"),
...
class ORFPodcastIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        station, show, show_id = self._match_valid_url(url).group(
            "station", "show", "id"
        )
        data = self._download_json(
...
class ORFIPTVIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        upload_date = unified_strdate(
            self._html_search_meta("dc.date", webpage, "upload date")
        )
...
class ORFONIE(InfoExtractor):
    IE_NAME = "orf:on"
    _VALID_URL = (
        r"https?://on\.orf\.at/video/(?P<id>\d+)(?:/(?P<segment>\d+))?"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        video_id, segment_id = self._match_valid_url(url).group(
            "id", "segment"
        )

        encrypted_id = base64.b64encode(
            f"3dSlfek03nsLKdj4Jsd{video_id}".encode()
        ).decode()
        ...
        selected_segment = traverse_obj(
            segments, (lambda _, v: str(v["id"]) == segment_id, any)
        )
        ...
        if self._configuration_arg(
            ...
            return self.playlist_result(
                (
                    self._extract_video_info(str(segment["id"]), segment)
