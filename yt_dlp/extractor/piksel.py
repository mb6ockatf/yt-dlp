...
class PikselIE(InfoExtractor):
    ...
    def _call_api(
        ...
        response = (
            traverse_obj(
                self._download_json(url, display_id, query=query, fatal=fatal),
                ("response", {dict}),
            )
            or {}
        )
        failure = (
            traverse_obj(response, ("failure", "reason"))
            if response
    ...
    def _real_extract(self, url):
        ...
        query = (
            {"refid": ref_id, "prefid": display_id}
            if ref_id
            else {"v": display_id}
        )
            ...
            if asset_id:
                process_asset_files(
                    try_get(
                        ...
                        lambda x: x["WsAssetFileResponse"]["AssetFiles"],
                    )
                )
