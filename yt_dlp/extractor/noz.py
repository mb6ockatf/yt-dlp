...
class NozIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        duration = int_or_none(
            xpath_text(doc, ".//article/movie/file/duration")
        )
        formats = []
        for qnode in doc.findall(".//article/movie/file/qualities/qual"):
            ...
            if http_url:
                ...
                m3u8_url = (
                    m3u8_url_ele.text if m3u8_url_ele is not None else None
                )
                if m3u8_url:
                    formats.extend(
