...
class RDSIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        item = self._parse_json(
            self._search_regex(
                r"(?s)itemToPush\s*=\s*({.+?});", webpage, "item"
            ),
            display_id,
            js_to_json,
        )
        ...
        title = (
            ...
            or self._html_search_meta("title", webpage, "title", fatal=True)
        )
        description = self._og_search_description(
            webpage
        ) or self._html_search_meta("description", webpage, "description")
        thumbnail = (
            item.get("urlImageBig")
