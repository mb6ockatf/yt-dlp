    ...
    def _real_extract(self, url):
        ...
        title = (
            ...
            or self._html_search_regex(
                r"<title\b[^>]*>([^<]+)</title\b", webpage, "title"
            )
        )

        jwplayer_sources = self._parse_json(
            self._search_regex(
                r"""(?s)jwplayer\s*\(\s*['"]mvplayer['"]\s*\)\s*\.\s*setup\s*\(.*?\bsources\s*:\s*(\[.*?])\s*[,});]""",
                webpage,
                "jwplayer sources",
                fatal=False,
            )
        ...
        upload_date = (
            unified_strdate(
                ...
                re.sub(
                    "|".join(self._MONTH_NAMES_KA),
                    lambda m: MONTH_NAMES["en"][
                        self._MONTH_NAMES_KA.index(m.group(0))
                    ],
                    upload_date,
                    flags=re.I,
                )
        ...
        return {
            ...
            "view_count": int_or_none(
                get_element_by_class("mv_vid_views", webpage)
            ),
            "like_count": int_or_none(
                get_element_by_id("likes_count", webpage)
            ),
            "dislike_count": int_or_none(
                get_element_by_id("dislikes_count", webpage)
            ),
        }
