...
class CPACIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        content = self._download_json(
            "https://www.cpac.ca/api/1/services/contentModel.json?url=/site/website/episode/index.xml&crafterSite=cpacca&id="
            + video_id,
            video_id,
        )
        video_url = try_get(
            content, lambda x: x["page"]["details"]["videoUrl"], str
        )
        formats = []
        if video_url:
            ...
            formats = self._extract_m3u8_formats(
                video_url, video_id, m3u8_id="hls", ext="mp4"
            )
            for fmt in formats:
        ...
        return {
            ...
            "description": str_or_none(
                content["details"].get(f"description_{url_lang}_t")
            ),
            "timestamp": unified_timestamp(
                content["details"].get("liveDateTime")
            ),
            "categories": [category] if category else None,
            "thumbnail": urljoin(
                url, str_or_none(content["details"].get(f"image_{url_lang}_s"))
            ),
...
class CPACPlaylistIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        url_lang = (
            "fr"
            if any(x in url for x in ("/emission?", "/rechercher?"))
            else "en"
        )
        pl_type, list_type = (
            ("program", "itemList")
            if any(x in url for x in ("/program?", "/emission?"))
            else ("search", "searchResult")
        )
        api_url = f"https://www.cpac.ca/api/1/services/contentModel.json?url=/site/website/{pl_type}/index.xml&crafterSite=cpacca&{video_id}"
        content = self._download_json(api_url, video_id)
        entries = []
        total_pages = int_or_none(
            try_get(content, lambda x: x["page"][list_type]["totalPages"]),
            default=1,
        )
        for page in range(1, total_pages + 1):
            ...
            for item in (
                try_get(content, lambda x: x["page"][list_type]["item"], list)
                or []
            ):
                episode_url = urljoin(
                    url, try_get(item, lambda x: x[f"url_{url_lang}_s"])
                )
                if episode_url:
        ...
        return self.playlist_result(
            ...
            playlist_title=try_get(
                content, lambda x: x["page"]["program"][f"title_{url_lang}_t"]
            )
            or video_id.split("=")[-1],
            playlist_description=try_get(
                content,
                lambda x: x["page"]["program"][f"description_{url_lang}_t"],
            ),
        )
