    ...
    def _fetch_token(self, site, url):
        ...
        if cookies and cookies.get("token"):
            self._TOKENS[site] = self._search_regex(
                r"22authorizationToken\%22:\%22([^\%]+)\%22",
                cookies["token"].value,
                "token",
            )
    ...
    def _call_api(self, site, path, video_id, url, query):
        ...
        try:
            return self._download_json(
                self._API_BASE + path,
                video_id,
                headers={"Authorization": self._TOKENS.get(site)},
                query=query,
            )
...
class ViewLiftEmbedIE(ViewLiftBaseIE):
    ...
    def _real_extract(self, url):
        ...
        subs = {}
        for sub in (
            traverse_obj(content_data, ("contentDetails", "closedCaptions"))
            or []
        ):
            sub_url = sub.get("url")
            if not sub_url:
...
class ViewLiftIE(ViewLiftBaseIE):
    ...
    def suitable(cls, url):
        return (
            False if ViewLiftEmbedIE.suitable(url) else super().suitable(url)
        )
            ...
            for episode in season.get("episodes") or []:
                ...
                if path:
                    yield self.url_result(
                        f"https://www.{domain}{path}", ie=self.ie_key()
                    )
    ...
    def _real_extract(self, url):
        ...
        seasons = next(
            (
                m["contentData"][0]["seasons"]
                for m in modules
                if m.get("moduleType") == "ShowDetailModule"
            ),
            None,
        )
        if seasons:
            return self.playlist_result(
                self._show_entries(domain, seasons), display_id
            )
