...
class AntennaGrWatchIE(AntennaBaseIE):
    ...
    def _real_extract(self, url):
        ...
        info["_old_archive_ids"] = [
            make_archive_id("Ant1NewsGrWatch", video_id)
        ]
        return info
    ...
    def _real_extract(self, url):
        ...
        if not embed_urls:
            raise ExtractorError(
                f"no videos found for {video_id}", expected=True
            )
...
class Ant1NewsGrEmbedIE(AntennaBaseIE):
    ...
    def _real_extract(self, url):
        ...
        canonical_url = self._request_webpage(
            ...
            errnote="Could not resolve canonical player URL",
        ).url
