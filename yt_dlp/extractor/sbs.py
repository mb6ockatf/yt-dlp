...
class SBSIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        formats, subtitles = self._extract_smil_formats_and_subtitles(
            update_url_query(
                f"{self._PLAYER_API}/video_smil", {"id": video_id}
            ),
            video_id,
        )
        ...
        if not formats:
            urlh = self._request_webpage(
                HEADRequest("https://sbs-vod-prod-01.akamaized.net/"),
                video_id,
                note="Checking geo-restriction",
                fatal=False,
                expected_status=403,
            )
            if urlh:
            ...
            self.raise_no_formats(
                "No formats are available", video_id=video_id
            )

        media = (
            traverse_obj(
                self._download_json(
                    f"{self._PLAYER_API}/video_stream",
                    video_id,
                    fatal=False,
                    query={"id": video_id, "context": "tv"},
                ),
                ("video_object", {dict}),
            )
            or {}
        ...
        return {
            ...
            **traverse_obj(
                media,
                {
                    ...
                    "series": (
                        (("partOfSeries", "name"), "seriesTitle"),
                        {str},
                    ),
                    "series_id": (
                        (("partOfSeries", "uuid"), "seriesID"),
                        {str},
                    ),
                    ...
                    "age_limit": (
                        ("classificationID", "contentRating"),
                        {str.upper},
            ...
            **traverse_obj(
                media,
                {
                    "categories": (
                        ("genres", ...),
                        ("taxonomy", ("genre", "subgenre"), "name"),
                        {str},
                    ),
                    "tags": (
                        (
                            "consumerAdviceTexts",
                            ("sbsSubCertification", "consumerAdvice"),
                        ),
                        ...,
                        {str},
                    ),
                    "thumbnails": (
                        "thumbnails",
                        lambda _, v: url_or_none(v["contentUrl"]),
                        {
                            "id": ("name", {str}),
                            "url": "contentUrl",
