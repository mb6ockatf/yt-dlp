...
class SonyLIVIE(InfoExtractor):
    ...
    def _perform_login(self, username, password):
        ...
        if username.lower() == "token" and try_call(
            lambda: jwt_decode_hs256(password)
        ):
        ...
        if otp_verify_json["resultCode"] == "KO":
            raise ExtractorError(otp_request_json["message"], expected=True)
        self._HEADERS["authorization"] = otp_verify_json["resultObj"][
            "accessToken"
        ]
    ...
    def _call_api(self, version, path, video_id):
        try:
            ...
            if (
                ...
                and e.cause.status == 406
                and self._parse_json(
    ...
    def _real_extract(self, url):
        ...
        if not self.get_param("allow_unplayable_formats") and content.get(
            "isEncrypted"
        ):
            self.report_drm(video_id)
        dash_url = content["videoURL"]
        headers = {
            "x-playback-session-id": "%s-%d"
            % (uuid.uuid4().hex, time.time() * 1000),
        }
        ...
        metadata = self._call_api("1.6", "IN/DETAIL/" + video_id, video_id)[
            "containers"
        ][0]["metadata"]
        ...
        for sub in content.get("subtitle", []):
            ...
            subtitles.setdefault(
                sub.get("subtitleLanguageName", "ENG"), []
            ).append(
                {
                    "url": sub_url,
                }
        ...
        return {
            ...
            "description": metadata.get("longDescription")
...
class SonyLIVSeriesIE(InfoExtractor):
    ...
    def _entries(self, show_id, sort_order):
        ...
        headers["security_token"] = self._download_json(
            f"{self._API_BASE}/1.4/A/ENG/WEB/ALL/GETTOKEN",
            show_id,
            "Downloading security token",
            headers=headers,
        )["resultObj"]
        ...
        for season in seasons:
            ...
            note = (
                traverse_obj(season, ("metadata", "title", {str})) or "season"
            )
            cursor = 0
            for page_num in itertools.count(1):
                ...
                for episode in episodes:
                    ...
                    yield self.url_result(
                        f"sonyliv:{video_id}", SonyLIVIE, video_id
                    )
                cursor += 100
    ...
    def _real_extract(self, url):
        ...
        sort_order = self._configuration_arg(
            "sort_order", [self._SORT_ORDERS[0]]
        )[0]
        ...
        return self.playlist_result(
            self._entries(show_id, sort_order), playlist_id=show_id
        )
