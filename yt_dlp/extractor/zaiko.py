...
class ZaikoIE(ZaikoBaseIE):
    _VALID_URL = (
        r"https?://(?:[\w-]+\.)?zaiko\.io/event/(?P<id>\d+)/stream(?:/\d+)+"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        player_page = self._download_webpage(
            stream_meta["stream-access"]["video_source"],
            video_id,
        ...
        initial_event_info = (
            traverse_obj(player_meta, ("initial_event_info", {dict})) or {}
        )
        ...
        live_status, msg, expected = {
            ...
            "archiving": (
                "post_live",
                "Event VOD is still being processed",
                True,
            ),
        ...
        if traverse_obj(initial_event_info, ("is_jwt_protected", {bool})):
            stream_url = self._download_json(
                ...
                "Failed to download JWT-protected stream URL",
            )["playback_url"]
        else:
        ...
        formats = (
            self._extract_m3u8_formats(
                stream_url, video_id, live=True, fatal=False
            )
            if stream_url
            else []
        )
...
class ZaikoETicketIE(ZaikoBaseIE):
    _VALID_URL = (
        r"https?://(?:www.)?zaiko\.io/account/eticket/(?P<id>[\w=-]{49})"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        ticket_id = (
            try_call(
                lambda: base64.urlsafe_b64decode(ticket_id[1:])
                .decode()
                .replace("|", "-")
            )
            or ticket_id
        ...
        return self.playlist_result(
            [
                ...
                for stream in traverse_obj(eticket, ("streams", ..., "url"))
            ],
            ticket_id,
            **traverse_obj(
                eticket,
