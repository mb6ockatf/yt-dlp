...
class JioCinemaBaseIE(InfoExtractor):
    ...
    def _call_api(
        self, url, video_id, note="Downloading API JSON", headers={}, data={}
    ):
    ...
    def _refresh_token(self):
        if (
            not JioCinemaBaseIE._REFRESH_TOKEN
            or not JioCinemaBaseIE._DEVICE_ID
        ):
    ...
    def _is_token_expired(self, token):
        return (try_call(lambda: jwt_decode_hs256(token)["exp"]) or 0) <= int(
            time.time() - 180
        )
    ...
    def _perform_login(self, username, password):
        ...
        if username.lower() == "token":
            if try_call(lambda: jwt_decode_hs256(password)):
                ...
                refresh_token = self._configuration_arg(
                    "refresh_token", [""], ie_key=JioCinemaIE
                )[0]
        ...
        elif username.lower() == "device" and re.fullmatch(
            rf"(?:{UUID_RE}|\d+)", password
        ):
            ...
            if (
                not JioCinemaBaseIE._REFRESH_TOKEN
                or not JioCinemaBaseIE._ACCESS_TOKEN
            ):
                raise ExtractorError(
                ...
                guest_token,
                {
                    "deviceInfo": {
                        "consumptionDeviceName": (
                            "iPhone" if is_iphone else "Android"
                        ),
                        "info": {
                            "platform": {
                                "name": "iPhone OS" if is_iphone else "Android"
                            },
...
class JioCinemaIE(JioCinemaBaseIE):
    ...
    def _extract_formats_and_subtitles(self, playback, video_id):
        ...
        return {
            ...
            "formats": traverse_obj(
                formats,
                (
                    lambda _, v: "/_definst_/smil:vod/" not in v["url"]
                    or v["height"] <= 480
                ),
            ),
            "subtitles": subtitles,
    ...
    def _real_extract(self, url):
        ...
        playback = self._call_api(
            f"https://apis-jiovoot.voot.com/playbackjv/v3/{video_id}",
            video_id,
        ...
        if status_code == 474:
            ...
            raise ExtractorError(
                "The requested content is not available", expected=True
            )
                ...
                (
                    "data",
                    {
                        ...
                        "season": (
                            "tournamentName",
                            {str},
                            {lambda x: x if x != "Season 0" else None},
                        ),
                        "season_number": (
                            "episode",
                            "season",
    ...
    def _entries(self, series_id):
        seasons = traverse_obj(
            self._download_json(
                ...
                query={"responseType": "common"},
            ),
            (
                "trays",
                lambda _, v: v["trayId"] == "season-by-show-multifilter",
                "trayTabs",
                lambda _, v: v["id"],
            ),
        )
        ...
        for season_num, season in enumerate(seasons, start=1):
            ...
            for page_num in itertools.count(1):
                episodes = traverse_obj(
                    ...
                    (
                        "result",
                        lambda _, v: v["id"] and url_or_none(v["slug"]),
                    ),
                )
                if not episodes:
                    break
