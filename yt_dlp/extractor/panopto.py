...
class PanoptoBaseIE(InfoExtractor):
    ...
    def _call_api(
        self, base_url, path, video_id, data=None, fatal=True, **kwargs
    ):
...
class PanoptoIE(PanoptoBaseIE):
            ...
            "info_dict": {
                ...
                "subtitles": {
                    "en-US": [
                        {
                            "ext": "srt",
                            "data": "md5:a3f4d25963fdeace838f327097c13265",
                        }
                    ],
                    "es-ES": [
                        {
                            "ext": "srt",
                            "data": "md5:57e9dad365fd0fbaf0468eac4949f189",
                        }
                    ],
                },
            },
    ...
    def suitable(cls, url):
        return (
            False if PanoptoPlaylistIE.suitable(url) else super().suitable(url)
        )
    ...
    def _mark_watched(self, base_url, video_id, delivery_info):
        ...
        if invocation_id and stream_id and duration:
            ...
            self._download_webpage(
                base_url + "/Services/Analytics.svc/AddStreamRequests",
                video_id,
                fatal=False,
    ...
    def _extract_chapters(timestamps):
        ...
        for timestamp in timestamps or []:
            ...
            start, duration = int_or_none(timestamp.get("Time")), int_or_none(
    ...
    def _extract_mhtml_formats(base_url, timestamps):
        ...
        for timestamp in timestamps or []:
            ...
            obj_id, obj_sn = timestamp.get("ObjectIdentifier"), timestamp.get(
                "ObjectSequenceNumber"
            )
            if (
                timestamp.get("EventTargetType") == "PowerPoint"
                and obj_id is not None
                and obj_sn is not None
            ):
                image_frags.setdefault("slides", []).append(
                    {
                        "url": base_url
                        + f"/Pages/Viewer/Image.aspx?id={obj_id}&number={obj_sn}",
                        "duration": duration,
                    }
                )

            obj_pid, session_id, abs_time = (
                timestamp.get("ObjectPublicIdentifier"),
                timestamp.get("SessionID"),
                timestamp.get("AbsoluteTime"),
            )
            if None not in (obj_pid, session_id, abs_time):
                image_frags.setdefault("chapter", []).append(
                    {
                        "url": base_url
                        + f"/Pages/Viewer/Thumb.aspx?eventTargetPID={obj_pid}&sessionPID={session_id}&number={obj_sn}&isPrimary=false&absoluteTime={abs_time}",
                        "duration": duration,
                    }
                )
        ...
        def _gen_lines():
            for i, line in enumerate(data):
                ...
                if duration:
                    ...
                    end_time = (
                        traverse_obj(data, (i + 1, "Time"))
                        or delivery["Duration"]
                    )
    ...
    def _get_subtitles(self, base_url, video_id, delivery):
        ...
        for lang in delivery.get("AvailableLanguages") or []:
            ...
            subtitles.setdefault(
                self._SUB_LANG_MAPPING.get(lang) or "default", []
            ).append(
                {
                    "ext": "srt",
    ...
    def _real_extract(self, url):
        ...
        podcast_formats, podcast_subtitles = (
            self._extract_streams_formats_and_subtitles(
                video_id, delivery.get("PodcastStreams"), format_note="PODCAST"
            )
        )

        streams_formats, streams_subtitles = (
            self._extract_streams_formats_and_subtitles(
        ...
        subtitles = self._merge_subtitles(
            podcast_subtitles,
            streams_subtitles,
            self.extract_subtitles(base_url, video_id, delivery),
        )
        ...
        return {
            ...
            "timestamp": (
                session_start_time - 11640000000
                if session_start_time
                else None
            ),
            "duration": delivery.get("Duration"),
            "thumbnail": base_url
            + f"/Services/FrameGrabber.svc/FrameRedirect?objectId={video_id}&mode=Delivery&random={random.random()}",
...
class PanoptoPlaylistIE(PanoptoBaseIE):
    ...
    def _real_extract(self, url):
        base_url, playlist_id = self._match_valid_url(url).group(
            "base_url", "id"
        )
        ...
        if video_id:
            if self.get_param("noplaylist"):
                ...
                return self.url_result(
                    base_url + f"/Pages/Viewer.aspx?id={video_id}",
        ...
        return self.playlist_result(
            self._entries(
                base_url, playlist_id, playlist_info["SessionListId"]
            ),
...
class PanoptoListIE(PanoptoBaseIE):
    ...
    def _fetch_page(self, base_url, query_params, display_id, page):
        ...
        for result in get_first(response, "Results", default=[]):
            ...
            yield {
                ...
                "url": traverse_obj(
                    result, "ViewerUrl", "EmbedUrl", get_all=False
                )
                or (base_url + f"/Pages/Viewer.aspx?id={item_id}"),
    ...
    def _real_extract(self, url):
        ...
        info["entries"] = OnDemandPagedList(
            functools.partial(
                self._fetch_page, base_url, query_params, display_id
            ),
            self._PAGE_SIZE,
        )
