...
class NintendoIE(InfoExtractor):
    ...
    _TESTS = [
        {
            ...
            "info_dict": {
                ...
                "age_limit": 17,
                "_old_archive_ids": [
                    "nintendo J2bXdmaTE6fe3dWJTPcc7m23FNbc_A1V"
                ],
            },
        },
        {
    ...
    def _create_asset_url(self, path):
        return urljoin(
            "https://assets.nintendo.com/", urllib.parse.quote(path)
        )
    ...
    def _real_extract(self, url):
        ...
        self.write_debug(
            f"Using locale {parsed_locale} (from {locale})", only_once=True
        )
        ...
        if not direct_info:
            raise ExtractorError(
                f"No Nintendo Direct with id {slug} exists", expected=True
            )
        ...
        result["formats"] = self._extract_m3u8_formats(
            self._create_asset_url(
                f"/video/upload/sp_full_hd/v1/{asset_id}.m3u8"
            ),
            slug,
        )
