...
class SverigesRadioBaseIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if not audio_id:
            ...
            audio_id = (
                traverse_obj(
                    ...
                    (
                        {extract_attributes},
                        ("data-audio-id", "data-publication-id"),
                    ),
                    get_all=False,
                )
                or self._parse_json(
                    get_element_by_id("gtm-metadata", webpage), display_id
                )["pageId"]
            )
        ...
        item = self._download_json(
            self._BASE_URL + "audiometadata",
            audio_id,
            "Downloading audio JSON metadata",
            query=query,
        )["items"][0]
        ...
        for quality in self._QUALITIES:
            ...
            abr = int_or_none(
                self._search_regex(
                    r"_a(\d+)\.m4a", audio_url, "audio bitrate", default=None
                )
            ) or self._CODING_FORMAT_TO_ABR_MAP.get(coding_format)
            formats.append(
                {
                    "abr": abr,
