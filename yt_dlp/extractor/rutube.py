...
class RutubeEmbedIE(RutubeBaseIE):
    ...
    _VALID_URL = (
        r"https?://rutube\.ru/(?:video|play)/embed/(?P<id>[0-9]+)(?:[?#/]|$)"
    )
    ...
    def _entries(self, playlist_id, *args, **kwargs):
        ...
        for pagenum in itertools.count(1):
            page = self._download_json(
                next_page_url
                or self._next_page_url(pagenum, playlist_id, *args, **kwargs),
                playlist_id,
...
class RutubeMovieIE(RutubePlaylistBaseIE):
    ...
    def _real_extract(self, url):
        ...
        movie = self._download_json(
            self._MOVIE_TEMPLATE % movie_id, movie_id, "Downloading movie JSON"
        )
...
class RutubeChannelIE(RutubePlaylistBaseIE):
    ...
    def _real_extract(self, url):
        playlist_id, slug, section = self._match_valid_url(url).group(
            "id", "slug", "section"
        )
        ...
        if slug:
            ...
            playlist_id = traverse_obj(
                ...
                (
                    "api",
                    "queries",
                    lambda k, _: k.startswith("channelIdBySlug"),
                    "data",
                    "channel_id",
                    {int},
                    {str_or_none},
