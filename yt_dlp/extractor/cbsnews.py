...
class CBSNewsBaseIE(InfoExtractor):
    ...
    def _extract_video(self, item, video_url, video_id):
        if (
            mimetype2ext(item.get("format"), default=determine_ext(video_url))
            == "mp4"
        ):
            ...
            if anvato_id:
                return self.url_result(
                    ...
                    _old_archive_ids=[make_archive_id(self, anvato_id)],
                )
...
class CBSNewsIE(CBSNewsBaseIE):
    ...
    IE_DESC = "CBS News"
    _VALID_URL = (
        r"https?://(?:www\.)?cbsnews\.com/(?:news|video)/(?P<id>[\w-]+)"
    )
    ...
    def _real_extract(self, url):
        ...
        if not video_url:
            self.raise_no_formats(
                "No video content was found", expected=True, video_id=video_id
            )
...
class CBSLocalBaseIE(CBSNewsBaseIE):
    def _real_extract(self, url):
        ...
        if not video_url:
            ...
            return self.url_result(
                smuggle_url(
                    f'anvato:{anv_data.get("anvack") or self._ANVACK}:{anvato_id}',
                    {
...
class CBSNewsLiveBaseIE(CBSNewsBaseIE):
    ...
    def _real_extract(self, url):
        ...
        data = traverse_obj(
            self._download_json(
                "https://feeds-cbsn.cbsnews.com/2.0/rundown/",
                video_id,
                query={
                    "partner": "cbsnsite",
                    "edition": video_id,
        ...
        video_url = traverse_obj(
            data,
            (("videoUrlDAI", ("videoUrl", "base")), {url_or_none}),
            get_all=False,
        )
        if not video_url:
...
class CBSNewsLiveVideoIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        video_info = self._download_json(
            "http://feeds.cbsn.cbsnews.com/rundown/story",
            display_id,
            query={
                "device": "desktop",
                "dvr_slug": display_id,
