...
class ITVIE(InfoExtractor):
    ...
    def _get_subtitles(
        self, video_id, variants, ios_playlist_url, headers, *args, **kwargs
    ):
        subtitles = {}
        ...
        platform_tag_subs, featureset_subs = next(
            (
                ...
                for platform_tag, featuresets in reversed(
                    list(variants.items())
                )
    ...
    def _real_extract(self, url):
        ...
        platform_tag_video, featureset_video = next(
            (
                ...
                for featureset in featuresets
                if set(try_get(featureset, lambda x: x[:2]) or [])
                == {"aes", "hls"}
            ),
            (None, None),
        )
        ...
        ios_playlist_url = (
            params.get("data-video-playlist") or params["data-video-id"]
        )
        ...
        video_data = (
            try_get(ios_playlist, lambda x: x["Playlist"]["Video"], dict) or {}
        )
        ...
        if not info:
            json_ld = self._parse_json(
                self._search_regex(
                    JSON_LD_RE, webpage, "JSON-LD", "{}", group="json_ld"
                ),
                video_id,
                fatal=False,
            )
        ...
        return merge_dicts(
            {
                ...
                "subtitles": self.extract_subtitles(
                    video_id, variants, ios_playlist_url, headers
                ),
                "duration": parse_duration(video_data.get("Duration")),
                "description": clean_html(
                    get_element_by_class("episode-info__synopsis", webpage)
                ),
    ...
    def _real_extract(self, url):
        ...
        json_map = (
            try_get(
                ...
                lambda x: x["props"]["pageProps"]["article"]["body"][
                    "content"
                ],
            )
            or []
