    ...
    def _parse_video_data(self, video_data, video_id, is_live):
        ...
        return {
            ...
            "thumbnail": format_field(
                img, None, "http://admin.mangomolo.com/analytics/%s"
            ),
...
class AWAANLiveIE(AWAANBaseIE):
    IE_NAME = "awaan:live"
    _VALID_URL = (
        r"https?://(?:www\.)?(?:awaan|dcndigital)\.ae/(?:#/)?live/(?P<id>\d+)"
    )
    _TEST = {
    ...
    def _real_extract(self, url):
        ...
        embed_url = (
            ...
            + urllib.parse.urlencode(
                {
                    "id": base64.b64encode(
                        channel_data["user_id"].encode()
                    ).decode(),
                    "channelid": base64.b64encode(
                        channel_data["id"].encode()
                    ).decode(),
