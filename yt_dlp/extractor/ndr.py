...
class NDRIE(NDRBaseIE):
    ...
    _VALID_URL = (
        r"https?://(?:\w+\.)*ndr\.de/(?:[^/]+/)*(?P<id>[^/?#]+),[\da-z]+\.html"
    )
    _TESTS = [
        {
    ...
    def _extract_embed(self, webpage, display_id, url):
        embed_url = (
            self._html_search_meta(
                "embedURL", webpage, "embed URL", default=None
            )
        ...
        if re.match(r"^[a-z]+\d+$", embed_url):
            ...
            embed_url = (
                f"ndr:{ndr_id}" if ndr_id else NDR_INFO_URL_TPL % (embed_url,)
            )
        if not embed_url:
...
class NDREmbedBaseIE(
    InfoExtractor
):  # XXX: Conventionally, Concrete class names do not end in BaseIE
            ...
            if ext == "f4m":
                formats.extend(
                    self._extract_f4m_formats(
                        src + "?hdcore=3.7.0&plugin=aasp-3.7.0.39.44",
                        video_id,
                        f4m_id="hds",
                        fatal=False,
                    )
