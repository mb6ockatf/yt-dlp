...
class ShemarooMeIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?shemaroome\.com/(?:movies|shows)/(?P<id>[^?#]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        release_date = self._html_search_regex(
            (
                r'itemprop="uploadDate">\s*([\d-]+)',
                r'id="release_date" value="([\d-]+)',
            ),
            webpage,
            "release date",
            fatal=False,
