...
class IPrimaIE(InfoExtractor):
    ...
    def _perform_login(self, username, password):
        ...
        login_page = self._download_webpage(
            f"{self._AUTH_ROOT}/oauth2/login",
            None,
        ...
        if "/profile-select" in login_handle.url:
            ...
            login_handle = self._request_webpage(
                f"{self._AUTH_ROOT}/user/profile-select-perform/{profile_id}",
                None,
        ...
        token_data = self._download_json(
            f"{self._AUTH_ROOT}/oauth2/token",
            None,
    ...
    def _raise_access_error(self, error_code):
        if error_code == "PLAY_GEOIP_DENIED":
            ...
            self.raise_no_formats(
                "Access to stream infos forbidden", expected=True
            )
    ...
    def _real_extract(self, url):
        ...
        if not video_id:
            ...
            video_id = traverse_obj(
                nuxt_data,
                (..., "content", "additionals", "videoPlayId", {str}),
                get_all=False,
            )
        ...
        if not video_id:
            ...
            video_id = traverse_obj(
                nuxt_data, lambda _, v: re.fullmatch(r"p\d+", v), get_all=False
            )
...
class IPrimaCNNIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        ) or self._search_regex(r"<h1>([^<]+)", webpage, "title")
