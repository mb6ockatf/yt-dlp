...
class SaitosanIE(InfoExtractor):
    ...
    IE_NAME = "Saitosan"
    _VALID_URL = (
        r"https?://(?:www\.)?saitosan\.net/bview.html\?id=(?P<id>[0-9]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        self._download_webpage(
            base, b_id, data=payload, note="Polling socket with payload"
        )
        ...
        self._download_webpage(
            base, b_id, data="1:1", note="Closing socket", fatal=False
        )
        ...
        return {
            ...
            "formats": self._extract_m3u8_formats(
                m3u8_url, b_id, "mp4", live=True
            ),
            "thumbnail": m3u8_url.replace("av.m3u8", "thumb"),
            "uploader": try_get(
                b_data, lambda x: x["broadcast_user"]["name"]
            ),  # same as title
            "is_live": True,
        }
