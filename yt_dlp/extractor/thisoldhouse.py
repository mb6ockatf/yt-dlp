...
class ThisOldHouseIE(InfoExtractor):
    ...
    def _perform_login(self, username, password):
        ...
        urlh = self._request_webpage(
            ...
            query={"redirect_to": "https://www.thisoldhouse.com/insider"},
        )
            ...
            auth_form = self._download_webpage(
                ...
                data=json.dumps(
                    filter_dict(
                        {
                            **{
                                ("client_id" if k == "client" else k): v[0]
                                for k, v in parse_qs(urlh.url).items()
                            },
                            "tenant": "thisoldhouse",
                            "username": username,
                            "password": password,
                            ...
                            "sso": True,
                            "_csrf": try_call(
                                lambda: self._get_cookies(self._LOGIN_URL)[
                                    "_csrf"
                                ].value
                            ),
    ...
    def _real_extract(self, url):
        ...
        if video_url:
            video_url = self._request_webpage(
                HEADRequest(video_url), video_id, "Resolving Zype URL"
            ).url
        ...
        if not parse_qs(video_url).get("videoId"):
            video_url = self._request_webpage(
                HEADRequest(video_url), video_id, "Resolving Brightcove URL"
            ).url
        return self.url_result(
            smuggle_url(video_url, {"referrer": url}),
            BrightcoveNewIE,
            video_id,
        )
