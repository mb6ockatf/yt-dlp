...
class ArteTVIE(ArteTVBaseIE):
    ...
    def _fix_accessible_subs_locale(subs):
        ...
        for lang, sub_formats in subs.items():
            for fmt in sub_formats:
                ...
                suffix = (
                    "acc"
                    if url.endswith("-MAL.m3u8")
                    else "forced" if "_VO" not in url else None
                )
                updated_subs.setdefault(
                    join_nonempty(lang, suffix), []
                ).append(fmt)
        return updated_subs
    ...
    def _real_extract(self, url):
        ...
        geoblocking = (
            traverse_obj(
                config, ("data", "attributes", "restriction", "geoblocking")
            )
            or {}
        )
        if geoblocking.get("restrictedArea"):
            raise GeoRestrictedError(
                ...
                countries=self._COUNTRIES_MAP.get(
            ...
            if m:
                lang_pref = int(
                    "".join(
                        ...
                        for x in (
                            m.group("vlang")
                            == language_code,  # we prefer voice in the requested language
                            not m.group(
                                "audio_desc"
                            ),  # and not the audio description version
                            bool(
                                m.group("original_voice")
                            ),  # but if voice is not in the requested language, at least choose the original voice
                            m.group("sub_lang")
                            == language_code,  # if subtitles are present, we prefer them in the requested language
                            not m.group(
                                "has_sub"
                            ),  # but we prefer no subtitles otherwise
                            not m.group(
                                "sdh_sub"
                            ),  # and we prefer not the hard-of-hearing subtitles if there are subtitles
                        )
                    )
                )

            short_label = traverse_obj(
                stream_version, "shortLabel", expected_type=str, default="?"
            )
        ...
        entries = [
            {
                ...
                "thumbnail": url_or_none(
                    traverse_obj(video, ("mainImage", "url"))
                ),
                "duration": int_or_none(
                    traverse_obj(video, ("duration", "seconds"))
                ),
            }
            for video in traverse_obj(
                playlist, ("items", lambda _, v: v["config"]["url"])
            )
        ]
...
class ArteTVCategoryIE(ArteTVBaseIE):
    ...
    def suitable(cls, url):
        return not any(
            ie.suitable(url) for ie in (ArteTVIE, ArteTVPlaylistIE)
    ...
    def _real_extract(self, url):
        ...
        title = (
            strip_or_none(
                self._generic_title("", webpage, default="").rsplit("|", 1)[0]
            )
            or None
        )
