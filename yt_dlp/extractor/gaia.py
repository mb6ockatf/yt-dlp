    ...
    def _real_initialize(self):
        ...
        if auth:
            auth = self._parse_json(
                urllib.parse.unquote(auth.value), None, fatal=False
            )
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "description": strip_or_none(
                get_field_value("body") or get_field_value("teaser")
            ),
            "timestamp": int_or_none(node.get("created")),
            "subtitles": subtitles,
            "duration": int_or_none(vdata.get("duration")),
            "like_count": int_or_none(
                try_get(fivestar, lambda x: x["up_count"]["value"])
            ),
            "dislike_count": int_or_none(
                try_get(fivestar, lambda x: x["down_count"]["value"])
            ),
