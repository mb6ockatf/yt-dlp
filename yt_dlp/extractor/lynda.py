...
class LyndaBaseIE(InfoExtractor):
    ...
    def _perform_login(self, username, password):
        ...
        # Already logged in
        if any(
            re.search(p, signin_page)
            for p in (r"isLoggedIn\s*:\s*true", r"logout\.aspx", r">Log out<")
        ):
            return
...
class LyndaIE(LyndaBaseIE):
    ...
    def _real_extract(self, url):
        ...
        video = self._download_json(
            "https://www.lynda.com/ajax/player",
            video_id,
        ...
        if not video:
                    ...
                    formats.append(
                        {
                            "url": format_url,
                            "format_id": (
                                f"{cdn}-{format_id}" if cdn else format_id
                            ),
            ...
            conviva = self._download_json(
                "https://www.lynda.com/ajax/player/conviva",
                video_id,
                "Downloading conviva JSON",
                query=query,
            )
        ...
        if "Status" in video:
            raise ExtractorError(
                "lynda returned error: {}".format(video["Message"]),
                expected=True,
            )
...
class LyndaCourseIE(LyndaBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            entries, course_id, course_title, course_description
        )
