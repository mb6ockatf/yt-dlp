...
class NoicePodcastIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        nextjs_data = self._search_nextjs_data(webpage, display_id)["props"][
            "pageProps"
        ]["contentDetails"]
        ...
        return {
            ...
            "subtitles": subtitles,
            "description": (
                nextjs_data.get("description")
                or clean_html(nextjs_data.get("htmlDescription"))
                or self._html_search_meta(
                    ["description", "og:description"], webpage
                )
            ),
            "thumbnail": nextjs_data.get("image")
            or self._html_search_meta("og:image", webpage),
            ...
            "modified_timestamp": parse_iso8601(
                nextjs_data.get("updatedAt")
                or self._html_search_meta("og:updated_time", webpage)
            ),
            ...
            "channel_id": traverse_obj(
                nextjs_data, ("catalog", "id"), "catalogId"
            ),
            **traverse_obj(
                nextjs_data,
                (
