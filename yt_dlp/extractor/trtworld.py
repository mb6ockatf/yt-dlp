...
class TrtWorldIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        nuxtjs_data = self._search_nuxt_data(webpage, display_id)["videoData"][
            "content"
        ]["platforms"]
        formats = []
            ...
            if determine_ext(media_url) == "m3u8":
                formats.extend(
                    self._extract_m3u8_formats(
                        media_url, display_id, fatal=False
                    )
                )
            else:
                formats.append(
        ...
        if not formats:
            if youtube_id := traverse_obj(
                nuxtjs_data, ("youtube", "metadata", "youtubeId")
            ):
