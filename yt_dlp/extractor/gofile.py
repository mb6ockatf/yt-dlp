...
class GofileIE(InfoExtractor):
    ...
    def _entries(self, file_id):
        ...
        if password:
            query_params["password"] = hashlib.sha256(
                password.encode()
            ).hexdigest()
        ...
        if status == "error-passwordRequired":
            ...
            raise ExtractorError(
                f"{self.IE_NAME} said: status {status}", expected=True
            )

        found_files = False
        for file in (
            try_get(files, lambda x: x["data"]["children"], dict) or {}
        ).values():
            file_type, file_format = file.get("mimetype").split("/", 1)
            if (
                file_type not in ("video", "audio")
                and file_format != "vnd.mts"
            ):
                continue
        ...
        if not found_files:
            raise ExtractorError(
                "No video/audio found at provided URL.", expected=True
            )

    def _real_extract(self, url):
        file_id = self._match_id(url)
        return self.playlist_result(
            self._entries(file_id), playlist_id=file_id
        )
