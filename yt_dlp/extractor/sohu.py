...
class SohuIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        vid = self._html_search_regex(
            r'var vid ?= ?["\'](\d+)["\']', webpage, "video path"
        )
        ...
        for format_id in ("nor", "high", "super", "ori", "h2644k", "h2654k"):
            ...
            if not vid_id:
                continue
            vid_id = str(vid_id)
            formats_json[format_id] = (
                vid_data if vid == vid_id else _fetch_data(vid_id, mytv)
            )
        ...
        for i in range(part_count):
            ...
            for format_id, format_data in formats_json.items():
                ...
                clip_url = traverse_obj(
                    data,
                    (("clipsURL", "mp4PlayUrl"), i, {url_or_none}),
                    get_all=False,
                )
                if not clip_url:
        ...
        if len(playlist) == 1:
            ...
            info = {
                ...
                "duration": traverse_obj(
                    vid_data, ("data", "totalDuration", {float_or_none})
                ),
            }

        if mytv:
            publish_time = unified_timestamp(
                self._search_regex(
                    r'publishTime:\s*["\'](\d+-\d+-\d+ \d+:\d+)["\']',
                    webpage,
                    "publish time",
                    fatal=False,
                )
            ...
            publish_time = traverse_obj(
                vid_data, ("tv_application_time", {unified_timestamp})
            )
...
class SohuVIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        return self.url_result(
            urljoin(f"http://{subdomain}.sohu.com/", path), SohuIE
        )
