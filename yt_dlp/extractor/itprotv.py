...
class ITProTVBaseIE(InfoExtractor):
    ...
    def _call_api(self, ep, item_id, webpage):
        return self._download_json(
            ...
            item_id,
            note=f"Fetching {ep} data API",
            headers={"Authorization": f"Bearer {self._fetch_jwt(webpage)}"},
        )[ep]
...
class ITProTVIE(ITProTVBaseIE):
    _VALID_URL = (
        r"https?://app\.itpro\.tv/course/(?P<course>[\w-]+)/(?P<id>[\w-]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        episode_id, course_name = self._match_valid_url(url).group(
            "id", "course"
        )
        ...
        chapter_number, chapter = next(
            (
                (i, topic)
                for i, topic in enumerate(course.get("topics") or [], 1)
        ...
        return {
            ...
            "formats": [
                ...
                for h in (320, 480, 720, 1080)
                if episode.get(f"jwVideo{h}Embed")
            ],
...
class ITProTVCourseIE(ITProTVBaseIE):
    ...
    def _real_extract(self, url):
        ...
        entries = [
            self.url_result(
                urljoin(url, f'{course_id}/{episode["url"]}'),
                ITProTVIE,
                episode["url"],
                episode.get("title"),
