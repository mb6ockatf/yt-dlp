...
class TelegramEmbedIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        channel_id, msg_id = self._match_valid_url(url).group(
            "channel_id", "id"
        )
        ...
        for video in re.findall(
            r'<a class="tgme_widget_message_video_player(?s:.+?)</time>', embed
        ):
            ...
            videos.append(
                {
                    ...
                    "webpage_url": update_url_query(
                        webpage_url, {"single": True}
                    ),
        ...
        if self._yes_playlist(playlist_id, msg_id):
            ...
            return traverse_obj(
                videos, lambda _, x: x["id"] == msg_id, get_all=False
            )
