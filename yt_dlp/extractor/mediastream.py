...
class MediaStreamBaseIE(InfoExtractor):
    ...
    def _extract_mediastream_urls(self, webpage):
        yield from traverse_obj(
            list(self._yield_json_ld(webpage, None, default={})),
            (
                lambda _, v: v["@type"] == "VideoObject",
                ("embedUrl", "contentUrl"),
                {
                    lambda x: (
                        x if re.match(rf"{self._BASE_URL_RE}/\w+", x) else None
                    )
                },
            ),
        )
        ...
        for mobj in re.finditer(
            ...
            video_type = (
                "live-stream"
                if mobj.group("video_type") == "live" or mobj.group("live")
                else "embed"
            )
        ...
        for video_format in player_config["src"]:
            if video_format == "hls":
                ...
                fmts, subs = self._extract_m3u8_formats_and_subtitles(
                    update_url_query(
                        player_config["src"][video_format], filter_dict(params)
                    ),
                    video_id,
                )
                formats.extend(fmts)
