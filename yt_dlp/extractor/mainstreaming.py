...
class MainStreamingIE(InfoExtractor):
        ...
        for entry in playlist_content:
            ...
            yield {
                ...
                "duration": int_or_none(
                    traverse_obj(entry, ("duration", "totalSeconds"))
                ),
    ...
    def _get_webtools_host(host):
        if not host.startswith("webtools"):
            host = (
                "webtools" + ("-" if not host.startswith(".") else "") + host
            )
        return host
    ...
    def _call_api(
        ...
        return self._call_webtools_api(
            host, "/api/v2/" + path, item_id, query, note, fatal
        )

    def _call_webtools_api(
        self,
        host: str,
        path: str,
        item_id: str,
        query=None,
        note="Downloading webtools API JSON",
        fatal=False,
    ):
        # webtools docs: https://webtools.msvdn.net/
        return self._download_json(
            urljoin(self._get_webtools_base_url(host), path),
            item_id,
            query=query,
            note=note,
            fatal=fatal,
    ...
    def _real_extract(self, url):
        ...
        content_info = try_get(
            self._call_api(
                ...
                f"content/{video_id}",
                video_id,
                note="Downloading content info API JSON",
            ),
        ...
        if alternative_content_id:
            self.report_warning(
                f"Ignoring alternative content ID: {alternative_content_id}"
            )
        ...
        if content_type == 20:
            ...
            heartbeat = (
                self._call_api(
                    host,
                    f"heartbeat/{video_id}",
                    video_id,
                    note="Checking stream status",
                )
                or {}
            )
            if heartbeat.get("heartBeatUp") is False:
                self.raise_no_formats(
                    f'MainStreaming said: {heartbeat.get("responseMessage")}',
                    expected=True,
                )
                live_status = "was_live"
        ...
        # Playlist
            ...
            return self.playlist_result(
                self._playlist_entries(
                    host, content_info.get("playlistContents")
                ),
                video_id,
                title,
                description,
        ...
        # Normal video content?
            ...
            formats.append(
                {
                    "url": format_base_url % "original.mp4",
        ...
        return {
            ...
            "subtitles": subtitles,
            "thumbnail": urljoin(
                self._get_webtools_base_url(host), f"image/{video_id}/poster"
            ),
        }
