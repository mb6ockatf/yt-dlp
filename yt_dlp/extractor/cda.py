...
class CDAIE(InfoExtractor):
    ...
    def _perform_login(self, username, password):
        ...
        self._API_HEADERS["User-Agent"] = (
            f"pl.cda 1.0 (version {app_version}; Android {android_version}; {phone_model})"
        )

        cached_bearer = self.cache.load(self._BEARER_CACHE, username) or {}
        if (
            cached_bearer.get("valid_until", 0)
            > dt.datetime.now().timestamp() + 5
        ):
            self._API_HEADERS["Authorization"] = (
                f'Bearer {cached_bearer["token"]}'
            )
            return
        ...
        password_hash = (
            base64.urlsafe_b64encode(
                hmac.new(
                    ...
                    "".join(
                        f"{bytes((bt & 255, )).hex():0>2}"
                        for bt in hashlib.md5(password.encode()).digest()
                    ).encode(),
                    hashlib.sha256,
                ).digest()
            )
        ...
        token_res = self._download_json(
            f"{self._BASE_API_URL}/oauth/token",
            None,
            "Logging in",
            data=b"",
            headers={
                **self._API_HEADERS,
                "Authorization": self._LOGIN_REQUEST_AUTH,
            },
            query={
                "grant_type": "password",
                "login": username,
        ...
        self.cache.store(
            ...
            username,
            {
                ...
                "valid_until": token_res["expires_in"]
                + dt.datetime.now().timestamp(),
            },
        )
        self._API_HEADERS["Authorization"] = (
            f'Bearer {token_res["access_token"]}'
        )
    ...
    def _api_extract(self, video_id):
        meta = self._download_json(
            f"{self._BASE_API_URL}/video/{video_id}",
            video_id,
    ...
    def _web_extract(self, video_id):
        ...
        if re.search(
            r"niedostępn[ey] w(?:&nbsp;|\s+)Twoim kraju\s*<", webpage
        ):
        ...
        def extract_format(page, version):
            ...
            video_quality = next(
                (k for k, v in qualities.items() if v == video_quality),
                video_quality,
            )
            ...
            for quality, cda_quality in qualities.items():
                ...
                video_url = self._download_json(
                    f"https://www.cda.pl/video/{video_id}",
                    video_id,
                    headers={
                    ...
                    errnote=f"Failed to fetch {quality} url",
                    fatal=False,
                )
                if try_get(video_url, lambda x: x["result"]["status"]) == "ok":
                    video_url = try_get(
                        video_url, lambda x: x["result"]["resp"]
                    )
                    info_dict["formats"].append(
                        {
                            "url": video_url,
...
class CDAFolderIE(InfoExtractor):
    _MAX_PAGE_SIZE = 36
    _VALID_URL = (
        r"https?://(?:www\.)?cda\.pl/(?P<channel>\w+)/folder/(?P<id>\d+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        def extract_page_entries(page):
            webpage = self._download_webpage(
                f"https://www.cda.pl/{channel}/folder/{folder_id}/vfilm/{page + 1}",
                folder_id,
            ...
            for video_id in items:
                yield self.url_result(
                    f"https://www.cda.pl/video/{video_id}", CDAIE, video_id
                )
