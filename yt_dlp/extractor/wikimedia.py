...
class WikimediaIE(InfoExtractor):
    ...
    _VALID_URL = (
        r"https?://commons\.wikimedia\.org/wiki/File:(?P<id>[^/#?]+)\.\w+"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        subtitles = {}
        for sub in set(
            re.findall(r'\bsrc\s*=\s*["\'](/w/api[^"]+)["\']', webpage)
        ):
        ...
        return {
            ...
            "url": self._html_search_regex(
                r'<source\s[^>]*\bsrc="([^"]+)"', webpage, "video URL"
            ),
            ...
            "license": self._html_search_regex(
                ...
                get_element_by_class("licensetpl", webpage),
                "license",
                default=None,
            ),
            "uploader": self._html_search_regex(
                r">\s*Author\s*</td>\s*<td\b[^>]*>\s*([^<]+)\s*</td>",
                webpage,
                "video author",
                default=None,
            ),
