...
class ServusIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "description": self._get_description(next_data)
    ...
    def _report_errors(self, video):
        ...
        if not playability_errors:
            ...
            details = traverse_obj(
                video,
                ("playabilityErrorDetails", "FSK_BLOCKED"),
                expected_type=dict,
            )
            message = (
                format_field(
                    "".join(
                        (
                            format_field(
                                details, "minEveningHour", " from %02d:00"
                            ),
                            format_field(
                                details, "maxMorningHour", " to %02d:00"
                            ),
                            format_field(
                                details, "minAge", " (Minimum age %d)"
                            ),
                        )
                    ),
                    None,
                ...
                or "Blocked by FSK with unknown availability"
            )
            ...
            message = (
                format_field(
                    ...
                    "Only available from %s",
                )
                or "Video not yet available with unknown availability"
            )
