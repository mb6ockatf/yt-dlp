    ...
    def _parse_media_info(self, media_info, video_id, fsk):
        ...
        if subtitle_url:
            subtitles["de"] = [
                ...
                {
                    "ext": "vtt",
                    "url": subtitle_url.replace("/ebutt/", "/webvtt/")
                    + ".vtt",
                },
            ]
...
class ARDIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        upload_date = unified_strdate(
            xpath_text(video_node, "./broadcastDate")
        )
...
class ARDBetaMediathekIE(InfoExtractor):
    ...
    _TESTS = [
        {
            ...
            "info_dict": {
                ...
                "channel": "MDR",
                "_old_archive_ids": [
                    "ardbetamediathek Y3JpZDovL21kci5kZS9zZW5kdW5nLzI4MjA0MC80MjIwOTEtNDAyNTM0"
                ],
            },
        },
        {
        ...
        },
        {
            ...
            "info_dict": {
                ...
                "channel": "ARD-Aktuell",
                "_old_archive_ids": [
                    "ardbetamediathek Y3JpZDovL2Rhc2Vyc3RlLmRlL3RhZ2Vzc2NoYXUvZmM4ZDUxMjgtOTE0ZC00Y2MzLTgzNzAtNDZkNGNiZWJkOTll"
                ],
            },
        },
        {
            ...
            "info_dict": {
                ...
                "_old_archive_ids": [
                    "ardbetamediathek N2I2YmM5MzgtNWFlOS00ZGFlLTg2NzMtYzNjM2JlNjk4MDg3"
                ],
            },
        },
        {
            ...
            "info_dict": {
                ...
                "duration": 1759,
                "_old_archive_ids": [
                    "ardbetamediathek Y3JpZDovL3dkci5kZS9CZWl0cmFnLXNvcGhvcmEtOWFkMTc0ZWMtMDA5ZS00ZDEwLWFjYjctMGNmNTdhNzVmNzUz"
                ],
            },
        },
        {
    ...
    def _extract_episode_info(self, title):
        ...
        return traverse_obj(
            ...
            (
                ...,
                {functools.partial(re.match, string=title)},
                {
    ...
    def _real_extract(self, url):
        ...
        if self._get_cookies(self._TOKEN_URL).get("ams"):
            token = self._download_json(
                ...
                "Unable to fetch age verification token",
                fatal=False,
            )
            id_token = traverse_obj(token, ("idToken", {str}))
            decoded_token = traverse_obj(
                id_token, ({jwt_decode_hs256}, {dict})
            )
            user_id = traverse_obj(
                decoded_token, (("user_id", "sub"), {str}), get_all=False
            )
            if not user_id:
                self.report_warning(
                    "Unable to extract token, continuing without authentication"
                )
            else:
                ...
                if decoded_token.get("age_rating") != 18:
                    self.report_warning(
                        "Account is not verified as 18+; video may be unavailable"
                    )
        ...
        old_id = traverse_obj(
            page_data, ("tracking", "atiCustomVars", "contentId", {int})
        )
        if old_id is not None:
            ...
            self.report_warning(
                f"Could not extract contentId{bug_reports_message()}"
            )
            video_id = display_id
            archive_ids = None
        ...
        player_data = traverse_obj(
            ...
            (
                "widgets",
                lambda _, v: v["type"] in ("player_ondemand", "player_live"),
                {dict},
            ),
            get_all=False,
        )
        ...
        media_data = traverse_obj(
            player_data, ("mediaCollection", "embedded", {dict})
        )
        ...
        for stream in traverse_obj(media_data, ("streams", ..., {dict})):
            ...
            for media in traverse_obj(
                stream, ("media", lambda _, v: url_or_none(v["url"]))
            ):
                media_url = media["url"]

                audio_kind = traverse_obj(
                    media, ("audios", 0, "kind", {str}), default=""
                ).replace("standard", "")
                lang_code = (
                    traverse_obj(media, ("audios", 0, "languageCode", {str}))
                    or "deu"
                )
        ...
        for sub in traverse_obj(media_data, ("subtitles", ..., {dict})):
            for sources in traverse_obj(
                sub, ("sources", lambda _, v: url_or_none(v["url"]))
            ):
                subtitles.setdefault(
                    sub.get("languageCode") or "deu", []
                ).append(
                    {
                        "url": sources["url"],
                        "ext": {"webvtt": "vtt", "ebutt": "ttml"}.get(
                            sources.get("kind")
                        ),
                    }
                )

        age_limit = traverse_obj(
            page_data,
            ("fskRating", {lambda x: remove_start(x, "FSK")}, {int_or_none}),
        )
        return {
            "id": video_id,
            "display_id": display_id,
            ...
            **traverse_obj(
                media_data,
                {
                    "chapters": (
                        ...
                        "jumpmarks@all",
                        "chapterArray",
                        lambda _, v: int_or_none(v["chapterTime"]),
                        {
...
class ARDMediathekCollectionIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        playlist_id, display_id, playlist_type, season_number, version = (
        ...
        def call_api(page_num):
            api_path = (
                "compilations/ard"
                if playlist_type == "sammlung"
                else "widgets/ard/asset"
            )
            return self._download_json(
                f"https://api.ardmediathek.de/page-gateway/{api_path}/{playlist_id}",
                playlist_id,
                f"Downloading playlist page {page_num}",
                query={
                    "pageNumber": page_num,
                    ...
                    **(
                        {
                            ...
                            "withOriginalversion": (
                                "true" if version == "OV" else "false"
                            ),
                            "withAudiodescription": (
                                "true" if version == "AD" else "false"
                            ),
                        }
                        if season_number
                        else {}
        ...
        def fetch_page(page_num):
            for item in traverse_obj(
                call_api(page_num), ("teasers", ..., {dict})
            ):
                item_id = traverse_obj(
                    item,
                    ("links", "target", ("urlId", "id")),
                    "id",
                    get_all=False,
                )
                ...
                yield self.url_result(
                    ...
                    ie=(
                        ...
                        if item_mode == "sammlung"
                        else ARDBetaMediathekIE
                    ),
                    **traverse_obj(
                        item,
                        {
        ...
        return self.playlist_result(
            OnDemandPagedList(fetch_page, self._PAGE_SIZE),
            full_id,
            display_id=display_id,
            title=page_data.get("title"),
            description=page_data.get("synopsis"),
        )
