...
class BundestagIE(InfoExtractor):
    ...
    def _bt_extract_share_formats(self, video_id):
        share_data = self._download_json(
            f"{self._SHARE_URL}{video_id}",
            video_id,
            note="Downloading share format JSON",
        )
        if traverse_obj(share_data, ("status", "code", {int})) != 1:
            self.report_warning(
                format_field(
                    share_data,
                    [("status", "message", {str})],
                    "Share API response: %s",
                    default="Unknown Share API Error",
                )
                + bug_reports_message()
            )
            return
    ...
    def _real_extract(self, url):
        ...
        if not formats:
            self.raise_no_formats(
                "Could not find suitable formats", video_id=video_id
            )
