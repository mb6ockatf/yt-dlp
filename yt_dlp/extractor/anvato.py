...
class AnvatoIE(InfoExtractor):
    ...
    def _server_time(self, access_key, video_id):
        return int_or_none(
            traverse_obj(
                self._download_json(
                    f"{self._API_BASE_URL}/server_time",
                    video_id,
                    query={"anvack": access_key},
    ...
    def _get_video_json(self, access_key, video_id, extracted_token):
        ...
        video_data_url = (
            f"{self._API_BASE_URL}/mcp/video/{video_id}?anvack={access_key}"
        )
        server_time = self._server_time(access_key, video_id)
        input_data = (
            f"{server_time}~{md5_text(video_data_url)}~{md5_text(server_time)}"
        )

        auth_secret = bytes(
            aes_encrypt(list(input_data[:64].encode()), list(self._AUTH_KEY))
        )
        query = {
            "X-Anvato-Adst-Auth": base64.b64encode(auth_secret).decode(
                "ascii"
            ),
            "rtyp": "fp",
        }
    ...
    def _get_anvato_videos(self, access_key, video_id, token):
        ...
        for published_url in video_data["published_urls"]:
            ...
            a_format = {
                ...
                "format_id": join_nonempty(
                    "http", published_url.get("cdn_name")
                ).lower(),
                "tbr": tbr or None,
            }
        ...
        return {
            ...
            "thumbnail": video_data.get("src_image_url")
    ...
    def _real_extract(self, url):
        ...
        access_key, video_id = self._match_valid_url(url).group(
            "access_key_or_mcp", "id"
        )
        if access_key not in self._ANVACK_TABLE:
            access_key = (
                self._MCP_TO_ACCESS_KEY_TABLE.get(access_key) or access_key
            )
