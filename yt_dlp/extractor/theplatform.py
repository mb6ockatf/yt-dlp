...
class ThePlatformBaseIE(OnceIE):
    ...
    def _extract_theplatform_smil(
        self, smil_url, video_id, note="Downloading SMIL data"
    ):
    ...
    def _download_theplatform_metadata(self, path, video_id):
        info_url = (
            f"http://link.theplatform.{self._TP_TLD}/s/{path}?format=preview"
        )
    ...
    def _parse_theplatform_metadata(self, info):
        ...
        def extract_site_specific_field(field):
            ...
            return traverse_obj(
                info, lambda k, v: v and k.endswith(f"${field}"), get_all=False
            )
        ...
        return {
            ...
            "categories": traverse_obj(
                ...
                (
                    "categories",
                    lambda _, v: v.get("label") in ("category", None),
                    "name",
                    {str},
                ),
            )
            ...
            "tags": traverse_obj(
                ...
                (
                    "keywords",
                    {lambda x: re.split(r"[;,]\s?", x) if x else None},
                ),
            ),
            "location": extract_site_specific_field("region"),
            "series": extract_site_specific_field("show")
            or extract_site_specific_field("seriesTitle"),
            "season_number": int_or_none(
                extract_site_specific_field("seasonNumber")
            ),
            "episode_number": int_or_none(
                extract_site_specific_field("episodeNumber")
            ),
            "media_type": extract_site_specific_field("programmingType")
            or extract_site_specific_field("type"),
        }
...
class ThePlatformIE(ThePlatformBaseIE, AdobePassIE):
    ...
    def _sign_url(url, sig_key, sig_secret, life=600, include_qs=False):
        ...
        relative_path = re.match(
            r"https?://link\.theplatform\.com/s/([^?]+)", url
        ).group(1)
        clear_text = bytes.fromhex(
            flags + expiration_date + str_to_hex(relative_path)
        )
        checksum = hmac.new(
            sig_key.encode("ascii"), clear_text, hashlib.sha1
        ).hexdigest()
    ...
    def _real_extract(self, url):
        ...
        if "guid" in qs_dict:
            ...
            return self.url_result(
                "http://feed.theplatform.com/f/{}/{}?byGuid={}".format(
        ...
        # Explicitly specified SMIL (see https://github.com/ytdl-org/youtube-dl/issues/7385)
            ...
            smil_url += (
                "?" if "?" not in smil_url else "&" + "formats=m3u,mpeg4"
            )
            ...
            release_url = (
                ...
                or f"http://link.theplatform.com/s/{path}?mbr=true"
            )
        ...
        if not traverse_obj(
            formats, lambda _, v: v["format_id"].startswith("hls")
        ):
            m3u8_url = update_url(
                url, query="mbr=true&manifest=m3u", fragment=None
            )
            urlh = self._request_webpage(
                HEADRequest(m3u8_url),
                video_id,
                "Checking for HLS formats",
...
class ThePlatformFeedIE(ThePlatformBaseIE):
    ...
    _TESTS = [
        {
            ...
            "info_dict": {
                ...
                "duration": 467.0,
                "categories": [
                    "MSNBC/Issues/Democrats",
                    "MSNBC/Issues/Elections/Election 2016",
                ],
                "uploader": "NBCU-NEWS",
            },
        },
    ...
    def _extract_feed_info(
        ...
        main_smil_url = (
            "http://link.theplatform.com/s/%s/media/guid/%d/%s"
            % (provider_id, account_id, entry["guid"])
            if account_id
            else entry.get("plmedia$publicUrl")
        )
        ...
        for item in entry["media$content"]:
            ...
            file_asset_types = (
                item.get("plfile$assetTypes")
                or parse_qs(smil_url)["assetTypes"]
            )
            for asset_type in file_asset_types:
                ...
                cur_formats, cur_subtitles = self._extract_theplatform_smil(
                    update_url_query(main_smil_url or smil_url, query),
                    video_id,
                    f"Downloading SMIL data for {asset_type}",
                )
        ...
        categories = [
            item["media$name"] for item in entry.get("media$categories", [])
        ]

        ret = self._extract_theplatform_metadata(
            f"{provider_id}/{first_video_id}", video_id
        )
