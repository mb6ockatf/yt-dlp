...
class PeerTVIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        video_key = self._html_search_regex(
            r"player\.peer\.tv/js/([a-zA-Z0-9]+)", webpage, "video key"
        )

        js = self._download_webpage(
            f"https://player.peer.tv/js/{video_key}/",
            video_id,
            headers={"Referer": "https://www.peer.tv/"},
            note="Downloading session id",
        )

        session_id = self._search_regex(
            r'["\']session_id["\']:\s*["\']([a-zA-Z0-9]+)["\']',
            js,
            "session id",
        )
        ...
        player_webpage = self._download_webpage(
            ...
            video_id,
            note="Downloading player webpage",
        )
        ...
        m3u8_url = self._parse_json(
            m3u8_url, video_id, transform_source=js_to_json
        )
