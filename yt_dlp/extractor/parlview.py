...
class ParlviewIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        media = self._download_json(self._API_URL % video_id, video_id).get(
            "media"
        )
        timestamp = (
            try_get(
        ...
        if not stream:
            ...
            self.raise_no_formats(
                'Unknown type of stream was detected: "{}"'.format(
        ...
        return {
            ...
            "title": self._html_search_regex(
                r"<h2>([^<]+)<", webpage, "title", fatal=False
            ),
            "formats": formats,
            "duration": int_or_none(media.get("duration")),
            "timestamp": unified_timestamp(
                timestamp.split("/", 1)[1].replace("_", " ")
            ),
