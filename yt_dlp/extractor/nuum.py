...
class NuumBaseIE(InfoExtractor):
    ...
    def _parse_video_data(self, container, extract_formats=True):
        stream = (
            traverse_obj(container, ("media_container_streams", 0, {dict}))
            or {}
        )
        media = traverse_obj(stream, ("stream_media", 0, {dict})) or {}
        media_url = traverse_obj(
            media,
            ("media_meta", ("media_archive_url", "media_url"), {url_or_none}),
            get_all=False,
        )
        ...
        return filter_dict(
            {
                ...
                **traverse_obj(
                    media,
                    {
                        ...
                        "thumbnail": (
                            "media_meta",
                            ("media_preview_archive_url", "media_preview_url"),
                            {url_or_none},
                        ),
                    },
                    get_all=False,
    ...
    def _fetch_page(self, channel_id, tab_type, tab_id, page):
        ...
        for container in traverse_obj(media_containers, (..., {dict})):
            ...
            yield self.url_result(
                f'https://nuum.ru/videos/{metadata["id"]}',
                NuumMediaIE,
                **metadata,
            )
