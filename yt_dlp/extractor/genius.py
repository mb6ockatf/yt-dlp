...
class GeniusIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        display_id, is_article = self._match_valid_url(url).group(
            "id", "article"
        )
        ...
        video_id = traverse_obj(
            ...
            (
                ...
                (
                    ...
                    (
                        "dfp_kv",
                        lambda _, v: v["name"] == "brightcove_video_id",
                        "values",
                        ...,
                    ),
                ),
        ...
        config = self._search_json(
            r"var\s*APP_CONFIG\s*=", webpage, "config", video_id, default={}
        )
...
class GeniusLyricsIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?genius\.com/(?P<id>[^?/#]+)-lyrics(?:[?/#]|$)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        artist = traverse_obj(
            ...
            (
                "songPage",
                "trackingData",
                lambda _, x: x["key"] == "Primary Artist",
                "value",
            ),
            get_all=False,
