...
class WrestleUniverseBaseIE(InfoExtractor):
    ...
    def _TOKEN(self):
        if not self._REAL_TOKEN or not self._TOKEN_EXPIRY:
            token = try_call(
                lambda: self._get_cookies("https://www.wrestle-universe.com/")[
                    "token"
                ].value
            )
    ...
    def _real_initialize(self):
        ...
        self._DEVICE_ID = self._configuration_arg(
            "device_id", [None], ie_key=self._NETRC_MACHINE
        )[0]
    ...
    def _call_api(
        ...
        return self._download_json(
            f"https://{self._API_HOST}/v1/{self._API_PATH}/{video_id}{param}",
            video_id,
            note=f"Downloading {msg} JSON",
            errnote=f"Failed to download {msg} JSON",
            data=data,
            headers=headers,
            query=query,
            fatal=fatal,
    ...
    def _call_encrypted_api(
        self, video_id, param="", msg="API", data={}, query={}, fatal=True
    ):
        ...
        token = base64.b64encode(
            private_key.public_key().export_key("DER")
        ).decode()
        api_json = self._call_api(
            video_id,
            param,
            msg,
            data={
            ...
            metadata = (
                traverse_obj(
                    ...
                    (
                        "props",
                        "pageProps",
                        *variadic(props_keys, (str, bytes, dict, set)),
                        {dict},
                    ),
                )
                or {}
...
class WrestleUniverseVODIE(WrestleUniverseBaseIE):
    ...
    def _real_extract(self, url):
        ...
        video_data = self._call_api(
            video_id, ":watch", "watch", data={"deviceId": self._DEVICE_ID}
        )
        ...
        return {
            ...
            **traverse_obj(
                metadata,
                {
                    ...
                    "chapters": (
                        "videoChapters",
                        lambda _, v: isinstance(v.get("start"), int),
                        {
                            "title": ("displayName", {str}),
