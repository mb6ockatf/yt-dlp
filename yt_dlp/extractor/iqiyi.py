...
class IqiyiIE(InfoExtractor):
    ...
    def get_raw_data(self, tvid, video_id):
        ...
        return self._download_json(
            ...
            video_id,
            transform_source=lambda s: remove_start(s, "var tvInfoJs="),
            query=params,
    ...
    def _real_extract(self, url):
        ...
        for _ in range(5):
            ...
            if raw_data["code"] != "A00000":
                ...
                raise ExtractorError(
                    "Unable to load data. Error code: " + raw_data["code"]
                )
        ...
        title = (
            get_element_by_id("widget-videotitle", webpage)
            or clean_html(
                get_element_by_attribute("class", "mod-play-tit", webpage)
            )
...
class IqIE(InfoExtractor):
    ...
    def _extract_cmd5x_function(self, webpage, video_id):
        return self._search_regex(
            ...
            self._extract_vms_player_js(webpage, video_id),
            "signature function",
        )
    ...
    def _update_bid_tags(self, webpage, video_id):
        ...
        self._BID_TAGS = {
            bid: traverse_obj(
                ...
                (bid, "value"),
                expected_type=str,
                default=self._BID_TAGS.get(bid),
            )
    ...
    def _real_extract(self, url):
        ...
        dash_paths = self._parse_json(
            PhantomJSwrapper(self, timeout=120_000)
            .get(
                url,
                note2="Executing signature code (this may take a couple minutes)",
                html="<!DOCTYPE html>",
                video_id=video_id,
                jscode=self._DASH_JS
                % {
                    ...
                    "bid_list": "["
                    + ",".join(["0", *self._BID_TAGS.keys()])
                    + "]",
                    "ut_list": "[" + ",".join(ut_list) + "]",
                    "cmd5x_func": self._extract_cmd5x_function(
                        webpage, video_id
                    ),
                },
            )[1]
        ...
        initial_format_data = self._download_json(
            urljoin("https://cache-video.iq.com", dash_paths["0"]),
            video_id,
            note="Downloading initial video format info",
            errnote="Unable to download initial video format info",
        )["data"]

        preview_time = traverse_obj(
            initial_format_data,
            ("boss_ts", (None, "data"), ("previewTime", "rtime")),
        ...
        if traverse_obj(
            ...
            self.report_warning(
                "This preview video is limited{}".format(
                    format_field(preview_time, None, " to %s seconds")
                )
            )
        ...
        for bid in set(
            ...
            format_data = traverse_obj(
                self._download_json(
                    urljoin("https://cache-video.iq.com", dash_path),
                    video_id,
            ...
            video_format = (
                traverse_obj(
                    format_data,
                    ("program", "video", lambda _, v: str(v["bid"]) == bid),
                    expected_type=dict,
                    get_all=False,
                )
                or {}
            ...
            if video_format.get("m3u8"):
                ...
                if ff == "ts":
                    ...
                    mpd_data = traverse_obj(
                        self._parse_json(
                            video_format["m3u8"], video_id, fatal=False
                        ),
                        ("payload", ..., "data"),
                        expected_type=str,
                    )
        ...
        for sub_format in traverse_obj(
            ...
            lang = self._LID_TAGS.get(
                str_or_none(sub_format.get("lid")), sub_format.get("_name")
            )
                ...
                [
                    {
                        "ext": format_ext,
                        "url": urljoin(
                            initial_format_data.get(
                                "dstl", "http://meta.video.iqiyi.com"
                            ),
                            sub_format[format_key],
                        ),
                    }
                    for format_key, format_ext in [
                        ("srt", "srt"),
                        ("webvtt", "vtt"),
                    ]
                    if sub_format.get(format_key)
                ]
            )

        extra_metadata = (
            page_data.get("albumInfo")
            if video_info.get("albumId") and page_data.get("albumInfo")
            else video_info
        )
        return {
            "id": video_id,
            ...
            "cast": traverse_obj(
                extra_metadata, ("actorArr", ..., "name"), expected_type=str
            ),
...
class IqAlbumIE(InfoExtractor):
        ...
        for page_range in page_ranges:
            ...
            for video in page["data"]["epg"]:
                yield self.url_result(
                    "https://www.iq.com/play/%s"
                    % (video.get("playLocSuffix") or video["qipuIdStr"]),
                    IqIE.ie_key(),
                    video.get("qipuIdStr"),
                    video.get("name"),
                )
    ...
    def _real_extract(self, url):
        ...
        album_data = next_data["props"]["initialState"]["album"][
            "videoAlbumInfo"
        ]

        if album_data.get("videoType") == "singleVideo":
            return self.url_result(
                f"https://www.iq.com/play/{album_id}", IqIE.ie_key()
            )
