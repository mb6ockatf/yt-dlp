...
class RedGifsBaseInfoExtractor(InfoExtractor):
        ...
        for first_attempt in True, False:
            ...
            try:
                ...
                headers["x-customheader"] = (
                    f"https://www.redgifs.com/watch/{video_id}"
                )
    ...
    def _paged_entries(self, ep, item_id, query, fields):
        ...
        page_fetcher = functools.partial(
            ...
            ep,
            item_id,
            self._prepare_api_query(query, fields),
        )
        return (
            page_fetcher(page)
            if page
            else OnDemandPagedList(page_fetcher, self._PAGE_SIZE)
        )
...
class RedGifsUserIE(RedGifsBaseInfoExtractor):
    ...
    def _real_extract(self, url):
        username, query_str = self._match_valid_url(url).group(
            "username", "query"
        )
