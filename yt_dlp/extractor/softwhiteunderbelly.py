...
class SoftWhiteUnderbellyIE(InfoExtractor):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?softwhiteunderbelly\.com/videos/(?P<id>[\w-]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        if '<div id="watch-unauthorized"' in webpage:
            if self._get_cookies("https://www.softwhiteunderbelly.com").get(
                "_session"
            ):
        ...
        return {
            ...
            "url": VHXEmbedIE._smuggle_referrer(
                embed_url, "https://www.softwhiteunderbelly.com"
            ),
            "id": embed_id,
            ...
            "thumbnail": update_url(
                self._og_search_thumbnail(webpage) or "", query=None
            )
            or None,
        }
