    ...
    def _get_subtitles(self, sub_url, video_id):
        ...
        if subtitle_location:
            enc_subtitles = self._download_webpage(
                ...
                fatal=False,
                headers={"Origin": "https://" + self._BASE},
            )
...
Format: Marked,Start,End,Style,Name,MarginL,MarginR,MarginV,Effect,Text"""
            for current in sub:
                ...
                alignment = self._POS_ALIGN_MAP.get(
                    position_align, 2
                ) + self._LINE_ALIGN_MAP.get(line_align, 0)
                ssa += (
                    os.linesep
    ...
    def _perform_login(self, username, password):
        try:
            access_token = (
                self._download_json(
                    self._API_BASE_URL + "authentication/login",
                    None,
                    "Logging in",
    ...
    def _real_extract(self, url):
        ...
        if not user.get("hasAccess"):
            ...
            self.raise_login_required(
                "This video requires a subscription", method="password"
            )

        token = self._download_json(
            user.get("refreshTokenUrl")
            or (self._PLAYER_BASE_URL + "refresh/token"),
            video_id,
        ...
        links_url = try_get(options, lambda x: x["video"]["url"]) or (
            video_base_url + "link"
        )
        ...
        for _ in range(3):
            ...
            try:
                ...
                if (
                    e.cause.status == 403
                    and error.get("code") == "player-bad-geolocation-country"
                ):
        ...
        video = (
            self._download_json(
                self._API_BASE_URL + f"video/{video_id}",
                video_id,
...
class ADNSeasonIE(ADNBaseIE):
    ...
    def _real_extract(self, url):
        ...
        show = self._download_json(
            f"{self._API_BASE_URL}show/{video_show_slug}/",
            video_show_slug,
        ...
        episodes = self._download_json(
            f"{self._API_BASE_URL}video/show/{show_id}",
            video_show_slug,
