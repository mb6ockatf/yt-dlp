...
class PodchaserIE(InfoExtractor):
    ...
    def _parse_episode(episode, podcast):
        return {
            ...
            "categories": list(
                set(
                    traverse_obj(
                        podcast, (("summary", None), "categories", ..., "text")
                    )
                )
            ),
    ...
    def _real_extract(self, url):
        ...
        podcast = self._call_api(
            f"podcasts/{podcast_id}", episode_id or podcast_id
        )
        if not episode_id:
            return self.playlist_result(
                OnDemandPagedList(
                    functools.partial(self._fetch_page, podcast_id, podcast),
                    self._PAGE_SIZE,
                ),
                str_or_none(podcast.get("id")),
                podcast.get("title"),
