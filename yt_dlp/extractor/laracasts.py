    ...
    def _parse_episode(self, episode):
        ...
        return self.url_result(
            VimeoIE._smuggle_referrer(
                f'https://player.vimeo.com/video/{episode["vimeoId"]}',
                "https://laracasts.com/",
            ),
            VimeoIE,
...
class LaracastsIE(LaracastsBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self._parse_episode(
            self._get_prop_data(url, display_id)["lesson"]
        )
...
class LaracastsPlaylistIE(LaracastsBaseIE):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?laracasts\.com/series/(?P<id>[\w-]+)/?(?:[?#]|$)"
    )
    _TESTS = [
        {
