...
class MotherlessIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if (
            ">The content you are trying to view is for friends only."
            in webpage
        ):
            raise ExtractorError(
                f"Video {video_id} is for friends only", expected=True
            )

        title = self._html_search_regex(
            (
                r'(?s)<div[^>]+\bclass=["\']media-meta-title[^>]+>(.+?)</div>',
                r'id="view-upload-title">\s+([^<]+)<',
            ),
            webpage,
            "title",
        )
        ...
        if not upload_date:
            ...
            if uploaded_ago:
                ...
                upload_date = (
                    dt.datetime.now(dt.timezone.utc) - dt.timedelta(**kwargs)
                ).strftime("%Y%m%d")

        comment_count = len(
            re.findall(
                r"""class\s*=\s*['"]media-comment-contents\b""", webpage
            )
        )
...
class MotherlessPaginatedIE(InfoExtractor):
    ...
    def _extract_entries(self, webpage, base):
        for mobj in re.finditer(
            r'href="[^"]*(?P<href>/[A-F0-9]+)"\s+title="(?P<title>[^"]+)',
            webpage,
        ):
            ...
            if video_id:
                yield self.url_result(
                    video_url, MotherlessIE, video_id, mobj.group("title")
                )
    ...
    def _real_extract(self, url):
        ...
        webpage = self._download_webpage(
            real_url, item_id, "Downloading page 1"
        )
        ...
        def get_page(idx):
            ...
            current_page = (
                webpage
                if not idx
                else self._download_webpage(
                    real_url,
                    item_id,
                    note=f"Downloading page {page}",
                    query={"page": page, **self._EXTRA_QUERY},
                )
            )
        ...
        return self.playlist_result(
            ...
            item_id,
            remove_end(
                self._html_extract_title(webpage), " | MOTHERLESS.COM ™"
            ),
        )
...
class MotherlessUploaderIE(MotherlessPaginatedIE):
    _VALID_URL = (
        r"https?://(?:www\.)?motherless\.com/u/(?P<id>\w+)/?(?:$|[?#])"
    )
    _TESTS = [
        {
