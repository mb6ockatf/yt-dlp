...
class GameJoltBaseIE(InfoExtractor):
    ...
    def _call_api(self, endpoint, *args, **kwargs):
        ...
        return self._download_json(self._API_BASE + endpoint, *args, **kwargs)[
            "payload"
        ]
    ...
    def _parse_content_as_text(self, content):
        ...
        for outer_content in outer_contents:
            if outer_content.get("type") != "paragraph":
                joined_contents.append(
                    self._parse_content_as_text(outer_content)
                )
                continue
            inner_contents, inner_content_text = (
                outer_content.get("content") or [],
                "",
            )
    ...
    def _get_comments(self, post_num_id, post_hash_id):
        sort_by, scroll_id = (
            self._configuration_arg(
                "comment_sort", ["hot"], ie_key=GameJoltIE.ie_key()
            )[0],
            -1,
        )
        ...
        for page in itertools.count(1):
            comments_data = self._call_api(
                ...
                % (
                    post_num_id,
                    sort_by,
                    "scroll_id" if is_scrolled else "page",
                    scroll_id if is_scrolled else page,
                ),
                post_hash_id,
                note=f"Downloading comments list page {page}",
            )
            ...
            for comment in traverse_obj(
                ...
                yield {
                    ...
                    "text": self._parse_content_as_text(
                        self._parse_json(
                            comment["comment_content"], post_hash_id
                        )
                    ),
                    "timestamp": int_or_none(
                        comment.get("posted_on"), scale=1000
                    ),
            ...
            scroll_id = int_or_none(
                comments_data["comments"][-1].get("posted_on")
            )
    ...
    def _parse_post(self, post_data):
        ...
        lead_content = (
            self._parse_json(
                post_data.get("lead_content") or "{}", post_id, fatal=False
            )
            or {}
        )
        description, full_description = (
            post_data.get("leadStr")
            or self._parse_content_as_text(
                self._parse_json(post_data.get("lead_content"), post_id)
            ),
            None,
        )
        if post_data.get("has_article"):
            article_content = self._parse_json(
                ...
                or self._call_api(
                    f'web/posts/article/{post_data.get("id", post_id)}',
                    post_id,
                    note="Downloading article metadata",
        ...
        info_dict = {
            ...
            "extractor": "GameJolt",
            "webpage_url": str_or_none(post_data.get("url"))
            or f"https://gamejolt.com/p/{post_id}",
            "id": post_id,
            "title": description,
            ...
            "uploader_url": format_field(
                user_data, "url", "https://gamejolt.com%s"
            ),
            "categories": [
                try_get(
                    category,
        ...
        video_data = (
            traverse_obj(
                post_data, ("videos", ...), expected_type=dict, get_all=False
            )
            or {}
        )
        formats, subtitles, thumbnails = [], {}, []
        for media in video_data.get("media") or []:
            media_url, mimetype, ext, media_id = (
                media["img_url"],
                media.get("filetype", ""),
                ...
                formats.append(
                    {
                        ...
                        "filesize": media.get("filesize"),
                        "acodec": (
                            "none" if "video-card" in media_url else None
                        ),
                    }
                )
        ...
        for media in post_data.get("media", []):
            if determine_ext(
                media["img_url"]
            ) != "gif" or "gif" not in media.get("filetype", ""):
                continue
            gif_entries.append(
                {
                    "id": media["hash"],
...
class GameJoltIE(GameJoltBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?gamejolt\.com/p/(?:[\w-]*-)?(?P<id>\w{8})"
    )
    _TESTS = [
        {
            # No audio
        ...
        },
        {
            ...
            "info_dict": {
                ...
                "categories": [
                    "Game Dev - General",
                    "Arts n' Crafts - Creations",
                    "Pixel Art - showcase",
                    "Friday Night Funkin' - Mods",
                    "Newgrounds - Friday Night Funkin (13+)",
                ],
        ...
        },
        {
            ...
            "info_dict": {
                ...
                "categories": [
                    "Five Nights At The AGK Studio Comunity - NEWS game"
                ],
    ...
    def _real_extract(self, url):
        ...
        post_data = self._call_api(f"web/posts/view/{post_id}", post_id)[
            "post"
        ]
...
class GameJoltPostListBaseIE(GameJoltBaseIE):
    def _entries(
        ...
        endpoint,
        list_id,
        note="Downloading post list",
        errnote="Unable to download post list",
        initial_items=[],
    ):
        page_num, scroll_id = 1, None
        items = (
            initial_items
            or self._call_api(endpoint, list_id, note=note, errnote=errnote)[
                "items"
            ]
        )
        while items:
...
class GameJoltUserIE(GameJoltPostListBaseIE):
    ...
    def _real_extract(self, url):
        ...
        user_data = self._call_api(
            f"web/profile/@{user_id}",
            user_id,
            note="Downloading user info",
            errnote="Unable to download user info",
        )["user"]
        bio = self._parse_content_as_text(
            self._parse_json(
                user_data.get("bio_content", "{}"), user_id, fatal=False
            )
            or {}
        )
        return self.playlist_result(
            self._entries(
                f"web/posts/fetch/user/@{user_id}?tab=active",
                user_id,
                "Downloading user posts",
                "Unable to download user posts",
            ),
            str_or_none(user_data.get("id")),
            user_data.get("display_name") or user_data.get("name"),
            bio,
        )
...
class GameJoltGameIE(GameJoltPostListBaseIE):
    ...
    def _real_extract(self, url):
        ...
        game_data = self._call_api(
            f"web/discover/games/{game_id}",
            game_id,
            note="Downloading game info",
            errnote="Unable to download game info",
        )["game"]
        description = self._parse_content_as_text(
            self._parse_json(
                game_data.get("description_content", "{}"),
                game_id,
                fatal=False,
            )
            or {}
        ...
        return self.playlist_result(
            self._entries(
                f"web/posts/fetch/game/{game_id}",
                game_id,
                "Downloading game posts",
                "Unable to download game posts",
            ),
            game_id,
            game_data.get("title"),
            description,
...
class GameJoltGameSoundtrackIE(GameJoltBaseIE):
    ...
    def _real_extract(self, url):
        ...
        game_overview = self._call_api(
            f"web/discover/games/overview/{game_id}",
            game_id,
            note="Downloading soundtrack info",
            errnote="Unable to download soundtrack info",
        )
        return self.playlist_result(
            [
                {
                    ...
                    "release_timestamp": int_or_none(
                        song.get("posted_on"), scale=1000
                    ),
                }
                for song in game_overview.get("songs") or []
            ],
            game_id,
            traverse_obj(
                game_overview,
...
class GameJoltCommunityIE(GameJoltPostListBaseIE):
    ...
    def _real_extract(self, url):
        display_id, community_id, channel_id, sort_by = self._match_valid_url(
            url
        ).group("id", "community", "channel", "sort")
        channel_id, sort_by = channel_id or "featured", sort_by or "new"

        community_data = self._call_api(
            f"web/communities/view/{community_id}",
            display_id,
            note="Downloading community info",
            errnote="Unable to download community info",
        )["community"]
        channel_data = (
            traverse_obj(
                self._call_api(
                    f"web/communities/view-channel/{community_id}/{channel_id}",
                    display_id,
...
class GameJoltSearchIE(GameJoltPostListBaseIE):
    ...
    def _search_entries(self, query, filter_mode, display_query):
        initial_search_data = self._call_api(
            f"web/search/{filter_mode}?q={query}",
            display_query,
            note=f"Downloading {filter_mode} list",
            errnote=f"Unable to download {filter_mode} list",
        )
        entries_num = traverse_obj(
            initial_search_data, "count", f"{filter_mode}Count"
        )
        if not entries_num:
            return
        for page in range(
            1, math.ceil(entries_num / initial_search_data["perPage"]) + 1
        ):
            search_results = self._call_api(
                f"web/search/{filter_mode}?q={query}&page={page}",
                display_query,
                note=f"Downloading {filter_mode} list page {page}",
                errnote=f"Unable to download {filter_mode} list",
            )
            for result in search_results[filter_mode]:
                yield self.url_result(
                    self._URL_FORMATS[filter_mode].format(**result)
                )
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            (
                self._search_entries(query, filter_mode, display_query)
                if filter_mode
                else self._entries(
                    f"web/posts/fetch/search/{query}",
                    display_query,
                    initial_items=self._call_api(
                        f"web/search?q={query}",
                        display_query,
                        note="Downloading initial post list",
                        errnote="Unable to download initial post list",
                    )["posts"],
                )
            ),
            display_query,
