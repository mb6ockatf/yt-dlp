...
class KinjaEmbedIE(InfoExtractor):
    ...
    _EMBED_REGEX = [
        rf'(?x)<iframe[^>]+?src=(?P<q>["\'])(?P<url>(?:(?:https?:)?//{_DOMAIN_REGEX})?{_COMMON_REGEX}(?:(?!\1).)+)\1'
    ]
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        if video_type == "kinjavideo":
            ...
            if poster_id:
                thumbnail = "https://i.kinja-img.com/gawker-media/image/upload/{}.{}".format(
            ...
            return {
                ...
                "timestamp": int_or_none(
                    try_get(
                        data, lambda x: x["postInfo"]["publishTimeMillis"]
                    ),
                    1000,
                ),
                "thumbnail": thumbnail,
            ...
            video_data = self._download_json(
                "https://api.vmh.univision.com/metadata/v1/content/"
                + video_id,
                video_id,
            )["videoMetadata"]
            ...
            return {
                ...
                "title": title,
                "thumbnail": try_get(
                    iptc, lambda x: x["cloudinaryLink"]["link"], str
                ),
                "uploader": fmg.get("network"),
                "duration": int_or_none(iptc.get("fileDuration")),
                "formats": formats,
                "description": try_get(
                    iptc, lambda x: x["description"]["en"], str
                ),
