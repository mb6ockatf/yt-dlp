...
class AENetworksBaseIE(ThePlatformIE):  # XXX: Do not subclass from concrete IE
    ...
    def _extract_aen_smil(self, smil_url, video_id, auth=None):
        ...
        for q in TP_SMIL_QUERY:
            ...
            m_url = self._sign_url(
                m_url, self._THEPLATFORM_KEY, self._THEPLATFORM_SECRET
            )
            try:
                tp_formats, tp_subtitles = self._extract_theplatform_smil(
                    m_url,
                    video_id,
                    "Downloading %s SMIL data"
    ...
    def _extract_aetn_info(self, domain, filter_key, filter_value, url):
        ...
        result = traverse_obj(
            result,
            ("results", lambda k, v: k == 0 and v[filter_key] == filter_value),
            get_all=False,
        )
        if not result:
        ...
        if theplatform_metadata.get("AETN$isBehindWall"):
            resource = self._get_mvpd_resource(
                requestor_id,
                theplatform_metadata["title"],
                theplatform_metadata.get("AETN$PPL_pplProgramId")
        ...
        info.update(
            {
                ...
                "episode_number": int_or_none(
                    result.get("tvSeasonEpisodeNumber")
                ),
            }
        )
        return info
...
class AENetworksIE(AENetworksBaseIE):
    ...
    _VALID_URL = (
        AENetworksBaseIE._BASE_URL_REGEX
        + r"""(?P<id>
  ...
  %s(slug: "%s") {
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            entries,
            playlist.get("id"),
            playlist.get(self._PLAYLIST_TITLE_KEY),
            description,
        )
...
class HistoryTopicIE(AENetworksBaseIE):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?history\.com/topics/[^/]+/(?P<id>[\w+-]+?)-video"
    )
    _TESTS = [
        {
