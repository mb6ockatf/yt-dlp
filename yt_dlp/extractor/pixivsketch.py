...
class PixivSketchBaseIE(InfoExtractor):
    def _call_api(
        self, video_id, path, referer, note="Downloading JSON metadata"
    ):
...
class PixivSketchIE(PixivSketchBaseIE):
    ...
    def _real_extract(self, url):
        video_id, uploader_id = self._match_valid_url(url).group(
            "id", "uploader_id"
        )
        ...
        return {
            ...
            "uploader": traverse_obj(
                data, ("user", "name"), ("owner", "user", "name")
            ),
            "uploader_id": traverse_obj(
                data, ("user", "unique_name"), ("owner", "user", "unique_name")
            ),
            "channel_id": str(
                traverse_obj(
                    data,
                    ("user", "pixiv_user_id"),
                    ("owner", "user", "pixiv_user_id"),
                )
            ),
            "age_limit": (
                18 if data.get("is_r18") else 15 if data.get("is_r15") else 0
            ),
