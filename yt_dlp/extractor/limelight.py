...
class LimelightBaseIE(InfoExtractor):
    ...
    def _call_playlist_service(
        self, item_id, method, fatal=True, referer=None
    ):
        headers = {}
        if referer:
        ...
        try:
            return self._download_json(
                ...
                % (self._PLAYLIST_SERVICE_PATH, item_id, method),
                item_id,
            ...
            if isinstance(e.cause, HTTPError) and e.cause.status == 403:
                error = self._parse_json(
                    e.cause.response.read().decode(), item_id
    ...
    def _extract_info(self, pc, mobile, i, referer):
        ...
        for stream in pc_item.get("streams", []):
            ...
            if not stream_url or stream_url in urls:
                continue
            if not self.get_param("allow_unplayable_formats") and stream.get(
                "drmProtected"
            ):
                continue
        ...
        for flag in mobile_item.get("flags"):
            if flag == "ClosedCaptions":
                closed_captions = (
                    self._call_playlist_service(
                        video_id,
                        "getClosedCaptionsDetailsByMediaId",
                        False,
                        referer,
                    )
                    or []
        ...
        return {
            ...
            "duration": float_or_none(
                get_meta("durationInMilliseconds"), 1000
            ),
            "thumbnail": get_meta("previewImageUrl")
            or get_meta("thumbnailImageUrl"),
            "subtitles": subtitles,
        }
...
class LimelightChannelListIE(LimelightBaseIE):
    ...
    def _real_extract(self, url):
        ...
        entries = [
            self.url_result(
                "limelight:channel:{}".format(channel["id"]),
                "LimelightChannel",
            )
