...
class LeFigaroVideoEmbedIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        player_data = self._search_nextjs_data(webpage, display_id)["props"][
            "pageProps"
        ]["initialProps"]["pageData"]["playerData"]

        return self.url_result(
            f'jwplatform:{player_data["videoId"]}',
...
class LeFigaroVideoSectionIE(InfoExtractor):
    ...
    def _get_api_response(self, display_id, page_num, note=None):
        return self._download_json(
            "https://api-graphql.lefigaro.fr/graphql",
            display_id,
            note=note,
            query={
    ...
    def _real_extract(self, url):
        display_id = self._match_id(url)
        initial_response = self._get_api_response(display_id, page_num=1)[
            "data"
        ]["playlist"]
        ...
        def page_func(page_num):
            ...
            return [
                ...
                for video in api_response["data"]["playlist"]["jsonLd"][0][
                    "itemListElement"
                ]
            ]
        ...
        entries = InAdvancePagedList(
            page_func,
            math.ceil(initial_response["videoCount"] / self._PAGE_SIZE),
            self._PAGE_SIZE,
        )
