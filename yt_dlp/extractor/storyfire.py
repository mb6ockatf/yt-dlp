    ...
    def _parse_video(self, video):
        ...
        return {
            ...
            "uploader_url": format_field(
                uploader_id, None, "https://storyfire.com/user/%s/video"
            ),
...
class StoryFireIE(StoryFireBaseIE):
    _VALID_URL = (
        StoryFireBaseIE._VALID_URL_BASE + r"video-details/(?P<id>[0-9a-f]{24})"
    )
    _TEST = {
    ...
    def _real_extract(self, url):
        ...
        video = self._call_api("generic/video-detail", video_id, "video")[
            "video"
        ]
...
class StoryFireSeriesIE(StoryFireBaseIE):
    _VALID_URL = (
        StoryFireBaseIE._VALID_URL_BASE
