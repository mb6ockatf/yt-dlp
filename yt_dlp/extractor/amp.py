    ...
    def _extract_feed_info(self, url):
        ...
        if not item:
            raise ExtractorError(
                "{} said: {}".format(self.IE_NAME, feed["error"])
            )
        ...
        def get_media_node(name, default=None):
            ...
            return (
                media_group.get(media_name)
                or item.get(media_name)
                or item.get(name, default)
            )
                ...
                subtitles.setdefault(subtitle.get("lang") or "en", []).append(
                    {
                        ...
                        or determine_ext(subtitle_href),
                    }
                )
        ...
        for media_data in media_content:
            ...
            if ext == "f4m":
                ...
                formats.append(
                    {
                        "format_id": media_data.get("media-category", {})
                        .get("@attributes", {})
                        .get("label"),
                        "url": media_url,
        ...
        timestamp = unified_timestamp(
            item.get("pubDate"), " "
        ) or parse_iso8601(item.get("dc-date"))
        ...
        return {
            ...
            "duration": int_or_none(
                media_content[0].get("@attributes", {}).get("duration")
            ),
            "subtitles": subtitles,
            "formats": formats,
        }
