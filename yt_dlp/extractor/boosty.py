...
class BoostyIE(InfoExtractor):
    ...
    def _extract_formats(self, player_urls, video_id):
        ...
        for player_url in traverse_obj(
            player_urls, lambda _, v: url_or_none(v["url"])
        ):
            url = player_url["url"]
            ...
            if format_type in (
                ...
                formats.extend(
                    self._extract_m3u8_formats(
                        url, video_id, m3u8_id="hls", fatal=False
                    )
                )
    ...
    def _real_extract(self, url):
        ...
        if auth_cookie is not None:
            try:
                ...
                auth_headers["Authorization"] = (
                    f'Bearer {auth_data["accessToken"]}'
                )
        ...
        if not entries and not post.get("hasAccess"):
            self.raise_login_required(
                "This post requires a subscription", metadata_available=True
            )
        elif not entries:
