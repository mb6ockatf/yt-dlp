    ...
    def _real_initialize(self):
        if not self._OTV_USER_ID:
            self.raise_login_required(
                f"Login required. {self._LOGIN_HINT}", method=None
            )
...
class DangalPlayIE(DangalPlayBaseIE):
    ...
    def _real_extract(self, url):
        series_slug, episode_slug = self._match_valid_url(url).group(
            "series", "id"
        )
        ...
        try:
            details = self._download_json(
                f"{self._API_BASE}/v2/users/get_all_details.gzip",
                episode_slug,
                "Downloading playback details JSON",
                headers={
                    "Accept": "application/json",
                    "Content-Type": "application/json",
                },
                data=self._generate_api_data(metadata),
            )["data"]
            ...
            if isinstance(e.cause, HTTPError) and e.cause.status == 422:
                error_info = (
                    traverse_obj(
                        ...
                        ({json.loads}, "error", {dict}),
                    )
                    or {}
                )
                if error_info.get("code") == "1016":
                    self.raise_login_required(
                        f"Your token has expired or is invalid. {self._LOGIN_HINT}",
                        method=None,
                    )
        ...
        m3u8_url = traverse_obj(
            ...
            (
                (
                    "adaptive_url",
                    ("adaptive_urls", "hd", "hls", ..., "playback_url"),
                ),
                {url_or_none},
                any,
            ),
        ...
        for subcategory in subcategories:
            ...
            for ep in traverse_obj(
                ...
                yield self.url_result(
                    ...
                    DangalPlayIE,
                    **self._extract_episode_info(
                        ep, episode_slug, series_slug
                    ),
                )

    def _real_extract(self, url):
        series_slug, subcategory = self._match_valid_url(url).group(
            "id", "sub"
        )
        subcategories = (
            [subcategory]
            if subcategory
                ...
                self._call_api(
                    f"catalogs/shows/items/{series_slug}.gzip",
                    series_slug,
                    "Downloading season info JSON",
                    query={"item_language": ""},
                ),
        ...
        return self.playlist_result(
            self._entries(subcategories, series_slug),
            join_nonempty(series_slug, subcategory),
        )
