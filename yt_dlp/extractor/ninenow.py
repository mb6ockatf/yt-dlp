...
class NineNowIE(InfoExtractor):
    IE_NAME = "9now.com.au"
    _VALID_URL = (
        r"https?://(?:www\.)?9now\.com\.au/(?:[^/]+/){2}(?P<id>[^/?#]+)"
    )
    _GEO_COUNTRIES = ["AU"]
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        for kind in ("episode", "clip"):
            ...
            common_data = {
                "episode": (
                    cache.get(current_key) or next(iter(cache.values()))
                )[kind],
                "season": (
                    cache.get(current_key) or next(iter(cache.values()))
                ).get("season", None),
        ...
        if not self.get_param("allow_unplayable_formats") and try_get(
            common_data, lambda x: x["episode"]["video"]["drm"], bool
        ):
            self.report_drm(display_id)
        brightcove_id = try_get(
            common_data, lambda x: x["episode"]["video"]["brightcoveId"], str
        ) or "ref:{}".format(common_data["episode"]["video"]["referenceId"])
        video_id = (
            str_or_none(
                try_get(common_data, lambda x: x["episode"]["video"]["id"])
            )
            or brightcove_id
        )
        ...
        season_number = try_get(
            common_data, lambda x: x["season"]["seasonNumber"], int
        )
        episode_number = try_get(
            common_data, lambda x: x["episode"]["episodeNumber"], int
        )
        timestamp = unified_timestamp(
            try_get(common_data, lambda x: x["episode"]["airDate"], str)
        )
        release_date = unified_strdate(
            try_get(common_data, lambda x: x["episode"]["availability"], str)
        )
        thumbnails_data = (
            try_get(
                common_data, lambda x: x["episode"]["image"]["sizes"], dict
            )
            or {}
        )
        thumbnails = [
        ...
        return {
            ...
            "title": title,
            "description": try_get(
                common_data, lambda x: x["episode"]["description"], str
            ),
            "duration": float_or_none(
                try_get(
                    common_data,
                    lambda x: x["episode"]["video"]["duration"],
                    float,
                ),
                1000,
            ),
