...
class VimeoBaseInfoExtractor(InfoExtractor):
    ...
    def _verify_video_password(self, video_id, password, token):
        ...
        try:
            ...
            if (
                isinstance(error.cause, HTTPError)
                and error.cause.status == 418
            ):
    ...
    def _parse_config(self, config, video_id):
        ...
        for files_type in ("hls", "dash"):
            for cdn_name, cdn_data in (
                try_get(config_files, lambda x: x[files_type]["cdns"]) or {}
            ).items():
                ...
                if re.search(sep_pattern, manifest_url):
                    for suffix, repl in (("", "video"), ("_sep", "sep/video")):
                        sep_manifest_urls.append(
                            (
                                format_id + suffix,
                                re.sub(sep_pattern, f"/{repl}/", manifest_url),
                            )
                        )
                else:
        ...
        if not is_live:
            ...
            thumbnails.extend(
                traverse_obj(
                    video_data,
                    (("thumbnail", "thumbnail_url"), {"url": {url_or_none}}),
                )
            )
        ...
        return {
            ...
            "uploader_id": (
                video_uploader_url.split("/")[-1]
                if video_uploader_url
                else None
            ),
            ...
            "chapters": sorted(
                traverse_obj(
                    ...
                    (
                        "embed",
                        "chapters",
                        lambda _, v: int(v["timecode"]) is not None,
                        {
                            "title": ("title", {str}),
            ...
            "release_timestamp": traverse_obj(
                live_event, ("ingest", "scheduled_start_time", {parse_iso8601})
            ),
    ...
    def _call_videos_api(
        self, video_id, jwt_token, unlisted_hash=None, **kwargs
    ):
    ...
    def _extract_original_format(
        self, url, video_id, unlisted_hash=None, jwt=None, api_data=None
    ):
        ...
        if (
            ...
            if self._is_valid_url(
                download_url, video_id, f"{source_name} video"
            ):
                ext = (
                    try_get(source_file, lambda x: x["extension"], str)
                    or determine_ext(download_url, None)
                    or "mp4"
                ).lower()
                return {
                    "url": download_url,
        ...
        jwt = jwt or traverse_obj(
            self._download_json(
                "https://vimeo.com/_rv/viewer",
                video_id,
        ...
        for download_data in traverse_obj(
            ...
            if not download_url or download_data.get("quality") != "source":
                continue
            ext = determine_ext(
                parse_qs(download_url).get("filename", [""])[0].lower(),
                default_ext=None,
            )
            if not ext:
...
class VimeoIE(VimeoBaseInfoExtractor):
            ...
            "info_dict": {
                ...
                "chapters": [
                    ...
                    },
                    {
                        "start_time": 10,
                        "end_time": 106,
                        "title": "Welcoming Dr Arielle Schwartz",
                    },
                    {
                        "start_time": 106,
                        "end_time": 305,
                    ...
                    },
                    {
                        "start_time": 305,
                        "end_time": 594,
                        "title": "Vagal toning practices",
                    },
                    {
                        "start_time": 594,
                        "end_time": 888,
                        "title": "Trauma and difficulty letting go",
                    },
                    {
                        "start_time": 888,
                        "end_time": 1059,
                    ...
                    },
                    {
                        "start_time": 1059,
                        "end_time": 1471,
                        "title": "A strategy for helping sleep issues",
                    },
                    {
                        "start_time": 1471,
                        "end_time": 1667,
                    ...
                    },
                    {
                        "start_time": 1667,
                        "end_time": 2121,
                        "title": "Wisdom in stillness",
                    },
                    {
                        "start_time": 2121,
                        "end_time": 2386,
                        "title": "What helps us be more able to let go?",
                    },
                    {
                        "start_time": 2386,
                        "end_time": 2510,
                        "title": "Practical tips to help ourselves",
                    },
                    {
                        "start_time": 2510,
                        "end_time": 2636,
                        "title": "Where to find out more",
                    },
                ],
            },
            "params": {
                "http_headers": {
                    "Referer": "https://sleepsuperconference.com"
                },
                "skip_download": "m3u8",
            },
            "expected_warnings": ["Failed to parse XML: not well-formed"],
    ...
    def _extract_from_api(self, video_id, unlisted_hash=None):
        ...
        for retry in (False, True):
            try:
                video = self._call_videos_api(
                    video_id, viewer["jwt"], unlisted_hash
                )
                break
            except ExtractorError as e:
                if (
        ...
        info = self._parse_config(
            self._download_json(video["config_url"], video_id), video_id
        )
        ...
        info.update(
            {
                ...
                "view_count": int_or_none(
                    try_get(video, lambda x: x["stats"]["plays"])
                ),
            }
        )
        connections = (
            try_get(video, lambda x: x["metadata"]["connections"], dict) or {}
        )
        for k in ("comment", "like"):
            info[k + "_count"] = int_or_none(
                try_get(connections, lambda x: x[k + "s"]["total"])
            )
        return info
    ...
    def _real_extract(self, url):
        ...
        try:
            ...
            ) or error.cause.status not in (403, 429):
                raise
            ...
            raise ExtractorError(
                ...
                f'try replacing "https:" with "http:" in the input URL. {dcip_msg}.',
                expected=True,
            )
        ...
        if channel_id:
            ...
            video_description = clean_html(
                get_element_by_class("description", webpage)
            )
            info_dict.update(
                {
        ...
        if not config_url:
            ...
            video_description = clean_html(
                ...
                or page_config.get("description_html_escaped")
            )
        ...
        def is_rented():
            ...
            for purchase_option in vod.get("purchase_options") or []:
                ...
                if label and (
                    label.startswith("You rented this")
        ...
        like_count = int_or_none(
            self._search_regex(
                r"UserLikes:(\d+)", webpage, "like count", default=None
            )
        )
        ...
        info_dict["_format_sort_fields"] = info_dict_config[
            "_format_sort_fields"
        ]
...
class VimeoChannelIE(VimeoBaseInfoExtractor):
    ...
    def _title_and_entries(self, list_id, base_url):
        for pagenum in itertools.count(1):
            ...
            webpage = self._download_webpage(
                page_url, list_id, f"Downloading page {pagenum}"
            )
            ...
            if (
                re.search(self._MORE_PAGES_INDICATOR, webpage, re.DOTALL)
                is None
            ):
                break
    ...
    def _real_extract(self, url):
        ...
        return self._extract_videos(
            channel_id, self._BASE_URL_TEMPL % channel_id
        )
...
class VimeoAlbumIE(VimeoBaseInfoExtractor):
    ...
    def _fetch_page(self, album_id, authorization, hashed_pass, page):
        ...
        for video in videos:
            ...
            video_id = (
                self._search_regex(
                    r"/videos/(\d+)", uri, "video_id", default=None
                )
                if uri
                else None
            )
    ...
    def _real_extract(self, url):
        ...
        entries = OnDemandPagedList(
            functools.partial(self._fetch_page, album_id, jwt, hashed_pass),
            self._PAGE_SIZE,
        )
...
class VimeoGroupsIE(VimeoChannelIE):  # XXX: Do not subclass from concrete IE
    IE_NAME = "vimeo:group"
    _VALID_URL = (
        r"https://vimeo\.com/groups/(?P<id>[^/]+)(?:/(?!videos?/\d+)|$)"
    )
    _TESTS = [
        {
...
class VimeoReviewIE(VimeoBaseInfoExtractor):
    ...
    def _real_extract(self, url):
        user, video_id, review_hash = self._match_valid_url(url).group(
            "user", "id", "hash"
        )
        data_url = (
            f"https://vimeo.com/{user}/review/data/{video_id}/{review_hash}"
        )
        ...
        if data.get("isLocked") is True:
            ...
            self._verify_video_password(
                video_id, video_password, viewer["xsrft"]
            )
            data = self._download_json(data_url, video_id)
        ...
        source_format = self._extract_original_format(
            ...
            video_id,
            unlisted_hash=clip_data.get("unlistedHash"),
            jwt=viewer.get("jwt"),
        )
        if source_format:
...
class VimeoLikesIE(VimeoChannelIE):  # XXX: Do not subclass from concrete IE
    _VALID_URL = (
        r"https://(?:www\.)?vimeo\.com/(?P<id>[^/]+)/likes/?(?:$|[?#]|sort:)"
    )
    IE_NAME = "vimeo:likes"
...
class VHXEmbedIE(VimeoBaseInfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        config_url = self._parse_json(
            self._search_regex(
                r"window\.OTTData\s*=\s*({.+})", webpage, "ott data"
            ),
            video_id,
            js_to_json,
        )["config_url"]
