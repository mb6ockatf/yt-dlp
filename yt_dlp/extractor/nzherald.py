...
class NZHeraldIE(InfoExtractor):
    IE_NAME = "nzherald"
    _VALID_URL = (
        r"https?://(?:www\.)?nzherald\.co\.nz/[\w\/-]+\/(?P<id>[A-Z0-9]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        if not bc_url:
            ...
            if not video_metadata:
                custom_video_id = traverse_obj(
                    fusion_metadata,
                    ("customVideo", "embed", "id"),
                    expected_type=str,
                )
                if custom_video_id:
                    video_metadata = self._download_json(
                        "https://www.nzherald.co.nz/pf/api/v3/content/fetch/full-content-by-id",
                        article_id,
                        query={
                            "query": json.dumps(
                                {"id": custom_video_id, "site": "nzh"}
                            ),
                            "_website": "nzh",
                        },
                    )
            bc_video_id = traverse_obj(
                video_metadata
                or fusion_metadata,  # fusion metadata is the video metadata for video-only pages
                "brightcoveId",
            ...
            if not bc_video_id:
                if (
                    ...
                    raise ExtractorError(
                        "This article does not have a video.", expected=True
                    )
                else:
                    raise ExtractorError(
                        "Failed to extract brightcove video id"
                    )
