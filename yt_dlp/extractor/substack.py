...
class SubstackIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?P<username>[\w-]+)\.substack\.com/p/(?P<id>[\w-]+)"
    )
    _TESTS = [
        {
    ...
    def _extract_embed_urls(cls, url, webpage):
        ...
        if mobj:
            ...
            yield parsed._replace(
                netloc=f'{mobj.group("subdomain")}.substack.com'
            ).geturl()
    ...
    def _real_extract(self, url):
        display_id, username = self._match_valid_url(url).group(
            "id", "username"
        )
        ...
        if domain:
            canonical_url = (
                urllib.parse.urlparse(url)._replace(netloc=domain).geturl()
            )
        ...
        return {
            ...
            "uploader_id": str_or_none(
                traverse_obj(webpage_info, ("post", "publication_id"))
            ),
