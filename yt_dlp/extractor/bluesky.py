...
class BlueskyIE(InfoExtractor):
    ...
    def _get_service_endpoint(self, did, video_id):
        ...
        return (
            traverse_obj(
                ...
                (
                    "service",
                    lambda _, x: x["type"] == "AtprotoPersonalDataServer",
                    "serviceEndpoint",
                    {url_or_none},
                    any,
                ),
    ...
    def _real_extract(self, url):
        ...
        entries.extend(
            self._extract_videos(
                post,
                video_id,
                embed_path=("embed", "media"),
                record_subpath=("embed", "media"),
            )
        )
        # app.bsky.embed.record.view
        if nested_post := traverse_obj(
            post, ("embed", "record", ("record", None), {dict}, any)
        ):
            entries.extend(
                self._extract_videos(
                    nested_post,
        ...
        if not entries:
            raise ExtractorError(
                "No video could be found in this post", expected=True
            )
        if len(entries) == 1:
            return entries[0]
    ...
    def _build_profile_url(path):
        return format_field(
            path, None, "https://bsky.app/profile/%s", default=None
        )
    ...
    def _extract_videos(
        ...
        entries = []
        if external_uri := traverse_obj(
            root,
            (
                ((*record_path, *record_subpath), embed_path),
                "external",
                "uri",
                {url_or_none},
                any,
        ...
        if playlist := traverse_obj(
            root, (*embed_path, "playlist", {url_or_none})
        ):
            ...
            formats.append(
                {
                    ...
                    "url": update_url_query(
                        self._BLOB_URL_TMPL.format(endpoint),
                        {"did": did, "cid": video_cid},
                    ),
                    **traverse_obj(
                        root,
                        (
                ...
                subtitles.setdefault(sub_data.get("lang") or "und", []).append(
                    {
                        "url": update_url_query(
                            ...
                            {
                                "did": did,
                                "cid": sub_data["file"]["ref"]["$link"],
                            },
                        ),
                        "ext": traverse_obj(
                            sub_data, ("file", "mimeType", {mimetype2ext})
                        ),
                    }
                )
                    ...
                    root,
                    {
                        ...
                        "age_limit": (
                            ...
                            "val",
                            {
                                lambda x: (
                                    18
                                    if x in ("sexual", "porn", "graphic-media")
                                    else None
                                )
                            },
                            any,
