...
class NaverBaseIE(InfoExtractor):
    ...
    def process_subtitles(vod_data, process_url):
        ...
        for caption in traverse_obj(vod_data, ("captions", "list", ...)):
            ...
            type_ = (
                "automatic_captions"
                if caption.get("type") == "auto"
                else "subtitles"
            )
            lang = (
                caption.get("locale")
                or join_nonempty("language", "country", from_dict=caption)
                or "und"
            )
            if caption.get("type") == "fan":
                lang += "_fan{}".format(
                    next(
                        ...
                        if f"{lang}_fan{i}" not in ret[type_]
                    )
                )
            ret[type_].setdefault(lang, []).extend(
                {
                    "url": sub_url,
                    "name": join_nonempty(
                        "label", "fanName", from_dict=caption, delim=" - "
                    ),
                }
    ...
    def _extract_video_info(self, video_id, vid, key):
        ...
        formats = []
        get_list = (
            lambda x: try_get(video_data, lambda y: y[x + "s"]["list"], list)
            or []
        )
        ...
        def extract_formats(streams, stream_type, query={}):
            for stream in streams:
                ...
                formats.append(
                    {
                        "format_id": "{}_{}".format(
                            stream.get("type") or stream_type,
                            dict_get(encoding_option, ("name", "id")),
                        ),
                        "url": stream_url,
                        "ext": "mp4",
                        ...
                        "protocol": (
                            "m3u8_native" if stream_type == "HLS" else None
                        ),
            ...
            if videos:
                ...
                formats.extend(
                    self._extract_m3u8_formats(
                        update_url_query(stream_url, query),
                        video_id,
                        "mp4",
                        "m3u8_native",
                        m3u8_id=stream_type,
    ...
    def _call_api(self, path, video_id):
        ...
        md = base64.b64encode(
            hmac.HMAC(
                key,
                f"{api_endpoint[:255]}{msgpad}".encode(),
...
class NaverIE(NaverBaseIE):
    _VALID_URL = (
        r"https?://(?:m\.)?tv(?:cast)?\.naver\.com/(?:v|embed)/(?P<id>\d+)"
    )
    _GEO_BYPASS = False
    _TESTS = [
        {
...
class NaverLiveIE(NaverBaseIE):
    ...
    def _real_extract(self, url):
        video_id = self._match_id(url)
        data = self._call_api(
            f"/live-end/normal/{video_id}/play-info?renewLastPlayDate=true",
            video_id,
        )
        ...
        return {
            ...
            "formats": self._extract_m3u8_formats(
                traverse_obj(
                    data, ("playbackBody", {json.loads}, "media", 0, "path")
                ),
                video_id,
                live=True,
            ),
            **traverse_obj(
                ...
                (
                    "live",
                    {
                        ...
                        "description": "description",
                        "like_count": (
                            ("likeCount", "likeItCount"),
                            {int_or_none},
                        ),
                        "thumbnail": ("thumbnailImageUrl", {url_or_none}),
                        "start_time": (
                            ("startTime", "startDateTime", "startYmdt"),
                            {parse_iso8601},
                        ),
                    },
                ),
...
class NaverNowIE(NaverBaseIE):
    ...
    def _extract_replay(self, show_id, replay_id):
        vod_info = self._download_json(
            f"{self._API_URL}/shows/now.{show_id}/vod/{replay_id}", replay_id
        )
        in_key = self._download_json(
            f"{self._API_URL}/shows/now.{show_id}/vod/{replay_id}/inkey",
            replay_id,
        )["inKey"]
        return merge_dicts(
            {
                ...
                "timestamp": unified_timestamp(
                    traverse_obj(vod_info, ("episode", "start_time"))
                ),
    ...
    def _extract_show_replays(self, show_id):
        ...
        while True:
            show_vod_info = self._download_json(
                f"{self._API_URL}/vod-shows/now.{show_id}",
                show_id,
    ...
    def _extract_show_highlights(self, show_id, highlight_id=None):
        ...
        while True:
            highlights_videos = self._download_json(
                f"{self._API_URL}/shows/now.{show_id}/highlights/videos/",
                show_id,
                query={"page": page, "page_size": page_size},
            ...
            for highlight in highlights_videos.get("results") or []:
                if highlight_id and highlight.get("clip_no") != int(
                    highlight_id
                ):
                    continue
                yield merge_dicts(
    ...
    def _extract_highlight(self, show_id, highlight_id):
        try:
            ...
            raise ExtractorError(
                f"Unable to find highlight {highlight_id} for show {show_id}"
            )
    ...
    def _real_extract(self, url):
        ...
        show_info = self._download_json(
            f"{self._API_URL}/shows/now.{show_id}/",
            show_id,
        ...
        return self.playlist_result(
            itertools.chain(
                self._extract_show_replays(show_id),
                self._extract_show_highlights(show_id),
            ),
            show_id,
            show_info.get("title"),
        )
