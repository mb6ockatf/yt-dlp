...
class YleAreenaIE(InfoExtractor):
    _VALID_URL = (
        r"https?://areena\.yle\.fi/(?P<podcast>podcastit/)?(?P<id>[\d-]+)"
    )
    _GEO_COUNTRIES = ["FI"]
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        video_id, is_podcast = self._match_valid_url(url).group(
            "id", "podcast"
        )
        json_ld = self._search_json_ld(
            self._download_webpage(url, video_id), video_id, default={}
        )
        ...
        season_number, episode_number, episode, series = self._search_regex(
            ...
            "episode metadata",
            group=("season_no", "episode_no", "episode", "series"),
            default=(None, None, None, None),
        )
        description = traverse_obj(
            video_data, ("ongoing_ondemand", "description", "fin", {str})
        )
        ...
        info_dict, metadata = {}, {}
        if is_podcast and traverse_obj(
            video_data, ("ongoing_ondemand", "media_url", {url_or_none})
        ):
            metadata = video_data["ongoing_ondemand"]
            info_dict["url"] = metadata["media_url"]
        elif traverse_obj(
            video_data, ("ongoing_event", "manifest_url", {url_or_none})
        ):
            metadata = video_data["ongoing_event"]
            metadata.pop(
                "duration", None
            )  # Duration is not accurate for livestreams
            info_dict["live_status"] = "is_live"
        elif traverse_obj(
            video_data, ("ongoing_ondemand", "manifest_url", {url_or_none})
        ):
            metadata = video_data["ongoing_ondemand"]
        # XXX: Has all externally-hosted Kaltura content been moved to native hosting?
        elif kaltura_id := traverse_obj(
            video_data, ("ongoing_ondemand", "kaltura", "id", {str})
        ):
            metadata = video_data["ongoing_ondemand"]
            info_dict.update(
                {
                    ...
                    "url": smuggle_url(
                        f"kaltura:1955031:{kaltura_id}", {"source_url": url}
                    ),
