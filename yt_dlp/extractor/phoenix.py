...
class PhoenixIE(ZDFBaseIE):
    ...
    def _real_extract(self, url):
        ...
        article = self._download_json(
            f"https://www.phoenix.de/response/id/{article_id}",
            article_id,
        ...
        duration = int_or_none(
            try_get(
                details,
                lambda x: x["tracking"]["nielsen"]["content"]["length"],
            )
        )
        timestamp = unified_timestamp(details.get("editorialDate"))
        series = try_get(
            details,
            lambda x: x["tracking"]["nielsen"]["content"]["program"],
            str,
        )
        ...
        thumbnails = []
        teaser_images = (
            try_get(details, lambda x: x["teaserImageRef"]["layouts"], dict)
            or {}
        )
