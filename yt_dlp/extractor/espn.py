...
class ESPNIE(OnceIE):
    ...
    def _real_extract(self, url):
        ...
        clip = self._download_json(
            f"http://api-app.espn.com/v1/video/clips/{video_id}", video_id
        )["videos"][0]
...
class ESPNArticleIE(InfoExtractor):
    ...
    def suitable(cls, url):
        return (
            False
            if (ESPNIE.suitable(url) or WatchESPNIE.suitable(url))
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "upload_date": unified_strdate(
                dict_get(data_json, ("publishedAt", "recordedAt"))
            ),
    ...
    def _real_extract(self, url):
        ...
        # TV Provider required
            ...
            resource = self._get_mvpd_resource(
                "ESPN", video_data["name"], video_id, None
            )
            ...
            asset = self._download_json(
                f"https://watch.auth.api.espn.com/video/auth/media/{video_id}/asset?apikey=uiqlbgzdwuru14v627vdusswb",
                video_id,
                data=f"adobeToken={urllib.parse.quote_plus(base64.b64encode(auth))}&drmSupport=HLS".encode(),
            )
            m3u8_url, headers = asset["stream"], {}
