...
class RuutuIE(InfoExtractor):
    ...
    def _extract_embed_urls(cls, url, webpage):
        ...
        settings = try_call(
            lambda: json.loads(
                re.search(
                    r"jQuery\.extend\(Drupal\.settings, ({.+?})\);", webpage
                ).group(1),
                strict=False,
            )
        )
    ...
    def _real_extract(self, url):
        ...
        video_xml = self._download_xml(
            f"{self._API_BASE}/media-xml-cache",
            video_id,
            query={"id": video_id},
        )
                ...
                if child.tag.endswith("Files"):
                    ...
                    auth_video_url = url_or_none(
                        self._download_webpage(
                            f"{self._API_BASE}/auth/access/v2",
                            video_id,
                    ...
                    if ext == "m3u8":
                        ...
                        format_id = (
                            f"{proto}-{label if label else tbr}"
                            if label or tbr
                            else proto
                        )
                        ...
                        width, height = (
                            int_or_none(x)
                            for x in child.get("resolution", "x").split("x")[
                                :2
                            ]
                        )
                        formats.append(
        ...
        if not formats:
            if not self.get_param("allow_unplayable_formats") and xpath_text(
                video_xml, "./Clip/DRM", default=None
            ):
        ...
        return {
            ...
            "duration": int_or_none(
                xpath_text(video_xml, ".//Runtime", "duration")
            )
            or int_or_none(pv("runtime")),
            "age_limit": int_or_none(
                xpath_text(video_xml, ".//AgeLimit", "age limit")
            ),
