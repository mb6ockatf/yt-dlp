...
class FacebookIE(InfoExtractor):
    ...
    def _perform_login(self, username, password):
        ...
        try:
            ...
            if (
                re.search(r'<form(.*)name="login"(.*)</form>', login_results)
                is not None
            ):
            ...
            check_req.headers["Content-Type"] = (
                "application/x-www-form-urlencoded"
            )
            check_response = self._download_webpage(
                check_req, None, note="Confirming login"
            )
            if (
                re.search(r'id="checkpointSubmitButton"', check_response)
                is not None
            ):
                self.report_warning(
    ...
    def _extract_from_url(self, url, video_id):
        ...
        def extract_metadata(webpage):
            ...
            media = traverse_obj(
                ...
                (
                    ...
                    lambda k, v: (
                        k == "media"
                        and str(v["id"]) == video_id
                        and v["__typename"] == "Video"
                    ),
                ),
                expected_type=dict,
            )
            ...
            uploader_data = (
                ...
                or get_first(
                    post,
                    (..., "video", lambda k, v: k == "owner" and v["name"]),
                )
            ...
            info_json_ld["title"] = (
                re.sub(
                    r"\s*\|\s*Facebook$",
                    "",
                    title or info_json_ld.get("title") or page_title or "",
                )
        ...
        def extract_from_jsmods_instances(js_data):
            if js_data:
                return extract_video_data(
                    try_get(js_data, lambda x: x["jsmods"]["instances"], list)
                    or []
                )
                ...
                formats.extend(
                    self._parse_mpd_formats(
                        compat_etree_fromstring(
                            urllib.parse.unquote_plus(dash_manifest)
                        ),
        ...
        def process_formats(info):
            ...
            for f in info["formats"]:
                ...
                f.setdefault("downloader_options", {})["http_chunk_size"] = (
                    250 << 20
                )

        def yield_all_relay_data(_filter):
            for relay_data in re.findall(
                rf"data-sjs>({{.*?{_filter}.*?}})</script>", webpage
            ):
        ...
        def extract_relay_prefetched_data(_filter, target_keys=None):
            ...
            if target_keys is not None:
                path = lambda k, v: k == "data" and any(
                    target in v for target in variadic(target_keys)
                )
            return (
                traverse_obj(
                    yield_all_relay_data(_filter),
                    (
                        ...,
                        "require",
                        (None, (..., ..., ..., "__bbox", "require")),
                        lambda _, v: any(
                            key.startswith("RelayPrefetchedStreamCache")
                            for key in v
                        ),
                        ...,
                        ...,
            ...
            if data:
                ...
                def parse_graphql_video(video):
                    ...
                    if reel_info:
                        ...
                        video["owner"] = traverse_obj(
                            video, ("short_form_video_context", "video_owner")
                        )
                    ...
                    fmt_data = (
                        traverse_obj(
                            video, ("videoDeliveryLegacyFields", {dict})
                        )
                        or video
                    )
                        ...
                        if determine_ext(playable_url) == "mpd":
                            formats.extend(
                                self._extract_mpd_formats(
                                    playable_url, video_id, fatal=False
                                )
                            )
                        else:
                            formats.append(
                    ...
                    dash_manifests = traverse_obj(
                        fmt_data,
                        ("dash_manifests", lambda _, v: v["manifest_xml"]),
                    )
                    for idx, dash_manifest in enumerate(dash_manifests):
                        extract_dash_manifest(
                            dash_manifest,
                            formats,
                    ...
                    for prog_fmt in traverse_obj(
                        fmt_data,
                        (
                            "progressive_urls",
                            lambda _, v: v["progressive_url"],
                        ),
                    ):
                        format_id = traverse_obj(
                            prog_fmt, ("metadata", "quality", {str.lower})
                        )
                        formats.append(
                            {
                                "format_id": format_id,
                    ...
                    is_broadcast = traverse_obj(
                        video, ("is_video_broadcast", {bool})
                    )
                    for caption in traverse_obj(
                        ...
                        if (
                            caption.get("localized_creation_method")
                            or is_broadcast
                        ):
                            automatic_captions.setdefault(
                                caption["locale"], []
                            ).append(subs)
                        else:
                            subtitles.setdefault(caption["locale"], []).append(
                                subs
                            )
                    captions_url = traverse_obj(
                        video, ("captions_url", {url_or_none})
                    )
                    if (
                        captions_url
                        and not automatic_captions
                        and not subtitles
                    ):
                        locale = self._html_search_meta(
                            ["og:locale", "twitter:locale"],
                            webpage,
                            "locale",
                            default="en_US",
                        )
                        (automatic_captions if is_broadcast else subtitles)[
                            locale
                        ] = [{"url": captions_url}]
                    ...
                    if title:
                        ...
                        info["title"] = (
                            description or f"Facebook video #{v_id}"
                        )
                    entries.append(info)
                ...
                attachments = traverse_obj(
                    ...
                    (
                        ...
                        (
                            "styles",
                            "style_type_renderer",
                            ("throwbackStyles", "attachment_target_renderer"),
                        ),
                        "attachment",
                        {dict},
                    ),
                ...
                video = (
                    traverse_obj(
                        data,
                        ("event", "cover_media_renderer", "cover_video"),
                        "video",
                        expected_type=dict,
                    )
                    or {}
        ...
        if not video_data and "/watchparty/" in url:
            ...
            if prefetched_data:
                lsd = try_get(
                    prefetched_data, lambda x: x["login_data"]["lsd"], dict
                )
                if lsd:
            ...
            living_room = self._download_json(
                urljoin(url, self._api_config["graphURI"]),
                video_id,
            ...
            for edge in (
                try_get(
                    living_room,
                    lambda x: x["recap"]["watched_content"]["edges"],
                )
                or []
            ):
        ...
        if not video_data:
            ...
            tahoe_data = self._download_webpage(
                self._VIDEO_PAGE_TAHOE_TEMPLATE % video_id,
                video_id,
...
class FacebookPluginsVideoIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        return self.url_result(
            urllib.parse.unquote(self._match_id(url)), FacebookIE.ie_key()
        )
...
class FacebookRedirectURLIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if not redirect_url:
            raise ExtractorError(
                "Invalid facebook redirect URL", expected=True
            )
...
class FacebookReelIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        video_id = self._match_id(url)
        return self.url_result(
            f"https://m.facebook.com/watch/?v={video_id}&_rdr",
            FacebookIE,
            video_id,
        )
...
class FacebookAdsIE(InfoExtractor):
    ...
    def _extract_formats(self, video_dict):
        formats = []
        for format_key, format_url in traverse_obj(
            video_dict,
            (
                {dict.items},
                lambda _, v: v[0] in self._FORMATS_MAP and url_or_none(v[1]),
            ),
        ):
            formats.append(
                {
    ...
    def _real_extract(self, url):
        ...
        if not title or title == "{{product.name}}":
            title = join_nonempty(
                "display_format", "page_name", delim=" by ", from_dict=data
            )
        ...
        markup = traverse_obj(
            ...
            (
                ...
                "__bbox",
                "markup",
                lambda _, v: v[0].startswith(markup_id),
                ...,
                "__html",
                {clean_html},
                {lambda x: not x.startswith("{{product.") and x},
                any,
            ),
        )
            ...
            traverse_obj(
                data,
                {
                    "description": (
                        ...
                        {
                            lambda x: (
                                x if not x.startswith("{{product.") else None
                            )
                        },
                    ),
        ...
        for idx, entry in enumerate(
            traverse_obj(
                ...
                (
                    ("videos", "cards"),
                    lambda _, v: any(
                        url_or_none(v.get(f)) for f in self._FORMATS_MAP
                    ),
                ),
            ),
            1,
            ...
            entries.append(
                {
                    ...
                    "description": traverse_obj(
                        entry, "body", "link_description"
                    )
                    or info_dict.get("description"),
                    "thumbnail": url_or_none(
                        entry.get("video_preview_image_url")
                    ),
