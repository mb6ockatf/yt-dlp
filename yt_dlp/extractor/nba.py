...
class NBAWatchBaseIE(NBACVPBaseIE):
    _VALID_URL_BASE = (
        r"https?://(?:(?:www\.)?nba\.com(?:/watch)?|watch\.nba\.com)/"
    )
...
class NBAWatchIE(NBAWatchBaseIE):
    IE_NAME = "nba:watch"
    _VALID_URL = (
        NBAWatchBaseIE._VALID_URL_BASE
...
class NBAWatchCollectionIE(NBAWatchBaseIE):
    ...
    _VALID_URL = (
        NBAWatchBaseIE._VALID_URL_BASE + r"list/collection/(?P<id>[^/?#&]+)"
    )
    _TESTS = [
        {
    ...
    def _fetch_page(self, collection_id, page):
        ...
        videos = self._download_json(
            ...
            + collection_id,
            collection_id,
            f"Downloading page {page} JSON metadata",
            query={
    ...
    def _real_extract(self, url):
        ...
        entries = OnDemandPagedList(
            functools.partial(self._fetch_page, collection_id), self._PAGE_SIZE
        )
...
class NBABaseIE(NBACVPBaseIE):
    ...
    def _call_api(self, team, content_id, query, resource):
        return self._download_json(
            ...
            content_id,
            f"Download {resource} JSON metadata",
            query=query,
            headers={
    ...
    def _extract_video(self, video, team, extract_all=True):
        ...
        info = {
            ...
            or video.get("headline")
            or video["shortHeadline"],
            "description": video.get("description"),
        ...
        subtitles = {}
        captions = (
            try_get(video, lambda x: x["videoCaptions"]["sidecars"], dict)
            or {}
        )
        ...
        if extract_all:
            ...
            if (
                source_url
                and not source_url.startswith("s3://")
                and self._is_valid_url(source_url, video_id, "source")
            ):
                formats.append(
                    {
                        "format_id": "source",
...
class NBAEmbedIE(NBABaseIE):
    ...
    def _real_extract(self, url):
        ...
        if not team:
            return self.url_result(
                "https://watch.nba.com/video/" + content_id,
                NBAWatchIE.ie_key(),
            )
        video = self._call_api(
            team, content_id, {"videoid": content_id}, "video"
        )[0]
...
class NBAIE(NBABaseIE):
    IE_NAME = "nba"
    _VALID_URL = (
        NBABaseIE._VALID_URL_BASE
        + f"(?!{NBABaseIE._CHANNEL_PATH_REGEX})video/(?P<id>(?:[^/]+/)*[^/?#&]+)"
    )
    _TESTS = [
        {
...
class NBAChannelIE(NBABaseIE):
    ...
    _VALID_URL = (
        NBABaseIE._VALID_URL_BASE
        + f"(?:{NBABaseIE._CHANNEL_PATH_REGEX})/(?P<id>[^/?#&]+)"
    )
    _TESTS = [
        {
