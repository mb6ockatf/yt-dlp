    ...
    def _extract_formats(self, spl_url, video):
        for ext, pref, streams in (
            (
                "ts",
                -1,
                traverse_obj(video, ("http_stream", "qualities")) or {},
            ),
            ...
            for format_id, stream in streams.items():
                ...
                yield {
                    ...
                    "duration": float_or_none(
                        stream.get("duration"), scale=1000
                    ),
                    "width": traverse_obj(stream, ("resolution", 0)),
                    "height": traverse_obj(stream, ("resolution", 1))
                    or int_or_none(format_id.replace("p", "")),
