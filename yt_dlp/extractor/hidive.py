...
class HiDiveIE(InfoExtractor):
    ...
    def _call_api(self, video_id, title, key, data={}, **kwargs):
        ...
        return (
            self._download_json(
                "https://www.hidive.com/play/settings",
                video_id,
    ...
    def _real_extract(self, url):
        video_id, title, key = self._match_valid_url(url).group(
            "id", "title", "key"
        )
        ...
        for rendition_id, rendition in settings["renditions"].items():
            ...
            m3u8_url = url_or_none(
                try_get(rendition, lambda x: x["bitrates"]["hls"])
            )
        ...
        for rendition_id, rendition in settings["renditions"].items():
            ...
            for cc_file in rendition.get("ccFiles") or []:
                ...
                cc_lang = try_get(
                    cc_file,
                    (lambda x: x[1].replace(" ", "-").lower(), lambda x: x[0]),
                    str,
                )
