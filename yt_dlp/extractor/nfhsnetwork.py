...
class NFHSNetworkIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        broadcast = (publisher.get("broadcasts") or publisher.get("vods"))[
            0
        ]  # some (older) videos don't have a broadcasts object
        ...
        uploader_prefix = (
            ...
            if pub_type == "school"
                ...
                "associations"
                if "association" in pub_type
                else (
                    "affiliates"
                    if (pub_type == "publisher" or pub_type == "affiliate")
                    else "schools"
                )
            )
        )
        uploader_page = "https://www.nfhsnetwork.com/{}/{}".format(
            uploader_prefix, publisher.get("slug")
        )
        location = "{}, {}".format(data.get("city"), data.get("state_name"))
        description = broadcast.get("description")
        is_live = (
            broadcast.get("on_air")
            or broadcast.get("status") == "on_air"
            or False
        )
        ...
        title = self._og_search_title(webpage) or self._html_search_regex(
            r'<h1 class="sr-hidden">(.*?)</h1>', webpage, "title"
        )
        ...
        key = (
            broadcast.get("key")
            if is_live
            else try_get(publisher, lambda x: x["vods"][0]["key"])
        )
        ...
        formats = self._extract_m3u8_formats(
            m3u8_url, video_id, "mp4", live=is_live
        )
