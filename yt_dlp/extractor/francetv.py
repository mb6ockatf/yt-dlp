...
class FranceTVBaseInfoExtractor(InfoExtractor):
    def _make_url_result(self, video_id, url=None):
        ...
        if url:
            full_id = smuggle_url(
                full_id, {"hostname": urllib.parse.urlparse(url).hostname}
            )
...
class FranceTVIE(InfoExtractor):
    ...
    def _extract_video(self, video_id, hostname=None):
        ...
        for device_type, browser in [
            ("desktop", "chrome"),
            ("mobile", "safari"),
        ]:
            dinfo = self._download_json(
                f"https://k7.ftven.fr/videos/{video_id}",
                video_id,
                f"Downloading {device_type} {browser} video JSON",
                query=filter_dict(
                    {
                ...
                fatal=False,
                expected_status=422,
            )  # 422 json gives detailed error code/message
        ...
        for video in traverse_obj(videos, lambda _, v: url_or_none(v["url"])):
            ...
            if token_url := traverse_obj(
                video, ("token", (None, "akamai"), {url_or_none}, any)
            ):
                tokenized_url = traverse_obj(
                    self._download_json(
                        token_url,
                        video_id,
            ...
            if ext == "f4m":
                ...
                for f in traverse_obj(
                    fmts,
                    lambda _, v: v["vcodec"] == "none"
                    and v.get("tbr") is None,
                ):
                    if mobj := re.match(
                        rf"{format_id}-[Aa]udio-\w+-(?P<bitrate>\d+)",
                        f["format_id"],
                    ):
                        f.update(
                            {
        ...
        for f in formats:
            if f.get("acodec") != "none" and f.get("language") in (
                "qtz",
                "qad",
            ):
                f["language_preference"] = -10
                f["format_note"] = "audio description{}".format(
                    format_field(f, "format_note", ", %s")
                )
            ...
            formats.append(
                {
                    ...
                    "fragments": [
                        ...
                        for sheet in traverse_obj(
                            spritesheets, (..., {url_or_none})
                        )
                    ],
                }
            )
...
class FranceTVSiteIE(FranceTVBaseInfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        nextjs_data = traverse_obj(
            re.findall(
                r"<script[^>]*>\s*self\.__next_f\.push\(\s*(\[.+?\])\s*\);?\s*</script>",
                webpage,
            ),
            (
                ...,
        ...
        if traverse_obj(
            nextjs_data, (..., ..., "children", ..., "isLive", {bool}, any)
        ):
...
class FranceTVInfoIE(FranceTVBaseInfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        dailymotion_urls = tuple(
            DailymotionIE._extract_embed_urls(url, webpage)
        )
        if dailymotion_urls:
