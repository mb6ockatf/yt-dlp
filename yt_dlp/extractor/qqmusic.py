...
class QQMusicBaseIE(InfoExtractor):
    def _get_cookie(self, key, default=None):
        return getattr(
            self._get_cookies("https://y.qq.com").get(key), "value", default
        )
...
class QQMusicIE(QQMusicBaseIE):
    ...
    def _real_extract(self, url):
        ...
        data = self._make_fcu_req(
            ...
            mid,
            note="Downloading formats and lyric",
        ...
        for media_info in traverse_obj(
            ...
            (
                ...
                "data",
                "midurlinfo",
                lambda _, v: v["songmid"] == mid and v["purl"],
            ),
        ):
            ...
            formats.append(
                {
                    "url": urljoin(
                        "https://dl.stream.qqmusic.qq.com", media_info["purl"]
                    ),
                    "format": format_id,
                    "format_id": format_id,
                    "size": traverse_obj(
                        info_data, ("file", f"size_{format_id}", {int_or_none})
                    ),
        ...
        if lrc_content:
            info_dict["subtitles"] = {
                "origin": [{"ext": "lrc", "data": lrc_content}]
            }
...
class QQMusicSingerIE(QQMusicBaseIE):
            ...
            (
                ...
                ...,
                {
                    lambda x: self.url_result(
                        f'https://y.qq.com/n/ryqq/songDetail/{x["mid"]}',
                        QQMusicIE,
                        x["mid"],
                        x.get("title"),
                    )
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            OnDemandPagedList(
                functools.partial(self._fetch_page, mid, self._PAGE_SIZE),
                self._PAGE_SIZE,
            ),
            mid,
            **traverse_obj(
...
class QQMusicAlbumIE(QQPlaylistBaseIE):
    ...
    def _real_extract(self, url):
        ...
        album_json = self._download_json(
            ...
            mid,
            "Download album page",
            query={"albummid": mid, "format": "json"},
        )["data"]
...
class QQMusicToplistIE(QQPlaylistBaseIE):
    ...
    def _real_extract(self, url):
        ...
        toplist_json = self._download_json(
            "http://i.y.qq.com/v8/fcg-bin/fcg_v8_toplist_cp.fcg",
            list_id,
        ...
        return self.playlist_result(
            ...
            playlist_title=join_nonempty(
                *traverse_obj(
                    toplist_json,
                    ((("topinfo", "ListName"), "update_time"), None),
                ),
                delim=" ",
            ),
            playlist_description=traverse_obj(
                toplist_json, ("topinfo", "info")
            ),
        )
...
class QQMusicVideoIE(QQMusicBaseIE):
    ...
    def _real_extract(self, url):
        ...
        if (
            traverse_obj(
                video_info, ("mvInfo", "data", video_id, "play_forbid_reason")
            )
            == 3
        ):
        ...
        return {
            "id": video_id,
            "formats": self._parse_url_formats(
                traverse_obj(video_info, ("mvUrl", "data", video_id))
            ),
            **traverse_obj(
                video_info,
                (
