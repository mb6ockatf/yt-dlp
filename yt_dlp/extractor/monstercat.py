...
class MonstercatIE(InfoExtractor):
    ...
    def _extract_tracks(self, table, album_meta):
        for td in re.findall(
            r"<tr[^<]*>((?:(?!</tr>)[\w\W])+)", table
        ):  # regex by chatgpt due to lack of get_elements_by_tag
            title = traverse_obj(
                td,
                (
                    {find_element(cls="d-inline-flex flex-column")},
                    {lambda x: x.partition(" <span")},
                    0,
                    {clean_html},
                ),
            )
            ids = (
                traverse_obj(
                    ...
                    (
                        {
                            find_element(
                                cls="btn-play cursor-pointer mr-small",
                                html=True,
                            )
                        },
                        {extract_attributes},
            ...
            track_number = traverse_obj(
                td, ({find_element(cls="py-xsmall")}, {int_or_none})
            )
            if not track_id or not release_id:
                self.report_warning(
                    f"Skipping track {track_number}, ID(s) not found"
                )
                self.write_debug(
                    f"release_id={release_id!r} track_id={track_id!r}"
                )
                continue
            yield {
                **album_meta,
                ...
                "artists": traverse_obj(
                    td,
                    (
                        {find_element(cls="d-block fs-xxsmall")},
                        {clean_html},
                        all,
                    ),
                ),
    ...
    def _real_extract(self, url):
        ...
        tracklist_table = (
            traverse_obj(html, {find_element(cls="table table-small")}) or ""
        )
        ...
        album_meta = {
            ...
            "album_artists": traverse_obj(
                ...
                (
                    {
                        find_element(
                            cls="h-normal text-uppercase mb-desktop-medium mb-smallish"
                        )
                    },
                    {clean_html},
                    all,
            ...
            "release_date": traverse_obj(
                html,
                (
                    {
                        find_element(
                            cls="font-italic mb-medium d-tablet-none d-phone-block"
                        )
                    },
                    {lambda x: x.partition("Released ")},
                    2,
                    {strip_or_none},
                    {unified_strdate},
                ),
        ...
        return self.playlist_result(
            self._extract_tracks(tracklist_table, album_meta),
            playlist_id=url_id,
            **album_meta,
        )
