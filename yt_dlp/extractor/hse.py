    ...
    def _extract_formats_and_subtitles(self, sources, video_id):
        if not sources:
            raise ExtractorError(
                "No video found", expected=True, video_id=video_id
            )
        formats, subtitles = [], {}
        for src in sources:
...
class HSEShowIE(HSEShowBaseInfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        formats, subtitles = self._extract_formats_and_subtitles(
            traverse_obj(json_data, ("tvShowPage", "tvShowVideo", "sources")),
            video_id,
        )
        ...
        return {
            ...
            "timestamp": unified_timestamp(
                f'{show.get("date")} {show.get("hour")}:00'
            ),
...
class HSEProductIE(HSEShowBaseInfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        video = (
            traverse_obj(
                json_data, ("productContent", "productContent", "videos", 0)
            )
            or {}
        )
        formats, subtitles = self._extract_formats_and_subtitles(
        ...
        return {
            "id": video_id,
            "title": traverse_obj(
                json_data, ("productDetail", "product", "name", "short")
            )
            or video_id,
            "formats": formats,
            "subtitles": subtitles,
            ...
            "uploader": traverse_obj(
                json_data, ("productDetail", "product", "brand", "brandName")
            ),
        }
