    ...
    def _real_extract(self, url):
        ...
        if not traverse_obj(
            ...
            raise ExtractorError(
                f"{video_id} is not a video or animated image", expected=True
            )
        webpage = (
            self._download_webpage(
                f"https://i.imgur.com/{video_id}.gifv", video_id, fatal=False
            )
            or ""
        )
        formats = []
        ...
        if media_url:
            if not media_fmt.get("ext"):
                media_fmt["ext"] = mimetype2ext(
                    traverse_obj(data, ("media", 0, "mime_type"))
                ...
                return {
                    p: int_or_none(
                        self._og_search_property(
                            f"{media_type}:{p}", webpage, default=None
                        )
                    )
        ...
        search = functools.partial(
            self._html_search_meta, html=webpage, default=None
        )
        ...
        return {
            ...
            **traverse_obj(
                data,
                {
                    "uploader_id": (
                        "account_id",
                        {lambda a: str(a) if int_or_none(a) else None},
                    ),
                    "uploader": (
                        "account",
                        "username",
                        {lambda x: strip_or_none(x) or None},
                    ),
            ...
            **traverse_obj(
            ...
            "thumbnails": (
                ...
                if (
                    thumbnail_url := search(
                        ["thumbnailUrl", "twitter:image", "og:image"]
                    )
                )
                else None
            ),
...
class ImgurGalleryBaseIE(ImgurBaseIE):
    ...
    def _real_extract(self, url):
        ...
        data = self._call_api(
            "albums", gallery_id, fatal=False, expected_status=404
        )
        ...
        if traverse_obj(data, "is_album"):
            ...
            items = traverse_obj(
                ...
                (
                    "media",
                    lambda _, v: v.get("type") == "video"
                    or v["metadata"]["is_animated"],
                    "id",
...
class ImgurAlbumIE(ImgurGalleryBaseIE):
    IE_NAME = "imgur:album"
    _VALID_URL = (
        r"https?://(?:i\.)?imgur\.com/a/(?:[^/?#]+-)?(?P<id>[a-zA-Z0-9]+)"
    )
    _GALLERY = False
    _TESTS = [
        {
