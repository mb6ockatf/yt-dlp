...
class RheinMainTVIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        source, img = self._search_regex(
            r"(?s)(?P<source><source[^>]*>)(?P<img><img[^>]*>)",
            webpage,
            "video",
            group=("source", "img"),
        )
        ...
        ism_manifest_url = source.get("src") or next(
            json_ld.get("embedUrl")
            for json_ld in raw_json_ld
