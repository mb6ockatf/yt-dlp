...
class LikeeIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        info = self._parse_json(
            self._search_regex(
                r"window\.data\s=\s({.+?});", webpage, "video info"
            ),
            video_id,
        ...
        return {
            ...
            "duration": int_or_none(
                traverse_obj(info, ("option_data", "dur"))
            ),
            "formats": formats,
        }
...
class LikeeUserIE(InfoExtractor):
        ...
        while True:
            ...
            for item in items:
                ...
                yield self.url_result(
                    f"https://likee.video/{user_name}/video/{last_post_id}"
                )
    ...
    def _real_extract(self, url):
        ...
        info = self._parse_json(
            self._search_regex(
                r"window\.data\s*=\s*({.+?});", webpage, "user info"
            ),
            user_name,
            transform_source=js_to_json,
        )
        ...
        return self.playlist_result(
            self._entries(user_name, user_id),
            user_id,
            traverse_obj(info, ("userinfo", "user_name")),
        )
