...
class SCTEBaseIE(InfoExtractor):
    ...
    def _perform_login(self, username, password):
        ...
        def is_logged(webpage):
            return any(
                re.search(p, webpage)
                for p in (r'class=["\']welcome\b', r">Sign Out<")
            )
...
class SCTEIE(SCTEBaseIE):
    _WORKING = False
    _VALID_URL = (
        r"https?://learning\.scte\.org/mod/scorm/view\.php?.*?\bid=(?P<id>\d+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        context = decode_packed_codes(
            self._download_webpage(f"{content_base}mobile/data.js", video_id)
        )

        data = self._parse_xml(
            self._search_regex(
                r'CreateData\(\s*"(.+?)"', context, "data"
            ).replace(r"\'", "'"),
            video_id,
        )
