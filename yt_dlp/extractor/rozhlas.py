...
class RozhlasIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?prehravac\.rozhlas\.cz/audio/(?P<id>[0-9]+)"
    )
    _TESTS = [
        {
...
class RozhlasBaseIE(InfoExtractor):
    def _extract_formats(self, entry, audio_id):
        ...
        for audio in traverse_obj(
            entry, ("audioLinks", lambda _, v: url_or_none(v["url"]))
        ):
...
class RozhlasVltavaIE(RozhlasBaseIE):
    _VALID_URL = (
        r"https?://(?:\w+\.rozhlas|english\.radio)\.cz/[\w-]+-(?P<id>\d+)"
    )
    _TESTS = [
        {
    ...
    def _extract_video(self, entry):
        ...
        return {
            "id": audio_id,
            "chapter": (
                traverse_obj(entry, ("meta", "ga", "contentNameShort"))
                if chapter_number
                else None
            ),
...
class MujRozhlasIE(RozhlasBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?mujrozhlas\.cz/(?:[^/]+/)*(?P<id>[^/?#&]+)"
    )
    _TESTS = [
        {
    ...
    def _call_api(self, path, item_id, msg="API JSON"):
        return self._download_json(
            ...
            errnote=f"Failed to download {msg}",
        )["data"]
    ...
    def _entries(self, api_url, playlist_id):
        for page in itertools.count(1):
            episodes = self._download_json(
                api_url,
                playlist_id,
                note=f"Downloading episodes page {page}",
                errnote=f"Failed to download episodes page {page}",
                fatal=False,
            )
    ...
    def _real_extract(self, url):
        ...
        info = self._search_json(
            r"\bvar\s+dl\s*=", webpage, "info json", display_id
        )

        entity = info["siteEntityBundle"]

        if entity in ("episode", "serialPart"):
            return self._extract_audio_entry(
                self._call_api(
            ...
            playlist_id = (
                info["contentShow"].split(":")[0]
                if entity == "show"
                else info["contentId"]
            )
            data = self._call_api(
                f"{entity}s", playlist_id, f"{entity} playlist JSON"
            )
            api_url = data["relationships"]["episodes"]["links"]["related"]
            return self.playlist_result(
                self._entries(api_url, playlist_id),
                playlist_id,
                **traverse_obj(
                    data,
                    (
