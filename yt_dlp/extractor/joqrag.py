...
class JoqrAgIE(InfoExtractor):
    ...
    _VALID_URL = [
        r"https?://www\.uniqueradio\.jp/agplayer5/(?:player|inc-player-hls)\.php",
    ...
    def _extract_start_timestamp(self, video_id, is_live):
        def extract_start_time_from(date_str):
            ...
            start_time = self._search_regex(
                ...
                self._download_webpage(
                    f"https://www.joqr.co.jp/qr/agdailyprogram/?date={date}",
                    video_id,
            ...
            if start_time:
                return unified_timestamp(
                    f'{dt_.strftime("%Y/%m/%d")} {start_time} +09:00'
                )
            return None
        ...
        if (
            not is_live
            or start_timestamp < datetime_from_str("now").timestamp()
        ):
    ...
    def _real_extract(self, url):
        ...
        metadata = self._download_webpage(
            "https://www.uniqueradio.jp/aandg",
            video_id,
            note="Downloading metadata",
            errnote="Failed to download metadata",
        )
        ...
        if not title or title == "放送休止":
            ...
            if release_timestamp:
                msg += " and will start at " + dt.datetime.fromtimestamp(
                    release_timestamp
            ...
            m3u8_path = self._search_regex(
                ...
                self._download_webpage(
                    ...
                    video_id,
                    note="Downloading player data",
                    errnote="Failed to download player data",
                ),
                "m3u8 url",
            )
