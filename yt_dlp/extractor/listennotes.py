...
class ListenNotesIE(InfoExtractor):
    ...
    def _clean_description(self, description):
        return clean_html(
            re.sub(r"(</?(div|p)>\s*)+", "<br/><br/>", description or "")
        )
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "title": (
                data.get("data-title")
                or traverse_obj(
                    webpage, ({find_element(tag="h1")}, {clean_html})
                )
                or self._html_search_meta(
                    ("og:title", "title", "twitter:title"), webpage, "title"
                )
            ),
            "description": (
            ...
            "duration": parse_duration(
                traverse_obj(data, "audio_length", "data-duration") or duration
            ),
