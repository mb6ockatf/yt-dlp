...
class VRTBaseIE(GigyaBaseIE):
    ...
    def _extract_formats_and_subtitles(self, data, video_id):
        ...
        for target in traverse_obj(
            data,
            ("targetUrls", lambda _, v: url_or_none(v["url"]) and v["type"]),
        ):
        ...
        for sub in traverse_obj(
            data,
            ("subtitleUrls", lambda _, v: v["url"] and v["type"] == "CLOSED"),
        ):
...
class VRTIE(VRTBaseIE):
    ...
    def _real_extract(self, url):
        ...
        attrs = extract_attributes(
            get_element_html_by_class("vrtvideo", webpage) or ""
        )
...
class VrtNUIE(VRTBaseIE):
    IE_DESC = "VRT MAX"
    _VALID_URL = (
        r"https?://(?:www\.)?vrt\.be/vrtnu/a-z/(?:[^/]+/){2}(?P<id>[^/?#&]+)"
    )
    _TESTS = [
        {
    ...
    def _perform_login(self, username, password):
        ...
        if auth_info.get("errorDetails"):
            raise ExtractorError(
                f'Unable to login. VrtNU said: {auth_info["errorDetails"]}',
                expected=True,
            )
        ...
        for retry in self.RetryManager():
            ...
            try:
                self._request_webpage(
                    "https://token.vrt.be/vrtnuinitlogin",
                    None,
                    note="Requesting XSRF Token",
                    errnote="Could not get XSRF Token",
                    query={
                        "provider": "site",
                    ...
                    data=urlencode_postdata(
                        {
                            ...
                            "signatureTimestamp": auth_info[
                                "signatureTimestamp"
                            ],
                            "_csrf": self._get_cookies("https://login.vrt.be")
                            .get("OIDCXSRF")
                            .value,
                        }
                    ),
    ...
    def _real_extract(self, url):
        ...
        watch_info = (
            traverse_obj(
                details,
                ("actions", lambda _, v: v["type"] == "watch-episode", {dict}),
                get_all=False,
            )
            or {}
        )
        video_id = join_nonempty(
            "episodePublicationId",
            "episodeVideoId",
            delim="$",
            from_dict=watch_info,
        )
        if "$" not in video_id:
        ...
        vrtnutoken = (
            self._download_json(
                "https://token.vrt.be/refreshtoken",
                video_id,
                note="Retrieving vrtnutoken",
                errnote="Token refresh failed",
            )["vrtnutoken"]
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "description": clean_html(
                get_element_by_class("dish-description", webpage)
            )
...
class Radio1BeIE(VRTBaseIE):
    ...
    def _real_extract(self, url):
        display_id = self._match_id(url)
        webpage = self._download_webpage(url, display_id)
        next_js_data = self._search_nextjs_data(webpage, display_id)["props"][
            "pageProps"
        ]["item"]

        return self.playlist_result(
            self._extract_video_entries(next_js_data, display_id),
            **merge_dicts(
                traverse_obj(
                    next_js_data,
                    (
