...
class KukuluLiveIE(InfoExtractor):
    ...
    def _get_quality_meta(self, video_id, desc, code, force_h264=None):
        ...
        qs = self._download_webpage(
            "https://live.erinn.biz/live.player.fplayer.php",
            video_id,
            f"Downloading {desc} quality metadata",
            f"Unable to download {desc} quality metadata",
            query=filter_dict(
                {
                    "hash": video_id,
    ...
    def _real_extract(self, url):
        ...
        title = clean_html(
            get_element_by_id(
                "livetitle",
                html.replace("<SPAN", "<span").replace("SPAN>", "span>"),
            )
        )
        ...
        if self._search_regex(
            ...
            for desc, code in [("high", "Z"), ("low", "ForceLow")]:
                ...
                if (
                    desc == "high"
                    and traverse_obj(quality_meta, ("vcodec", 0)) == "HEVC"
                ):
                    self._add_quality_formats(
                        formats,
                        self._get_quality_meta(
            ...
            for i, segment in enumerate(segments, 1):
                yield {
                    ...
                    "timestamp": traverse_obj(
                        segment, ("time_start", {int_or_none})
                    ),
                    "thumbnail": thumbnail,
                    "formats": [
                        {
                            "url": urljoin(
                                "https://live.erinn.biz", segment["file"]
                            ),
                            "ext": "mp4",
        ...
        return self.playlist_result(
            entries(sources), video_id, title, description, multi_video=True
        )
