...
class SVTPlayBaseIE(SVTBaseIE):
    _SVTPLAY_RE = (
        r'root\s*\[\s*(["\'])_*svtplay\1\s*\]\s*=\s*(?P<json>{.+?})\s*;\s*\n'
    )
...
class SVTPlayIE(SVTPlayBaseIE):
    ...
    def _real_extract(self, url):
        ...
        if data:
            video_info = try_get(
                data,
                lambda x: x["context"]["dispatcher"]["stores"][
                    "VideoTitlePageStore"
                ]["data"]["video"],
                dict,
            )
            if video_info:
                ...
                info_dict.update(
                    {
                        "title": data["context"]["dispatcher"]["stores"][
                            "MetaStore"
                        ]["title"],
                        "thumbnail": thumbnail,
                    }
            ...
            svt_id = try_get(
                data,
                lambda x: x["statistics"]["dataLake"]["content"]["id"],
                str,
            )
...
class SVTSeriesIE(SVTPlayBaseIE):
    ...
    def suitable(cls, url):
        return (
            False
            if SVTIE.suitable(url) or SVTPlayIE.suitable(url)
...
class SVTPageIE(SVTBaseIE):
    ...
    def _real_extract(self, url):
        ...
        data = (
            traverse_obj(
                urql_state, (..., "data", {str}, {json.loads}), get_all=False
            )
            or {}
        )
        ...
        def entries():
            for video_id in set(
                ...
                info = self._extract_video(
                    self._download_json(
                        f"https://api.svt.se/video/{video_id}", video_id
                    ),
                    video_id,
                )
                info["title"] = title
