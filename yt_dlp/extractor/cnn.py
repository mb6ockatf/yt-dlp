...
class CNNIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        entries = []
        for player_data in traverse_obj(
            webpage,
            (
                {
                    find_elements(
            ...
            if parent_uri:
                ...
                for direct_url in traverse_obj(
                    ...
                    if mobj := re.search(
                        r"-(?P<res>\d+x\d+)_(?P<tbr>\d+)k\.mp4", direct_url
                    ):
                ...
                for sub_data in traverse_obj(
                    ...
                    (
                        "closedCaptions",
                        "types",
                        lambda _, v: url_or_none(v["track"]["url"]),
                        "track",
                    ),
                ):
                    subtitles.setdefault(
                        sub_data.get("lang") or "en", []
                    ).append(
                        {
                            "url": sub_data["url"],
...
class CNNIndonesiaIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        upload_date, video_id, display_id = self._match_valid_url(url).group(
            "upload_date", "id", "display_id"
        )
        webpage = self._download_webpage(url, display_id)
