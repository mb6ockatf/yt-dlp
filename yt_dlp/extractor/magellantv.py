...
class MagellanTVIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        context = self._search_nextjs_data(webpage, video_id)["props"][
            "pageProps"
        ]["reactContext"]
        data = traverse_obj(
            context,
            ((("video", "detail"), ("series", "currentEpisode")), {dict}, any),
        )
        ...
        for m3u8_url in set(
            traverse_obj(
                data,
                ((("manifests", ..., "hls"), "jwp_video_url"), {url_or_none}),
            )
        ):
        ...
        if not formats and (
            error := traverse_obj(
                context, ("errorDetailPage", "errorMessage", {str})
            )
        ):
