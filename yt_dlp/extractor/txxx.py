...
def get_formats(host, video_file):
    return [
        {
            "url": urljoin(
                f"https://{host}", decode_base64(video["video_url"])
            ),
            "format_id": try_call(
                lambda: variadic(video["format"])[0].lstrip("_")
            ),
            "quality": index,
        }
...
class TxxxIE(InfoExtractor):
    ...
    """
    _EMBED_REGEX = [
        rf'<iframe[^>]+?src=(["\'])(?P<url>(?:https?:)?//(?:www\.)?(?:{"|".join(map(re.escape, _DOMAINS))})/embed/[^"\']*)\1'
    ]
    _TESTS = [
        {
    ...
    def _call_api(self, url, video_id, fatal=False, **kwargs):
        ...
        if traverse_obj(content, "error"):
            raise self._error_or_warning(
                ExtractorError(
                    f'Txxx said: {content["error"]}', expected=True
                ),
                fatal=fatal,
            )
        return content or {}
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "uploader": traverse_obj(
                video_info, ("video", "user", "username")
            ),
            "duration": parse_duration(
                traverse_obj(video_info, ("video", "duration"))
            ),
            "view_count": int_or_none(
                traverse_obj(video_info, ("video", "statistics", "viewed"))
            ),
            "like_count": int_or_none(
                traverse_obj(video_info, ("video", "statistics", "likes"))
            ),
            "dislike_count": int_or_none(
                traverse_obj(video_info, ("video", "statistics", "dislikes"))
            ),
            "age_limit": 18,
            "thumbnail": traverse_obj(
                video_info, ("video", "thumbsrc", {url_or_none})
            ),
    ...
    def _real_extract(self, url):
        ...
        json_ld = self._json_ld(
            self._search_json(
                ...
                contains_pattern="{[^<]+?VideoObject[^<]+};",
            ),
            video_id,
            fatal=True,
        )

        video_file = self._parse_json(
            decode_base64(
                self._search_regex(
