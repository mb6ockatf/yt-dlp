...
class Zee5IE(InfoExtractor):
    ...
    def _perform_login(self, username, password):
        if (
            ...
            and self._USER_TOKEN is None
        ):
            self.report_login()
            otp_request_json = self._download_json(
                f"https://b2bapi.zee5.com/device/sendotp_v1.php?phoneno=91{username}",
                None,
                note="Sending OTP",
            )
        ...
        elif username.lower() == "token" and try_call(
    ...
    def _real_extract(self, url):
        ...
        json_data = self._download_json(
            ...
            query={
                ...
                or self.get_param("geo_bypass_country")
                or "IN",
        ...
        if "premium" in asset_data["business_type"]:
            raise ExtractorError(
                "Premium content is DRM protected.", expected=True
            )
        if not asset_data.get("hls_url"):
            self.raise_login_required(
                self._LOGIN_HINT, metadata_available=True, method=None
            )
        formats, m3u8_subs = self._extract_m3u8_formats_and_subtitles(
            asset_data["hls_url"], video_id, "mp4", fatal=False
        )
        ...
        return {
            ...
            "season_number": int_or_none(
                try_get(show_data, lambda x: x["seasons"][0]["orderid"])
            ),
            "episode_number": int_or_none(
                try_get(asset_data, lambda x: x["orderid"])
            ),
...
class Zee5SeriesIE(InfoExtractor):
    ...
    def _entries(self, show_id):
        ...
        show_json = self._download_json(
            show_url, video_id=show_id, headers=headers
        )
        for season in show_json.get("seasons") or []:
            ...
            while next_url:
                ...
                for episode in (
                    try_get(episodes_json, lambda x: x["episode"], list) or []
                ):
