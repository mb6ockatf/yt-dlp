...
class AppleConnectIE(InfoExtractor):
    _VALID_URL = (
        r"https?://itunes\.apple\.com/\w{0,2}/?post/(?:id)?sa\.(?P<id>[\w-]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        try:
            ...
            raise ExtractorError(
                "This post doesn't contain a video", expected=True
            )

        video_data = self._parse_json(video_json, video_id)
        timestamp = str_to_int(
            self._html_search_regex(
                r'data-timestamp="(\d+)"', webpage, "timestamp"
            )
        )
        like_count = str_to_int(
            self._html_search_regex(
                r"(\d+) Loves", webpage, "like count", default=None
            )
        )
