...
class NhkBaseIE(InfoExtractor):
    ...
    _BASE_URL_REGEX = (
        r"https?://www3\.nhk\.or\.jp/nhkworld/(?P<lang>[a-z]{2})/"
    )
    ...
    def _call_api(self, m_id, lang, is_video, is_episode, is_clip):
        return (
            self._download_json(
                ...
                m_id,
                query={"apikey": "EJfK8jdS57GqlupFgAfAAwr573q01y6k"},
            )["data"]["episodes"]
            or []
        )
    ...
    def _get_api_info(self, refresh=True):
        ...
        movie_player_js = self._download_webpage(
            "https://movie-a.nhk.or.jp/world/player/js/movie-player.js",
            None,
    ...
    def _extract_episode_info(self, url, episode=None):
        ...
        lang, m_type, episode_id = NhkVodIE._match_valid_url(url).group(
            "lang", "type", "id"
        )
...
class NhkVodProgramIE(NhkBaseIE):
    ...
    def _real_extract(self, url):
        ...
        def entries():
            for episode in episodes:
                if episode_path := episode.get("url"):
                    yield self._extract_episode_info(
        ...
        return self.playlist_result(
            entries(), program_id, program_title, program_description
        )
...
class NhkForSchoolBangumiIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        program_values = {
            g.group(1): g.group(3)
            for g in re.finditer(
                r'(?:program|clip)Obj\.([a-zA-Z_]+)\s*=\s*(["\'])([^"]+?)\2;',
                webpage,
            )
        }
        # extract all chapters
        chapter_durations = [
            parse_duration(g.group(1))
            for g in re.finditer(
                r"chapterTime\.push\(\'([0-9:]+?)\'\);", webpage
            )
        ]
        chapter_titles = [
            " ".join([g.group(1) or "", unescapeHTML(g.group(2))]).strip()
            for g in re.finditer(
        ...
        if (
            chapter_durations
            and chapter_titles
            and len(chapter_durations) == len(chapter_titles)
        ):
...
class NhkForSchoolSubjectIE(InfoExtractor):
    ...
    _VALID_URL = (
        r"https?://www\.nhk\.or\.jp/school/(?P<id>{})/?(?:[\?#].*)?$".format(
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_from_matches(
            re.finditer(
                rf'href="((?:https?://www\.nhk\.or\.jp)?/school/{re.escape(subject_id)}/[^/]+/)"',
                webpage,
            ),
            subject_id,
...
class NhkForSchoolProgramListIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        title = self._generic_title("", webpage) or self._html_search_regex(
            r"<h3>([^<]+?)とは？\s*</h3>", webpage, "title", fatal=False
        )
        title = (
            re.sub(r"\s*\|\s*NHK\s+for\s+School\s*$", "", title)
            if title
            else None
        )
        ...
        bangumi_list = self._download_json(
            f"https://www.nhk.or.jp/school/{program_id}/meta/program.json",
            program_id,
        )
        # they're always bangumi
        bangumis = [
            self.url_result(
                f"https://www2.nhk.or.jp/school/movie/bangumi.cgi?das_id={x}"
            )
            for x in traverse_obj(
                bangumi_list, ("part", ..., "part-video-dasid")
            )
            or []
        ]
...
class NhkRadiruIE(InfoExtractor):
    ...
    _TESTS = [
        ...
        },
        {
            ...
            "expected_warnings": [
                "Failed to get extended metadata. API returned empty list."
            ],
        },
    ]
    ...
    def _extract_extended_metadata(self, episode_id, aa_vinfo):
        ...
        detail_url = try_call(
            lambda: self._API_URL_TMPL.format(
                area=area, service=service, dateid=aa_vinfo[3]
            )
        )
        if not detail_url:
            return {}
        ...
        station = (
            " ".join(
                traverse_obj(full_meta, (("service", "area"), "name", {str}))
            )
            or None
        )
        thumbnails = [
            {
                ...
                "preference": (
                    1
                    if id_.startswith("thumbnail")
                    else -2 if id_.startswith("logo") else -1
                ),
                **traverse_obj(
                    thumb,
                    {
            ...
            for id_, thumb in traverse_obj(
                full_meta, ("images", {dict.items}, lambda _, v: v[1]["url"])
            )
        ]
    ...
    def _extract_episode_info(self, episode, programme_id, series_meta):
        ...
        aa_vinfo = traverse_obj(
            episode, ("aa_contents_id", {lambda x: x.split(";")})
        )
        ...
        return {
            **series_meta,
            "id": episode_id,
            "formats": self._extract_m3u8_formats(
                episode.get("stream_url"), episode_id, fatal=False
            ),
    ...
    def _extract_news_info(self, headline, programme_id, series_meta):
        ...
        return {
            **series_meta,
            "id": episode_id,
            "formats": self._extract_m3u8_formats(
                episode.get("file_name"), episode_id, fatal=False
            ),
            "container": "m4a_dash",  # force fixup, AAC-only HLS
            "was_live": True,
            "series": series_meta.get("title"),
            "thumbnail": url_or_none(headline.get("headline_image"))
            or series_meta.get("thumbnail"),
            **traverse_obj(
                episode,
                {
    ...
    def _real_initialize(self):
        ...
        NhkRadiruIE._API_URL_TMPL = try_call(
            lambda: f'https:{api_config.find(".//url_program_detail").text}'
        )

    def _real_extract(self, url):
        site_id, corner_id, headline_id = self._match_valid_url(url).group(
            "site", "corner", "headline"
        )
        ...
        if site_id == "F261":  # XXX: News programmes use old API (for now?)
            ...
            if headline_id:
                headline = traverse_obj(
                    ...
                    (
                        "detail_list",
                        lambda _, v: v["headline_id"] == headline_id,
                        any,
                    ),
                )
                if not headline:
        ...
        fallback_station = join_nonempty(
            "NHK", traverse_obj(meta, ("radio_broadcast", {str})), delim=" "
        )
        series_meta = {
            "series": join_nonempty(
                "title", "corner_name", delim=" ", from_dict=meta
            ),
        ...
        if headline_id:
            episode = traverse_obj(
                meta,
                ("episodes", lambda _, v: v["id"] == int(headline_id), any),
            )
            if not episode:
                raise ExtractorError(
                    "Content not found; it has most likely expired",
...
class NhkRadioNewsPageIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        return self.url_result(
            "https://www.nhk.or.jp/radio/ondemand/detail.html?p=F261_01",
            NhkRadiruIE,
        )
...
class NhkRadiruLiveIE(InfoExtractor):
    _GEO_COUNTRIES = ["JP"]
    _VALID_URL = (
        r"https?://www\.nhk\.or\.jp/radio/player/\?ch=(?P<id>r[12]|fm)"
    )
    _TESTS = [
        {
        ...
        },
        {
            ...
            "params": {
                "extractor_args": {"nhkradirulive": {"area": ["fukuoka"]}}
            },
            "info_dict": {
                "id": "r2-fukuoka",
    ...
    def _real_extract(self, url):
        ...
        if not data:
            raise ExtractorError(
                "Invalid area. Valid areas are: {}".format(
                    ", ".join(
                        [i.text for i in config.findall(".//data//area")]
                    )
                ),
                expected=True,
            )
        ...
        noa_info = self._download_json(
            f'https:{config.find(".//url_program_noa").text}'.format(
                area=data.find("areakey").text
            ),
            station,
            note=f"Downloading {area} station metadata",
            fatal=False,
        )
        present_info = traverse_obj(
            noa_info,
            ("nowonair_list", self._NOA_STATION_IDS.get(station), "present"),
        )

        return {
            "title": " ".join(
                traverse_obj(
                    present_info, (("service", "area"), "name", {str})
                )
            ),
            ...
            "formats": self._extract_m3u8_formats(
                data.find(f"{station}hls").text, station
            ),
            "is_live": True,
        }
