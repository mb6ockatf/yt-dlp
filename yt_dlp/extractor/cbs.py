...
class CBSBaseIE(ThePlatformFeedIE):  # XXX: Do not subclass from concrete IE
    ...
    def _extract_common_video_info(
        self, content_id, asset_types, mpx_acc, extra_info
    ):
...
class CBSIE(CBSBaseIE):
    ...
    def _extract_video_info(self, content_id, site="cbs", mpx_acc=2198311517):
        ...
        title = xpath_text(video_data, "videoTitle", "title") or xpath_text(
            video_data, "videotitle", "title"
        )
        ...
        for item in items_data.findall(".//item"):
            ...
            if asset_type in asset_types:
                ...
                excluded in asset_type
                for excluded in ("HLS_FPS", "DASH_CENC", "OnceURL")
            ):
        ...
        return self._extract_common_video_info(
            content_id,
            asset_types,
            mpx_acc,
            extra_info={
                ...
                "season_number": int_or_none(
                    xpath_text(video_data, "seasonNumber")
                ),
                "episode_number": int_or_none(
                    xpath_text(video_data, "episodeNumber")
                ),
                "duration": int_or_none(
                    xpath_text(video_data, "videoLength"), 1000
                ),
...
class ParamountPressExpressIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        player = extract_attributes(
            get_element_html_by_id("vcbrightcoveplayer", webpage) or ""
        )
