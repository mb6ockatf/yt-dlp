...
class MGTVIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:w(?:ww)?\.)?mgtv\.com/[bv]/(?:[^/]+/)*(?P<id>\d+)\.html"
    )
    IE_DESC = "芒果TV"
    IE_NAME = "MangoTV"
    ...
    def _real_extract(self, url):
        ...
        try:
            ...
            if isinstance(e.cause, HTTPError) and e.cause.status == 401:
                error = self._parse_json(
                    e.cause.response.read().decode(), None
                )
        ...
        stream_domain = traverse_obj(
            stream_data, ("stream_domain", ..., {url_or_none}), get_all=False
        )

        formats = []
        for idx, stream in enumerate(
            traverse_obj(stream_data, ("stream", lambda _, v: v["url"]))
        ):
            stream_name = traverse_obj(
                stream, "name", "standardName", "barName", expected_type=str
            )
            resolution = traverse_obj(
                self._RESOLUTIONS,
                (stream_name, 1 if stream.get("scale") == "16:9" else 0),
            )
            format_url = traverse_obj(
                self._download_json(
                    urljoin(stream_domain, stream["url"]),
                    video_id,
                    fatal=False,
    ...
    def _get_subtitles(self, video_id, domain):
        info = (
            self._download_json(
                f"https://pcweb.api.mgtv.com/video/title?videoId={video_id}",
                video_id,
                fatal=False,
            )
            or {}
