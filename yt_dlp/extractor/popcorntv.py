...
class PopcornTVIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        timestamp = unified_timestamp(
            self._html_search_meta("uploadDate", webpage, "timestamp")
        )
        duration = int_or_none(
            self._html_search_meta("duration", webpage), invscale=60
        )
        view_count = int_or_none(
            self._html_search_meta("interactionCount", webpage, "view count")
        )
