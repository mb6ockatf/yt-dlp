...
class TwitterBaseIE(InfoExtractor):
    ...
    def _extract_variant_formats(self, variant, video_id):
        ...
        if not variant_url:
            ...
            for f in traverse_obj(
                fmts,
                lambda _, v: v["vcodec"] == "none" and v.get("tbr") is None,
            ):
                if mobj := re.match(
                    r"hls-[Aa]udio-(?P<bitrate>\d{4,})", f["format_id"]
                ):
                    f["tbr"] = int_or_none(mobj.group("bitrate"), 1000)
            return fmts, subs
        else:
            tbr = (
                int_or_none(dict_get(variant, ("bitrate", "bit_rate")), 1000)
                or None
            )
            f = {
                "url": variant_url,
    ...
    def _set_base_headers(self, legacy=False):
        bearer_token = (
            self._LEGACY_AUTH
            if legacy and not self.is_logged_in
            else self._AUTH
        )
        return filter_dict(
            {
                "Authorization": f"Bearer {bearer_token}",
                "x-csrf-token": try_call(
                    lambda: self._get_cookies(self._API_BASE)["ct0"].value
                ),
            }
        )
    ...
    def _call_login_api(self, note, headers, query={}, data=None):
        response = self._download_json(
            f"{self._API_BASE}onboarding/task.json",
            None,
            note,
            headers=headers,
            query=query,
        ...
        if error:
            raise ExtractorError(
                f"Login failed, Twitter API says: {error}", expected=True
            )
    ...
    def _perform_login(self, username, password):
        ...
        next_subtask = self._call_login_api(
            "Downloading flow token",
            headers,
            query={"flow_name": "login"},
            data=self._LOGIN_INIT_DATA,
        )

        while not self.is_logged_in:
            if next_subtask == "LoginJsInstrumentationSubtask":
                next_subtask = self._call_login_api(
                    "Submitting JS instrumentation response",
                    headers,
                ...
                next_subtask = self._call_login_api(
                    "Submitting alternate identifier",
                    headers,
                    data=build_login_json(
                        input_dict(
                            next_subtask,
                ...
                next_subtask = self._call_login_api(
                    "Submitting account duplication check",
                    headers,
                    data=build_login_json(
                        {
                            "subtask_id": next_subtask,
    ...
    def _call_api(self, path, video_id, query={}, graphql=False):
        ...
        result = self._download_json(
            ...
            headers=headers,
            query=query,
            expected_status=allowed_status,
    ...
    def _call_graphql_api(self, endpoint, media_id):
        ...
        return traverse_obj(
            self._call_api(endpoint, media_id, query=query, graphql=True),
            "data",
        )
...
class TwitterIE(TwitterBaseIE):
    ...
    def _graphql_to_legacy(self, data, twid):
        result = (
            traverse_obj(
                ...
                (
                    ...
                    0,
                    "entries",
                    lambda _, v: v["entryId"] == f"tweet-{twid}",
                    "content",
                    "itemContent",
                    "tweet_results",
                    "result",
        ...
        if typename not in (
            ...
            self.report_warning(
                f"Unknown typename: {typename}", twid, only_once=True
            )

        if "tombstone" in result:
            cause = remove_end(
                traverse_obj(result, ("tombstone", "text", "text", {str})),
                ". Learn more",
            )
            raise ExtractorError(
                f'Twitter API says: {cause or "Unknown error"}', expected=True
            )
            ...
            raise ExtractorError(
                reason or "Requested tweet is unavailable", expected=True
            )
        ...
        binding_values = {
            ...
            for binding_value in traverse_obj(
                status, ("card", "binding_values", ..., {dict})
            )
        }
        if binding_values:
    ...
    def _generate_syndication_token(self, twid):
        ...
        return js_number_to_string((int(twid) / 1e15) * math.pi, 36).translate(
            translation
        )
    ...
    def _extract_status(self, twid):
        ...
        try:
            if self.is_logged_in or self._selected_api == "graphql":
                status = self._graphql_to_legacy(
                    self._call_graphql_api(self._GRAPHQL_ENDPOINT, twid), twid
                )
        ...
        return (
            traverse_obj(status, "retweeted_status", None, expected_type=dict)
            or {}
        )
    ...
    def _real_extract(self, url):
        ...
        info = {
            ...
            "channel_id": str_or_none(status.get("user_id_str"))
            or str_or_none(user.get("id_str")),
            "uploader_id": uploader_id,
            "uploader_url": format_field(
                uploader_id, None, "https://twitter.com/%s"
            ),
        ...
        def extract_from_video_info(media):
                    ...
                    thumbnails.append(
                        {
                            ...
                            "width": int_or_none(
                                size.get("w") or size.get("width")
                            ),
                            "height": int_or_none(
                                size.get("h") or size.get("height")
                            ),
                        }
                    )
            ...
            return {
                ...
                "view_count": traverse_obj(
                    media, ("mediaStats", "viewCount", {int_or_none})
                ),  # No longer available
                "duration": float_or_none(
                    traverse_obj(media, ("video_info", "duration_millis")),
                    1000,
                ),
        ...
        def extract_from_card_info(card):
            ...
            if card_name == "player":
                ...
                yield {
                    "_type": "url",
                    "url": get_binding_value("url")
                    or get_binding_value("player_url"),
            ...
            # poll4choice_video, ...
                ...
                content_id = get_binding_value(
                    "%s_content_id" % (card_name if is_amplify else "player")
                )
                ...
                yield {
                    ...
                    "duration": int_or_none(
                        get_binding_value("content_duration_seconds")
                    ),
                }
        ...
        videos = traverse_obj(
            ...
            (
                ...
                "extended_entities",
                "media",
                lambda _, m: m["type"] != "photo",
                {dict},
            ),
        )

        if self._yes_playlist(
            twid, selected_index, video_label="URL-specified video number"
        ):
            selected_entries = (
            ...
            if not desired_obj:
                raise ExtractorError(
                    f"Video #{selected_index} is unavailable", expected=True
                )
            elif desired_obj.get("type") != "video":
                raise ExtractorError(
                    f"Media #{selected_index} is not a video", expected=True
                )
        ...
        entries = [
            {**info, **data, "display_id": twid} for data in selected_entries
        ]
        if not entries:
...
class TwitterBroadcastIE(TwitterBaseIE, PeriscopeBaseIE):
    ...
    _VALID_URL = (
        TwitterBaseIE._BASE_REGEX + r"i/broadcasts/(?P<id>[0-9a-zA-Z]{13})"
    )
    ...
    def _real_extract(self, url):
        ...
        info["uploader_id"] = broadcast.get("twitter_username") or info.get(
            "uploader_id"
        )
        info["uploader_url"] = format_field(
            broadcast,
            "twitter_username",
            "https://twitter.com/%s",
            default=None,
        ...
        source = self._call_api(
            f"live_video_stream/status/{media_key}", media_key
        )["source"]
        m3u8_url = source.get("noRedirectPlaybackUrl") or source["location"]
        if "/live_video_stream/geoblocked/" in m3u8_url:
            self.raise_geo_restricted()
        m3u8_id = urllib.parse.parse_qs(
            urllib.parse.urlparse(m3u8_url).query
        ).get("type", [None])[0]
...
class TwitterSpacesIE(TwitterBaseIE):
    ...
    _VALID_URL = (
        TwitterBaseIE._BASE_REGEX + r"i/spaces/(?P<id>[0-9a-zA-Z]{13})"
    )
    ...
    def _real_extract(self, url):
        ...
        if live_status == "is_upcoming":
            self.raise_no_formats(
                "Twitter Space not started yet", expected=True
            )
        elif not is_live and not metadata.get("is_space_available_for_replay"):
            self.raise_no_formats(
                "Twitter Space ended and replay is disabled", expected=True
            )
            ...
            formats = (
                self._extract_m3u8_formats(
                    source,
                    metadata["media_key"],
                    "m4a" if is_audio_space else "mp4",
        ...
        if not formats and live_status == "post_live":
            self.raise_no_formats(
                "Twitter Space ended but not downloadable yet", expected=True
            )
...
class TwitterShortenerIE(TwitterBaseIE):
    ...
    def _real_extract(self, url):
        ...
        new_url = self._request_webpage(
            url, shortcode, headers={"User-Agent": "curl"}
        ).url
        __UNSAFE_LINK = (
