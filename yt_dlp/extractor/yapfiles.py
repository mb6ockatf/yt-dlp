...
class YapFilesIE(InfoExtractor):
    _WORKING = False
    _YAPFILES_URL = (
        r"//(?:(?:www|api)\.)?yapfiles\.ru/get_player/*\?.*?\bv=(?P<id>\w+)"
    )
    ...
    def _real_extract(self, url):
        ...
        player = self._download_json(player_url, video_id, query=query)[
            "player"
        ]
        ...
        playlist = self._download_json(playlist_url, video_id)["player"][
            "main"
        ]
        ...
        for format_id in QUALITIES:
            is_hd = format_id == "hd"
            format_url = url_or_none(
                playlist.get("file%s" % ("_hd" if is_hd else ""))
            )
            if not format_url:
                continue
            formats.append(
