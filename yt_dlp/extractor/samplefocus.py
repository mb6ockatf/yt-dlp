...
class SampleFocusIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?samplefocus\.com/samples/(?P<id>[^/?&#]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        for author_id, author, body in re.findall(
            r'(?s)<p[^>]+class="comment-author"><a[^>]+href="/users/([^"]+)">([^"]+)</a>.+?<p[^>]+class="comment-body">([^>]+)</p>',
            webpage,
        ):
            comments.append(
                {
