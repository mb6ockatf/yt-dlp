...
class WordpressPlaylistEmbedIE(InfoExtractor):
    ...
    def _extract_from_webpage(self, url, webpage):
        ...
        for i, j in enumerate(
            get_elements_by_class("wp-playlist-script", webpage)
        ):
            playlist_json = (
                self._parse_json(
                    j,
            ...
            entries = [
                {
                    ...
                    "duration": parse_duration(
                        traverse_obj(track, ("meta", "length_formatted"))
                    ),
                    "description": track.get("description"),
                    "height": int_or_none(
                        traverse_obj(
                            track, ("dimensions", "original", "height")
                        )
                    ),
                    "width": int_or_none(
                        traverse_obj(
                            track, ("dimensions", "original", "width")
                        )
                    ),
                }
                for track in traverse_obj(
                    playlist_json, ("tracks", ...), expected_type=dict
                )
            ]
            yield self.playlist_result(
                entries,
                self._generic_id(url) + f"-wp-playlist-{i + 1}",
                "Wordpress Playlist",
            )
...
class WordpressMiniAudioPlayerEmbedIE(InfoExtractor):
    ...
    def _extract_from_webpage(self, url, webpage):
        ...
        file_exts = re.findall(
            r"a\[href\s*\*=\s*\'\.([a-zA-Z\d]+)\'", mb_player_params
        )
        if not file_exts:
            return
