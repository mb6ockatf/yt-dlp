    ...
    def _perform_login(self, username, password):
        ...
        action_url = urljoin(
            ...
            self._search_regex(
                ...
                login_page,
                "post url",
                default="https://www.linkedin.com/uas/login-submit",
                group="url",
            ),
        )
        ...
        login_submit_page = self._download_webpage(
            action_url, None, "Logging in", data=urlencode_postdata(data)
        )
...
class LinkedInIE(LinkedInBaseIE):
    ...
    def _real_extract(self, url):
        ...
        video_attrs = extract_attributes(
            self._search_regex(r"(<video[^>]+>)", webpage, "video")
        )
        ...
        return {
            ...
            "uploader": traverse_obj(
                ...
                (
                    lambda _, v: v["@type"] == "SocialMediaPosting",
                    "author",
                    "name",
                    {str},
                ),
...
class LinkedInLearningIE(LinkedInLearningBaseIE):
    ...
    def json2srt(self, transcript_lines, duration=None):
        ...
        for line, (line_dict, next_dict) in enumerate(
            ...
            start_time, caption = (
                line_dict["transcriptStartAt"] / 1000,
                line_dict["caption"],
            )
            end_time = (
                next_dict["transcriptStartAt"] / 1000
                if next_dict
                else duration or start_time + 1
            )
            srt_data += (
                f"{line + 1}\n"
    ...
    def _real_extract(self, url):
        ...
        for width, height in ((640, 360), (960, 540), (1280, 720)):
            video_data = self._call_api(
                course_slug, "selectedVideo", video_slug, height
            )["selectedVideo"]
        ...
        transcript_lines = try_get(
            video_data, lambda x: x["transcript"]["lines"], expected_type=list
        )
        if transcript_lines:
            subtitles["en"] = [
                {
...
class LinkedInLearningCourseIE(LinkedInLearningBaseIE):
    ...
    def suitable(cls, url):
        return (
            False
            if LinkedInLearningIE.suitable(url)
