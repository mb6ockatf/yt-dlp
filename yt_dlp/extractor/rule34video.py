        ...
        for col in get_elements_by_class("col", webpage):
            ...
            if label == "Categories:":
                ...
                uploader_url = extract_attributes(
                    get_element_html_by_class("name", col) or ""
                ).get("href")
        ...
        return {
            **traverse_obj(
            ...
            "like_count": parse_count(
                get_element_by_class("voters count", webpage)
            ),
            "comment_count": int_or_none(
                self._search_regex(
                    r"[^(]+\((\d+)\)",
                    get_element_by_attribute("href", "#tab_comments", webpage),
                    "comment count",
                    fatal=False,
                )
            ),
            ...
            "tags": list(
                map(
                    unescapeHTML,
                    re.findall(
                        r'<a class="tag_item"[^>]+\bhref="https://rule34video\.com/tags/\d+/"[^>]*>(?P<tag>[^>]*)</a>',
                        webpage,
                    ),
                )
            ),
