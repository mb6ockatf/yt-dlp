...
class CuriosityStreamBaseIE(InfoExtractor):
    ...
    def _call_api(self, path, video_id, query=None):
        ...
        if not self._auth_token:
            auth_cookie = self._get_cookies("https://curiositystream.com").get(
                "auth_token"
            )
            if auth_cookie:
    ...
    def _perform_login(self, username, password):
        result = self._download_json(
            "https://api.curiositystream.com/v1/login",
            None,
            note="Logging in",
    ...
    def _real_extract(self, url):
        ...
        for closed_caption in media.get("closed_captions", []):
            ...
            if not sub_url:
                continue
            lang = (
                closed_caption.get("code")
                or closed_caption.get("language")
                or "en"
            )
        ...
        return {
            ...
            "thumbnail": media.get("image_large")
            or media.get("image_medium")
            or media.get("image_small"),
    ...
    def _real_extract(self, url):
        ...
        for media in collection.get("media", []):
            ...
            media_type, ie = (
                ...
                if media.get("is_collection")
                else ("video", CuriosityStreamIE)
            )
            entries.append(
                self.url_result(
...
class CuriosityStreamCollectionsIE(CuriosityStreamCollectionBaseIE):
    ...
    _VALID_URL = (
        r"https?://(?:app\.)?curiositystream\.com/collections/(?P<id>\d+)"
    )
