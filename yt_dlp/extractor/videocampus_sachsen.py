...
class VideocampusSachsenIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        host, video_id, tmp_id, display_id, embed_id = self._match_valid_url(
            url
        ).group("host", "id", "tmp_id", "display_id", "embed_id")
        webpage = (
            self._download_webpage(url, video_id or tmp_id, fatal=False) or ""
        )
    ...
    def _real_extract(self, url):
        ...
        url_part = (
            ...
            if album_id
                ...
                f"category/{name}/category_id/{channel_id}"
                if mode == "category"
                else (
                    f"title/{name}/channel/{channel_id}"
                    if mode == "channel"
                    else f"tag/{tag_id}"
                )
            )
        ...
        return self.playlist_result(
            OnDemandPagedList(
                functools.partial(
                    self._fetch_page, host, url_part, playlist_id, data
                ),
                self._PAGE_SIZE,
            ),
