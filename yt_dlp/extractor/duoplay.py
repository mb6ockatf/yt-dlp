...
class DuoplayIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        episode = traverse_obj(
            parse_qs(url), ("ep", 0, {int_or_none}, {str_or_none})
        )
        video_id = join_nonempty(telecast_id, episode, delim="_")
        webpage = self._download_webpage(url, video_id)
        video_player = try_call(
            lambda: extract_attributes(
        ...
        episode_attr = (
            self._parse_json(
                video_player.get(":episode") or "", video_id, fatal=False
            )
            or {}
        )
        ...
        return {
            ...
            "formats": self._extract_m3u8_formats(
                manifest_url, video_id, "mp4", query={"s": session_token}
            ),
            **traverse_obj(
                episode_attr,
                {
                    ...
                    "timestamp": (
                        "airtime",
                        {lambda x: unified_timestamp(x + " +0200")},
                    ),
            ...
            **(
                ...
                if episode_attr.get("category") != "movies"
                else {}
            ),
        }
