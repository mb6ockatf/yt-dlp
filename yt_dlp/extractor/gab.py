...
class GabIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        for idx, media in enumerate(json_data["media_attachments"]):
            ...
            metadata = media["meta"]
            format_metadata = {
                "acodec": parse_codecs(metadata.get("audio_encode")).get(
                    "acodec"
                ),
                "asr": int_or_none(
            ...
            formats = [
                ...
                for url, f in (
                    (media.get("url"), metadata.get("original") or {}),
                    (media.get("source_mp4"), metadata.get("playable") or {}),
                )
                if url
            ]
            ...
            entries.append(
                {
                    ...
                    or parse_duration(metadata.get("length")),
