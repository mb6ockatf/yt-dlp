...
class NiconicoChannelPlusBaseIE(InfoExtractor):
    ...
    def _find_fanclub_site_id(self, channel_name):
        ...
        fanclub_id = traverse_obj(
            ...
            (
                lambda _, v: v["domain"]
                == f"{self._WEBPAGE_BASE_URL}/{channel_name}",
                "id",
            ),
            get_all=False,
        )
        if not fanclub_id:
            raise ExtractorError(
                f"Channel {channel_name} does not exist", expected=True
            )
        return fanclub_id
...
class NiconicoChannelPlusIE(NiconicoChannelPlusBaseIE):
    ...
    def _real_extract(self, url):
        content_code, channel_id = self._match_valid_url(url).group(
            "code", "channel"
        )
        ...
        if live_status == "is_upcoming":
            ...
            formats = self._extract_m3u8_formats(
                ...
                m3u8_url=data_json["video_stream"]["authenticated_url"].format(
        ...
        return {
            ...
            "channel": self._get_channel_base_info(fanclub_site_id).get(
                "fanclub_site_name"
            ),
            "channel_id": channel_id,
            "channel_url": f"{self._WEBPAGE_BASE_URL}/{channel_id}",
            "age_limit": traverse_obj(
                self._get_channel_user_info(fanclub_site_id),
            ...
            **traverse_obj(
            ...
            "__post_extractor": self.extract_comments(
                ...
                comment_group_id=traverse_obj(
                    data_json, ("video_comment_setting", "comment_group_id")
                ),
            ),
        }
    ...
    def _fetch_paged_channel_video_list(
        ...
        for content_code in traverse_obj(
            ...
            yield self.url_result(
                f"{self._WEBPAGE_BASE_URL}/{channel_name}/video/{content_code}",
...
class NiconicoChannelPlusChannelVideosIE(NiconicoChannelPlusChannelBaseIE):
    ...
    _VALID_URL = (
        r"https?://nicochannel\.jp/(?P<id>[a-z\d\._-]+)/videos(?:\?.*)?"
    )
    _TESTS = [
        {
            # query: None
    ...
    def _real_extract(self, url):
        ...
        fanclub_site_id = self._find_fanclub_site_id(channel_id)
        channel_name = self._get_channel_base_info(fanclub_site_id).get(
            "fanclub_site_name"
        )
        qs = parse_qs(url)
        ...
        return self.playlist_result(
            OnDemandPagedList(
                functools.partial(
                    ...
                    f"fanclub_sites/{fanclub_site_id}/video_pages",
                    filter_dict(
                        {
                            "tag": traverse_obj(qs, ("tag", 0)),
                            "sort": traverse_obj(
                                qs, ("sort", 0), default="-released_at"
                            ),
                            "vod_type": traverse_obj(
                                qs, ("vodType", 0), default="0"
                            ),
                        }
                    ),
                    channel_id,
            ...
            playlist_id=f"{channel_id}-videos",
