...
class IVXPlayerIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        video_id, player_key = self._match_valid_url(url).group(
            "video_id", "player_key"
        )
        ...
        return {
            ...
            "duration": int_or_none(
                traverse_obj(json_data, ("ivx", "duration"))
            ),
            "timestamp": parse_iso8601(
                traverse_obj(json_data, ("ivx", "published_at"))
            ),
            "formats": formats,
            "subtitles": subtitles,
...
class TempoIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        _, video_id, player_key = next(
            IVXPlayerIE._extract_embed_urls(url, webpage)
        ).split(":")
        ...
        return self.url_result(
            ...
            thumbnail=self._html_search_meta("twitter:image:src", webpage)
