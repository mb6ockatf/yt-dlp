...
class PeriscopeBaseIE(InfoExtractor):
    ...
    def _call_api(self, method, query, item_id):
        return self._download_json(
            f"https://api.periscope.tv/api/v2/{method}", item_id, query=query
        )

    def _parse_broadcast_data(self, broadcast, video_id):
        title = broadcast.get("status") or "Periscope Broadcast"
        uploader = broadcast.get("user_display_name") or broadcast.get(
            "username"
        )
        ...
        thumbnails = [
            ...
            for image in ("image_url", "image_url_medium", "image_url_small")
        ...
        return {
            ...
            or int_or_none(broadcast.get("created_at_ms"), scale=1000),
            "release_timestamp": int_or_none(
                broadcast.get("scheduled_start_ms"), scale=1000
            ),
            "uploader": uploader,
            "uploader_id": broadcast.get("user_id")
            or broadcast.get("username"),
            "thumbnails": thumbnails,
            "view_count": int_or_none(broadcast.get("total_watched")),
            "concurrent_view_count": int_or_none(
                broadcast.get("total_watching")
            ),
            "tags": broadcast.get("tags"),
            "live_status": {
                "running": "is_live",
                "not_started": "is_upcoming",
            }.get(traverse_obj(broadcast, ("state", {str.lower})))
            or "was_live",
        }
    ...
    def _extract_common_format_info(broadcast):
        return (
            broadcast.get("state").lower(),
            int_or_none(broadcast.get("width")),
            int_or_none(broadcast.get("height")),
        )
    ...
    def _extract_pscp_m3u8_formats(
        self, m3u8_url, video_id, format_id, state, width, height, fatal=True
    ):
        m3u8_formats = self._extract_m3u8_formats(
            ...
            "mp4",
            entry_protocol=(
                "m3u8_native" if state in ("ended", "timed_out") else "m3u8"
            ),
            m3u8_id=format_id,
            fatal=fatal,
...
class PeriscopeIE(PeriscopeBaseIE):
    ...
    IE_NAME = "periscope"
    _VALID_URL = (
        r"https?://(?:www\.)?(?:periscope|pscp)\.tv/[^/]+/(?P<id>[^/?#]+)"
    )
    _EMBED_REGEX = [
...
class PeriscopeUserIE(PeriscopeBaseIE):
    ...
    def _real_extract(self, url):
        ...
        user_id = user["id"]
        session_id = data_store["SessionToken"]["public"]["broadcastHistory"][
            "token"
        ]["session_id"]
