...
class MusicdexBaseIE(InfoExtractor):
    def _return_info(self, track_json, album_json, video_id):
        return {
            ...
            "url": format_field(
                track_json, "url", "https://www.musicdex.org/%s"
            ),
            ...
            "artists": [
                artist.get("name")
                for artist in track_json.get("artists") or []
            ],
            "album_artists": [
                artist.get("name")
                for artist in album_json.get("artists") or []
            ],
            "thumbnail": format_field(
                album_json, "image", "https://www.musicdex.org/%s"
            ),
            "album": album_json.get("name"),
            "release_year": try_get(
                album_json,
                lambda x: date_from_str(
                    unified_strdate(x["release_date"])
                ).year,
            ),
...
class MusicdexSongIE(MusicdexBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self._return_info(
            data_json, data_json.get("album") or {}, video_id
        )
...
class MusicdexAlbumIE(MusicdexBaseIE):
    ...
    def _real_extract(self, url):
        ...
        entries = [
            self._return_info(track, data_json, track["id"])
            for track in data_json.get("tracks") or []
            if track.get("id")
        ]
        ...
        return {
            ...
            "thumbnail": format_field(
                data_json, "image", "https://www.musicdex.org/%s"
            ),
            "release_year": try_get(
                data_json,
                lambda x: date_from_str(
...
class MusicdexPageIE(
    ...
    def _entries(self, playlist_id):
        ...
        while next_page_url:
            data_json = self._download_json(next_page_url, playlist_id)[
                "pagination"
            ]
...
class MusicdexArtistIE(MusicdexPageIE):
    ...
    def _real_extract(self, url):
        ...
        for album in self._entries(playlist_id):
            entries.extend(
                self._return_info(track, album, track["id"])
        ...
        return {
            ...
            "view_count": data_json.get("plays"),
            "thumbnail": format_field(
                data_json, "image_small", "https://www.musicdex.org/%s"
            ),
            "entries": entries,
        }
...
class MusicdexPlaylistIE(MusicdexPageIE):
    ...
    def _real_extract(self, url):
        ...
        entries = [
            self._return_info(track, track.get("album") or {}, track["id"])
            for track in self._entries(playlist_id) or []
            if track.get("id")
        ]
