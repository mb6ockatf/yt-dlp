...
class TVAIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        entity = self._search_nextjs_data(webpage, entity_id)["props"][
            "pageProps"
        ]["staticEntity"]
        ...
        return {
            ...
            "url": smuggle_url(
                self._BC_URL_TMPL.format(video_id), {"geo_countries": ["CA"]}
            ),
            ...
            **traverse_obj(
                entity,
                {
                    ...
                    "season_number": (
                        "slug",
                        {lambda x: re.search(r"/s(?:ai|ea)son-(\d+)/", x)},
                        1,
                        {int_or_none},
                    ),
