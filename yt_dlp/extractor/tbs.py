...
class TBSIE(TurnerBaseIE):
    ...
    def _real_extract(self, url):
        ...
        video_data = next(
            v
            for v in drupal_settings["turner_playlist"]
            if is_live or v.get("url") == path
        )

        media_id = video_data["mediaID"]
        title = video_data["title"]
        tokenizer_query = urllib.parse.parse_qs(
            urllib.parse.urlparse(drupal_settings["ngtv_token_url"]).query
        )
        ...
        info.update(
            {
                ...
                "description": strip_or_none(
                    video_data.get("descriptionNoTags")
                    or video_data.get("shortDescriptionNoTags")
                ),
