...
class OnDemandChinaEpisodeIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        video_info = self._download_json(
            "https://odc-graphql.odkmedia.io/graphql",
            display_id,
        ...
        try:
            source_json = self._download_json(
                f'https://odkmedia.io/odc/api/v2/playback/{video_info["id"]}/',
                display_id,
            ...
            if isinstance(e.cause, HTTPError):
                error_data = self._parse_json(
                    e.cause.response.read(), display_id
                )["detail"]
        ...
        for source in traverse_obj(source_json, ("sources", ...)):
            if source.get("type") == "hls":
                ...
                self.report_warning(
                    f'Unsupported format {source.get("type")}', display_id
                )
        ...
        return {
            ...
            "thumbnail": (
                traverse_obj(video_info, ("images", "thumbnail"))
                or self._html_search_meta(
                    ["og:image", "twitter:image"], webpage
                )
            ),
            "title": (
                traverse_obj(video_info, "title", "titleEn")
                or self._html_search_meta(
            ...
            "alt_title": traverse_obj(
                video_info, "titleKo", "titleZhHans", "titleZhHant"
            ),
            "description": (
                traverse_obj(
                    video_info,
            ...
            "subtitles": subtitles,
            "tags": try_call(
                lambda: self._html_search_meta("keywords", webpage).split(", ")
            ),
        }
