...
def default_plugin_paths():
    ...
    yield from (
        path for path in map(Path, sys.path) if path != _BASE_PACKAGE_PATH
    )
        ...
        self.packages = set(
            itertools.chain.from_iterable(
                itertools.accumulate(
                    name.split("."), lambda a, b: ".".join((a, b))
                )
        ...
        for path in orderedSet(candidate_locations, lazy=True):
            ...
            try:
                if candidate.is_dir():
                    ...
                    path.suffix in (".zip", ".egg", ".whl") and path.is_file()
                ):
...
def iter_modules(subpackage):
    ...
    with contextlib.suppress(ModuleNotFoundError):
        ...
        yield from pkgutil.iter_modules(
            path=pkg.__path__, prefix=f"{fullname}."
        )
...
def load_plugins(plugin_spec: PluginSpec):
    ...
    for finder, module_name, _ in iter_modules(name):
        ...
        regular_classes.update(
            get_regular_classes(module, module_name, suffix)
        )
    ...
    if "default" in plugin_dirs.value:
        with contextlib.suppress(FileNotFoundError):
            ...
            regular_classes.update(
                get_regular_classes(plugins, spec.name, suffix)
            )
