...
__name__ = __name__.rsplit(".", 1)[
    0
]  # noqa: A001: Pretend to be the parent module
...
def get_elements_by_attribute(*args, **kwargs):
    ...
    return [
        content
        for content, _ in get_elements_text_and_html_by_attribute(
            *args, **kwargs
        )
    ]
...
def get_elements_html_by_attribute(*args, **kwargs):
    ...
    return [
        whole
        for _, whole in get_elements_text_and_html_by_attribute(
            *args, **kwargs
        )
    ]
...
def get_elements_text_and_html_by_attribute(
    attribute, value, html, *, tag=r"[\w:.-]+", escape_value=True
):
    """
    ...
    for m in re.finditer(partial_element_re, html):
        content, whole = get_element_text_and_html_by_tag(
            m.group("tag"), html[m.start() :]
        )
    ...
    def handle_endtag(self, tag):
        ...
        while self.tagstack:
            ...
            raise compat_HTMLParseError(
                f"matching opening tag for closing {tag} tag not found"
            )
        if not self.tagstack:
...
def get_element_text_and_html_by_tag(tag, html):
    ...
    content_start = find_or_raise(
        html[whole_start:],
        ">",
        compat_HTMLParseError(f"malformed opening {tag} tag"),
    )
    ...
    with HTMLBreakOnClosingTagParser() as parser:
        ...
        if not parser.tagstack or parser.tagstack[0] != tag:
            raise compat_HTMLParseError(
                f"parser did not match opening {tag} tag"
            )
...
class LenientJSONDecoder(json.JSONDecoder):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.transform_source, self.ignore_extra = (
            transform_source,
            ignore_extra,
        )
            ...
            try:
                ...
                if e.pos is None:
                    ...
                    if s is not None:
                        continue
                raise type(e)(
                    f"{e.msg} in {s[e.pos - 10:e.pos + 10]!r}", s, e.pos
                )
    ...
    for attempt in range(2):
        try:
            try:
                ...
                stream = locked_file(
                    filename, open_mode, block=False
                ).__enter__()
            except OSError:
...
class Popen(subprocess.Popen):
    ...
    def __init__(
        self, args, *remaining, env=None, text=False, shell=False, **kwargs
    ):
        if env is None:
            env = os.environ.copy()
...
_timetuple = collections.namedtuple(
...
class ExtractorError(YoutubeDLError):
    ...
    def format_traceback(self):
        return (
            join_nonempty(
                self.traceback
                and "".join(traceback.format_tb(self.traceback)),
                self.cause
                and "".join(
                    traceback.format_exception(
                        None, self.cause, self.cause.__traceback__
                    )[1:]
                ),
                delim="\n",
            )
...
def unified_strdate(date_str, day_first=True):
    ...
    for expression in date_formats(day_first):
        with contextlib.suppress(ValueError):
            upload_date = dt.datetime.strptime(date_str, expression).strftime(
                "%Y%m%d"
            )
...
def unified_timestamp(date_str, day_first=True):
    ...
    date_str = re.sub(
        ...
        " ",
        re.sub(
            r"(?i)[,|]|(mon|tues?|wed(nes)?|thu(rs)?|fri|sat(ur)?|sun)(day)?",
            "",
            date_str,
        ),
    )
...
def subtitles_filename(filename, sub_lang, sub_format, expected_real_ext=None):
    return replace_extension(
        filename, sub_lang + "." + sub_format, expected_real_ext
    )
...
def datetime_from_str(date_str, precision="auto", format="%Y%m%d"):
    ...
    if match is not None:
        ...
        if unit == "month" or unit == "year":
            new_date = datetime_add_months(
                start_time, time * 12 if unit == "year" else time
            )
            unit = "day"
        else:
            if unit == "week":
...
def date_from_str(date_str, format="%Y%m%d", strict=False):
    ...
    if strict and not re.fullmatch(
        r"\d{8}|(now|today|yesterday)(-\d+(day|week|month|year)s?)?", date_str
    ):
        raise ValueError(f'Invalid date format "{date_str}"')
    return datetime_from_str(
        date_str, precision="microsecond", format=format
    ).date()
...
def datetime_round(dt_, precision="day"):
    ...
    timestamp = roundto(
        calendar.timegm(dt_.timetuple()), unit_seconds[precision]
    )
...
def system_identifier():
    ...
    if python_implementation == "PyPy" and hasattr(sys, "pypy_version_info"):
        python_implementation += (
            " version %d.%d.%d" % sys.pypy_version_info[:3]
        )
    libc_ver = []
    with contextlib.suppress(
        OSError
    ):  # We may not have access to the executable
...
if sys.platform == "win32":
    ...
    def _unlock_file(f):
        ...
        if not UnlockFileEx(
            handle, 0, whole_low, whole_high, f._lock_file_overlapped_p
        ):
        ...
        flags = functools.reduce(
            ...
            (
                ...
                (
                    os.O_RDONLY
                    if not writable
                    else os.O_RDWR if readable else os.O_WRONLY
                ),
            ),
        )

        self.f = os.fdopen(
            os.open(filename, flags, 0o666), mode, encoding=encoding
        )
...
def format_decimal_suffix(num, fmt="%d%s", *, factor=1000):
    ...
    exponent = (
        0
        if num == 0
        else min(int(math.log(num, factor)), len(POSSIBLE_SUFFIXES))
    )
...
def parse_resolution(s, *, lenient=False):
    ...
    if lenient:
        ...
        mobj = re.search(
            r"(?<![a-zA-Z0-9])(?P<w>\d+)\s*[xX×,]\s*(?P<h>\d+)(?![a-zA-Z0-9])",
            s,
        )
    if mobj:
        return {
...
def int_or_none(
    ...
    try:
        return (
            (int(v) if base is None else int(v, base=base)) * invscale // scale
        )
...
def url_or_none(url):
    ...
    return (
        url
        if re.match(
            r"(?:(?:https?|rt(?:m(?:pt?[es]?|fp)|sp[su]?)|mms|ftps?):)?//", url
        )
        else None
    )
...
def strftime_or_none(timestamp, date_format="%Y%m%d", default=None):
    ...
    try:
        if isinstance(timestamp, (int, float)):  # unix timestamp
            ...
            datetime_object = dt.datetime.fromtimestamp(
                0, dt.timezone.utc
...
class LazyList(collections.abc.Sequence):
    ...
    def __copy__(self):
        return type(self)(
            self._iterable, reverse=self._reversed, _cache=self._cache
        )
...
class PagedList:
    ...
    def getpage(self, pagenum):
        ...
        if page_results is None:
            page_results = (
                []
                if pagenum > self._pagecount
                else list(self._pagefunc(pagenum))
            )
        if self._use_cache:
...
class OnDemandPagedList(PagedList):
    ...
    def _getslice(self, start, end):
        for pagenum in itertools.count(start // self._pagesize):
            ...
            startv = (
                start % self._pagesize if firstid <= start < nextfirstid else 0
            )
            endv = (
...
class PlaylistEntries:
    ...
    def parse_playlist_items(cls, string):
        for segment in string.split(","):
            ...
            start, end, step, has_range = mobj.group(
            ...
            yield (
                slice(
                    int_or_none(start), float_or_none(end), int_or_none(step)
                )
                if has_range
                else int(start)
            )
            ...
            for i, entry in self[index]:
                ...
                try:
                    ...
                    if not self.ydl.params.get("lazy_playlist"):
                        ...
                        self.ydl._match_entry(
                            entry, incomplete=True, silent=True
                        )
...
def lowercase_escape(s):
    unicode_escape = codecs.getdecoder("unicode_escape")
    return re.sub(
        r"\\u[0-9a-fA-F]{4}", lambda m: unicode_escape(m.group(0))[0], s
    )
...
def multipart_encode(data, boundary=None):
    ...
    while True:
        if boundary is None:
            boundary = "---------------" + str(
                random.randrange(0x0FFFFFFF, 0xFFFFFFFF)
            )
...
def is_iterable_like(
    x, allowed_types=collections.abc.Iterable, blocked_types=NO_DEFAULT
):
...
def try_get(src, getter, expected_type=None):
    return try_call(
        *variadic(getter), args=(src,), expected_type=expected_type
    )
...
def merge_dicts(*dicts):
    ...
    for a_dict in dicts:
        for k, v in a_dict.items():
            if (v is not None and k not in merged) or (
                isinstance(v, str) and merged[k] == ""
            ):
                merged[k] = v
    return merged
...
def parse_age_limit(s):
    ...
    m = re.match(
        r"^TV[_-]?({})$".format(
            "|".join(k[3:] for k in TV_PARENTAL_GUIDELINES)
        ),
        s,
    )
    if m:
...
def js_to_json(code, vars={}, *, strict=False):
    ...
    def process_escape(match):
        ...
        return (
            ...
            else R"\u00" if escape == "x" else "" if escape == "\n" else escape
        )
    ...
    if not strict:
        ...
        code = re.sub(
            r"new \w+\((.*?)\)", lambda m: json.dumps(m.group(0)), code
        )
...
def mimetype2ext(mt, default=NO_DEFAULT):
    ...
    ext = traversal.traverse_obj(
        MAP, mimetype, subtype, subtype.rsplit("+")[-1]
    )
    if ext:
        return ext
...
def parse_codecs(codecs_str):
    ...
    if not codecs_str:
        return {}
    split_codecs = list(
        filter(None, map(str.strip, codecs_str.strip().strip(",").split(",")))
    )
    vcodec, acodec, scodec, hdr = None, None, None, None
    for full_codec in split_codecs:
        full_codec = re.sub(
            r"^([^.]+)", lambda m: m.group(1).lower(), full_codec
        )
        ...
        if parts[0] in (
            ...
            if parts[0] in ("dvh1", "dvhe"):
                ...
                parts[0] == "av1" and traversal.traverse_obj(parts, 3) == "10"
            ):
                hdr = "HDR10"
            elif parts[:2] == ["vp9", "2"]:
                hdr = "HDR10"
...
def get_compatible_ext(*, vcodecs, acodecs, vexts, aexts, preferences=None):
    ...
    for ext in preferences or vexts:
        ...
        if (
            ...
            or current_exts == {ext}
            or any(
                ext_sets.issuperset(current_exts)
...
def render_table(header_row, data, delim=False, extra_gap=0, hide_empty=False):
    ...
    def filter_using_list(row, filter_array):
        return [
            col
            for take, col in itertools.zip_longest(
                filter_array, row, fillvalue=True
            )
            if take
        ]
    ...
    if delim:
        table = [
            header_row,
            [delim * (ml + extra_gap) for ml in max_lens],
            *data,
        ]
        table[1][-1] = table[1][-1][
            : -extra_gap * len(delim)
    ...
    for row in table:
        for pos, text in enumerate(map(str, row)):
            if "\t" in text:
                row[pos] = (
                    text.replace("\t", " " * (max_lens[pos] - width(text)))
                    + " " * extra_gap
                )
            else:
                row[pos] = text + " " * (
                    max_lens[pos] - width(text) + extra_gap
                )
...
def _match_one(filter_part, dct, incomplete):
    ...
    if m:
        ...
        if numeric_comparison is not None and m["op"] in STRING_OPERATORS:
            raise ValueError(
                "Operator {} only supports string values!".format(m["op"])
            )
...
def match_filter_func(filters, breaking_filters=None):
    ...
    breaking_filters = match_filter_func(breaking_filters) or (
        lambda _, __: None
    )
    ...
    def _match_func(info_dict, incomplete=False):
        ...
        if not filters or any(
            match_str(f, info_dict, incomplete) for f in filters
        ):
            return NO_DEFAULT if interactive and not incomplete else None
        else:
            video_title = (
                info_dict.get("title") or info_dict.get("id") or "entry"
            )
...
class download_range_func:
    def __init__(self, chapters, ranges, from_info=False):
        self.chapters, self.ranges, self.from_info = (
            chapters,
            ranges,
            from_info,
        )
    ...
    def __call__(self, info_dict, ydl):
        ...
        for start, end in self.ranges or []:
            yield {
                "start_time": self._handle_negative_timestamp(
                    start, info_dict
                ),
                "end_time": self._handle_negative_timestamp(end, info_dict),
            }

        if self.from_info and (
            info_dict.get("start_time") or info_dict.get("end_time")
        ):
            yield {
    ...
    def _handle_negative_timestamp(time, info):
        return (
            max(info["duration"] + time, 0)
            if info.get("duration") and time < 0
            else time
        )
    ...
    def __repr__(self):
        return (
            f"{__name__}.{type(self).__name__}({self.chapters}, {self.ranges})"
        )
                ...
                if style:
                    ...
                    for k, v in sorted(style.items()):
                        if (
                            ...
                            and self._applied_styles[-1].get(k) == v
                        ):
                            continue
                        if k == "color":
                            font += f' color="{v}"'
...
def _configuration_args(
    main_key, argdict, exe, keys=None, default=[], use_compat=True
):
...
def pkcs1pad(data, length):
    ...
    pseudo_random = [
        random.randint(0, 254) for _ in range(length - len(data) - 3)
    ]
...
def _base_n_table(n, table):
    ...
    table = (
        table
        or "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"
    )[:n]
...
def decode_packed_codes(code):
    ...
    return re.sub(
        r"\b(\w+)\b", lambda mobj: symbol_table[mobj.group(0)], obfuscated_code
    )
...
def write_xattr(path, key, value):
    ...
    try:
        _, stderr, returncode = Popen.run(
            (
                [exe, "-w", key, value, path]
                if exe == "xattr"
                else [exe, "-n", key, "-v", value, path]
            ),
            text=True,
            stdout=subprocess.PIPE,
...
def iri_to_uri(iri):
    ...
    if iri_parts.username:
        net_location += urllib.parse.quote(
            iri_parts.username, safe=r"!$%&'()*+,~"
        )
        if iri_parts.password is not None:
            net_location += ":" + urllib.parse.quote(
                iri_parts.password, safe=r"!$%&'()*+,~"
            )
        net_location += "@"

    net_location += iri_parts.hostname.encode(
        "idna"
    ).decode()  # Punycode for Unicode hostnames.
    ...
    return urllib.parse.urlunparse(
        (
            ...
            urllib.parse.quote_plus(
                iri_parts.params, safe=r"!$%&'()*+,/:;=@|~"
            ),
            # Not totally sure about the `safe` argument, since the source does not explicitly mention the query URI component.
            urllib.parse.quote_plus(
                iri_parts.query, safe=r"!$%&'()*+,/:;=?@{|}~"
            ),
            urllib.parse.quote_plus(
                iri_parts.fragment, safe=r"!#$%&'()*+,/:;=?@{|}~"
            ),
        )
    )
...
def random_uuidv4():
    return re.sub(
        r"[xy]",
        lambda x: _HEX_TABLE[random.randint(0, 15)],
...
def get_executable_path():
    ...
    return os.path.dirname(
        os.path.abspath(_get_variant_and_executable_path()[1])
    )
...
def join_nonempty(*values, delim="-", from_dict=None):
    if from_dict is not None:
        values = (
            traversal.traverse_obj(from_dict, variadic(v)) for v in values
        )
    ...
    * Update dimensions
    ...
    return [
        merge_dicts(
            {
                "url": re.sub(
                    url_width_re, str(max_dimensions[0]), thumbnail["url"]
                )
            },
            dict(zip(_keys, max_dimensions)),
            thumbnail,
        )
...
def parse_http_range(range):
    ...
    return (
        int(crg.group(1)),
        int_or_none(crg.group(2)),
        int_or_none(crg.group(3)),
    )
...
class Config:
    ...
    def load_configs(self):
        ...
        for location in opts.config_locations or []:
            if location == "-":
                ...
                self._loaded_paths.add(location)
                self.append_config(
                    shlex.split(read_stdin("options"), comments=True),
                    label="stdin",
                )
                continue
    ...
    def __str__(self):
        ...
        return join_nonempty(
            self.own_args is not None
            and f"{label[0].upper()}{label[1:]}: {self.hide_login_info(self.own_args)}",
...
def merge_headers(*dicts):
    ...
    return {
        k.title(): v
        for k, v in itertools.chain.from_iterable(map(dict.items, dicts))
    }
...
def cached_method(f):
    ...
    def wrapper(self, *args, **kwargs):
        ...
        cache = (
            vars(self)
            .setdefault("_cached_method__cache", {})
            .setdefault(f.__name__, {})
        )
        if key not in cache:
...
class RetryManager:
    ...
    @staticmethod
    def report_retry(
        e, count, retries, *, sleep_func, info, warn, error=None, suffix=None
    ):
        """Utility function for reporting retries"""
        if count > retries:
            if error:
                return (
                    error(f"{e}. Giving up after {count - 1} retries")
                    if count > 1
                    else error(str(e))
                )
            raise e
        ...
        warn(
            f'{e}. Retrying{format_field(suffix, None, " %s")} ({count}/{retries})...'
        )

        delay = (
            float_or_none(sleep_func(n=count - 1))
...
class FormatSorter:
        ...
        if key not in prop_obj:
            ...
            if key == "field":
                ...
                default = (
                    "order"
                    if type_ == "ordered"
                    else "float_string" if field else "ignore"
                )
            else:
                default = {
                    "type": "field",
    ...
    def _resolve_field_value(self, field, value, convert_none=False):
        ...
        if conversion == "string":
            ...
            order_list = (
                self._use_free_order
                and self._get_field_setting(field, "order_free")
            ) or self._get_field_setting(field, "order")
            use_regex = self._get_field_setting(field, "regex")
            list_length = len(order_list)
            empty_pos = (
                order_list.index("") if "" in order_list else list_length + 1
            )
    ...
    def evaluate_params(self, params, sort_extractor):
        ...
        sort_list = (
            tuple(
                ...
                if self._get_field_setting(field, "forced")
            )
            + (
                tuple()
                if params.get("format_sort_force", False)
                    ...
                    field
                    for field in self.default
                    if self._get_field_setting(field, "priority")
                )
            )
        ...
        for item in sort_list:
            ...
            if match is None:
                raise ExtractorError(
                    f'Invalid format sort string "{item}" given by extractor'
                )
            ...
            has_multiple_fields = (
                self._get_field_setting(field, "type") == "combined"
            )
            has_multiple_limits = (
                has_limit
                and has_multiple_fields
                and not self._get_field_setting(field, "same_limit")
            )

            fields = (
                self._get_field_setting(field, "field")
                if has_multiple_fields
                else (field,)
            )
            limits = (
                limit_text.split(":")
                if has_multiple_limits
                else (limit_text,) if has_limit else tuple()
            )
                ...
                add_item(
                    ...
                    (
                        ...
                        if i < limit_count
                            ...
                            limits[0]
                            if has_limit and not has_multiple_limits
                            else None
                        )
                    ),
                )
    ...
    def print_verbose_info(self, write_debug):
        if self._sort_user:
            write_debug(
                "Sort order given by user: {}".format(
                    ", ".join(self._sort_user)
                )
            )
        if self._sort_extractor:
            write_debug(
                "Sort order given by extractor: {}".format(
                    ", ".join(self._sort_extractor)
                )
            )
        write_debug(
                ...
                ", ".join(
                    [
                        "{}{}{}".format(
                            (
                                "+"
                                if self._get_field_setting(field, "reverse")
                                else ""
                            ),
                            field,
                            (
                                "{}{}({})".format(
                                    (
                                        "~"
                                        if self._get_field_setting(
                                            field, "closest"
                                        )
                                        else ":"
                                    ),
                                    self._get_field_setting(
                                        field, "limit_text"
                                    ),
                                    self._get_field_setting(field, "limit"),
                                )
                                if self._get_field_setting(field, "limit_text")
                                is not None
                                else ""
                            ),
                        )
    ...
    def _calculate_field_preference_from_value(
        self, format_, field, type_, value
    ):
        ...
        if type_ == "extractor":
            ...
            value = (
                0
                if (
                    (in_list is None or value in in_list)
                    and (not_in_list is None or value not in not_in_list)
                )
                else -1
            )
        elif type_ == "ordered":
        ...
        val_num = float_or_none(
            value, default=self._get_field_setting(field, "default")
        )
        is_num = (
            self._get_field_setting(field, "convert") != "string"
            ...
            if value is None
                ...
                if not is_num  # if a field has mixed strings and numbers, strings are sorted higher
                    ...
                    (
                        0,
                        -abs(value - limit),
                        value - limit if reverse else limit - value,
                    )
                    if closest
                    else (
                        (0, value, 0)
    ...
    def _calculate_field_preference(self, format_, field):
        type_ = self._get_field_setting(
            field, "type"
        )  # extractor, boolean, ordered, field, multiple
        ...
        if type_ == "multiple":
            ...
            value = self._get_field_setting(field, "function")(
                get_value(f) for f in actual_fields
            )
        else:
            value = get_value(field)
        return self._calculate_field_preference_from_value(
            format_, field, type_, value
        )
    ...
    def _fill_sorting_fields(format):
        ...
        if format.get("vcodec") == "none":
            format["audio_ext"] = (
                format["ext"] if format.get("acodec") != "none" else "none"
            )
        ...
        if (
            ...
            and re.match("[hx]265|he?vc?", format.get("vcodec") or "")
        ):
        ...
        if not format.get("vbr") and format.get("vcodec") != "none":
            format["vbr"] = (
                try_call(lambda: format["tbr"] - format["abr"]) or None
            )
        if not format.get("abr") and format.get("acodec") != "none":
            format["abr"] = (
                try_call(lambda: format["tbr"] - format["vbr"]) or None
            )
        if not format.get("tbr"):
            format["tbr"] = (
                try_call(lambda: format["vbr"] + format["abr"]) or None
            )

    def calculate_preference(self, format):
        self._fill_sorting_fields(format)
        return tuple(
            self._calculate_field_preference(format, field)
            for field in self._order
        )
