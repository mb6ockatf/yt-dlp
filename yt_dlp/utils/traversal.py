...
def traverse_obj(
    ...
    *paths,
    ...
    def apply_key(key, obj, is_last):
        ...
        if obj is None and traverse_string:
            ...
            if isinstance(obj, collections.abc.Mapping):
                result = obj.values()
            elif is_iterable_like(obj) or isinstance(
            ...
            iter_obj = (
                (k, _traverse_obj(obj, v, False, is_last))
            ...
            if isinstance(key, int) or casesense:
                ...
                result = next(
                    (
                        v
                        for k, v in obj.groupdict().items()
                        if casefold(k) == key
                    ),
                    None,
                )
            ...
            if is_iterable_like(
                obj, (collections.abc.Sequence, xml.etree.ElementTree.Element)
            ):
        ...
        elif isinstance(obj, xml.etree.ElementTree.Element) and isinstance(
            key, str
        ):
            ...
            def apply_specials(element):
                ...
                if special == "text()":
                    return element.text
                raise SyntaxError(
                    f"apply_specials is missing case for {special!r}"
                )
    ...
    def _traverse_obj(obj, path, allow_empty, test_type):
        ...
        return (
            results[0] if results else {} if allow_empty and is_dict else None
        )
    ...
    def func(value):
        if value is None:
            raise _RequiredError(
                f"Unable to extract {name}", expected=expected
            )
...
@typing.overload
def find_element(
    *, attr: str, value: str, tag: str | None = None, html=False, regex=False
): ...
...
@typing.overload
def find_element(
    *, id: str, tag: str | None = None, html=False, regex=False
): ...
...
def find_element(
    *,
    ...
    assert (
        tag or id or cls or (attr and value)
    ), "One of tag, id, cls or (attr AND value) is required"
    ANY_TAG = r"[\w:.-]+"
    ...
    if attr and value:
        ...
        func = (
            get_element_html_by_attribute if html else get_element_by_attribute
        )
        return functools.partial(
            func, attr, value, tag=tag or ANY_TAG, escape_value=not regex
        )
        ...
        return functools.partial(
            func, id, tag=tag or ANY_TAG, escape_value=not regex
        )
...
def find_elements(
    *, tag=None, cls=None, attr=None, value=None, html=False, regex=False
):
    # deliberately using `cls=` for ease of readability
    assert cls or (
        attr and value
    ), "One of cls or (attr AND value) is required"
    ...
    if attr and value:
        ...
        func = (
            get_elements_html_by_attribute
            if html
            else get_elements_by_attribute
        )
        return functools.partial(
            func, attr, value, tag=tag or r"[\w:.-]+", escape_value=not regex
        )
...
def get_first(obj, *paths, **kwargs):
    return traverse_obj(
        obj,
        *((..., *variadic(keys)) for keys in paths),
        **kwargs,
        get_all=False,
    )
