...
from ..networking._urllib import (
    ProxyHandler as PerRequestProxyHandler,
)  # noqa: F401
from ..networking._urllib import (
    ...
    def __exit__(self, type, value, traceback):
        try:
            return self.run_with_loop(
                self.conn.__aexit__(type, value, traceback), self.loop
            )
        finally:
            self.loop.close()
...
def sanitized_Request(url, *args, **kwargs):
    ...
    if auth_header is not None:
        headers = (
            args[1] if len(args) >= 2 else kwargs.setdefault("headers", {})
        )
...
def make_HTTPS_handler(params, **kwargs):
    return YoutubeDLHTTPSHandler(
        ...
        context=make_ssl_context(
            ...
            client_certificate_password=params.get(
