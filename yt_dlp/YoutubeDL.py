...
class YoutubeDL:
    ...
    def __init__(self, params=None, auto_init=True):
        ...
        self._out_files = Namespace(
            ...
            console=next(
                filter(supports_terminal_sequences, (sys.stderr, sys.stdout)),
                None,
            ),
        )
        ...
        def process_color_policy(stream):
            ...
            policy = (
                traverse_obj(
                    self.params, ("color", (stream_name, None), {str}, any)
                )
                or "auto"
            )
        ...
        check_deprecated(
            "usetitle", "--title", '-o "%(title)s-%(id)s.%(ext)s"'
        )
        ...
        self.archive = preload_download_archive(
            self.params.get("download_archive")
        )
        ...
        try:
            ie_names = orderedSet_from_options(
                self.params.get("allowed_extractors", ["default"]),
                {
                    "all": list(all_ies),
                    "default": [
                        name for name, ie in all_ies.items() if ie._ENABLED
                    ],
                },
                use_regex=True,
            )
            ...
            raise ValueError(
                f"Wrong regex for allowed_extractors: {e.pattern}"
            )
    ...
    def to_screen(self, message, skip_eol=False, quiet=None, only_once=False):
        ...
        self._write_string(
            "{}{}".format(
                self._bidi_workaround(message), ("" if skip_eol else "\n")
            ),
    ...
    def to_console_title(
        self, message=None, progress_state=None, percent=None
    ):
        if not self.params.get("consoletitle"):
            return

        if message:
            success = self._send_console_code(
                f"\033]0;{remove_terminal_sequences(message)}\007"
            )
            if (
                not success
                and os.name == "nt"
                and ctypes.windll.kernel32.GetConsoleWindow()
            ):
    ...
    def _format_text(
        ...
        *,
        ...
        return (
            ...
            else text if fallback is None else fallback
        )
    ...
    def raise_no_formats(self, info, forced=False, *, msg=None):
        ...
        ignored, expected = self.params.get("ignore_no_formats_error"), bool(
            msg
        )
        msg = (
            msg
            or (has_drm and "This video is DRM protected")
    ...
    def get_output_path(self, dir_type="", filename=None):
        ...
        assert isinstance(
            paths, dict
        ), '"paths" parameter must be a dictionary'
        path = os.path.join(
    ...
    def escape_outtmpl(outtmpl):
        ...
        return re.sub(
            ...
            lambda mobj: ("" if mobj.group("has_key") else "%")
            + mobj.group(0),
            outtmpl,
        )
    ...
    def prepare_outtmpl(self, outtmpl, info_dict, sanitize=False):
        ...
        if info_dict.get("resolution") is None:
            info_dict["resolution"] = self.format_resolution(
                info_dict, default=None
            )
        ...
        field_size_compat_map = {
            ...
            "playlist_autonumber": number_of_digits(
                info_dict.get("n_entries") or 0
            ),
        ...
        MATH_OPERATORS_RE = r"(?:{})".format(
            "|".join(map(re.escape, MATH_FUNCTIONS.keys()))
        )
            ...
            if offset_key:
                ...
                while offset_key:
                    ...
                    if operator is None:
                        ...
                        continue
                    item, multiplier = (
                        (item[1:], -1) if item[0] == "-" else (item, 1)
                    )
        ...
        if callable(sanitize):
            self.deprecation_warning(
                'Passing a callable "sanitize" to YoutubeDL.prepare_outtmpl is deprecated'
            )
        elif not sanitize:
            pass
        elif (
        ...
        def create_key(outer_mobj):
            ...
            while mobj:
                mobj = mobj.groupdict()
                default = (
                    mobj["default"] if mobj["default"] is not None else default
                )
            ...
            if value is None:
                ...
                value, fmt = (
                    delim.join(
                        map(str, variadic(value, allowed_types=(str, bytes)))
                    ),
                    str_fmt,
                )
            elif fmt[-1] == "j":  # json
                ...
                value, fmt = (
                    filename_sanitizer(
                        last_field, value, restricted="#" in flags
                    ),
                    str_fmt,
                )
            elif fmt[-1] == "c":
            ...
            key = "{}\0{}".format(
                key.replace("%", "%\0"), outer_mobj.group("format")
            )
            TMPL_DICT[key] = value
    ...
    def evaluate_outtmpl(self, outtmpl, info_dict, *args, **kwargs):
        outtmpl, info_dict = self.prepare_outtmpl(
            outtmpl, info_dict, *args, **kwargs
        )
    ...
    def _prepare_filename(self, info_dict, *, outtmpl=None, tmpl_type=None):
        ...
        try:
            ...
            if tmpl_type in ("", "temp"):
                final_ext, ext = self.params.get("final_ext"), info_dict.get(
                    "ext"
                )
                if (
                    final_ext
            ...
            if trim_file_name:
                ...
                filename = join_nonempty(
                    no_ext[:trim_file_name], *ext, delim="."
                )
    ...
    def prepare_filename(
        self, info_dict, dir_type="", *, outtmpl=None, warn=False
    ):
    ...
    def _match_entry(self, info_dict, incomplete=False, silent=False):
        ...
        def check_filter():
            if _type in ("playlist", "multi_video"):
                ...
                lambda: self.get_info_extractor(
                    info_dict["ie_key"]
                ).is_single_video(info_dict["url"])
            ):
                return
            ...
            if age_restricted(
                info_dict.get("age_limit"), self.params.get("age_limit")
            ):
    ...
    def _wait_for_video(self, ie_result={}):
        ...
        format_dur = (
            lambda dur: "%02d:%02d:%02d" % timetuple_from_msec(dur * 1000)[:-1]
        )
        last_msg = ""
        ...
        diff = try_get(
            ie_result, lambda x: x["release_timestamp"] - time.time()
        )
    ...
    def process_ie_result(self, ie_result, download=True, extra_info=None):
        ...
        if result_type in ("url", "url_transparent"):
            ie_result["url"] = sanitize_url(
                ie_result["url"],
                scheme=(
                    "http" if self.params.get("prefer_insecure") else "https"
                ),
            )
            if ie_result.get("original_url") and not extra_info.get(
                "original_url"
            ):
                extra_info = {
                    "original_url": ie_result["original_url"],
                    **extra_info,
                }
        ...
        if result_type == "video":
            ...
            if additional_urls:
                ...
                self.to_screen(
                    "[info] {}: {} additional URL(s) requested".format(
                        ie_result["id"], len(additional_urls)
                    )
                )
                self.write_debug(
                    'Additional URLs: "{}"'.format(
            ...
            new_result.update(
                filter_dict(
                    ie_result,
                    lambda k, v: v is not None and k not in exempted_fields,
                )
            )
    ...
    def _playlist_infodict(ie_result, strict=False, **kwargs):
        ...
        return {
            **info,
            "playlist_index": 0,
            "__last_playlist_index": max(
                ie_result.get("requested_entries") or (0, 0)
            ),
            "extractor": ie_result["extractor"],
            "extractor_key": ie_result["extractor_key"],
        }
    ...
    def __process_playlist(self, ie_result, download):
        ...
        extra = self._playlist_infodict(
            ie_result, n_entries=int_or_none(n_entries)
        )
        ...
        if write_playlist_files and not self.params.get("simulate"):
            ...
            self._write_thumbnails(
                "playlist",
                ie_result,
                self.prepare_filename(ie_copy, "pl_thumbnail"),
            )

        if lazy:
            if self.params.get("playlistreverse") or self.params.get(
                "playlistrandom"
            ):
                self.report_warning(
        ...
        failures = 0
        max_failures = self.params.get("skip_playlist_after_errors") or float(
            "inf"
        )
        for i, (playlist_index, entry) in enumerate(entries):
            ...
            entry["__x_forwarded_for_ip"] = ie_result.get(
        ...
        ie_result["entries"] = [
            e for _, e in resolved_entries if e is not NO_DEFAULT
        ]
    ...
    def _check_formats(self, formats):
        for f in formats:
            ...
            if success:
                ...
                self.to_screen(
                    "[info] Unable to download format {}. Skipping...".format(
                        f["format_id"]
                    )
                )
    ...
    def _default_format_spec(self, info_dict):
        ...
        if not prefer_best and not can_merge():
            ...
            evaluate_formats = lambda spec: self._select_formats(
                formats, self.build_format_selector(spec)
            )
        ...
        compat = (
            self.params.get("allow_multiple_audio_streams")
    ...
    def build_format_selector(self, format_spec):
        ...
        def _remove_unused_ops(tokens):
            ...
            ALLOWED_OPS = ("/", "+", ",", "(", ")")
            last_string, last_start, last_end, last_line = (
                None,
                None,
                None,
                None,
            ...
            for type_, string_, start, _, _ in tokens:
                ...
                if type_ == getattr(tokenize, "ENCODING", None):
                    ...
                    if string_ == ")":
                        ...
                        selector_2 = _parse_format_selection(
        ...
        def _merge(formats_pair):
            ...
            output_ext = get_compatible_ext(
                ...
                preferences=(
                    try_call(
                        lambda: self.params["merge_output_format"].split("/")
                    )
                    or (
                        self.params.get("prefer_free_formats")
                        and ("webm", "mkv")
                    )
                ),
            )
                ...
                new_dict.update(
                    {
                        ...
                        "stretched_ratio": the_only_video.get(
                            "stretched_ratio"
                        ),
                ...
                if format_spec == "all":
                    ...
                    if mobj is not None:
                        ...
                        _filter_f = (
                            ...
                            if format_type
                            and format_modified  # bv*, ba*, wv*, wa*
                            else (
                                (
                        ...
                        if format_spec in self._format_selection_exts["audio"]:
                            ...
                            format_spec in self._format_selection_exts["video"]
                        ):
                            filter_f = (
                                lambda f: f.get("ext") == format_spec
                                and f.get("acodec") != "none"
                            ...
                            format_spec
                            in self._format_selection_exts["storyboards"]
                        ):
                            filter_f = (
                                lambda f: f.get("ext") == format_spec
                                and f.get("acodec") == "none"
                            ...
                            filter_f = (
                                lambda f: f.get("format_id") == format_spec
                            )  # id
                    ...
                    def selector_function(ctx):
                        ...
                        matches = LazyList(
                            _check_formats(
                                matches[:: -1 if format_reverse else 1]
                            )
                        )
                        try:
        ...
        try:
            ...
            raise syntax_error(
                "Missing closing/opening brackets or parenthesis",
                (0, len(format_spec)),
            )
    ...
    def _calc_headers(self, info_dict, load_cookies=False):
        res = HTTPHeaderDict(
            self.params["http_headers"], info_dict.get("http_headers")
        )
        clean_headers(res)

        if load_cookies:  # For --load-info-json
            self._load_cookies(
                res.get("Cookie"), autoscope=info_dict["url"]
            )  # compat
    ...
    def _fill_common_fields(self, info_dict, final=True):
        ...
        for ts_key, date_key in (
            ...
            if (
                info_dict.get(date_key) is None
                and info_dict.get(ts_key) is not None
            ):
                # Working around out-of-range timestamp values (e.g. negative ones on Windows,
                # see http://bugs.python.org/issue1646728)
                with contextlib.suppress(ValueError, OverflowError, OSError):
                    upload_date = dt.datetime.fromtimestamp(
                        info_dict[ts_key], dt.timezone.utc
                    )
        ...
        for old_key, new_key in self._deprecated_multivalue_fields.items():
            if new_key in info_dict and old_key in info_dict:
                ...
                info_dict[old_key] = ", ".join(
                    v.replace(",", "\N{FULLWIDTH COMMA}") for v in new_value
                )
    ...
    def process_video_result(self, info_dict, download=True):
        ...
        if (
            info_dict.get("section_end")
            and info_dict.get("section_start") is not None
        ):
            info_dict["duration"] = round(
                info_dict["section_end"] - info_dict["section_start"], 3
            )
        if (info_dict.get("duration") or 0) <= 0 and info_dict.pop(
            "duration", None
        ):
            self.report_warning(
        ...
        dummy_chapter = {
            "end_time": 0,
            "start_time": info_dict.get("duration"),
        }
        for idx, (prev, current, next_) in enumerate(
            zip(
                (dummy_chapter, *chapters),
                chapters,
                (*chapters[1:], dummy_chapter),
            ),
            1,
        ):
        ...
        if not self.params.get("allow_unplayable_formats"):
            formats = [
                f
                for f in formats
                if not f.get("has_drm") or f["has_drm"] == "maybe"
            ]
        ...
        if not get_from_start:
            info_dict["title"] += " " + dt.datetime.now().strftime(
                "%Y-%m-%d %H:%M"
            )
        if info_dict.get("is_live") and formats:
            formats = [
                f
                for f in formats
                if bool(f.get("is_from_start")) == get_from_start
            ]
        ...
        common_exts = set(
            itertools.chain(*self._format_selection_exts.values())
        )
        for format_id, ambiguous_formats in formats_dict.items():
            ...
            for i, fmt in enumerate(ambiguous_formats):
                ...
                if (
                    fmt["format_id"] != fmt["ext"]
                    and fmt["format_id"] in common_exts
                ):
        ...
        if self.params.get("check_formats") is True:
            formats = LazyList(
                self._check_formats(formats[::-1]), reverse=True
            )
        ...
        if (
            self._match_entry(info_dict, incomplete=self._format_fields)
            is not None
        ):
            return info_dict
        ...
        while True:
            if interactive_format_selection:
                ...
                try:
                    format_selector = (
                        self.build_format_selector(req_format)
                        if req_format
                        else None
                    )
            ...
            formats_to_download = self._select_formats(
        ...
        requested_ranges = tuple(
            self.params.get("download_ranges", lambda *_: [{}])(
                info_dict, self
            )
        )
        ...
        if download:
            ...
            for fmt, chapter in itertools.product(
                ...
                new_info.update(fmt)
                offset, duration = info_dict.get(
                    "section_start"
                ) or 0, info_dict.get("duration") or float("inf")
                end_time = offset + min(
                    chapter.get("end_time", duration), duration
                )
                # duration may not be accurate. So allow deviations <1sec
                if (
                    end_time == float("inf")
                    or end_time > offset + duration + 1
                ):
                    end_time = None
            ...
            write_archive = {
                f.get("__write_download_archive", False)
    ...
    def process_subtitles(
        ...
        for lang in requested_langs:
            ...
            if formats is None:
                self.report_warning(
                    f"{lang} subtitles not available for {video_id}"
                )
                continue
            for ext in formats_preference:
                ...
                self.report_warning(
                    ...
                    "using {}. Use --list-subs for a list of available subtitles".format(
    ...
    def _forceprint(self, key, info_dict):
        ...
        if info_dict.get("requested_formats") is not None:
            ...
            info_copy["urls"] = info_dict["url"] + info_dict.get(
                "play_path", ""
            )
        ...
        def format_tmpl(tmpl):
            ...
            return "\n".join(
                map(
                    fmt.format,
                    [tmpl] if mobj.group("dict") else tmpl.split(","),
                )
            )
        ...
        for tmpl, file_tmpl in self.params["print_to_file"].get(key, []):
            ...
            if self._ensure_dir_exists(filename):
                with open(filename, "a", encoding="utf-8", newline="") as f:
                    f.write(
                        self.evaluate_outtmpl(tmpl, info_copy) + os.linesep
                    )
    ...
    def __forced_printings(self, info_dict, filename=None, incomplete=True):
        ...
        def print_field(field, actual_field=None, optional=False):
            ...
            if self.params.get(f"force{field}") and (
                info_copy.get(field) is not None
                or (not optional and not incomplete)
            ):
        ...
        if (
            self.params.get("forceduration")
            and info_copy.get("duration") is not None
        ):
    ...
    def existing_file(self, filepaths, *, default_overwrite=True):
        ...
        if existing_files and not self.params.get(
    ...
    def process_info(self, info_dict):
        ...
        def check_max_downloads():
            if self._num_downloads >= float(
                self.params.get("max_downloads") or "inf"
            ):
                raise MaxDownloadsReached
        ...
        if (
            self._write_description(
                "video",
                info_dict,
                self.prepare_filename(info_dict, "description"),
            )
            is None
        ):
            return
        ...
        if self.params.get("writelink"):
            link_type = (
                ...
                else "desktop" if sys.platform.startswith("linux") else "url"
            )
        ...
        if self.params.get("skip_download"):
            ...
            try:
                ...
                def existing_video_file(*filepaths):
                    ...
                    file = self.existing_file(
                        itertools.chain(
                            *zip(map(converted, filepaths), filepaths)
                        ),
                ...
                if info_dict.get("requested_formats") is not None:
                    ...
                    dl_filename = existing_video_file(
                        full_filename, temp_filename
                    )
                    ...
                    if dl_filename is not None:
                        ...
                        if self.params.get("allow_unplayable_formats"):
                            ...
                            if not self.params.get("ignoreerrors"):
                                ...
                                return
                            self.report_warning(
                                f"{msg}. The formats won't be merged"
                            )

                        if temp_filename == "-":
                            reason = (
                                "using a downloader other than ffmpeg"
                                if FFmpegFD.can_merge_formats(
                                    info_dict, self.params
                                )
                                else (
                        ...
                        for f in info_dict["requested_formats"]:
                            ...
                            if temp_filename != "-":
                                ...
                                f["filepath"] = fname
                                downloaded.append(fname)
                            partial_success, real_download = self.dl(
                                fname, new_info
                            )
            ...
            if success and full_filename != "-":
                ...
                def fixup():
                    ...
                    ext = info_dict.get("ext")
                    postprocessed_by_ffmpeg = info_dict.get(
                        "requested_formats"
                    ) or any(
                        (
                    ...
                    if not postprocessed_by_ffmpeg:
                        ...
                        ffmpeg_fixup(
                            ...
                            and (
                                info_dict.get("is_live")
                                or info_dict.get("is_dash_periods")
                            ),
                ...
                try:
                    replace_info_dict(
                        self.post_process(
                            dl_filename, info_dict, files_to_move
                        )
                    )
    ...
    def download_with_info_file(self, info_filename):
        with contextlib.closing(
            ...
            infos = [
                self.sanitize_info(
                    info, self.params.get("clean_infojson", True)
                )
                for info in variadic(json.loads("\n".join(f)))
            ]
        for info in infos:
            try:
                self.__download_wrapper(self.process_ie_result)(
                    info, download=True
                )
    ...
    def sanitize_info(info_dict, remove_private_keys=False):
        ...
        def filter_fn(obj):
            if isinstance(obj, dict):
                return {
                    k: filter_fn(v) for k, v in obj.items() if not reject(k, v)
                }
    ...
    def _delete_downloaded_files(self, *files_to_delete, info={}, msg=None):
        for filename in set(filter(None, files_to_delete)):
            ...
            if filename in info.get(
                "__files_to_move", []
            ):  # NB: Delete even if None
    ...
    def _list_format_headers(self, *headers):
        if self.params.get("listformats_table", True) is not False:
            return [
                self._format_out(header, self.Styles.HEADERS)
    ...
    def _get_formats(self, info_dict):
        if info_dict.get("formats") is None:
            if (
                info_dict.get("url")
                and info_dict.get("_type", "video") == "video"
            ):
                return [info_dict]
            return []
    ...
    def render_formats_table(self, info_dict):
        ...
        table = [
            [
                ...
                format_field(
                    f,
                    func=self.format_resolution,
                    ignore=("audio only", "images"),
                ),
                format_field(f, "fps", "\t%d", func=round),
                format_field(
                    f, "dynamic_range", "%s", ignore=(None, "SDR")
                ).replace("HDR", ""),
                ...
                (
                    ...
                    or format_field(
                        filesize_from_tbr(
                            f.get("tbr"), info_dict.get("duration")
                        ),
                        None,
                ...
                join_nonempty(
                    ...
                    join_nonempty(
                        (
                            self._format_out(
                                "UNSUPPORTED", self.Styles.BAD_FORMAT
                            )
                        ...
                        (
                            ...
                            if f.get("has_drm") == "maybe"
                                ...
                                self._format_out("DRM", self.Styles.BAD_FORMAT)
                                if f.get("has_drm")
                                else None
                            )
                        ),
                        ...
                        format_field(
                            f, "container", ignore=(None, f.get("ext"))
                        ),
                        delim=", ",
                    ),
                    delim=" ",
    ...
    def print_debug_header(self):
        ...
        def get_encoding(stream):
            ret = str(
                getattr(
                    stream, "encoding", f"missing ({type(stream).__name__})"
                )
            )
            additional_info = []
        ...
        write_debug(
            join_nonempty(
                ...
                _make_label(
                    ORIGIN,
                    CHANNEL.partition("@")[2] or __version__,
                    __version__,
                ),
        ...
        if self.params["compat_opts"]:
            write_debug(
                "Compatibility options: {}".format(
                    ", ".join(self.params["compat_opts"])
                )
            )
        ...
        exe_versions, ffmpeg_features = (
            FFmpegPostProcessor.get_versions_and_features(self)
        )
        ffmpeg_features = {key for key, val in ffmpeg_features.items() if val}
        if ffmpeg_features:
            exe_versions["ffmpeg"] += " ({})".format(
                ",".join(sorted(ffmpeg_features))
            )
        ...
        for plugin_type, plugins in (
            ...
            if not display_list:
                continue
            write_debug(
                f'{plugin_type} Plugins: {", ".join(sorted(display_list))}'
            )
    ...
    def cookiejar(self):
        ...
        try:
            ...
            self.report_error(
                ...
                tb="".join(
                    traceback.format_exception(
                        None, cause, cause.__traceback__
                    )
                ),
            )
            raise
    ...
    def _opener(self):
        ...
        return handler._get_instance(
            cookiejar=self.cookiejar, proxies=self.proxies
        )
        ...
        try:
            ...
            for ue in e.unsupported_errors:
                ...
                if (
                    ...
                    r"unsupported (?:extensions: impersonate|impersonate target)",
                    ue.msg.lower(),
                ):
                    raise RequestError(
    ...
    def build_request_director(self, handlers, preferences=None):
        ...
        for handler in handlers:
            director.add_handler(
                handler(
                    ...
                    proxies=proxies,
                    prefer_system_certs="no-certifi"
                    in self.params["compat_opts"],
        ...
        if "prefer-legacy-http-handler" in self.params["compat_opts"]:
            director.preferences.add(
                lambda rh, _: 500 if rh.RH_KEY == "Urllib" else 0
            )
        return director

    @functools.cached_property
    def _request_director(self):
        return self.build_request_director(
    ...
    def _write_info_json(self, label, ie_result, infofn, overwrite=None):
        ...
        if not self.params.get("writeinfojson"):
            ...
            self.to_screen(
                f"[info] {label.title()} metadata is already present"
            )
            return "exists"
        ...
        try:
            write_json_file(
                self.sanitize_info(
                    ie_result, self.params.get("clean_infojson", True)
                ),
                infofn,
            )
            return True
            ...
            self.report_error(
                f"Cannot write {label} metadata to JSON file {infofn}"
            )
            return None
    ...
    def _write_description(self, label, ie_result, descfn):
        ...
        if not self.params.get("writedescription"):
            ...
            return None
        elif not self.params.get("overwrites", True) and os.path.exists(
            descfn
        ):
            self.to_screen(
                f"[info] {label.title()} description is already present"
            )
            ...
            try:
                self.to_screen(
                    f"[info] Writing {label} description to: {descfn}"
                )
                with open(descfn, "w", encoding="utf-8") as descfile:
                    descfile.write(ie_result["description"])
            except OSError:
                self.report_error(
                    f"Cannot write {label} description file {descfn}"
                )
                return None
        return True
    ...
    def _write_subtitles(self, info_dict, filename):
        ...
        if not (
            self.params.get("writesubtitles")
            or self.params.get("writeautomaticsub")
        ):
        ...
        for sub_lang, sub_info in subtitles.items():
            ...
            existing_sub = self.existing_file(
            ...
            self.to_screen(
                f"[info] Writing video subtitles to: {sub_filename}"
            )
    ...
    def _write_thumbnails(
        ...
        if write_all or self.params.get("writethumbnail", False):
            ...
            if not thumbnails:
                self.to_screen(
                    f"[info] There are no {label} thumbnails to download"
                )
                return ret
        ...
        for idx, t in list(enumerate(thumbnails))[::-1]:
            ...
            existing_thumb = self.existing_file(
