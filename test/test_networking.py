...
class HTTPTestRequestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/video.html":
            ...
            self.send_header(
                "Location",
                f"http://127.0.0.1:{http_server_port(self.server)}/a/b/./../../headers",
            )
            self.send_header("Content-Length", "0")
            self.end_headers()
            ...
            self.send_header(
                "Location",
                f"http://localhost:{self.connection.getsockname()[1]}/headers",
            )
            self.send_header("Content-Length", "0")
            self.end_headers()
            ...
            new_url = (
                f"http://127.0.0.1:{http_server_port(self.server)}/中文.html"
            )
            self.send_response(301)
            ...
            for encoding in filter(
                None, (e.strip() for e in encodings.split(","))
            ):
...
class TestRequestHandlerBase:
    ...
    def setup_class(cls):
        ...
        cls.http_server_thread = threading.Thread(
            target=cls.http_httpd.serve_forever
        )
        ...
        cls.https_httpd.socket = sslctx.wrap_socket(
            cls.https_httpd.socket, server_side=True
        )
        cls.https_port = http_server_port(cls.https_httpd)
        cls.https_server_thread = threading.Thread(
            target=cls.https_httpd.serve_forever
        )
...
class TestHTTPRequestHandler(TestRequestHandlerBase):
    ...
    def test_verify_cert(self, handler):
        with handler() as rh:
            with pytest.raises(CertificateVerifyError):
                validate_and_send(
                    rh, Request(f"https://127.0.0.1:{self.https_port}/headers")
                )
    ...
    def test_ssl_error(self, handler):
        ...
        sslctx = ssl.SSLContext(ssl.PROTOCOL_TLS_SERVER)
        https_httpd.socket = sslctx.wrap_socket(
            https_httpd.socket, server_side=True
        )
        https_port = http_server_port(https_httpd)
        https_server_thread = threading.Thread(
            target=https_httpd.serve_forever
        )
        ...
        with handler(verify=False) as rh:
            with pytest.raises(
                SSLError, match=r"(?i)ssl(?:v3|/tls).alert.handshake.failure"
            ) as exc_info:
                validate_and_send(
                    rh, Request(f"https://127.0.0.1:{https_port}/headers")
                )
    ...
    def test_legacy_ssl_extension(self, handler):
        ...
        with handler(verify=False) as rh:
            res = validate_and_send(
                rh,
                Request(
                    f"https://127.0.0.1:{https_port}/headers",
                    extensions={"legacy_ssl": True},
                ),
            )
    ...
    def test_percent_encode(self, handler):
        with handler() as rh:
            ...
            res = validate_and_send(
                rh, Request(f"http://127.0.0.1:{self.http_port}/中文.html")
            )
            assert res.status == 200
            res.close()
            # don't normalize existing percent encodings
            res = validate_and_send(
                rh, Request(f"http://127.0.0.1:{self.http_port}/%c7%9f")
            )
            assert res.status == 200
            res.close()
    ...
    @pytest.mark.skip_handler(
        "CurlCFFI", "not supported by curl-cffi (non-standard)"
    )
    def test_unicode_path_redirection(self, handler):
        with handler() as rh:
            r = validate_and_send(
                rh,
                Request(
                    f"http://127.0.0.1:{self.http_port}/302-non-ascii-redirect"
                ),
            )
            assert (
                r.url
                == f"http://127.0.0.1:{self.http_port}/%E4%B8%AD%E6%96%87.html"
            )
            r.close()
            ...
            for bad_status in (400, 500, 599, 302):
                with pytest.raises(HTTPError):
                    validate_and_send(
                        rh,
                        Request(
                            f"http://127.0.0.1:{self.http_port}/gen_{bad_status}"
                        ),
                    )

            # Should not raise an error
            validate_and_send(
                rh, Request(f"http://127.0.0.1:{self.http_port}/gen_200")
            ).close()
    ...
    def test_response_url(self, handler):
        with handler() as rh:
            ...
            res = validate_and_send(
                rh, Request(f"http://127.0.0.1:{self.http_port}/redirect_301")
            )
            assert res.url == f"http://127.0.0.1:{self.http_port}/method"
            res.close()
            res2 = validate_and_send(
                rh, Request(f"http://127.0.0.1:{self.http_port}/gen_200")
            )
    ...
    def test_redirect(self, handler, redirect_status, method, expected):
        with handler() as rh:
            ...
            assert expected[2] == (
                "content-length" in headers.decode().lower()
            )
        ...
        with handler() as rh:
            with pytest.raises(HTTPError, match="redirect loop"):
                validate_and_send(
                    rh,
                    Request(
                        f"http://127.0.0.1:{self.http_port}/redirect_loop"
                    ),
                )
    ...
    def test_incompleteread(self, handler):
        with handler(timeout=2) as rh:
            with pytest.raises(
                IncompleteRead, match="13 bytes read, 234221 more expected"
            ):
                validate_and_send(
                    rh,
                    Request(
                        f"http://127.0.0.1:{self.http_port}/incompleteread"
                    ),
                ).read()
    ...
    def test_cookie_sync_only_cookiejar(self, handler):
        ...
        with handler() as rh:
            validate_and_send(
                rh,
                Request(
                    f"http://127.0.0.1:{self.http_port}/get_cookie",
    ...
    def test_cookie_sync_delete_cookie(self, handler):
        ...
        with handler(cookiejar=cookiejar) as rh:
            validate_and_send(
                rh, Request(f"http://127.0.0.1:{self.http_port}/get_cookie")
            )
            data = validate_and_send(
                rh, Request(f"http://127.0.0.1:{self.http_port}/headers")
            ).read()
    ...
    def test_headers(self, handler):
        ...
        with handler(
            headers=HTTPHeaderDict({"test1": "test", "test2": "test2"})
        ) as rh:
            # Global Headers
            data = (
                validate_and_send(
            ...
            data = (
                validate_and_send(
                    ...
                    Request(
                        f"http://127.0.0.1:{self.http_port}/headers",
                        headers={"test2": "changed", "test3": "test3"},
                    ),
                )
                .read()
                .lower()
    ...
    def test_read_timeout(self, handler):
        ...
        with handler(timeout=0.1) as rh:
            ...
            validate_and_send(
                rh,
                Request(
                    f"http://127.0.0.1:{self.http_port}/timeout_1",
                    extensions={"timeout": 4},
                ),
            )
        ...
        with handler(source_address=source_address) as rh:
            data = (
                validate_and_send(
                    rh,
                    Request(
                        f"http://127.0.0.1:{self.http_port}/source_address"
                    ),
                )
                .read()
                .decode()
        ...
        with handler() as rh:
            data = (
                validate_and_send(
                    rh,
                    Request(
                        f"http://localhost:{self.http_port}/trailing_garbage"
                    ),
                )
                .read()
                .decode()
    ...
    def test_request_disable_proxy(self, handler):
        for proxy_proto in handler._SUPPORTED_PROXY_SCHEMES or ["http"]:
            ...
            with handler(
                proxies={"http": f"{proxy_proto}://10.255.255.255"}, timeout=5
            ) as rh:
    ...
    @pytest.mark.skip_handlers_if(
        lambda _, handler: Features.NO_PROXY
        not in handler._SUPPORTED_FEATURES,
    ...
    @pytest.mark.skip_handlers_if(
        lambda _, handler: Features.ALL_PROXY
        not in handler._SUPPORTED_FEATURES,
    ...
    def test_allproxy(self, handler):
        ...
        with handler(
            proxies={"all": "http://10.255.255.255"}, timeout=0.1
        ) as rh:
            with pytest.raises(TransportError):
                validate_and_send(
                    rh, Request(f"http://127.0.0.1:{self.http_port}/headers")
        ...
        with handler(timeout=0.1) as rh:
            with pytest.raises(TransportError):
                validate_and_send(
                    ...
                    Request(
                        f"http://127.0.0.1:{self.http_port}/headers",
                        proxies={"all": "http://10.255.255.255"},
                    ),
                ).close()

    @pytest.mark.skip_handlers_if(
        lambda _, handler: handler not in ["Urllib", "CurlCFFI"],
        "handler does not support keep_header_casing",
    )
...
class TestClientCertificate:
    ...
    def setup_class(cls):
        ...
        sslctx.load_cert_chain(certfn, None)
        cls.httpd.socket = sslctx.wrap_socket(
            cls.httpd.socket, server_side=True
        )
    ...
    def _run_test(self, handler, **handler_kwargs):
        with handler(
            ...
            **handler_kwargs,
            ...
            validate_and_send(
                rh, Request(f"https://127.0.0.1:{self.port}/video.html")
            ).read().decode()
        ...
        self._run_test(
            ...
            client_cert={
                "client_certificate": os.path.join(
                    self.certdir, "clientwithkey.crt"
                ),
            },
        )
    ...
    def test_certificate_nocombined_nopass(self, handler):
        self._run_test(
            ...
            client_cert={
                ...
                "client_certificate_key": os.path.join(
                    self.certdir, "client.key"
                ),
            },
        )
    ...
    def test_certificate_combined_pass(self, handler):
        self._run_test(
            ...
            client_cert={
                "client_certificate": os.path.join(
                    self.certdir, "clientwithencryptedkey.crt"
                ),
    ...
    def test_certificate_nocombined_pass(self, handler):
        self._run_test(
            ...
            client_cert={
                "client_certificate": os.path.join(self.certdir, "client.crt"),
                "client_certificate_key": os.path.join(
                    self.certdir, "clientencrypted.key"
                ),
                "client_certificate_password": "foobar",
            },
        )
...
class TestHTTPImpersonateRequestHandler(TestRequestHandlerBase):
    def test_supported_impersonate_targets(self, handler):
        with handler(headers=std_headers) as rh:
            ...
            for target in rh.supported_targets:
                ...
                assert res.status == 200
                assert (
                    std_headers["user-agent"].lower()
                    not in res.read().decode().lower()
                )
    ...
    def test_response_extensions(self, handler):
        with handler() as rh:
            for target in rh.supported_targets:
                request = Request(
                    f"http://127.0.0.1:{self.http_port}/gen_200",
                    extensions={"impersonate": target},
                )
                res = validate_and_send(rh, request)
                assert res.extensions["impersonate"] == rh._get_request_target(
                    request
                )
    ...
    def test_http_error_response_extensions(self, handler):
        with handler() as rh:
            for target in rh.supported_targets:
                request = Request(
                    f"http://127.0.0.1:{self.http_port}/gen_404",
                    extensions={"impersonate": target},
                )
                try:
                    validate_and_send(rh, request)
...
class TestRequestHandlerMisc:
    ...
    def test_wrap_request_errors(self):
        ...
        with TestRequestHandler(logger=FakeLogger()) as rh:
            with pytest.raises(
                UnsupportedRequest, match="test error"
            ) as exc_info:
                rh.validate(
                    Request("http://example.com", headers={"x-fail": "1"})
                )
...
class TestUrllibRequestHandler(TestRequestHandlerBase):
        ...
        def get_response():
            with handler() as rh:
                ...
                try:
                    validate_and_send(
                        rh,
                        Request(f"http://127.0.0.1:{self.http_port}/gen_404"),
                    )
                except HTTPError as e:
                    return e.response
    ...
    def test_httplib_validation_errors(
        self, handler, req, match, version_check
    ):
...
class TestRequestsRequestHandler(TestRequestHandlerBase):
        ...
        [
            ...
            (
                lambda: requests.exceptions.SSLError(
                    "12[CERTIFICATE_VERIFY_FAILED]34"
                ),
                CertificateVerifyError,
            ),
    ...
    def test_request_error_mapping(
        self, handler, monkeypatch, raised, expected
    ):
        with handler() as rh:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                lambda: urllib3.exceptions.ReadTimeoutError(None, None, None),
                TransportError,
                None,
            ),
            ...
            (
                lambda: urllib3.exceptions.HTTPError(),
                TransportError,
                None,
            ),  # catch-all
            (
                lambda: urllib3.exceptions.ProtocolError(
                    "error",
                    http.client.IncompleteRead(partial=b"abc", expected=4),
                ),
                IncompleteRead,
            ...
            (
                lambda: urllib3.exceptions.ProtocolError(
                    "error",
                    urllib3.exceptions.IncompleteRead(partial=3, expected=5),
                ),
                IncompleteRead,
    ...
    def test_response_error_mapping(
        self, handler, monkeypatch, raised, expected, match
    ):
...
class TestCurlCFFIRequestHandler(TestRequestHandlerBase):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                {"impersonate": ImpersonateTarget("chrome", "99")},
                {"impersonate": ImpersonateTarget("chrome", "110")},
            ),
        ],
    )
        ...
        [
            (
                lambda: curl_cffi.requests.errors.RequestsError(
                    "", code=curl_cffi.const.CurlECode.PARTIAL_FILE
                ),
                IncompleteRead,
                None,
            ),
            (
                lambda: curl_cffi.requests.errors.RequestsError(
                    "", code=curl_cffi.const.CurlECode.OPERATION_TIMEDOUT
                ),
                TransportError,
                None,
            ),
            (
                lambda: curl_cffi.requests.errors.RequestsError(
                    "", code=curl_cffi.const.CurlECode.RECV_ERROR
                ),
                TransportError,
                None,
            ),
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                lambda: curl_cffi.requests.errors.RequestsError(
                    "", code=curl_cffi.const.CurlECode.SSL_CONNECT_ERROR
                ),
                SSLError,
                None,
            ),
            (
                lambda: curl_cffi.requests.errors.RequestsError(
                    "", code=curl_cffi.const.CurlECode.TOO_MANY_REDIRECTS
                ),
                HTTPError,
                None,
            ),
            (
                lambda: curl_cffi.requests.errors.RequestsError(
                    "", code=curl_cffi.const.CurlECode.PROXY
                ),
                ProxyError,
                None,
            ),
...
class TestRequestHandlerValidation:
    ...
    EXTENSION_TESTS = [
        ...
        (
            ...
            [
                ...
                (
                    {"impersonate": ImpersonateTarget(None, None, None, None)},
                    False,
                ),
        ...
        (
            ...
            [
                ...
                (
                    {"somerandom": "test"},
                    False,
                ),  # but any extension is allowed through
            ],
        ),
        (
            "Websockets",
    ...
    def test_no_proxy(self, handler, fail, scheme):
        run_validation(
            handler,
            fail,
            Request(f"{scheme}://", proxies={"no": "127.0.0.1,github.com"}),
        )
        run_validation(
            handler,
            fail,
            Request(f"{scheme}://"),
            proxies={"no": "127.0.0.1,github.com"},
        )
    ...
    def test_empty_proxy(self, handler, scheme):
        run_validation(
            handler, False, Request(f"{scheme}://", proxies={scheme: None})
        )
        run_validation(
            handler, False, Request(f"{scheme}://"), proxies={scheme: None}
        )
    ...
    @pytest.mark.parametrize(
        "proxy_url", ["//example.com", "example.com", "127.0.0.1", "/a/b/c"]
    )
    @pytest.mark.parametrize(
        "handler,scheme",
        [
    ...
    def test_invalid_proxy_url(self, handler, scheme, proxy_url):
        run_validation(
            handler,
            UnsupportedRequest,
            Request(f"{scheme}://", proxies={scheme: proxy_url}),
        )
    ...
    def test_url_scheme(self, handler, scheme, fail, handler_kwargs):
        run_validation(
            handler, fail, Request(f"{scheme}://"), **(handler_kwargs or {})
        )
    ...
    def test_proxy_key(self, handler, scheme, proxy_key, proxy_scheme, fail):
        run_validation(
            ...
            Request(
                f"{scheme}://",
                proxies={proxy_key: f"{proxy_scheme}://example.com"},
            ),
        )
        run_validation(
            handler,
    ...
    def test_proxy_scheme(self, handler, req_scheme, scheme, fail):
        run_validation(
            ...
            Request(
                f"{req_scheme}://",
                proxies={req_scheme: f"{scheme}://example.com"},
            ),
        )
        run_validation(
            handler,
            fail,
            Request(f"{req_scheme}://"),
            proxies={req_scheme: f"{scheme}://example.com"},
        )
    ...
    def test_invalid_request_type(self):
        ...
        for method in (rh.validate, rh.send):
            with pytest.raises(
                TypeError, match="Expected an instance of Request"
            ):
...
class TestRequestDirector:
    ...
    def test_handler_operations(self):
        ...
        assert (
            director.handlers.get(AnotherFakeRH.RH_KEY).RH_KEY
            == AnotherFakeRH.RH_KEY
        )
    ...
    def test_unsupported_handlers(self):
        class SupportedRH(RequestHandler):
            ...
            def _send(self, request: Request):
                return Response(
                    fp=io.BytesIO(b"supported"), headers={}, url=request.url
                )
    ...
    def test_preference(self):
        ...
        def some_preference(rh, request):
            return (
                0
                if not isinstance(rh, SomeRH)
                else 100 if "prefer" in request.headers else -1
            )
        ...
        assert (
            director.send(Request("http://", headers={"prefer": "1"})).read()
            == b"supported"
        )
    ...
    def test_close(self, monkeypatch):
        ...
        monkeypatch.setattr(
            director.handlers[FakeRH.RH_KEY], "close", mock_close
        )
        director.close()
        assert called
...
class TestYoutubeDLNetworking:
    ...
    def build_handler(ydl, handler: RequestHandler = FakeRH):
        return ydl.build_request_director([handler]).handlers.get(
            handler.RH_KEY
        )
    ...
    def test_file_urls_error(self):
        ...
        with FakeYDL() as ydl:
            with pytest.raises(
                RequestError, match=r"file:// URLs are disabled by default"
            ):
                ydl.urlopen("file://")
    ...
    def test_websocket_unavailable_error(self, scheme):
        with AllUnsupportedRHYDL() as ydl:
            with pytest.raises(
                RequestError, match=r"This request requires WebSocket support"
            ):
    ...
    def test_legacy_server_connect_error(self):
        with FakeRHYDL() as ydl:
            for error in (
                ...
                with pytest.raises(
                    RequestError, match=r"Try using --legacy-server-connect"
                ):
    ...
    def test_pass_impersonate_param(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda cls, handlers, preferences=None: brh(cls, handlers=[IRH]),
        )
    ...
    def test_get_impersonate_targets(self):
        ...
        for target_client in ("abc", "xyz", "asd"):
            ...
            class TestRH(ImpersonateRequestHandler):
                ...
                _SUPPORTED_URL_SCHEMES = ("http",)
                _SUPPORTED_IMPERSONATE_TARGET_MAP = {
                    ImpersonateTarget(target_client): "test"
                }
        ...
        with FakeYDL() as ydl:
            ...
            assert not ydl._impersonate_target_available(
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                "/bad/proxy",
                "/bad/proxy",
            ),  # clean_proxies should ignore bad proxies
        ],
    )
    def test_clean_proxy(self, proxy_key, proxy_url, expected, monkeypatch):
        ...
        with FakeRHYDL() as ydl:
            req = ydl.urlopen(
                Request("test://", proxies={proxy_key: proxy_url})
            ).request
    ...
    def test_clean_proxy_header(self):
        with FakeRHYDL() as ydl:
            req = ydl.urlopen(
                Request("test://", headers={"ytdl-request-proxy": "//foo.bar"})
            ).request
            assert "ytdl-request-proxy" not in req.headers
            assert req.proxies == {"all": "http://foo.bar"}

        with FakeYDL(
            {"http_headers": {"ytdl-request-proxy": "//foo.bar"}}
        ) as ydl:
            rh = self.build_handler(ydl)
    ...
    def test_clean_header(self):
        ...
        with FakeYDL(
            {"http_headers": {"Ytdl-socks-proxy": "socks://localhost:1080"}}
        ) as ydl:
            rh = self.build_handler(ydl)
...
class TestRequest:
    ...
    def test_headers(self):
        ...
        assert req.headers == HTTPHeaderDict(
    ...
    def test_content_length_header(self):
        req = Request(
            "http://example.com", headers={"Content-Length": "0"}, data=b""
        )
    ...
    def test_proxies(self):
        req = Request(
            url="http://example.com", proxies={"http": "http://127.0.0.1:8080"}
        )
...
class TestImpersonateTarget:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ImpersonateTarget("abc", None, None, None),
                ImpersonateTarget("abc", "120", None, None),
                True,
                False,
            ),
            (
                ImpersonateTarget("abc", None, "xyz", "test"),
                ImpersonateTarget("abc", "120", "xyz", None),
                True,
                False,
            ),
            (
