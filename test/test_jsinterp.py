...
class TestJSInterpreter(unittest.TestCase):
    ...
    def test_date(self):
        ...
        self._test(
            jsi, 86000, args=["Wednesday 31 December 1969 18:01:26 MDT"]
        )
    ...
    def test_for_loop(self):
        self._test(
            "function f() { a=0; for (i=0; i-10; i++) {a++} return a }", 10
        )
    ...
    def test_literal_list(self):
        self._test(
            'function f() { return [1, 2, "asdf", [5, 6, 7]][3] }', [5, 6, 7]
        )
    ...
    def test_null(self):
        ...
        self._test(
            "function f() { return [null >= 0, null <= 0]; }", [True, True]
        )
    ...
    def test_replace(self):
        self._test(
            'function f() { let a="data-name".replace("data-", ""); return a }',
            "name",
        )
        self._test(
            'function f() { let a="data-name".replace(new RegExp("^.+-"), ""); return a; }',
            "name",
        )
        self._test(
            'function f() { let a="data-name".replace(/^.+-/, ""); return a; }',
            "name",
        )
        self._test(
            'function f() { let a="data-name".replace(/a/g, "o"); return a; }',
            "doto-nome",
        )
        self._test(
            'function f() { let a="data-name".replaceAll("a", "o"); return a; }',
            "doto-nome",
        )
    ...
    def test_slice(self):
        ...
        self._test(
            "function f(){return [0, 1, 2, 3, 4, 5, 6, 7, 8].slice(0, 1)}", [0]
        )
        self._test(
