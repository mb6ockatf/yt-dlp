...
class TestWgetFD(unittest.TestCase):
    def test_make_cmd(self):
        with FakeYDL() as ydl:
            ...
            self.assertNotIn(
                "--load-cookies", downloader._make_cmd("test", TEST_INFO)
            )
...
class TestCurlFD(unittest.TestCase):
    def test_make_cmd(self):
        with FakeYDL() as ydl:
            ...
            self.assertNotIn(
                "--cookie", downloader._make_cmd("test", TEST_INFO)
            )
            # Test cookie header is added
            ydl.cookiejar.set_cookie(http.cookiejar.Cookie(**TEST_COOKIE))
            self.assertIn("--cookie", downloader._make_cmd("test", TEST_INFO))
            self.assertIn(
                "test=ytdlp", downloader._make_cmd("test", TEST_INFO)
            )
...
class TestAria2cFD(unittest.TestCase):
    def test_make_cmd(self):
        with FakeYDL() as ydl:
            ...
            self.assertIn(
                f"--load-cookies={downloader._cookies_tempfile}", cmd
            )
