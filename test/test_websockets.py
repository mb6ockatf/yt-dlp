        ...
        if isinstance(message, bytes):
            ...
            if message == "headers":
                return websocket.send(
                    json.dumps(dict(websocket.request.headers.raw_items()))
                )
            elif message == "path":
...
def ws_validate_and_send(rh, req):
    ...
    for i in range(max_tries):
        try:
            ...
            if i < (
                max_tries - 1
            ) and "connection closed during handshake" in str(e):
...
@pytest.mark.skipif(
    not websockets,
    reason="websockets must be installed to test websocket request handlers",
)
...
class TestWebsSocketRequestHandlerConformance:
    ...
    def test_ssl_error(self, handler):
        with handler(verify=False) as rh:
            with pytest.raises(
                SSLError, match=r"ssl(?:v3|/tls) alert handshake failure"
            ) as exc_info:
    ...
    def test_legacy_ssl_extension(self, handler):
        with handler(verify=False) as rh:
            ws = ws_validate_and_send(
                rh,
                Request(self.legacy_wss_host, extensions={"legacy_ssl": True}),
            )
            assert ws.status == 101
            ws.close()
    ...
    def test_remove_dot_segments(self, handler):
        with handler() as rh:
            ...
            ws = ws_validate_and_send(
                rh, Request(f"{self.ws_base_url}/a/b/./../../test")
            )
            assert ws.status == 101
            ws.send("path")
            assert ws.recv() == "/test"
    ...
    @pytest.mark.parametrize(
        "status", (200, 204, 301, 302, 303, 400, 500, 511)
    )
    def test_raise_http_error(self, handler, status):
        with handler() as rh:
            with pytest.raises(HTTPError) as exc_info:
                ws_validate_and_send(
                    rh, Request(f"{self.ws_base_url}/gen_{status}")
                )
    ...
    def test_read_timeout(self, handler, params, extensions):
        with handler(**params) as rh:
            with pytest.raises(TransportError):
                ws_validate_and_send(
                    rh, Request(self.ws_base_url, extensions=extensions)
                )
    ...
    def test_cookies(self, handler):
        ...
        with handler(cookiejar=cookiejar) as rh:
            ...
            ws.send("headers")
            assert (
                HTTPHeaderDict(json.loads(ws.recv()))["cookie"] == "test=ytdlp"
            )
            ws.close()
        ...
        with handler() as rh:
            ...
            ws = ws_validate_and_send(
                rh,
                Request(self.ws_base_url, extensions={"cookiejar": cookiejar}),
            )
            ws.send("headers")
            assert (
                HTTPHeaderDict(json.loads(ws.recv()))["cookie"] == "test=ytdlp"
    ...
    @pytest.mark.skip_handler(
        "Websockets", "Set-Cookie not supported by websockets"
    )
        ...
        (
            {
                "client_certificate": os.path.join(
                    MTLS_CERT_DIR, "clientwithkey.crt"
                )
            },
            {
                "client_certificate": os.path.join(
                    MTLS_CERT_DIR, "client.crt"
                ),
                "client_certificate_key": os.path.join(
                    MTLS_CERT_DIR, "client.key"
                ),
            },
            {
                "client_certificate": os.path.join(
                    MTLS_CERT_DIR, "clientwithencryptedkey.crt"
                ),
            ...
            },
            {
                ...
                "client_certificate_key": os.path.join(
                    MTLS_CERT_DIR, "clientencrypted.key"
                ),
                "client_certificate_password": "foobar",
            },
        ),
    ...
    def test_request_disable_proxy(self, handler):
        for proxy_proto in handler._SUPPORTED_PROXY_SCHEMES or ["ws"]:
            ...
            with handler(
                proxies={"ws": f"{proxy_proto}://10.255.255.255"}, timeout=5
            ) as rh:
                # When a proxy is explicitly set to None for the request
                ws = ws_validate_and_send(
                    rh, Request(self.ws_base_url, proxies={"http": None})
                )
        ...
        for proxy_proto in handler._SUPPORTED_PROXY_SCHEMES or ["ws"]:
            ...
            with handler(
                ...
                for no_proxy in (
                    ...
                    ws = ws_validate_and_send(
                        rh, Request(self.ws_base_url, proxies={"no": no_proxy})
                    )
    ...
    def test_allproxy(self, handler):
        supported_proto = traverse_obj(
            handler._SUPPORTED_PROXY_SCHEMES, 0, default="ws"
        )
        ...
        with handler(
            proxies={"all": f"{supported_proto}://10.255.255.255"}, timeout=0.1
        ) as rh:
            with pytest.raises(TransportError):
        ...
        with handler(timeout=0.1) as rh:
            with pytest.raises(TransportError):
                ws_validate_and_send(
                    ...
                    Request(
                        self.ws_base_url,
                        proxies={"all": f"{supported_proto}://10.255.255.255"},
                    ),
                ).close()
...
class TestWebsocketsRequestHandler:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                lambda: websockets.exceptions.InvalidURI(
                    msg="test", uri="test://"
                ),
                RequestError,
            ),
            ...
            (
                lambda: websockets.exceptions.InvalidHeader(name="test"),
                TransportError,
            ),
            (lambda: websockets.exceptions.NegotiationError(), TransportError),
            # Catch-all
            (
                lambda: websockets.exceptions.WebSocketException(),
                TransportError,
            ),
    ...
    def test_ws_recv_error_mapping(
        self, handler, monkeypatch, raised, expected, match
    ):
        from yt_dlp.networking._websockets import WebsocketsResponseAdapter
