...
class TestUtil(unittest.TestCase):
    ...
    def test_sanitize_filename(self):
        ...
        self.assertEqual(
            sanitize_filename("--gasdgf", is_id=False), "_-gasdgf"
        )
        self.assertEqual(sanitize_filename(".gasdgf"), ".gasdgf")
        self.assertEqual(sanitize_filename(".gasdgf", is_id=True), ".gasdgf")
    ...
    def test_sanitize_filename_restricted(self):
        ...
        self.assertEqual(
            sanitize_filename("abc_d-e", restricted=True), "abc_d-e"
        )

        self.assertEqual(sanitize_filename("123", restricted=True), "123")

        self.assertEqual(
            "abc_de", sanitize_filename("abc/de", restricted=True)
        )
        self.assertFalse(
        ...
        self.assertEqual(
            "xxx", sanitize_filename("xxx/<>\\*|", restricted=True)
        )
        self.assertEqual(
            "yes_no", sanitize_filename("yes? no", restricted=True)
        )
        self.assertEqual(
            "this_-_that", sanitize_filename("this: that", restricted=True)
        )

        tests = "aäb\u4e2d\u56fd\u7684c"
        self.assertEqual(sanitize_filename(tests, restricted=True), "aab_c")
        self.assertTrue(
            sanitize_filename("\xf6", restricted=True) != ""
        )  # No empty filename
    ...
    def _test_sanitize_path(self):
        ...
        self.assertEqual(
            sanitize_path("\\\\?\\UNC\\ComputerName\\abc"),
        ...
        self.assertEqual(sanitize_path("\\\\?\\C:/abc"), "\\\\?\\C:\\abc")
        self.assertEqual(
            sanitize_path("\\\\?\\C:\\ab?c\\de:f"), "\\\\?\\C:\\ab#c\\de#f"
        )
        self.assertEqual(sanitize_path("\\\\?\\C:\\abc"), "\\\\?\\C:\\abc")
        ...
        self.assertEqual(sanitize_path("abc.../def..."), "abc..#\\def..#")
        self.assertEqual(
            sanitize_path("C:\\abc:%(title)s.%(ext)s"),
            "C:\\abc#%(title)s.%(ext)s",
        )
        ...
        for test, expected in [
            ...
            assert result == sanitize_path(
                result
            ), f"{test} changed after sanitizing again"
            if nt_path_normpath:
                assert result == nt_path_normpath(
                    test
                ), f"{test} does not match nt._path_normpath"
    ...
    def test_prepend_extension(self):
        ...
        self.assertEqual(
            prepend_extension("abc.ext", "temp", "ext"), "abc.temp.ext"
        )
        self.assertEqual(
            prepend_extension("abc.unexpected_ext", "temp", "ext"),
            "abc.unexpected_ext.temp",
        )
        ...
        for ext, result in self._uncommon_extensions:
            ...
            if result:
                self.assertEqual(
                    prepend_extension("abc.ext", ext, "ext"), result
                )
            else:
                with self.assertRaises(_UnsafeExtensionError):
    ...
    def test_replace_extension(self):
        ...
        self.assertEqual(
            replace_extension("abc.ext", "temp", "ext"), "abc.temp"
        )
        self.assertEqual(
            replace_extension("abc.unexpected_ext", "temp", "ext"),
            "abc.unexpected_ext.temp",
        )
        ...
        # Test uncommon extensions
        self.assertEqual(
            replace_extension("abc.ext", "bin"), "abc.unknown_video"
        )
    ...
    def test_subtitles_filename(self):
        self.assertEqual(
            subtitles_filename("abc.ext", "en", "vtt"), "abc.en.vtt"
        )
        self.assertEqual(
            subtitles_filename("abc.ext", "en", "vtt", "ext"), "abc.en.vtt"
        )
        self.assertEqual(
            subtitles_filename("abc.unexpected_ext", "en", "vtt", "ext"),
    ...
    def test_date_from_str(self):
        ...
        self.assertEqual(
            date_from_str("now+14day"), date_from_str("now+2week")
        )
        self.assertEqual(
            date_from_str("20200229+365day"), date_from_str("20200229+1year")
        )
        self.assertEqual(
            date_from_str("20210131+28day"), date_from_str("20210131+1month")
        )

    def test_datetime_from_str(self):
        self.assertEqual(
            datetime_from_str("yesterday", precision="day"),
            datetime_from_str("now-1day", precision="auto"),
        )
        self.assertEqual(
            datetime_from_str("now+7day", precision="day"),
            datetime_from_str("now+1week", precision="auto"),
        )
        self.assertEqual(
            datetime_from_str("now+14day", precision="day"),
            datetime_from_str("now+2week", precision="auto"),
        )
        self.assertEqual(
            datetime_from_str("20200229+365day", precision="day"),
            datetime_from_str("20200229+1year", precision="auto"),
        )
        self.assertEqual(
            datetime_from_str("20210131+28day", precision="day"),
            datetime_from_str("20210131+1month", precision="auto"),
        )
        self.assertEqual(
            datetime_from_str("20210131+59day", precision="day"),
            datetime_from_str("20210131+2month", precision="auto"),
        )
        self.assertEqual(
            datetime_from_str("now+1day", precision="hour"),
            datetime_from_str("now+24hours", precision="auto"),
        )
        self.assertEqual(
            datetime_from_str("now+23hours", precision="hour"),
    ...
    def test_unified_dates(self):
        ...
        self.assertEqual(
            unified_strdate("2012/10/11 01:56:38 +0000"), "20121011"
        )
        self.assertEqual(unified_strdate("1968 12 10"), "19681210")
        self.assertEqual(unified_strdate("1968-12-10"), "19681210")
        self.assertEqual(unified_strdate("31-07-2022 20:00"), "20220731")
        self.assertEqual(
            unified_strdate("28/01/2014 21:00:00 +0100"), "20140128"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            unified_strdate("2/2/2015 6:47:40 PM", day_first=False), "20150202"
        )
    ...
    def test_unified_timestamps(self):
        ...
        self.assertEqual(
            unified_timestamp("2012/10/11 01:56:38 +0000"), 1349920598
        )
        self.assertEqual(unified_timestamp("1968 12 10"), -33436800)
        self.assertEqual(unified_timestamp("1968-12-10"), -33436800)
        self.assertEqual(
            unified_timestamp("28/01/2014 21:00:00 +0100"), 1390939200
        )
        self.assertEqual(
        ...
        self.assertEqual(
            unified_timestamp("May 16, 2016 11:15 PM"), 1463440500
        )
        self.assertEqual(
            unified_timestamp("Feb 7, 2016 at 6:35 pm"), 1454870100
        )
        self.assertEqual(unified_timestamp("2017-03-30T17:52:41Q"), 1490896361)
        self.assertEqual(
            unified_timestamp("Sep 11, 2013 | 5:49 AM"), 1378878540
        )
        self.assertEqual(
            unified_timestamp("December 15, 2017 at 7:49 am"), 1513324140
        )
        self.assertEqual(
            unified_timestamp("2018-03-14T08:32:43.1493874+00:00"), 1521016363
        )
        self.assertEqual(
            unified_timestamp("Sunday, 26 Nov 2006, 19:00"), 1164567600
        )
        self.assertEqual(
            unified_timestamp("wed, aug 16, 2008, 12:00pm"), 1218931200
        )

        self.assertEqual(unified_timestamp("December 31 1969 20:00:01 EDT"), 1)
        self.assertEqual(
            unified_timestamp("Wednesday 31 December 1969 18:01:26 MDT"), 86
        )
        self.assertEqual(
            unified_timestamp("12/31/1969 20:01:18 EDT", False), 78
        )

    def test_determine_ext(self):
        self.assertEqual(
            determine_ext("http://example.com/foo/bar.mp4/?download"), "mp4"
        )
        self.assertEqual(
            determine_ext("http://example.com/foo/bar/?download", None), None
        )
        self.assertEqual(
            determine_ext("http://example.com/foo/bar.nonext/?download", None),
            None,
        )
        self.assertEqual(
            determine_ext("http://example.com/foo/bar/mp4?download", None),
            None,
        )
        self.assertEqual(
            determine_ext("http://example.com/foo/bar.m3u8//?download"), "m3u8"
        )
    ...
    def test_xpath_with_ns(self):
        ...
        doc = compat_etree_fromstring(testxml)
        find = lambda p: doc.find(
            xpath_with_ns(p, {"media": "http://example.com/"})
        )
        self.assertTrue(find("media:song") is not None)
        self.assertEqual(find("media:song/media:author").text, "The Author")
        self.assertEqual(
            find("media:song/url").text, "http://server.com/download.mp3"
        )
    ...
    def test_xpath_element(self):
        ...
        self.assertEqual(
            xpath_element(doc, "div/bar", default="default"), "default"
        )
        self.assertEqual(
            xpath_element(doc, ["div/bar"], default="default"), "default"
        )
        ...
        self.assertRaises(
            ExtractorError, xpath_element, doc, "div/bar", fatal=True
        )
        self.assertRaises(
            ExtractorError, xpath_element, doc, ["div/bar"], fatal=True
        )
    ...
    def test_xpath_text(self):
        ...
        self.assertEqual(
            xpath_text(doc, "div/bar", default="default"), "default"
        )
        self.assertTrue(xpath_text(doc, "div/bar") is None)
        self.assertRaises(
            ExtractorError, xpath_text, doc, "div/bar", fatal=True
        )
    ...
    def test_xpath_attr(self):
        ...
        self.assertEqual(
            xpath_attr(doc, "div/bar", "x", default="default"), "default"
        )
        self.assertEqual(
            xpath_attr(doc, "div/p", "y", default="default"), "default"
        )
        self.assertRaises(
            ExtractorError, xpath_attr, doc, "div/bar", "x", fatal=True
        )
        self.assertRaises(
            ExtractorError, xpath_attr, doc, "div/p", "y", fatal=True
        )
    ...
    def test_base_url(self):
        ...
        self.assertEqual(
            base_url("http://foo.de/bar/baz"), "http://foo.de/bar/"
        )
        self.assertEqual(
            base_url("http://foo.de/bar/baz?x=z/x/c"), "http://foo.de/bar/"
        )
        self.assertEqual(
            base_url("http://foo.de/bar/baz&x=z&w=y/x/c"),
            "http://foo.de/bar/baz&x=z&w=y/x/",
        )
    ...
    def test_urljoin(self):
        ...
        self.assertEqual(
            urljoin(b"http://foo.de/", "/a/b/c.txt"), "http://foo.de/a/b/c.txt"
        )
        self.assertEqual(
            urljoin("http://foo.de/", b"/a/b/c.txt"), "http://foo.de/a/b/c.txt"
        )
        self.assertEqual(
            urljoin(b"http://foo.de/", b"/a/b/c.txt"),
            "http://foo.de/a/b/c.txt",
        )
        self.assertEqual(
            urljoin("//foo.de/", "/a/b/c.txt"), "//foo.de/a/b/c.txt"
        )
        self.assertEqual(
            urljoin("http://foo.de/", "a/b/c.txt"), "http://foo.de/a/b/c.txt"
        )
        ...
        self.assertEqual(
            urljoin("http://foo.de/", "http://foo.de/a/b/c.txt"),
            "http://foo.de/a/b/c.txt",
        )
        self.assertEqual(
            urljoin("http://foo.de/", "//foo.de/a/b/c.txt"),
            "//foo.de/a/b/c.txt",
        )
        self.assertEqual(
            urljoin(None, "http://foo.de/a/b/c.txt"), "http://foo.de/a/b/c.txt"
        )
        self.assertEqual(
            urljoin(None, "//foo.de/a/b/c.txt"), "//foo.de/a/b/c.txt"
        )
        self.assertEqual(
            urljoin("", "http://foo.de/a/b/c.txt"), "http://foo.de/a/b/c.txt"
        )
        self.assertEqual(
            urljoin(["foobar"], "http://foo.de/a/b/c.txt"),
            "http://foo.de/a/b/c.txt",
        )
        ...
        self.assertEqual(
            urljoin("http://foo.de/a/b/c.txt", ".././../d.txt"),
            "http://foo.de/d.txt",
        )
        self.assertEqual(
            urljoin("http://foo.de/a/b/c.txt", "rtmp://foo.de"),
            "rtmp://foo.de",
        )
    ...
    def test_parse_duration(self):
        ...
        self.assertEqual(
            parse_duration("3 hours 11 minutes 53 seconds"), 11513
        )
    ...
    def test_urlencode_postdata(self):
        data = urlencode_postdata(
            {"username": "foo@bar.com", "password": "1234"}
        )
    ...
    def test_update_url_query(self):
        ...
        self.assertEqual(
            parse_qs(
                update_url_query(
                    "http://example.com/path",
                    {"fields": ("id,formats,subtitles", "thumbnails")},
                )
            ),
            parse_qs(
                "http://example.com/path?fields=id,formats,subtitles&fields=thumbnails"
            ),
        )
        self.assertEqual(
            parse_qs(
        ...
        self.assertEqual(
            parse_qs(
                update_url_query(
                    "http://example.com/path?system=LINUX&system=WINDOWS",
                    {"system": "LINUX"},
                )
            ),
        ...
        self.assertEqual(
            ...
            parse_qs(
                "http://example.com/path?test=%E7%AC%AC%E4%BA%8C%E8%A1%8C%D1%82%D0%B5%D1%81%D1%82"
            ),
        )
    ...
    def test_multipart_encode(self):
        ...
        self.assertEqual(
            multipart_encode(
                {"欄位".encode(): "值".encode()}, boundary="AAAAAA"
            )[0],
    ...
    def test_merge_dicts(self):
        ...
        self.assertEqual(
            merge_dicts({"a": None}, {"a": ""}, {"a": "abc"}), {"a": "abc"}
        )

    def test_encode_compat_str(self):
        self.assertEqual(
            encode_compat_str(b"\xd1\x82\xd0\xb5\xd1\x81\xd1\x82", "utf-8"),
            "тест",
        )
    ...
    def test_parse_iso8601(self):
        ...
        self.assertEqual(
            parse_iso8601("2014-03-23T23:04:26-07:00"), 1395641066
        )
        self.assertEqual(
            parse_iso8601(
                "2014-03-23T23:04:26", timezone=dt.timedelta(hours=-7)
            ),
            1395641066,
        )
        self.assertEqual(
            parse_iso8601("2014-03-23T23:04:26", timezone=NO_DEFAULT), None
        )
        # default does not override timezone in date_str
        self.assertEqual(
            parse_iso8601(
                "2014-03-23T23:04:26-07:00", timezone=dt.timedelta(hours=-10)
            ),
            1395641066,
        )
        ...
        self.assertEqual(
            parse_iso8601("2014-03-23T22:04:26.1234Z"), 1395612266
        )
    ...
    def test_strip_jsonp(self):
        ...
        stripped = strip_jsonp(
            'window.cb && window.cb({"status": "success"});'
        )
        d = json.loads(stripped)
        self.assertEqual(d, {"status": "success"})
    ...
    def test_parse_codecs(self):
        ...
        self.assertEqual(
            parse_codecs("avc1.77.30, mp4a.40.2"),
            {
                "vcodec": "avc1.77.30",
                "acodec": "mp4a.40.2",
                "dynamic_range": None,
        ...
        self.assertEqual(
            parse_codecs("mp4a.40.5,avc1.42001e"),
            {
                "vcodec": "avc1.42001e",
                "acodec": "mp4a.40.5",
                "dynamic_range": None,
        ...
        self.assertEqual(
            parse_codecs(", h264,,newcodec,aac"),
            {
                "vcodec": "h264",
                "acodec": "aac",
                "dynamic_range": None,
        ...
        self.assertEqual(
            parse_codecs("vp09.02.50.10.01.09.18.09.00"),
            {
        ...
        self.assertEqual(
            parse_codecs("av01.0.12M.10.0.110.09.16.09.0"),
            {
        ...
        self.assertEqual(
            parse_codecs("unknownvcodec, unknownacodec"),
            {
    ...
    def test_escape_rfc3986(self):
        ...
        self.assertEqual(
            escape_rfc3986("%D1%82%D0%B5%D1%81%D1%82"),
    ...
    def test_normalize_url(self):
        ...
        self.assertEqual(
            normalize_url("http://vimeo.com/56015672#at=0"),
            "http://vimeo.com/56015672#at=0",
        )

        self.assertEqual(
            normalize_url("http://www.example.com/../a/b/../c/./d.html"),
    ...
    def test_remove_dot_segments(self):
        ...
        self.assertEqual(
            remove_dot_segments("/abc/./.././d/././e/.././f/./../../ghi"),
            "/ghi",
        )
        self.assertEqual(remove_dot_segments("/"), "/")
    ...
    def test_js_to_json_edgecases(self):
        ...
        self.assertEqual(
            json.loads(on),
            R"^$#",
            msg="Unnecessary escapes should be stripped",
        )
        ...
        self.assertEqual(
            json.loads(on),
            '"""',
            msg="Unnecessary quote escape should be escaped",
        )
        ...
        self.assertEqual(
            json.loads(on),
            ["spam", '("eggs")'],
            msg="Date regex should match a single string",
        )
    ...
    def test_extract_attributes(self):
        ...
        self.assertEqual(extract_attributes("<e x=y>"), {"x": "y"})
        self.assertEqual(
            extract_attributes("<e x=\"a 'b' c\">"), {"x": "a 'b' c"}
        )
        self.assertEqual(
            extract_attributes("<e x='a \"b\" c'>"), {"x": 'a "b" c'}
        )
        ...
        self.assertEqual(extract_attributes('<e x="&#x79;">'), {"x": "y"})
        self.assertEqual(
            extract_attributes('<e x="&amp;">'), {"x": "&"}
        )  # XML
        self.assertEqual(extract_attributes('<e x="&quot;">'), {"x": '"'})
        self.assertEqual(
            extract_attributes('<e x="&pound;">'), {"x": "£"}
        )  # HTML 3.2
        self.assertEqual(
            extract_attributes('<e x="&lambda;">'), {"x": "λ"}
        )  # HTML 4.0
        ...
        self.assertEqual(
            extract_attributes("<e x=y a>"), {"x": "y", "a": None}
        )
        self.assertEqual(extract_attributes("<e x= y>"), {"x": "y"})
        self.assertEqual(
            extract_attributes("<e x=1 y=2 x=3>"), {"y": "2", "x": "3"}
        )
        ...
        self.assertEqual(extract_attributes('<e \nx="\ny\n">'), {"x": "\ny\n"})
        self.assertEqual(
            extract_attributes("<e CAPS=x>"), {"caps": "x"}
        )  # Names lowercased
        self.assertEqual(extract_attributes("<e x=1 X=2>"), {"x": "2"})
        self.assertEqual(extract_attributes("<e X=1 x=2>"), {"x": "2"})
        self.assertEqual(
            extract_attributes("<e _:funny-name1=1>"), {"_:funny-name1": "1"}
        )
        self.assertEqual(
            extract_attributes('<e x="Fáilte 世界 \U0001f600">'),
            {"x": "Fáilte 世界 \U0001f600"},
        )
        self.assertEqual(
            extract_attributes('<e x="décompose&#769;">'),
            {"x": "décompose\u0301"},
        )
    ...
    def test_parse_resolution(self):
        ...
        self.assertEqual(
            parse_resolution(" 1920x1080"), {"width": 1920, "height": 1080}
        )
        self.assertEqual(
            parse_resolution("1920×1080 "), {"width": 1920, "height": 1080}
        )
        self.assertEqual(
            parse_resolution("1920 x 1080"), {"width": 1920, "height": 1080}
        )
        self.assertEqual(parse_resolution("720p"), {"height": 720})
        self.assertEqual(parse_resolution("4k"), {"height": 2160})
        self.assertEqual(parse_resolution("8K"), {"height": 4320})
        self.assertEqual(
            parse_resolution("pre_1920x1080_post"),
...
ffmpeg version 2.4.4 Copyright (c) 2000-2014 the FFmpeg ..."""
    ...
    def test_is_html(self):
        ...
        self.assertTrue(
            is_html(b"\xef\xbb\xbf<!DOCTYPE foo>\xaaa")  # UTF-8 with BOM
        )
        self.assertTrue(
            is_html(  # UTF-16-LE
    ...
    def test_render_table(self):
        ...
        self.assertEqual(
            render_table(["\ta", "bcd"], [["1\t23", 4], ["\t9999", 51]]),
            "   a bcd\n" "1 23 4\n" "9999 51",
        )

        self.assertEqual(
            render_table(["a", "bcd"], [[123, 4], [9999, 51]], delim="-"),
            "a    bcd\n" "--------\n" "123  4\n" "9999 51",
        )

        self.assertEqual(
            render_table(
                ["a", "bcd"], [[123, 4], [9999, 51]], delim="-", extra_gap=2
            ),
            "a      bcd\n" "----------\n" "123    4\n" "9999   51",
        )
    ...
    def test_match_str(self):
        ...
        self.assertTrue(
            match_str(r"x=foo \& bar & x^=foo", {"x": "foo & bar"})
        )
        self.assertTrue(
            match_str(r'x="foo \& bar" & x^=foo', {"x": "foo & bar"})
        )
...
"""
    ...
    def test_cli_option(self):
        self.assertEqual(
            cli_option({"proxy": "127.0.0.1:3128"}, "--proxy", "proxy"),
            ["--proxy", "127.0.0.1:3128"],
        )
        self.assertEqual(cli_option({"proxy": None}, "--proxy", "proxy"), [])
        self.assertEqual(cli_option({}, "--proxy", "proxy"), [])
        self.assertEqual(
            cli_option({"retries": 10}, "--retries", "retries"),
            ["--retries", "10"],
        )
    ...
    def test_get_element_by_attribute(self):
        ...
        self.assertEqual(
            get_element_by_attribute("class", "foo bar", html), "nice"
        )
        self.assertEqual(get_element_by_attribute("class", "foo", html), None)
        self.assertEqual(
            get_element_by_attribute("class", "no-such-foo", html), None
        )

        html = self.GET_ELEMENT_BY_ATTRIBUTE_TEST_STRING

        self.assertEqual(
            get_element_by_attribute("itemprop", "author", html), "foo"
        )
    ...
    def test_get_element_html_by_attribute(self):
        ...
        self.assertEqual(
            get_element_html_by_attribute("class", "foo bar", html),
            html.strip(),
        )
        self.assertEqual(
            get_element_html_by_attribute("class", "foo", html), None
        )
        self.assertEqual(
            get_element_html_by_attribute("class", "no-such-foo", html), None
        )

        html = self.GET_ELEMENT_BY_ATTRIBUTE_TEST_STRING

        self.assertEqual(
            get_element_html_by_attribute("itemprop", "author", html),
            html.strip(),
        )
    ...
    def test_get_elements_by_class(self):
        ...
        self.assertEqual(
            get_elements_by_class("foo", html), ["nice", "also nice"]
        )
    ...
    def test_get_elements_html_by_class(self):
        ...
        self.assertEqual(
            get_elements_html_by_class("foo", html),
    ...
    def test_get_elements_by_attribute(self):
        ...
        self.assertEqual(
            get_elements_by_attribute("class", "foo bar", html),
            ["nice", "also nice"],
        )
        self.assertEqual(get_elements_by_attribute("class", "foo", html), [])
        self.assertEqual(
            get_elements_by_attribute("class", "no-such-foo", html), []
        )
    ...
    def test_get_elements_html_by_attribute(self):
        ...
        self.assertEqual(
            get_elements_html_by_attribute("class", "foo bar", html),
            self.GET_ELEMENTS_BY_CLASS_RES,
        )
        self.assertEqual(
            get_elements_html_by_attribute("class", "foo", html), []
        )
        self.assertEqual(
            get_elements_html_by_attribute("class", "no-such-foo", html), []
        )
    ...
    def test_get_elements_text_and_html_by_attribute(self):
        ...
        self.assertEqual(
            list(
                get_elements_text_and_html_by_attribute("class", "foo", html)
            ),
            [],
        )
        self.assertEqual(
        ...
        self.assertEqual(
            list(
                get_elements_text_and_html_by_attribute(
                    "class",
                    "foo",
                    '<a class="foo">nice</a><span class="foo">nice</span>',
                    tag="a",
                )
            ),
    ...
    GET_ELEMENT_BY_TAG_RES_OUTERDIV_HTML = (
        GET_ELEMENT_BY_TAG_TEST_STRING.strip()[32:276]
    )
    GET_ELEMENT_BY_TAG_RES_OUTERDIV_TEXT = (
        GET_ELEMENT_BY_TAG_RES_OUTERDIV_HTML[5:-6]
    )
    GET_ELEMENT_BY_TAG_RES_INNERSPAN_HTML = (
        GET_ELEMENT_BY_TAG_TEST_STRING.strip()[78:119]
    )
    GET_ELEMENT_BY_TAG_RES_INNERSPAN_TEXT = (
        GET_ELEMENT_BY_TAG_RES_INNERSPAN_HTML[6:-7]
    )
    ...
    def test_get_element_text_and_html_by_tag(self):
        ...
        self.assertEqual(
            ...
            (
                self.GET_ELEMENT_BY_TAG_RES_OUTERDIV_TEXT,
                self.GET_ELEMENT_BY_TAG_RES_OUTERDIV_HTML,
            ),
        )
        self.assertEqual(
            get_element_text_and_html_by_tag("span", html),
            (
                self.GET_ELEMENT_BY_TAG_RES_INNERSPAN_TEXT,
                self.GET_ELEMENT_BY_TAG_RES_INNERSPAN_HTML,
            ),
        )
        self.assertRaises(
            compat_HTMLParseError,
    ...
    def test_iri_to_uri(self):
        ...
        self.assertEqual(
            iri_to_uri("http://日本語.jp/"), "http://xn--wgv71a119e.jp/"
        )
        self.assertEqual(
            iri_to_uri("http://导航.中国/"), "http://xn--fet810g.xn--fiqs8s/"
        )
    ...
    def test_hide_login_info(self):
        ...
        self.assertEqual(
            Config.hide_login_info(["-u", "foo", "-u", "bar"]),
            ["-u", "PRIVATE", "-u", "PRIVATE"],
        )
        self.assertEqual(
            Config.hide_login_info(["--username=foo"]), ["--username=PRIVATE"]
        )
        ...
        try:
            for lock_mode in MODES:
                with locked_file(FILE, lock_mode, False) as f:
                    if lock_mode == "r":
                        self.assertEqual(
                            f.read(), TEXT * 2, "Wrong file content"
                        )
                    else:
                        f.write(TEXT)
                    for test_mode in MODES:
                        ...
                        try:
                            ...
                            if not testing_write:  # FIXME: blocked read access
                                print(
                                    f"Known issue: Exclusive lock ({lock_mode}) blocks read access ({test_mode})"
                                )
                                continue
                            self.assertTrue(
                                testing_write,
                                f"{test_mode} is blocked by {lock_mode}",
                            )
                        else:
                            self.assertFalse(
                                testing_write,
                                f"{test_mode} is not blocked by {lock_mode}",
                            )
        finally:
    ...
    def test_determine_file_encoding(self):
        ...
        self.assertEqual(
            determine_file_encoding(b"--verbose -x --audio-format mkv\n"),
            (None, 0),
        )

        self.assertEqual(
            determine_file_encoding(b"\xef\xbb\xbf"), ("utf-8", 3)
        )
        self.assertEqual(
            determine_file_encoding(b"\x00\x00\xfe\xff"), ("utf-32-be", 4)
        )
        self.assertEqual(
            determine_file_encoding(b"\xff\xfe"), ("utf-16-le", 2)
        )

        self.assertEqual(
            determine_file_encoding(b"\xff\xfe# coding: utf-8\n--verbose"),
            ("utf-16-le", 2),
        )
        ...
        self.assertEqual(
            determine_file_encoding("# coding: utf-32-be".encode("utf-32-be")),
            ("utf-32-be", 0),
        )
        self.assertEqual(
            determine_file_encoding("# coding: utf-16-le".encode("utf-16-le")),
            ("utf-16-le", 0),
        )
    ...
    def test_get_compatible_ext(self):
        ...
        self.assertEqual(
            get_compatible_ext(
                vcodecs=[None], acodecs=[None], vexts=["flv"], aexts=["flv"]
            ),
            "flv",
        )
        ...
        self.assertEqual(
            get_compatible_ext(
                vcodecs=[None], acodecs=[None], vexts=["mp4"], aexts=["m4a"]
            ),
            "mp4",
        )
        self.assertEqual(
            get_compatible_ext(
                vcodecs=[None], acodecs=[None], vexts=["mp4"], aexts=["webm"]
            ),
            "mkv",
        )
        self.assertEqual(
            get_compatible_ext(
                vcodecs=[None], acodecs=[None], vexts=["webm"], aexts=["m4a"]
            ),
            "mkv",
        )
        self.assertEqual(
            get_compatible_ext(
                vcodecs=[None], acodecs=[None], vexts=["webm"], aexts=["webm"]
            ),
            "webm",
        )
        self.assertEqual(
            get_compatible_ext(
                vcodecs=[None], acodecs=[None], vexts=["webm"], aexts=["weba"]
            ),
            "webm",
        )
    ...
    def test_try_call(self):
        ...
        self.assertEqual(
            try_call(lambda: 1, expected_type=int),
            1,
            msg="int fn with expected_type int should give int",
        )
        self.assertEqual(
            try_call(lambda: 1, expected_type=dict),
            None,
            msg="int fn with wrong expected_type should give None",
        )
        self.assertEqual(
            try_call(total, args=(0, 1, 0), expected_type=int),
            1,
            msg="fn should accept arglist",
        )
        self.assertEqual(
            try_call(
                total, kwargs={"a": 0, "b": 1, "c": 0}, expected_type=int
            ),
            1,
        ...
        self.assertEqual(
            try_call(lambda x: {}, total, args=(42,), expected_type=int),
            42,
    ...
    def test_http_header_dict(self):
        ...
        self.assertEqual(
            list(headers.sensitive().items()), [("ytdl-test", "0")]
        )
        headers["ytdl-test"] = 1
        self.assertEqual(list(headers.items()), [("Ytdl-Test", "1")])
        self.assertEqual(
            list(headers.sensitive().items()), [("ytdl-test", "1")]
        )
        headers["Ytdl-test"] = "2"
        self.assertEqual(list(headers.items()), [("Ytdl-Test", "2")])
        self.assertEqual(
            list(headers.sensitive().items()), [("Ytdl-test", "2")]
        )
        ...
        self.assertEqual(
            set(headers.items()), {("Ytdl-Test", "2"), ("X-Dlp", "data")}
        )
        self.assertEqual(
            set(headers.sensitive().items()),
            {("Ytdl-test", "2"), ("X-dlp", "data")},
        )
        self.assertEqual(dict(headers), {"Ytdl-Test": "2", "X-Dlp": "data"})
        self.assertEqual(len(headers), 2)
        self.assertEqual(headers.copy(), headers)
        headers2 = HTTPHeaderDict(
            {"X-dlp": "data3"}, headers, **{"X-dlP": "data2"}
        )
        self.assertEqual(
            set(headers2.items()), {("Ytdl-Test", "2"), ("X-Dlp", "data2")}
        )
        self.assertEqual(
            set(headers2.sensitive().items()),
            {("Ytdl-test", "2"), ("X-dlP", "data2")},
        )
        ...
        self.assertEqual(
            set(headers3.sensitive().items()), {("Ytdl-test", "2")}
        )
        ...
        self.assertEqual(
            set(headers4.sensitive().items()), {("ytdl-test", "data;")}
        )
        ...
        self.assertEqual(
            set(headers5.sensitive().items()), {("ytdl-test", "data;")}
        )
    ...
    def test_extract_basic_auth(self):
        assert extract_basic_auth("http://:foo.bar") == (
            "http://:foo.bar",
            None,
        )
        ...
        assert extract_basic_auth("http://:pass@foo.bar") == (
            "http://foo.bar",
            "Basic OnBhc3M=",
        )
        assert extract_basic_auth("http://user:@foo.bar") == (
            "http://foo.bar",
            "Basic dXNlcjo=",
        )
        assert extract_basic_auth("http://user:pass@foo.bar") == (
            "http://foo.bar",
            "Basic dXNlcjpwYXNz",
        )
    ...
    def test_windows_escaping(self):
        ...
        for argument in tests:
            ...
            args = [
                sys.executable,
                "-c",
                "import sys; print(end=sys.argv[1])",
                argument,
                "end",
            ]
