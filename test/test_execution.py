...
class TestExecution(unittest.TestCase):
    def run_yt_dlp(
        self, exe=(sys.executable, "yt_dlp/__main__.py"), opts=("--version",)
    ):
        stdout, stderr, returncode = Popen.run(
            [*exe, "--ignore-config", *opts],
            cwd=rootDir,
            text=True,
    ...
    def test_lazy_extractors(self):
        try:
            ...
            if stderr and stderr.startswith(
                "Deprecated Feature: Support for Python"
            ):
                stderr = ""
            self.assertFalse(stderr)

            subprocess.check_call(
                [sys.executable, "test/test_all_urls.py"],
                cwd=rootDir,
                stdout=subprocess.DEVNULL,
            )
        finally:
