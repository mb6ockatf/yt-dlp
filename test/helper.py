...
def expect_dict(self, got_dict, expected_dict):
    if message := _expect_value(
        "dictionaries differ", got_dict, expected_dict, None
    ):
        self.fail(message)
...
def expect_info_dict(self, got_dict, expected_dict):
    ...
    if got_dict.get("_type") not in ("playlist", "multi_video"):
        ...
        for key in mandatory_fields:
            self.assertTrue(
                got_dict.get(key), f"Missing mandatory field {key}"
            )
    ...
    self.assertFalse(
        invalid_keys,
        f'Invalid fields returned by the extractor: {", ".join(invalid_keys)}',
    )
