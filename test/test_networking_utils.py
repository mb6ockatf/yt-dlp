...
class TestNetworkingUtils:
    ...
    def test_make_socks_proxy_unknown(self):
        with pytest.raises(
            ValueError, match="Unknown SOCKS proxy version: socks"
        ):
    ...
    def test_load_certifi(self):
        ...
        if context_default.get_ca_certs() == context_certifi.get_ca_certs():
            pytest.skip(
                "System uses certifi as default. The test is not valid"
            )
    ...
    @pytest.mark.parametrize(
        "headers,supported_encodings,expected",
        [
            (
                {"Accept-Encoding": "br"},
                ["gzip", "br"],
    ...
    def test_add_accept_encoding_header(
        self, headers, supported_encodings, expected
    ):
...
class TestNetworkingExceptions:
    ...
    def test_redirect_http_error(self):
        ...
        assert (
            str(error)
            == error.msg
            == "HTTP Error 301: Moved Permanently (redirect loop detected)"
        )
