...
class TestTraversal:
    def test_traversal_base(self):
        assert traverse_obj(_TEST_DATA, ("str",)) == "str", "allow tuple path"
        assert traverse_obj(_TEST_DATA, ["str"]) == "str", "allow list path"
        assert (
            traverse_obj(_TEST_DATA, (value for value in ("str",))) == "str"
        ), "allow iterable path"
        assert (
            traverse_obj(_TEST_DATA, "str") == "str"
        ), "single items should be treated as a path"
        assert traverse_obj(_TEST_DATA, 100) == 100, "allow int path"
        assert traverse_obj(_TEST_DATA, 1.2) == 1.2, "allow float path"
        assert (
            traverse_obj(_TEST_DATA, None) == _TEST_DATA
        ), "`None` should not perform any modification"

    def test_traversal_ellipsis(self):
        assert traverse_obj(_TEST_DATA, ...) == [
            x for x in _TEST_DATA.values() if x not in (None, {})
        ], "`...` should give all non discarded values"
        assert traverse_obj(_TEST_DATA, ("urls", 0, ...)) == list(
            _TEST_DATA["urls"][0].values()
        ), "`...` selection for dicts should select all values"
        assert traverse_obj(_TEST_DATA, (..., ..., "url")) == [
            "https://www.example.com/0",
            "https://www.example.com/1",
        ], "nested `...` queries should work"
        assert traverse_obj(_TEST_DATA, (..., ..., "index")) == list(
            range(4)
        ), "`...` query result should be flattened"
        assert traverse_obj(iter(range(4)), ...) == list(
            range(4)
        ), "`...` should accept iterables"

    def test_traversal_function(self):
        filter_func = lambda x, y: x == "urls" and isinstance(y, list)
        assert traverse_obj(_TEST_DATA, filter_func) == [
            _TEST_DATA["urls"]
        ], "function as query key should perform a filter based on (key, value)"
        assert traverse_obj(
            _TEST_DATA, lambda _, x: isinstance(x[0], str)
        ) == ["str"], "exceptions in the query function should be catched"
        assert traverse_obj(iter(range(4)), lambda _, x: x % 2 == 0) == [
            0,
            2,
    ...
    def test_traversal_set(self):
        ...
        assert traverse_obj(_TEST_DATA, (..., {str.upper})) == [
            "STR"
        ], "Function in set should be a transformation"
        assert traverse_obj(_TEST_DATA, (..., {str})) == [
            "str"
        ], "Type in set should be a type filter"
        assert traverse_obj(_TEST_DATA, (..., {str, int})) == [
            100,
            "str",
        ], "Multiple types in set should be a type filter"
        assert (
            traverse_obj(_TEST_DATA, {dict}) == _TEST_DATA
        ), "A single set should be wrapped into a path"
        assert traverse_obj(_TEST_DATA, (..., {str.upper})) == [
            "STR"
        ], "Transformation function should not raise"
        expected = [
            x for x in map(str_or_none, _TEST_DATA.values()) if x is not None
        ]
        assert (
            traverse_obj(_TEST_DATA, (..., {str_or_none})) == expected
        ), "Function in set should be a transformation"
        assert (
            traverse_obj(_TEST_DATA, ("fail", {lambda _: "const"})) == "const"
    ...
    def test_traversal_slice(self):
        ...
        assert (
            traverse_obj(_TEST_DATA, ("dict", slice(1))) is None
        ), "slice on a dictionary should not throw"
        assert (
            traverse_obj(_SLICE_DATA, slice(1)) == _SLICE_DATA[:1]
        ), "slice key should apply slice to sequence"
        assert (
            traverse_obj(_SLICE_DATA, slice(1, 2)) == _SLICE_DATA[1:2]
        ), "slice key should apply slice to sequence"
        assert (
            traverse_obj(_SLICE_DATA, slice(1, 4, 2)) == _SLICE_DATA[1:4:2]
        ), "slice key should apply slice to sequence"

    def test_traversal_alternatives(self):
        assert (
            traverse_obj(_TEST_DATA, "fail", "str") == "str"
        ), "multiple `paths` should be treated as alternative paths"
        assert (
            traverse_obj(_TEST_DATA, "str", 100) == "str"
        ), "alternatives should exit early"
        assert (
            traverse_obj(_TEST_DATA, "fail", "fail") is None
        ), "alternatives should return `default` if exhausted"
        assert (
            traverse_obj(_TEST_DATA, (..., "fail"), 100) == 100
        ), "alternatives should track their own branching return"
        assert traverse_obj(_TEST_DATA, ("dict", ...), ("data", ...)) == list(
            _TEST_DATA["data"]
        ), "alternatives on empty objects should search further"

    def test_traversal_branching_nesting(self):
        assert traverse_obj(_TEST_DATA, ("urls", (3, 0), "url")) == [
            "https://www.example.com/0"
    ...
    def test_traversal_expected_type(self):
        ...
        assert (
            traverse_obj(_EXPECTED_TYPE_DATA, "str", expected_type=str)
            == "str"
        ), "accept matching `expected_type` type"
        assert (
            traverse_obj(_EXPECTED_TYPE_DATA, "str", expected_type=int) is None
        ), "reject non matching `expected_type` type"
        assert (
            traverse_obj(
                _EXPECTED_TYPE_DATA, "int", expected_type=lambda x: str(x)
            )
            == "0"
        ), "transform type using type function"
        assert (
            traverse_obj(
                _EXPECTED_TYPE_DATA, "str", expected_type=lambda _: 1 / 0
            )
            is None
        ), "wrap expected_type fuction in try_call"
        assert traverse_obj(_EXPECTED_TYPE_DATA, ..., expected_type=str) == [
            "str"
        ], "eliminate items that expected_type fails on"
        assert traverse_obj(
            _TEST_DATA, {0: 100, 1: 1.2}, expected_type=int
        ) == {0: 100}, "type as expected_type should filter dict values"
        assert traverse_obj(
            _TEST_DATA, {0: 100, 1: 1.2, 2: "None"}, expected_type=str_or_none
        ) == {
            0: "100",
            1: "1.2",
        ...
        assert (
            traverse_obj(
                _TEST_DATA, ({0: 1.2}, 0, {int_or_none}), expected_type=int
            )
            == 1
        ), "expected_type should not filter non final dict values"
        assert traverse_obj(
            _TEST_DATA, {0: {0: 100, 1: "str"}}, expected_type=int
        ) == {0: {0: 100}}, "expected_type should transform deep dict values"
        assert traverse_obj(
            _TEST_DATA, [({0: "..."}, {0: "..."})], expected_type=type(...)
        ) == [
            {0: ...},
            {0: ...},
        ], "expected_type should transform branched dict values"
        assert traverse_obj({1: {3: 4}}, [(1, 2), 3], expected_type=int) == [
            4
        ], "expected_type regression for type matching in tuple branching"
        assert (
            traverse_obj(_TEST_DATA, ["data", ...], expected_type=int) == []
    ...
    def test_traversal_get_all(self):
        ...
        assert (
            traverse_obj(_GET_ALL_DATA, ("key", ...), get_all=False) == 0
        ), "if not `get_all`, return only first matching value"
        assert traverse_obj(_GET_ALL_DATA, ..., get_all=False) == [
            0,
            1,
            2,
    ...
    def test_traversal_casesense(self):
        ...
        assert (
            traverse_obj(_CASESENSE_DATA, "key") is None
        ), "dict keys should be case sensitive unless `casesense`"
        assert (
            traverse_obj(_CASESENSE_DATA, "keY", casesense=False) == "value0"
        ), "allow non matching key case if `casesense`"
        assert traverse_obj(
            _CASESENSE_DATA, [0, ("keY",)], casesense=False
        ) == ["value1"], "allow non matching key case in branch if `casesense`"
        assert traverse_obj(
            _CASESENSE_DATA, [0, ([0, "keY"],)], casesense=False
        ) == [
            "value2"
    ...
    def test_traversal_traverse_string(self):
        ...
        assert (
            traverse_obj(_TRAVERSE_STRING_DATA, ("str", 0)) is None
        ), "do not traverse into string if not `traverse_string`"
        assert (
            traverse_obj(
                _TRAVERSE_STRING_DATA, ("str", 0), traverse_string=True
            )
            == "s"
        ), "traverse into string if `traverse_string`"
        assert (
            traverse_obj(_TRAVERSE_STRING_DATA, (1.2, 1), traverse_string=True)
            == "."
        ), "traverse into converted data if `traverse_string`"
        assert (
            traverse_obj(
                _TRAVERSE_STRING_DATA, ("str", ...), traverse_string=True
            )
            == "str"
        ...
        assert (
            traverse_obj(
                _TRAVERSE_STRING_DATA,
                ("str", lambda i, v: i or v == "s"),
                traverse_string=True,
            )
            == "str"
        ), "function should result in string if `traverse_string`"
        assert traverse_obj(
            _TRAVERSE_STRING_DATA, ("str", (0, 2)), traverse_string=True
        ) == ["s", "r"], "branching should result in list if `traverse_string`"
        assert (
            traverse_obj({}, (0, ...), traverse_string=True) == []
        ), "branching should result in list if `traverse_string`"
        assert (
            traverse_obj({}, (0, lambda x, y: True), traverse_string=True)
            == []
        ), "branching should result in list if `traverse_string`"
        assert (
            traverse_obj({}, (0, slice(1)), traverse_string=True) == []
        ), "branching should result in list if `traverse_string`"

    def test_traversal_re(self):
        mobj = re.fullmatch(r"0(12)(?P<group>3)(4)?", "0123")
        assert traverse_obj(mobj, ...) == [
            x for x in mobj.groups() if x is not None
        ], "`...` on a `re.Match` should give its `groups()`"
        assert traverse_obj(mobj, lambda k, _: k in (0, 2)) == [
            "0123",
            "3",
        ], "function on a `re.Match` should give groupno, value starting at 0"
        assert (
            traverse_obj(mobj, "group") == "3"
        ), "str key on a `re.Match` should give group with that name"
        assert (
            traverse_obj(mobj, 2) == "3"
        ), "int key on a `re.Match` should give group with that name"
        assert (
            traverse_obj(mobj, "gRoUp", casesense=False) == "3"
        ), "str key on a `re.Match` should respect casesense"
        assert (
            traverse_obj(mobj, "fail") is None
        ), "failing str key on a `re.Match` should return `default`"
        assert (
            traverse_obj(mobj, "gRoUpS", casesense=False) is None
        ), "failing str key on a `re.Match` should return `default`"
        assert (
            traverse_obj(mobj, 8) is None
        ), "failing int key on a `re.Match` should return `default`"
        assert traverse_obj(mobj, lambda k, _: k in (0, "group")) == [
            "0123",
            "3",
    ...
    def test_traversal_xml_etree(self):
        ...
        assert (
            traverse_obj(etree, ".//neighbor/@name") == expected
        ), "`@<attribute>` at end of path should give that attribute"
        assert traverse_obj(etree, "//neighbor/@fail") == [
            None,
            None,
            None,
            None,
        ...
        assert traverse_obj(etree, ("//neighbor/@", 2)) == {
            "name": "Malaysia",
            "direction": "N",
        }, "`@` should give the full attribute dict"
        assert traverse_obj(etree, "//year/text()") == [
            "2008",
            "2011",
            "2011",
        ], "`text()` at end of path should give the inner text"
        assert traverse_obj(etree, "//*[@direction]/@direction") == [
            "E",
            "W",
            "N",
            "W",
        ...
        assert (
            traverse_obj(etree, (0, "@name")) == "Liechtenstein"
        ), "special transformations should act on current element"
        assert traverse_obj(
            etree, ("country", 0, ..., "text()", {int_or_none})
        ) == [
            1,
            2008,
            141100,
    ...
    def test_traversal_unbranching(self):
        assert traverse_obj(_TEST_DATA, [(100, 1.2), all]) == [
            100,
            1.2,
        ], "`all` should give all results as list"
        assert (
            traverse_obj(_TEST_DATA, [(100, 1.2), any]) == 100
        ), "`any` should give the first result"
        assert traverse_obj(_TEST_DATA, [100, all]) == [
            100
        ], "`all` should give list if non branching"
        assert (
            traverse_obj(_TEST_DATA, [100, any]) == 100
        ), "`any` should give single item if non branching"
        assert traverse_obj(_TEST_DATA, [("dict", "None", 100), all]) == [
            100
        ], "`all` should filter `None` and empty dict"
        assert (
            traverse_obj(_TEST_DATA, [("dict", "None", 100), any]) == 100
        ...
        assert (
            traverse_obj(
                _TEST_DATA, [("dict", "None", 100, 1.2), all, {float}]
            )
            is None
        ), "`all` should reset branching status"
        assert (
            traverse_obj(
                _TEST_DATA, [("dict", "None", 100, 1.2), any, {float}]
            )
            is None
        ), "`any` should reset branching status"
        assert traverse_obj(
            _TEST_DATA, [("dict", "None", 100, 1.2), all, ..., {float}]
        ) == [1.2], "`all` should allow further branching"
        assert traverse_obj(
            _TEST_DATA, [("dict", "None", "urls", "data"), any, ..., "index"]
        ) == [0, 1], "`any` should allow further branching"
    ...
    def test_traversal_morsel(self):
        ...
        for key, value in values.items():
            assert (
                traverse_obj(morsel, key) == value
            ), "Morsel should provide access to all values"
        assert traverse_obj(morsel, ...) == list(
            values.values()
        ), "`...` should yield all values"
        assert traverse_obj(morsel, lambda k, v: True) == list(
            values.values()
        ), "function key should yield all values"
        assert (
            traverse_obj(morsel, [(None,), any]) == morsel
    ...
    def test_traversal_filter(self):
        ...
        assert traverse_obj(data, [..., filter]) == [
            True,
            1,
            1.1,
            "str",
...
class TestTraversalHelpers:
    def test_traversal_require(self):
        ...
        assert (
            traverse_obj(_TEST_DATA, ["str", {require("value")}]) == "str"
    ...
    def test_subs_list_to_dict(self):
        ...
        assert traverse_obj(
            [
                {
                    "name": "en",
                    "url": "https://example.com/subs/en2",
                    "prio": True,
                },
                {
                    "name": "en",
                    "url": "https://example.com/subs/en1",
                    "prio": False,
                },
            ],
            [
    ...
    def test_find_element(self):
        ...
        assert (
            find_element(cls="a", html=True)(_TEST_HTML)
            == '<div class="a">1</div>'
        )
        ...
        assert (
            find_element(id="x", html=True)(_TEST_HTML)
            == '<div class="a" id="x" custom="z">2</div>'
        )
        assert find_element(attr="data-id", value="y")(_TEST_HTML) == "3"
        assert (
            find_element(attr="data-id", value="y(?:es)?")(_TEST_HTML) is None
        )
        assert (
            find_element(attr="data-id", value="y(?:es)?", regex=True)(
                _TEST_HTML
            )
            == "3"
        )
        assert (
            find_element(attr="data-id", value="y", html=True)(_TEST_HTML)
            == '<div class="b" data-id="y" custom="z">3</div>'
        )
    ...
    def test_find_elements(self):
        ...
        assert find_elements(attr="custom", value="z")(_TEST_HTML) == [
            "2",
            "3",
        ]
        assert find_elements(attr="custom", value="[ez]")(_TEST_HTML) == []
        assert find_elements(attr="custom", value="[ez]", regex=True)(
            _TEST_HTML
        ) == ["2", "3", "5"]
...
class TestDictGet:
    def test_dict_get(self):
        ...
        for key, false_value in FALSE_VALUES.items():
            ...
            assert (
                dict_get(d, ("b", "c", key), skip_false_values=False)
                == false_value
            )
