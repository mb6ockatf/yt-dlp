    ...
    def do_proxy_auth(self, username, password):
        ...
        try:
            auth_username, auth_password = (
                base64.b64decode(auth).decode().split(":", 1)
            )
        except Exception:
            return self.proxy_auth_error()

        if auth_username != (username or "") or auth_password != (
            password or ""
        ):
            return self.proxy_auth_error()
        return True
        ...
        if self.path.endswith("/proxy_info"):
            payload = json.dumps(
                ...
                or {
                    ...
                    "path": self.path,
                    "proxy": ":".join(
                        str(y) for y in self.connection.getsockname()
                    ),
                }
            )
...
class HTTPSProxyHandler(HTTPProxyHandler):
    def __init__(self, request, *args, **kwargs):
        ...
        if isinstance(request, ssl.SSLSocket):
            request = SSLTransport(
                request, ssl_context=sslctx, server_side=True
            )
        else:
...
def proxy_server(
    proxy_server_class, request_handler, bind_ip=None, **proxy_server_kwargs
):
...
class HTTPProxyHTTPTestContext(HTTPProxyTestContext):
    ...
    def proxy_info_request(
        ...
        request = Request(
            f'http://{target_domain or "127.0.0.1"}:{target_port or "40000"}/proxy_info',
            **req_kwargs,
        )
...
class HTTPProxyHTTPSTestContext(HTTPProxyTestContext):
    ...
    def proxy_info_request(
        ...
        request = Request(
            f'https://{target_domain or "127.0.0.1"}:{target_port or "40000"}/proxy_info',
            **req_kwargs,
        )
        handler.validate(request)
        return json.loads(handler.send(request).read().decode())
...
@pytest.mark.parametrize(
    "ctx", ["http"], indirect=True
)  # pure http proxy can only support http
class TestHTTPProxy:
    ...
    def test_http_auth(self, handler, ctx):
        with ctx.http_server(
            HTTPProxyHandler, username="test", password="test"
        ) as server_address:
            with handler(
                proxies={
                    ctx.REQUEST_PROTO: f"http://test:test@{server_address}"
                }
            ) as rh:
                proxy_info = ctx.proxy_info_request(rh)
                assert proxy_info["proxy"] == server_address
    ...
    def test_http_bad_auth(self, handler, ctx):
        with ctx.http_server(
            ...
            with handler(
                proxies={
                    ctx.REQUEST_PROTO: f"http://test:bad@{server_address}"
                }
            ) as rh:
    ...
    @pytest.mark.skip_handler(
        "Urllib", "urllib does not support https proxies"
    )
    ...
    def test_http_with_idn(self, handler, ctx):
        with ctx.http_server(HTTPProxyHandler) as server_address:
            with handler(
                ...
                proxy_info = ctx.proxy_info_request(
                    rh, target_domain="中文.tw"
                )
                assert proxy_info["proxy"] == server_address
                assert proxy_info["path"].startswith("http://xn--fiq228c.tw")
                assert (
                    proxy_info["headers"]["Host"].split(":", 1)[0]
                    == "xn--fiq228c.tw"
                )
...
class TestHTTPConnectProxy:
    ...
    def test_http_connect_auth(self, handler, ctx):
        with ctx.http_server(
            HTTPConnectProxyHandler, username="test", password="test"
        ) as server_address:
            with handler(
                verify=False,
                proxies={
    ...
    def test_https_connect_proxy_auth(self, handler, ctx):
        with ctx.http_server(
            HTTPSConnectProxyHandler, username="test", password="test"
        ) as server_address:
            with handler(
                verify=False,
                proxies={
                    ctx.REQUEST_PROTO: f"https://test:test@{server_address}"
                },
            ) as rh:
                proxy_info = ctx.proxy_info_request(rh)
                assert proxy_info["proxy"] == server_address
