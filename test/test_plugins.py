...
class TestPlugins(unittest.TestCase):
    ...
    def test_postprocessor_classes(self):
        ...
        self.assertIn(
            f"{PACKAGE_NAME}.postprocessor.normal", sys.modules.keys()
        )
        ...
        try:
            for plugin_type in ("extractor", "postprocessor"):
                package = importlib.import_module(
                    f"{PACKAGE_NAME}.{plugin_type}"
                )
                self.assertIn(
                    zip_path / PACKAGE_NAME / plugin_type,
    ...
    def test_reloading_plugins(self):
        ...
        try:
            for plugin_type in ("extractor", "postprocessor"):
                ...
                self.assertIn(
                    reload_plugins_path / PACKAGE_NAME / plugin_type,
                    map(Path, package.__path__),
                )
            ...
            self.assertIn("NormalPluginPP", plugins_pp.keys())
            self.assertTrue(
                plugins_pp["NormalPluginPP"].REPLACED,
    ...
    def test_extractor_override_plugin(self):
        ...
        self.assertEqual(
            GenericIE.IE_NAME, "generic+override+underscore-override"
        )
        importlib.invalidate_caches()
    ...
    def test_load_all_plugin_types(self):
        ...
        self.assertNotIn(
            f"{PACKAGE_NAME}.extractor.normal", sys.modules.keys()
        )
        self.assertNotIn(
            f"{PACKAGE_NAME}.postprocessor.normal", sys.modules.keys()
        )
    ...
    def test_get_plugin_spec(self):
        ...
        self.assertEqual(
            plugin_specs.value.get("extractor"), EXTRACTOR_PLUGIN_SPEC
        )
        self.assertEqual(
