...
class SocksProxyHandler(BaseRequestHandler):
    def __init__(
        self, request_handler_class, socks_server_kwargs, *args, **kwargs
    ):
    ...
    def handle(self):
        ...
        if auth is not None and Socks5Auth.AUTH_USER_PASS not in methods:
            self.connection.sendall(
                struct.pack(
                    "!BB", SOCKS5_VERSION, Socks5Auth.AUTH_NO_ACCEPTABLE
                )
            )
            ...
            self.connection.sendall(
                struct.pack("!BB", SOCKS5_VERSION, Socks5Auth.AUTH_USER_PASS)
            )
            ...
            self.connection.sendall(
                struct.pack("!BB", SOCKS5_VERSION, Socks5Auth.AUTH_NONE)
            )
        else:
            self.connection.sendall(
                struct.pack(
        ...
        if address_type == Socks5AddressType.ATYP_IPV4:
            socks_info["ipv4_address"] = socket.inet_ntoa(
                self.connection.recv(4)
            )
        elif address_type == Socks5AddressType.ATYP_DOMAINNAME:
            socks_info["domain_address"] = self.connection.recv(
                ord(self.connection.recv(1))
            ).decode()
        elif address_type == Socks5AddressType.ATYP_IPV6:
            socks_info["ipv6_address"] = socket.inet_ntop(
...
class Socks4ProxyHandler(StreamRequestHandler, SocksProxyHandler):
    ...
    def _read_until_null(self):
        return b"".join(
            iter(functools.partial(self.connection.recv, 1), b"\x00")
        )
    ...
    def handle(self):
        ...
        version, command, dest_port, dest_ip = struct.unpack(
            "!BBHI", self.connection.recv(8)
        )
...
class SocksHTTPTestRequestHandler(
    http.server.BaseHTTPRequestHandler, SocksTestRequestHandler
):
    def do_GET(self):
...
class SocksWebSocketTestRequestHandler(SocksTestRequestHandler):
    def handle(self):
        ...
        connection = websockets.sync.server.ServerConnection(
            socket=self.request, protocol=protocol, close_timeout=0
        )
...
def socks_server(
    socks_server_class, request_handler, bind_ip=None, **socks_server_kwargs
):
...
class SocksProxyTestContext(abc.ABC):
    ...
    def socks_server(self, server_class, *args, **kwargs):
        return socks_server(
            server_class, self.REQUEST_HANDLER_CLASS, *args, **kwargs
        )

    @abc.abstractmethod
    def socks_info_request(
        self, handler, target_domain=None, target_port=None, **req_kwargs
    ) -> dict:
...
class HTTPSocksTestProxyContext(SocksProxyTestContext):
    ...
    def socks_info_request(
        ...
        request = Request(
            f'http://{target_domain or "127.0.0.1"}:{target_port or "40000"}/socks_info',
            **req_kwargs,
        )
...
class WebSocketSocksTestProxyContext(SocksProxyTestContext):
    ...
    def socks_info_request(
        ...
        request = Request(
            f'ws://{target_domain or "127.0.0.1"}:{target_port or "40000"}',
            **req_kwargs,
        )
        handler.validate(request)
...
class TestSocks4Proxy:
    ...
    def test_socks4_auth(self, handler, ctx):
        with handler() as rh:
            with ctx.socks_server(
                Socks4ProxyHandler, user_id="user"
            ) as server_address:
                with pytest.raises(ProxyError):
                    ctx.socks_info_request(
                        rh, proxies={"all": f"socks4://{server_address}"}
                    )
                response = ctx.socks_info_request(
    ...
    def test_socks4a_ipv4_target(self, handler, ctx):
        with ctx.socks_server(Socks4ProxyHandler) as server_address:
            with handler(proxies={"all": f"socks4a://{server_address}"}) as rh:
                response = ctx.socks_info_request(
                    rh, target_domain="127.0.0.1"
                )
                assert response["version"] == 4
                assert (response["ipv4_address"] == "127.0.0.1") != (
                    response["domain_address"] == "127.0.0.1"
                )
    ...
    def test_ipv4_client_source_address(self, handler, ctx):
        with ctx.socks_server(Socks4ProxyHandler) as server_address:
            ...
            with handler(
                proxies={"all": f"socks4://{server_address}"},
    ...
    def test_socks4_errors(self, handler, ctx, reply_code):
        with ctx.socks_server(
            Socks4ProxyHandler, cd_reply=reply_code
        ) as server_address:
    ...
    def test_timeout(self, handler, ctx):
        with ctx.socks_server(Socks4ProxyHandler, sleep=2) as server_address:
            with handler(
                proxies={"all": f"socks4://{server_address}"}, timeout=0.5
            ) as rh:
...
class TestSocks5Proxy:
    ...
    def test_socks5_user_pass(self, handler, ctx):
        with ctx.socks_server(
            Socks5ProxyHandler, auth=("test", "testpass")
        ) as server_address:
            with handler() as rh:
                with pytest.raises(ProxyError):
                    ctx.socks_info_request(
                        rh, proxies={"all": f"socks5://{server_address}"}
                    )

                response = ctx.socks_info_request(
                    rh,
                    proxies={
                        "all": f"socks5://test:testpass@{server_address}"
                    },
                )
    ...
    def test_ipv4_client_source_address(self, handler, ctx):
        with ctx.socks_server(Socks5ProxyHandler) as server_address:
            ...
            verify_address_availability(source_address)
            with handler(
                proxies={"all": f"socks5://{server_address}"},
                source_address=source_address,
            ) as rh:
                response = ctx.socks_info_request(rh)
                assert response["client_address"][0] == source_address
    ...
    def test_socks5_errors(self, handler, ctx, reply_code):
        with ctx.socks_server(
            Socks5ProxyHandler, reply=reply_code
        ) as server_address:
            with handler(proxies={"all": f"socks5://{server_address}"}) as rh:
                with pytest.raises(ProxyError):
                    ctx.socks_info_request(rh)
    ...
    def test_timeout(self, handler, ctx):
        with ctx.socks_server(Socks5ProxyHandler, sleep=2) as server_address:
            with handler(
                proxies={"all": f"socks5://{server_address}"}, timeout=1
            ) as rh:
                with pytest.raises(TransportError):
                    ctx.socks_info_request(rh)
