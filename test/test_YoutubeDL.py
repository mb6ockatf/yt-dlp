...
class TestFormatSelection(unittest.TestCase):
    ...
    def test_format_selection_string_ops(self):
        ...
        self.assertRaises(
            ExtractorError, ydl.process_ie_result, info_dict.copy()
        )
    ...
    def test_format_selection_issue_10083(self):
        ...
        ydl = YDL(
            {"format": "best[height>360]/bestvideo[height>360]+bestaudio"}
        )
        ydl.process_ie_result(info_dict.copy())
        self.assertEqual(
            ydl.downloaded_info_dicts[0]["format_id"], "video+audio"
        )
    ...
    def test_default_format_spec_without_ffmpeg(self):
        ydl = YDL({})
        self.assertEqual(
            ydl._default_format_spec({}), "best/bestvideo+bestaudio"
        )
        ...
        ydl = YDL({})
        self.assertEqual(
            ydl._default_format_spec({"is_live": True}),
    ...
    @patch(
        "yt_dlp.postprocessor.ffmpeg.FFmpegMergerPP.can_merge", lambda _: True
    )
    def test_default_format_spec_with_ffmpeg(self):
        ydl = YDL({})
        self.assertEqual(
            ydl._default_format_spec({}), "bestvideo*+bestaudio/best"
        )
...
class TestYoutubeDL(unittest.TestCase):
    def test_subtitles(self):
        ...
        result = get_info(
            {"writesubtitles": True, "subtitlesformat": "foo/srt"}
        )
        subs = result["requested_subtitles"]
        ...
        result = get_info(
            {"writesubtitles": True, "subtitleslangs": ["all", "-en"]}
        )
        subs = result["requested_subtitles"]
        self.assertTrue(subs)
        self.assertEqual(set(subs.keys()), {"es", "fr"})
        ...
        result = get_info(
            {"writesubtitles": True, "subtitleslangs": ["-en", "en"]}
        )
        subs = result["requested_subtitles"]
        self.assertTrue(subs)
        self.assertEqual(set(subs.keys()), {"en"})
        ...
        result = get_info(
            {"writeautomaticsub": True, "subtitleslangs": ["es", "pt"]}
        )
        subs = result["requested_subtitles"]
        self.assertTrue(subs)
        self.assertEqual(set(subs.keys()), {"es", "pt"})
    ...
    def test_prepare_outtmpl_and_filename(self):
        ...
        test(
            "%(title5)a %(height)a", (R"'\xe1\xe9\xed \U0001d400' 1080", None)
        )
        ...
        test(
            "%(formats.:.{id,height}.id)l", ", ".join(f["id"] for f in FORMATS)
        )
        ...
        def gen():
            yield from range(5)
            raise self.assertTrue(
                False, "LazyList should not be evaluated till here"
            )
    ...
    def test_postprocessors(self):
        ...
        self.assertTrue(
            os.path.exists(audiofile), f"{audiofile} doesn't exist"
        )
        os.unlink(filename)
        os.unlink(audiofile)
    ...
    def test_playlist_items_selection(self):
        ...
        def test_selection(params, expected_ids, evaluate_all=False):
            ...
            if evaluate_all:
                ...
                pagedlist_eval = INDICES[
                    PAGE_SIZE * page_num(min(expected_ids))
                    - PAGE_SIZE : PAGE_SIZE * page_num(max(expected_ids))
                ]
            ...
            for name, func, expected_eval in (
                ...
                results = [
                    (
                        v["playlist_autonumber"] - 1,
                        (int(v["id"]), v["playlist_index"]),
                    )
        ...
        test_selection(
            {"playliststart": 2, "playlistreverse": True}, INDICES[:0:-1]
        )
        test_selection(
            {"playlist_items": "2,4", "playlistreverse": True}, [4, 2]
        )
        test_selection({"playlist_items": "4,2"}, [4, 2])
    ...
    def test_ignoreerrors_for_playlist_with_url_transparent_iterable_entries(
        self,
    ):
    ...
    def test_header_cookies(self):
        ...
        test(
            "test=value; Domain=.yt.dlp",
            [cookie("test", "value", domain=".yt.dlp")],
        )
        test(
            "test=value",
            [cookie("test", "value")],
    ...
    def test_infojson_cookies(self):
        ...
        def make_info(
            info_header_cookies=False,
            fmts_header_cookies=False,
            cookies_field=False,
        ):
            fmt = {"url": TEST_URL}
            ...
            return _make_result(
                [fmt],
                http_headers=COOKIE_HEADER if info_header_cookies else None,
            )
        ...
        test(
            make_info(
                info_header_cookies=True,
                fmts_header_cookies=True,
                cookies_field=True,
            ),
            "all cookies fields",
        )
    ...
    def test_add_headers_cookie(self):
        ...
        self.assertIsNone(
            check_for_cookie_header(result),
            msg="http_headers cookies for wrong domain",
        )
        self.assertFalse(
            result.get("cookies"),
            msg="Cookies set in cookies field for wrong domain",
        )
        self.assertFalse(
            ydl.cookiejar.get_cookie_header(fmt["url"]),
