...
def main():
    ...
    DummyInfoExtractor = type(
        "InfoExtractor", (InfoExtractor,), {"IE_NAME": NO_ATTR}
    )
    module_src = "\n".join(
        (
            MODULE_TEMPLATE,
...
def build_ies(ies, bases, attr_base):
    ...
    yield "\n_CLASS_LOOKUP = {%s}" % ", ".join(
        f"{name!r}: {name}" for name in names
    )
        ...
        for c in classes[:]:
            ...
            for b in sorted(bases, key=lambda x: x.__name__):
                if b not in classes and b not in returned_classes:
                    assert (
                        b.__name__ != "GenericIE"
                    ), "Cannot inherit from GenericIE"
                    classes.insert(0, b)
                    restart = True
            if restart:
                break
