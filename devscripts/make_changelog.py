...
class CommitGroup(enum.Enum):
    ...
    def get(cls, value: str) -> tuple[CommitGroup | None, str | None]:
        group, _, subgroup = (
            group.strip().lower() for group in value.partition("/")
        )
...
def unique(items):
    return sorted(
        {item.strip().lower(): item for item in items if item}.values()
    )


class Changelog:
    MISC_RE = re.compile(
        r"(?:^|\b)(?:lint(?:ing)?|misc|format(?:ting)?|fixes)(?:\b|$)",
        re.IGNORECASE,
    )
    ...
    def _format_group(self, group):
        ...
        detail_groups = itertools.groupby(
            sorted_group, lambda item: (item.details or "").lower()
        )
        for _, items in detail_groups:
            ...
            sub_detail_groups = itertools.groupby(
                items, lambda item: tuple(map(str.lower, item.sub_details))
            )
            ...
            sorted_items.append(
                CommitInfo(
                    ...
                    ", ".join(
                        ...
                        for info in sorted(
                            commit_infos,
                            key=lambda item: item.commit.hash or "",
                        )
                    ),
                    [],
    ...
    def format_single_change(self, info: CommitInfo):
        ...
        if info.commit.authors:
            message = (
                f"{message} by {self._format_authors(info.commit.authors)}"
            )

        if info.fixes:
            fix_message = ", ".join(
                f"{self._format_message_link(None, fix.hash)}"
                for fix in info.fixes
            )

            authors = sorted(
                {author for fix in info.fixes for author in fix.authors},
                key=str.casefold,
            )
            if authors != info.commit.authors:
                fix_message = (
                    f"{fix_message} by {self._format_authors(authors)}"
                )
    ...
    def _format_issues(self, issues):
        return ", ".join(
            f"[#{issue}]({self.repo_url}/issues/{issue})" for issue in issues
        )

    @staticmethod
    def _format_authors(authors):
        return ", ".join(
            f"[{author}]({BASE_URL}/{author})" for author in authors
        )
...
class CommitRange:
    ...
    EXTRACTOR_INDICATOR_RE = re.compile(
        r"(?:Fix|Add)\s+Extractors?", re.IGNORECASE
    )
    REVERT_RE = re.compile(r"(?:\[[^\]]+\]\s+)?(?i:Revert)\s+([\da-f]{40})")
    FIXES_RE = re.compile(
        r"(?i:(?:bug\s*)?fix(?:es)?(?:\s+bugs?)?(?:\s+in|\s+for)?|Improve)\s+([\da-f]{40})"
    )
    UPSTREAM_MERGE_RE = re.compile(r"Update to ytdl-commit-([\da-f]+)")

    def __init__(self, start, end, default_author=None):
        self._start, self._end = start, end
        self._commits, self._fixes = self._get_commits_and_fixes(
            default_author
        )
        self._commits_added = []
    ...
    def __iter__(self):
        return iter(
            itertools.chain(self._commits.values(), self._commits_added)
        )
    ...
    def _get_commits_and_fixes(self, default_author):
        ...
        for i, commit_hash in enumerate(lines):
            ...
            for line in iter(lambda: next(lines), self.COMMIT_SEPARATOR):
                if match := self.AUTHOR_INDICATOR_RE.match(line):
                    authors = sorted(
                        map(str.strip, line[match.end() :].split(",")),
                        key=str.casefold,
                    )
                if not fix_commitish and (
        ...
        for commitish, fix_commits in fixes.items():
            if commitish in commits:
                hashes = ", ".join(
                    commit.hash[:HASH_LENGTH] for commit in fix_commits
                )
                logger.info(
                    f"Found fix(es) for {commitish[:HASH_LENGTH]}: {hashes}"
                )
                for fix_commit in fix_commits:
                    del commits[fix_commit.hash]
            else:
                logger.debug(
                    f"Commit with fixes not in changes: {commitish[:HASH_LENGTH]}"
                )
    ...
    def groups(self):
        ...
        for commit in self:
            ...
            if upstream_re:
                commit.short = (
                    f"[upstream] Merged with youtube-dl {upstream_re.group(1)}"
                )

            match = self.MESSAGE_RE.fullmatch(commit.short)
            if not match:
                logger.error(
                    f"Error parsing short commit message: {commit.short!r}"
                )
                continue

            prefix, sub_details_alt, message, issues = match.groups()
            issues = (
                [issue.strip()[1:] for issue in issues.split(",")]
                if issues
                else []
            )

            if prefix:
                groups, details, sub_details = zip(
                    *map(self.details_from_prefix, prefix.split(","))
                )
...
def create_changelog(args):
    ...
    if not args.no_override:
        if args.override_path.exists():
            ...
            logger.warning(
                f"File {args.override_path.as_posix()} does not exist"
            )
