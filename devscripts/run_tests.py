...
def parse_args():
    ...
    parser.add_argument(
        "test",
        help='an extractor test, test path, or one of "core" or "download"',
        nargs="*",
    )
    parser.add_argument(
        "-k",
        help='run a test matching EXPRESSION. Same as "pytest -k"',
        metavar="EXPRESSION",
    )
    parser.add_argument(
...
def run_tests(*tests, pattern=None, ci=False):
    ...
    run_core = (
        "core" in tests or "tests" in tests or (not pattern and not tests)
    )
    ...
    if run_core:
        print(
            '"pytest" needs to be installed to run core tests',
            file=sys.stderr,
            flush=True,
        )
        return 1
...
if __name__ == "__main__":
    try:
        ...
        sys.exit(
            run_tests(*args.test, pattern=args.k, ci=bool(os.getenv("CI")))
        )
